...
const repo = process.env.GH_REPO ?? process.env.GITHUB_REPOSITORY ?? "Neopilot-Ai/neocode"
...
if (values.help) {
  console.log(`
Usage: bun .neo/skills/release-jetbrains/script/check-pin.ts

Checks the CLI pin that a JetBrains release would lock. Prepare tags origin/main,
so this reads packages/neo-jetbrains/package.json from origin/main and compares
it with the latest published Neo CLI release plus the local worktree pin.
...
const pinMain = JSON.parse(await $`git show origin/main:packages/neo-jetbrains/package.json`.text()).version as string
const pinLocal = (await Bun.file("packages/neo-jetbrains/package.json").json()).version as string
const propsMain = await $`git show origin/main:packages/neo-jetbrains/gradle.properties`.text()
const propsLocal = await Bun.file("packages/neo-jetbrains/gradle.properties").text()
...
async function previous() {
  ...
  if (!tag) return null
  const res = await $`git show ${tag}:packages/neo-jetbrains/package.json`.nothrow().text()
...
function pinned(text: string) {
  const value = text.split(/\r?\n/).flatMap((line) => {
    ...
    if (key.trim() !== "neo.cli.pinned") return []
