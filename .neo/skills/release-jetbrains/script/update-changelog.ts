...
const path = "packages/neo-jetbrains/CHANGELOG.md"
...
if (values.help) {
  console.log(
    `Usage: bun .neo/skills/release-jetbrains/script/update-changelog.ts --version <version> --file <section.md>`,
  )
  process.exit(0)
}
...
async function temp(body: object) {
  const file = `/tmp/neo-jetbrains-changelog-${Date.now()}.json`
