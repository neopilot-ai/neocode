...
- CLI already exposes everything needed; no CLI release or SDK regen required. Pinned CLI version is `7.4.1` (`packages/neo-jetbrains/package.json`).
...
- JetBrains split-mode packages differ from AGENTS.md's stated `ai.neocode.jetbrains`; actual source roots are:
  - shared → `packages/neo-jetbrains/shared/src/main/kotlin/ai/neocode/rpc/`
  - backend → `packages/neo-jetbrains/backend/src/main/kotlin/ai/neocode/backend/`
  - frontend → `packages/neo-jetbrains/frontend/src/main/kotlin/ai/neocode/client/`
- All target files confirmed to exist: `SessionDto.kt`, `NeoSessionRpcApi.kt`, `NeoBackendChatManager.kt`, `NeoSessionRpcApiImpl.kt`, `NeoSessionService.kt`, `SessionController.kt`, `SessionModel.kt`, `SessionModelEvent.kt`, `SessionView.kt`, `MessageView.kt`, `MessageToolbar.kt`, `PromptPanel.kt`, `PromptEditorTextField.kt`, plus reuse targets `DiffStatBadge.kt`, `HoverIcon.kt`, `PermissionDiffView.kt`.
...
File: `packages/neo-vscode/webview-ui/src/i18n/en.ts:138`
...
- In the 19 non-English locale files under `packages/neo-vscode/webview-ui/src/i18n/` (ar, br, bs, da, de, es, fr, it, ja, ko, nl, no, pl, ru, th, tr, uk, zh, zht), overwrite `revert.banner.hint` with the identical new English string; leave a note that these need re-translation.
...
- `shared/.../rpc/NeoSessionRpcApi.kt`: add
...
- `backend/.../app/NeoBackendChatManager.kt`: add `revert(id, dir, messageID, partID?)` → `POST /session/$id/revert?directory=…` with `{messageID, partID?}` body, and `unrevert(id, dir)` → `POST /session/$id/unrevert?directory=…`. Mirror the existing `compact()` OkHttp pattern; add JSON body builders to `NeoCliDataParser`.
- `backend/.../rpc/NeoSessionRpcApiImpl.kt`: implement both, resolve worktree dir via `sessions.getDirectory(id, directory)`, wrap in `ready { }` like `compact`/`abort`.

### 3. Frontend service

- `frontend/.../app/NeoSessionService.kt`: add `suspend revert(...)` / `unrevert(...)` wrappers over the RPC, using `durable {}` per split-mode rules.
...
- New `session/ui/RevertBanner.kt` Swing view, built once and mutated via `update()` per retained-Swing rules. Listens to `RevertChanged` + `DiffUpdated`. Contents:
  ...
  - per-file diff rows reusing `ai.neocode.client.ui.DiffStatBadge` fed from `model.diff` (`DiffFileDto.additions/deletions`),
...
### 7. i18n (`frontend/src/main/resources/messages/NeoBundle*.properties`)

- Add to base `NeoBundle.properties`: `revert.banner.count.one`, `revert.banner.count.other`, `revert.banner.redo`, `revert.banner.redo.all`, `revert.banner.hint` (new wording), `revert.disabled.agentBusy`.
- Other locale bundles fall back to base until translated; do not add stale translations.

### 8. Tests

- Backend: extend `NeoSessionRpcApiImplTest` / a `NeoBackendChatManager` test against `MockCliServer` asserting the exact HTTP path + JSON body for `revert`/`unrevert`, and that a session carrying `revert` deserializes into `SessionRevertDto`.
...
| Shared | `SessionDto.kt` (+`SessionRevertDto`), `NeoSessionRpcApi.kt` |
| Backend | `NeoBackendChatManager.kt`, `NeoCliDataParser`, `NeoSessionRpcApiImpl.kt` |
| Frontend service/controller/model | `NeoSessionService.kt`, `SessionController.kt`, `SessionModel.kt`, `SessionModelEvent.kt` |
| Frontend views | new `RevertBanner.kt` (reusing `DiffStatBadge`), `MessageView.kt`, `MessageToolbar.kt`, `SessionView.kt`, `PromptPanel.kt` |
| i18n | `NeoBundle.properties` |
| Tests | backend RPC/chat-manager, frontend controller + view tests, `FakeSessionRpcApi` |

## Validation

- VS Code string change: `bun run typecheck` from repo root (or from `packages/neo-vscode/`).
- JetBrains: `./gradlew typecheck` and `./gradlew test` from `packages/neo-jetbrains/` (Java 21).
- Manual: `./gradlew runIde` (or split mode) to exercise rollback → redo → redo-all and reverted-transcript hiding.

## Risks / Notes

- Confirm no `neocode_change` markers are needed for the VS Code i18n edits (Neo-owned webview path — none required).
