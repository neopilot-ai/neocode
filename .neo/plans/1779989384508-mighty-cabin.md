...
5. Verification.
   - Run the targeted JetBrains frontend session scroll tests from `packages/neo-jetbrains/` using Gradle test filtering for `SessionScrollTest` if supported.
   - Run `./gradlew typecheck` from `packages/neo-jetbrains/` if the targeted tests pass or if Gradle filtering is unavailable.
...
- `packages/neo-jetbrains/frontend/src/test/kotlin/ai/neocode/client/session/SessionScrollTest.kt`
