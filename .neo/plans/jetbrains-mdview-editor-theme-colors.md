...
- From `packages/neo-jetbrains/`, run `./gradlew frontend:test --tests '*MdView*'`.
- From `packages/neo-jetbrains/`, run `./gradlew frontend:test --tests '*PlanExitViewTest*'` if the focused test is not covered by the MdView filter.
- From `packages/neo-jetbrains/`, run `bun run typecheck`.
