...
- Use Neo/VS Code-aligned session icons in session views.
...
## Findings
- Session view icons are centralized in `packages/neo-jetbrains/frontend/src/main/kotlin/ai/neocode/client/session/views/SessionViewIcons.kt` and loaded from `frontend/src/main/resources/icons/views/*.svg`.
...
1. **Keep icon sources aligned with VS Code/shared UI**
   - Treat `packages/ui/src/components/icon.tsx` as the source for Neo web/session glyph shapes.
   - Re-check `SessionViewIcons.kt` entries against available JetBrains assets; only update or add SVGs if a session view uses a Neo icon missing from `frontend/src/main/resources/icons/views/`.
...
3. **Normalize collapse/expand chevrons for session parts**
   ...
   - Use a single base Neo chevron glyph for both states, matching the current custom chevron used by the session header (`/icons/chevron-down.svg` / equivalent `SessionViewIcons.chevronDown`).
...
6. **Verification**
   - Run the smallest relevant JetBrains checks from `packages/neo-jetbrains/`:
     - `./gradlew test --tests "ai.neocode.client.session.views.ReasoningViewTest" --tests "ai.neocode.client.session.views.base.AbstractSessionPartViewTest" --tests "ai.neocode.client.session.views.QuestionResultViewTest" --tests "ai.neocode.client.session.ui.header.SessionHeaderPanelTest"`
     - `./gradlew typecheck`
   - If the filtered Gradle test syntax is not accepted by the project, run `./gradlew test` from `packages/neo-jetbrains/` instead.

## Notes
- No shared upstream `opencode` files are involved; changes stay under `packages/neo-jetbrains/`.
