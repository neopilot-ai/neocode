...
## Affected Areas
- `packages/neo-jetbrains/shared/src/main/kotlin/ai/neocode/rpc/dto/ProviderDto.kt`
- `packages/neo-jetbrains/backend/src/main/kotlin/ai/neocode/backend/workspace/NeoWorkspaceState.kt`
- `packages/neo-jetbrains/backend/src/main/kotlin/ai/neocode/backend/cli/NeoCliDataParser.kt`
- `packages/neo-jetbrains/backend/src/main/kotlin/ai/neocode/backend/rpc/NeoWorkspaceDtoMapper.kt`
- `packages/neo-jetbrains/frontend/src/main/kotlin/ai/neocode/client/session/model/SessionModel.kt`
- `packages/neo-jetbrains/frontend/src/main/kotlin/ai/neocode/client/session/SessionUi.kt`
- `packages/neo-jetbrains/frontend/src/main/kotlin/ai/neocode/client/settings/models/ModelsSettingsUi.kt`
- `packages/neo-jetbrains/frontend/src/main/kotlin/ai/neocode/client/session/ui/model/ModelPicker*.kt`
- `packages/neo-jetbrains/frontend/src/main/resources/messages/NeoBundle*.properties`
- Existing tests under `packages/neo-jetbrains/frontend/src/test/...` and `backend/src/test/...`
...
2. Parse and map the extra metadata in the JetBrains backend.
   - Extend `ModelInfo` in `NeoWorkspaceState.kt` with matching nullable metadata fields.
   - Update `NeoCliDataParser.parseModel` and `parseModelDto` to read the fields currently used by VS Code: `inputPrice`, `outputPrice`, `contextLength`, `releaseDate`, `latest`, `cost`, `capabilities.input`, `options.description`, `autoRouting.models`, and `terminalBench`.
   ...
   - Update `NeoWorkspaceDtoMapper.model` to copy all new fields to `ModelDto`.
...
5. Rework popup layout and size behavior.
   ...
   - Persist state under a key such as `neo.model.picker.expanded` using `PropertiesComponent.getInstance().getBoolean(key, false)` and `setValue(key, value.toString())`.
...
7. Add localization strings.
   - Add base `NeoBundle.properties` entries for maximize/minimize tooltips, detail labels, pricing labels, context, capabilities, terminal-bench, description, auto-routing choices, and unavailable values.
...
## Validation
- From `packages/neo-jetbrains/`: run `./gradlew typecheck` or `bun run typecheck`.
- From `packages/neo-jetbrains/`: run targeted tests for backend parser/serialization and frontend model picker tests, or `./gradlew test` if targeted Gradle filters are not straightforward.
