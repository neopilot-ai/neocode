...
All work is inside `packages/neo-jetbrains/`. No shared `packages/opencode/` files change,
so no `neocode_change` markers are needed.
...
- Test harnesses already exist: frontend `SessionControllerTestBase` + `FakeSessionRpcApi`
  ...
  `NeoBackendChatManagerTest`. Banner tests instantiate `RevertBanner(model, redo, redoAll)`
...
- `frontend/src/main/resources/messages/NeoBundle.properties` (+ translated `NeoBundle_*.properties`)
- `backend/.../app/NeoBackendChatManager.kt` (`revert`/`unrevert`), `backend/.../cli/NeoCliDataParser.kt` (`buildRevertJson`)
- `backend/.../app/NeoBackendSessionManager.kt` (`revertDto` duplication)
- Tests: `frontend/.../session/controller/TurnLifecycleTest.kt` (or new `RevertFlowTest.kt`),
  ...
  `backend/.../testing/MockCliServer.kt`, `backend/.../app/NeoBackendChatManagerTest.kt`,
  `backend/.../cli/NeoCliDataParserTest.kt`
...
1. Add bundle key in `NeoBundle.properties` (place near the other `revert.banner.*` keys):
   - `revert.banner.filesNotRestored=Snapshots are off — only the conversation was reverted; your files were not changed.`
   - Add the same key to every translated `NeoBundle_*.properties` file. If translations are
...
2. In `RevertBanner.kt`:
   ...
   - In `update()`, after computing `revert`, set
     ...
     `NeoBundle.message("revert.banner.filesNotRestored")`.
...
4. In `NeoBackendSessionManager.kt`, collapse the two identical `revertDto(...)` overloads
   ...
   a pure refactor covered by existing `NeoBackendSessionManagerTest` parsing assertions.
...
9. `MockCliServer.kt` — add routing + capture fields (the current generic `/session/ses_*`
   ...
   - Cases (place beside the `summarize`/`prompt_async` cases):
     ...
     - `bare.matches(Regex("/session/ses_[^/]+/unrevert")) && method == "POST"` → store path+body,
       `respond(output, 200, sessionCreate)`.
10. `NeoBackendChatManagerTest.kt` — mirror the existing `compact posts summarize request` test:
...
11. `NeoCliDataParserTest.kt` — add `buildRevertJson` unit tests:
...
12. Remove the unused bundle key `revert.disabled.agentBusy` from `NeoBundle.properties` and all
...
- Targeted while iterating: run the touched test classes (`TurnLifecycleTest`/`RevertFlowTest`,
  `SessionMessageListPanelTest`, `SessionModelTest`, `NeoBackendChatManagerTest`,
  `NeoCliDataParserTest`).
...
Add `.changeset/<slug>.md` (`"@neocode/neo-jetbrains": patch`) describing the user-facing
