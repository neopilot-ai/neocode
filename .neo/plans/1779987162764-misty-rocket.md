...
The JetBrains plugin currently imports `com.intellij.util.SVGLoader` in `packages/neo-jetbrains/frontend/src/main/kotlin/ai/neocode/client/ui/SvgIconColorizer.kt` and calls `SVGLoader.load(ByteArrayInputStream(patch()), scale.toFloat())` from `SvgIcon.image(Graphics)`.
...
1. Add Neo-owned SVG rendering code under the frontend UI package, for example:
   - `packages/neo-jetbrains/frontend/src/main/kotlin/ai/neocode/client/ui/SvgLoader.kt`
...
3. If jsvg does not expose intrinsic size cleanly from public API, adjust the API to avoid duplicating SVG parsing:
   - Add `width` and `height` parameters to the Neo loader.
...
4. Add the jsvg dependency explicitly to the plugin frontend module instead of relying on the IntelliJ-bundled library:
   - In `packages/neo-jetbrains/gradle/libs.versions.toml`, add `jsvg = "2.1.0"` and `jsvg = { module = "com.github.weisj:jsvg", version.ref = "jsvg" }`.
   - In `packages/neo-jetbrains/frontend/build.gradle.kts`, add `implementation(libs.jsvg)`.
...
5. Update `SvgIconColorizer.kt`:
   ...
   - Replace `SVGLoader.load(ByteArrayInputStream(patch()), scale.toFloat())` with the Neo-owned loader call.
   - Keep the existing color patching, cache key, scale calculation, and size parsing intact.

6. Add a focused frontend unit test:
   - New test file: `frontend/src/test/kotlin/ai/neocode/client/ui/SvgIconColorizerTest.kt` or `SvgLoaderTest.kt`.
...
7. Run verification from `packages/neo-jetbrains/`:
   - `./gradlew typecheck`
...
- This change is in `packages/neo-jetbrains/`, a Neo-owned package, so `neocode_change` markers are not needed.
