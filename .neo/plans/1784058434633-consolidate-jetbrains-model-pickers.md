...
All work is under `packages/neo-jetbrains/frontend/`. No backend/RPC/DTO changes. Files are
Neo-owned (no `neocode_change` markers).
...
- **Location**: New package `ai/neocode/client/ui/picker/` with `PickerPopup<T>` and
...
9. **Strings**: add a "Deselect All" key to `NeoBundle.properties` (e.g.
...
- `./gradlew typecheck`
- `./gradlew test` (iterate with `--tests ai.neocode.client.session.ui.model.ModelPickerTest`
  and `--tests ai.neocode.client.settings.providers.ProvidersSettingsUiTest`).
