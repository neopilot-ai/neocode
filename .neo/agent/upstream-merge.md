...
**Do not load the `neocode-merge-minimizer` skill.** That skill is for
authoring new Neo changes against shared upstream files; during an upstream
merge it gives the wrong guidance (it nudges toward extracting Neo logic out
...
- does a `neocode_change` marker in HEAD encode a bug fix, a feature, or a
  defensive check?
...
When HEAD includes a non-obvious Neo-specific wrapper (e.g. a helper in
`packages/opencode/src/neocode/`), find out why it exists before deciding to
keep or bypass it:

```bash
git log --all --oneline -S "<symbol>" -- packages/opencode/src/neocode/
git log --all --oneline -- <neo-file>
```
...
- one-sentence rationale (what Neo behaviour is preserved, what upstream
...
3. State which Neo behaviour must survive and why (reference PR numbers /
   `neocode_change` comments when possible).
...
- `.worktrees/opencode-merge/neo-main` — Neo base snapshot
- `.worktrees/opencode-merge/auto-merge` — automated merge snapshot (original
  conflict reference)

Apply in order:

- prefer upstream code and architecture whenever compatible with Neo behaviour
- preserve Neo-specific behaviour marked with `neocode_change`
- keep `neocode_change` markers around Neo-specific code in shared opencode
  files
- when upstream refactors a region that HEAD had annotated with
  `neocode_change`, **check whether the marker encodes a bug fix or a feature
  ...
  `Effect.gen` block.
- when a `take-theirs` drops a line that was the target of a Neo pre-filter,
  the upstream line may be actively wrong for Neo — e.g. an inner `continue`
  filter whose condition collides with an outer filter Neo added. Re-read the
  surrounding 20 lines before committing to `take-theirs`.
- if Neo-specific code must be refactored to fit new upstream architecture,
  explain the refactor in the final summary
- if upstream moved the relevant logic to another file, port the Neo behaviour
  there and list both paths in the final summary. Verify the new file already
  carries the Neo-renamed symbols (e.g. `x-neo-directory`) by diffing against
  pristine upstream.
- if upstream extracts shared policy into a helper, move Neo-specific additions
...
- if upstream deleted a file, analyse whether the Neo behaviour should be
...
**commenting it out with `neocode_change` markers** over deletion when the
surrounding structure (an `if`, a loop) still makes sense. That keeps the
intent visible to the next merger. Example:

```ts
} else if (input?.scope !== "project" && !Flag.NEO_EXPERIMENTAL_WORKSPACES) {
...
shows up in `git diff --diff-filter=U`; the new sibling may need a Neo feature
...
- `neocode_change` comments in the *auto-merge* snapshot that didn't end up in
...
- **Orphaned imports and references.** A rename upstream may leave a Neo
...
- **Partial auto-merges.** Upstream may have refactored a region Neo
  ...
  path, the file compiles upstream but breaks on Neo.
...
- other CI guards that touched files imply (knip for `neo-vscode/`,
  `check-neocode-change`, source-links, visual regression,
  `script/check-forbidden-strings.ts`)
- if you encounter a hardcoded upstream URL, repo path, or attribution string
  during conflict resolution that obviously shouldn't ship in Neo (e.g. another
...
shared code, but the change cascades into Neo-only files (neocode tests,
neo-specific source, plugins) that still reference the old symbol. Those
...
### 9.6. Handle upstream-added tests that diverge from Neo

Upstream sometimes adds tests that encode design contracts Neo intentionally
breaks. These auto-merge cleanly and then fail. Three resolution patterns:

- **Rewrite the test** when the test is a contract assertion and Neo has a
  different but equally valid contract. Invert or adjust the assertion with a
  `neocode_change` marker explaining the divergence.
- **Skip the test** when the test relies on patterns that Neo has replaced
  ...
  Neo's extensions break). Mark with `neocode_change` and a rationale
  explaining what would need to change for the test to run.
- **Delete the test** when it covers functionality Neo deliberately removed
  (fallback paths, deprecated endpoints, products Neo doesn't ship). Note
...
`packages/extensions/zed/extension.toml` (version field + 5 Neopilot-Ai download
...
(e.g. `1.14.30`), whose release tag does not exist on Neo's pipeline, so the
...
(excluding `node_modules`, hidden dirs, and `packages/neo-jetbrains/` which
tracks its own cadence), plus the Zed extension toml. It is idempotent — rerun
it any time to rebase the version back onto Neo main (useful during
...
- **Non-trivial merge decisions**: a short section per file (or group of
  ...
  Focus on *what Neo behaviour survived* and *what upstream features were
  adopted*. Link to Neo PRs when a `neocode_change` encodes a specific fix.
...
- Upstream can reintroduce bugs a Neo `neocode_change` had already fixed —
  during big refactors check every `neocode_change` the refactor touched.
- "Take-theirs" on an inner conditional is often wrong when Neo added an outer
...
- Stricter DOM lib types (upstream TS upgrade) can surface latent casting
  issues around `WebSocket.send`, `Headers`, etc. — prefer narrowing the Neo
...
- Neo code may rely on ambient context (async-local storage, globally-set
  flags, process env) being populated at a lifecycle moment that upstream
  refactors away. If Neo behaviour reads ambient state during init, forked
...
- Tests that intercept via process-global or module-global spies can become
  ...
  the spy.
- When Neo extends a shared data shape with extra optional fields, different
  ...
  every Neo addition — audit the encoding assumption before adding fields.
...
- Upstream-added tests can encode a design contract Neo deliberately breaks.
  The test passes upstream because upstream doesn't share Neo's requirement.
  Decide between refactoring Neo to match the upstream contract or rewriting
  the test to assert Neo's divergent contract — with a `neocode_change`
