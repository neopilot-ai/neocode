...
- Change the constructor to accept what it needs to save and report progress:
  ...
  Pass `save = { service<NeoProviderService>().saveCustom(it) }` from `custom()` so
  the existing workspace-reload + profile-refresh side effects in
  `NeoProviderService.action` still run.
...
- Extend `doValidate()` to require at least one model:
  ...
  if (models.text.split(',').none { it.isNotBlank() })
      return ValidationInfo(NeoBundle.message("settings.providers.customModelsRequired"), models)
  ```
...
if (!present) return NeoBundle.message("settings.providers.customNotUsable")
return null
```
...
### 2. `backend/.../provider/NeoBackendProviderSettingsManager.kt`
...
### 3. `frontend/.../resources/messages/NeoBundle.properties`
...
### Backend — `backend/.../provider/NeoBackendProviderSettingsManagerTest.kt`
...
- Wiring the already-implemented `fetchCustomModels` RPC
  (`NeoProviderService.fetchCustomModels`) into the dialog as a "Fetch models"
