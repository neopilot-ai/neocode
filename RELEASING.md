...
Neo Code uses a fully automated CI pipeline triggered via GitHub Actions `workflow_dispatch`. A single workflow handles version bumping, building all artifacts, publishing to every distribution channel, and updating package registries.

## How to Trigger a Release

1. Go to the [`publish` workflow](https://github.com/Neopilot-Ai/neocode/actions/workflows/publish.yml) in GitHub Actions.
...
- Runs `packages/opencode/script/build.ts` to compile the Neo CLI binary.
...
- Uploads the `dist/` directory as a workflow artifact (`neo-cli`) for subsequent jobs.

### 3. Build VS Code Extension (`build-vscode`)

- Downloads the CLI artifacts from the previous job.
- Runs `packages/neo-vscode/script/build.ts` to build VSIX packages for all target platforms:
...
- Uploads the VSIX files as a workflow artifact (`neo-vscode`).
...
- Publishes platform-specific binary packages to **npm** (e.g. `@neocode/cli-linux-x64`, `@neocode/cli-darwin-arm64`, etc.).
- Publishes the main `@neocode/cli` package to **npm** with optional dependencies on the binary packages.
- Builds and pushes a multi-arch **Docker image** (`ghcr.io/neopilot-ai/neocode`) to GitHub Container Registry (linux/amd64 + linux/arm64).

#### SDK (`@neocode/sdk`)

- Builds and publishes the TypeScript SDK to **npm**.

#### Plugin (`@neocode/plugin`)
...
- **AUR (Arch Linux)**: Clones `neo-bin` from the AUR, updates the `PKGBUILD` with new version and SHA256 checksums, and pushes.
- **Homebrew**: Clones `Neopilot-Ai/homebrew-tap`, updates the `neo.rb` formula with new version, download URLs, and SHA256 checksums, and pushes.

## Prerequisites and Permissions

### Repository Access

- The workflow only runs in the `Neopilot-Ai/neocode` repository (guarded by `if: github.repository == 'Neopilot-Ai/neocode'`).
...
| Secret | Purpose |
|---|---|
| `NEO_API_KEY` | Neo API key used during version computation |
| `NEO_ORG_ID` | Neo organization ID |
| `NEO_MAINTAINER_APP_ID` | GitHub App ID for the neo-maintainer bot (used for git commits) |
| `NEO_MAINTAINER_APP_SECRET` | GitHub App secret for the neo-maintainer bot |
