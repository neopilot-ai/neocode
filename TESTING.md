...
How to spin up the **local main-branch** Neo backend and test it with `curl` / `fetch`. Aimed at a running Neo CLI agent iterating on backend fixes without rebuilding the VS Code extension or TUI.
...
NEO_SERVER_PASSWORD="$PASS" bun dev serve --port 0 >/tmp/neo-serve.log 2>&1 &
echo $! >/tmp/neo-serve.pid
while ! grep -q "neo server listening" /tmp/neo-serve.log 2>/dev/null; do sleep 0.1; done
PORT=$(grep -oE "listening on http://[^:]+:[0-9]+" /tmp/neo-serve.log | grep -oE "[0-9]+$")
AUTH="Authorization: Basic $(printf 'neo:%s' "$PASS" | base64 | tr -d '\n')"
BASE="http://127.0.0.1:$PORT"

# Call any endpoint
curl -sS -H "$AUTH" -H "x-neo-directory: $PWD" "$BASE/global/health"

# Stop when done
kill "$(cat /tmp/neo-serve.pid)" 2>/dev/null || true
rm -f /tmp/neo-serve.pid /tmp/neo-serve.log
```

## 1. What this doc is for

Testing local backend fixes against a real running server, talking to it over HTTP the same way the VS Code extension, TUI, and `neo run --attach` do — but without any of those clients. Every request is a `curl` the agent can copy-paste.

For in-process tests (no socket, fastest loop) see `packages/opencode/test/neocode/server/permission-allow-everything.test.ts` for the `Server.Default().app.request(...)` pattern. That's the right tool inside the `packages/opencode/` test suite; this doc is for out-of-process HTTP testing.

## 2. `neo serve` vs `bun dev serve` — important

| Command | What it runs |
|---|---|
| `neo serve` | The npm-installed production CLI on `$PATH`. **Not the code in this repo.** |
...
Do **not** use `createNeoServer()` from `@neocode/sdk/v2` to test local code: it spawns the PATH `neo` binary (`packages/sdk/js/src/v2/server.ts:38-136`), which is the wrong tool here.
...
```bash
NEO_SERVER_PASSWORD=$(openssl rand -hex 16) \
  bun dev serve --port 0 >/tmp/neo-serve.log 2>&1 &
echo $! >/tmp/neo-serve.pid
while ! grep -q "neo server listening" /tmp/neo-serve.log; do sleep 0.1; done
PORT=$(grep -oE "listening on http://[^:]+:[0-9]+" /tmp/neo-serve.log | grep -oE "[0-9]+$")
```
...
```bash
NEO_SERVER_PASSWORD=secret \
  bun dev serve --port 4096 --hostname 127.0.0.1 \
  >/tmp/neo-serve.log 2>&1 &
echo $! >/tmp/neo-serve.pid
while ! grep -q "neo server listening" /tmp/neo-serve.log; do sleep 0.1; done
PORT=4096
```
...
Omit `NEO_SERVER_PASSWORD` entirely. The server prints `Warning: NEO_SERVER_PASSWORD is not set; server is unsecured.` and the auth middleware is bypassed — fine for throwaway local testing, never for anything else.
...
| `--mdns-domain` | `neo.local` |  |
| `--cors` | `[]` | Extra allowed origins. |

## 4. The two mandatory request knobs

### Auth header (only if `NEO_SERVER_PASSWORD` was set)

```bash
AUTH="Authorization: Basic $(printf 'neo:%s' "$NEO_SERVER_PASSWORD" | base64 | tr -d '\n')"
```

The username is literally `neo` (same as the VS Code extension). Skip this whole block if you launched without a password.

### Directory header on every call

```bash
DIR_HEADER="x-neo-directory: $PWD"
```
...
|---|---|
| `NEO_SERVER_PASSWORD` | Enable Basic auth. Omit for auth-bypassed local testing. |
| `NEO_DB=":memory:"` | Skip on-disk SQLite — hermetic runs. |
| `NEO_DISABLE_DEFAULT_PLUGINS=true` | Don't auto-load bundled plugins. |
| `NEO_WORKSPACE_ID=<id>` | Single-workspace mode; disables control-plane routes. |
| `NEO_TELEMETRY_LEVEL=off` | Disable PostHog during tests. |
| `NEO_CONFIG_CONTENT='{…}'` | Inline JSON config without writing a file. |
...
import { createNeoClient } from "@neocode/sdk/v2"

const port = process.env.PORT!
const pass = process.env.NEO_SERVER_PASSWORD
const headers = pass ? { Authorization: "Basic " + Buffer.from("neo:" + pass).toString("base64") } : undefined
...
```bash
PORT="$PORT" NEO_SERVER_PASSWORD="$NEO_SERVER_PASSWORD" bun /tmp/probe.ts
rm /tmp/probe.ts
```

Reminder: this script **connects to** the server you launched in Section 3 — it does not start one. `createNeoServer()` from the SDK would spawn the PATH `neo` binary (production CLI), which defeats the point of testing local code.

## 9. Pitfalls

- Running `neo serve` instead of `bun dev serve` runs the installed prod binary, not your edits.
- Missing `x-neo-directory` (or `?directory=`) returns `400` from `InstanceMiddleware`.
...
- `NEO_SERVER_PASSWORD` must be set **before** launch — changing it after doesn't rotate credentials.
- When sharing `/tmp/neo-serve.log` / `.pid` across terminals, unique-suffix the paths to avoid clobbering parallel runs.
