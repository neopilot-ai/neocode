...
Guidance for the automated reviewer (neo-code-bot) on PRs in this repo.
...
- `knip` unused exports
- `neocode_change` marker rules — both directions:
  - Missing markers on shared opencode files (`script/check-opencode-annotations.ts`)
  - Markers present in neo-only paths like `packages/neo-vscode/`, `packages/neo-ui/`, `packages/opencode/src/neocode/` (`bun run check-neocode-change`)
- Workflow allowlist drift (`script/check-workflows.ts`)
- Stale `packages/neo-docs/source-links.md` (`script/extract-source-links.ts`)
...
- Generated artifact freshness (`check-neo-generated-artifacts.yml`)
...
Neo CLI is a fork of opencode. Minimizing diff against upstream is a top priority.

- If a change modifies a shared opencode file (anything under `packages/opencode/` not in a path containing `neocode`), ask whether the logic could live in a Neo-only directory instead (`packages/opencode/src/neocode/`, `packages/neo-gateway/`, etc.) or be reduced to a smaller hook.
- Refactors or reorganizations of upstream code are a red flag — flag them unless clearly justified.
- See `.neo/skills/neocode-merge-minimizer/SKILL.md` for the decision rules.

### 4. Cloud config schema mirror

When `Config.Info` in `packages/opencode/src/config/config.ts` gains a new `neocode_change` field, the matching JSON Schema entry must also be added in the cloud repo (`apps/web/src/app/config.json/extras.ts`). CI does **not** check this — flag it as a reminder if you see a new config field added.
...
For changes under `packages/neo-vscode/webview-ui/`:
