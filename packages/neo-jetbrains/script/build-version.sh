...
if grep -Eq '^[[:space:]]*neo\.cli\.pinned[[:space:]]*=[[:space:]]*false[[:space:]]*$' "$plugin/gradle.properties"; then
  echo "neo.cli.pinned=false is a dev-only mode and cannot be released. Set neo.cli.pinned=true before building a version." >&2
  exit 1
fi
...
./gradlew buildPlugin -Pproduction=true -Pneo.version="$version" -Pneo.channel=eap

if [[ "$sign" == "1" ]]; then
  ./gradlew signPlugin -Pproduction=true -Pneo.version="$version" -Pneo.channel=eap
fi
...
if [[ "$skip_verification" == "1" ]]; then
  ...
  if [[ "$sign" == "1" ]]; then
    ./gradlew verifyPluginSignature -Pproduction=true -Pneo.version="$version" -Pneo.channel=eap
  fi
  ./gradlew verifyPlugin -Pproduction=true -Pneo.version="$version" -Pneo.channel=eap
fi
