...
resolve() {
  case "$target" in
    client|frontend)
      for file in \
        "$plugin/.intellijPlatform/sandbox/neo.jetbrains/neo-frontend/neo-dev.log" \
    ...
    backend)
      for file in \
        "$plugin/.intellijPlatform/sandbox/neo.jetbrains/neo-backend/neo-dev.log" \
...

{
  sub(/\r$/, "")
  cls = logger($0)
  if ((mode == "client" || mode == "frontend") && cls !~ /NeoSessionService$/) next
  if (mode == "backend" && cls !~ /NeoBackendChatManager$/) next
