...
group = "ai.neocode.jetbrains"

fun port(value: String): Int {
    val text = value.trim()
    require(text.isNotEmpty()) {
        "neo.splitModeServerPort must be an integer from 0 to 65535; use 0 or omit it for a random port"
    }
    val n = text.toIntOrNull()
        ?: error("neo.splitModeServerPort must be an integer from 0 to 65535; use 0 or omit it for a random port")
...
val pinned = providers.gradleProperty("neo.cli.pinned").map { it.trim().toBoolean() }.orElse(true).get()
val override = providers.gradleProperty("neo.version").orNull?.trim()?.takeIf { it.isNotEmpty() }
val prop = providers.gradleProperty("neo.jetbrains.version").orNull?.trim()?.takeIf { it.isNotEmpty() }
val tag = gitTag()?.removePrefix("jetbrains/v")
val ver = override?.let(::checked) ?: prop?.let(::checked) ?: if (release) checked(
    tag ?: error("Missing JetBrains plugin version. Publish builds must set neo.jetbrains.version or run from a jetbrains/v<version> tag."),
) else checked(tag ?: "0.0.0-dev")

if (release && !pinned) error(
    "neo.cli.pinned=false is a dev-only mode and cannot be released. Set neo.cli.pinned=true before a production/publish build."
)

val channel = providers.gradleProperty("neo.channel").map { it.trim() }.orElse("default")
val splitPort = providers.gradleProperty("neo.splitModeServerPort").map(::port).orElse(0)
val isolated = providers.gradleProperty("neo.dev.storage.isolated").map { it.toBoolean() }.orElse(false)
val worktreeRoot = providers.gradleProperty("neo.dev.worktree.root").orElse(
...
changelog {
    ...
    repositoryUrl = "https://github.com/Neopilot-Ai/neocode"
    groups = listOf("Added", "Changed", "Fixed", "Removed", "Security")
    combinePreReleases = false
}

val notes = providers.gradleProperty("neo.changeNotes").orElse(
    provider {
...
tasks.withType<RunIdeTask> {
    val level = providers.gradleProperty("neo.dev.log.level").orNull ?: "DEBUG"
    val content = providers.gradleProperty("neo.dev.log.chat.content").orNull ?: "off"
    val preview = providers.gradleProperty("neo.dev.log.chat.preview.max").orNull ?: "160"
    systemProperty("neo.dev.log.level", level)
    systemProperty("neo.dev.log.chat.content", content)
    systemProperty("neo.dev.log.chat.preview.max", preview)
    systemProperty("neo.dev.storage.isolated", isolated.get().toString())
    systemProperty("neo.dev.worktree.root", worktreeRoot.get())
}
