...
class AgentBehaviorConfigurable : SearchableConfigurable {
    ...
    override fun getDisplayName(): String = NeoBundle.message("settings.agentBehavior.displayName")

    override fun createComponent(): JComponent {
        val panel = Stack.vertical()
        panel.border = JBUI.Borders.empty(UiStyle.Gap.lg(), 0, 0, 0)
        val desc = JBLabel(NeoBundle.message("settings.agentBehavior.description"))
        desc.border = JBUI.Borders.emptyBottom(UiStyle.Gap.pad())
        panel.next(desc)
        listOf(
            NeoBundle.message("settings.agentBehavior.agents.displayName") to AgentsConfigurable.ID,
            NeoBundle.message("settings.agentBehavior.mcp.displayName") to McpConfigurable.ID,
        ).forEach { (label, id) ->
    ...
    companion object {
        const val ID = "ai.neocode.jetbrains.settings.agentBehavior"
    }
}
