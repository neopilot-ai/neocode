...
internal abstract class BaseSettingsUi<C : BaseContentPanel, D, P, R, W>(
    ...
    protected var appState: NeoAppStateDto = app.state.value
        private set
    ...
    @RequiresEdt
    private fun updateApp(state: NeoAppStateDto) {
        appState = state
        if (state.status != NeoAppStatusDto.READY) {
    ...
    private fun load() {
        ...
        if (appState.status != NeoAppStatusDto.READY || workspaceLoading || workspaceLoaded) return
    ...
    @RequiresEdt
    protected abstract fun draft(state: NeoAppStateDto): D
    ...
    @RequiresEdt
    protected open fun onSaveFailedAfterDispose(change: P) = NeoNotifications.error(failedText())
    ...
    @RequiresEdt
    protected open fun unavailable(state: NeoAppStateDto) = Unit
