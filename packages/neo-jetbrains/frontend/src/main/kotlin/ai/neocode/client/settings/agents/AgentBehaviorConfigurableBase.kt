...
abstract class AgentBehaviorConfigurableBase<T : JComponent> : DraftReadyConfigurable<T>() {
    final override fun create(cs: CoroutineScope): T {
        ...
        cs.launch {
            val dir = service<NeoWorkspaceService>().resolveProjectDirectory(selected?.projectIdOrNull(), hint)
    ...
    private companion object {
        val LOG = NeoLog.create(AgentBehaviorConfigurableBase::class.java)
    }
}
