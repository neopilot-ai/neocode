...
class SessionSidePanelManager(
    ...
    private val resolve: (String) -> Workspace = { dir -> service<NeoWorkspaceService>().workspace(dir) },
    private val status: () -> Map<String, SessionActivityKind> = { project.service<NeoSessionService>().activity() },
    ...
    private fun schedule(ui: SessionUi) {
        cancel(ui)
        val delay = Registry.intValue("neo.session.inactive.disposeTimeoutMs").coerceAtLeast(0)
