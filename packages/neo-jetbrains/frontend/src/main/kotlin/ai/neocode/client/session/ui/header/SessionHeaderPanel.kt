package ai.neocode.client.session.ui.header

import ai.neocode.client.plugin.NeoBundle
import ai.neocode.client.session.model.SessionHeaderSnapshot
...
import ai.neocode.client.session.views.todo.TodoListPanel
...
class SessionHeaderPanel(
    ...
    companion object {
        ...
        internal const val EXPANDED_KEY = "neo.session.header.expanded"
    }
    ...
    private val compact = HoverIcon().apply {
        ...
        toolTipText = NeoBundle.message("session.header.compact.description")
        accessibleContext.accessibleName = NeoBundle.message("session.header.compact")
        addActionListener { controller.compact() }
    }
    private val expand = JBLabel().apply {
        cursor = java.awt.Cursor.getPredefinedCursor(java.awt.Cursor.HAND_CURSOR)
        toolTipText = NeoBundle.message("session.header.expand")
        accessibleContext.accessibleName = NeoBundle.message("session.header.expand")
    ...
    private val tokenTitle = JBLabel(NeoBundle.message("session.header.tokens"))
    ...
    private val todoRow = JPanel(FlowLayout(FlowLayout.LEFT, UiStyle.Gap.sm(), 0)).apply {
        ...
        cursor = java.awt.Cursor.getPredefinedCursor(java.awt.Cursor.HAND_CURSOR)
        toolTipText = NeoBundle.message("session.header.todos.toggle")
        accessibleContext.accessibleName = NeoBundle.message("session.header.todos.toggle")
        add(todoArrow)
        add(todos)
    }
    ...
    private fun setTokens(value: TokensDto?) {
        ...
        tokenTitle.text = NeoBundle.message("session.header.tokens")
        tokens.toolTipText = NeoBundle.message("session.header.tokens.description")
        tokenTitle.isVisible = total > 0
        set(input, if (sent > 0) num(sent) else null)
        set(output, if (received > 0) num(received) else null)
        set(cacheWrite, if (write > 0) NeoBundle.message("session.header.cache.write", num(write)) else null)
        set(cacheRead, if (read > 0) NeoBundle.message("session.header.cache.read", num(read)) else null)
    ...
    private fun setCost(value: String?) {
        ...
        cost.text = costValue
        cost.icon = null
        val tip = costValue.takeIf { it.isNotBlank() }?.let { NeoBundle.message("session.header.cost.tooltip", it) }
    ...
    private fun setExpand(expanded: Boolean) {
        ...
        expand.toolTipText = NeoBundle.message(key)
        expand.accessibleContext.accessibleName = NeoBundle.message(key)
    }
