...
import ai.neocode.client.app.NeoProviderService
import ai.neocode.client.plugin.NeoBundle
import ai.neocode.client.settings.base.BaseContentPanel
import ai.neocode.client.settings.base.SettingsPanel
import ai.neocode.client.settings.base.SettingsListConfig
import ai.neocode.client.settings.base.SettingsListSelection
import ai.neocode.client.settings.base.SettingsToolbarAction
import ai.neocode.client.settings.base.SettingsListView
...
import ai.neocode.client.ui.picker.PickerListRenderer
import ai.neocode.client.ui.picker.PickerPopup
import ai.neocode.log.NeoLog
import ai.neocode.rpc.dto.CustomModelDto
...
import ai.neocode.rpc.dto.ProviderAuthMethodDto
import ai.neocode.rpc.dto.ProviderAuthOptionDto
...
internal fun customSaveError(id: String, result: ProviderActionResultDto): String? {
    ...
    if (result.state.providers.none { it.id == id }) return NeoBundle.message("settings.providers.customNotUsable")
    return null
}
...
internal class ProvidersSettingsUi(
    ...
    companion object {
        val LOG = NeoLog.create(ProvidersSettingsUi::class.java)
    }

    private val add = SettingsToolbarAction(
        NeoBundle.message("settings.providers.addCustom"),
        NeoBundle.message("settings.providers.addCustom.description"),
        AllIcons.General.Add,
        { !busy },
    ) { custom() }
    private val refresh = SettingsToolbarAction(
        NeoBundle.message("settings.providers.refresh"),
        NeoBundle.message("settings.providers.refresh.description"),
        AllIcons.Actions.Refresh,
        { !busy },
    ) { reload() }
    private val view = ProvidersContent(::connect, ::oauth, ::disconnect, ::enable, ::edit)
    private val search = SearchTextField(false).apply {
        textEditor.emptyText.text = NeoBundle.message("settings.providers.search")
    }
    ...
    fun reload() {
        ...
        if (!launch("reload") { id ->
            val next = service<NeoProviderService>().state(directory)
    ...
    private fun syncLoading() {
        checkEdt()
        showProgress(NeoBundle.message("settings.providers.loading"))
    }
    ...
    private fun connect(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("connect provider=${provider.id}") { id ->
            val result = service<NeoProviderService>().connect(ProviderConnectDto(directory, provider.id, key, metadata))
    ...
    private fun oauth(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("authorize provider=${provider.id}") { id ->
            val ready = service<NeoProviderService>().authorize(ProviderOAuthAuthorizeDto(directory, provider.id, method))
                    ...
                    if (ready.method == "auto" && url != null) {
                        showOAuthDevice(
                            ...
                            DeviceOAuthInfo(
                                ...
                                expiresIn = (NeoProviderService.OAUTH_RPC_TIMEOUT_MS / 1000).toInt(),
            ...
            val result = service<NeoProviderService>().callback(ProviderOAuthCallbackDto(directory, provider.id, method, code))
            apply(id, result.state, result.error)
        }) return
        showProgress(
            NeoBundle.message("settings.providers.oauth.starting", provider.name),
            NeoBundle.message("settings.providers.oauth.cancel"),
        ) { cancelOAuth(request) }
    }
    ...
    private fun disconnect(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("disconnect provider=${provider.id}") { id ->
            val result = service<NeoProviderService>().disconnect(ProviderDisconnectDto(directory, provider.id))
            apply(id, result.state, result.error)
        }) return
        syncLoading()
    }
    ...
    private fun enable(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("enable provider=${provider.id}") { id ->
            val result = service<NeoProviderService>().enable(ProviderEnableDto(directory, provider.id))
            apply(id, result.state, result.error)
        }) return
        syncLoading()
    }
    ...
    private fun openCustomDialog(existing: CustomProviderEdit?) {
        ...
        val dialog = CustomProviderDialog(
            cs,
            directory,
            { service<NeoProviderService>().fetchCustomModels(it) },
            { service<NeoProviderService>().saveCustom(it) },
            existing,
        )
        if (!dialog.showAndGet()) return
    ...
    private fun syncOAuthWaiting(id: Int) {
        checkEdt()
        if (!active(id)) return
        val expiry = System.currentTimeMillis() + NeoProviderService.OAUTH_RPC_TIMEOUT_MS
        fun text(): String {
            val ms = (expiry - System.currentTimeMillis()).coerceAtLeast(0)
            val remain = ((ms + 999) / 1000).toInt()
            val min = remain / 60
            val sec = remain % 60
            return NeoBundle.message("settings.providers.oauth.waitingTimed", "$min:${sec.toString().padStart(2, '0')}")
        }
        stopTimer()
        showProgress(text(), NeoBundle.message("settings.providers.oauth.cancel")) { cancelOAuth(id) }
        timer = Timer(1000) {
            if (!active(id)) {
                stopTimer()
                return@Timer
...
internal class ProvidersContent(
    ...
    private val view = SettingsListView(NeoBundle.message("settings.providers.noMatches"), SettingsListConfig.Preferred) { key, id ->
        activate(key, id)
    }
    private var state = ProviderSettingsDto()
    private var busy = false
...
private class ApiKeyDialog(title: String, method: ProviderAuthMethodDto?) : DialogWrapper(true) {
    ...
    override fun createCenterPanel(): JComponent {
        ...
        panel.next(JBLabel(NeoBundle.message("settings.providers.apiKey")))
        panel.next(key)
    ...
    override fun doValidate(): ValidationInfo? {
        if (key().isBlank()) return ValidationInfo(NeoBundle.message("settings.providers.apiKeyRequired"), key)
        return null
    }
...
internal class CustomProviderDialog(
    ...
    private val pick = JButton(NeoBundle.message("settings.providers.customSelectModels"))
    ...
    init {
        title = if (existing != null) {
            NeoBundle.message("settings.providers.customEditTitle")
        } else {
            NeoBundle.message("settings.providers.customTitle")
        }
        setOKButtonText(
            if (existing != null) NeoBundle.message("settings.providers.customSave")
            else NeoBundle.message("settings.providers.customAdd"),
        )
        init()
        initValidation()
    ...
    override fun createCenterPanel(): JComponent {
        val panel = Stack.vertical(UiStyle.Gap.sm())
        listOf(
            NeoBundle.message("settings.providers.customId") to id,
            NeoBundle.message("settings.providers.customName") to name,
            NeoBundle.message("settings.providers.customUrl") to url,
            NeoBundle.message("settings.providers.apiKey") to key,
            NeoBundle.message("settings.providers.customEnv") to env,
        ).forEach { (label, field) ->
            panel.next(JBLabel(label))
            panel.next(field)
        }
        panel.next(JBLabel(NeoBundle.message("settings.providers.customModels")))
    ...
    override fun doValidate(): ValidationInfo? {
        if (id.text.isBlank()) return ValidationInfo(NeoBundle.message("settings.providers.customIdRequired"), id)
        if (url.text.isBlank()) return ValidationInfo(NeoBundle.message("settings.providers.customUrlRequired"), url)
        actionError?.let { return ValidationInfo(it) }
        if (!fetching && modelIds().isEmpty()) return ValidationInfo(NeoBundle.message("settings.providers.customModelsRequired"), models)
        return null
    }
    ...
    private fun selectModels() {
        ...
        job = cs.launch {
            ...
            withContext(edt) {
                ...
                if (ids.isEmpty()) {
                    fail(NeoBundle.message("settings.providers.customModelsEmpty"))
                    return@withContext
                }
                showModelPopup(ids)
            }
    ...
    private fun startFetch() {
        ...
        models.text = NeoBundle.message("settings.providers.customFetchingModels")
        syncActions()
    }
    ...
    private fun fetchValidationError(): String? {
        if (url.text.isBlank()) return NeoBundle.message("settings.providers.customUrlRequired")
        if (!url.text.trim().let { it.startsWith("http://") || it.startsWith("https://") }) return NeoBundle.message("settings.providers.customUrlInvalid")
        return null
    }
    ...
    private fun showModelPopup(ids: List<String>) {
        ...
        val select = ActionLink(NeoBundle.message("settings.providers.customModelsSelectAll"))
        val clear = ActionLink(NeoBundle.message("settings.providers.customModelsUnselectAll"))
    ...
    private fun syncActions() {
        ...
        pick.text = if (fetching) {
            NeoBundle.message("settings.providers.customCancelModels")
        } else {
            NeoBundle.message("settings.providers.customSelectModels")
        }
    }
