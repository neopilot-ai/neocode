...
import ai.neocode.client.actions.SendPromptAction
import ai.neocode.client.actions.StopSessionAction
...
import ai.neocode.client.session.model.PromptAttachment
import ai.neocode.client.session.model.PromptAttachmentExtractor
...
import ai.neocode.client.ui.iconButton
...
class PromptPanel(
    ...
    private val completion: NeoPromptCompletionProvider? = null,
    ...
    companion object {
        private val LOG = NeoLog.create(PromptPanel::class.java)
    ...
    private val reset = HoverIcon().apply {
        ...
        toolTipText = NeoBundle.message("model.picker.reset")
        accessibleContext.accessibleName = NeoBundle.message("model.picker.reset")
        isVisible = false
    ...
    private val enhance = HoverIcon().apply {
        icon = WAND_ICON
        toolTipText = NeoBundle.message("prompt.action.enhance")
        accessibleContext.accessibleName = NeoBundle.message("prompt.action.enhance")
    ...
    private fun key(kind: NeoPromptCompletionProvider.HighlightKind): TextAttributesKey = when (kind) {
        NeoPromptCompletionProvider.HighlightKind.MENTION -> MENTION_KEY
        NeoPromptCompletionProvider.HighlightKind.COMMAND -> COMMAND_KEY
        NeoPromptCompletionProvider.HighlightKind.INVALID -> INVALID_KEY
    }
    ...
    private fun enhance() {
        ...
        if (source.isBlank()) {
            editor.text = NeoBundle.message("prompt.action.enhance.description")
            syncEditorHeight()
            focus()
            return
        }
    ...
    private fun completeEnhancement(id: Long, source: String, result: Result<String>) {
        ...
        result.onSuccess {
            ...
            NeoNotifications.error(
                project,
                NeoBundle.message("prompt.action.enhance.failed"),
                NeoBundle.message("prompt.action.enhance.failed.description"),
            )
        }
    }
    ...
    private fun syncEnhance() {
        ...
        enhance.toolTipText = if (enhancing) {
            NeoBundle.message("prompt.action.enhance.loading")
        } else {
            NeoBundle.message("prompt.action.enhance")
        }
    }
        ...
        cs.launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    notify(NeoBundle.message("prompt.attachment.send.failed", e.message ?: e.javaClass.simpleName))
                }
            }
        }
    }
    ...
    private fun installCompletionShortcut(ed: com.intellij.openapi.editor.Editor) {
        ...
        val action = completionAction ?: object : DumbAwareAction(NeoBundle.message("prompt.completion.action")) {
    ...
    private fun addAttachment(item: PromptAttachment) {
        if (!attachment && PromptAttachmentExtractor.media(item.mime)) {
            ...
            notify(NeoBundle.message("prompt.attachment.unsupported.model"))
            return
        }
    ...
    private fun promptDto(text: String, files: List<PromptPartDto>) = ai.neocode.rpc.dto.PromptDto(
        parts = buildList {
    ...
    private fun notify(text: String) {
        com.intellij.notification.Notification("Neo Code", text, com.intellij.notification.NotificationType.WARNING).notify(project)
    }
    ...
    private fun syncAutoApprove() {
        ...
        auto.toolTipText = if (autoApprove) {
            NeoBundle.message("prompt.action.autoApprove.enabled.tooltip")
        } else {
            NeoBundle.message("prompt.action.autoApprove.disabled.tooltip")
        }
        auto.accessibleContext.accessibleName = if (autoApprove) {
            NeoBundle.message("prompt.action.autoApprove.disable")
        } else {
            NeoBundle.message("prompt.action.autoApprove.enable")
        }
        auto.repaint()
    }
    ...
    private fun tooltip(): String {
        val id = if (busy) StopSessionAction.ID else SendPromptAction.ID
        val text = if (busy) {
            NeoBundle.message("prompt.button.stop")
        } else {
            NeoBundle.message("prompt.button.send")
        }
        ...
        return XmlStringUtil.wrapInHtml(
            XmlStringUtil.escapeString(tip) + "<br>" +
                XmlStringUtil.escapeString(NeoBundle.message("prompt.button.send.tooltip.stop", stop))
        )
    }
    ...
    private fun placeholder(): String {
        ...
        if (send.isNotEmpty() && line.isNotEmpty()) {
            return NeoBundle.message("prompt.placeholder.with.shortcuts", send, line)
        }
        if (send.isNotEmpty()) return NeoBundle.message("prompt.placeholder.with.send", send)
        if (line.isNotEmpty()) return NeoBundle.message("prompt.placeholder.with.newline", line)
        return NeoBundle.message("prompt.placeholder")
    }
