...
import ai.neocode.client.ui.RoundedContentPanel
...
internal class LoggedInProfileUi(
    ...
    companion object {
        private val LOG = NeoLog.create(LoggedInProfileUi::class.java)
    }
    ...
    private val logoLabel = JBLabel(IconLoader.getIcon("/icons/neo-profile.svg", LoggedInProfileUi::class.java)).apply {
        name = "neo.profile.logo.loggedIn"
        accessibleContext.accessibleName = NeoBundle.message("settings.neo.displayName")
    }

    private val titleLabel = JBLabel(NeoBundle.message("profile.balance.title")).apply {
        foreground = UiStyle.Colors.weak()
    }
    ...
    private val refreshBtn = JButton(NeoBundle.message("profile.action.refresh"), AllIcons.Actions.Refresh)
        .also {
            it.isOpaque = false
    ...
    private val passTitleLabel = JBLabel(NeoBundle.message("profile.pass.title")).apply {
    ...
    private val passBonusLabel = JBLabel(NeoBundle.message("profile.pass.bonusLabel")).apply {
        foreground = UiStyle.Colors.weak()
        font = UiStyle.Fonts.small()
    }
    ...
    private val passRenewLabel = JBLabel(NeoBundle.message("profile.pass.renewsLabel")).apply {
        foreground = UiStyle.Colors.weak()
        font = UiStyle.Fonts.small()
    }
    ...
    private val passRenewRow = Stack.horizontal(UiStyle.Gap.md())
        ...
        .fill(UiStyle.Gap.md())
        .next(passRenewValue)
    private val passLink = ActionLink(NeoBundle.message("profile.pass.subscribe")) {
        pass()
    }.apply {
        name = "neo.profile.passSubscribe"
    }
    ...
    val dashboardBtn = JButton(NeoBundle.message("profile.action.dashboard"))
        .also { it.addActionListener { dashboard() } }
    val topUpBtn = JButton(NeoBundle.message("profile.action.topUp"))
        .also { it.addActionListener { topUp() } }
    val logoutBtn = JButton(NeoBundle.message("profile.action.logout"))
    ...
    fun update(profile: ProfileDto) {
        ...
        val item = if (personal) profile.neoPass else null
    ...
    @RequiresEdt
    private fun syncPass(item: ProfileNeoPassDto?, show: Boolean): Boolean {
        var changed = false
    ...
    fun setRefreshing(refreshing: Boolean) {
        ...
        val text = if (refreshing) NeoBundle.message("profile.action.refreshing")
        else NeoBundle.message("profile.action.refresh")
    ...
    private fun applyOrganizations(profile: ProfileDto) {
        ...
        val keys: List<Pair<String?, String>> = (if (personal) listOf(null to NeoBundle.message("profile.personalAccount")) else emptyList()) +
