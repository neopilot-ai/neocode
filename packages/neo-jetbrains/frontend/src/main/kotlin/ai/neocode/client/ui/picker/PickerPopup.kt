package ai.neocode.client.ui.picker
...
internal class PickerPopup<T>(
    ...
    private val emptyText: String = NeoBundle.message("model.picker.no.matches"),
) {
    ...
    private val field = if (search) SearchTextField(false).apply {
        textEditor.emptyText.text = NeoBundle.message("model.picker.search")
    } else null
    ...
    private fun syncExpand() {
        ...
        expand.toolTipText = if (expanded) {
            NeoBundle.message("model.picker.details.minimize")
        } else {
            NeoBundle.message("model.picker.details.maximize")
        }
