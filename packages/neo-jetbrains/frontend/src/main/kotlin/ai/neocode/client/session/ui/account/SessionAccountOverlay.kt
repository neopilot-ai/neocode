...
import ai.neocode.client.settings.profile.formatBalance
...
internal class SessionAccountOverlay(
    ...
    private val profileBtn = HoverIcon().apply {
        ...
        toolTipText = NeoBundle.message("action.Neo.ShowProfile.description")
        accessibleContext.accessibleName = NeoBundle.message("action.Neo.ShowProfile.text")
    ...
    @RequiresEdt
    private fun updateLoggedIn(prof: ai.neocode.rpc.dto.ProfileDto, switching: Boolean, target: String?): Boolean {
        var layout = false

        val orgs = prof.organizations
        val personal = prof.hasPersonalAccount
        val next = (if (personal) listOf(AccountChoice(null, NeoBundle.message("profile.personalAccount"))) else emptyList()) +
        ...
        val tip = if (switching) {
            NeoBundle.message("profile.switchingAccount")
        } else {
            NeoBundle.message("session.account.switcher")
        }
    ...
    @RequiresEdt
    private fun syncBalance(prof: ai.neocode.rpc.dto.ProfileDto): Boolean {
        var layout = false
        ...
        if (next == null) {
            ...
            val tip = NeoBundle.message("session.account.balance", next)
