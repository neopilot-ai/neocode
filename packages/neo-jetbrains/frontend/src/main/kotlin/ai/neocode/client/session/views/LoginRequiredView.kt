 ...
 * enters [ai.neocode.client.session.model.SessionState.LoginRequired].
 *
 * Mirrors the anchored placement of [PermissionView] and [question.QuestionView]:
 * it stays as a stable child inside [ai.neocode.client.session.ui.SessionMessageListPanel]
...
class LoginRequiredView(
    ...
    init {
        ...
        card.setHeader(NeoBundle.message("session.login.required.title"))
        card.setActions(listOf(
            BaseQuestionView.Action(ID_DISMISS, NeoBundle.message("session.login.required.dismiss"), primary = false) { dismiss() },
            BaseQuestionView.Action(ID_OPEN, NeoBundle.message("session.login.required.button"), primary = true) { openProfile() },
        ))
    ...
    internal fun openProfileButton() = button(NeoBundle.message("session.login.required.button"))
    internal fun dismissButton() = button(NeoBundle.message("session.login.required.dismiss"))
