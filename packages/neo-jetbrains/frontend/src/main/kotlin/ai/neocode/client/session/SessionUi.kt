...
import ai.neocode.client.migration.MigrationUiController
import ai.neocode.client.migration.MigrationUiState
import ai.neocode.client.migration.ui.MigrationOverlayPanel
...
import ai.neocode.client.session.scroll.SessionScroll
import ai.neocode.client.session.ui.ConnectionPanel
import ai.neocode.client.session.ui.empty.EmptySessionPanel
import ai.neocode.client.session.ui.LoadingPanel
import ai.neocode.client.session.ui.ReasoningPicker
import ai.neocode.client.session.ui.RevertBanner
import ai.neocode.client.session.ui.mode.ModePicker
import ai.neocode.client.session.ui.model.ModelPicker
import ai.neocode.client.session.ui.prompt.NeoPromptCompletionProvider
import ai.neocode.client.session.ui.prompt.MentionAction
import ai.neocode.client.session.ui.prompt.PromptPanel
import ai.neocode.client.session.ui.prompt.SlashAction
import ai.neocode.client.session.ui.prompt.mentionParts as promptMentionParts
import ai.neocode.client.session.ui.account.SessionAccountOverlay
import ai.neocode.client.session.ui.popup.HeaderPopupController
import ai.neocode.client.session.ui.SessionDropOverlay
import ai.neocode.client.session.ui.SessionRootPanel
import ai.neocode.client.session.ui.SessionMessageListPanel
import ai.neocode.client.session.ui.attachment.AttachmentEditorKind
import ai.neocode.client.session.ui.attachment.attachmentParams
import ai.neocode.client.session.ui.attachment.ensureAttachmentEditorKind
import ai.neocode.client.session.ui.attachment.isEmbeddedAttachment
import ai.neocode.client.session.ui.header.SessionHeaderPanel
import ai.neocode.client.session.ui.selection.SessionContextMenu
import ai.neocode.client.session.ui.selection.SessionHoverCopyOverlay
...
import ai.neocode.client.session.controller.EVENT_FLUSH_MS
import ai.neocode.client.session.controller.SessionController
import ai.neocode.client.session.controller.SessionControllerEvent
import ai.neocode.client.session.ui.style.SessionUiStyle
import ai.neocode.client.session.views.LoginRequiredView
import ai.neocode.client.session.views.permission.PermissionView
import ai.neocode.client.session.views.question.QuestionView
...
import ai.neocode.client.vfs.NeoVfsManager
...
import ai.neocode.rpc.dto.SessionRevertDto
...
class SessionUi(
    ...
    private val migration: MigrationUiController = service<NeoMigrationService>(),
    private val timers: UiTimerSource = UiTimers,
) : JPanel(BorderLayout()), Disposable, SessionEditorStyleTarget, UiDataProvider {

    companion object {
        private val LOG = NeoLog.create(SessionUi::class.java)
    ...
    private val flushMs =
        Registry.intValue("neo.session.flushMs", EVENT_FLUSH_MS.toInt())
            .takeIf { it > 0 }
            ?.toLong()
            ?: EVENT_FLUSH_MS
    ...
    private val controller = SessionController(
        ...
        comp = this,
        flushMs = flushMs,
        condense = Registry.`is`("neo.session.condense", true),
        displayMs = displayMs,
    ...
    private lateinit var completion: NeoPromptCompletionProvider
    ...
    private fun buildUi() {
        ...
        completion = NeoPromptCompletionProvider(
            workspace = workspace,
            service = workspaces,
    ...
    private fun syncPromptRevert() {
        ...
        val text = msg.parts.values.filterIsInstance<ai.neocode.client.session.model.Text>().firstOrNull()?.content?.toString() ?: return
        prompt.setText(text)
    ...
    private fun slashActions(): List<SlashAction> {
        val fns: Map<SlashAction.Spec, () -> Unit> = mapOf(
            ...
            SlashAction.SETTINGS to { openNeoSettings() },
            SlashAction.HELP to { BrowserUtil.browse("https://neo.khulnasoft.com/docs") },
        )
    ...
    private fun bind(spec: SlashAction.Spec, action: () -> Unit) = SlashAction(
        spec.name,
        NeoBundle.message(spec.descriptionKey),
        spec.hints,
        {
    ...
    private fun openAttachment(messageId: String, item: FileAttachment) {
        ...
        if (isEmbeddedAttachment(url)) {
            ...
            LOG.info("kind=attachment-open route=neo-vfs session=$id message=$messageId part=${item.id} name=${attachmentName(item)}")
            ensureAttachmentEditorKind()
            project.service<NeoVfsManager>().open(
    ...
    private fun openNeoSettings() {
        ShowSettingsUtil.getInstance().showSettingsDialog(
            project,
            Predicate { cfg: Configurable ->
                cfg is ConfigurableWithId && cfg.getId() == NeoSettingsConfigurable.ID
            },
            { _: Configurable -> },
        )
    }
...
private fun ai.neocode.client.session.model.QuestionItem.planFollowup() =
