...
import ai.neocode.client.ui.PickerButton
import ai.neocode.client.ui.picker.PickerPopup
import ai.neocode.client.ui.picker.popupBackground
...
class ModelPicker : PickerButton() {
    ...
    var emptyText: String = NeoBundle.message("settings.models.notSet")
    ...
    init {
        isEnabled = false
        text = " "
        toolTipText = NeoBundle.message("model.picker.tooltip")
    ...
    private fun refresh() {
        ...
        toolTipText = if (item?.let(ModelText::collectsData) == true) ModelText.dataCollectedTooltip() else NeoBundle.message("model.picker.tooltip")
        isEnabled = true
...
internal const val MODEL_PICKER_EXPANDED_KEY = "neo.model.picker.expanded"
...
internal object ModelText {
    private val small = setOf("neo-auto/small", "auto-small")
    ...
    fun buttonLabel(item: ModelPicker.Item): String {
        ...
        if (item.provider == "neo") return part
        val provider = item.providerName.trim()
        if (provider.isEmpty()) return part
        return "$provider / $part"
    }

    fun small(item: ModelPicker.Item): Boolean = item.provider == "neo" && item.id in small

    fun providerSort(id: String): Int = if (id == "neo") 0 else 1

    fun dataCollected(): String = NeoBundle.message("model.picker.dataCollected")

    fun dataCollectedTooltip(): String = XmlStringUtil.wrapInHtmlLines(
        NeoBundle.message("model.picker.tooltip"),
        NeoBundle.message("model.picker.dataCollected.current"),
    )

    fun freeLabel(): String = NeoBundle.message("model.picker.free")
