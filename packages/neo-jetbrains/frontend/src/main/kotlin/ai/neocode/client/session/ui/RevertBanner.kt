...
class RevertBanner(
    ...
    private val hint = JBLabel(NeoBundle.message("revert.banner.hint")).apply {
        font = JBFont.small()
    }

    private val notice = JBLabel(NeoBundle.message("revert.banner.filesNotRestored")).apply {
        font = JBFont.small()
    }

    init {
        isOpaque = false
        card.setHeaderIcon(AllIcons.Actions.Back, NeoBundle.message("revert.message.rollback"))
        body.next(files).next(hint).next(notice)
        card.setContent(body)
        card.setActions(listOf(
            BaseQuestionView.Action("redo", NeoBundle.message("revert.banner.redo"), primary = false) { redoAction() },
            BaseQuestionView.Action("all", NeoBundle.message("revert.banner.redo.all"), primary = false) { redoAllAction() },
        ))
    ...
    fun update() {
        ...
        card.setHeader(NeoBundle.message(if (total == 1) "revert.banner.count.one" else "revert.banner.count.other", total))
