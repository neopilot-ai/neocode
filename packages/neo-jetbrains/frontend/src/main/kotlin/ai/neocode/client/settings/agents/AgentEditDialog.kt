...
import ai.neocode.client.settings.models.ModelSettingPicker
...
internal class AgentEditDialog(
    ...
    private val model = ModelSettingPicker().apply {
        picker.emptyText = NeoBundle.message("settings.agentBehavior.agents.edit.default")
    ...
    init {
        title = NeoBundle.message("settings.agentBehavior.agents.edit.title", agent.displayName ?: agent.name)
        init()
        initValidation()
    }
    ...
    override fun createCenterPanel(): JComponent {
        ...
        SettingsRows().apply {
            row(SettingsStackedRow(
                NeoBundle.message("settings.agentBehavior.agents.edit.name"),
                value = identity(),
                action = exportButton().takeIf { !agent.native },
            ))
            row(SettingsRow(
                NeoBundle.message("settings.agentBehavior.agents.edit.mode"),
                NeoBundle.message("settings.agentBehavior.agents.edit.mode.description"),
                mode,
            ))
            panel.next(this)
        }
        SettingsRows().apply {
            row(SettingsStackedRow(
                NeoBundle.message("settings.agentBehavior.agents.edit.description"),
                if (canEditDescription(agent)) {
                    NeoBundle.message("settings.agentBehavior.agents.edit.description.description")
                } else {
                    NeoBundle.message("settings.agentBehavior.agents.edit.description.native.description")
                },
                scroll(description),
            ))
            row(SettingsStackedRow(
                if (agent.native) {
                    NeoBundle.message("settings.agentBehavior.agents.edit.prompt.native")
                } else {
                    NeoBundle.message("settings.agentBehavior.agents.edit.prompt")
                },
                if (agent.native) {
                    NeoBundle.message("settings.agentBehavior.agents.edit.prompt.native.description")
                } else {
                    NeoBundle.message("settings.agentBehavior.agents.edit.prompt.description")
                },
                scroll(prompt),
            ))
            panel.next(this)
        }
        panel.section(NeoBundle.message("settings.agentBehavior.agents.edit.model")).apply {
            row(SettingsRow(
                NeoBundle.message("settings.agentBehavior.agents.edit.model.override"),
                NeoBundle.message("settings.agentBehavior.agents.edit.model.override.description"),
                model,
            ))
            row(SettingsRow(
                NeoBundle.message("settings.agentBehavior.agents.edit.variant"),
                NeoBundle.message("settings.agentBehavior.agents.edit.variant.description"),
                variant,
            ))
            row(SettingsRow(
                NeoBundle.message("settings.agentBehavior.agents.edit.temperature"),
                NeoBundle.message("settings.agentBehavior.agents.edit.temperature.description"),
                temperature,
            ))
            row(SettingsRow(
                NeoBundle.message("settings.agentBehavior.agents.edit.topP"),
                NeoBundle.message("settings.agentBehavior.agents.edit.topP.description"),
                top,
            ))
            row(SettingsRow(
                NeoBundle.message("settings.agentBehavior.agents.edit.steps"),
                NeoBundle.message("settings.agentBehavior.agents.edit.steps.description"),
                steps,
            ))
        }
        if (canEditVisibility(agent)) panel.section(NeoBundle.message("settings.agentBehavior.agents.edit.visibility")).apply {
            row(SettingsRow(
                NeoBundle.message("settings.agentBehavior.agents.edit.hidden"),
                NeoBundle.message("settings.agentBehavior.agents.edit.hidden.description"),
                SettingsToggle(hidden) { hidden = it },
            ))
            row(SettingsRow(
                NeoBundle.message("settings.agentBehavior.agents.edit.disabled"),
                NeoBundle.message("settings.agentBehavior.agents.edit.disabled.description"),
    ...
    override fun getDimensionServiceKey(): String = "Neo.AgentEditDialog"
    ...
    private fun validateMode(): ValidationInfo? {
        ...
        if (value == NeoCliParser.MODE_PRIMARY || value == NeoCliParser.MODE_SUBAGENT || value == NeoCliParser.MODE_ALL) return null
        return ValidationInfo(NeoBundle.message("settings.agentBehavior.agents.edit.mode.invalid"), mode)
    }
    ...
    private fun validateNumber(field: NumericField, key: String, min: Double? = null, max: Double? = null): ValidationInfo? {
        ...
        if (parsed == null || !parsed.isFinite()) return ValidationInfo(NeoBundle.message(key), field)
        if (min != null && parsed < min) return ValidationInfo(NeoBundle.message(key), field)
        if (max != null && parsed > max) return ValidationInfo(NeoBundle.message(key), field)
        return null
    }
    ...
    private fun validateSteps(): ValidationInfo? {
        ...
        return ValidationInfo(NeoBundle.message("settings.agentBehavior.agents.edit.steps.invalid"), steps)
    }

    private class NumericField(value: String, decimal: Boolean) : JBTextField(value) {
        init {
            columns = NUMERIC_COLUMNS
            emptyText.text = NeoBundle.message("settings.agentBehavior.agents.edit.default")
    ...
    private fun identity(): JComponent {
        val badges = listOfNotNull(
            badge(
                NeoBundle.message("settings.agentBehavior.badge.subagent"),
                UiStyle.Badge.Primary,
            ).takeIf { NeoCliParser.isSubagent(agent.mode) },
            badge(
                NeoBundle.message("settings.agentBehavior.badge.custom"),
                UiStyle.Badge.Primary,
            ).takeIf { canDelete(agent) },
            badge(NeoBundle.message("settings.agentBehavior.badge.hidden")).takeIf { agent.hidden },
            badge(NeoBundle.message("settings.agentBehavior.badge.disabled")).takeIf { agent.disable },
            badge(
                NeoBundle.message("settings.agentBehavior.badge.deprecated"),
                UiStyle.Badge.Alert,
    ...
    private fun export() {
        ...
        val descriptor = FileSaverDescriptor(
            NeoBundle.message("settings.agentBehavior.agents.edit.export.title"),
            NeoBundle.message("settings.agentBehavior.agents.edit.export.description"),
            "agent.json",
        )
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            runCatching {
                ...
                notify(NotificationType.INFORMATION, NeoBundle.message("settings.agentBehavior.agents.edit.export.success", wrapper.file.name))
            }.onFailure { err ->
                notify(NotificationType.ERROR, NeoBundle.message("settings.agentBehavior.agents.edit.export.failed"), err.message)
            }
        }
    }
    ...
    private fun notify(type: NotificationType, title: String, content: String? = null) {
        ApplicationManager.getApplication().invokeLater {
            val notification = NotificationGroupManager.getInstance()
                .getNotificationGroup("Neo Code")
                ?.createNotification(title, content.orEmpty(), type)
                ?: Notification("Neo Code", title, content.orEmpty(), type)
    ...
    private class PromptField(value: String) : EditorTextField(
        ...
        init {
            ...
            setPlaceholder(NeoBundle.message("settings.agentBehavior.agents.edit.prompt.placeholder"))
