...
class NeoProviderService internal constructor(
    private val cs: CoroutineScope,
    private val rpc: NeoProviderRpcApi?,
) {
    constructor(cs: CoroutineScope) : this(cs, null)

    companion object {
        private val LOG = NeoLog.create(NeoProviderService::class.java)
        private const val RPC_TIMEOUT_MS = 20_000L
        internal const val OAUTH_RPC_TIMEOUT_MS = 90_000L
    }

    private suspend fun <T> call(name: String, timeoutMs: Long = RPC_TIMEOUT_MS, block: suspend NeoProviderRpcApi.() -> T): T {
        val start = System.currentTimeMillis()
        LOG.info("provider settings rpc $name: start")
        val api = rpc
        return try {
            val result = withTimeout(timeoutMs) {
                if (api != null) block(api) else durable { block(NeoProviderRpcApi.getInstance()) }
            }
    ...
    private suspend fun action(directory: String, timeoutMs: Long = RPC_TIMEOUT_MS, block: suspend NeoProviderRpcApi.() -> ProviderActionResultDto): ProviderActionResultDto {
        ...
        service<NeoWorkspaceService>().reload(directory)
        service<NeoAppService>().refreshProfileAsync()
