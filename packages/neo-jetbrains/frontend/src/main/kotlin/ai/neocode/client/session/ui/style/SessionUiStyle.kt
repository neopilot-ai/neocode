package ai.neocode.client.session.ui.style
...
object SessionUiStyle {
    ...
    object View {
        ...
        /** Markdown colors that mirror Neo's VS Code webview tokens. */
        object Markdown {
            fun string(): Color = JBColor.namedColor(
                "Neo.Session.Markdown.String",
                JBColor(0xA31515, 0xCE9178),
            )
        }

        object Todo {
            fun checkBg(): Color = JBColor.namedColor("Neo.Session.Todo.Checkbox.Background", Color.WHITE)

            fun checkFg(): Color = JBColor.namedColor("Neo.Session.Todo.Checkbox.Foreground", Color(0x1F, 0x23, 0x28))
    ...
    object Timeline {
        val READ: Color = JBColor.namedColor("Neo.Session.Timeline.Read", Color(0x37, 0x94, 0xff))
        val WRITE: Color = JBColor.namedColor("Neo.Session.Timeline.Write", Color(0x00, 0x7f, 0xd4))
        val TOOL: Color = JBColor.namedColor("Neo.Session.Timeline.Tool", Color(0x00, 0x7a, 0xcc))
        val SUCCESS: Color = JBColor.namedColor("Label.successForeground", UIUtil.getLabelSuccessForeground())
        val ERROR: Color = JBColor.namedColor("Neo.Session.Timeline.Error", UIUtil.getErrorForeground())
        val TEXT: Color = JBColor.namedColor("Neo.Session.Timeline.Text", UIUtil.getContextHelpForeground())
        val STEP: Color = JBColor.namedColor("Neo.Session.Timeline.Step", JBColor.border())
    }
}
