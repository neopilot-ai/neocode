package ai.neocode.client.settings.auth
...
internal class DeviceOAuthPanel(
    ...
    val qrLabel = JBLabel().apply {
        ...
        name = "$prefix.qr"
        accessibleContext.accessibleName = NeoBundle.message("profile.login.qr")
        accessibleContext.accessibleDescription = copy.qrDescription
    }

    private val openBtn = JButton(NeoBundle.message("profile.login.openBrowser"))
    private val cancelBtn = JButton(NeoBundle.message("profile.login.cancel")).also { it.addActionListener { cancel() } }
    private val copyUrlBtn = HoverIcon().apply {
        icon = AllIcons.Actions.Copy
        toolTipText = NeoBundle.message("profile.login.copyUrl")
    }
    private val codePanel = RoundedContentPanel(UiStyle.Gap.sm(), UiStyle.Gap.md()).apply {
        ...
        addMouseListener(object : MouseAdapter() {
            override fun mouseClicked(e: MouseEvent) {
                val c = code ?: return
                copyToClipboard(c, NeoBundle.message("profile.login.codeCopied"), this@DeviceOAuthPanel)
            }
        })
    }
    ...
    private val codeHint = JBLabel(NeoBundle.message("profile.login.clickToCopy")).apply {
        foreground = UiStyle.Colors.weak()
        horizontalAlignment = SwingConstants.CENTER
    }
    private val waitIcon = AsyncProcessIcon("NeoOAuth")
    ...
    private fun build() {
        ...
        add(stepLabel(NeoBundle.message("profile.login.step.one"), NeoBundle.message("profile.login.step.url")), gbc(row++, UiStyle.Gap.md()))
        add(urlRow(), gbc(row++, UiStyle.Gap.sm()))
        add(qrLabel, gbc(row++, UiStyle.Gap.md()).centered())
        val s2 = stepLabel(NeoBundle.message("profile.login.step.two"), NeoBundle.message("profile.login.step.code"))
        step2 = s2
    ...
    fun update(info: DeviceOAuthInfo) {
        ...
        if (info.url != last) {
            ...
            copyUrlBtn.addActionListener { copyToClipboard(info.url, NeoBundle.message("profile.login.urlCopied"), copyUrlBtn) }
            qrLabel.icon = try {
    ...
    private fun syncTime() {
        ...
        val min = remain / 60
        val sec = remain % 60
        waitLabel.text = NeoBundle.message("profile.login.waitingTimed", "$min:${sec.toString().padStart(2, '0')}")
    }
