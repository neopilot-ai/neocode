...
internal abstract class SettingsListPanel(
    ...
    protected val view = SettingsListView(NeoBundle.message("settings.agentBehavior.empty"), cfg) { key, id ->
        onCell(key, id)
    }
    ...
    protected open fun emptyText(): String = NeoBundle.message("settings.agentBehavior.empty")

    protected open fun loadingText(): String = NeoBundle.message("settings.agentBehavior.loading")
    ...
    private fun toolbar(): JComponent {
        ...
        if (showRefresh()) {
            ...
            actions += SettingsToolbarAction(
                NeoBundle.message("settings.agentBehavior.refresh"),
                NeoBundle.message("settings.agentBehavior.refresh.description"),
    ...
    protected suspend fun waitForReady() {
        val flow = service<NeoAppService>().state
        val saw = withTimeoutOrNull(RELOAD_START_TIMEOUT_MS) {
            flow.first { it.status != NeoAppStatusDto.READY }
        } != null
        if (!saw) return
    ...
    private companion object {
        val LOG = NeoLog.create(SettingsListPanel::class.java)
