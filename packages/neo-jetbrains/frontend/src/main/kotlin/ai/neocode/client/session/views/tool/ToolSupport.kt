...
private val LOG = NeoLog.create(ToolParts::class.java)
...
internal fun title(tool: Tool) = when (tool.name) {
    "read" -> NeoBundle.message("session.part.tool.read")
    "bash" -> NeoBundle.message("session.part.tool.shell")
    else -> toolTitle(tool)
}
...
internal fun stateText(tool: Tool) = when (tool.state) {
    ToolExecState.PENDING -> NeoBundle.message("session.part.tool.pending")
    ToolExecState.RUNNING -> NeoBundle.message("session.part.tool.running")
    ToolExecState.COMPLETED -> ""
    ToolExecState.ERROR -> NeoBundle.message("session.part.tool.error")
}
...
internal fun target(tool: Tool): Target? {
    ...
    val path = NeoCliParser.tag(out, "path") ?: return null
    val type = NeoCliParser.tag(out, "type") ?: return null
...
private class Preview {
    ...
    fun build(): String {
        ...
        text.append(NeoBundle.message("session.part.tool.truncated"))
