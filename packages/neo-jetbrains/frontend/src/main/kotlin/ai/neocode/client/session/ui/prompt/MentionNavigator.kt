...
internal class MentionNavigator(
    ...
    private val provider: NeoPromptCompletionProvider,
    ...
    fun install() {
        ...
        object : DumbAwareAction(NeoBundle.message("prompt.mention.goto")) {
    ...
    private fun syncTooltip(hit: NeoPromptCompletionProvider.MentionHit?) {
        val text = hit?.takeIf { !it.resolved }?.let { NeoBundle.message("prompt.mention.unresolved", it.value) }
        if (editor.contentComponent.toolTipText != text) editor.contentComponent.toolTipText = text
    }

    private fun syncLink(hit: NeoPromptCompletionProvider.MentionHit?, mouse: MouseEvent) {
