package ai.neocode.client.session.views.question
...
class QuestionView(
    ...
    private val back = HoverIcon().apply {
        ...
        toolTipText = NeoBundle.message("session.question.back")
    ...
    private val fwd = HoverIcon().apply {
        ...
        icon = ico
        disabledIcon = IconLoader.getDisabledIcon(ico)
        toolTipText = NeoBundle.message("session.question.next")
    ...
    private fun syncPage() {
        ...
        body.removeAll()
        if (review(q)) {
            card.setHeader(NeoBundle.message("session.question.review.title"))
            addReview(q)
        } else {
            val item = q.items[idx]
            val hint = NeoBundle.message(
    ...
    private fun syncHeader(q: Question) {
        ...
        summary.text = NeoBundle.message("session.question.summary", shown, total)
    ...
    private fun syncFooter(q: Question) {
        ...
        actions.add(BaseQuestionView.Action(ID_DISMISS, NeoBundle.message("session.question.dismiss"), primary = false) { doReject() })

        if (review(q)) {
            actions.add(BaseQuestionView.Action(ID_BACK, NeoBundle.message("session.question.back"), primary = false) { goBack() })
            actions.add(BaseQuestionView.Action(ID_MAIN, NeoBundle.message("session.question.submit"), primary = true) { doReply() })
        } else {
            val label = when {
                direct(q) -> NeoBundle.message("session.question.submit")
                lastItem(q) -> NeoBundle.message("session.question.review")
                else -> NeoBundle.message("session.question.next")
            }
    ...
    private fun reviewRow(item: QuestionItem, i: Int): JPanel {
        ...
        val answer = text(
            joined.ifBlank { NeoBundle.message("session.question.review.notAnswered") },
            UiStyle.Colors.fg(),
            true,
        )
    ...
    private fun customRow(item: QuestionItem, set: MutableSet<String>): JPanel {
        ...
        val label = text(NeoBundle.message("session.question.custom.label"), UiStyle.Colors.fg(), true)
    ...
    private fun buildCustomEditor(): SessionEditorTextField {
        ...
        ed.setPlaceholder(NeoBundle.message("session.question.custom.placeholder"))
