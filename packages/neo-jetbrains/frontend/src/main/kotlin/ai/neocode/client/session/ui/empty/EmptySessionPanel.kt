...
class EmptySessionPanel(
    ...
    init {
        ...
        val logo = JBLabel(
            IconLoader.getIcon("/icons/neo-content.svg", EmptySessionPanel::class.java),
        ).apply {
            horizontalAlignment = JBLabel.CENTER
        }
    ...
    internal fun feedbackText() = NeoBundle.message("feedback.button")
    ...
    internal fun explanationText() = NeoBundle.message("session.empty.welcome")
    ...
    internal open class ShowHistoryButton(
        text: String = NeoBundle.message("session.showHistory"),
    ...
    private fun welcomeHtml() = XmlStringUtil.wrapInHtml(
        "<div style='text-align:center'>${XmlStringUtil.escapeString(NeoBundle.message("session.empty.welcome"))}</div>"
    )
