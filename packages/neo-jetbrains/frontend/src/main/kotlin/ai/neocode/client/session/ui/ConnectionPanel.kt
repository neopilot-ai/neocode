...
import ai.neocode.client.actions.NeoActionPlaces
...
import ai.neocode.client.session.controller.SessionControllerListener
...
class ConnectionPanel(
    ...
    companion object {
        internal const val CLI_GROUP_ID = "Neo.CliGroup"
    ...
    private val retry = ActionLink(NeoBundle.message("session.connection.retry")) {
        showRecoveryPopup()
    }.apply {
        isVisible = false
    ...
    private fun showConnecting() {
        ...
        label.text = NeoBundle.message("session.connection.connecting")
        detail = null
        expanded = false
    ...
    private fun showDownloading(percent: Int, version: String?, platform: String?) {
        ...
        label.text = if (version != null && platform != null) {
            NeoBundle.message("session.connection.downloading.version", version, platform, pct)
        } else {
            NeoBundle.message("session.connection.downloading", pct)
        }
        detail = null
        expanded = false
        toggle.isVisible = false
    ...
    private fun showRecoveryPopup() {
        JBPopupFactory.getInstance()
            .createActionGroupPopup(
                ...
                true,
                NeoActionPlaces.connectionRetryPopup(),
            )
    ...
    private fun recoveryGroup(): ActionGroup {
        ...
        group.add(object : DumbAwareAction(NeoBundle.message("session.connection.retry")) {
            override fun actionPerformed(e: AnActionEvent) {
                controller.retryConnection()
            }
        })
        group.addSeparator()
        ActionManager.getInstance().getAction("Neo.Restart")?.let { group.add(it) }
        ActionManager.getInstance().getAction("Neo.Reinstall")?.let { group.add(it) }
        return group
    }
