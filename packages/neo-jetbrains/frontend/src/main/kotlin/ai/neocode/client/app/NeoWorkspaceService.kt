...
class NeoWorkspaceService internal constructor(
    private val cs: CoroutineScope,
    private val rpc: NeoWorkspaceRpcApi?,
) {
    /** Platform constructor — resolves RPC from the service container. */
    constructor(cs: CoroutineScope) : this(cs, null)

    companion object {
        private val LOG = NeoLog.create(NeoWorkspaceService::class.java)
        private val INIT = NeoWorkspaceStateDto(NeoWorkspaceStatusDto.PENDING)
    }
    ...
    private suspend fun <T> call(block: suspend NeoWorkspaceRpcApi.() -> T): T {
        val api = rpc
        return if (api != null) block(api) else durable { block(NeoWorkspaceRpcApi.getInstance()) }
    }

    private fun <T> stream(block: suspend NeoWorkspaceRpcApi.() -> Flow<T>): Flow<T> = flow {
        val api = rpc
        if (api != null) block(api).collect { emit(it) }
        else durable { block(NeoWorkspaceRpcApi.getInstance()).collect { emit(it) } }
    }
