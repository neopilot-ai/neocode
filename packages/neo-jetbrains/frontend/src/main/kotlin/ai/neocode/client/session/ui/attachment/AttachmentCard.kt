...
open class AttachmentCard(
    ...
    private val action = remove?.let { callback ->
        CloseButton().apply {
            isVisible = false
            toolTipText = NeoBundle.message("prompt.attachment.remove", item.name)
    ...
    init {
        ...
        toolTipText = tip
        accessibleContext?.accessibleName = NeoBundle.message("prompt.attachment.open", item.name)
        add(content)
        if (action != null) {
            add(action)
...
private fun tooltip(item: AttachmentCardItem): String = XmlStringUtil.wrapInHtml(
    StringUtil.escapeXmlEntities(
        NeoBundle.message("prompt.attachment.tooltip", item.name, item.mime, location(item)),
    ).replace("\n", "<br>"),
)
...
private fun location(item: AttachmentCardItem): String {
    ...
    if (uri?.scheme == "data") return NeoBundle.message("prompt.attachment.embedded")
