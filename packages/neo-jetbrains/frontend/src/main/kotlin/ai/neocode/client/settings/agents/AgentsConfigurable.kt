...
import ai.neocode.client.settings.base.SettingsDraftPage
import ai.neocode.client.settings.base.SettingsDraftState
...
class AgentsConfigurable : AgentBehaviorConfigurableBase<JComponent>() {
    ...
    override fun getDisplayName(): String = NeoBundle.message("settings.agentBehavior.agents.displayName")
    ...
    companion object { const val ID = "ai.neocode.jetbrains.settings.agentBehavior.agents" }
}
...
internal class AgentsSettingsUi(
    ...
    private val app get() = service<NeoAppService>()
    ...
    override suspend fun fetch(): List<SettingsListItem> {
        val agents = service<NeoAgentBehaviorService>().agents(dir)
        models = items(service<NeoWorkspaceService>().models(dir).providers)
        val next = agentsDraft(service<NeoAppService>().state.value.config, agents)
    ...
    override fun toolbarRight(): JComponent = Stack.horizontal(UiStyle.Gap.sm())
        .next(JBLabel(NeoBundle.message("settings.agentBehavior.agents.default")))
        .next(makePicker())
    ...
    override fun searchPlaceholder() = NeoBundle.message("settings.agentBehavior.agents.search")
    ...
    override fun applyDraft() {
        ...
        if (!launch("apply") { id ->
            ...
            for (name in target.deleted) {
                if (!behavior.removeAgent(dir, name)) {
                    failed = NeoBundle.message("settings.agentBehavior.agents.delete.failed")
                    break
                }
            ...
            if (failed == null) {
                for ((name, input) in target.created) {
                    if (!behavior.createAgent(dir, input)) {
                        failed = NeoBundle.message("settings.agentBehavior.agents.create.failed")
                        break
                    }
            ...
            if (failed == null) {
                for ((name, item) in target.imported) {
                    if (app.updateConfig(item) == null) {
                        failed = NeoBundle.message("settings.agentBehavior.agents.import.failed")
                        break
                    }
            ...
            if (failed == null) {
                ...
                if (change != null && app.updateConfig(change) == null) {
                    failed = NeoBundle.message("settings.agentBehavior.save.failed")
                }
            ...
            withContext(edt) {
                if (!active(id)) {
                    if (failed != null) NeoNotifications.error(failed)
                    return@withContext
                }
                details = agents
                if (failed != null) {
        ...
        showProgress(NeoBundle.message("settings.agentBehavior.saving"))
    }
    ...
    private fun rows(): List<SettingsListItem> = displayRows(base, draft).map { row ->
        ...
        object : SettingsListItem {
            ...
            override val badges = listOfNotNull(
                SettingsBadge(
                    NeoBundle.message("settings.agentBehavior.badge.notApplied"),
                    UiStyle.Badge.Secondary,
                ).takeIf { row.intent == AgentIntent.Modified || row.intent == AgentIntent.New },
                SettingsBadge(
                    NeoBundle.message("settings.agentBehavior.badge.willRemove"),
                    UiStyle.Badge.Alert,
                ...
                ).takeIf { NeoCliParser.isSubagent(item.mode) },
                SettingsBadge(
                    NeoBundle.message("settings.agentBehavior.badge.custom"),
                    UiStyle.Badge.Primary,
                ).takeIf { canDelete(item) },
                SettingsBadge(NeoBundle.message("settings.agentBehavior.badge.hidden")).takeIf { item.hidden },
                SettingsBadge(NeoBundle.message("settings.agentBehavior.badge.disabled")).takeIf { item.disable },
                SettingsBadge(
            ...
            override val cells = when (row.intent) {
                ...
                -> listOf(SettingsListCell(UNDO_CELL, NeoBundle.message("settings.agentBehavior.undo"), primary = true))
                AgentIntent.Unchanged,
                AgentIntent.Modified,
                -> listOfNotNull(
                    SettingsListCell(EDIT_CELL, NeoBundle.message("settings.agentBehavior.edit")),
                    SettingsListCell(
                    DELETE_CELL,
    ...
    private fun syncNames() {
        names = listOf("") + displayRows(base, draft)
            ...
            .map { it.agent }
            .filter { NeoCliParser.defaultAgentCandidate(it.mode, it.hidden) && !it.disable }
            .map { it.name }
    }
    ...
    private fun addAction(): DefaultActionGroup = DefaultActionGroup(
        NeoBundle.message("settings.agentBehavior.agents.add"),
        true,
    ).apply {
    ...
    internal inner class CreateAction : DumbAwareAction(NeoBundle.message("settings.agentBehavior.agents.create")) {
    ...
    internal inner class ImportAction : DumbAwareAction(NeoBundle.message("settings.agentBehavior.agents.import")) {
        override fun getActionUpdateThread() = ActionUpdateThread.EDT
...
private fun chooseImportFile(_parent: JComponent): VirtualFile? {
    val descriptor = FileChooserDescriptor(true, false, false, false, false, false).apply {
        title = NeoBundle.message("settings.agentBehavior.agents.import.title")
        description = NeoBundle.message("settings.agentBehavior.agents.import.description")
