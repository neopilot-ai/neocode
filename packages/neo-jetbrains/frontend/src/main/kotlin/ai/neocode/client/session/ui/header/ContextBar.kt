...
private data class ContextData(
    ...
    fun tip(): String {
        val lines = mutableListOf(NeoBundle.message("session.header.context.used", num(used), num(limit)))
        if (output > 0) lines.add(NeoBundle.message("session.header.context.reserved", num(output)))
        if (available > 0) lines.add(NeoBundle.message("session.header.context.available", num(available)))
