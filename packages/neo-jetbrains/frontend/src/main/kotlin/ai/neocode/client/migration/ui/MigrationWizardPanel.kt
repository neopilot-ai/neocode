...
import ai.neocode.client.migration.MigrationItemUiProgress
import ai.neocode.client.migration.MigrationSelectionBuilder
import ai.neocode.client.migration.MigrationSettingsUiSelections
import ai.neocode.client.migration.MigrationUiPhase
import ai.neocode.client.migration.MigrationUiSelections
import ai.neocode.client.migration.MigrationUiState
import ai.neocode.client.migration.groupStatus
...
class MigrationWizardPanel : JPanel(BorderLayout()) {
    ...
    private val settingsRow = MigrationItemRow(NeoBundle.message("migration.row.settings"), MigrationItemCategoryDto.settings)
    private val providerRow = MigrationItemRow(NeoBundle.message("migration.row.providers"), MigrationItemCategoryDto.provider)
    private val mcpRow = MigrationItemRow(NeoBundle.message("migration.row.mcp"), MigrationItemCategoryDto.mcpServer)
    private val modesRow = MigrationItemRow(NeoBundle.message("migration.row.modes"), MigrationItemCategoryDto.customMode)
    private val sessionsRow = MigrationItemRow(NeoBundle.message("migration.row.sessions"), MigrationItemCategoryDto.session)
    private val modelRow = MigrationItemRow(NeoBundle.message("migration.row.model"), MigrationItemCategoryDto.defaultModel)

    private val question = BaseQuestionView()
    private val keepBox = JBCheckBox(NeoBundle.message("migration.keep_legacy_settings"), true)

    private val emptyLabel = JBLabel(NeoBundle.message("migration.empty")).apply {
    ...
    init {
        ...
        question.setHeader(
            NeoBundle.message("migration.migrate.title"),
            NeoBundle.message("migration.migrate.subtitle"),
        )
        question.setContent(buildContent())
        question.setActions(
            listOf(
                BaseQuestionView.Action(ACTION_SKIP, NeoBundle.message("migration.button.skip"), primary = false) {
                    onSkip?.invoke()
                },
                BaseQuestionView.Action(ACTION_LATER, NeoBundle.message("migration.button.later"), primary = false) {
                    onLater?.invoke()
                },
                BaseQuestionView.Action(ACTION_MIGRATE, NeoBundle.message("migration.button.migrate"), primary = true) {
                    onStart?.invoke(currentSelections())
                },
                BaseQuestionView.Action(ACTION_DONE, NeoBundle.message("migration.button.done"), primary = true) {
                    onDone?.invoke()
                },
                BaseQuestionView.Action(ACTION_CONTINUE, NeoBundle.message("migration.button.continue"), primary = true) {
    ...
    private fun updateButtons(phase: MigrationUiPhase, running: Boolean) {
        ...
        question.setActionText(
            ACTION_MIGRATE,
            if (running) NeoBundle.message("migration.button.migrating") else NeoBundle.message("migration.button.migrate"),
        )
