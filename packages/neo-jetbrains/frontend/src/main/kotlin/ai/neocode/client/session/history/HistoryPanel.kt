...
class HistoryPanel(
    ...
    private val repoOnly = JBCheckBox(NeoBundle.message("history.cloud.repo.only"), true).apply {
        isVisible = false
    ...
    private val localInfo = TabInfo(localPanel)
        .setText(NeoBundle.message("history.tab.local"))
        .setForeSideComponent(back())
    private val cloudInfo = TabInfo(cloudPanel)
        .setText(NeoBundle.message("history.tab.cloud"))
        .setForeSideComponent(back())
    ...
    private fun search(model: HistoryModel<out HistoryItem>) = SearchTextField(false).apply {
        textEditor.emptyText.text = NeoBundle.message("history.search.placeholder")
    ...
    private fun back(): JComponent {
        val label = NeoBundle.message("history.back")
    ...
    private fun localList() = HistoryList(controller.local).apply {
        ...
        cursor = Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)
        emptyText.text = NeoBundle.message("history.empty")
    ...
    private fun <T : HistoryItem> syncList(list: JBList<T>, model: HistoryModel<T>) {
        ...
        list.emptyText.text = when {
            model.loading -> NeoBundle.message("history.loading")
            model.error != null -> model.error.orEmpty()
            else -> NeoBundle.message("history.empty")
        }
    }
    ...
    private fun installContextMenu(list: JBList<out HistoryItem>) {
        val group = ActionManager.getInstance().getAction("Neo.History.ContextMenu")
    ...
    private fun confirm(item: LocalHistoryItem) {
        ...
        val result = com.intellij.openapi.ui.Messages.showYesNoDialog(
            this,
            NeoBundle.message("history.delete.confirm.message", title(item)),
    ...
    internal fun confirmDelete(items: List<LocalHistoryItem>) {
        ...
        val msg = if (active.size == 1)
            NeoBundle.message("history.delete.confirm.message", title(active[0]))
        else
            NeoBundle.message("history.delete.confirm.message.multiple", active.size)
        val result = com.intellij.openapi.ui.Messages.showYesNoDialog(
            this,
            msg,
            NeoBundle.message("history.delete.confirm.title"),
    ...
    private class LoadMoreButton : JButton(NeoBundle.message("history.cloud.load.more")) {
