package ai.neocode.client.session.model
 ...
 *
 * **EDT-only access** — no synchronization. [ai.neocode.client.session.controller.SessionController] guarantees all
...
class SessionModel {
    ...
    var app: NeoAppStateDto = NeoAppStateDto(NeoAppStatusDto.DISCONNECTED)
    var version: String? = null

    var workspace: NeoWorkspaceStateDto = NeoWorkspaceStateDto(NeoWorkspaceStatusDto.PENDING)
    ...
    @RequiresEdt
    fun isReady(): Boolean = app.status == NeoAppStatusDto.READY && workspace.status == NeoWorkspaceStatusDto.READY
...
private fun ai.neocode.rpc.dto.PartTimeDto.durationMs(): Long? {
