...
import ai.neocode.client.session.ui.selection.SessionCopyButton
import ai.neocode.client.ui.ToolbarButtonAction
import ai.neocode.client.ui.UiStyle
import ai.neocode.client.ui.layout.Stack
import ai.neocode.client.ui.toolbarButton
...
internal class MessageToolbar(
    ...
    tooltip: String = NeoBundle.message("session.copy.hover"),
) : JPanel(FlowLayout(FlowLayout.LEFT, 0, 0)) {
    constructor(text: () -> String?, revert: (() -> Unit)?) : this(
        text,
        revert?.let {
            listOf(ToolbarButtonAction(AllIcons.Actions.Rollback, NeoBundle.message("revert.message.rollback"), it))
        }.orEmpty(),
        NeoBundle.message("session.copy.prompt"),
    )
