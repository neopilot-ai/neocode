...
internal data class AgentEditDraft(
    ...
    val mode: String = NeoCliParser.MODE_PRIMARY,
    val defaultMode: String = NeoCliParser.MODE_PRIMARY,
...
internal fun agentsDraft(config: ConfigDto?, details: List<AgentDetailDto>): AgentsDraft {
    ...
    val agent = config?.defaultAgent?.takeIf { name ->
        val item = items[name]
        item != null && NeoCliParser.defaultAgentCandidate(item.mode, item.hidden) && !item.disable
    }
...
internal fun updateAgent(draft: AgentsDraft, agent: AgentEditDraft): AgentsDraft {
    ...
    val clear = !restricted && (agent.hidden || agent.disable || NeoCliParser.isSubagent(agent.mode))
...
internal fun patch(from: AgentsDraft, to: AgentsDraft): ConfigPatchDto? {
    ...
    if (from.defaultAgent != to.defaultAgent) values[NeoCliParser.CONFIG_DEFAULT_AGENT] = to.defaultAgent
    ...
    val imported = draft.imported.map { (name, patch) ->
        ...
        AgentDisplayRow(
            AgentEditDraft(
                ...
                model = cfg?.model,
                variant = cfg?.variant,
                mode = cfg?.mode ?: NeoCliParser.MODE_PRIMARY,
                defaultMode = cfg?.mode ?: NeoCliParser.MODE_PRIMARY,
