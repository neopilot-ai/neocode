...
import ai.neocode.client.session.model.AgentItem
import ai.neocode.client.session.model.ModelLimitItem
import ai.neocode.client.session.model.ModelItem
import ai.neocode.client.session.model.SessionModel
...
import ai.neocode.client.session.model.PermissionMeta
import ai.neocode.client.session.model.PermissionRequestState
import ai.neocode.client.session.model.Question
import ai.neocode.client.session.model.QuestionItem
import ai.neocode.client.session.model.QuestionOption
import ai.neocode.client.session.model.Reasoning
import ai.neocode.client.session.model.ToolCallRef
import ai.neocode.client.session.model.Text
import ai.neocode.client.plugin.NeoPluginSettings
import ai.neocode.client.session.SessionRef
import ai.neocode.client.telemetry.Telemetry
import ai.neocode.client.util.UiTimer
...
import ai.neocode.rpc.dto.MessageDto
...
class SessionController(
  parent: Disposable,
  ref: SessionRef? = null,
  private val sessions: NeoSessionService,
  private val workspace: Workspace,
  private val app: NeoAppService,
  ...
  private val log: NeoLog = LOG,
    ...
    companion object {
        private val LOG = NeoLog.create(SessionController::class.java)
    ...
    val autoApprove: Boolean get() = NeoPluginSettings.getAutoApprove()
        ...
        cs.launch {
            try {
                ...
                edt {
                    ...
                    val msg = e.message ?: NeoBundle.message("session.error.prompt")
                    updateModel {
    ...
    fun setAutoApprove(value: Boolean) {
        assertEdt()
        NeoPluginSettings.setAutoApprove(value)
    ...
    fun compact() {
        ...
        cs.launch {
            try {
                ...
                edt {
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: NeoBundle.message("session.error.compact")))
                    }
                }
            }
        }
    ...
    fun revert(message: String, part: String? = null) {
        ...
        val op = beginReverting(
            NeoBundle.message("session.status.rollingback"),
    ...
    fun unrevert() {
        ...
        if (revertOp != null) return
        val op = beginReverting(
            NeoBundle.message("session.status.redoing"),
    ...
    fun cancelRevert() {
        ...
        if (state is SessionState.Reverting) {
            model.setState(state.copy(text = NeoBundle.message("session.status.operation.finishing")))
        }
        revertJob?.cancel()
    }
    ...
    fun retryConnection() {
        ...
        if (model.app.status != NeoAppStatusDto.READY || model.app.status == NeoAppStatusDto.ERROR) {
            app.retryAsync()
            return
        }
        ...
        if (model.workspace.status == NeoWorkspaceStatusDto.ERROR) {
            workspace.reload()
        }
    }
            ...
            try {
                ...
                edt {
                    updatePermission(
                        ...
                        e.message ?: NeoBundle.message("session.permission.error"),
                    )
                }
            }
        }
    ...
    private fun approve(id: String, restore: () -> Permission) {
        ...
        cs.launch {
            try {
                ...
                edt {
                    if (disposed) return@edt
                    model.setState(SessionState.Busy(NeoBundle.message("session.status.considering")))
                }
                ...
                edt {
                    ...
                    model.setState(SessionState.AwaitingPermission(restore().copy(
                        ...
                        message = e.message ?: NeoBundle.message("session.permission.error"),
                    )))
                }
            }
        }
    ...
    init {
        ...
        cs.launch {
            app.state.collect { state ->
                if (state.status == NeoAppStatusDto.READY) app.fetchVersionAsync()
        ...
        cs.launch {
            workspace.state.collect { state ->
                fire(SessionControllerEvent.WorkspaceChanged) {
                    ...
                    if (state.status != NeoWorkspaceStatusDto.READY) return@fire
                    ...
                    model.models = state.providers?.let { providers ->
                        providers.providers
                            .filter { it.id == NEO_PROVIDER || it.id in providers.connected }
                            .flatMap { provider ->
                ...
                if (state.status == NeoWorkspaceStatusDto.READY) {
                    fire(SessionControllerEvent.WorkspaceReady)
                    edt {
        ...
        cs.launch {
            try {
                ...
                edt {
                    ...
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: NeoBundle.message("history.error.local")))
                    }
                    showSession()
                    loaded(false)
                }
        ...
        cs.launch {
            try {
                ...
                edt {
                    if (disposed) return@edt
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: NeoBundle.message("history.error.cloud")))
                    }
                    showSession()
                    loaded(false)
                }
    ...
    private fun seedStatus(dto: SessionStatusDto) {
        ...
        val state = when (dto.type) {
            "busy" -> SessionState.Busy(NeoBundle.message("session.status.considering"))
    ...
    private fun handle(event: ChatEventDto) {
        ...
        when (event) {
            ...
            is ChatEventDto.TurnOpen -> {
                ...
                tool = null
                if (revertOp != null) {
                    revertDeferred = SessionState.Busy(NeoBundle.message("session.status.considering"))
                    return
                }
                model.setState(SessionState.Busy(NeoBundle.message("session.status.considering")))
            }
    ...
    private fun error(event: ChatEventDto.Error, reveal: Boolean) {
        ...
        if (isPaidModelAuthRequired(event.error)) {
            ...
            model.setState(SessionState.LoginRequired(NeoBundle.message("session.login.required.description")))
            return
        }
        val msg = event.error?.message ?: event.error?.type ?: NeoBundle.message("session.error.unknown")
        ...
        val state = when (dto.type) {
            ...
            "busy" -> {
                ...
                if (current is SessionState.Idle || current is SessionState.Error)
                    SessionState.Busy(NeoBundle.message("session.status.considering"))
    ...
    private fun onRevertTimeout(op: RevertOp) {
        ...
        revertJob?.cancel()
        failReverting(op, RuntimeException(NeoBundle.message("session.error.revert.timeout")))
    }
    ...
    private fun failReverting(op: RevertOp, e: Exception) {
        ...
        revertDeferred = null
        model.setState(SessionState.Error(e.message ?: NeoBundle.message("session.error.unknown")))
    }
    ...
    private fun configModel(agent: String): String? {
        if (model.app.status != NeoAppStatusDto.READY) return null
        val cfg = model.app.config
    ...
    private fun status(): String = when (partType) {
        "reasoning" -> NeoBundle.message("session.status.thinking")
        "text" -> NeoBundle.message("session.status.writing")
        "tool" -> when (tool) {
            "task" -> NeoBundle.message("session.status.delegating")
            "todowrite", "todoread" -> NeoBundle.message("session.status.planning")
            "read" -> NeoBundle.message("session.status.gathering")
            "glob", "grep", "list" -> NeoBundle.message("session.status.searching.codebase")
            "webfetch", "websearch", "codesearch" -> NeoBundle.message("session.status.searching.web")
            "edit", "write" -> NeoBundle.message("session.status.editing")
            "bash" -> NeoBundle.message("session.status.commands")
            else -> NeoBundle.message("session.status.considering")
        }
        else -> NeoBundle.message("session.status.considering")
    }
    ...
    private fun resolveConnectionState(): SessionControllerEvent.ConnectionChanged {
        ...
        if (app.status == NeoAppStatusDto.ERROR) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                NeoBundle.message("session.connection.error.app"),
                app.errors.toErrorText() ?: app.error,
            )
        }

        if (app.status == NeoAppStatusDto.DOWNLOADING) {
        ...
        if (workspace.status == NeoWorkspaceStatusDto.ERROR) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                NeoBundle.message("session.connection.error.workspace"),
                workspace.errors.toErrorText() ?: workspace.error,
                "workspace",
            )
        }

        if (app.status == NeoAppStatusDto.READY && workspace.status == NeoWorkspaceStatusDto.READY && app.warnings.isNotEmpty()) {
        ...
        if (app.status == NeoAppStatusDto.READY && workspace.status == NeoWorkspaceStatusDto.READY) {
...
private fun summary(count: Int): String {
    val base = NeoBundle.message("session.connection.warning.config")
...
private const val NEO_AUTO_MODEL = "neo-auto/free"
...
private fun resolveModelSelection(
    ...
    fallback: ModelSelectionDto? = ModelSelectionDto(NEO_PROVIDER, NEO_AUTO_MODEL),
): ModelSelectionDto? {
...
private fun valid(providers: ProvidersDto?, item: ModelSelectionDto?): ModelSelectionDto? {
    ...
    if (item.providerID != NEO_PROVIDER && item.providerID !in providers.connected) return null
