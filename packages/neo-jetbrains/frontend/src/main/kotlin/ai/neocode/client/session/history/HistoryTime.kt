...
internal object HistoryTime {
    ...
    fun title(section: HistorySection): String = when (section) {
        HistorySection.TODAY -> NeoBundle.message("history.group.today")
        HistorySection.YESTERDAY -> NeoBundle.message("history.group.yesterday")
        HistorySection.WEEK -> NeoBundle.message("history.group.week")
        HistorySection.MONTH -> NeoBundle.message("history.group.month")
        HistorySection.OLDER -> NeoBundle.message("history.group.older")
    }
    ...
    fun relative(item: HistoryItem, now: Long = System.currentTimeMillis()): String {
        ...
        if (diff < MINUTE) return NeoBundle.message("history.time.moments")
        if (diff < HOUR) return NeoBundle.message("history.time.minutes", (diff / MINUTE).coerceAtLeast(1))
        if (diff < DAY) return NeoBundle.message("history.time.hours", (diff / HOUR).coerceAtLeast(1))
        if (diff < 7 * DAY) return NeoBundle.message("history.time.days", (diff / DAY).coerceAtLeast(1))
        ...
        if (months < 1) return NeoBundle.message("history.time.days", (diff / DAY).coerceAtLeast(1))
        if (months < 12) return NeoBundle.message("history.time.months", months)
        return NeoBundle.message("history.time.years", months / 12)
    }
