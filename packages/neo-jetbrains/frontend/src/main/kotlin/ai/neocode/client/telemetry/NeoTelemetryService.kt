...
package ai.neocode.client.telemetry
...
object Telemetry {
    fun send(event: String, properties: Map<String, String> = emptyMap()) {
        NeoTelemetryService.getInstance().send(event, properties)
    }
}

@Service(Service.Level.APP)
class NeoTelemetryService internal constructor(
    ...
    companion object {
        private val LOG = NeoLog.create(NeoTelemetryService::class.java)
        private const val MAX_PENDING = 64
        private const val TIMEOUT_MS = 5_000L

        fun getInstance(): NeoTelemetryService = service()
    }
    ...
    fun send(event: String, properties: Map<String, String> = emptyMap()) {
        ...
        cs.launch {
            try {
                if (NeoLog.sandbox()) {
                    val payload = NeoLog.payload(LOG) + properties
                ...
                val sent = withTimeoutOrNull(TIMEOUT_MS) {
                    ...
                    else durable { NeoAppRpcApi.getInstance().captureTelemetry(dto) }
                    true
                }
