...
internal fun modelPickerRows(
    ...
    emptyText: String = NeoBundle.message("settings.models.notSet"),
    ...
    if (q.isBlank()) {
        ...
        if (fav.isNotEmpty()) {
            val section = NeoBundle.message("model.picker.favorites")
            out += fav.map { ModelPickerRow(it, section, favorite = true) }
        }
    }
    if (recommended.isNotEmpty()) {
        val section = NeoBundle.message("model.picker.recommended")
