...
class BaseSettingsUiWorkspaceTest : BasePlatformTestCase() {
    ...
    fun `test model state updates are delivered on edt`() {
        rpc.models = ModelStateDto(favorite = listOf(ModelSelectionDto("neo", "new")))
        rpc.state.value = state("ready")
        val view = create("/test")
    ...
    private fun state(model: String) = NeoAppStateDto(
    ...
    private class FakePanel(
        cs: CoroutineScope,
        app: NeoAppService,
        workspaces: NeoWorkspaceService,
        hint: String,
        ...
        override fun draft(state: NeoAppStateDto): Draft = Draft(state.config?.model ?: "none")
