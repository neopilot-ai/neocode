...
import ai.neocode.client.testing.FakeMigrationRpcApi
...
class NeoMigrationServiceTest : BasePlatformTestCase() {

    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeMigrationRpcApi
    private lateinit var service: NeoMigrationService
    private lateinit var app: MutableStateFlow<NeoAppStateDto>
    ...
    override fun setUp() {
        ...
        app = MutableStateFlow(NeoAppStateDto(NeoAppStatusDto.DISCONNECTED))
        autocomplete.clear()
        telemetry.clear()
        service = NeoMigrationService(scope, rpc, app, { autocomplete.add(it) }) { event, props -> telemetry.add(event to props) }
    }
    ...
    fun `test migration required app state shows needed without polling`() {
        app.value = NeoAppStateDto(NeoAppStatusDto.MIGRATION_REQUIRED, migration = sampleDetection())
        settle()
    ...
    fun `test ready app state hides migration`() {
        app.value = NeoAppStateDto(NeoAppStatusDto.MIGRATION_REQUIRED, migration = sampleDetection())
        settle()
        app.value = NeoAppStateDto(NeoAppStatusDto.READY)
        settle()
    ...
    fun `test duplicate migration required does not reset running migration`() {
        ...
        app.value = NeoAppStateDto(NeoAppStatusDto.MIGRATION_REQUIRED, migration = detection)
        settle()
    ...
    fun `test complete event without items finalizes pending session progress`() = runBlocking {
        app.value = NeoAppStateDto(NeoAppStatusDto.MIGRATION_REQUIRED, migration = sampleDetection().copy(
