...
import ai.neocode.client.settings.base.SettingsListRenderer
import ai.neocode.client.settings.base.SettingsListActionCell
import ai.neocode.client.settings.base.settingsListCellAt
import ai.neocode.client.settings.base.settingsListCellBounds
import ai.neocode.client.settings.base.settingsListSectionTitle
import ai.neocode.client.settings.base.settingsListVisibleCells
import ai.neocode.client.testing.FakeProviderRpcApi
import ai.neocode.client.ui.UiStyle
import ai.neocode.rpc.dto.CustomModelFetchResultDto
import ai.neocode.rpc.dto.CustomProviderConfigDto
...
import ai.neocode.rpc.dto.ProviderMetadataDto
...
class ProvidersSettingsUiTest : BasePlatformTestCase() {
    ...
    fun `test custom save error reports dropped provider`() {
        ...
        assertEquals(NeoBundle.message("settings.providers.customNotUsable"), customSaveError("my-openai", result))
    }
    ...
    fun `test custom dialog cancels model fetch and ignores late result`() {
        ...
        edt {
            assertEquals(NeoBundle.message("settings.providers.customFetchingModels"), field.text)
            assertEquals(NeoBundle.message("settings.providers.customCancelModels"), pick.text)
            ...
            pick.doClick()
            assertEquals("", field.text)
            assertEquals(NeoBundle.message("settings.providers.customSelectModels"), pick.text)
    ...
    fun `test custom dialog edit prefill locks id and leaves key blank`() {
        ...
        edt {
            ...
            assertEquals(NeoBundle.message("settings.providers.customSave"), dialog.okText())
            dispose(dialog)
        }
    }
    ...
    fun `test popular rows use vscode order including neo`() {
        val rows = providerListRows(
            ProviderSettingsDto(
                providers = listOf(
                    provider("openrouter", "OpenRouter", priority = 5),
                    provider("neo", "Neo", priority = 0),
        ...
        assertEquals(listOf("neo", "anthropic", "deepseek", "openai", "google", "openrouter", "vercel"), rows.map { it.key })
    ...
    fun `test connected neo gateway has no provider settings actions`() {
        val rows = providerListRows(
            ProviderSettingsDto(
                providers = listOf(provider("neo", "Neo Gateway")),
                connected = listOf("neo"),
            ),
            "",
        )

        assertEquals(listOf("neo"), rows.map { it.key })
        assertEquals("Connected providers", settingsListSectionTitle(rows, 0))
    ...
    fun `test provider oauth waiting countdown can be cancelled without refresh`() {
        val callback = CompletableDeferred<ai.neocode.rpc.dto.ProviderActionResultDto>()
        val rpc = installProvider(
            ProviderSettingsDto(
                providers = listOf(provider("github-copilot", "GitHub Copilot")),
                auth = mapOf("github-copilot" to listOf(ProviderAuthMethodDto("oauth", "OAuth"))),
        ...
        callback.complete(ai.neocode.rpc.dto.ProviderActionResultDto(providerState(provider("stale", "Stale"))))
    ...
    fun `test provider oauth auto response shows device auth panel`() {
        ...
        edt {
            ...
            assertEquals("https://auth.openai.com/device", fieldsByName(panel, "neo.provider.oauth.url").single().text)
            val qr = components(panel).filterIsInstance<JBLabel>().single { it.name == "neo.provider.oauth.qr" }
    ...
    fun `test provider action failure returns error state`() = runBlocking {
        ...
        rpc.disconnectError = IllegalStateException("Neo backend is not ready")
        val service = NeoProviderService(cs, rpc)

        val result = withContext(kotlinx.coroutines.Dispatchers.Default) {
            service.disconnect(ProviderDisconnectDto("/test", "openai"))
        }

        assertEquals("Neo backend is not ready", result.error)
    ...
    private fun installProvider(state: ProviderSettingsDto): FakeProviderRpcApi {
        ...
        ApplicationManager.getApplication().replaceService(
            NeoProviderService::class.java,
            NeoProviderService(cs, rpc),
            testRootDisposable,
        )
        return rpc
    }
