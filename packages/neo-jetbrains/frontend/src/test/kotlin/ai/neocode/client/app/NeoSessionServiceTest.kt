...
class NeoSessionServiceTest : BasePlatformTestCase() {
    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeSessionRpcApi
    private lateinit var service: NeoSessionService

    override fun setUp() {
        super.setUp()
        scope = CoroutineScope(SupervisorJob())
        rpc = FakeSessionRpcApi()
        service = NeoSessionService(project, scope, rpc)
    }
    ...
    fun `test events logs normal completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        service = NeoSessionService(project, scope, rpc, log)
