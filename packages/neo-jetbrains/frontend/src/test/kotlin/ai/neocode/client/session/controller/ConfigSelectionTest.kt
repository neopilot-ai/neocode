...
class ConfigSelectionTest : SessionControllerTestBase() {
        ...
        projectRpc.state.value = workspaceReady(
            providers = listOf(
                ProviderDto(
                    id = "neo",
                    name = "Neo",
                    models = mapOf("neo-auto/free" to ModelDto(id = "neo-auto/free", name = "Auto")),
                ),
                ProviderDto(
                    id = "anthropic",
                    name = "Anthropic",
            ...
            connected = listOf("neo", "anthropic", "openai"),
            defaults = emptyMap(),
        )
        val m = controller()
        collect(m)
    ...
    fun `test no valid candidates falls back to neo auto`() {
        ...
        assertEquals("neo/neo-auto/free", m.model.defaultModel)
        assertEquals("neo/neo-auto/free", m.model.model)
        assertFalse(m.model.modelOverride)
    }
    ...
    fun `test selectAgent uses saved model for selected agent`() {
        ...
        projectRpc.state.value = workspaceReady(
            ...
            connected = listOf("neo", "openai"),
            defaults = mapOf("code" to "neo/gpt-5", "plan" to "neo/gpt-5"),
        )
        val m = controller()
        collect(m)
        flush()
    ...
    fun `test selectVariant persists current model variant`() {
        ...
        assertEquals("high", m.model.variant)
        assertEquals("neo/gpt-5", appRpc.variants.single().key)
