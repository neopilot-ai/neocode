...
import ai.neocode.client.session.views.AttachmentView
import ai.neocode.client.session.views.PromptAttachmentView
import ai.neocode.client.session.views.tool.ReadToolView
import ai.neocode.client.session.views.TextView
import ai.neocode.client.plugin.NeoBundle
import ai.neocode.client.session.views.tool.ShellToolView
...
class SessionUiUpdateTest : BasePlatformTestCase() {
    ...
    fun `test read tool renders as ReadToolView`() {
        ...
        assertTrue(tv is ai.neocode.client.session.views.tool.ReadToolView)
    }
    ...
    fun `test glob tool renders as GlobToolView`() {
        ...
        val tv = panel.findMessage("a1")!!.part("t1")
        assertTrue(tv is ai.neocode.client.session.views.tool.GlobToolView)
    }
    ...
    fun `test grep tool renders as SearchToolView`() {
        ...
        val tv = panel.findMessage("a1")!!.part("t1")
        assertTrue(tv is ai.neocode.client.session.views.tool.SearchToolView)
    }
    ...
    fun `test compaction part appears in panel`() {
        ...
        assertTrue(mv.part("cp1") is ai.neocode.client.session.views.CompactionView)
    }
    ...
    fun `test unknown part type falls back to GenericView`() {
        ...
        val gv = mv.part("g1")
        assertNotNull(gv)
        assertTrue(gv is ai.neocode.client.session.views.base.GenericView)
        assertTrue((gv as ai.neocode.client.session.views.base.GenericView).labelText().contains("snapshot"))
    ...
    fun `test assistant file part renders as attachment view`() {
        ...
        assertFalse(buttons(view).any { it.accessibleContext.accessibleName == NeoBundle.message("prompt.attachment.remove", "a.png") })
    }
