...
class AgentBehaviorConfigurableTest : BasePlatformTestCase() {
    ...
    fun `test id matches xml registration`() {
        ...
        assertEquals("ai.neocode.jetbrains.settings.agentBehavior", cfg.id)
    }

    fun `test child ids match xml registration`() {
        assertEquals("ai.neocode.jetbrains.settings.agentBehavior.agents", AgentsConfigurable.ID)
        assertEquals("ai.neocode.jetbrains.settings.agentBehavior.mcp", McpConfigurable.ID)
    }
