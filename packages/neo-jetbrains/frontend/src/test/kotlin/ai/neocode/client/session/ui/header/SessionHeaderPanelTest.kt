...
import ai.neocode.client.session.model.StepFinish
import ai.neocode.client.session.model.Tool
import ai.neocode.client.session.model.ToolExecState
import ai.neocode.client.session.model.ToolKind
import ai.neocode.client.session.ui.style.SessionEditorStyle
import ai.neocode.client.session.ui.style.SessionUiStyle
import ai.neocode.client.session.controller.SessionControllerTestBase
...
class SessionHeaderPanelTest : SessionControllerTestBase() {
    ...
    fun `test starts hidden for empty header`() {
        appRpc.state.value = ai.neocode.rpc.dto.NeoAppStateDto(ai.neocode.rpc.dto.NeoAppStatusDto.READY)
    ...
    fun `test timeline colors honor semantic named color keys`() {
        val old = UIManager.getColor("Neo.Session.Timeline.Read")
        val color = Color(12, 34, 56)

        try {
            UIManager.put("Neo.Session.Timeline.Read", color)

            assertEquals(color.rgb, SessionUiStyle.Timeline.READ.rgb)
        } finally {
            UIManager.put("Neo.Session.Timeline.Read", old)
        }
    }
    ...
    private fun promptedHeader(): ai.neocode.client.session.controller.SessionController {
        appRpc.state.value = ai.neocode.rpc.dto.NeoAppStateDto(
            ai.neocode.rpc.dto.NeoAppStatusDto.READY,
            config = ai.neocode.rpc.dto.ConfigDto(model = "neo/gpt-5"),
        )
        projectRpc.state.value = workspaceReady(
            providers = listOf(
                ProviderDto(
                    ...
                    models = mapOf(
                        "gpt-5" to ModelDto(
                            id = "gpt-5",
                            name = "GPT-5",
                            limit = ai.neocode.rpc.dto.ModelLimitDto(context = 2_000_000, output = 200_000),
                        ),
                    ),
                ),
            ),
