...
class AttachmentEditorKindTest : BasePlatformTestCase() {
    fun testAttachmentParamsUseStableIdentityFields() {
        ...
        val path = NeoPath(AttachmentEditorKind.ID, params).canonical()
        val json = NeoVirtualFileSystem.getInstance().getPath(path)
        val decoded = NeoVirtualFileSystem.decode(json)
    ...
    fun testSameParamsMapToSameVirtualPath() {
        ...
        val one = NeoVirtualFileSystem.getInstance().getPath(NeoPath(AttachmentEditorKind.ID, params))
        val two = NeoVirtualFileSystem.getInstance().getPath(NeoPath(AttachmentEditorKind.ID, params.toList().reversed().toMap()))

        assertEquals(one, two)
        assertFalse(one.contains("/system/neo/editors"))
        assertFalse(one.contains("neoattachment"))
    }
    ...
    fun testDuplicatePartAttachmentsMapToDistinctVirtualFiles() {
        ...
        assertFalse(NeoPath(AttachmentEditorKind.ID, one) == NeoPath(AttachmentEditorKind.ID, two))
    }

    fun testVirtualFilesAreExcludedFromEditorHistory() {
        ensureAttachmentEditorKind()
        val file = NeoVirtualFile(NeoPath(AttachmentEditorKind.ID, mapOf(
            "directory" to "/repo",
            "sessionId" to "ses1",
            "messageId" to "msg1",
            "partId" to "part1",
    ...
    fun testAttachmentEditorKindAndVirtualFilesCanBeCleared() {
        ...
        val path = NeoPath(AttachmentEditorKind.ID, mapOf(
            "directory" to "/repo",
            "sessionId" to "ses1",
            "messageId" to "msg1",
            "partId" to "part1",
        ...
        assertNotNull(file)
        assertNotNull(service<NeoEditorKindRegistry>().get(AttachmentEditorKind.ID))
        assertNotNull(service<NeoVirtualFileKindRegistry>().get(AttachmentEditorKind.ID))

        unregisterAttachmentEditorKind()
        fs.clear()

        assertNull(service<NeoEditorKindRegistry>().get(AttachmentEditorKind.ID))
        assertNull(service<NeoVirtualFileKindRegistry>().get(AttachmentEditorKind.ID))
    ...
    fun testFetchUsesAttachmentKeyBeforeDuplicatePartId() {
        ...
        app.state.value = NeoAppStateDto(NeoAppStatusDto.READY)
        val first = PartDto(
            id = "part1",
            sessionID = "ses1",
            messageID = "msg1",
        ...
        ApplicationManager.getApplication().replaceService(NeoAppService::class.java, NeoAppService(cs, app), testRootDisposable)
        project.replaceService(NeoSessionService::class.java, NeoSessionService(project, cs, rpc), testRootDisposable)
        val item = FileAttachment("part1").apply {
            mime = "text/plain"
        ...
        try {
            NeoAttachmentEditorService(project, cs).load(ref("ses1", "msg1", item, "note.txt", "/repo"), parent) {
                results.add(it)
            }
    ...
    fun testLoadShowsConnectionFailedUntilRetryBecomesReady() = runBlocking {
        ...
        app.state.value = NeoAppStateDto(NeoAppStatusDto.ERROR)
        ApplicationManager.getApplication().replaceService(NeoAppService::class.java, NeoAppService(cs, app), testRootDisposable)
        project.replaceService(NeoSessionService::class.java, NeoSessionService(project, cs, rpc), testRootDisposable)
        val item = FileAttachment("part1").apply {
