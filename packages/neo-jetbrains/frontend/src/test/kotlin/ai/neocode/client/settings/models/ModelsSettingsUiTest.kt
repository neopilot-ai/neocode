...
class ModelsSettingsUiTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        app = NeoAppService(appScope, rpc)
        workspaces = NeoWorkspaceService(appScope, workspaceRpc)
        val state = NeoAppStateDto(
            NeoAppStatusDto.READY,
            config = ConfigDto(model = "neo/old"),
    ...
    fun `test failed save after dispose shows notification`() {
        ...
        flushUntil {
            notes.any { it.groupId == "Neo Code" && it.type == NotificationType.ERROR }
        }
    ...
    fun `test logged out save keeps login banner stable`() {
        ...
        edt { ui = ModelsSettingsUi(uiScope, app, workspaces, directory = "/test") }
        val panel = requireUi()
        flushUntil { text(panel).contains("Old") && text(panel).contains("Sign in to Neo Code") }
        ...
        edt {
            select(panel, "new")
            panel.applyDraft()
            assertTrue(text(panel).contains("Sign in to Neo Code"))
    ...
    private fun state(model: String): NeoAppStateDto = NeoAppStateDto(
        NeoAppStatusDto.READY,
    ...
    private fun select(panel: ModelsSettingsUi, id: String) {
        ...
        picker.onSelect(ModelPicker.Item(id, id.replaceFirstChar { it.titlecase() }, "neo", "Neo"))
    }
