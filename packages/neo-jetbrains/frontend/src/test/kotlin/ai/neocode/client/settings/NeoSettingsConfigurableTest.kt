...
class NeoSettingsConfigurableTest : BasePlatformTestCase() {

    fun `test id matches xml registration`() {
        val cfg = NeoSettingsConfigurable()
        assertEquals("ai.neocode.jetbrains.settings", cfg.id)
    }

    fun `test child profile id matches xml registration`() {
        // Verify the constants used in XML registrations are stable
        assertEquals("ai.neocode.jetbrains.settings.profile", UserProfileConfigurable.ID)
    }

    fun `test child models id matches xml registration`() {
        assertEquals("ai.neocode.jetbrains.settings.models", ModelsConfigurable.ID)
    }

    fun `test child provider and behavior ids match xml registration`() {
        assertEquals("ai.neocode.jetbrains.settings.providers", ProvidersConfigurable.ID)
        assertEquals("ai.neocode.jetbrains.settings.agentBehavior", AgentBehaviorConfigurable.ID)
    }
    ...
    fun `test root implements SearchableConfigurable but not Parent`() {
        ...
        val interfaces = NeoSettingsConfigurable::class.java.interfaces
        assertFalse(
            "NeoSettingsConfigurable must not implement SearchableConfigurable.Parent",
    ...
    fun `test open invokes select with child found by id`() {
        ...
        assertEquals("ai.neocode.jetbrains.settings.profile", UserProfileConfigurable.ID)
        assertEquals("ai.neocode.jetbrains.settings.profile", profile.id)
    }

    fun `test isModified always false`() {
        assertFalse(NeoSettingsConfigurable().isModified)
    }
