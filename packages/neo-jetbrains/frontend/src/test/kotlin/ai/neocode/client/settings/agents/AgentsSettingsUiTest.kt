...
import ai.neocode.client.testing.fire
...
import ai.neocode.client.settings.base.settingsListCellBounds
import ai.neocode.client.testing.FakeAgentBehaviorRpcApi
import ai.neocode.client.testing.FakeAppRpcApi
import ai.neocode.client.testing.FakeWorkspaceRpcApi
import ai.neocode.rpc.dto.AgentConfigDto
...
class AgentsSettingsUiTest : BasePlatformTestCase() {
    ...
    fun `test changing default agent saves patch`() {
        ...
        assertEquals("ask", appRpc.configPatches.single().values[NeoCliParser.CONFIG_DEFAULT_AGENT])
    ...
    fun `test apply waits for backend ready before refetching`() {
        ...
        flushUntil { rows(panel).size == 6 }
        appRpc.afterConfig = {
            app._state.value = app._state.value.copy(status = NeoAppStatusDto.LOADING)
        ...
        assertTrue(edt { text(panel).contains(NeoBundle.message("settings.agentBehavior.saving")) })

        app._state.value = app._state.value.copy(status = NeoAppStatusDto.READY)
    ...
    fun `test importing an agent stages patch until apply`() {
        ...
        edt {
            ...
            assertEquals(NeoCliParser.MODE_SUBAGENT, patch.mode)
    ...
    fun `test importing invalid JSON shows settings error message`() {
        ...
        val message = NeoBundle.message("settings.agentBehavior.agents.import.invalidJson")
    ...
    fun `test apply failure reloads and keeps remaining staged changes`() {
        ...
        flushUntil { text(panel).contains(NeoBundle.message("settings.agentBehavior.agents.create.failed")) }
    ...
    private fun install() {
        ...
        agentRpc = FakeAgentBehaviorRpcApi().apply {
            agents = listOf(
                AgentDetailDto("ask", displayName = "Ask", description = "Ask questions", mode = NeoCliParser.MODE_PRIMARY, native = true),
                AgentDetailDto("code", displayName = "Code", description = "Code things", mode = NeoCliParser.MODE_PRIMARY, native = true),
                AgentDetailDto("generated", description = "Generated", mode = NeoCliParser.MODE_SUBAGENT, native = false, removable = false),
                AgentDetailDto("hidden", description = "Hidden custom", mode = NeoCliParser.MODE_PRIMARY, native = false, removable = true, hidden = true),
                AgentDetailDto("old", description = "Old custom", mode = NeoCliParser.MODE_PRIMARY, native = false, removable = true, deprecated = true),
                AgentDetailDto("worker", description = "Worker", mode = NeoCliParser.MODE_SUBAGENT, native = false, removable = true),
            )
        }
        val workspaceRpc = FakeWorkspaceRpcApi().apply { models = ModelsWorkspaceDto(providers()) }
        app = NeoAppService(cs, appRpc)
        val ready = NeoAppStateDto(
            NeoAppStatusDto.READY,
            config = ConfigDto(
                defaultAgent = "code",
        ...
        ApplicationManager.getApplication().replaceService(NeoAppService::class.java, app, testRootDisposable)
        ApplicationManager.getApplication().replaceService(NeoAgentBehaviorService::class.java, NeoAgentBehaviorService(cs, agentRpc), testRootDisposable)
        ApplicationManager.getApplication().replaceService(NeoWorkspaceService::class.java, NeoWorkspaceService(cs, workspaceRpc), testRootDisposable)
    }

    private fun providers() = ProvidersDto(
        providers = listOf(ProviderDto("neo", "Neo", models = mapOf("gpt-5" to ModelDto("gpt-5", "GPT-5")))),
