...
class ViewSwitchingTest : SessionControllerTestBase() {
    ...
    private fun session(id: String) = ai.neocode.rpc.dto.SessionDto(
        id = id,
        projectID = "prj",
        directory = "/test",
        title = "Title $id",
        version = "1",
        time = ai.neocode.rpc.dto.SessionTimeDto(created = 1.0, updated = 2.0),
    )
    ...
    fun `test empty session overlay show includes logged in profile`() {
        ...
        appRpc.state.value = NeoAppStateDto(NeoAppStatusDto.READY, profile = prof)
        val m = controller()
        val events = collect(m)
    ...
    fun `test selecting personal account emits switching overlay`() {
        ...
        appRpc.state.value = NeoAppStateDto(
            ...
            profile = ProfileDto(
                email = "user@example.com",
                currentOrgId = "org_1",
                organizations = listOf(ProfileOrganizationDto("org_1", "Neo", "OWNER")),
            ),
        )
        val m = controller()
        val events = collect(m)
