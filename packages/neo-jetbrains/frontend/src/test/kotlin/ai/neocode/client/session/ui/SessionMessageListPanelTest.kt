...
import ai.neocode.client.session.views.PlanExitView
...
import ai.neocode.client.session.views.question.QuestionResultView
import ai.neocode.client.session.views.question.QuestionView
import ai.neocode.client.session.ui.selection.SessionCopyTarget
import ai.neocode.client.session.views.MessageToolbar
import ai.neocode.client.session.views.MessageView
import ai.neocode.client.session.views.TextView
import ai.neocode.client.session.views.TurnView
import ai.neocode.client.session.views.base.PartView
import ai.neocode.client.session.views.tool.TaskToolView
import ai.neocode.client.session.views.tool.ToolView
import ai.neocode.client.session.views.todo.TodoWriteView
import ai.neocode.client.ui.DiffStatBadge
import ai.neocode.client.ui.layout.Stack
import ai.neocode.rpc.dto.DiffFileDto
...
import ai.neocode.rpc.dto.TodoDto
...
class SessionMessageListPanelTest : BasePlatformTestCase() {
    ...
    fun `test user prompt toolbar omits rollback when revert handler is absent`() {
        ...
        assertFalse(components(toolbar).filterIsInstance<JButton>().any { it.toolTipText == NeoBundle.message("revert.message.rollback") })
    }
    ...
    fun `test user prompt toolbar shows rollback when revert handler is present`() {
        ...
        val rollback = components(toolbar)
            ...
            .first { it.toolTipText == NeoBundle.message("revert.message.rollback") }
        rollback.doClick()
    ...
    fun `test text markdown link uses panel url opener`() {
        ...
        model.upsertMessage(msg("a1", "assistant"))
        model.updateContent("a1", part("p1", "a1", "text", text = "[docs](https://neocode.ai/docs)"))

        val view = item.findMessage("a1")!!.part("p1") as TextView
        view.md.simulateLink("https://neocode.ai/docs")

        assertEquals(listOf("https://neocode.ai/docs"), urls)
    }
    ...
    fun `test rollback banner uses session dialog card with standard actions`() {
        ...
        assertEquals(
            listOf(NeoBundle.message("revert.banner.redo"), NeoBundle.message("revert.banner.redo.all")),
            buttons.map { it.text },
        )
        assertEquals(listOf(NeoBundle.message("revert.banner.redo")), buttons.filter { it.isVisible }.map { it.text })
        ...
        val hint = components(banner)
            ...
            .first { it.text == NeoBundle.message("revert.banner.hint") }
    ...
    fun `test rollback banner shows redo all only for multiple reverted messages`() {
        ...
        assertTrue(components(banner).filterIsInstance<JButton>().first { it.text == NeoBundle.message("revert.banner.redo.all") }.isVisible)

        model.setRevert(SessionRevertDto("u2"))
        banner.update()

        assertFalse(components(banner).filterIsInstance<JButton>().first { it.text == NeoBundle.message("revert.banner.redo.all") }.isVisible)
    }
    ...
    fun `test rollback banner buttons invoke actions`() {
        ...
        components(banner).filterIsInstance<JButton>().first { it.text == NeoBundle.message("revert.banner.redo") }.doClick()
        components(banner).filterIsInstance<JButton>().first { it.text == NeoBundle.message("revert.banner.redo.all") }.doClick()
    ...
    fun `test rollback state disables banner actions and shows inline progress`() {
        ...
        val buttons = components(banner).filterIsInstance<JButton>()
        assertTrue(buttons.filter { it.text == NeoBundle.message("revert.banner.redo") }.all { !it.isEnabled })
        assertTrue(buttons.filter { it.text == NeoBundle.message("revert.banner.redo.all") }.all { !it.isEnabled })
    ...
    fun `test rollback banner explains snapshotless history only revert`() {
        ...
        val notice = components(banner).filterIsInstance<JBLabel>()
            .first { it.text == NeoBundle.message("revert.banner.filesNotRestored") }
    ...
    fun `test completed plan update replaces tool view and keeps open file action`() {
        ...
        model.updateContent(
            ...
            toolPart(
                ...
                metadata = mapOf("plan" to ".neo/plans/x.md"),
            ),
        )

        val view = mv.part("tp1") as PlanExitView
        view.simulateLink(".neo/plans/x.md")

        assertEquals(listOf(".neo/plans/x.md"), opened)
    }
