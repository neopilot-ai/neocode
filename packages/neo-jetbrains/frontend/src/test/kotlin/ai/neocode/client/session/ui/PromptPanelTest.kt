...
import ai.neocode.client.session.ui.attachment.AttachmentCard
import ai.neocode.client.session.ui.attachment.AttachmentCardItem
...
import ai.neocode.client.session.ui.prompt.PROMPT_ATTACHMENT_PASTE_HANDLER_KEY
import ai.neocode.client.session.ui.prompt.PromptAttachmentPasteHandler
import ai.neocode.client.session.ui.prompt.PromptAttachmentPasteProvider
...
import ai.neocode.client.test.CopyProviderSink
import ai.neocode.client.testing.FakeWorkspaceRpcApi
import ai.neocode.rpc.dto.FileSearchResultDto
...
class PromptPanelTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        workspaces = NeoWorkspaceService(scope, rpc)
    }
    ...
    fun `test empty enhance explanation resizes wrapped input`() {
        ...
        assertTrue(editor.preferredSize.height > min)
        assertEquals(NeoBundle.message("prompt.action.enhance.description"), editor.text)
    }
    ...
    fun `test prompt editor component undo redo shortcuts target prompt editor`() {
        ...
        invokeComponentAction("Neo Session Undo", editor)
        assertEquals("", editor.document.text)
        invokeComponentAction("Neo Session Redo", editor)
        assertEquals("hello", editor.document.text)
    }
    ...
    fun `test prompt local completion shortcut opens mention lookup`() {
        rpc.searchResult = ai.neocode.rpc.dto.FileSearchResultDto(
            files = listOf(ai.neocode.rpc.dto.WorkspaceFileDto("src/deploy.ts", "deploy.ts")),
        )
        val panel = PromptPanel(project = project, onSend = { _, _ -> }, onAbort = {}, onEnhance = { _, _ -> }, completion = completion())
        val field = panel.defaultFocusedComponent as EditorTextField
    ...
    fun `test attachment tooltip hides embedded binary content`() {
        ...
        assertTrue(tip.contains("Location: ${NeoBundle.message("prompt.attachment.embedded")}"))
    ...
    fun `test prompt paste provider invokes registered handler`() {
        ...
        val item = FileListTransferable(listOf(File.createTempFile("neo-paste", ".txt")))
    ...
    fun `test file list paste adds attachment`() {
        val panel = PromptPanel(project, { _, _ -> }, {}, { _, _ -> })
        val file = File.createTempFile("neo-paste", ".txt")
    ...
    fun `test frontend file attachment defers data url encoding until send`() {
        ...
        val item = ai.neocode.client.session.model.PromptAttachmentExtractor.files(listOf(file)).single()
    ...
    fun `test pasted frontend file sends data url payload`() {
        var sent: ai.neocode.rpc.dto.PromptPartDto? = null
    ...
    fun `test file paste ignores companion image flavor`() {
        val panel = PromptPanel(project, { _, _ -> }, {}, { _, _ -> })
        val file = File.createTempFile("neo-paste", ".png")
    ...
    fun `test prompt button switches between send and stop state`() {
        ...
        assertEquals(KeymapUtil.createTooltipText("Send", "Neo.SendPrompt"), panel.buttonForTest().toolTipText)
    ...
    fun `test auto approve button toggles and updates tooltip`() {
        ...
        assertEquals(NeoBundle.message("prompt.action.autoApprove.enable"), button.accessibleContext.accessibleName)
        assertEquals(NeoBundle.message("prompt.action.autoApprove.disabled.tooltip"), button.toolTipText)
        val icon = button.icon
        ...
        assertNotSame(icon, button.icon)
        assertEquals(NeoBundle.message("prompt.action.autoApprove.disable"), button.accessibleContext.accessibleName)
        assertEquals(NeoBundle.message("prompt.action.autoApprove.enabled.tooltip"), button.toolTipText)
    ...
    private fun autoApproveButton(panel: PromptPanel): JButton {
        val enable = NeoBundle.message("prompt.action.autoApprove.enable")
        val disable = NeoBundle.message("prompt.action.autoApprove.disable")
    ...
    private fun attachmentRemoveButton(panel: PromptPanel, item: PromptAttachment): JButton {
        val name = NeoBundle.message("prompt.attachment.remove", item.name)
    ...
    private fun enhanceButton(panel: PromptPanel): JButton {
        val name = NeoBundle.message("prompt.action.enhance")
        return buttons(panel).first { it.accessibleContext.accessibleName == name }
    }
    ...
    private fun completion() = NeoPromptCompletionProvider(
    ...
    private fun invokeCompletionAction(editor: Editor) {
        val action = ActionUtil.getActions(editor.contentComponent).first { item ->
            item.templatePresentation.text == "Neo Prompt Completion"
        }
