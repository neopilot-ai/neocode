...
class SessionUpdateQueueTest : SessionControllerTestBase() {
    ...
    fun `test per token snapshot plus delta interleave does not double text or code`() {
        ...
        val text = (m.model.message("msg1")!!.parts["prt1"] as ai.neocode.client.session.model.Text).content.toString()
    ...
    fun `test update hooks run on EDT around history and recovery`() {
        appRpc.state.value = ai.neocode.rpc.dto.NeoAppStateDto(ai.neocode.rpc.dto.NeoAppStatusDto.READY)
        projectRpc.state.value = workspaceReady()
        rpc.history.add(ai.neocode.rpc.dto.MessageWithPartsDto(msg("msg1", "ses_test", "assistant"), emptyList()))
