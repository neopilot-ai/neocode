...
class SessionFileLinksTest : BasePlatformTestCase() {
    ...
    fun `test isFileHref separates file refs from urls`() {
        ...
        assertFalse(SessionFileLinks.isFileHref("https://neocode.ai/docs"))
    ...
    fun `test managed worktree storage filter only rejects worktree subtree`() {
        ...
        assertFalse(isManagedWorktreeStorage(".neo/plans/x.md"))
        assertTrue(isManagedWorktreeStorage(".neo/worktrees"))
        assertTrue(isManagedWorktreeStorage(".neo/worktrees/foo/backend/src/Main.java"))
    }
