...
import ai.neocode.client.session.ui.RevertProgress
import ai.neocode.client.session.ui.SessionDropOverlay
import ai.neocode.client.session.ui.SessionLayoutPanel
...
class SessionUiLayoutTest : SessionUiTestBase() {
    ...
    fun `test account overlay shows after recents complete`() {
        appRpc.state.value = NeoAppStateDto(NeoAppStatusDto.READY, profile = ProfileDto(email = "user@example.com"))
        rpc.recent.add(session("ses_1"))
        ui = newUi(displayMs = 1_000)
