...
/**
 * Fake [NeoAppRpcApi] for testing.
 *
 * Push state changes via [state]. Health check returns [health].
 *
 * Every `suspend` method asserts it is NOT called on the EDT.
 */
class FakeAppRpcApi : NeoAppRpcApi {

    val state = MutableStateFlow(NeoAppStateDto(NeoAppStatusDto.DISCONNECTED))
    ...
    var fakeDeviceAuth = DeviceAuthDto(code = "TEST-1234", verificationUrl = "https://auth.neo.khulnasoft.com/device")
