...
class BaseSettingsUiTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        app = NeoAppService(appScope, FakeAppRpcApi())
        workspaces = NeoWorkspaceService(appScope, FakeWorkspaceRpcApi())
    }
    ...
    fun `test login banner can be shown`() {
        ...
        edt { assertTrue(text(view).contains("Sign in to Neo Code")) }
    }
    ...
    fun `test login banner can be disabled`() {
        ...
        edt { assertFalse(text(view).contains("Sign in to Neo Code")) }
    }
    ...
    private class FakePanel(
        ...
        override fun draft(state: NeoAppStateDto): Draft = draft
