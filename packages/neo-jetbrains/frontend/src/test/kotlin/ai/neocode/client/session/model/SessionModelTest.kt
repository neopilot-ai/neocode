...
class SessionModelTest : BasePlatformTestCase() {
    ...
    fun `test initial app and workspace state`() {
        assertEquals(NeoAppStatusDto.DISCONNECTED, model.app.status)
        assertEquals(NeoWorkspaceStatusDto.PENDING, model.workspace.status)
    ...
    fun `test isReady requires app and workspace readiness`() {
        model.app = NeoAppStateDto(NeoAppStatusDto.READY)
        assertFalse(model.isReady())

        model.workspace = NeoWorkspaceStateDto(NeoWorkspaceStatusDto.READY)
