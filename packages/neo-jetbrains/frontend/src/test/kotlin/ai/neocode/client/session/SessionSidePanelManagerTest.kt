...
import ai.neocode.client.session.history.HistoryPanel
...
import ai.neocode.client.testing.TestUiTimers
...
class SessionSidePanelManagerTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        app = NeoAppService(coroutines.scope, FakeAppRpcApi().also {
            it.state.value = NeoAppStateDto(NeoAppStatusDto.READY)
        })
        workspaces = NeoWorkspaceService(coroutines.scope, FakeWorkspaceRpcApi().also {
    ...
    fun `test inactive sessions keep queued style updates`() {
        ...
        val style = ai.neocode.client.session.ui.style.SessionEditorStyle.create(family = "Courier New", size = 24)
    ...
    fun `test history overlays update before hidden timeout`() {
        ...
        assertEquals(NeoBundle.message("history.badge.question"), history.badgeText(0))
    }
    ...
    private fun setInactiveDisposeTimeout(ms: Int) {
        val key = "neo.session.inactive.disposeTimeoutMs"
    ...
    private fun JPanel.controller(): ai.neocode.client.session.controller.SessionController {
        val field = SessionUi::class.java.getDeclaredField("controller")
        field.isAccessible = true
        return field.get(this) as ai.neocode.client.session.controller.SessionController
    }
