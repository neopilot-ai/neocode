...
import ai.neocode.client.session.history.CloudHistoryItem
import ai.neocode.client.session.history.HistoryController
import ai.neocode.client.session.history.HistoryDataKeys
import ai.neocode.client.session.history.HistorySelection
import ai.neocode.client.session.history.HistorySource
...
class HistorySessionActionsTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        sessions = NeoSessionService(project, coroutines.scope, rpc)
        val workspaces = NeoWorkspaceService(coroutines.scope, FakeWorkspaceRpcApi().also {
            it.state.value = NeoWorkspaceStateDto(status = NeoWorkspaceStatusDto.READY)
        })
    ...
    fun `test rename action passes untitled fallback to input`() {
        ...
        assertEquals(listOf(NeoBundle.message("history.untitled")), prompts)
        assertTrue(rpc.renames.isEmpty())
    }
    ...
    fun `test frontend descriptor registers history actions`() {
        val xml = javaClass.classLoader.getResourceAsStream("neo.jetbrains.frontend.xml")
            ?.bufferedReader()
            ?.use { it.readText() }
            ?: error("missing frontend descriptor")

        assertTrue(xml.contains("id=\"Neo.Session.Open\""))
        assertTrue(xml.contains("id=\"Neo.Session.Rename\""))
        assertTrue(xml.contains("id=\"Neo.Session.Delete\""))
        assertTrue(xml.contains("id=\"Neo.History.ContextMenu\""))
        assertTrue(xml.contains("id=\"Neo.Session.ContextMenu\""))
        assertTrue(xml.contains("ref=\"Neo.Session.Open\""))
        assertTrue(xml.contains("ref=\"Neo.Session.Rename\""))
        assertTrue(xml.contains("ref=\"Neo.Session.Delete\""))
