 ...
 * reconnect. JetBrains now does the same in [ai.neocode.client.session.controller.SessionController.recoverPending].
 */
class SessionRecoveryTest : SessionControllerTestBase() {
    ...
    // ------ Status seeding from NeoSessionService.statuses ------
    ...
    fun `test retry status is seeded with message attempt and next`() {
        ...
        assertSession(
            """
            [code] [neo/gpt-5] [retry] [Rate limited]
            """,
            m, show = true,
        )
    ...
    fun `test offline status is seeded with message and requestId`() {
        ...
        assertSession(
            """
            [code] [neo/gpt-5] [offline] [No network]
            """,
            m, show = true,
        )
