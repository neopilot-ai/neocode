...
class NeoPromptCompletionProviderTest : BasePlatformTestCase() {
    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeWorkspaceRpcApi
    private lateinit var provider: NeoPromptCompletionProvider

    override fun setUp() {
        super.setUp()
        scope = CoroutineScope(SupervisorJob() + Dispatchers.Default)
        rpc = FakeWorkspaceRpcApi()
        val workspaces = NeoWorkspaceService(scope, rpc)
        provider = NeoPromptCompletionProvider(
    ...
    fun `test accepting mention mid token replaces glued suffix`() {
        ...
        assertEquals(
            NeoPromptCompletionProvider.MentionHit(0, 22, "backend/deploy-dev.sh", true),
    ...
    fun `test highlights known slash command at start`() {
        assertEquals(
            listOf(NeoPromptCompletionProvider.Highlight(0, 4, NeoPromptCompletionProvider.HighlightKind.COMMAND)),
    ...
    fun `test highlights client slash command aliases at start`() {
        assertEquals(
            listOf(NeoPromptCompletionProvider.Highlight(0, 6, NeoPromptCompletionProvider.HighlightKind.COMMAND)),
            provider.highlights("/clear"),
        )
    }

    fun `test highlights server slash command at start`() {
        rpc.state.value = NeoWorkspaceStateDto(NeoWorkspaceStatusDto.READY, commands = listOf(CommandDto("deploy")))

        waitFor { provider.highlights("/deploy prod").isNotEmpty() }

        assertEquals(
            listOf(NeoPromptCompletionProvider.Highlight(0, 7, NeoPromptCompletionProvider.HighlightKind.COMMAND)),
    ...
    fun `test highlights special mentions without tracked paths`() {
        assertEquals(
            listOf(
                NeoPromptCompletionProvider.Highlight(4, 16, NeoPromptCompletionProvider.HighlightKind.MENTION),
            ),
    ...
    fun `test serverCommand does not route client aliases`() {
        rpc.state.value = NeoWorkspaceStateDto(NeoWorkspaceStatusDto.READY, commands = listOf(CommandDto("clear")))
    ...
    fun `test highlights tracked mentions longest first`() {
        ...
        assertEquals(
            listOf(NeoPromptCompletionProvider.Highlight(4, 14, NeoPromptCompletionProvider.HighlightKind.MENTION)),
    ...
    fun `test highlights unresolved mention after validation`() {
        ...
        assertEquals(
            listOf(NeoPromptCompletionProvider.Highlight(4, 16, NeoPromptCompletionProvider.HighlightKind.INVALID)),
    ...
    fun `test highlights existing hand typed mention after validation`() {
        ...
        assertEquals(
            listOf(NeoPromptCompletionProvider.Highlight(4, 13, NeoPromptCompletionProvider.HighlightKind.MENTION)),
    ...
    fun `test mentionAt resolves tracked file mention`() {
        ...
        assertEquals(
            NeoPromptCompletionProvider.MentionHit(4, 13, "src/a.ts", true),
    ...
    fun `test mentionAt marks unresolved file mention after validation`() {
        ...
        assertEquals(
            NeoPromptCompletionProvider.MentionHit(4, 15, "missing.ts", false),
    ...
    private fun noMatches() = NeoBundle.message("prompt.completion.noMatches")
}
