...
class ModelPickerTest : BasePlatformTestCase() {
    ...
    fun `test section order starts with favorites and recommended`() {
        val rows = modelPickerRows(listOf(
            ...
            item("auto", "Neo Auto", "neo", "Neo", 0.0),
        ...
        assertEquals("neo/auto", rows[1].key)
    ...
    fun `test neo provider group is first and other providers keep source order`() {
        val rows = modelPickerRows(listOf(
            item("gpt", "GPT", "openai", "OpenAI"),
            item("auto", "Auto", "neo", "Neo"),
            item("claude", "Claude", "anthropic", "Anthropic"),
        ), emptyList(), "")

        assertEquals(listOf("Neo", "OpenAI", "Anthropic"), rows.indices.mapNotNull { modelPickerSectionTitle(rows, it) })
    }
    ...
    fun `test small rows are included only when requested`() {
        ...
        assertEquals(listOf("neo/auto"), default.mapNotNull { it.key })
        assertEquals(listOf("neo/auto", "neo/auto-small"), small.mapNotNull { it.key }.sorted())
    }
    ...
    fun `test non-neo selected model uses provider prefix`() {
        val picker = ModelPicker()
    ...
    fun `test non-neo selected model strips duplicate provider prefix`() {
        val picker = ModelPicker()
    ...
    fun `test non-neo selected model strips vscode provider prefix`() {
        val picker = ModelPicker()
    ...
    fun `test neo selected model remains unprefixed`() {
        val picker = ModelPicker()

        picker.setItems(listOf(item("auto", "Neo Auto", "neo", "Neo")))
    ...
    fun `test details panel displays preview metadata and updates favorite star`() {
        ...
        val item = ModelPicker.Item(
            ...
            options = ModelOptionsDto("Fast routed model https://neocode.ai"),
        ...
        assertTrue(panel.html().any { it.contains("href=\"https://neocode.ai\"") })
    ...
    fun `test details panel updates retained description component`() {
        ...
        val item = ModelPicker.Item(
            ...
            options = ModelOptionsDto("First https://neocode.ai"),
        )

        panel.update(item)
        val count = panel.treeSize()
        val html = panel.htmlPanes().single()
        panel.update(item.copy(id = "second", display = "Second", options = ModelOptionsDto("Second https://neocode.ai/docs")))
    ...
    fun `test selected paid model with training flag indicates data collection`() {
        ...
        picker.setItems(listOf(item("paid", "Paid", "neo", "Neo", training = true)))
    ...
    fun `test selected free model without training flag does not indicate data collection`() {
        ...
        picker.setItems(listOf(item("free", "Free", "neo", "Neo", free = true)))
    ...
    fun `test display parts sanitize free suffix`() {
        val parts = ModelText.parts(item("auto", "Auto Free (free)", "neo", "Neo"))
    ...
    fun `test renderer shows empty favorite star only for selected row`() {
        val row = ModelPickerRow(item("auto", "Auto", "neo", "Neo"), "Neo", false)
    ...
    fun `test renderer keeps favorite star visible`() {
        val row = ModelPickerRow(item("auto", "Auto", "neo", "Neo"), "Neo", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { setOf("neo/auto") })
        val list = JBList(model)
    ...
    fun `test renderer shows free badge for free model`() {
        val row = ModelPickerRow(ModelPicker.Item("auto", "Auto", "neo", "Neo", free = true), "Neo", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { emptySet() })
        val list = JBList(model)
    ...
    fun `test renderer shows data collection warning for paid training model`() {
        val row = ModelPickerRow(item("paid", "Paid", "neo", "Neo", training = true), "Neo", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { emptySet() })
        val list = JBList(model)
    ...
    fun `test renderer shows BYOK instead of free when both are available`() {
        val row = ModelPickerRow(
            ModelPicker.Item("claude", "Claude", "neo", "Neo", free = true, byok = true),
            "Neo",
            false,
        )
        val model = CollectionListModel(listOf(row))
