...
class PromptLifecycleTest : SessionControllerTestBase() {
    ...
    override fun setUp() {
        super.setUp()
        edt { NeoPluginSettings.unsetAutoApprove() }
    }
    ...
    fun `test prompt records aggregate mention telemetry`() {
        appRpc.state.value = ai.neocode.rpc.dto.NeoAppStateDto(ai.neocode.rpc.dto.NeoAppStatusDto.READY, config = ai.neocode.rpc.dto.ConfigDto(model = "neo/gpt-5"))
    ...
    fun `test PermissionAsked moves state to AwaitingPermission`() {
        ...
        assertSession(
            ...
            [code] [neo/gpt-5] [awaiting-permission]
            """,
            m,
        )
    }
    ...
    fun `test PermissionReplied resumes Busy state`() {
        ...
        assertSession(
            """
            [code] [neo/gpt-5] [busy] [considering next steps]
            """,
            m,
        )
    }
    ...
    fun `test QuestionAsked moves state to AwaitingQuestion`() {
        ...
        assertSession(
            ...
            [code] [neo/gpt-5] [awaiting-question]
            """,
            m,
        )
    }
    ...
    fun `test auto approve drains pending permissions during recovery`() {
        ...
        projectRpc.state.value = workspaceReady()
        rpc.pendingPermissionList.add(permission("perm_pending"))
        edt { NeoPluginSettings.setAutoApprove(true) }
    ...
    fun `test auto approve persists in properties`() {
        ...
        assertFalse(NeoPluginSettings.getAutoApprove())
        edt { m.setAutoApprove(true) }

        assertTrue(NeoPluginSettings.getAutoApprove())
    ...
    fun `test child non-permission events do not change root state`() {
        ...
        emit(ChatEventDto.SessionStatusChanged("ses_child", ai.neocode.rpc.dto.SessionStatusDto("busy")), flush = false)
        ...
        val stateEvents = modelEvents.filterIsInstance<ai.neocode.client.session.model.SessionModelEvent.StateChanged>()
