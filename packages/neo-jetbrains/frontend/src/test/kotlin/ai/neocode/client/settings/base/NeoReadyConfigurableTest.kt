...
class NeoReadyConfigurableTest : BasePlatformTestCase() {
    ...
    fun `test disconnected shows unavailable and does not create ready component`() {
        ...
        edt {
            val text = text(root)
            assertTrue(text, text.contains(NeoBundle.message("settings.cli.unavailable.title")))
            assertTrue(text, text.contains(NeoBundle.message("settings.cli.unavailable.message")))
    ...
    fun `test ready transition creates ready component once`() {
        ...
        rpc.state.value = NeoAppStateDto(NeoAppStatusDto.READY)
        flushUntil { edt { cfg?.created == 1 } }
        rpc.state.value = NeoAppStateDto(NeoAppStatusDto.DISCONNECTED)
        flush()

        edt {
            assertEquals(1, cfg?.created)
            assertTrue(text(root).contains("Ready content"))
            assertFalse(text(root).contains(NeoBundle.message("settings.cli.unavailable.title")))
        }
    }
