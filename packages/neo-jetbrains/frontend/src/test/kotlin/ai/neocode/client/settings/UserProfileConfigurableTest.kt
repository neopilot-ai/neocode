...
import ai.neocode.client.settings.profile.ProfileUi
import ai.neocode.client.settings.profile.formatResetDate
import ai.neocode.client.settings.profile.formatShortBalance
import ai.neocode.client.testing.FakeAppRpcApi
import ai.neocode.client.testing.TestCoroutines
...
class UserProfileConfigurableTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        rpc = FakeAppRpcApi()
        app = NeoAppService(coroutines.scope, rpc)
        app._state.value = NeoAppStateDto(NeoAppStatusDto.READY)
        edt {
            panel = ProfileUi(
                profile = null,
    ...
    fun `test login updates profile UI`() {
        ...
        edt {
            ...
            buttons(panel).first { it.text == "Login with Neo Code" }.doClick()
        }
        flush()
        ...
        assertEquals(listOf("https://auth.neo.khulnasoft.com/device"), urls)
    }

    fun `test logout updates profile UI`() {
        val profile = ProfileDto(email = "alice@test.com", name = "Alice")
        rpc.fakeProfile = profile
        app._state.value = NeoAppStateDto(NeoAppStatusDto.READY, profile = profile)
        edt { panel.update(profile, NeoAppStatusDto.READY) }
        ...
        edt {
            ...
            assertTrue(buttons(panel).any { it.text == "Login with Neo Code" })
        }
    }
    ...
    fun `test organization switch updates balance UI`() {
        ...
        app._state.value = NeoAppStateDto(NeoAppStatusDto.READY, profile = personal)
        edt { panel.update(personal, NeoAppStatusDto.READY) }
    ...
    fun `test logged in profile uses compact stack and copyable email`() {
        ...
        edt {
            ...
            layout(panel)
            val logo = labelsByName(panel, "neo.profile.logo.loggedIn").single()
    ...
    fun `test personal profile shows neo pass usage`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(267.59),
            neoPass = ProfileNeoPassDto(
        ...
        edt {
            val t = text(panel)
            assertTrue(t, t.contains("Neo Pass"))
            ...
            assertTrue("pass meter should be visible", panelsByName(panel, "neo.profile.passPanel").single().isVisible)
            assertFalse(t, t.contains("Get Neo Pass"))
        }
    }

    fun `test neo pass short amounts use half up rounding`() {
        assertEquals("$3", formatShortBalance(2.5))
        assertEquals("$2", formatShortBalance(2.49))
    }

    fun `test neo pass hides empty bonus and invalid renewal rows`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(267.59),
        ...
        val invalid = profile.copy(
            neoPass = profile.neoPass?.copy(
                currentPeriodBonusCreditsUsd = 0.0,
                nextBillingAt = "not-a-date",
            ),
        )
        edt { panel.update(invalid, NeoAppStatusDto.READY) }
        ...
        val none = invalid.copy(neoPass = invalid.neoPass?.copy(nextBillingAt = null))
        edt { panel.update(none, NeoAppStatusDto.READY) }
    ...
    fun `test neo pass meter clamps base zero and avoids noop repaint`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(267.59),
        ...
        val meter = edt { componentsByName(panel, "neo.profile.passMeter").single() }
        ...
        edt { panel.update(full, NeoAppStatusDto.READY) }
    ...
    fun `test personal profile shows neo pass without balance`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            neoPass = ProfileNeoPassDto(
        ...
        val updated = profile.copy(
            neoPass = profile.neoPass?.copy(currentPeriodUsageUsd = 88.0),
        )
        ...
        edt {
            ...
            assertTrue("pass panel should be visible", panelsByName(panel, "neo.profile.passPanel").single().isVisible)
    ...
    fun `test personal profile without neo pass shows subscribe link`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(10.0),
        ...
        edt {
            val t = text(panel)
            assertTrue(t, t.contains("Get Neo Pass to add credits and earn bonuses"))
            buttons(panel).first { it.text == "Dashboard" }.doClick()
            buttons(panel).first { it.text == "Top up" }.doClick()
            buttons(panel).first { it.text == "Get Neo Pass to add credits and earn bonuses" }.doClick()
        }

        assertEquals(listOf("https://app.neo.khulnasoft.com/profile", "https://app.neo.khulnasoft.com/credits", "https://neo.khulnasoft.com/pricing/neo-pass"), urls)
    }

    fun `test neo pass renewal date uses utc`() {
    ...
    fun `test org profile hides neo pass`() {
        val orgs = listOf(ProfileOrganizationDto(id = "org_1", name = "Acme", role = "ADMIN"))
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
        ...
        edt {
            val t = text(panel)
            assertFalse(t, t.contains("Neo Pass"))
            assertFalse(t, t.contains("Get Neo Pass"))
        }
    }
    ...
    fun `test logged out update retains login button`() {
        edt {
            val btn = buttons(panel).first { it.text == "Login with Neo Code" }
            panel.update(null, NeoAppStatusDto.READY)
            val btn2 = buttons(panel).first { it.text == "Login with Neo Code" }
            assertSame(btn, btn2)
        }
    }

    fun `test logged out profile shows neo icon above login content`() {
        edt {
            panel.update(null, NeoAppStatusDto.READY)
            panel.setSize(800, 600)
            layout(panel)

            val logo = labelsByName(panel, "neo.profile.logo.loggedOut").single()
    ...
    fun `test account update retains name label`() {
        ...
        edt {
            panel.update(alice, NeoAppStatusDto.READY)
            val lbl = labels(panel).first { it.text == "Alice" }
            panel.update(bob, NeoAppStatusDto.READY)
    ...
    fun `test organization switch keeps account visible during transient null profile`() {
        ...
        val transientState = NeoAppStateDto(
            status = NeoAppStatusDto.READY,
            profile = null,
        ...
        edt {
            panel.update(personal, NeoAppStatusDto.READY)
    ...
    fun `test login shows device auth card before completion`() {
        ...
        flushUntil { text(panel).contains("Sign in to Neo Code") }

        edt {
            val t = text(panel)
            assertTrue(t, t.contains("Sign in to Neo Code"))
            assertTrue(t, t.contains("Step 1:"))
            assertTrue(t, t.contains("Open this URL"))
            assertTrue(t, t.contains("https://auth.neo.khulnasoft.com/device"))
        ...
        edt {
            val qr = labelsByName(panel, "neo.login.qr").firstOrNull()
            assertNotNull(qr)
    ...
    fun `test cancel login invalidates stale completion`() {
        ...
        edt { buttons(panel).first { it.text == "Login with Neo Code" }.doClick() }
        flushUntil { text(panel).contains("Sign in to Neo Code") }
    ...
    fun `test auth card retains qr label across sync`() {
        ...
        val qrBefore = edt { labelsByName(panel, "neo.login.qr").firstOrNull() }
        assertNotNull(qrBefore)

        // Force another sync call while still pending
        edt { panel.update(null, NeoAppStatusDto.READY) }
        flush()

        val qrAfter = edt { labelsByName(panel, "neo.login.qr").firstOrNull() }
    ...
    fun `test url field selects all on click`() {
        ...
        edt {
            val field = fieldsByName(panel, "neo.login.url").firstOrNull()
    ...
    fun `test balance card has card background`() {
        ...
        edt {
            val card = panelsByName(panel, "neo.profile.balanceCard").firstOrNull()
    ...
    fun `test code panel has card background`() {
        ...
        edt {
            val codePanel = panelsByName(panel, "neo.login.codePanel").firstOrNull()
    ...
    fun `test combo model not rebuilt on balance change with same org list`() {
        ...
        edt { panel.update(updated, NeoAppStatusDto.READY) }
    ...
    fun `test combo model updated in place when org list changes`() {
        ...
        app._state.value = NeoAppStateDto(NeoAppStatusDto.READY, profile = profile1)
        edt { panel.update(profile1, NeoAppStatusDto.READY) }
        ...
        edt { panel.update(profile2, NeoAppStatusDto.READY) }
    ...
    fun `test profile update does not trigger organization rpc`() {
        ...
        app._state.value = NeoAppStateDto(NeoAppStatusDto.READY, profile = profile)
        edt { panel.update(profile, NeoAppStatusDto.READY) }
        edt { panel.update(profile.copy(currentOrgId = "org_1"), NeoAppStatusDto.READY) }
        flush()
    ...
    fun `test connecting while logged in keeps logged-in card visible`() {
        ...
        edt { panel.update(null, NeoAppStatusDto.CONNECTING) }
    ...
    fun `test loading while logged in keeps logged-in card visible`() {
        ...
        edt { panel.update(null, NeoAppStatusDto.LOADING) }
    ...
    fun `test loading with null profile while logged in does not crash`() {
        ...
        edt { panel.update(NeoAppStateDto(NeoAppStatusDto.LOADING)) }
        ...
        edt { panel.update(switched, NeoAppStatusDto.READY) }
    ...
    fun `test connecting while logged in with org selected keeps combo selection`() {
        ...
        app._state.value = NeoAppStateDto(NeoAppStatusDto.READY, profile = profile)
        edt {
            panel.update(profile, NeoAppStatusDto.READY)
            combos(panel).single().selectedIndex = 1
        }
    ...
    fun `test preferred focus for logged-out is login button`() {
        edt {
            val focus = panel.preferredFocus()
            val loginBtn = buttons(panel).firstOrNull { it.text == "Login with Neo Code" }
    ...
    fun `test device auth without code hides code panel and step2 label`() {
        ...
        rpc.fakeDeviceAuth = DeviceAuthDto(code = null, verificationUrl = "https://auth.neo.khulnasoft.com/device")
