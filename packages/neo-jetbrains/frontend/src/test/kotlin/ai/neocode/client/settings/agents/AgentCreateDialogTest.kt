...
class AgentCreateDialogTest : BasePlatformTestCase() {
    ...
    fun `test default form is empty primary project`() {
        ...
        assertEquals(AgentCreateDto("", "", NeoCliParser.MODE_PRIMARY, null, "project"), edt { d.result() })
    }
    ...
    fun `test reads form values into dto`() {
        ...
        assertEquals(AgentCreateDto("reviewer", "Review carefully", NeoCliParser.MODE_SUBAGENT, "Reviews code", "global"), result)
    }
    ...
    private fun title(field: String) = NeoBundle.message("settings.agentBehavior.agents.create.$field")
