...
class SessionHeaderControllerTest : SessionControllerTestBase() {
    ...
    fun `test session updated refreshes model metadata`() {
        ...
        assertTrue(modelEvents.any { it is ai.neocode.client.session.model.SessionModelEvent.SessionUpdated })
    }
    ...
    fun `test compact calls RPC with selected model when eligible`() {
        ...
        assertEquals("neo", call.third.providerID)
    ...
    fun `test compact is blocked without model`() {
        appRpc.state.value = ai.neocode.rpc.dto.NeoAppStateDto(ai.neocode.rpc.dto.NeoAppStatusDto.READY, config = ai.neocode.rpc.dto.ConfigDto(model = null))
