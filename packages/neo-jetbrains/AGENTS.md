# AGENTS.md — Neo JetBrains Plugin

## Package Overview

- **Split-mode plugin** with three Gradle modules: `shared/`, `frontend/`, `backend/`. The module descriptors are `neo.jetbrains.shared.xml`, `neo.jetbrains.frontend.xml`, `neo.jetbrains.backend.xml` — these must stay in sync with `plugin.xml`'s `<content>` block.
...
- Kotlin source goes under `{module}/src/main/kotlin/ai/neocode/jetbrains/`. Package name is `ai.neocode.jetbrains` (matches `group` in root `build.gradle.kts`).
...
- `plugin.xml` `<content>` entries ↔ module XML descriptors (`neo.jetbrains.{shared,frontend,backend}.xml`)
- Service classes ↔ `<applicationService>`/`<projectService>` entries in the corresponding module XML
- `packages/neo-jetbrains/package.json` version ↔ GitHub CLI release tag consumed by the backend downloader
- `packages/neo-jetbrains/gradle.properties` `neo.cli.pinned` ↔ Gradle and release-script gates
- `.neo/skills/release-jetbrains/script/check-pin.ts` / `set-pin.ts` ↔ release skill and CLI pin documentation
...
- Non-light services that need XML registration go in `neo.jetbrains.backend.xml` under `<extensions defaultExtensionNs="com.intellij"><applicationService>` (or `<projectService>`).
...
- By default, the plugin does not bundle CLI binaries. At connect time the backend downloads the GitHub Release asset for the version pinned in `packages/neo-jetbrains/package.json`; `backend` resources include `neo.properties` with `cli.version` and `cli.pinned` for split-mode RPC and runtime use.
- For release questions, use the `release-jetbrains` skill and reference `.neo/skills/release-jetbrains/SKILL.md`; it verifies the CLI pin before creating immutable `jetbrains/v*` tags.
...
**Pin mode** (`neo.cli.pinned` in `packages/neo-jetbrains/gradle.properties`) controls release CLI vs local repo CLI.

| Ask | Do |
|---|---|
| Unpin / use local repo CLI | Set `neo.cli.pinned=false`, then run `./gradlew :backend:buildRepoCli` from `packages/neo-jetbrains/`. `:backend:stageRepoCli` bundles `packages/opencode/dist/@neocode/cli-<os>-<arch>/bin/`; runtime extracts it instead of downloading. |
| Re-pin / use release CLI | Set `neo.cli.pinned=true`. This is the default and the only releasable state. |

`neo.cli.pinned=false` is dev-only: OpenAPI generation runs from local `packages/opencode/` source and the local binary is bundled. Production Gradle builds, `script/build-version.sh`, and the release scripts hard-fail on `false`, so restore `true` before releasing.

**Pinned CLI version** (`packages/neo-jetbrains/package.json` `version`) controls which GitHub CLI release the plugin downloads and generates the client from. The JetBrains release locks the value already merged to `origin/main`.

| Ask | Do |
|---|---|
| Check whether the CLI pin is current | `bun .neo/skills/release-jetbrains/script/check-pin.ts` |
| Bump the pin to `<version>` / latest and test locally | `bun .neo/skills/release-jetbrains/script/set-pin.ts --version <x.y.z>` or `bun .neo/skills/release-jetbrains/script/set-pin.ts --latest`, then run `./gradlew typecheck && ./gradlew test` from `packages/neo-jetbrains/`. |
| Land a tested pin bump for release | `bun .neo/skills/release-jetbrains/script/set-pin.ts --version <x.y.z> --pr` or `bun .neo/skills/release-jetbrains/script/set-pin.ts --latest --pr`; merge the PR to `main`, then re-run `check-pin.ts` before dispatching prepare. |
...
For the full release process (resolve version, pin verification, prepare, changelog, publish), load the `release-jetbrains` skill: `.neo/skills/release-jetbrains/SKILL.md`.

### Server Protocol

- The plugin spawns `neo serve --port 0` (OS assigns random port) and reads stdout for `listening on http://...:(\d+)` to discover the port.
- A random 32-byte hex password is passed via `NEO_SERVER_PASSWORD` env var for Basic Auth.
- Fixed env vars set on every spawn: `NEO_CLIENT=jetbrains`, `NEO_PLATFORM=jetbrains`, `NEO_APP_NAME=neo-code`, `NEO_ENABLE_QUESTION_TOOL=true`, `NEO_DISABLE_CLAUDE_CODE=true`, `NEOCODE_FEATURE=jetbrains-plugin`.
- Unless already provided by the base environment, the backend sets `NEO_CONFIG_CONTENT` to make `edit` and `bash` permissions ask by default for JetBrains-launched CLI processes.
- This is the same protocol used by the VS Code extension (`packages/neo-vscode/src/services/cli-backend/server-manager.ts`).

### Dev Storage Isolation

- In development (`runIdeSplitMode`, `runIdeBackend`, `runIdeFrontend`, or `runIde`), the Gradle property `neo.dev.storage.isolated=true` makes the backend set `XDG_DATA_HOME`, `XDG_CONFIG_HOME`, `XDG_STATE_HOME`, and `XDG_CACHE_HOME` to `<worktree>/.neo-dev/{data,config,state,cache}` before spawning the CLI. The worktree root comes from the `neo.dev.worktree.root` JVM system property (auto-set by Gradle from the project directory).
- The checked-in `Run IDE (Backend)`, `Run IDE (Frontend)`, and `Run IDE (Split Mode)` run configurations enable isolation by default (`-Pneo.dev.storage.isolated=true`). Developers can disable it by passing `-Pneo.dev.storage.isolated=false`.
- Use standard `XDG_*_HOME` env vars for this isolation. Do not introduce custom `NEO_DATA_DIR`, `NEO_GLOBAL_CONFIG_DIR`, `NEO_STATE_DIR`, or `NEO_CACHE_DIR` env vars — the CLI core already respects `XDG_*_HOME` via `xdg-basedir`.
- The `.neo-dev/` directory is gitignored and created automatically on first run.
- The implementation lives in `NeoBackendCliManager.buildEnv()` / `devStorageEnv()`. Tests: `NeoBackendCliManagerEnvTest`.

### Debugging Session Event Logs

- Use `script/dev/part-update.sh client <session-id>` from `packages/neo-jetbrains/` to print frontend `message.part.delta` text by part id.
- Use `script/dev/part-update.sh backend <session-id>` from `packages/neo-jetbrains/` for backend sandbox events.
- Append with `>> file.txt` when you need to keep the output.
- For full chat payload previews in JetBrains dev runs, pass `-Pneo.dev.log.chat.content=<mode>` where `<mode>` is `off` (default, no content), `preview` (cleaned/truncated content), or `full` (cleaned full content).
- `-Pneo.dev.log.chat.preview.max=<n>` controls preview length, clamped from 1 to 2000.

## Build and Verification

- **Marketplace version build**: Use `script/build-version.sh <version>` from `packages/neo-jetbrains/` to clean, build, sign, and verify the JetBrains Marketplace plugin ZIP. Pass `--skip-verification` only when explicitly needed.
- **Test version build**: If the user asks for a JetBrains test build, still require a version and use `script/build-version.sh <version> --skip-signing --skip-verification` from `packages/neo-jetbrains/` so no signing secrets are needed. Add `--skip-clean` only when the user wants a faster incremental test build.
- **Typecheck**: `bun run typecheck` or `./gradlew typecheck` from `packages/neo-jetbrains/` — compiles all Kotlin sources including the generated API client. A cold pinned build downloads the pinned CLI release via `generateOpenApiSpec` and needs network access; Gradle-cached incremental runs skip the download. Repo CLI mode (`-Pneo.cli.pinned=false`) generates the spec from local source and bundles the staged local CLI binary.
- **Build local repo CLI for JetBrains dev**: `./gradlew :backend:buildRepoCli` from `packages/neo-jetbrains/` builds `packages/opencode/dist/@neocode/cli-<os>-<arch>/bin/`. `stageRepoCli` intentionally does not depend on this task; missing binaries fail with instructions instead of silently starting a slow CLI build.
- **Full build**: `bun run build` from `packages/neo-jetbrains/` (runs Gradle `buildPlugin`).
- **Gradle only**: `./gradlew buildPlugin` from `packages/neo-jetbrains/`.
...
- **Via Turbo**: `bun turbo build --filter=@neocode/neo-jetbrains` from repo root.
...
- JetBrains runtime behavior normally depends on the downloaded CLI release pinned by `packages/neo-jetbrains/package.json`; local `packages/opencode/` changes are used only with `neo.cli.pinned=false` repo CLI mode.
- If there are relevant server/API changes outside `packages/neo-jetbrains/`, warn the user that JetBrains may need a newly published/pinned CLI release and regenerated SDK artifacts.

## UI Guidelines

### Technology Choices

**Do not use Kotlin UI DSL v2 (`com.intellij.ui.dsl.builder`) in this plugin.** Use standard Swing with IntelliJ Platform components for all UI layout. The JetBrains modular template and some older sections of the codebase reference the DSL, but Neo should not introduce it.

**Do not use Kotlin Compose or `intellij.platform.compose` in this plugin.** The JetBrains modular template uses Compose for its demo tool window, but Neo should use standard Swing with IntelliJ Platform components only. Keep all plugin UI in the existing Swing-based stack.
...
**`UiStyle`** (`frontend/src/main/kotlin/ai/neocode/client/ui/UiStyle.kt`) is the single source of truth for reusable UI constants in this plugin.
...
**`SessionUiStyle`** (`frontend/src/main/kotlin/ai/neocode/client/session/ui/style/SessionUiStyle.kt`) owns static tokens specific to the chat/session UI:
...
| One-dimensional multi-component rows/columns | `ai.neocode.client.ui.layout.Stack` — see section below |
| Fluent platform panels | `JBPanel.withBorder(...)`, `.andTransparent()`, `.andOpaque()`, `.withBackground(...)` |
| Single-component alignment wrapper | `ai.neocode.client.ui.layout.Align` — see section below |

### Stack — One-Dimensional Multi-Component Layout

Use `Stack` (`ai.neocode.client.ui.layout.Stack`) when multiple Swing components should be laid out as one vertical column or one horizontal row without visual chrome. It is a transparent, no-border, no-color `JPanel(null)` that lays out visible children in insertion order.
...
- Register declaratively in module XML via `com.intellij.toolWindow` extension point (already done in `neo.jetbrains.frontend.xml`).
...
- Declare in module XML: `<notificationGroup id="Neo Code" displayType="BALLOON"/>`.
- Show: `Notification("Neo Code", "message", NotificationType.INFORMATION).notify(project)`.
...
- Tool-window-bound: `displayType="TOOL_WINDOW" toolWindowId="Neo Code"`.
...
- For compact icon-only actions, use `ai.neocode.client.ui.HoverIcon` so the control gets the standard 24×24 hover treatment. Do not create `JButton(icon)` or wrap a bare icon in a button just to make it clickable.
...
Settings UI has reusable primitives in `frontend/src/main/kotlin/ai/neocode/client/settings/base/`. Check these before adding new settings components or custom Swing assemblies.
...
- Keep settings UI on the EDT and continue using existing platform Swing components, `Stack`, `Align`, `UiStyle`, and localized `NeoBundle` strings according to the UI guidance above.
...
`frontend/src/main/kotlin/ai/neocode/client/session/`.
...
- After history load, `recoverPending()` seeds state in this priority order: (1) pending permission, (2) pending question, (3) current session status (`busy`/`retry`/`offline` from `NeoSessionService.statuses`), (4) `Idle`.
