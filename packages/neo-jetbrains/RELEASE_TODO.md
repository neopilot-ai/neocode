...
- Confirm `ai.neocode.jetbrains` is the final permanent Marketplace plugin ID before the first upload.
...
- Confirm the plugin is owned by the correct Neo vendor or organization.
...
- Confirm `NEO_MAINTAINER_APP_ID` and `NEO_MAINTAINER_APP_SECRET` are available to create release PRs and immediate release tags.
...
- Review and edit `packages/neo-jetbrains/CHANGELOG.md` in the generated release PR.
