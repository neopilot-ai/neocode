...
val generatedProps = layout.buildDirectory.dir("generated/neo-props")
val generatedCli = layout.buildDirectory.dir("generated/neo-cli-res")
val pinned = providers.gradleProperty("neo.cli.pinned").map { it.trim().toBoolean() }.orElse(true)
...
val writeNeoProperties by tasks.registering(WriteProperties::class) {
    description = "Write pinned Neo CLI properties"
    val out = generatedProps.map { it.file("neo.properties") }
...
val stageRepoCli by tasks.registering(StageRepoCliTask::class) {
    ...
    val bin = repoRootDir.dir("dist/@neocode/cli-${platform()}/bin")
    this.bin.set(bin)
    archive.set(generatedCli.map { it.file("neo-cli.zip") })
...
openApiGenerate {
    ...
    packageName.set("ai.neocode.jetbrains.api")
    apiPackage.set("ai.neocode.jetbrains.api.client")
    modelPackage.set("ai.neocode.jetbrains.api.model")
...
tasks.named("compileKotlin") {
    dependsOn(fixGeneratedApi, writeNeoProperties)
    if (repoCli.get()) dependsOn(stageRepoCli)
    inputs.dir(generatedApi)
}

tasks.named("processResources") {
    dependsOn(writeNeoProperties)
    if (repoCli.get()) dependsOn(stageRepoCli)
}
