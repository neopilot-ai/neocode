...
class NeoCliDownloaderTest {
    ...
    fun `downloads extracts and caches pinned cli`() = runBlocking {
        MockWebServer().use { server ->
            ...
            assertEquals(File(File(dir, "1.2.3"), NeoCliPlatform.current()).absolutePath, cli.parentFile.parentFile.absolutePath)
            assertEquals("#!/bin/sh\n", cli.readText())
            assertTrue(File(cli.parentFile, "neo-sandbox-mutation-worker.js").isFile)
            assertEquals("/api/v1.2.3", server.takeRequest().path)
            assertEquals("/release/v1.2.3/neo-${NeoCliPlatform.current()}.${NeoCliPlatform.archive()}", server.takeRequest().path)
            assertEquals(CliDownload(0, "1.2.3", NeoCliPlatform.current()), seen.first())
            assertTrue(seen.any { it.percent == 100 && it.version == "1.2.3" && it.platform == NeoCliPlatform.current() })
            assertTrue(
                log.messages.any {
                    it.startsWith("INFO: Neo CLI 1.2.3 for ${NeoCliPlatform.current()} is not cached; downloading new release into ") &&
                        it.contains("/.tmp/")
                }
            )
            assertTrue(log.messages.any { it.contains("Neo CLI path diagnostics:") && it.contains("cacheRoot=${dir.absolutePath}") })
            assertTrue(log.messages.any { it.contains("Neo CLI cache target:") && it.contains("exe=${cli.absolutePath}") })
            assertTrue(log.messages.any { it.contains("Neo CLI cache lock path:") && it.contains(File(dir, ".lock").canonicalPath) })

            val cachedProgress = mutableListOf<CliDownload>()
            val cached = NeoCliDownloader(
                log = log,
                root = dir,
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
            ...
            assertContains(log.messages, "INFO: Using cached Neo CLI 1.2.3 for ${NeoCliPlatform.current()} at ${cli.absolutePath}")

            File(cli.parentFile.parentFile, ".complete").writeText("ok\n")
            server.enqueue(metadata(bytes))
            server.enqueue(MockResponse().setResponseCode(200).setBody(Buffer().write(bytes)))
            val stale = NeoCliDownloader(
                log = log,
                root = dir,
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
    ...
    fun `prunes stale versions and removes the extracted archive`() = runBlocking {
        MockWebServer().use { server ->
            val stale = File(File(dir, "0.0.1"), NeoCliPlatform.current())
            ...
            val archived = File(cli.parentFile.parentFile, "neo-${NeoCliPlatform.current()}.${NeoCliPlatform.archive()}")
    ...
    fun `rejects cli archive with mismatched digest`() = runBlocking {
        MockWebServer().use { server ->
            ...
            assertFalse(File(File(File(dir, "1.2.3"), NeoCliPlatform.current()), ".complete").exists())
        }
    }
    ...
    fun `fails clearly and logs when the release has no matching asset`() = runBlocking {
        MockWebServer().use { server ->
            ...
            val name = "neo-${NeoCliPlatform.current()}.${NeoCliPlatform.archive()}"
    ...
    fun `fails clearly and logs when the asset has no digest`() = runBlocking {
        MockWebServer().use { server ->
            server.enqueue(
                MockResponse().setResponseCode(200).setBody(
                    """{"assets":[{"name":"neo-${NeoCliPlatform.current()}.${NeoCliPlatform.archive()}"}]}"""
                )
            )
            val log = TestLog()
            val ex = assertFailsWith<IllegalStateException> {
    ...
    fun `cache lock times out clearly when already held in this process`() = runBlocking {
        ...
        RandomAccessFile(file, "rw").channel.use { channel ->
            channel.lock().use {
                ...
                assertContains(ex.message.orEmpty(), "Timed out waiting for Neo CLI cache lock")
                assertContains(ex.message.orEmpty(), file.canonicalPath)
                assertTrue(log.messages.any { it.contains("Waiting for Neo CLI cache lock") && it.contains(file.canonicalPath) })
                assertTrue(log.messages.any { it.contains("Timed out waiting for Neo CLI cache lock") && it.contains(file.canonicalPath) })
            }
        }
    }
    ...
    private fun archive(script: String = "#!/bin/sh\n"): ByteArray {
        val files = mapOf(
            "bin/${NeoCliPlatform.exe()}" to script.toByteArray(),
            "bin/neo-sandbox-mutation-worker.js" to "worker\n".toByteArray(),
        )
        if (NeoCliPlatform.archive() == "zip") return zip(files)
        return tar(files)
    }
    ...
    private fun metadata(digest: String) = MockResponse().setResponseCode(200).setBody(
        """{"assets":[{"name":"neo-${NeoCliPlatform.current()}.${NeoCliPlatform.archive()}","digest":"$digest"}]}"""
    )
