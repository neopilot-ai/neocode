...
import ai.neocode.backend.workspace.NeoBackendWorkspace
...
class NeoBackendWorkspaceTest {

    private val mock = MockCliServer()
    private val log = TestLog()
    private val scope = CoroutineScope(SupervisorJob() + Dispatchers.Default)
    private val apps = mutableListOf<NeoBackendAppService>()
    ...
    private fun setup(): NeoBackendAppService =
        NeoBackendAppService.create(scope, FakeCliServer(mock), log).also { apps.add(it) }

    private suspend fun connect(app: NeoBackendAppService) {
        app.connect()
        ...
        assertIs<NeoAppState.Ready>(state, "App startup failed; logs=${log.messages}")
    }

    private suspend fun ready(app: NeoBackendAppService): NeoBackendWorkspace {
        connect(app)
        return app.workspaces.get("/test/project")
    }

    private suspend fun loaded(ws: NeoBackendWorkspace) {
        withTimeout(15_000) {
            ws.state.first { it is NeoWorkspaceState.Ready }
        }
    }
    ...
    fun `get creates workspace on demand after Ready`() = runBlocking {
        ...
        val state = ws.state.value as NeoWorkspaceState.Ready
    ...
    fun `workspaces stopped on app disconnect`() = runBlocking {
        ...
        assertIs<NeoWorkspaceState.Pending>(ws.state.value)
    ...
    fun `workspace reaches Ready after creation`() = runBlocking {
        ...
        assertIs<NeoWorkspaceState.Ready>(ws.state.value)
    }
    ...
    fun `providers failure retries then transitions to Error`() = runBlocking {
        ...
        withTimeout(15_000) {
            ws.state.first { it is NeoWorkspaceState.Error }
        }

        val err = ws.state.value as NeoWorkspaceState.Error
    ...
    // ------ Data mapping ------
    // Detailed provider/command/path parsing correctness is covered in NeoCliDataParserTest.
    ...
    fun `concurrent get for same directory returns same instance`() = runBlocking {
        val port = mock.start()
        val http = NeoBackendHttpClients.api(mock.password)
        val api = DefaultApi(basePath = "http://127.0.0.1:$port", client = http)
        val events = MutableSharedFlow<SseEvent>()
        val sessions = NeoBackendSessionManager(scope, log)
        val manager = NeoBackendWorkspaceManager(scope, sessions, log)
    ...
    fun `concurrent load calls on workspace produce valid final state`() = runBlocking {
        ...
        val state = withTimeout(15_000) {
            ws.state.first {
                it is NeoWorkspaceState.Ready &&
                    it.providers.providers.firstOrNull()?.id == "openai"
            }
        } as NeoWorkspaceState.Ready
    ...
    fun `SSE global disposed triggers full app reload with new data`() = runBlocking {
        ...
        val prev = (app.appState.value as NeoAppState.Ready).rev
        val before = mock.requestCount("/global/config")
        val reload = async(start = CoroutineStart.UNDISPATCHED) {
            app.appState.first { it is NeoAppState.Ready && it.rev > prev }
        }
