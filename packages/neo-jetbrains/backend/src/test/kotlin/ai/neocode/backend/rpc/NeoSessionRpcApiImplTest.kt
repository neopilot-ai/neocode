...
class NeoSessionRpcApiImplTest {

    @Test
    fun `events logs normal completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        val api = NeoSessionRpcApiImpl(log = log, source = flowOf(ChatEventDto.TurnOpen("ses_test")))
    ...
    fun `events logs cancelled completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        val api = NeoSessionRpcApiImpl(log = log, source = flow { kotlinx.coroutines.awaitCancellation() })
    ...
    fun `events logs failed completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        val api = NeoSessionRpcApiImpl(log = log, source = flow { throw IllegalStateException("stream failed") })
