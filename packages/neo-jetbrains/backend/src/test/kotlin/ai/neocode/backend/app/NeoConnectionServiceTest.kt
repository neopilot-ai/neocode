...
import ai.neocode.backend.app.ConnectionState
import ai.neocode.backend.app.NeoConnectionService
...
class NeoConnectionServiceTest {
    ...
    fun `connect provides API client`() = runBlocking {
        val svc = NeoConnectionService(scope, fake, {}, log)
        svc.connect()
        mock.awaitSseConnection()
    ...
    fun `connect reports downloading then connecting before ready`() = runBlocking {
        ...
        val svc = NeoConnectionService(scope, server, {}, log)
    ...
    fun `SSE concurrent callbacks preserve callback order`() = runBlocking {
        ...
        val svc = NeoConnectionService(scope, fake, {}, blocked)
        val field = NeoConnectionService::class.java.getDeclaredField("listener")
        ...
        val sourceField = NeoConnectionService::class.java.getDeclaredField("source")
    ...
    fun `init error transitions to Error state`() = runBlocking {
        ...
        val svc = NeoConnectionService(scope, failing, {}, log)
        svc.connect()
    ...
    // extractType tests moved to NeoCliDataParserTest
    ...
    fun `dispose prevents reconnect on late SSE callbacks`() = runBlocking {
        val reconnects = AtomicInteger(0)
        val svc = NeoConnectionService(scope, fake, { reconnects.incrementAndGet() }, log)
        svc.connect()
        mock.awaitSseConnection()
    ...
    private class BlockingLog : NeoLog {
