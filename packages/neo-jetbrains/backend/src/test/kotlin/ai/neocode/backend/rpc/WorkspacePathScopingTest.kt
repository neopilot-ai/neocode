...
class WorkspacePathScopingTest {
    ...
    private val base: Path = Path.of(System.getProperty("user.home")).resolve("neo-scope-test").normalize()
    ...
    @Test
    fun `workspace scope keeps normal project and neo plan files`() {
        assertEquals("backend/src/Main.java", relativeWithinWorkspace(base, at("backend", "src", "Main.java")))
        assertEquals(".neo/plans/x.md", relativeWithinWorkspace(base, at(".neo", "plans", "x.md")))
    }

    @Test
    fun `workspace scope rejects managed worktree storage from main checkout`() {
        assertNull(relativeWithinWorkspace(base, at(".neo", "worktrees")))
        assertNull(relativeWithinWorkspace(base, at(".neo", "worktrees", "foo", "backend", "src", "Main.java")))
    }

    @Test
    fun `workspace scope allows files inside the active worktree`() {
        val root = at(".neo", "worktrees", "foo")
    ...
    fun `workspace scope rejects sibling worktrees from active worktree`() {
        val root = at(".neo", "worktrees", "foo")
        val sibling = at(".neo", "worktrees", "bar", "backend", "src", "Main.java")
    ...
    fun `workspace scope rejects nested managed worktree storage`() {
        ...
        assertNull(relativeWithinWorkspace(root, root.resolve(".neo/worktrees/bar/backend/src/Main.java")))
    }
    ...
    fun `git changes returns capped large diff without blocking`() = runBlocking {
        ...
        try {
            ...
            val diff = assertNotNull(NeoWorkspaceRpcApiImpl().gitChanges(dir.toString()))
    ...
    fun `git changes returns null outside git repositories`() = runBlocking {
        val dir = Files.createTempDirectory("neo-non-git")
        try {
            assertNull(NeoWorkspaceRpcApiImpl().gitChanges(dir.toString()))
        } finally {
            delete(dir)
        }
    }
    ...
    private fun repo(): Path? {
        ...
        val dir = Files.createTempDirectory("neo-git")
        git(dir, "init")
