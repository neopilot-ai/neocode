...
class NeoBackendProviderSettingsManagerTest {
    ...
    @Test
    fun `disconnecting neo gateway returns error without logout`() = runBlocking {
        mock.providers = """{
            "all":[{"id":"neo","name":"Neo Gateway","source":"custom","models":{}}],
            "default":{},
            "connected":["neo"],
            "failed":[]
        }""".trimIndent()
        ...
        mock.resetCounts()
        val result = manager.disconnect(ProviderDisconnectDto("/test", "neo"))

        assertEquals("Neo Gateway cannot be disconnected from provider settings.", result.error)
        assertFalse(result.profileCleared)
        assertNull(mock.lastAuthDeletePath)
        assertEquals(0, mock.requestCount("/auth/neo"))
        assertEquals(0, mock.requestCount("/global/dispose"))
    }
    ...
    fun `state waits through dispose triggered reload`() = runBlocking {
        ...
        val app = app()
        val manager = NeoBackendProviderSettingsManager(app)
        ...
        try {
            ...
            withTimeout(5_000) {
                app.appState.first { it is NeoAppState.Loading }
            }
    ...
    fun `awaitReady fails fast when disconnected`() = runBlocking {
        val app = NeoBackendAppService.create(scope, FakeCliServer(mock), TestLog())
    ...
    private suspend fun manager(): NeoBackendProviderSettingsManager {
        return NeoBackendProviderSettingsManager(app())
    }

    private suspend fun app(): NeoBackendAppService {
        val app = NeoBackendAppService.create(scope, FakeCliServer(mock), TestLog())
        app.connect()
        withTimeout(10_000) {
            app.appState.first { it is NeoAppState.Ready }
        }
        return app
    }
}
