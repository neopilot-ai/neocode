...
class NeoProcessJobTest {

    /**
     * The kill-on-close job is Windows-only. Everywhere else (including CI) [NeoProcessJob.assign]
    ...
    fun `assign is a no-op on non-Windows platforms`() {
        ...
        assertNull(NeoProcessJob.assign(4321L, TestLog()))
    }
    ...
    fun `closing the job terminates a disposable child on Windows`() {
        ...
        try {
            val job = assertNotNull(NeoProcessJob.assign(child.pid(), TestLog()), "job assigned on Windows")
