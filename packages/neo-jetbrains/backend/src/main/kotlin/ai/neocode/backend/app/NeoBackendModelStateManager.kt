...
class NeoBackendModelStateManager(
    private val log: NeoLog,
) {
    companion object {
        private val DEFAULT_DIR = Path.of(System.getProperty("user.home"), ".local", "state", "neo")
    }
    ...
    suspend fun state(): ModelStateDto = mutex.withLock {
        NeoCliDataParser.parseModelState(read().orEmpty())
    }
    ...
    suspend fun favorite(update: ModelFavoriteUpdateDto): ModelStateDto = mutex.withLock {
        ...
        val state = NeoCliDataParser.parseModelState(raw.orEmpty())
        ...
        write(NeoCliDataParser.buildModelStateJson(raw, updated))
        updated
    }
    ...
    private fun resolve(): Path? {
        ...
        return try {
            http.newCall(request).execute().use { response ->
                ...
                val dir = NeoCliDataParser.parsePathState(raw)?.let(Path::of) ?: DEFAULT_DIR
