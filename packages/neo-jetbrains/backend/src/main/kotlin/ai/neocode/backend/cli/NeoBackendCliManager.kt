...
import ai.neocode.backend.dev.NeoDevMode
...
/**
 * Manages the Neo CLI binary lifecycle.
 *
 * Downloads the pinned CLI into IntelliJ's system directory,
 * spawns `neo serve --port 0`, and exposes the result as [State].
 *
 * Concurrency is handled by the owning [NeoBackendAppService] — all public
 * methods except [exited] are called under its mutex. [exited] is called from
 * [NeoConnectionService]'s IO dispatcher and is thread-safe via the stale-ref
 * guard and volatile [process] field.
 */
class NeoBackendCliManager(
    private val log: NeoLog = NeoLog.create(NeoBackendCliManager::class.java),
    ...
    @Volatile
    private var job: NeoProcessJob? = null
    ...
    private suspend fun resolveCli(onProgress: (CliDownload) -> Unit): File {
        ...
        forceExtract = false
        if (!NeoProps.pinned()) {
            if (force) log.info("Force re-extracting local repo CLI ${NeoProps.cliVersion()}")
            val cli = NeoRepoCli.extract(force)
            onProgress(CliDownload(100, NeoProps.cliVersion(), NeoCliPlatform.current()))
            return cli
        }
        if (force) log.info("Force re-downloading CLI ${NeoProps.cliVersion()}")
        return NeoCliDownloader(log = log).resolve(NeoProps.cliVersion(), force, onProgress)
    }
    ...
    private suspend fun spawn(cli: File, start: Long): CliServer.State =
        withContext(Dispatchers.IO) {
            ...
            log.info("CLI env: NEO_CLIENT=jetbrains NEO_PLATFORM=jetbrains NEO_APP_NAME=neo-code")
            val proc = try {
                builder.start()
            } catch (e: Exception) {
            ...
            // can never be orphaned. Null on other platforms / when unavailable — see NeoProcessJob.
            val jobHandle = NeoProcessJob.assign(proc.pid(), log)
            ...
            }, "neo-cli-stderr").apply { isDaemon = true; start() }
            this@NeoBackendCliManager.stderr = err
    ...
    private fun clearJob(): NeoProcessJob? = synchronized(lock) {
        val current = job
        job = null
        current
    }
 ...
 */
private fun confirmKilled(proc: Process, kids: List<ProcessHandle>, log: NeoLog, timeoutSeconds: Long) {
    val parentExited = proc.waitFor(timeoutSeconds, TimeUnit.SECONDS)
 ...
 * order deadlocked IDE shutdown; the order is locked by NeoBackendCliShutdownTest. Never waits, so
...
internal fun closeStreams(proc: Process, log: NeoLog) {
...
internal fun startupDiagnostics(cli: File, env: Map<String, String>, log: NeoLog): String {
    val home = System.getProperty("user.home").orEmpty()
    val profile = EnvironmentUtil.getValue("USERPROFILE").orEmpty()
    val data = env["XDG_DATA_HOME"] ?: home.takeIf { it.isNotBlank() }?.let { File(it, ".local/share/neo").absolutePath }.orEmpty()
    ...
    if (data.isNotBlank() && remote(data)) {
        lines += "warning: Neo CLI data dir appears to be on a non-local drive (${root(data)}); SQLite WAL may hang. Set XDG_DATA_HOME/XDG_STATE_HOME/XDG_CONFIG_HOME/XDG_CACHE_HOME to a local disk."
    }
    val text = lines.joinToString("\n")
    log.info("CLI startup diagnostics:\n$text")
    if (data.isNotBlank() && remote(data)) {
        log.warn("Neo CLI data dir appears to be on a non-local drive (${root(data)}); SQLite WAL may hang. Set XDG_DATA_HOME/XDG_STATE_HOME/XDG_CONFIG_HOME/XDG_CACHE_HOME to a local disk.")
    }
    return text
}
...
internal suspend fun awaitReady(
    ...
    onTimeout: () -> Unit,
    diagnostics: () -> String,
    log: NeoLog = NeoLog.create(NeoBackendCliManager::class.java),
    ...
    }, "neo-cli-stdout").apply { isDaemon = true; start() }
    onThread(thread)
...
internal fun buildNeoCliEnv(
    ...
    put("NEO_PARENT_PID", ProcessHandle.current().pid().toString())
    put("NEO_CLIENT", "jetbrains")
    put("NEO_ENABLE_QUESTION_TOOL", "true")
    put("NEO_PLATFORM", "jetbrains")
    put("NEO_APP_NAME", "neo-code")
    put("NEO_TELEMETRY_LEVEL", if (NeoDevMode.enabled()) "off" else "all")
    if (!NeoClaudeCompatSettings.get()) put("NEO_DISABLE_CLAUDE_CODE", "true")
    put("NEOCODE_FEATURE", "jetbrains-plugin")
    putIfAbsent("NEO_CONFIG_CONTENT", DEFAULT_CONFIG)
    ideEnv(log).forEach { entry -> put(entry.key, entry.value) }
    devStorageEnv(log)?.forEach { entry -> put(entry.key, entry.value) }
}

private fun ideEnv(log: NeoLog): Map<String, String> = buildMap {
    runCatching {
        val info = ApplicationInfo.getInstance()
        val name = info.fullApplicationName
        val build = info.build.asString()
        put("NEO_EDITOR_NAME", name)
        put("NEOCODE_EDITOR_NAME", "$name $build")
    }.onFailure { log.info("Could not read ApplicationInfo: ${it.message}") }

    runCatching {
        val version = NeoPlugin.version()
        if (version != null) put("NEO_APP_VERSION", version)
    }.onFailure { log.info("Could not read plugin version: ${it.message}") }

    runCatching {
        put("NEO_MACHINE_ID", machineId())
    }.onFailure { log.info("Could not read machine ID: ${it.message}") }
}

private fun machineId(): String {
    val file = File(PathManager.getSystemPath(), "neo/machine-id")
...
private fun devStorageEnv(log: NeoLog): Map<String, String>? {
    val enabled = System.getProperty("neo.dev.storage.isolated", "false").toBoolean()
    if (!enabled) return null
    val root = System.getProperty("neo.dev.worktree.root") ?: run {
        log.warn("neo.dev.storage.isolated=true but neo.dev.worktree.root is not set; skipping dev storage isolation")
        return null
    }
    val dev = File(root, ".neo-dev")
