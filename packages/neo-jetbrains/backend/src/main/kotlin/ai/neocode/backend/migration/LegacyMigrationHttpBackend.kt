...
/**
 * OkHttp implementation of [LegacyMigrationBackend] against `neo serve`.
 *
 ...
 * constraints getting in the way. Mirrors the pattern in NeoBackendChatManager.
 *
...
class LegacyMigrationHttpBackend(
    ...
    override fun importProject(project: JsonObject): String {
        ...
        val url = "$base/neocode/session-import/project?directory=${encode(dir)}"
    ...
    override fun importSession(session: JsonObject): LegacyImportResult {
        ...
        val url = "$base/neocode/session-import/session?directory=${encode(dir)}"
    ...
    override fun importMessage(message: JsonObject) {
        ...
        val url = "$base/neocode/session-import/message?sessionID=${encode(sessionId)}"
    ...
    override fun importPart(part: JsonObject) {
        ...
        val url = "$base/neocode/session-import/part?sessionID=${encode(sessionId)}"
