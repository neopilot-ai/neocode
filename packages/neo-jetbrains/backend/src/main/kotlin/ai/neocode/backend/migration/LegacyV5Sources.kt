...
class LegacyV5Sources(
    ...
    private val root = File(home, ".neocode")
    private val storage = File(root, "globalStorage")
    private val scoped = File(storage, "neo code.neo-code")

    fun secretsJson(): String? = File(root, "secrets.json").text("secrets")
    fun globalStateXml(): String? = File(config, "options/neocode-extension-storage.xml").text("globalStateXml")
