...
class NeoBackendTelemetry(
    private val log: NeoLog = NeoLog.create(NeoBackendTelemetry::class.java),
) {
    companion object {
    ...
    private fun base(): Map<String, JsonPrimitive> =
        NeoLog.payload(log).mapValues { JsonPrimitive(it.value) }
}
