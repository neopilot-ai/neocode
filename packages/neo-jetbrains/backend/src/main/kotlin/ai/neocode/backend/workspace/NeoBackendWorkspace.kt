 ...
 * [NeoBackendWorkspaceManager] for each directory. Receives a
 * pre-connected [DefaultApi] — no null checks needed.
 *
 * Session operations delegate to [NeoBackendSessionManager] with
 * this workspace's [directory], so the frontend only needs one
 * object per directory.
 */
class NeoBackendWorkspace(
    ...
    private val _state = MutableStateFlow<NeoWorkspaceState>(NeoWorkspaceState.Pending)
    val state: StateFlow<NeoWorkspaceState> = _state.asStateFlow()
    ...
    fun load() {
        synchronized(loadLock) {
            ...
            val progress = AtomicReference(NeoWorkspaceLoadProgress())
            _state.value = NeoWorkspaceState.Loading(progress.get())
            ...
            try {
                    ...
                    launch {
                        ...
                        if (result.value != null) {
                            ...
                            progress.updateAndGet { it.copy(providers = true) }
                                .also { _state.value = NeoWorkspaceState.Loading(it) }
                        } else {
                ...
                _state.value = NeoWorkspaceState.Ready(
                    providers = prov!!,
                    agents = ag!!,
                    commands = cmd!!,
                    skills = sk!!,
    ...
    fun stop() {
        ...
        _state.value = NeoWorkspaceState.Pending
    }
    ...
    private suspend fun fetchProviders(): FetchResult<ProviderData> = withContext(Dispatchers.IO) {
        try {
            FetchResult.ok(NeoCliDataParser.parseProviders(fetch("/provider?directory=${encode(directory)}")))
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
    ...
    private suspend fun fetchCommands(): FetchResult<List<CommandInfo>> = withContext(Dispatchers.IO) {
        try {
            FetchResult.ok(NeoCliDataParser.parseCommands(fetch("/command?directory=${encode(directory)}")))
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
    ...
    private fun setWorkspaceError(message: String, errors: List<LoadError>) {
        ...
        _state.value = NeoWorkspaceState.Error(message, errors)
    }
