 ...
 *
 * Concurrency is handled by the owning [NeoBackendAppService] — `connect`,
 * `restart`, and `reinstall` are called under its mutex. Internal reconnect
 * attempts delegate back to the owner via [onReconnect].
 *
 * Not a service — owned and instantiated by [NeoBackendAppService].
 */
class NeoConnectionService(
  ...
  private val appLoadTimeoutMs: Long,
    ...
    constructor(
      ...
      server: CliServer,
      onReconnect: () -> Unit,
    ) : this(cs, server, onReconnect, NeoLog.create(NeoConnectionService::class.java), 30_000L)
     ...
     *
     * Called under [NeoBackendAppService]'s mutex — no internal guard needed.
     */
    suspend fun connect() {
        open()
    }
     ...
     *
     * Called under [NeoBackendAppService]'s mutex.
     */
    suspend fun restart() {
    ...
    private suspend fun open() {
        ...
        val ac = NeoBackendHttpClients.api(password)
        val lc = NeoBackendHttpClients.appLoad(password, appLoadTimeoutMs)
        val hc = NeoBackendHttpClients.health(password)
        apiClient = ac
        appLoadClient = lc
        healthClient = hc
    ...
    private val listener = object : EventSourceListener() {
        ...
        override fun onEvent(src: EventSource, id: String?, type: String?, data: String) {
            ...
            synchronized(lock) {
                ...
                lastEvent.set(System.currentTimeMillis())
                val kind = type ?: NeoCliDataParser.extractEventType(data)
     ...
     * goes through [NeoBackendAppService]'s mutex for a full restart.
     */
    ...
    private fun close() {
        api = null
        appLoadApi = null
        apiClient?.let { NeoBackendHttpClients.shutdown(it) }
        apiClient = null
        appLoadClient?.let { NeoBackendHttpClients.shutdown(it) }
        appLoadClient = null
        healthClient?.let { NeoBackendHttpClients.shutdown(it) }
        healthClient = null
    }
    ...
    fun dispose() {
        ...
        log.info("NeoConnectionService disposed")
    }
}
