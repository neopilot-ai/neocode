package ai.neocode.backend.provider
...
import ai.neocode.backend.rpc.NeoWorkspaceDtoMapper
...
internal class NeoBackendProviderSettingsManager(
    private val app: NeoBackendAppService,
) {
    companion object {
        private val LOG = NeoLog.create(NeoBackendProviderSettingsManager::class.java)
        private val JSON = "application/json".toMediaType()
        private val FETCH by lazy { NeoBackendHttpClients.modelFetch() }
    ...
    suspend fun state(directory: String): ProviderSettingsDto {
        ...
        val providers = load("providers", errors) {
            NeoCliDataParser.parseProviderSettingsProviders(get("/provider?directory=${enc(directory)}"))
        }
        val auth = load("provider_auth", errors) {
            NeoCliDataParser.parseProviderAuth(get("/provider/auth?directory=${enc(directory)}"))
        } ?: emptyMap()
    ...
    suspend fun connect(input: ProviderConnectDto): ProviderActionResultDto {
        val body = NeoCliDataParser.buildProviderAuthJson(input.key, input.metadata)
    ...
    suspend fun authorize(input: ProviderOAuthAuthorizeDto): ProviderOAuthReadyDto {
        val body = NeoCliDataParser.buildProviderOAuthJson(input.method, input.inputs)
        val raw = post("/provider/${enc(input.providerId)}/oauth/authorize?directory=${enc(input.directory)}", body, OAUTH_CALL_TIMEOUT_SECONDS)
        val parsed = NeoCliDataParser.parseOAuthReady(raw)
    ...
    suspend fun callback(input: ProviderOAuthCallbackDto): ProviderActionResultDto {
        val body = NeoCliDataParser.buildProviderOAuthJson(input.method, code = input.code)
    ...
    suspend fun disconnect(input: ProviderDisconnectDto): ProviderActionResultDto {
        ...
        if (input.providerId == "neo") {
            return ProviderActionResultDto(current, error = "Neo Gateway cannot be disconnected from provider settings.")
        }
        ...
        if (cfg?.npm == "@ai-sdk/openai-compatible") {
            patch(input.directory, cfg.scope, NeoCliDataParser.buildCustomProviderDeletePatch(input.providerId))
        ...
        if (provider?.source == "config") {
            ...
            patch(input.directory, scope, NeoCliDataParser.buildDisabledProviderPatch(ids))
            dispose()
            return ProviderActionResultDto(state(input.directory))
        }
        deleteAuth(input.providerId)
    ...
    suspend fun enable(input: ProviderEnableDto): ProviderActionResultDto {
        ...
        scopes.distinct().forEach { scope ->
            patch(input.directory, scope, NeoCliDataParser.buildDisabledProviderPatch(disabledFor(current, scope).filter { it != input.providerId }))
        }
        dispose()
        return ProviderActionResultDto(state(input.directory))
    }
    ...
    suspend fun saveCustom(input: CustomProviderSaveDto): ProviderActionResultDto {
        ...
        patch(NeoCliDataParser.buildCustomProviderPatch(input))
        if (input.envVar.isNullOrBlank()) {
            val key = input.apiKey?.takeIf { it.isNotBlank() }
            if (key != null) put("/auth/${enc(input.id)}", NeoCliDataParser.buildProviderAuthJson(key, emptyMap()))
        } else {
            deleteAuth(input.id)
        }
        dispose()
    ...
    suspend fun fetch(input: CustomModelFetchDto): CustomModelFetchResultDto {
        ...
        return try {
            ...
            CustomModelFetchResultDto(NeoCliDataParser.parseModelIds(raw))
        } catch (e: Exception) {
    ...
    private suspend fun <T> load(resource: String, errors: MutableList<LoadErrorDto>, block: suspend () -> T): T? {
        ...
        return try {
            ...
            errors.add(NeoWorkspaceDtoMapper.error(LoadError(resource = resource, detail = e.message)))
            null
        }
    }
    ...
    private fun parsed(raw: String): ParsedConfig {
        val result = NeoCliDataParser.parseProviderConfig(raw)
    ...
    private suspend fun request(request: Request, timeoutSeconds: Long = CALL_TIMEOUT_SECONDS): String {
        ...
        val http = app.http?.let { NeoBackendHttpClients.bounded(it, timeoutSeconds) }
            ?: throw IllegalStateException("Neo HTTP client is unavailable")
