 ...
 * or worktree). Each call routes to a [NeoBackendWorkspace]
 * via the workspace manager.
 */
@Rpc
interface NeoWorkspaceRpcApi : RemoteApi<Unit> {
    companion object {
        suspend fun getInstance(): NeoWorkspaceRpcApi {
            return RemoteApiProviderService.resolve(remoteApiDescriptor<NeoWorkspaceRpcApi>())
        }
    }
    ...
    suspend fun state(directory: String): Flow<NeoWorkspaceStateDto>
