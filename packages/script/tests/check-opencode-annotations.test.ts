...
function isExempt(file: string) {
  ...
  if (norm.split("/").some((part) => part.includes("neocode") || part.startsWith("neo-"))) return true
...
const MARKER_PREFIX = /(?:\/\/|\{?\s*\/\*|#)\s*neocode_change\b/
...
function coveredLines(text: string): Set<number> {
  ...
  if (first?.match(/(?:\/\/|\{?\s*\/\*|#)\s*neocode_change\s*-\s*new\s*file\b/)) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/(?:\/\/|\{?\s*\/\*|#)\s*neocode_change\s+start\b/)) {
      block = true
      covered.add(n)
      continue
    }

    if (line.match(/(?:\/\/|\{?\s*\/\*|#)\s*neocode_change\s+end\b/)) {
      covered.add(n)
      block = false
      continue
    }
...
describe("hasMarker", () => {
  const cases: Array<[string, boolean]> = [
    ...
    ["// neocode_change", true],
    ["  // neocode_change", true],
    ["const x = 1 // neocode_change", true],
    ["// neocode_change start", true],
    ["// neocode_change end", true],
    ["// neocode_change - new file", true],
    ["//   neocode_change", true],
    ["// neocode_change  ", true],
    ...
    ["{/* neocode_change end */}", true],
    ["{/* neocode_change - new file */}", true],
    ["{/* neocode_change - NeoNews added */}", true],
    ["{/*   neocode_change */}", true],
    ["{/* neocode_change  */}", true],
    ...
    ["# neocode_change", true],
    ["  # neocode_change", true],
    ["name: test # neocode_change", true],
    ['name = "zed" # neocode_change', true],
    ['export FOO="bar" # neocode_change', true],
    ["# neocode_change start", true],
    ["# neocode_change end", true],
    ...
    ["// neocode_changes", false],
    ["// neocode_changelog", false],
    ["/* neocode_change_log */", false],
    ["{/* neocode_changes */}", false],
    ["// neocode_changeable", false],
    ["", false],
    ["  ", false],
  ]
...
describe("isExempt", () => {
  const cases: Array<[string, boolean]> = [
    // exempt — "neocode" in path
    ["packages/opencode/src/neocode/foo.ts", true],
    ["packages/opencode/test/neocode/bar.test.ts", true],
    ["packages/opencode/src/some/neocode/deep/path.ts", true],
    ["packages/opencode/src/neocode/deep/nested/file.tsx", true],
    ["packages/opencode/src/neo-sessions/session.ts", true],
    ["packages/neo-ui/src/components/icon.tsx", true],
    ["packages/neo-vscode/src/extension.ts", true],
    ...
    // exempt — "neocode" in filename
    ["packages/opencode/src/foo/neocode.ts", true],
    ["packages/opencode/src/bar/neocode.test.ts", true],
    ["packages/opencode/src/file.neocode.ts", true],
    // exempt — case-insensitive
    ["packages/opencode/src/neocode/foo.ts", true],
    ["packages/opencode/src/NEOCODE/bar.ts", true],
    // NOT exempt
    ...
    // neocode_change is not the same as neocode
...
describe("isChecked", () => {
  const cases: Array<[string, boolean]> = [
    ...
    ["packages/neo-ui/src/components/icon.tsx", false],
    ["packages/neo-vscode/src/extension.ts", false],
...
describe("coveredLines", () => {
  ...
  test("whole-file JS annotation", () => {
    const covered = coveredLines("// neocode_change - new file\nexport const x = 1\nexport const y = 2")
  ...
  test("bare /* */ block markers", () => {
    const text = ["/* neocode_change start */", "const b = 2", "/* neocode_change end */"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("YAML block markers", () => {
    const text = ["# neocode_change start", "name: test", "# neocode_change end"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("TOML block markers", () => {
    const text = ["# neocode_change start", 'id = "opencode"', "# neocode_change end"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("shell block markers", () => {
    const text = ["# neocode_change start", "set -euo pipefail", "# neocode_change end"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("inline JS marker covers only that line", () => {
    const text = ["const a = 1", "const b = 2 // neocode_change", "const c = 3"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([2]))
  })

  test("inline JSX marker covers only that line", () => {
    const text = ["const a = 1", "{/* neocode_change */}", "const c = 3"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([2]))
  })

  test("inline JS marker with code on same line", () => {
    const text = "const url = Flag.NEO_MODELS_URL || 'https://models.dev' // neocode_change\n"
    const covered = coveredLines(text)
  ...
  test("JSX block marker with descriptive suffix", () => {
    const text = [
      "{/* neocode_change start - Neo-specific error display */}",
      "<ErrorDisplay />",
      "{/* neocode_change end */}",
    ].join("\n")
    const covered = coveredLines(text)
  ...
  test("marker line with extra text after marker is still covered", () => {
    const text = [
      "const a = 1",
      "// neocode_change start - this is neo specific",
      "const b = 2",
      "// neocode_change end",
    ].join("\n")
    const covered = coveredLines(text)
...
describe("checkLine (main loop simulation)", () => {
  ...
  test("covered line reports no violation", () => {
    const text = ["// neocode_change start", "const neo = 1", "// neocode_change end"].join("\n")
  ...
  test("marker lines are skipped even if uncovered", () => {
    ...
    const text = ["{/* neocode_change */}", "{/* neocode_change start */}"].join("\n")
  ...
  test("real-world TSX permission.tsx inline pattern", () => {
    ...
    expect(check(text, [2, 4])).toEqual(["line 2: <PermissionDeniedCard />", "line 4: <AnotherNeoComponent />"])
    expect(check(text, [1, 3])).toEqual([])
  })

  test("JS-style session/index.tsx pattern (from existing codebase)", () => {
    const text = ["const foo = 1", "<Toast />", "{/* neocode_change */}", "<Footer />", "</box>"].join("\n")
  ...
  test("whole-file annotated file — no violations even for unmarked lines", () => {
    const text = [
      "// neocode_change - new file",
      "export const neoFeature = true",
      "export const alsoNeo = 123",
...
// and a flag indicating whether the diff removes any neocode_change marker
// (i.e. the change is reverting Neo modifications back to upstream).
...
describe("parseDiff (revert detection)", () => {
  ...
  test("revert: hunk removes neocode_change marker block and adds upstream original", () => {
    ...
    const diff = [
      ...
      "+++ b/test.ts",
      "@@ -16,3 +16 @@ describe(...)",
      "-  // neocode_change start - TODO: skip flaky test",
      "-  test.skip('foo', async () => {",
      "-    // neocode_change end",
      "+  test('foo', async () => {",
    ].join("\n")
  ...
  test("revert: inline marker removed, upstream original added", () => {
    const diff = [
      "diff --git a/test.ts b/test.ts",
      "@@ -5 +5 @@",
      "-const url = Flag.X || 'fallback' // neocode_change",
      "+const url = Flag.X",
    ].join("\n")
    const out = parseDiff(diff)
  ...
  test("file-level revert: marker removed in one hunk covers other hunks", () => {
    // Mirrors the prompt.test.ts case from PR #9908: neocode_change marker
    // is removed in hunk A, while a separate hunk B replaces references that
    // depended on the removed Neo construct.
    const diff = [
      "diff --git a/test.ts b/test.ts",
      "@@ -218 +217,0 @@",
      "-const unixSkip = it.live.skip // neocode_change - skip flaky tests",
      "@@ -1589 +1583 @@ unixSkip(",
      "-unixSkip(",
      "+unix(",
    ].join("\n")
  ...
  test("multiple neocode_change start/end markers removed across hunks", () => {
    const diff = [
      "diff --git a/test.ts b/test.ts",
      "@@ -1432,2 +1431 @@",
      "-// neocode_change start - flaky on Linux CI",
      "-unixSkip(",
      "+unix(",
      "@@ -1469 +1466,0 @@",
      "-// neocode_change end",
  ...
  test("YAML/shell marker removal also triggers revert", () => {
    const diff = [
      ...
      "@@ -10 +10 @@",
      "-      - uses: actions/checkout@v6 # neocode_change",
...
describe("MARKER_PREFIX regex edge cases", () => {
  test("handles { followed immediately by /*", () => {
    expect(hasMarker("{/* neocode_change */}")).toBe(true)
  })
  ...
  test("does not match {/* without neocode_change", () => {
  ...
  test("does not match neocode_changes (word boundary)", () => {
    expect(hasMarker("// neocode_changes")).toBe(false)
    expect(hasMarker("// neocode_changelog")).toBe(false)
    expect(hasMarker("{/* neocode_changes */}")).toBe(false)
    expect(hasMarker("// neocode_changeable")).toBe(false)
  })
})
...
describe("isExempt — Windows backslash paths", () => {
  test("Windows paths with backslashes", () => {
    expect(isExempt("packages\\opencode\\src\\neocode\\foo.ts")).toBe(true)
    expect(isExempt("packages\\opencode\\test\\neocode\\bar.test.ts")).toBe(true)
...
describe("coveredLines — additional patterns", () => {
  test("block with descriptive suffix is still recognized", () => {
    const text = [
      "{/* neocode_change start - Neo-specific indexing display */}",
      "<IndexingStatus />",
      "{/* neocode_change end */}",
    ].join("\n")
    const covered = coveredLines(text)
  ...
  test("empty file content", () => {
    const covered = coveredLines("// neocode_change start\n  \n// neocode_change end")
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("multiple separate JS inline markers", () => {
    const text = [
      "const a = 1 // neocode_change",
      "const b = 2",
      "const c = 3 // neocode_change",
      "const d = 4",
    ].join("\n")
    const covered = coveredLines(text)
  ...
  test("consecutive block markers (no content)", () => {
    const text = ["// neocode_change start", "// neocode_change end"].join("\n")
    const covered = coveredLines(text)
  ...
  test("trailing empty line after block end is not covered", () => {
    const text = "// neocode_change start\nconst a = 1\n// neocode_change end\n\n"
    const covered = coveredLines(text)
...
describe("checkLine — additional patterns", () => {
  ...
  test("real-world TUI routes — line between marker and code should be uncovered", () => {
    ...
    const text = ["{/* neocode_change start */}", "", "<NeoIndexing />", "", "{/* neocode_change end */}"].join("\n")
  ...
  test("end marker on same line as content is covered", () => {
    const text = "const a = 1\n{/* neocode_change end */} // block already closed, still covered\n"
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([2]))
  })
