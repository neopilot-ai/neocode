...
describe("CodeIndexConfigManager", () => {
  ...
  test("configures Neo with hosted auth options and explicit model metadata", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "neo",
        openAiKey: undefined,
        neoApiKey: "neo-token",
        neoBaseUrl: "https://example.test/api/gateway/",
    ...
    expect(cfg.isFeatureConfigured).toBe(true)
    expect(cfg.getConfig().neoOptions).toEqual({
      apiKey: "neo-token",
  ...
  test("requires Neo model metadata from Cloud config", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "neo",
        openAiKey: undefined,
  ...
  test("uses configured dimension for Neo models outside the fallback catalog", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "neo",
        openAiKey: undefined,
  ...
  describe("loadConfiguration restart checks", () => {
    ...
    test("requires restart when Neo auth changes", () => {
      const cfg = new CodeIndexConfigManager(
        createInput({
          embedderProvider: "neo",
          openAiKey: undefined,
