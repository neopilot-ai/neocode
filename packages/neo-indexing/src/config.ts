...
export const IndexingConfig = z
  .object({
    ...
    neo: z
      ...
      .strict()
      .optional()
      .describe("Neo-hosted embedding provider options"),
    openai: z
...
export const IndexingSchema = Schema.Struct({
  ...
  ).annotate({ description: "Neo-hosted embedding provider options" }),
...
export function toIndexingConfigInput(cfg: IndexingConfig | undefined): IndexingConfigInput {
  ...
  return {
    ...
    neoApiKey: cfg?.neo?.apiKey,
    neoBaseUrl: cfg?.neo?.baseUrl,
    neoOrganizationId: cfg?.neo?.organizationId,
