...
import { NeoEmbedder } from "./embedders/neo"
...
export class CodeIndexServiceFactory {
  ...
  public createEmbedder(): IEmbedder {
    ...
    if (provider === "neo") {
      if (!config.neoOptions?.apiKey) throw new Error("Neo API key is required for embedding.")
      if (!config.modelId) throw new Error("Neo embedding model is required.")
      return new NeoEmbedder({
        apiKey: config.neoOptions.apiKey,
        baseUrl: config.neoOptions.baseUrl,
        organizationId: config.neoOptions.organizationId,
