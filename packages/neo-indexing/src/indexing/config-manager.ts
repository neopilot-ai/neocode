...
export class CodeIndexConfigManager {
  ...
  private neoOptions?: { apiKey: string; baseUrl?: string; organizationId?: string }
  ...
  private applyInput(input: IndexingConfigInput): void {
    ...
    this.neoOptions = input.neoApiKey
      ? { apiKey: input.neoApiKey, baseUrl: input.neoBaseUrl, organizationId: input.neoOrganizationId }
      : undefined
  ...
  private captureSnapshot(): PreviousConfigSnapshot {
    return {
      ...
      neoApiKey: this.neoOptions?.apiKey ?? "",
      neoBaseUrl: this.neoOptions?.baseUrl ?? "",
      neoOrganizationId: this.neoOptions?.organizationId ?? "",
  ...
  public isConfigured(): boolean {
    ...
    if (provider === "neo")
      return !!(this.neoOptions?.apiKey && this.modelId && this.currentModelDimension && hasStore)
  ...
  doesConfigChangeRequireRestart(prev: PreviousConfigSnapshot): boolean {
    ...
    // Auth changes
    if ((prev.neoApiKey ?? "") !== (this.neoOptions?.apiKey ?? "")) return true
    if ((prev.neoBaseUrl ?? "") !== (this.neoOptions?.baseUrl ?? "")) return true
    if ((prev.neoOrganizationId ?? "") !== (this.neoOptions?.organizationId ?? "")) return true
