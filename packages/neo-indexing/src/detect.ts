export const INDEXING_PLUGIN_NAMES = ["neo-indexing", "@neocode/neo-indexing"] as const
...
function isPathSpecifier(value: string): boolean {
  ...
  if (normalized.includes("/.opencode/") || normalized.includes("/.neo/") || normalized.includes("/.neocode/")) {
    return true
  }
...
function normalizePath(value: string): string {
  ...
  const scoped = parts.findIndex((part, i) => part === "@neocode" && parts[i + 1] === "neo-indexing")
  if (scoped >= 0) return "@neocode/neo-indexing"

  const workspace = parts.findIndex((part, i) => part === "packages" && parts[i + 1] === "neo-indexing")
  if (workspace >= 0) return "@neocode/neo-indexing"
