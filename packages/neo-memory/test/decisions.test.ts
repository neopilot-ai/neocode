...
describe("memory marker metadata", () => {
  ...
  test("decodes legacy metadata that only carries sources", () => {
    expect(
      MemoryMarkerMeta.fromParts([
        { type: "text", metadata: { neoMemory: { type: "startup", tokens: 3, sources: ["project.md"] } } },
      ]),
    ).toEqual({
      type: "startup",
      tokens: 3,
  ...
  test("persists recall snippets only in verbose mode", () => {
    ...
    expect(MemoryMarkerMeta.metadata(marker).neoMemory).not.toHaveProperty("items")
    expect(MemoryMarkerMeta.metadata(marker, true).neoMemory).toHaveProperty("items", ["private memory"])
  })
