...
import { NeoMemory } from "../src/effect/index"
...
async function tmp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "neo-memory-effect-"))
  return {
...
describe("MemoryCapture (fake ports)", () => {
  test("turn-close typed LLM saves environment memory and audit records", async () => {
    const t = await tmp()
    try {
      await NeoMemory.enable({ root: t.root })
      await NeoMemory.configure({ root: t.root, settings: { autoConsolidate: true } })
      ...
      const shown = await NeoMemory.show({ root: t.root })
  ...
  test("auto-consolidate off skips digest and typed model writes", async () => {
    ...
    try {
      await NeoMemory.enable({ root: t.root })
      await NeoMemory.configure({ root: t.root, settings: { autoConsolidate: false } })
...
describe("MemoryTimers signal ref-counting", () => {
  test("shares one controller per root and drops it once the last capture releases", () => {
    const root = "/neo-memory/ref-count-root"
