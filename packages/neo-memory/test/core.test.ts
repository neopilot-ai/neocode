...
async function tmp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "neo-memory-"))
  return {
    dir,
...
describe("memory core package", () => {
  ...
  test("uses unicode-safe project and memory identifiers", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({
        root: t.root,
        key: "設定_é",
        text: "日本語の設定は packages/neo-vscode に保存します。",
      })
      const shown = await Memory.show({ root: t.root })

      expect(id.display).toBe("proyecto_ñ_日本")
      expect(id.folder).toContain("proyecto_ñ_日本-")
      expect(shown.sources.project).toContain("- 設定_é :: 日本語の設定は packages/neo-vscode に保存します。")
  ...
  test("targeted recall dedupes lower-value session hits when typed memory answers", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({
        root: t.root,
        key: "memory_tests",
        text: "Run memory tests from packages/opencode with bun test ./test/neocode/memory.",
      })
    ...
    await use(async (t) => {
      ...
      await Memory.apply({
        ...
        ops: [
          ...
          { action: "add", key: "設定", text: "日本語の設定は packages/neo-vscode に保存します。" },
        ],
      })
    ...
    await use(async (t) => {
      ...
      await MemoryFiles.writeSource(
        ...
        [
          ...
          "- vscode_unit_tests :: Run VS Code memory unit tests from packages/neo-vscode with bun test tests/unit/memory-command.test.ts.",
          ...Array.from(
            { length: 40 },
            (_, idx) =>
  ...
  test("index suppresses bulk session digests covered by typed memory", async () => {
    await use(async (t) => {
      ...
      // ...but a compact pointer keeps its id targetable via neo_memory_recall mode=digest.
