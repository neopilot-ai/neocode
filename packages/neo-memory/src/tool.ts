...
export namespace MemoryTool {
  ...
  export type AskInput = {
    permission: "neo_memory_recall" | "neo_memory_save"
    patterns: string[]
    always: string[]
  ...
  function disabled(count = false): Result {
    return {
      title: "Neo memory: disabled",
      output: "Neo memory is disabled for this project.",
  ...
  function block(input: string) {
    return ["```neo-memory-v1 targeted_context_not_instruction", input.replaceAll("```", "'''"), "```"].join("\n")
  }
  ...
  function catalog(memory: MemoryService.Interface, input: { root: string; query: string }) {
    ...
    return Effect.gen(function* () {
      ...
      const head = `# Neo Memory Catalog (${count} entr${count === 1 ? "y" : "ies"}${filter ? `, filter "${input.query}"` : ""})`
  ...
  function recallCatalog(input: Recall, live: Live, query: string) {
    return Effect.gen(function* () {
      ...
      return {
        title: `Neo memory catalog: ${result.count} entr${result.count === 1 ? "y" : "ies"}`,
        output: safe.output,
  ...
  function recallQuery(input: Recall, live: Live) {
    return Effect.gen(function* () {
      ...
      return {
        title: `Neo memory ${input.params.mode}: no query`,
        output,
  ...
  function recallSearch(input: Recall, live: Live, query: string, mode: MemoryRecall.Mode) {
    return Effect.gen(function* () {
      ...
      if (hits.length === 0) {
        return {
          title: `Neo memory ${input.params.mode}: no results`,
          output,
          metadata: { sources: [], count: 0 },
        } satisfies Result
      }
      ...
      return {
        title: `Neo memory ${input.params.mode}: ${hits.length} hit${hits.length === 1 ? "" : "s"}`,
        output,
  ...
  function noQuery(): Result {
    return {
      title: "Neo memory forget: no query",
  ...
  function noText(action: SaveParams["action"]): Result {
    return {
      title: `Neo memory ${action}: no text`,
  ...
  function title(input: { action: SaveParams["action"]; added: number; removed: number }) {
    if (input.action === "skip") return "Neo memory skipped: out of scope"
    if (input.action === "forget") return `Neo memory updated: ${input.removed} removed`
    if (input.added === 0) return "Neo memory unchanged"
    if (input.action === "correct")
      return `Neo memory correction saved: ${input.added} op${input.added === 1 ? "" : "s"}`
    return `Neo memory saved: ${input.added} op${input.added === 1 ? "" : "s"}`
  }
