...
type ConfigureInput = NeoMemory.Input & {
  settings: Partial<Pick<MemorySchema.State, "autoConsolidate" | "verbose">>
}

type ApplyInput = NeoMemory.Input & {
...
type RememberInput = NeoMemory.Input & {
  text: string
  key?: string
...
type CorrectInput = NeoMemory.Input & {
  text: string
  key?: string
}

type ForgetInput = NeoMemory.Input & {
  query: string
}

type RecallInput = NeoMemory.Input & {
  query: string
  sessionID?: string
}
...
type RecordInput = NeoMemory.Input & {
  sessionID: string
  topic?: string
  summary: string
  time?: number
...
export namespace MemoryService {
  ...
  export interface Interface {
    readonly prepare: (input: NeoMemory.Input) => Effect.Effect<string, Failure>
    readonly status: (input: NeoMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.status>>, Failure>
    readonly show: (input: NeoMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.show>>, Failure>
    readonly enable: (input: NeoMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.enable>>, Failure>
    readonly disable: (
      input: NeoMemory.Input,
    ) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.disable>>, Failure>
    readonly rebuild: (
      input: NeoMemory.Input,
    ) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.rebuild>>, Failure>
    readonly configure: (
      input: ConfigureInput,
    ) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.configure>>, Failure>
    readonly apply: (input: ApplyInput) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.apply>>, Failure>
    readonly remember: (input: RememberInput) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.remember>>, Failure>
    readonly correct: (input: CorrectInput) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.correct>>, Failure>
    readonly forget: (input: ForgetInput) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.forget>>, Failure>
    readonly purge: (input: NeoMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.purge>>, Failure>
    readonly recall: (input: RecallInput) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.recall>>, Failure>
    ...
    readonly recordSession: (
      input: RecordInput,
    ) => Effect.Effect<Awaited<ReturnType<typeof NeoMemory.recordSession>>, Failure>
  ...
  export class Service extends Context.Service<Service, Interface>()("@neocode/MemoryService") {}

  export function make() {
    const locks = new Map<SessionID, { sema: Semaphore.Semaphore; holders: number }>()
    let settle = IDLE_SETTLE_MS
    return Service.of({
      prepare: (input) => bridge(() => NeoMemory.prepare(input)),
      status: (input) => bridge(() => NeoMemory.status(input)),
      show: (input) => bridge(() => NeoMemory.show(input)),
      enable: (input) => bridge(() => NeoMemory.enable(input)),
      disable: (input) => bridge(() => NeoMemory.disable(input)),
      rebuild: (input) => bridge(() => NeoMemory.rebuild(input)),
      configure: (input) => bridge(() => NeoMemory.configure(input)),
      apply: (input) => bridge(() => NeoMemory.apply(input)),
      remember: (input) => bridge(() => NeoMemory.remember(input)),
      correct: (input) => bridge(() => NeoMemory.correct(input)),
      forget: (input) => bridge(() => NeoMemory.forget(input)),
      purge: (input) => bridge(() => NeoMemory.purge(input)),
      recall: (input) => bridge(() => NeoMemory.recall(input)),
      search: (input) => bridge(() => MemoryRecall.search(input)),
      recordSession: (input) => bridge(() => NeoMemory.recordSession(input)),
