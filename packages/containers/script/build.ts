...
const reg = process.env.REGISTRY ?? "ghcr.io/neopilot-ai" // neocode_change
...
const images = ["base", "bun-node", "jetbrains", "rust", "tauri-linux", "publish"] // neocode_change

const setup = async () => {
  if (!push) return
  const list = await $`docker buildx ls`.text()
  // neocode_change start
  if (list.includes("neo")) {
    await $`docker buildx use neo`
    return
  }
  await $`docker buildx create --name neo --use`
  // neocode_change end
}
