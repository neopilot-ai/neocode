export const dict = {
  ...
  "provider.connect.neoGateway.line1":
    "Neo Gatewayは、コーディングエージェント向けに厳選された信頼性の高い最適化モデルへのアクセスを提供します。",
  "provider.connect.neoGateway.line2": "1つのAPIキーで、Claude、GPT、Gemini、GLMなどのモデルにアクセスできます。",
  "provider.connect.neoGateway.visit.prefix": "",
  "provider.connect.neoGateway.visit.link": "neo.khulnasoft.com",
  "provider.connect.neoGateway.visit.suffix": " にアクセスしてAPIキーを取得してください。",
  "provider.connect.neoGateway.byok.prefix": "詳細な使用統計については、",
  "provider.connect.neoGateway.byok.link": "Neo's Gateway経由でBYOK",
  "provider.connect.neoGateway.byok.suffix": "をご利用ください。",

  // Provider settings translations
  "settings.providers.group.recommended": "おすすめ",
  "settings.providers.note.neo": "500以上のAIモデルにアクセス",
  ...
  "marketplace.install.about.mcp":
    "MCPサーバーは、外部サービスやローカルプログラムを操作するための追加ツールをNeoに提供します。",
  "marketplace.install.about.agent": "エージェントは、独自の指示と権限を持つ再利用可能な役割を追加します。",
  "marketplace.install.about.skill":
    "スキルは、必要に応じてNeoが読み込めるタスク固有の指示とリソースを追加します。",
  "marketplace.install.mcp.warning":
    "MCPサーバーはローカルコマンドを実行したり、外部サービスに接続したりできます。権限で自動的に許可されていない限り、Neoはツールを使用する前に許可を求めます。",
  ...
  "snapshot.slowRepo.answer.disable.description":
    "このプロジェクトでは Neo のスナップショットを無効にします。Neo による変更の取り消し/やり直しはできなくなりますが、git は引き続きすべてを追跡します。",
