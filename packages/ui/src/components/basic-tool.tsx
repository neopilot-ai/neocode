...
import { Icon, type IconProps } from "./icon" // neocode_change: added Icon
...
export interface BasicToolProps {
  ...
  forceOpen?: boolean
  defer?: boolean
  retainDetails?: boolean // neocode_change
  hasDetails?: boolean // neocode_change
  locked?: boolean
  animated?: boolean
  allowPendingToggle?: boolean // neocode_change
...
export function BasicTool(props: BasicToolProps) {
  ...
  const hasDetails = () => props.hasDetails ?? !!hasChildren() // neocode_change
  ...
  createEffect(
    on(
      ...
      (value) => {
        ...
        if (!value) {
          cancel()
          if (!props.retainDetails) setState("ready", false) // neocode_change
          return
        }
  ...
  const handleOpenChange = (value: boolean) => {
    if (pending() && !props.allowPendingToggle) return // neocode_change
    if (props.hideDetails) return // neocode_change
    if (props.locked && !value) return
    setOpen(value)
    props.onOpenChange?.(value) // neocode_change
  }
