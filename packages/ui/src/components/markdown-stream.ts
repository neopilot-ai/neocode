...
import { stableBlocks } from "../neocode/markdown-stable-blocks" // neocode_change
...
export function stream(text: string, live: boolean) {
  ...
  const candidate = tokens.findLast((token) => token.type !== "space") // neocode_change
  const blocks = candidate && !open(candidate.raw) ? stableBlocks(tokens, heal) : undefined // neocode_change
  if (blocks) return blocks // neocode_change
