...
// The "Neo" Pierre/Shiki theme used by every diff review surface (Code / Diff /
// File / SessionReview) and by markdown code highlighting. Pierre resolves the
// theme by name when a worker pool initializes (resolveThemes(["Neo"])) or when
// getSharedHighlighter() attaches it; if the name was never registered it throws
// "resolveTheme: No valid loader for Neo".
//
...
// diff component imports the worker factory, which calls ensureNeoDiffTheme()
...
// without markdown (e.g. neo-console) to fire a racy `void import(...)` purely to
...
// the registration through ensureNeoDiffTheme() instead.

export const NEO_DIFF_THEME = "Neo"
...
export function ensureNeoDiffTheme(): void {
  if (RegisteredCustomThemes.has(NEO_DIFF_THEME)) return

  registerCustomTheme(NEO_DIFF_THEME, () => {
