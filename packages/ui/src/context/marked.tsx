...
// neocode_change: marked-shiki highlighted code blocks synchronously during
...
// neocode_change start: import types for double-dollar math extension
import type { MarkedExtension, TokenizerAndRendererExtension } from "marked"
// neocode_change end
import { bundledLanguages, type BundledLanguage } from "shiki"
import { parseFilePath } from "../file-path" // neocode_change
import { createSimpleContext } from "./helper"
import { getSharedHighlighter } from "@pierre/diffs" // neocode_change
import { ensureNeoDiffTheme } from "../pierre/neo-diff-theme" // neocode_change

// neocode_change start: the "Neo" diff/highlight theme registration moved to
// ../pierre/neo-diff-theme so the diff worker pool can register it without
...
// route registration through ensureNeoDiffTheme() instead.
ensureNeoDiffTheme()
// neocode_change end

// neocode_change start: double-dollar-only math rules for marked.
const BLOCK = /^\$\$\n((?:\\[^]|[^\\])+?)\n\$\$(?:\n|$)/
const INLINE = /^\$\$(?!\$)((?:\\.|[^\\\n])*?(?:\\.|[^\\\n$]))\$\$/
// neocode_change end

// neocode_change start: isolate KaTeX from the markdown root dir=auto.
...
function renderMathInText(text: string): string {
  ...
  // neocode_change: removed single-dollar inline math ($...$) rendering.
...
// neocode_change: parseFilePath imported from ../file-path

// neocode_change start: highlight cache for deferred highlighting
...
function replaceWithHighlighted(block: Element, html: string, sourceHash: string) {
  ...
  const dir = pre.getAttribute("dir") // neocode_change
  if (dir) highlighted.setAttribute("dir", dir) // neocode_change
...
export async function deferredHighlight(
  ...
  const highlighter = await getSharedHighlighter({ themes: ["Neo"], langs: [] })
      ...
      setTimeout(() => {
        ...
        try {
          ...
          const highlight = () => {
            ...
            const html = highlighter.codeToHtml(code, { lang: language, theme: "Neo", tabindex: false })
...
// neocode_change start: expose the parser setup for Neo markdown tests.
export const createMarkedParser = (props: { nativeParser?: NativeMarkdownParser }) => {
  // neocode_change start: two-pass parser — first pass skips Shiki highlighting
  ...
  const parser = marked.use(
    ...
    // neocode_change start: enable only double-dollar math.
    // Single $ is far more common as a currency symbol in agent responses
    ...
    // neocode_change end
    // neocode_change: markedShiki removed — the custom `code` renderer
