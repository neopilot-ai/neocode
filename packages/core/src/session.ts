...
import { and, asc, desc, eq, gt, isNotNull, like, lt, or, type SQL } from "drizzle-orm" // neocode_change
...
import { normalize } from "./neocode/session-message" // neocode_change
  ...
  Effect.gen(function* () {
    ...
    const decode = (row: typeof SessionMessageTable.$inferSelect) =>
      decodeMessage(normalize({ ...row.data, id: row.id, type: row.type })).pipe(
        // neocode_change - normalize released tool content on paginated reads
        Effect.mapError(
          () =>
    ...
    const result = Service.of({
      ...
      messages: Effect.fn("V2Session.messages")(function* (input) {
          ...
          ? yield* db
              ...
              .where(
                and(
                  ...
                  isNotNull(SessionMessageTable.seq), // neocode_change
                ),
              )
              .get()
              .pipe(Effect.orDie)
        ...
        const where = boundary
          ...
          : and(eq(SessionMessageTable.session_id, input.sessionID), isNotNull(SessionMessageTable.seq)) // neocode_change
        const query = db
          .select()
          .from(SessionMessageTable)
          .where(where)
      ...
      interrupt: Effect.fn("V2Session.interrupt")((sessionID) =>
        Effect.uninterruptible(
          Effect.gen(function* () {
            ...
            // neocode_change start - keep interrupt operational while preserving released durable event compatibility.
