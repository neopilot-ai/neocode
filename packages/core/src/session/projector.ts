...
import { and, desc, eq, isNotNull, sql } from "drizzle-orm" // neocode_change
...
import * as StoredMessage from "../neocode/session-message" // neocode_change
...
function sessionRow(info: SessionV1.SessionInfo): typeof SessionTable.$inferInsert {
  return {
    ...
    share_url: info.share?.url ?? null, // neocode_change - full session updates must clear removed shares
...
function run(db: DatabaseService, event: SessionEvent.Event) {
  return Effect.gen(function* () {
    const decodeRow = (row: typeof SessionMessageTable.$inferSelect) =>
      decodeMessage(StoredMessage.normalize({ ...row.data, id: row.id, type: row.type })) // neocode_change
...
export const layer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    yield* events.project(SessionEvent.Compaction.Ended, (event) => {
      if (event.data.messageID === undefined || event.data.reason === undefined) return Effect.void // neocode_change
      const seq = event.seq
