...
import { ProviderMetadata, StoredToolContent, ToolContent } from "@opencode-ai/llm" // neocode_change - durable events decode legacy content
...
export const InterruptRequested = EventV2.define({
  ...
  schema: Base,
}) // neocode_change - operational notification; released readers cannot decode a durable event with this type
...
export namespace Tool {
   ...
   */
  // neocode_change start - keep legacy persistence decoding out of public event schemas
  ...
  export const Progress = EventV2.define({
    ...
    sync: {
      ...options.sync,
      codec: Schema.Struct({ ...ProgressFields, content: Schema.Array(StoredToolContent) }), // neocode_change
    },
  ...
  export const Success = EventV2.define({
    ...
    sync: {
      ...options.sync,
      codec: Schema.Struct({ ...SuccessFields, content: Schema.Array(StoredToolContent) }), // neocode_change
    },
...
export namespace Compaction {
  ...
  // neocode_change start - keep the released v1 event key while storing enough data for both reader generations.
  const EndedFields = {
    ...Base,
...
const EphemeralDefinitions = [
  InterruptRequested, // neocode_change - preserve downgrade-readable durable streams
  Text.Delta,
  Tool.Input.Delta,
  Reasoning.Delta,
  Compaction.Delta,
