...
import { decorateFileSystem, ensureDirectory } from "@neocode/sandbox" // neocode_change
...
export namespace FSUtil {
  ...
  export const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const fs = decorateFileSystem(yield* FileSystem.FileSystem) // neocode_change
      ...
      const ensureDir = Effect.fn("FileSystem.ensureDir")(function* (path: string) {
        yield* ensureDirectory(fs, path) // neocode_change - mutate through the sandbox-confined filesystem
      })
      ...
      const writeWithDirs = Effect.fn("FileSystem.writeWithDirs")(function* (
        ...
        yield* write.pipe(
          Effect.catchIf(
            ...
            () =>
              Effect.gen(function* () {
                yield* ensureDirectory(fs, dirname(path)) // neocode_change - sandbox-confined mkdir
                yield* write
              }),
          ),
        )
