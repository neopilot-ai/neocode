...
export interface Interface {
  ...
  readonly replace: (sources: readonly (readonly [string, Source])[]) => Effect.Effect<void> // neocode_change
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      ...
      // neocode_change start - reconcile Neo's effective config without a request-scoped transform slot.
      replace: (sources) =>
        state.mutate((editor) =>
          Effect.sync(() => {
