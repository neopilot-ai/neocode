...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const project = (provider: ProviderV2.Info, active: Map<IntegrationSchema.ID, Credential.Stored>) => {
      ...
      // neocode_change start - preserve Neo organization routing from migrated OAuth credentials
      if (credential.value.type === "oauth") {
        body.apiKey = credential.value.access
        if (credential.value.metadata?.accountID) body.neocodeOrganizationId = credential.value.metadata.accountID
      }
      // neocode_change end
