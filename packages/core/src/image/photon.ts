...
import { DecodeError, PixelLimitError, ResizerUnavailableError, SizeError } from "../image" // neocode_change
import { allowed, dimensions, MAX_DIMENSION, MAX_PIXELS } from "../neocode/image-size" // neocode_change
...
export const make = Effect.gen(function* () {
  ...
  return Effect.fn("Image.Photon.normalize")(function* (
    ...
    // neocode_change start - reject decompression bombs before Photon allocates native pixels
    ...
    const decoded = yield* Effect.try({
      try: () => photon.PhotonImage.new_from_byteslice(input), // neocode_change
      catch: () => new DecodeError({ resource }),
    })
    try {
