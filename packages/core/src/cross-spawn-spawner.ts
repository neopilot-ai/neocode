...
import { prepareCommand as prepareSandbox } from "@neocode/sandbox" // neocode_change
import { tap as tapStdio, tapped } from "./neocode/stdio-tap" // neocode_change - Bun drops buffered stdio on close
import * as SpawnValidation from "./neocode/spawn-validation" // neocode_change
...
export const make = Effect.gen(function* () {
  ...
  const setupOutput = (
    ...
    let stdout = proc.stdout
      ? NodeStream.fromReadable({
          evaluate: () => tapped(proc, "stdout"), // neocode_change - read the spawn-time tap
          onError: (cause) => toPlatformError("fromReadable(stdout)", toError(cause), command),
        })
      : Stream.empty
    let stderr = proc.stderr
      ? NodeStream.fromReadable({
          evaluate: () => tapped(proc, "stderr"), // neocode_change - read the spawn-time tap
  ...
  const spawn = (command: ChildProcess.StandardCommand, opts: NodeChildProcess.SpawnOptions) =>
    Effect.callback<readonly [NodeChildProcess.ChildProcess, ExitSignal], PlatformError.PlatformError>((resume) => {
      ...
      tapStdio(proc) // neocode_change - must run in the same tick as spawn
      let end = false
  ...
  const spawnCommand: (
    ...
    function* (command) {
      switch (command._tag) {
        case "StandardCommand": {
          const validation = SpawnValidation.take(command) // neocode_change - retain target validation through preparation
          const dir = yield* cwd(command.options)
          // neocode_change start - prepare agent-scoped commands through the selected sandbox backend
          ...
          // neocode_change start - close target-swap races at the raw spawn boundary
          if (validation)
          ...
          const [proc, signal] = yield* Effect.acquireRelease(
            // neocode_change start - spawn the prepared command and options
            spawn(target, {
              cwd: dir,
            ...
            Effect.fnUntraced(function* ([proc, signal]) {
              ...
              const kill = timeout(proc, command, target.options) // neocode_change
              if (done) {
              ...
              // neocode_change start - preserve kill options from the prepared command
