...
import { Flag } from "./flag/flag" // neocode_change
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["config.json", "neo.json", "neo.jsonc", "opencode.json", "opencode.jsonc"] // neocode_change
    ...
    const discovered = locationIsGlobal || Flag.NEO_DISABLE_PROJECT_CONFIG // neocode_change
      ? []
      : yield* fs
          .up({
            targets: [".neo", ".neocode", ...names.toReversed()], // neocode_change
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => [".neo", ".neocode"].includes(path.basename(item))) // neocode_change
        .toReversed()
    ...
    const directPaths = discovered.filter((item) => ![".neo", ".neocode"].includes(path.basename(item))).toReversed()
    // neocode_change end
    ...
    // global config, project files, then Neo config-directory files. // neocode_change
