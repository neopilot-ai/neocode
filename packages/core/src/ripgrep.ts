...
export interface GlobInput {
  ...
  readonly follow?: boolean
  readonly signal?: AbortSignal
  readonly validate?: Effect.Effect<void, unknown> // neocode_change - bind approved searches at spawn
}
...
export interface Interface {
  ...
  readonly glob: (input: GlobInput) => Effect.Effect<SearchResult<Entry>, Error> // neocode_change
  readonly grep: (input: GrepInput) => Effect.Effect<SearchResult<Match>, Error | InvalidPatternError> // neocode_change
}

// neocode_change start - retain truncation state through model-facing tools
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = <A>(input: {
      ...
      readonly validate?: Effect.Effect<void, unknown> // neocode_change - spawn-bound target validation
    }) => {
      const program = Effect.scoped(
        Effect.gen(function* () {
          const filepath = yield* binary.filepath
          // neocode_change start - validate approved targets after all spawn preparation
    ...
    return Service.of({
      glob: (input) =>
        run<string>({
          ...
          limit: input.limit,
          signal: input.signal,
          validate: input.validate, // neocode_change - preserve spawn-bound target validation
          args: [
            "--no-config",
            "--files",
            ...(input.hidden ? ["--hidden"] : []),
          ...
          // neocode_change start - retain spawn metadata after mapping paths
          Effect.map((result) => ({
            ...result,
      ...
      grep: (input) =>
        run<RawMatchData>({
          ...
          // neocode_change start - retain spawn metadata after mapping matches
          Effect.map((result) => ({
            ...result,
