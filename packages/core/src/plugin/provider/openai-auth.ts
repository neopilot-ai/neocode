  ...
  authorize: () =>
    Effect.gen(function* () {
      ...
      const server = createServer((request, response) => {
        ...
        // neocode_change start - unrelated localhost requests must not terminate the active OAuth attempt
...
function headers(contentType: string) {
  return { "Content-Type": contentType, "User-Agent": `neo/${InstallationVersion}` } // neocode_change
}
...
const successPage =
  "<!doctype html><title>Neo</title><h1>Authorization successful</h1><p>You can close this window.</p>" // neocode_change
const errorPage = (message: string) =>
  `<!doctype html><title>Neo</title><h1>Authorization failed</h1><p>${message.replace(/[&<>"']/g, "")}</p>` // neocode_change
