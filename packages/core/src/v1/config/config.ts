...
import { IndexingConfig as NeoIndexingConfig, IndexingSchema as NeoIndexingSchema } from "@neocode/neo-indexing/config"
import z from "zod"
// neocode_change end
...
// neocode_change start - indexing configuration
export const Indexing = NeoIndexingConfig
...
const Percent = Schema.Number.check(Schema.isGreaterThan(0), Schema.isLessThanOrEqualTo(100)) // neocode_change

const IndexingRef = NeoIndexingSchema.annotate({ [ZodOverride]: NeoIndexingConfig }) // neocode_change
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServerV1.Server).annotate({
    description: "Server configuration for the neo serve command", // neocode_change
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommandV1.Info)).annotate({
    description: "Command configuration, see https://neo.khulnasoft.com/docs/customize/workflows", // neocode_change
  }),
  ...
  // neocode_change start
  // NOTE: Any new neocode_change key added to Config.Info must also be mirrored in
  // apps/web/src/app/config.json/extras.ts in the cloud repo, otherwise
  // $schema: https://app.neo.khulnasoft.com/config.json will not recognize it.
  remote_control: Schema.optional(Schema.Boolean).annotate({
    description: "Enable remote control of sessions via Neo Cloud. Equivalent to running /remote on startup.",
  }),
    ...
    Schema.Struct({
      context_sidebar_width: Schema.optional(
        Schema.Int.check(Schema.isBetween({ minimum: 250, maximum: 800 })).annotate({
          description: "Width of the Neo Console project context sidebar in pixels",
        }),
      ),
      diff_style: Schema.optional(Schema.Literals(["unified", "split"])).annotate({
        description: "Default diff layout in Neo Console project reviews",
      }),
    }),
  ).annotate({ description: "Neo Console user interface configuration" }),
  ...
  hide_prompt_training_models: Schema.optional(Schema.Boolean).annotate({
    description: "Hide Neo Gateway models that may train on your prompts from model listings",
  }),
  ...
  agent: Schema.optional(
    ...
    // neocode_change start
  ).annotate({ description: "Agent configuration, see https://neo.khulnasoft.com/docs/customize/custom-subagents" }), // neocode_change
  ...
  commit_message: CommitMessageSchema, // neocode_change
  ...
  experimental: Schema.optional(
    Schema.Struct({
      ...
      sandbox: Schema.optional(Schema.Boolean).annotate({
        description:
          "Run agent tools inside a sandbox that restricts writes to project and Neo state directories and can restrict outbound network access",
      }),
