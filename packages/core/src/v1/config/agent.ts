...
// neocode_change start - agent skill/MCP/VS Code extension requirements schema
...
const AgentSchema = Schema.StructWithRest(
  Schema.Struct({
    model: Schema.optional(Schema.NullOr(Schema.String)), // neocode_change - nullable for delete sentinel
    // neocode_change start - nullable for delete sentinel
    variant: Schema.optional(Schema.NullOr(Schema.String)).annotate({
      description: "Default model variant for this agent (applies only when using the agent's configured model).",
    }),
    // neocode_change end
    temperature: Schema.optional(Schema.NullOr(Schema.Finite)), // neocode_change - nullable for delete sentinel
    top_p: Schema.optional(Schema.NullOr(Schema.Finite)), // neocode_change - nullable for delete sentinel
    prompt: Schema.optional(Schema.NullOr(Schema.String)), // neocode_change - nullable for delete sentinel
    ...
    // neocode_change start - typed metadata carriers so they never fall into `options` (provider params)
    ...
    requirements: Schema.optional(Requirements), // neocode_change
  }),
...
const KNOWN_KEYS = new Set([
  ...
  "top_p",
  "mode",
  "displayName", // neocode_change
  "source", // neocode_change
  "hidden",
  "color",
  "steps",
  "maxSteps",
...
const normalize = (agent: Schema.Schema.Type<typeof AgentSchema>): Schema.Schema.Type<typeof AgentSchema> => {
  ...
  // neocode_change start - preserve null delete sentinel (?? would collapse null to maxSteps)
