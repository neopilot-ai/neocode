...
import { SessionSchema } from "./session/schema" // neocode_change
...
export const Info = Schema.Struct({
  ...
  pid: NonNegativeInt,
  sessionID: Schema.optional(Schema.NullOr(SessionSchema.ID)), // neocode_change
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("Pty.create")(function* (input: PreparedCreate) {
      ...
      // neocode_change - expose the pty id to the spawned shell so a nested `neo tui`/`neo run` can
      // detect it is running inside a neo-spawned terminal (read via process.env.NEO_PTY_ID)
      const env = { ...input.env, NEO_PTY_ID: id }
    ...
    const update = Effect.fn("Pty.update")(function* (id: PtyID, input: UpdateInput) {
      ...
      // neocode_change start - associate nested Neo TUI terminals with the viewed session
