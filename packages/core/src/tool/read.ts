...
import { LocationMutation } from "../location-mutation" // neocode_change
...
export const layer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    const mutation = yield* LocationMutation.Service // neocode_change
        ...
        [name]: Tool.make({
          ...
          execute: (input, context) => {
            return Effect.gen(function* () {
              // neocode_change start - retain external-directory authorization and canonical resources
              ...
              if (target.type === "directory")
                return yield* reader.list(target, { offset: input.offset, limit: input.limit }) // neocode_change - approved identity
              const content = yield* reader.read(target, resource, {
                offset: input.offset,
                limit: input.limit,
              }) // neocode_change - approved identity
