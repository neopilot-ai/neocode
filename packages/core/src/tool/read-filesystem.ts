...
// neocode_change start - bind approved reads to filesystem identity
...
export const inspect = Effect.fn("ReadTool.inspect")(function* (fs: FSUtil.Interface, input: string) {
  ...
  // neocode_change start - retain the approved identity for descriptor verification
...
// neocode_change - reject targets replaced after permission approval
...
export const read = Effect.fn("ReadTool.read")(function* (
  ...
  return yield* Effect.scoped(
    Effect.gen(function* () {
      // neocode_change start - open the approved path and verify the descriptor identity
      ...
      const consume = (chunk: Uint8Array) => {
        ...
        while (true) {
          ...
          if (truncated) break // neocode_change - stop after the first extra line proves another page exists
        }
      }
      yield* Effect.sync(() => consume(first))
      while (true) {
        if (truncated) break // neocode_change
        const chunk = yield* file.readAlloc(64 * 1024).pipe(Effect.orDie)
        if (Option.isNone(chunk)) break
        yield* Effect.sync(() => consume(chunk.value))
      }
      // neocode_change start - a completed page must not decode the rest of the file
      if (!truncated) {
...
// neocode_change - verify approved directory identity before listing
