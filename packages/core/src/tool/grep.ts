...
import * as SearchTarget from "../neocode/search-target"
// neocode_change end
import { Location } from "../location"
import { Reference } from "../reference" // neocode_change
...
export const Input = Schema.Struct({
  ...
  limit: FileSystem.SearchLimit.pipe(Schema.optional).annotate({ // neocode_change
    description: "Maximum matches to return",
  }),
})

// neocode_change start - retain bounded-search status in tool results and model output
...
export const layer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    const global = yield* Global.Service // neocode_change
    const ripgrep = yield* Ripgrep.Service
    const location = yield* Location.Service
    const references = yield* Reference.Service // neocode_change
      ...
      .register({
        [name]: Tool.make({
          ...
          toModelOutput: ({ output }) => [
            {
              type: "text",
              // neocode_change start - model paths remain absolute while the typed result retains metadata
              text: toModelOutput({
                ...output,
            ...
            Effect.gen(function* () {
              yield* permission.assert({
                ...
                metadata: {
                  root: ".",
                  path: input.path,
                  reference: input.reference, // neocode_change
              ...
              // neocode_change start - enforce the active Location despite RelativePath being a nominal brand
              ...
              return yield* ripgrep
                .grep({
                  ...
                  file: target.type === "file" ? path.basename(target.path) : undefined, // neocode_change
                  include: input.include,
                  // neocode_change start
                ...
                .pipe(
                  // neocode_change start - preserve search status after canonical path mapping
                  Effect.map(
                    (result) =>
                      new Result({
                        ...result,
