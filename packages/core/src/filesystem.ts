...
import { Context, Effect, Layer, Option, Schema } from "effect" // neocode_change
...
import * as SearchTarget from "./neocode/search-target" // neocode_change
...
export const DEFAULT_SEARCH_LIMIT = 100 // neocode_change - preserve bounded Neo tool searches
export const MAX_SEARCH_LIMIT = 100 // neocode_change
export const SearchLimit = PositiveInt.check(Schema.isLessThanOrEqualTo(MAX_SEARCH_LIMIT)) // neocode_change
...
const baseLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const resolve = Effect.fnUntraced(function* (input?: RelativePath) {
      ...
      const target = yield* SearchTarget.inspect(fs, real).pipe(Effect.orDie) // neocode_change
      return { absolute, real, directory: location.directory, root, target } // neocode_change
    })
    return Service.of({
      find: search.find,
      glob: search.glob,
      ...
      read: Effect.fn("FileSystem.read")(function* (input) {
        ...
        if (target.target.type !== "file") return yield* Effect.die(new Error("Path is not a file")) // neocode_change
        // neocode_change start - read from the validated descriptor, not a second pathname lookup.
      ...
      list: Effect.fn("FileSystem.list")(function* (input = {}) {
        const target = yield* resolve(input.path)
        if (target.target.type !== "directory") return yield* Effect.die(new Error("Path is not a directory")) // neocode_change
        // neocode_change start - reject directory replacement during enumeration
