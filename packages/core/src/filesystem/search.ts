...
import { scanning } from "../neocode/fff"
// neocode_change end
...
export const ripgrepLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // neocode_change start - confine every search to the canonical active Location.
        ...
        Effect.gen(function* () {
          ...
          return yield* ripgrep
            .glob({
              ...
              validate: SearchTarget.validate(fs, target), // neocode_change
            })
            .pipe(
              Effect.map((result) =>
                result.items.map( // neocode_change
                  (entry) =>
    ...
    const result = yield* Effect.try({
      try: () =>
        Fff.create({
          ...
          aiMode: true,
          ...scanning(location.directory), // neocode_change - permit broad scanning only at the exact boundary.
        }),
      catch: (cause) => cause,
    }).pipe(Effect.orDie)
...
export const defaultLayer = Layer.unwrap(
  Effect.sync(() => (Flag.NEO_DISABLE_FFF || !Fff.available() ? ripgrepLayer : fffLayer)),
)
