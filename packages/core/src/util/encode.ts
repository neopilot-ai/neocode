export function base64Encode(value: string) {
  const normalized = value.replace(/\\/g, "/") // neocode_change - normalize Windows paths
  const bytes = new TextEncoder().encode(normalized) // neocode_change - encode as UTF-8 bytes
