...
import { existsSync, writeFileSync } from "fs" // neocode_change
import fs from "fs/promises"
import * as Global from "../global"
import { Schema } from "effect"
import { Glob } from "./glob"
import { createStream } from "rotating-file-stream" // neocode_change
import { NEO_RUN_ID } from "./opencode-process" // neocode_change
...
const keep = 10
const initializedRunID = "NEO_LOG_INITIALIZED_RUN_ID"
...
let write = stderr
let stream: ReturnType<typeof createStream> | undefined // neocode_change

export async function init(options: Options) {
  if (options.level) level = options.level
  void cleanup(Global.Path.log)
  // neocode_change start - initialize one rotating stream and truncate dev.log once per Neo run
  if (stream) {
    const active = stream
    stream = undefined
  ...
  const run = process.env[NEO_RUN_ID]
