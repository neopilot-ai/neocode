...
function layer(directory: string) {
  return Credential.layer.pipe(
    Layer.fresh, // neocode_change - rebuild so process-local credentials are re-read
...
describe("Credential", () => {
  ...
  // neocode_change start - process-provided credentials remain isolated from durable storage
  it.live("keeps valid NEO_AUTH_CONTENT credentials and isolated mutations process-local", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = process.env.NEO_AUTH_CONTENT
        process.env.NEO_AUTH_CONTENT = JSON.stringify({
          neocode: {
            type: "oauth",
            refresh: "refresh",
            access: "access",
        ...
        Effect.acquireRelease(
          ...
          Effect.flatMap((tmp) =>
            Effect.gen(function* () {
              ...
              const neocode = Integration.ID.make("neocode")
              const listed = yield* service.list(neocode)
              expect(listed).toHaveLength(1)
              expect(listed[0]).toMatchObject({
                integrationID: neocode,
                label: "Environment",
                value: {
                  type: "oauth",
                  refresh: "refresh",
              ...
              expect(yield* service.list(neocode)).toEqual([created])
              yield* service.update(created.id, { label: "Updated" })
              expect((yield* service.list(neocode))[0]?.label).toBe("Updated")
              yield* service.remove(created.id)
              expect(yield* service.list(neocode)).toEqual([])
      ...
      (previous) =>
        Effect.sync(() => {
          if (previous === undefined) delete process.env.NEO_AUTH_CONTENT
          else process.env.NEO_AUTH_CONTENT = previous
        }),
    ),
  )
