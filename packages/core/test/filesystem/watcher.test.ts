...
const describeWatcher =
  Watcher.hasNativeBinding() && (!process.env.CI || process.env.NEO_TEST_PROFILE === "darwin") // neocode_change
    ? describe
    : describe.skip
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    NEO_EXPERIMENTAL_FILEWATCHER: "true",
    NEO_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
    ...
    withTmp(
      (directory) =>
        Effect.gen(function* () {
          ...
          // neocode_change start - FSEvents may classify this overwrite as an add.
