...
describe("InstructionContext", () => {
  ...
  it.effect("honors the project instruction opt-out", () =>
    Effect.gen(function* () {
      const previous = process.env.NEO_DISABLE_PROJECT_CONFIG
      let scanned = false
      ...
      yield* SystemContextRegistry.Service.pipe(
        ...
        Effect.ensuring(
          Effect.sync(() => {
            if (previous === undefined) delete process.env.NEO_DISABLE_PROJECT_CONFIG
            else process.env.NEO_DISABLE_PROJECT_CONFIG = previous
          }),
        ),
      )
