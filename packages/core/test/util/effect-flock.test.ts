...
// neocode_change start - make worker finalization await the process close event without a Windows race
  ...
  it.live(
    ...
    () =>
      Effect.promise(async () => {
        ...
        try {
          await waitForFile(ready, 20_000) // neocode_change - hosted macOS can start this worker slowly after stress tests
          await stopWorker(proc) // neocode_change - stopWorker now awaits close before returning
    ...
    60_000, // neocode_change - match the wider worker readiness window
  )
})
