...
describe("Config", () => {
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(async () => {
            ...
            await Promise.all([
              fs.writeFile(path.join(project, "neo.json"), JSON.stringify({ model: "project/model" })),
              fs.writeFile(path.join(global, "neo.json"), JSON.stringify({ model: "global/model" })),
            ])
          })

          const prior = process.env.NEO_DISABLE_PROJECT_CONFIG
          process.env.NEO_DISABLE_PROJECT_CONFIG = "1"
          yield* Effect.addFinalizer(() =>
            Effect.sync(() => {
              if (prior === undefined) delete process.env.NEO_DISABLE_PROJECT_CONFIG
              else process.env.NEO_DISABLE_PROJECT_CONFIG = prior
            }),
          )
  ...
  // neocode_change start - V2 config discovery follows Neo roots and precedence
  it.live("loads Neo configuration roots up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await Promise.all(
              [root, directory].flatMap((dir) =>
                [".neocode", ".neo", ".opencode"].map((name) => fs.mkdir(path.join(dir, name), { recursive: true })),
              ),
            )
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "opencode.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "neo.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "neo.json"), JSON.stringify({ $schema: "root" })),
              ...
              fs.writeFile(path.join(root, ".neocode", "opencode.json"), JSON.stringify({ $schema: "root-neocode" })),
              fs.writeFile(path.join(root, ".neo", "neo.json"), JSON.stringify({ $schema: "root-neo" })),
              fs.writeFile(
                path.join(directory, ".neocode", "opencode.jsonc"),
                JSON.stringify({ $schema: "directory-neocode" }),
              ),
              fs.writeFile(path.join(directory, ".neo", "neo.jsonc"), JSON.stringify({ $schema: "directory-neo" })),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".neocode")),
              AbsolutePath.make(path.join(root, ".neo")),
              AbsolutePath.make(path.join(directory, ".neocode")),
              AbsolutePath.make(path.join(directory, ".neo")),
            ])
            ...
            expect(Config.latest(entries, "$schema")).toBe("directory-neo")
