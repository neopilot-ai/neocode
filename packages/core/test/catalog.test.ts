...
describe("CatalogV2", () => {
  it.effect("projects Neo organization routing from OAuth credentials", () => {
    const integrationID = Integration.ID.make("neocode")
    ...
    return Effect.gen(function* () {
      ...
      yield* transform((editor) => editor.provider.update(ProviderV2.ID.make("neocode"), () => {}))
      expect(yield* catalog.provider.get(ProviderV2.ID.make("neocode"))).toMatchObject({
        request: { body: { apiKey: "access", neocodeOrganizationId: "organization" } },
      })
