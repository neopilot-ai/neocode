...
// test/preload.ts pins NEO_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.NEO_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.NEO_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.NEO_MODELS_PATH = undefined
  Flag.NEO_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.NEO_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.NEO_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
...
describe("ModelsDev Service", () => {
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.NEO_DISABLE_MODELS_FETCH = false
        }),
        () =>
          provided(
            state,
  ...
  it.live("refresh(true) fetches via HttpClient and updates the cache", () =>
    Effect.gen(function* () {
      ...
      expect(final.calls[0].userAgent).toEndWith(`/${Flag.NEO_CLIENT}`)
    }),
  )
