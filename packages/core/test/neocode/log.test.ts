...
describe("Neo logger compatibility", () => {
  ...
  test("does not truncate the dev log twice during one run", async () => {
    ...
    const initialized = process.env.NEO_LOG_INITIALIZED_RUN_ID
    await using tmp = await tmpdir()
    Global.Path.log = tmp.path
    process.env.NEO_RUN_ID = "run-1"
    delete process.env.NEO_LOG_INITIALIZED_RUN_ID
    ...
    try {
      ...
      await Log.init({ print: true })
      Global.Path.log = previous
      if (run === undefined) delete process.env.NEO_RUN_ID
      else process.env.NEO_RUN_ID = run
      if (initialized === undefined) delete process.env.NEO_LOG_INITIALIZED_RUN_ID
      else process.env.NEO_LOG_INITIALIZED_RUN_ID = initialized
    }
  })
})
