...
describe("Neo reference compatibility", () => {
  it.live("materializes and refreshes configured Git references", () =>
    withRemote((fixture) => {
      ...
      return Effect.gen(function* () {
        ...
        process.env.NEO_REPO_CLONE_GITHUB_BASE_URL = base.endsWith("/") ? base : `${base}/`
        yield* Effect.addFinalizer(() =>
          Effect.sync(() => {
            if (previous === undefined) delete process.env.NEO_REPO_CLONE_GITHUB_BASE_URL
            else process.env.NEO_REPO_CLONE_GITHUB_BASE_URL = previous
          }),
        )
