    ...
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          ...
          expect(matches.items.map((item) => item.entry.path)).toContain(RelativePath.make(".opencode/config")) // neocode_change
          expect(matches.items.map((item) => item.entry.path)).not.toContain(RelativePath.make(".git/config")) // neocode_change
        }),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ),
  )
