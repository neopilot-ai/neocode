...
import * as StoredMessage from "@opencode-ai/core/neocode/session-message" // neocode_change
  ...
  it.effect("projects durable context messages supported by the updater", () =>
    Effect.gen(function* () {
      ...
      const rows = yield* db
        ...
        .all()
        .pipe(Effect.orDie)
      // neocode_change start - assert the projector itself writes the released-reader compaction shape.
