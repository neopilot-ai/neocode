...
describe("CommandPlugin.Plugin", () => {
  it.effect("registers built-in init and review commands", () =>
    Effect.gen(function* () {
      ...
      expect((yield* command.get("init"))?.template).toContain("future Neo sessions") // neocode_change
      expect((yield* command.get("init"))?.template).toContain("`neo.json`") // neocode_change
      expect((yield* command.get("init"))?.template).not.toContain("OpenCode") // neocode_change
