...
// neocode_change - no durable interrupt lookup: released database readers cannot decode that event type.
describe("SessionV2.prompt", () => {
  ...
  it.effect("delegates interruption through SessionExecution", () =>
    Effect.gen(function* () {
      ...
      expect(interruptSeqs[0]).toBe(-1) // neocode_change - ephemeral interrupts do not advance durable storage
      expect(yield* eventCount("session.next.interrupt.requested.1")).toBe(0) // neocode_change
