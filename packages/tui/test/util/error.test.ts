...
describe("util.error", () => {
  ...
  // neocode_change start - preserve Neo-branded CLI guidance
  test("uses Neo commands and capability messaging", () => {
    const model = cliErrorMessage({
      _tag: "ProviderModelNotFoundError",
      providerID: "anthropic",
      modelID: "claude-sonnet-4",
    })
    expect(model).toContain("neo models")
    expect(model).toContain("neo.json")
