...
export function DialogSessionList() {
  ...
  const [global, setGlobal] = createSignal(false) // neocode_change - show current worktree by default
  const deleteHint = useCommandShortcut("session.delete")
  const quickSwitch1 = useCommandShortcut("session.quick_switch.1")
  const quickSwitch9 = useCommandShortcut("session.quick_switch.9")

  // neocode_change start - always fetch from experimental endpoint (returns GlobalSession with worktree info)
  ...
  const [searchResults, searchActions] = createResource(
    () => ({ query: search(), global: global(), directory: project.instance.directory() }), // neocode_change
    async (input) => {
  ...
  const sessions = createMemo(() => searchResults() ?? []) // neocode_change - endpoint applies worktree scope
    ...
    dialog.replace(() => (
      <DialogSessionDeleteFailed
        ...
        onDelete={async () => {
          ...
          if (search()) await searchActions.refetch() // neocode_change - use createResource actions
  ...
  // neocode_change - support local and global sessions
  ...
  const options = createMemo(() => {
    ...
    const all = global() // neocode_change
    ...
    const displayOrder = orderByRecency(sessions()) // neocode_change - respect current scope
    ...
    function buildOption(id: string, category: string) {
      ...
      return {
        ...
        description: all && x.worktreeName ? `(${x.worktreeName})` : undefined, // neocode_change - worktree label
  ...
  return (
    <DialogSelect
      title={global() ? "Sessions (all worktrees)" : "Sessions (current worktree)"} // neocode_change
      options={options()}
      skipFilter={true}
        ...
        },
        {
          ...
          onTrigger: async (option) => {
            if (toDelete() === option.value) {
              ...
              void searchActions.refetch() // neocode_change
              setToDelete(undefined)
              return
            }
      ...
      // neocode_change start - preserve Ctrl+A worktree scope toggle with the upstream keymap engine
