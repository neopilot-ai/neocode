...
import { useNudge } from "@/neocode/cli/cmd/tui/context/nudge" // neocode_change
...
import type { AssistantMessage, FilePart, UserMessage } from "@neocode/sdk/v2"
...
import { NeoSessionTuiSync } from "@/neocode/session/tui-sync"
import { slashMatches } from "@/neocode/cli/cmd/command-display"
import * as AgentRequirements from "@/neocode/cli/agent-requirements"
import { createCostAlertController } from "@/neocode/cli/cmd/tui/cost-alert"
import { MemoryPrompt } from "@/neocode/cli/cmd/tui/component/memory-prompt"
// neocode_change end
import { NEO_BASE_MODE, useBindings, useCommandShortcut, useLeaderActive, useOpencodeKeymap } from "../../keymap"
import { useTuiConfig } from "../../config"
// neocode_change start - vim modal editing for the prompt
import { useVim, VimModeIndicator, vimToggleCommand } from "@/neocode/cli/cmd/tui/component/prompt"
// neocode_change end
...
export type PromptProps = {
  sessionID?: string
  directory?: string // neocode_change
  visible?: boolean
  disabled?: boolean
  onSubmit?: () => void
...
export function Prompt(props: PromptProps) {
  ...
  const nudge = useNudge() // neocode_change
  ...
  const [store, setStore] = createStore<{
    ...
    interrupt: number
    exitPress: number // neocode_change - track double ctrl+c to exit
    placeholder: number
  }>({
  ...
  // neocode_change start - sync local agent/model whenever newest user message changes
  ...
  createEffect(() => {
    ...
    if (!parts) return
    if (!NeoSessionTuiSync.model({ role: msg.role, parts })) return
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      // neocode_change start - vim modal editing toggle (palette + /vim)
      vimToggleCommand({
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.NEO_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
          workspace.open()
        },
  ...
  useBindings(() => ({
    ...
    bindings: tuiConfig.keybinds.gather("prompt.palette", [
      ...
      "prompt.stash.pop",
      "prompt.stash.list",
      "prompt.vim.toggle", // neocode_change
      "session.interrupt",
      "workspace.set",
      "session.move",
    ]),
  ...
  const ref: PromptRef = {
    ...
    reset() {
      ...
      vim.resetVim() // neocode_change - return to insert mode after the prompt is cleared
    },
    submit() {
      void submit()
    },
  ...
  // neocode_change start - close autocomplete while blocking overlays hide the prompt
  createEffect(() => {
  ...
  // neocode_change start - require a double Ctrl+C to exit from an empty focused prompt
  useBindings(() => ({
    target: inputTarget,
    enabled: inputTarget() !== undefined && !props.disabled && store.prompt.input === "",
    bindings: [
      ...
      commands: [
        {
          ...
          run() {
            ...
            vim.resetVim() // neocode_change - recalled history starts in insert mode
  ...
  async function submitInner() {
    ...
    // neocode_change start - in-memory cost alert command
    ...
    // neocode_change start - gate TUI sends on declared agent requirements
    ...
    if (store.mode === "shell") {
      ...
      void sdk.client.session.shell({
        sessionID,
        agent: local.agent.current()?.name ?? "", // neocode_change
        model: {
          providerID: selectedModel.providerID,
    ...
    toast.dismiss() // neocode_change - dismiss persistent config warning on first submit
    history.append({
      ...store.prompt,
      mode: currentMode,
    })
  ...
  async function pasteInputText(text: string) {
    ...
    if (
      (lineCount >= 5 || pastedContent.length > 800) && // neocode_change #7252 delay paste summary
  ...
  function clearPrompt() {
    ...
    setStore("extmarkToPartIndex", new Map())
    vim.resetVim() // neocode_change - don't leak stale vim mode/selection into an emptied prompt
  }

  // neocode_change start - cost-alert logic lives under neocode/; only prompt mutation stays here
  ...
  const highlight = createMemo(() => {
    ...
    return local.agent.color(agent.name ?? "") // neocode_change
  })
  ...
  const spinnerDef = createMemo(() => {
    const agent =
      status().type !== "idle"
        ...
        : local.agent.current()
    const color = agent ? local.agent.color(agent.name ?? "") : theme.border // neocode_change
    return {
      frames: createFrames({
        color,
        style: "blocks",
        ...
        >
          ...
          >
            <textarea
              ...
              /* neocode_change */ onCursorChange={() => {
                setCursorVersion((value) => value + 1)
                if (store.mode === "normal") auto()?.onCursorChange()
              }}
              /* neocode_change - KeyEvent type for vim key routing */ onKeyDown={(e: KeyEvent) => {
                if (props.disabled) {
                  e.preventDefault()
                  return
                }
                // neocode_change start - route keys through the vim layer when enabled
                if (vim.vimOnKey(e)) {
                  e.preventDefault()
                  e.stopPropagation()
                  return
                ...
                <Show when={local.agent.current()} fallback={<box height={1} />}>
                  {(agent) => (
                    <>
                      ...
                      {/* neocode_change start - vim mode indicator */}
                      <VimModeIndicator
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
          ...
          <Show when={status().type !== "retry"}>
            <box gap={2} flexDirection="row">
              {/* neocode_change start - show "ctrl+c again to exit" hint */}
