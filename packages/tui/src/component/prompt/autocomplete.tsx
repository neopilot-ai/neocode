...
import { slashDisplay } from "@/neocode/cli/cmd/command-display" // neocode_change
...
export type AutocompleteRef = {
  ...
  // neocode_change start - validate cursor moves and close overlays without mutating draft text
  onCursorChange: () => void
  dismiss: () => void
  // neocode_change end
  visible: false | "@" | "/"
...
export function Autocomplete(props: {
  ...
  const commands = createMemo((): AutocompleteOption[] => {
    ...
    for (const serverCommand of sync.data.command) {
      // neocode_change start - preserve suffixes like :skill when inserting selected slash commands
  ...
  useBindings(() => ({
    ...
    bindings: [
      ...
      // neocode_change start - close stale suggestions while allowing normal cursor movement
      {
        key: "right",
        fallthrough: true,
        cmd: () => {
  ...
  // neocode_change start - keep slash text intact when overlays hide the prompt,
  ...
  onMount(() => {
    ...
    props.ref({
      ...
      // neocode_change start - dismiss stale popup after cursor leaves active filter region
      onCursorChange() {
        if (!store.visible) return
