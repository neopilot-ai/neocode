import type { TuiPluginApi } from "@neocode/plugin/tui"
import { createMemo, For, type Accessor } from "solid-js"
import { DEFAULT_THEMES, useTheme } from "../../context/theme"
import { NEO_TIPS } from "@/neocode/cli/cmd/tui/feature-plugins/home/tips" // neocode_change
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const tip = createMemo(() => {
    ...
    const tips = [...NEO_TIPS, ...TIPS, process.platform !== "win32" ? TERMINAL_SUSPEND_TIP : INPUT_UNDO_TIP].flatMap(
      (item) => {
...
const TIPS: Tip[] = [
  /* neocode_change hide the entire list for if it is accidentally used
  ...
  "Add {highlight}.md{/highlight} files to {highlight}.neo/commands/{/highlight} to define reusable custom prompts", // neocode_change
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.neo/agents/{/highlight} for specialized AI personas", // neocode_change
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.neo/tools/{/highlight} to define new LLM tools", // neocode_change
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.neo/plugins/{/highlight} for event hooks", // neocode_change
  ...
  "Create JSON theme files in {highlight}.neo/themes/{/highlight} directory", // neocode_change
