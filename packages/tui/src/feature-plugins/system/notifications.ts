import type { Event } from "@neocode/sdk/v2"
import type { TuiAttentionSoundName, TuiPlugin, TuiPluginApi } from "@neocode/plugin/tui"
...
const tui: TuiPlugin = async (api) => {
  ...
  // neocode_change start - only completed turns need completion attention
  ...
  api.event.on("session.error", (event) => {
    ...
    if (event.properties.error?.name === "MessageAbortedError") return // neocode_change - manual stops do not need attention
