...
import type { PermissionRequest } from "@neocode/sdk/v2"
...
import { normalizeUrls } from "@/neocode/util/url"
import { MemoryPermissionRegistry } from "@/neocode/cli/cmd/tui/routes/session/memory-permission"
// neocode_change end
import { NEO_BASE_MODE, useBindings, useCommandShortcut } from "../../keymap"
...
function EditBody(props: { request: PermissionRequest }) {
  ...
  const hunks = createMemo(() => splitDiffHunks(diff())) // neocode_change
        ...
        {(() => {
          const info = () => {
            ...
            if (permission === "webfetch") {
              const url = normalizeUrls(typeof data.url === "string" ? data.url : "") // neocode_change
              return {
                icon: "%",
                title: `WebFetch ${url}`,
                body: (
            ...
            const custom = MemoryPermissionRegistry.render(permission, props.request) // neocode_change
            if (custom) return custom // neocode_change
          ...
          const header = () => (
            <box flexDirection="column" gap={0}>
              ...
              {/* neocode_change start - explain protected Neo configuration access */}
                ...
                <box paddingLeft={4} flexShrink={0}>
                  <text fg={theme.textMuted}>
                    {props.request.permission === "edit"
                      ...
                      : "Neo configuration access always requires approval"}
                  </text>
                </box>
              </Show>
              {/* neocode_change end */}
          ...
          // neocode_change start - hide "Always allow" for protected Neo configuration access
          ...
          const body = (
            <Prompt
              ...
              header={header()}
              body={current.body}
              /* neocode_change */ options={options}
              escapeKey="reject"
              fullscreen
              onSelect={(option) => {
                if (option === "always") {
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Tell Neo what to do differently</text>
        </box>
      </box>
      <box
