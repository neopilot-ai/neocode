...
import { Suggest } from "@/neocode/suggestion/tui/render"
import { SuggestPrompt } from "@/neocode/suggestion/tui/prompt"
...
import { NEO_BASE_MODE, useBindings, useCommandShortcut, useOpencodeKeymap } from "../../keymap"
import { PathFormatterProvider, usePathFormatter } from "../../context/path-format"
// neocode_change start
import { NeoErrorBlock } from "@/neocode/components/neo-error-display"
import { splitDiffHunks } from "@/neocode/tui/diff"
import { RoutedModelMeta } from "@/neocode/cli/cmd/tui/routes/session/routed-model-meta"
import { submitFeedback } from "@/neocode/cli/cmd/tui/feedback"
import { MemoryMessageMeta, MemorySessionTui } from "@/neocode/cli/cmd/tui/routes/session/memory"
...
const sessionBindingCommands = [
  ...
  "messages.copy",
  // neocode_change start - message feedback
...
export function Session() {
  ...
  const sdk = useSDK()
  const memory = MemorySessionTui.verbose({ sessionID: () => route.sessionID }) // neocode_change
  const editor = useEditorContext()
  onCleanup(MemorySessionTui.attach({ event, toast, sessionID: route.sessionID })) // neocode_change

  // neocode_change start - background processes are scoped to the visible session
  ...
  const sessionCommandList = createMemo(() => [
    ...
    },
    {
      ...
      value: "messages.copy",
      category: "Session",
      // neocode_change start - /copy copies the latest assistant response
      slash: {
        name: "copy",
      },
      // neocode_change end
    ...
    // neocode_change end
    {
      ...
      category: "Session",
      slash: {
        name: "copy-session", // neocode_change - transcript copy moved off /copy
      },
      run: async () => {
        try {
          const sessionData = session()
          if (!sessionData) return
          // neocode_change start - fetch all messages from server instead of truncated sync store
          ...
          const transcript = formatTranscript(
            sessionData,
            sessionMessages, // neocode_change
            {
        ...
        <box flexDirection="row" flexGrow={1} minHeight={0}>
          <box flexGrow={1} minHeight={0} paddingBottom={1} paddingLeft={2} paddingRight={2} gap={1}>
            <Show when={session()}>
              ...
              <box flexShrink={0}>
                {/* neocode_change start - arbitrate Neo terminal, question, suggestion, and network input */}
...
function AssistantMessage(props: {
  ...
  const routed = createMemo(() => RoutedModelMeta.info(ctx.providers(), props.parts, ctx.showDetails(), props.message)) // neocode_change
  const route = createMemo(() => routed().footer) // neocode_change
  ...
  return (
    <>
      {/* neocode_change start - provide compact routed-model metadata to part renderers */}
        ...
        <Match when={props.last || final() || props.message.error?.name === "MessageAbortedError"}>
          <box id={`assistant-summary-${props.message.id}`} paddingLeft={3}>
            <text marginTop={1}>
              ...
              {/* neocode_change start - show routed model in regular assistant footer */}
              <Show when={route()}>
...
// neocode_change start - register rendered step-finish parts
const PART_MAPPING = {
  text: TextPart,
  tool: ToolPart,
...
const INLINE_TOOL_ICON_WIDTH = 2 // neocode_change

// neocode_change start - show concrete routed models reported by gateway/provider responses
function StepFinishPart(props: { last: boolean; part: StepFinishPart; message: AssistantMessage }) {
  ...
  const routed = createMemo(() => {
    if (props.message.providerID !== "neo") return undefined
    if (!props.message.modelID.startsWith("neo-auto/")) return undefined
...
function TextPart(props: { last: boolean; part: TextPart; message: AssistantMessage }) {
  ...
  // neocode_change start - format markdown tables with fixed-width columns
    ...
    <Show when={props.part.text.trim()}>
      <box id={`text-${props.part.messageID}-${props.part.id}`} paddingLeft={3} marginTop={1} flexShrink={0}>
        <markdown
          ...
          content={content()} // neocode_change
  ...
  return (
    <Show when={!shouldHide()}>
      <Switch>
        ...
        {/* neocode_change start - preserve Neo tool-specific status rendering */}
...
// neocode_change start - Neo tool-specific status rendering
...
function Task(props: ToolProps) {
  ...
  const content = createMemo(() => {
    ...
    } else if (isRunning()) content.push(`↳ Starting...`) // neocode_change
...
function Edit(props: ToolProps) {
  ...
  const hunks = createMemo(() => splitDiffHunks(diffContent())) // neocode_change

  return (
    <Switch>
      <Match when={stringValue(props.metadata.diff) !== undefined}>
        <BlockTool title={"← Edit " + pathFormatter.format(stringValue(props.input.filePath))} part={props.part}>
          {/* neocode_change start - preserve separated multi-hunk edit rendering */}
...
function Question(props: ToolProps) {
  ...
  // neocode_change start - preserve dismissed content and the compact expandable summary
...
const toolDisplays = new Set([
  ...
  "question",
  "skill",
  // neocode_change start - retain dedicated Neo tool renderers
  "background_process",
