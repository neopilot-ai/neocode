import type {
  ...
  PermissionRequest,
  QuestionRequest,
  SuggestionRequest, // neocode_change
  SessionNetworkWait, // neocode_change
  LspStatus,
  McpStatus,
  McpResource,
  FormatterStatus,
  ...
  SnapshotFileDiff,
  ConsoleState,
  BackgroundProcessInfo, // neocode_change
  InteractiveTerminalSnapshot, // neocode_change
  IndexingStatus, // neocode_change
} from "@neocode/sdk/v2"
...
import { batch, createEffect, on, onMount } from "solid-js" // neocode_change
import path from "path"
import { useKV } from "./kv"
import { handleSuggestionEvent } from "@/neocode/suggestion/tui/sync" // neocode_change
import { appendTerminalOutput } from "@/neocode/interactive-terminal/output" // neocode_change
import { useToast } from "../ui/toast" // neocode_change
...
export const {
  ...
  init: () => {
    ...
    const [store, setStore] = createStore<{
      ...
      // neocode_change end
      config: Config
      globalConfig: Config // neocode_change
      session: Session[]
      session_status: {
      ...
      indexing: IndexingStatus // neocode_change
    }>({
      provider_next: {
        all: [],
        default: {},
      ...
      provider_auth: {},
      config: {},
      globalConfig: {}, // neocode_change
      status: "loading",
      agent: [],
      permission: {},
      question: {},
      suggestion: {}, // neocode_change
      network: {}, // neocode_change
      command: [],
      provider: [],
      provider_default: {},
      session: [],
      ...
      session_diff: {},
      todo: {},
      background_process: {}, // neocode_change
      interactive_terminal: {}, // neocode_change
      message: {},
      part: {},
      lsp: [],
      mcp: {},
      ...
      formatter: [],
      vcs: undefined,
      indexing: { state: "Disabled", message: "Indexing disabled.", processedFiles: 0, totalFiles: 0, percent: 0 }, // neocode_change
    })

    const event = useEvent()
    const project = useProject()
    const sdk = useSDK()
    const toast = useToast() // neocode_change
    ...
    const deleted = new Set<string>() // neocode_change
    const terminalDeleted = new Set<string>() // neocode_change
    let syncedWorkspace = project.workspace.current() // neocode_change
    let vcsVersion = 0 // neocode_change
    ...
    event.subscribe((event, { workspace }) => {
      switch (event.type) {
        ...
        case "session.deleted": {
          ...
          evict(event.properties.info.id) // neocode_change
          break
        }
    ...
    // neocode_change start - retain versioned Sync events used by Neo clients
    event.sync((event) => {
      switch (event.name) {
        ...
        case "session.updated.1": {
          ...
          setStore(
            "session",
            match.index,
            reconcile(event.data.info), // neocode_change - session.updated carries a full snapshot, including omitted optional fields
          )
          break
        }
    ...
    async function bootstrap(input: { fatal?: boolean } = {}) {
      ...
      const workspace = project.workspace.current()
      // neocode_change start - isolate workspace-scoped Neo state
      ...
      const version = vcsVersion // neocode_change
      ...
      const globalConfigPromise = sdk.client.global.config.get({ throwOnError: true }) // neocode_change
      await Promise.all([
        providersPromise,
        providerListPromise,
        agentsPromise,
        configPromise,
        globalConfigPromise, // neocode_change
        projectPromise,
        ...
        .then(async () => {
          ...
          const globalConfigResponse = globalConfigPromise.then((x) => x.data!) // neocode_change
          ...
          return Promise.all([
            ...
            agentsResponse,
            configResponse,
            globalConfigResponse, // neocode_change
            ...
            const globalConfig = responses[5] // neocode_change
            ...
            batch(() => {
              ...
              setStore("globalConfig", reconcile(globalConfig)) // neocode_change
    ...
    // neocode_change start - re-bootstrap when Agent Manager changes workspace
    createEffect(
      on(
            ...
            setStore(
              produce((draft) => {
                ...
                const infos = (messages.data ?? []).flatMap((message) => {
                  if (!tracker.messages.has(message.info.id)) return [strip(message.info)] // neocode_change
