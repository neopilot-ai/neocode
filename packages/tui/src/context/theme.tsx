...
const themeSource: ThemeSource = {
  async discover() {
    ...
    for (let current = process.cwd(); ; current = path.dirname(current)) {
      // neocode_change start - discover Neo config roots, not OpenCode roots
      directories.push(path.join(current, ".neocode"), path.join(current, ".neo"))
      // neocode_change end
...
export async function discoverThemes(directories: string[]) {
  ...
  for (const directory of directories) {
    ...
    for (const file of files) {
      // neocode_change start - one malformed custom theme must not discard all themes
  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "neo") // neocode_change
        draft.active = typeof active === "string" ? active : "neo" // neocode_change
        draft.ready = false
      }),
    )
        ...
        .then((themes) => {
          setCustomThemes(
            Object.entries(themes).reduce<Record<string, ThemeJson>>((result, [name, theme]) => {
              // neocode_change - protect built-ins and require core theme colors
              if (!(name in DEFAULT_THEMES) && isValidTheme(theme)) result[name] = theme
              return result
            }, {}),
          )
        })
        .catch(() => setStore("active", "neo")) // neocode_change
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "neo") // neocode_change
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.neo, store.mode) // neocode_change
    })
...
// neocode_change start - custom themes must contain the colors required for safe fallback rendering
