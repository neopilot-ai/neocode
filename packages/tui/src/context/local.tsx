...
import { useProject } from "./project" // neocode_change
...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    const sync = useSync()
    const sdk = useSDK()
    const project = useProject() // neocode_change
    ...
    function createAgent() {
      ...
      return {
        ...
        current() {
          // neocode_change start - fall back to first agent when current is removed (e.g. org switch)
        ...
        move(direction: 1 | -1) {
          batch(() => {
            ...
            if (!value) return // neocode_change - guard against empty agent list during org switch
    ...
    function createModel() {
      const [modelStore, setModelStore] = createStore<{
        ready: boolean
        // neocode_change start - persisted picks plus process-local overrides
        model: Record<
          string,
          | {
              providerID: string
      ...
      const state = {
        pending: false,
        writer: Promise.resolve() as Promise<unknown>, // neocode_change - serialize writes
      }

      // neocode_change start - keep configured-agent selections process-local
      ...
      function save() {
        ...
        state.pending = false
        // neocode_change start - serialize writes so a slow first write cannot overwrite a later one
        const data = {
      ...
      readJson<unknown>(filePath)
        .then((x) => {
          ...
          if (typeof value.model === "object" && value.model !== null)
            setModelStore("model", value.model as Record<string, { providerID: string; modelID: string } | undefined>) // neocode_change
        })
        .catch(() => {})
        .finally(() => {
      ...
      const currentModel = createMemo(() => {
        ...
        if (!a) return fallbackModel() // neocode_change - guard against empty agent list
        // neocode_change start - configured models beat stale persisted picks
        return (
          getFirstValidModel(
      ...
      return {
        ...
        // neocode_change start - expose persisted per-agent pick separately from overrides
        saved(name: string) {
          return modelStore.model[name]
        },
        // neocode_change end
        // neocode_change start - resolve once all queued writes (atomic write+rename) have settled.
        ...
        cycle(direction: 1 | -1) {
          ...
          const a = agent.current()
          if (!a) return
          apply(a.name, val, !a.model) // neocode_change
          save() // neocode_change
        },
        cycleFavorite(direction: 1 | -1) {
          ...
          const a = agent.current()
          if (!a) return
          apply(a.name, next, !a.model) // neocode_change
        ...
        set(model: { providerID: string; modelID: string }, options?: { recent?: boolean }) {
          batch(() => {
            ...
            const a = agent.current()
            if (!a) return
            apply(a.name, model, !a.model) // neocode_change
            if (options?.recent) {
    ...
    createEffect(() => {
      // neocode_change start - configured models resolve directly without persistence
