...
import { createStore, produce, reconcile } from "solid-js/store" // neocode_change
import { createSimpleContext } from "./helper"
import { useSDK } from "./sdk"
import { createSignal, onMount } from "solid-js"
import { hydrate } from "../neocode/hydration" // neocode_change
...
export const { use: useData, provider: DataProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const sdk = useSDK()
    const syncing = new Map<string, Promise<void>>() // neocode_change
    ...
    // neocode_change end
      switch (event.type) {
        ...
        case "session.next.compaction.ended":
          // neocode_change start - legacy v1 compaction events do not carry a projectable message identity.
    ...
    // neocode_change start - project live V2 session events into the hydrated message store
    ...
    const result = {
      session: {
        ...
        message: {
          ...
          async refresh(sessionID: string) {
            // neocode_change start - reconcile the snapshot without losing live updates received while it was loading
