...
import { InvalidRequestError } from "../errors" // neocode_change
...
>()("@opencode/HttpApiLocation", { error: InvalidRequestError }) {} // neocode_change - surface malformed headers as 400s
...
function ref(request: HttpServerRequest.HttpServerRequest) {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-neo-workspace"]
  const header = request.headers["x-neo-directory"]
  // neocode_change start - decode the SDK header without turning malformed client input into a defect
  return Effect.try({
    try: () => query.get("location[directory]") || (header ? decodeURIComponent(header) : process.cwd()),
    catch: () => new InvalidRequestError({ message: "Invalid encoded directory header", field: "x-neo-directory" }),
  }).pipe(
    Effect.map((directory) =>
      Location.Ref.make({
        directory: AbsolutePath.make(directory), // neocode_change
  ...
  Effect.gen(function* () {
    ...
    return LocationMiddleware.of((effect) =>
      Effect.gen(function* () {
        ...
        const location = yield* ref(request) // neocode_change - reject malformed encoded directory headers as 400s
