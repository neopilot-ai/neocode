...
import { Location } from "@opencode-ai/core/location" // neocode_change
  ...
  Effect.gen(function* () {
    ...
    return handlers
      ...
      .handle(
        "session.create",
        Effect.fn(function* (ctx) {
          const location = yield* Location.Service // neocode_change
          return {
            data: yield* session.create({
              id: ctx.payload.id,
              agent: ctx.payload.agent,
              model: ctx.payload.model,
              // neocode_change start - honor createNeoClient's configured Location
              location:
