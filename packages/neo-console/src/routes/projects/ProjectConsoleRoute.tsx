...
import { Badge } from "@neocode/neo-web-ui/badge"
import { Button } from "@neocode/neo-web-ui/button"
import { Card } from "@neocode/neo-web-ui/card"
import { Icon } from "@neocode/neo-web-ui/icon"
import { ResizeHandle } from "@neocode/neo-web-ui/resize-handle"
import { Spinner } from "@neocode/neo-web-ui/spinner"
import { File } from "@neocode/neo-web-ui/file"
import { FileComponentProvider } from "@neocode/neo-web-ui/context/file"
import { SessionReview, type SessionReviewDiffStyle } from "@neocode/neo-web-ui/session-review"
...
export function ProjectConsoleRoute() {
  ...
  const [selected, setSelected] = createSignal(window.localStorage.getItem(`neo.console.${params.project}.dir`) ?? "")
  const [active, setActive] = createSignal(window.localStorage.getItem(`neo.console.${params.project}.pty`) ?? "")
  ...
  function labelKey(dir: string) {
    return `neo.console.${project()}.worktree.${encodeURIComponent(dir)}.label`
  }
  ...
  function remember(dir: string, pty?: string) {
    window.localStorage.setItem(`neo.console.${project()}.dir`, dir)
    if (pty) {
      window.localStorage.setItem(`neo.console.${project()}.pty`, pty)
      return
    }
    window.localStorage.removeItem(`neo.console.${project()}.pty`)
  }
  ...
  function submitEditor() {
    ...
    run("Creating worktree", async () => {
      ...
      window.localStorage.setItem(`neo.console.${project()}.dir`, next.directory)
    })
  }
  ...
  function addSession(item = current()) {
    ...
    const label = `Neo ${terminalsFor(item.dir).length + 1}`
      ...
      <main class="project-console-main">
        ...
        <Show when={!terminal() && !snap.loading && !snap.error && !failure()}>
          <div class="project-terminal-empty">
            ...
            <span>Use + next to a worktree to start Neo CLI.</span>
          </div>
        </Show>
      </main>
