...
import { loadNeoProfile, logoutNeo, setNeoOrganization, type NeoProfileData, type ProjectQuery } from "../../client"
...
type Org = NonNullable<NeoProfileData["profile"]["organizations"]>[number]
type State = { kind: "connected"; data: NeoProfileData } | { kind: "disconnected" }

async function load(input: ProjectQuery): Promise<State> {
  if (wasDisconnected()) return { kind: "disconnected" }
  try {
    const data = await loadNeoProfile(input)
...
export function ProfileRoute() {
  ...
  const login = () => page(params(), "/neo/login")
  ...
  function choose(id: string | null) {
    ...
    setSaving(next)
    setError("")
    void setNeoOrganization(current, id)
        ...
        <div class="profile-page">
          <Show when={!disconnected()}>
            <header class="profile-header">
              <div>
                <p class="eyebrow">Neo Account</p>
                <h1>Your Profile</h1>
                <p>Manage your Neo identity, account context, credits, and billing shortcuts.</p>
              </div>
          ...
          <Show when={disconnected()}>
            <Card class="profile-connect-card">
              ...
              <div class="profile-connect-copy">
                <p class="eyebrow">Neo Account</p>
                <h1>Connect your Neo account</h1>
                <p>Sign in to view your credits, organizations, and account details in Neo Console.</p>
              </div>
              <A
                class="profile-primary-link"
                data-component="button"
          ...
          <Show when={!server.query() && !server.discoverable()}>
            <Card class="profile-banner" variant="warning">
              <strong>Neo server not found</strong>
              <span>Start Neo Console from a running Neo server or pass a server URL with ?server=.</span>
            </Card>
          </Show>
                  ...
                  <div class="profile-org-list">
                    ...
                    >
                      ...
                      <span class="profile-org-body">
                        ...
                        <span>Your personal Neo credits and settings</span>
                      </span>
