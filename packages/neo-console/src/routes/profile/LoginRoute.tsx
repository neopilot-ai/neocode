...
import { completeNeoLogin, loadNeoProfile, startNeoLogin, type ProjectQuery } from "../../client"
  ...
  function start(input: ProjectQuery | undefined = server.query()) {
    ...
    void startNeoLogin(input)
      .then((info) => {
        ...
        return completeNeoLogin(input, ctl.signal).then(() => true)
      })
      .then((ok) => {
        if (!ok || attempt() !== rev) return
        setAuth({ status: "success" })
        void loadNeoProfile(input)
    ...
    <section class="route-empty">
      <div class="profile-login-page">
        <header class="profile-login-header">
          <p class="eyebrow">Neo Login</p>
          <h1>Login to Neo</h1>
          <p>Authorize this Neo Console through the same device auth flow used by the editor clients.</p>
        </header>
        ...
        <Show when={!server.query() && !server.discoverable()}>
          <Card class="profile-login-card" variant="warning">
            ...
            <p>Start a local Neo server or pass a server URL with ?server=.</p>
