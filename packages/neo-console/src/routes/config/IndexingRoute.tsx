...
import type { IndexingConfig } from "@neocode/sdk/v2/client"
...
export function IndexingRoute() {
  ...
  const neoModels = createMemo<SelectOption<string>[]>(() => {
    const models = catalog()?.models ?? []
    if (models.length === 0) return [{ value: "", label: "No Neo embedding models available", disabled: true }]
    return models.map((model) => ({
      value: model.id,
      label: `${model.name} (${model.note ? `${model.note}, ` : ""}${model.dimension}d)`,
    }))
  })
  const neoModel = createMemo(() => {
    const data = catalog()
    if (!data) return ""
        ...
        <section class="builder-form agent-builder-stack">
          ...
          <Card class="ui-card agent-builder-card" padding={0}>
            ...
            <div class="ui-form agent-builder-form">
              <FieldCard
                label="Provider"
                description="Automatic uses Neo when signed in, otherwise the provider runtime default."
                actions={
                  <SourceBadge
              ...
              <FieldCard
                ...
                description={
                  provider() === "neo"
                    ? "Select a Neo-hosted embedding model."
              ...
              >
                <input
                  ...
                  disabled={Boolean(ctx.saving()) || provider() === "neo"}
                  onInput={(event) => number("dimension", event.currentTarget.value)}
                />
              </FieldCard>

              <Show when={provider() === "neo"}>
                <div class="indexing-note">
                  Neo embeddings use the account currently signed in to this Neo server. Model dimensions are supplied
                  by the catalog.
                </div>
              </Show>
                ...
                fallback={
                  ...
                  >
                    <input
                      ...
                      placeholder="Default Neo state directory"
                      disabled={Boolean(ctx.saving())}
                      onInput={(event) =>
