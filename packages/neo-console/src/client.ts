import { createNeoClient, type Config as EffectiveConfig } from "@neocode/sdk/v2/client"
import type {
  ...
  GlobalEvent,
  NeoEmbeddingModelCatalog,
  NeoProfileResponse,
  LspStatusResponse,
  McpStatusResponse,
  Pty as PtyInfo,
  PermissionRequest,
  Project as NeoProject,
  QuestionRequest,
  ProviderAuthAuthorization,
  ProviderAuthResponse,
  ProviderListResponse,
  Session as NeoSession,
  SessionStatus,
  ToolIdsResponse,
  ToolListResponse,
...
export type NeoProfileData = NeoProfileResponse

export type ProjectItem = NeoProject
...
function server(input: string) {
  ...
  const user = decode(url.username) || "neo"
  const pass = decode(url.password) || "neo"
  url.username = ""
  url.password = ""
  url.search = ""
  url.hash = ""
...
function owner(items: ProjectItem[], session: Pick<NeoSession, "projectID" | "directory">) {
...
export async function loadEmbeddingModels(input: Query): Promise<NeoEmbeddingModelCatalog> {
  return demand("Neo embedding models", await client(input).indexing.models())
}

export async function loadNeoProfile(input: ProjectQuery): Promise<NeoProfileData> {
  const sdk = client(input)
  const result = await sdk.neo.profile(directory(input))
  return demand("Neo profile", result)
}

export async function setNeoOrganization(input: ProjectQuery, organizationId: string | null) {
  const sdk = client(input)
  const result = await sdk.neo.organization.set({ ...directory(input), organizationId })
  demand("Switch Neo account", result)
  await sdk.global.dispose()
}

export async function logoutNeo(input: ProjectQuery) {
  const sdk = client(input)
  const result = await sdk.auth.remove({ providerID: "neo" })
  demand("Log out of Neo", result)
  await sdk.global.dispose()
}

export async function startNeoLogin(input: ProjectQuery): Promise<ProviderAuthAuthorization> {
  const sdk = client(input)
  const result = await sdk.provider.oauth.authorize({ ...directory(input), providerID: "neo", method: 0 })
  return demand("Start Neo login", result)
}

export async function completeNeoLogin(input: ProjectQuery, signal?: AbortSignal) {
  const sdk = client(input)
  const result = await sdk.provider.oauth.callback(
    { ...directory(input), providerID: "neo", method: 0 },
    signal ? { signal } : undefined,
  )
  demand("Complete Neo login", result)
  await sdk.global.dispose()
}
...
export async function loadProjectTerminals(input: ProjectQuery, dir: string): Promise<ProjectTerminalItem[]> {
  ...
  const sessions = new Map<string, NeoSession>()
...
export async function createProjectPty(input: Query, dir: string, title = "Neo session"): Promise<ProjectPtyInfo> {
  const sdk = client({ url: input.url, dir })
  const result = await sdk.pty.create({ directory: dir, command: "neo", cwd: dir, title })
