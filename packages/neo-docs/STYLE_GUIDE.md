...
description: "Guidelines for writing Neo Code documentation"
---

# Documentation Style Guide

This guide covers writing, formatting, and structuring documentation for the Neo Code docs site.

## Voice and Tone

Neo Code documentation should be:
...
- Use present tense
- Be specific: "Run `neo run` to execute a task" not "You can run neo run"
...
```markdown
{% callout type="tip" %}
**The easiest way to configure Neo is to ask the agent.** Just tell the agent what you want.
{% /callout %}
```
...
- Use `neo run` for CLI examples, not hypothetical commands

````markdown
```bash
neo run "create a utils.py file with a function that adds two numbers"
```
````
...
- `neo.jsonc` for configuration files
...
- Use generated screenshots from `packages/neo-docs/public/img/screenshot-tests/` when available
...
| Term | Use for |
|---|---|
| Neo Code | The product name |
| neo CLI | The command-line interface |
| VS Code extension | The VS Code extension |
| JetBrains plugin | The JetBrains IDE plugin |
| `neo serve` | The local HTTP server |
| `neo run` | The headless execution command |
