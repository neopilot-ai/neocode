...
This is the Neo Code documentation site. Neo Code is the leading open source agentic engineering platform.
...
```markdown
{% image src="/docs/img/neo-provider/connected-accounts.png" alt="Connect account screen" width="800" caption="Connect account screen" /%}
```

Note that this site is served under neo.khulnasoft.com/docs so the `/docs` prefix **must** be present in every image path.

#### Generated screenshots

When updating screenshots for active docs pages, prefer generated screenshot-test assets from `packages/neo-docs/public/img/screenshot-tests/` and reference them as `/docs/img/screenshot-tests/...`. Only replace a hand-captured image when the generated screenshot matches the docs content closely. Do not replace screenshots in VSCode Legacy docs tabs or sections.

If a docs page references a generated VS Code visual-regression screenshot, record that usage in `packages/neo-vscode/tests/visual-regression.spec.ts` by adding the story ID to the `DOCS` map. Keep `packages/neo-vscode/tests/visual-regression.spec.mts` in sync while that file exists. If no matching generated screenshot exists, add or update a Storybook story in `packages/neo-vscode/webview-ui/src/stories/` and let visual-regression CI generate the baseline.
...
Before writing documentation, review `packages/neo-docs/STYLE_GUIDE.md` for voice, tone, and formatting conventions.
