---
title: "Custom Modes"
description: "Create and configure custom modes in Neo Code"
---

# Custom Modes

Neo Code allows you to create **custom modes** (also called **agents**) to tailor Neo's behavior to specific tasks or workflows. Custom modes can be **global** (available across all projects), **project-specific** (defined within a single project), or **organization-managed** (provided by your Neo organization).

{% callout type="info" %}
The current VS Code extension (built on the Neo CLI) uses **agent Markdown files** to define custom modes. The legacy extension used `custom_modes.yaml` / `.neocodemodes`. See the tabs below for the relevant approach.
{% /callout %}
...
If your Neo organization provides custom modes, Neo adds them to your local experience as organization-sourced agents/custom modes. They appear alongside built-in and personal agents so members can select them directly where Neo shows user-selectable agents or modes.
...
### 1. Ask Neo! (Recommended)

Ask Neo to create an agent for you:

```
Create a new agent called "docs-writer" that can only read files and edit Markdown files.
```

Neo will generate the agent definition and write it to `.neo/agent/` in your project.
...
```
.neo/agents/my-agent.md
.neo/agent/my-agent.md
.neocode/agents/my-agent.md
```

For global agents, place files in your global config directory:

```
~/.config/neo/agent/my-agent.md
```
...
**Example agent file** (`.neo/agents/docs-writer.md`):
...
### 4. Config File (`neo.jsonc`)

Define agents under the `agent` key in your project's `neo.jsonc`:
...
2. Global config (`~/.config/neo/neo.jsonc`)
3. Project config (`neo.jsonc` at project root)
4. `.neo/` / legacy `.neocode/` directory configs and agent `.md` files
5. Environment variable overrides (`NEO_CONFIG_CONTENT`)
...
```jsonc
// neo.jsonc — override the built-in "code" agent
{
  "agent": {
    "code": {
      "model": "openai/gpt-4o",
...
Or as a `.md` file (`.neo/agents/code.md`):
...
If you have existing `.neocodemodes` or `custom_modes.yaml` files from the VSCode extension, the extension automatically migrates them on startup. The migration converts:
...
**Legacy `custom_modes.yaml` is not loaded from `~/.config/neo/`.** If you're migrating from the previous VS Code extension, global custom modes are read from `~/.neocode/cli/global/settings/custom_modes.yaml` (not from the CLI's XDG config directory). The recommended approach is to convert legacy modes to agent `.md` files and place them in `~/.config/neo/agent/` instead — see [Markdown files](#3-markdown-files-with-yaml-frontmatter) and [Migration](#migration-from-vscode-extension-modes) below.
{% /callout %}
...
The TUI also **remembers the last model you picked for each agent** across sessions. A config-pinned `model` acts as the default when no manual pick exists. To reset a pick and let the config take over, use the model picker (`Ctrl+X m`) and select a different model, or remove the saved pick from `~/.local/state/neo/model.json`.
...
If you have existing `.neocodemodes` or `custom_modes.yaml` files from the VSCode extension, the CLI automatically migrates them on startup. The migration converts:
...
| 2 | `~/.neocode/cli/global/settings/custom_modes.yaml` | YAML | Global |
| 3 | `~/.neocodemodes` | YAML | Global |
| 4 | `<project>/.neocodemodes` | YAML | Project (wins on conflict) |

{% callout type="info" %}
`~/.config/neo/` is the XDG config directory for the new agent format — legacy `custom_modes.yaml` placed there will **not** be loaded. Use `~/.config/neo/agent/*.md` or `~/.config/neo/neo.jsonc` for new agent definitions instead.
{% /callout %}
...
{% callout type="tip" %}
Ask Neo to create or validate permission glob patterns when you need more complex file restrictions.
{% /callout %}
...
### Basic Documentation Writer (`.neo/agents/docs-writer.md`)
...
### Test Engineer (`.neo/agents/test-engineer.md`)
...
### Security Reviewer (`.neo/agents/security-review.md`)
...
- **Agent not appearing:** Ensure the `.md` file is in a recognized directory (`.neo/agents/`, `.neo/agent/`, `.neocode/agents/`). Check that the `mode` property is `primary` or `all` if you expect it in the agent picker.
...
- **Legacy modes are auto-migrated:** If you have `.neocodemodes` files, they'll be converted on startup — no manual migration needed
...
- **Keep prompts focused:** The markdown body is your system prompt — write it as if briefing a colleague
- **Use `mode: subagent`** for helper agents that shouldn't be directly selectable by users
- **Test with `neo agent create`** to see how the CLI generates agent definitions, then customize from there
- **Legacy modes are auto-migrated:** If you have `.neocodemodes` files, they'll be converted on startup — no manual migration needed
...
Ready to explore more? Check out the [Show and Tell](https://github.com/Neopilot-Ai/neocode/discussions/categories/show-and-tell) to discover and share custom modes and agents created by the community!
