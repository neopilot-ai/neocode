...
description: "Provide custom instructions to guide Neo Code"
---

# Custom Instructions

Custom Instructions allow you to personalize how Neo Code behaves, providing specific guidance that shapes responses, coding style, and decision-making processes. Both the **VSCode** and **CLI** versions support custom instructions, though the mechanisms differ.

## What Are Custom Instructions?

Custom Instructions define specific Extension behaviors, preferences, and constraints beyond Neo's basic role definition. Examples include coding style, documentation standards, testing requirements, and workflow guidelines.
...
Neo automatically discovers instruction files at your project root and in parent directories (via `findUp`). The following filenames are recognized:

- **`AGENTS.md`** — The primary instruction file for Neo
...
The `instructions` key in `neo.jsonc` accepts an array of paths, globs, or URLs pointing to additional instruction files. You can manage these in **Settings → Agent Behaviour → Rules** subtab.
...
## Legacy `.neocoderules` Support

If your project contains `.neocoderules` files from the VSCode extension, these are still loaded via auto-migration. However, migrating to `AGENTS.md` is recommended for new projects.
...
Each agent can have its own custom prompt defined in its `.md` file (the markdown body) or via the `agent.<name>.prompt` key in `neo.jsonc`:
...
Or as the markdown body in `.neo/agents/code.md`:
...
The `instructions` key in `neo.jsonc` accepts an array of paths, globs, or URLs pointing to additional instruction files. Configure these in your `neo.jsonc`:
