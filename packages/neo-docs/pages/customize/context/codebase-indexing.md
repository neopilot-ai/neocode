...
Codebase Indexing enables semantic code search across your entire project using AI embeddings. Instead of searching for exact text matches, it understands the _meaning_ of your queries, helping Neo Code find relevant code even when you don't know specific function names or file locations.
...
4. **Provides the [`semantic_search`](/docs/automate/tools/semantic-search) tool** to Neo Code for intelligent code discovery
...
- **Enhanced AI Understanding**: Neo Code can better comprehend and work with your codebase
...
1. Open Neo Code **Settings** → **Indexing**, or click the indexing indicator at the bottom of the prompt input panel.
...
You can also edit the `indexing` section in `neo.jsonc` directly:
...
- **LanceDB** (default). Embedded and file-based, with no server to run. Stores data under your Neo data directory by default.
...
Open a Neo TUI session and run:
...
All changes are written to your `neo.jsonc` config and take effect immediately.
...
- `lancedb` uses `{ directory? }` and is the default. It is embedded and file-based, with no server to run. Neo uses its data directory when `directory` is omitted.
...
Set `indexing.fileExtensions` to a non-empty array to index only the listed file extensions. Values are case-insensitive and may be written with or without a leading dot. When this setting is omitted, Neo uses its built-in language list. Configured text formats without a Tree-sitter parser use line-based fallback chunking.
...
- Files matching `.gitignore` and [`.neocodeignore`](/docs/customize/context/neocodeignore) patterns
...
- **API Keys**: Stored in your `neo.jsonc` config. Treat that file as a secret in shared environments.
...
- Check that `indexing.enabled` is `true` in your `neo.jsonc`
...
Once indexed, Neo Code can use the [`semantic_search`](/docs/automate/tools/semantic-search) tool to find relevant code:
...
The tool provides Neo Code with:
...
- **Secure storage**: API keys are stored in your local `neo.jsonc` configuration
- **Fully local option**: Use **Ollama + LanceDB** for completely local processing
- **Access control**: Respects existing file permissions and `.neocodeignore`
