...
Neo Code includes two built-in subagents:
...
Add agents to the `agent` section of your `neo.jsonc` config file. Any key that doesn't match a built-in agent name creates a new custom agent.
...
- **Global**: `~/.config/neo/agents/`
- **Project-specific**: `.neo/agents/`
...
2. **Global config** (`~/.config/neo/config.json`)
3. **Project config** (`neo.jsonc` in the project root)
4. **Global agent markdown files** (`~/.config/neo/agents/*.md`)
5. **Project agent markdown files** (`.neo/agents/*.md`)
