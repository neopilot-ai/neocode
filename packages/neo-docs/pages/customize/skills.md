---
title: "Skills"
description: "Extend Neo Code capabilities with skills"
---

# Skills

Neo Code implements [Agent Skills](https://agentskills.io/home), a lightweight, open format for extending AI agent capabilities with specialized knowledge and workflows.
...
1. **Discovery**: Skills are scanned from designated directories when Neo Code initializes. Only the metadata (name, description, and file path) is read at this stage—not the full instructions.
...
Global skills are located in the `.neo` directory within your Home directory:

- Mac and Linux: `~/.neo/skills/`
- Windows: `\Users\<yourUser>\.neo\skills\`
...
Located in `.neo/skills/` within your project:
...
For interoperability with other tools, Neo Code also loads skills from:
...
You can configure extra skill locations and remote skill URLs in your `neo.jsonc` config (project or global):
...
When multiple skills share the same name, project-level skills (`.neo/skills/`) take precedence over global skills (`~/.neo/skills/`). Skills from compatibility directories (`.claude/skills/`, `.agents/skills/`) and additional configured paths are loaded alongside project and global skills.
...
- In the **CLI**: Skills are loaded when you start a new session or run `neo run`
...
In Neo Code, the `name` field **must match** the parent directory name:
...
- **[Neo Marketplace repository](https://github.com/Neopilot-Ai/neo-marketplace)** — Browse community skills on GitHub and manually download them into your skills directory
...
3. **Check file location**: Ensure `SKILL.md` is directly inside the skill directory (e.g., `.neo/skills/my-skill/SKILL.md`), not nested further.

4. **Check config paths**: If using `skills.paths` or `skills.urls`, verify the paths and URLs are correct in your `neo.jsonc`.
...
Have you created a skill that others might find useful? Share it with the community by contributing to the [Neo Marketplace](https://github.com/Neopilot-Ai/neo-marketplace)!

{% tabs %}
{% tab label="VSCode" %}

While the new platform does not yet have a built-in marketplace UI, skills from the [Neo Marketplace repository](https://github.com/Neopilot-Ai/neo-marketplace) can be manually downloaded into your `.neo/skills/` directory or loaded via `skills.urls` in config.
