---
title: "Workflows"
description: "Create automated workflows with Neo Code"
platform: new
---

# Workflows

Workflows (also called **slash commands** in the new extension) automate repetitive tasks by defining step-by-step instructions for Neo Code to execute.

{% image src="/docs/img/screenshot-tests/neo-vscode/visual-regression/settings/agent-behaviour-workflows-chromium-linux.png" alt="Workflows tab in Neo Code" width="420" caption="Workflows tab in Neo Code" /%}

## Creating Workflows

Workflows are Markdown files stored as **slash commands** in `.neo/commands/`:

- **Global commands**: `~/.config/neo/commands/` (available in all projects)
- **Project commands**: `[project]/.neo/commands/` (project-specific)
...
For example, a file at `.neo/commands/submit-pr.md` is invoked with `/submit-pr`.
...
The new extension automatically migrates legacy workflows from `.neocode/workflows/` to the new command format on startup. You can also manually move files and remove the `.md` extension from invocations.
...
Create a file called `submit-pr.md` in your `.neo/commands` directory:
