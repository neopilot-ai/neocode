...
description: "Architecture of the Neo JetBrains split-mode plugin"
---

# JetBrains Plugin Architecture

The JetBrains plugin (`packages/neo-jetbrains/`) is a split-mode Swing client of [Neo CLI runtime](/docs/contributing/architecture/cli-runtime). Frontend module renders IDE UI. Backend module owns project-local logic and one bundled `neo serve` server. Shared module defines cross-process RPC contracts and serializable payloads.
...
flowchart LR
  ...
  subgraph backend ["JetBrains backend"]
    ...
    conn["NeoConnectionService"]
    workspaces["Directory workspace cache"]
    cli["Extracted neo serve --port 0"]
  end
...
|---|---|---|
| `NeoAppRpcApi` | Application | Connect, state flow, health, retry, restart, reinstall, model state, profile, login, telemetry |
| `NeoWorkspaceRpcApi` | Directory | Resolve real backend project directory, workspace state flow, reload, file lookup, open file |
| `NeoSessionRpcApi` | Session and directory | Create/list sessions, prompt, stream events, permission and question replies, config update |
| `NeoMigrationRpcApi` | Legacy migration | Detect, run, and observe migration state |
...
```text
<PathManager.getSystemPath()>/neo/bin/neo
<PathManager.getSystemPath()>/neo/bin/neo.exe   # Windows
```

It chooses platform resource by OS and CPU architecture, reuses extracted binary when resource size matches, and can force re-extraction during reinstall flow. This editor-owned child is separate from detached local daemon managed by `neo daemon`.

| Area | Behavior |
|---|---|
| Spawn | Runs extracted binary as `neo serve --port 0` |
| Port | CLI server prefers `4096`, then asks OS for free port; backend reads listening line from stdout |
| Authentication | Generates random 32-byte hex password and passes `NEO_SERVER_PASSWORD`; username defaults to `neo` |
...
This mirrors CLI `InstanceStore`: directory remains isolation key while one editor-owned `neo serve` process serves multiple workspace contexts.
...
| Check | Command from `packages/neo-jetbrains/` |
|---|---|
...
| Concern | Source path |
|---|---|
| Split modules | `packages/neo-jetbrains/settings.gradle.kts` and module XML descriptors |
| Contributor constraints | `packages/neo-jetbrains/AGENTS.md` |
| CLI lifecycle | `packages/neo-jetbrains/backend/src/main/kotlin/ai/neocode/backend/cli/NeoBackendCliManager.kt` |
| Connection recovery | `packages/neo-jetbrains/backend/src/main/kotlin/ai/neocode/backend/app/NeoBackendConnectionService.kt` |
| Workspace cache | `packages/neo-jetbrains/backend/src/main/kotlin/ai/neocode/backend/workspace/NeoBackendWorkspaceManager.kt` |
| Kotlin client generation | `packages/neo-jetbrains/backend/build.gradle.kts` |
| RPC contracts | `packages/neo-jetbrains/shared/src/main/kotlin/ai/neocode/rpc/` |
