...
description: "Contributor patterns for Neo architecture implementation and fork maintenance"
---

# Development Patterns

This page turns architecture boundaries into contributor decisions. Read [Architecture Overview](/docs/contributing/architecture) and relevant subsystem page first, then use this guide before editing architecture-facing code in `Neopilot-Ai/neocode` or its cross-repository contracts.

{% callout type="info" title="Default rule" %}
Prefer Neo-owned seams over broad changes to shared OpenCode files. Follow neighboring style when changing existing modules.
{% /callout %}
...
|---|---|---|
| Additive Neo CLI behavior | `packages/opencode/src/neocode/` | Keeps Neo-only behavior out of upstream-owned files |
| Neo CLI test for additive behavior | `packages/opencode/test/neocode/` | Avoids shared tests that encode only Neo behavior |
| Required shared OpenCode edit | Small import, route, or injection seam in shared file plus `neocode_change` marker | Keeps upstream diff narrow and merge review obvious |
| VS Code, JetBrains, docs, indexing, UI, gateway, or telemetry change | Existing Neo-owned package | These packages are Neo-owned; do not add `neocode_change` markers |
...
| Neo-only config-key change | Update CLI Effect Schema and cloud JSON Schema overlay | Runtime acceptance and editor validation are separate cross-repository paths |
| Docs page move or removal | Update nav and add permanent redirect | Preserves external links and bookmarks |

## Neo-owned boundaries

Neo CLI forks upstream OpenCode. Prefer Neo-owned directories and packages for additive behavior:

| Prefer | Avoid unless necessary |
|---|---|
| `packages/opencode/src/neocode/` | Broad edits to shared `packages/opencode/src/` files |
| `packages/opencode/test/neocode/` | Shared tests that encode only Neo behavior |
| `packages/neo-vscode/`, `packages/neo-jetbrains/`, `packages/neo-docs/`, `packages/neo-indexing/` | Moving Neo-only behavior into upstream-owned modules |
| Narrow import or route seams in shared files | Refactors that enlarge upstream merge conflicts |

## Shared OpenCode files

Use `neocode_change` markers when Neo-specific code must modify shared upstream files.

| Change shape | Marker |
|---|---|
| One line | Trailing `// neocode_change` |
| Multi-line block | `// neocode_change start` and `// neocode_change end` |
| New file in shared path | Top-level `// neocode_change - new file` |
| JSX or TSX | JSX comment equivalents |

Marker exemptions apply to paths already owned by Neo, including paths whose names contain `neocode` and Neo packages such as `packages/neo-vscode/` or `packages/neo-ui/`. Do not add markers there.

| Guard | When to run |
|---|---|
| `bun run script/check-opencode-annotations.ts` | PR touches `packages/opencode/`; verifies shared OpenCode Neo edits are annotated |
...
| `bun run check-neocode-change` from `packages/neo-vscode/` | VS Code or Neo UI changes; markers must not appear in fully Neo-owned packages |
...
| Put additive Neo groups and handlers under `packages/opencode/src/neocode/server/httpapi/` | Reduces edits in shared upstream-owned files |
| Inject Neo APIs through narrow shared seam | Keeps upstream diff small and marker placement obvious |
...
Runtime config loading and editor validation are separate paths. New Neo-only config key requires CLI Effect Schema change in `Neopilot-Ai/neocode` and JSON Schema overlay change in `Neopilot-Ai/cloud`. Follow [CLI Config Schema](/docs/contributing/architecture/config-schema) for exact workflow.
...
Import specific export when practical. Use namespace shape (`Session.create`) when preserving existing API or grouped module access improves clarity. Existing Neo-owned namespaces remain valid; do not refactor them solely for style.
...
| Neo route seam | `packages/opencode/src/neocode/server/httpapi/` |
| JavaScript SDK generation | `packages/sdk/js/script/build.ts`{% linebreak /%}`script/generate.ts` |
| JetBrains client generation | `packages/neo-jetbrains/backend/build.gradle.kts` |
...
Keep Neo-specific logic extracted, shared seams narrow, markers accurate, and CI guards green before upstream merge work lands.
