...
description: "Overview of the Neo Code platform architecture"
---

# Architecture Overview

This page maps Neo Code's repository-defined architecture. It introduces the local runtime, editor clients, cloud service boundaries, and hosted execution products before the subsystem pages add implementation detail.
...
| Repository | Contents |
|---|---|
| [Neo&#8209;Org/neocode](https://github.com/Neopilot-Ai/neocode) | Neo CLI runtime, local daemon, Neo Console, VS Code extension, JetBrains plugin, JavaScript SDK, codebase indexing, Neo Gateway client, telemetry, docs, and shared UI packages |
| [Neo&#8209;Org/cloud](https://github.com/Neopilot-Ai/cloud) | Web control plane, Neo Gateway routes, Cloud Agent session runtime, automation, generated-application preview and deployment services, NeoClaw, Gas Town, billing, and supporting Workers |

## Three architecture layers

| Layer | Responsibility | Typical boundaries |
|---|---|---|
| Local runtime and clients | Runs local coding sessions and connects editor surfaces to one local agent engine | Neo CLI runtime, `neo serve` server, local daemon, Neo Console, VS Code extension, JetBrains plugin |
| Neo Cloud shared services | Handles hosted identity, authorization, model routing, billing, orchestration, and shared product services | Web control plane, Neo Gateway, Workers, queues, Durable Objects, persistence |
| Hosted product runtimes and automation | Runs scoped cloud work for coding, app generation, assistants, security analysis, and multi-agent orchestration | Cloud Agent, Automation Services, App Builder, Security Agent, NeoClaw, Gas Town, Wasteland |

Local execution and hosted execution are separate boundaries. Editor clients use a local `neo serve` server. Hosted automation can launch Cloud Agent execution sessions when cloud coding work is required.

## Terms used throughout

| Term | Meaning |
|---|---|
| Neo Code | Umbrella product across local clients, Neo CLI runtime, and Neo Cloud services |
| Neo CLI runtime | Local agent engine in `packages/opencode/`; owns tools, sessions, config, persistence, and provider routing |
...
| Web control plane | Hosted Neo Cloud application layer for identity, organization authorization, billing, product configuration, and API orchestration |
| Neo Gateway | First-party hosted model-routing boundary |
...
flowchart LR
  ...
  subgraph local ["Local Neo CLI boundary"]
    daemon["Local daemon manager"]
    server["neo serve server"]
    runtime["Neo CLI runtime"]
    router["Provider router"]
  end

  subgraph cloud ["Neo Cloud shared services"]
...
|---|---|---|---|
| Local coding | Neo CLI, Neo Console, VS Code, or JetBrains | Neo CLI runtime on developer machine | Editor clients talk to local `neo serve` server. Local runtime owns coding session and sends model requests directly or through Neo Gateway. |
| Hosted work | Webhook, source-control event, command, schedule, or hosted product | Neo Cloud services; Cloud Agent when coding is required | Cloud services coordinate work. Only flows that need repository changes launch Cloud Agent execution session. |
...
flowchart LR
  ...
  chat["Neo Chat, events, and notifications"]
  claw["NeoClaw"]
  town["Gas Town"]
  wasteland["Wasteland"]
...
| NeoClaw | Coordinates owner-scoped hosted assistant runtimes | [Cloud Platform](/docs/contributing/architecture/cloud-platform#neoclaw) | [NeoClaw ingress](/docs/contributing/architecture/cloud-security#neoclaw-ingress) |
...
These local surfaces live in [`Neopilot-Ai/neocode`](https://github.com/Neopilot-Ai/neocode). Package paths below are relative to that repository root.

| Surface | Package in `Neopilot-Ai/neocode` | Runtime model |
|---|---|---|
| Neo CLI TUI | `packages/opencode/` | Interactive local client with daemon attach and worker-backed fallback paths |
| `neo run` | `packages/opencode/` | Headless prompt execution through explicit attach, daemon attach, or embedded fallback |
| `neo serve` | `packages/opencode/` | Local HTTP + SSE server for local clients |
| Neo Console | `packages/neo-console/`{% linebreak /%}`packages/opencode/` | Browser UI served at `/console` by a started or reused local daemon |
| VS Code extension | `packages/neo-vscode/` | Extension host starts one shared editor-owned `neo serve` server and routes webviews through HTTP + global SSE; SDK directory selects local runtime instance |
| JetBrains plugin | `packages/neo-jetbrains/` | Split-mode Swing plugin; backend module starts one editor-owned `neo serve` server and caches workspace clients by directory |

## Cloud service families

Hosted service families live in [`Neopilot-Ai/cloud`](https://github.com/Neopilot-Ai/cloud). Paths below are relative to that repository root unless another repository is named.

| Boundary | Primary source paths | Role |
|---|---|---|
| Neo Cloud | `apps/web/`{% linebreak /%}`services/` | Hosted platform repository for identity, billing, routing, product configuration, automation, and scoped execution services |
| Web control plane | `apps/web/` | Hosted application layer for authorization, configuration, and API orchestration |
| Neo Gateway | `apps/web/src/app/api/gateway/`{% linebreak /%}`apps/web/src/lib/ai-gateway/`{% linebreak /%}Local integration: `Neopilot-Ai/neocode/packages/neo-gateway/` | First-party model-routing boundary and local client integration |
...
| Adjacent hosted boundaries | `services/app-builder/`{% linebreak /%}`services/neoclaw/`{% linebreak /%}`services/gastown/`{% linebreak /%}`services/wasteland/`{% linebreak /%}Supporting services | App Builder, NeoClaw, Gas Town, Wasteland, chat, notifications, and supporting services |

## Supporting packages

These supporting packages also live in [`Neopilot-Ai/neocode`](https://github.com/Neopilot-Ai/neocode). Package paths below are relative to that repository root.

| Package in `Neopilot-Ai/neocode` | Role |
|---|---|
| `packages/neo-indexing/` | Per-directory asynchronous codebase indexing engine behind Neo CLI bridge |
| `packages/sdk/js/` | Generated JavaScript client and handwritten wrapper for local server APIs |
| `packages/neo-gateway/` | Local Neo Gateway client integration used by Neo CLI runtime |
| `packages/neo-console/` | Browser UI served by local daemon at `/console` |

## Architecture pages

| Page | What it covers |
|---|---|
| [CLI Runtime](/docs/contributing/architecture/cli-runtime) | Local execution modes, daemon, server authentication, routing, persistence, snapshots, SDK, config, SSE, Neo Console, and indexing |
| [VS Code Extension](/docs/contributing/architecture/vscode-extension) | Shared local `neo serve` ownership, webview bridge, Agent Manager, PTYs, recovery, bundled resources, and build outputs |
| [JetBrains Plugin](/docs/contributing/architecture/jetbrains-plugin) | Split-mode modules, RPC, bundled local `neo serve` lifecycle, Kotlin SDK, recovery, and remote-development constraints |
...
- [NeoClaw Overview](/docs/neoclaw/overview) - customer-facing NeoClaw docs
