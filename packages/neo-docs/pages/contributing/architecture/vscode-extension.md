...
description: "Architecture of the Neo VS Code extension and Agent Manager"
---

# VS Code Extension Architecture

The VS Code extension (`packages/neo-vscode/`) is a client of [Neo CLI runtime](/docs/contributing/architecture/cli-runtime). It bundles platform CLI binary, starts one shared editor-owned `neo serve` server on demand, and drives that server through generated SDK HTTP calls plus global SSE.
...
Activation creates one `NeoConnectionService`. It owns one `ServerManager`, one active SDK client, and one SSE adapter. `ServerManager` owns child process lifecycle. This editor-owned child is separate from detached local daemon managed by `neo daemon`.

```mermaid
flowchart LR
  subgraph host ["VS Code extension host"]
    consumers["Sidebar, tabs, panels, services"]
    service["NeoConnectionService"]
    manager["ServerManager"]
    sdk["Generated SDK client"]
    sse["SdkSSEAdapter"]
  end

  server["bin/neo serve --port 0"]
...
| Binary | Uses extension `bin/neo`, or `bin/neo.exe` on Windows |
| Port | Starts `neo serve --port 0`; CLI server prefers `4096`, then asks OS for free port |
| Authentication | Generates random 32-byte hex password per spawn and passes it as `NEO_SERVER_PASSWORD`; username defaults to `neo` |
...
| Panels | Settings, profile and marketplace surfaces, sub-agent viewers, Agent Manager, NeoClaw |
| Diff | Diff Viewer, Diff Virtual, and diff source catalog |
| Editor assistance | Autocomplete and commit-message generation |
| Integrations | Browser automation MCP registration and NeoClaw bootstrap |
...
webview vscode.postMessage()
  -> NeoProvider host handler
  -> generated SDK HTTP request
  -> CLI runtime
  -> /global/event SSE
  -> SdkSSEAdapter
  -> NeoConnectionService subscribers
  -> NeoProvider directory/session filtering and stream coalescing
  -> webview postMessage()
```
...
| Backend | Shared `neo serve` process | Same shared process |
...
Agent Manager persists state in `.neo/agent-manager.json` and worktrees under `.neo/worktrees/`. Startup migration moves Agent Manager-owned data from legacy `.neocode/` paths when target items do not already exist and repairs git worktree refs.
...
| CLI PTY WebSocket tab | Agent Manager and `neo serve` server | Server-created PTY session streamed over loopback WebSocket |

Agent Manager PTY WebSocket URL uses `auth_token=<base64 neo:password>` query mode because browser WebSocket API cannot attach Basic header. Webview CSP permits loopback HTTP and WebSocket origins for active server port. CLI also exposes scope-bound short-lived PTY ticket API as alternate browser WebSocket auth mode.

## Config split

| Config owner | Examples |
|---|---|
| VS Code settings | `neo-code.new.*` extension UI, proxy, autocomplete, and integration settings |
| CLI config | Global and project `neo.jsonc`, `neo.json`, compatible OpenCode files, provider auth, tools, permissions, modes |
...
| Resource | Behavior |
|---|---|
| CLI executable | Platform binary under extension `bin/`; Windows uses `neo.exe` |
| CLI Tree-sitter WASM | Copied under `bin/tree-sitter`; backend spawn sets `NEO_TREE_SITTER_WASM_DIR` |
...
| Empty-window indexing | Sets `NEO_DISABLE_CODEBASE_INDEXING=vscode-no-workspace` so CLI reports indexing disabled |

Speech-to-text captures audio locally, then sends completed recording through shared editor-owned `neo serve` server to authenticated Neo Gateway transcription path. It is batch transcription, not direct provider streaming.
...
| NeoClaw webview | `webview-ui/neoclaw/index.tsx` | `dist/neoclaw.js` |
...
Extension host bundle targets Node/CommonJS. Browser webviews and shared worker use esbuild browser bundles. Run `bun run typecheck`, `bun run lint`, and targeted unit tests from `packages/neo-vscode/` after changing this area.
...
| Concern | Source path |
|---|---|
| Activation | `packages/neo-vscode/src/extension.ts` |
| Editor-owned server child process | `packages/neo-vscode/src/services/cli-backend/server-manager.ts` |
| Shared SDK and SSE ownership | `packages/neo-vscode/src/services/cli-backend/connection-service.ts` |
| SSE reconnect adapter | `packages/neo-vscode/src/services/cli-backend/sdk-sse-adapter.ts` |
| Agent Manager | `packages/neo-vscode/src/agent-manager/` |
| Build entries | `packages/neo-vscode/esbuild.js` |
