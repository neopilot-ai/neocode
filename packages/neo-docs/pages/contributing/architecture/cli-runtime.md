...
description: "Architecture of the Neo CLI runtime, daemon, server, persistence, SDK, and indexing"
---

# CLI Runtime Architecture

The CLI (`packages/opencode/`) is Neo Code's local agent engine. It owns agent execution, tools, sessions, provider integration, configuration, local persistence, directory routing, and HTTP surfaces used by editor clients and Neo Console.
...
| Term | Meaning |
|---|---|
| Neo CLI runtime | Local agent engine in `packages/opencode/` |
| `neo serve` server | Local HTTP and SSE process used by editor clients and Neo Console; selected browser-oriented paths also use WebSocket |
| Local daemon | Detached reusable `neo serve` server managed by `neo daemon` commands |
| Directory context | Normalized local filesystem directory used to select local runtime state |
| Local runtime instance | Directory-keyed runtime context inside one Neo CLI process |
...
| Process-shared state | Runtime service state shared by every directory context in one Neo CLI process |
| Modes | Configurable agent presets for tools, prompts, restrictions, and behavior |
| MCP | Protocol for extending agent tools |

One `neo serve` process can host several local runtime instances. Directory-keyed state stays isolated. Process-shared service state does not.

## Command entry points

| Entry point | Command or caller | Runtime model |
|---|---|---|
| Interactive TUI | `neo` | Attaches to local daemon when available; otherwise starts Bun worker and sends SDK-shaped requests over RPC |
| Headless run | `neo run` | Uses daemon attach when available, then embedded server fetch fallback |
| Attached run | `neo run --attach <url>` | Targets explicit running `neo serve` server |
| Explicit API server | `neo serve` | Starts HTTP + SSE server for external local clients |
| Local daemon | `neo daemon start` | Starts detached `neo serve` child for reuse |
| Browser console | `neo console` | Starts or reuses local daemon and opens daemon-served `/console` UI |
| Editor-spawned server | VS Code or JetBrains client | Starts bundled `neo serve --port 0` child owned by editor client, not local daemon manager |

```mermaid
flowchart LR
  run["neo run"]
  tui["neo TUI"]
  daemon["Detached daemon: neo serve"]
  worker["Bun worker"]
  rpc["RPC-backed fetch and global events"]
  embedded["Embedded Server.Default().app.fetch"]
  serve["Explicit neo serve"]
  editors["VS Code or JetBrains"]
  editorServer["Editor-owned neo serve --port 0"]
  runtime["Neo CLI runtime"]
...
Each running editor host starts one editor-owned `neo serve` server. That server can handle coding sessions for workspace root and additional worktree directories at same time. It does not start separate server process for each directory.

```mermaid
flowchart LR
  views["Editor views<br/>Workspace root and worktrees"]
  server["One editor-owned<br/>neo serve server"]
  store["InstanceStore"]
...
|---|---|---|
| Local `neo serve` access | HTTP, SSE, and selected WebSocket access to local server | Neo CLI server and spawning local client |
| Outbound provider authentication | Model provider, Neo Gateway, catalog, and indexing access | Neo CLI provider router and auth stores |
| Remote MCP OAuth | Browser authorization and credentials for remote MCP server | Neo CLI MCP runtime |

### Local `neo serve` access

Server Basic Auth is optional. It becomes required when `NEO_SERVER_PASSWORD` is non-empty. Default username is `neo`; `NEO_SERVER_USERNAME` can override it.
...
| PTY shell child | Removes `NEO_SERVER_PASSWORD` and `NEO_SERVER_USERNAME` from spawned user-shell environment |
...
Provider auth records use `api`, `oauth`, or `wellknown` variants in `${Global.Path.data}/auth.json`, written with mode `0600`. `NEO_AUTH_CONTENT` can supply process-local auth JSON. Separate v2 multi-account auth store also exists for account-oriented flows.

| Path | Behavior |
|---|---|
| Direct providers | Use provider-specific keys, OAuth records, environment values, and configured endpoints |
| Neo Gateway | Resolves Neo model access and model catalog through gateway client |
| Anonymous Neo | If no Neo key exists, provider loader sets API key value `anonymous`; gateway model catalog can fall back to public unauthenticated endpoint |
| Organization catalog | Neo model fetch includes organization ID when resolved from config, auth, or environment |
...
| Indexing auth | Resolves indexing-specific Neo config first, then provider config, auth record, provider options, and `NEO_API_KEY` / `NEO_ORG_ID` environment values |
...
2. `x-neo-directory` request header.
3. Server process cwd.

Local routing workspace selection is separate. Session workspace, `workspace` query parameter, and `NEO_WORKSPACE_ID` can select workspace context. Configured `NEO_WORKSPACE_ID` keeps requests local to current workspace runtime. Other selected workspaces resolve through workspace-routing adapter to local directory or remote target.
...
| Tool registry | Loads built-in, Neo-specific, MCP, and readiness-gated semantic search tools |
...
| Provider router | Resolves direct providers, Neo Gateway, custom endpoints, and credentials |
| HTTP server | Publishes REST, WebSocket, and SSE surfaces |

## Daemon lifecycle

`neo daemon start|status|stop|restart` manage a detached local `neo serve` child, with bare `neo daemon` equivalent to `neo daemon start`. `neo console` calls the same start path, so it reuses a healthy daemon instead of spawning a second process, while `neo console stop` aliases `neo daemon stop`.
...
| Child process | Detached `neo serve --hostname <host> --port <port>` process |
...
| Opt-out | `NEO_NO_DAEMON` disables automatic attach by clients; explicit daemon commands still manage daemon |

Daemon credentials differ from editor-spawned server credentials. Current daemon source stores username `neo`, password `neo`, and base64 Basic token in `daemon.json`. File permissions protect this local credential record. Editor clients generate random passwords per spawned server.
...
| Area | Behavior |
|---|---|
| Default database | `${Global.Path.data}/neo.db` |
| Override | `NEO_DB`; relative paths resolve under data directory; `:memory:` is accepted |
...
Snapshot implementation state is directory-keyed through `InstanceState`. One `Snapshot.Service` also owns process-shared slow-snapshot guard state outside directory cache. This distinction matters when multiple Agent Manager worktrees use same `neo serve` process.
...
3. Neo-specific API groups and handlers live under `packages/opencode/src/neocode/server/httpapi/` and enter shared API through narrow injection seams.
4. `packages/sdk/js/script/build.ts` generates TypeScript v2 client from CLI OpenAPI.
5. `packages/sdk/js/src/v2/client.ts` adds `createNeoClient()` wrapper for directory and workspace routing, Electron and Node fetch compatibility, and clearer empty-response errors.
...
| Order | Source |
|---|---|
| 1 | Legacy Neo migrations |
| 2 | Organization modes |
| 3 | Auth-record `.well-known/opencode` remote config |
| 4 | Global config files |
| 5 | Explicit `NEO_CONFIG` file |
| 6 | Project `neo.json[c]` and `opencode.json[c]` files plus discovered config directories |
| 7 | `NEO_CONFIG_DIR` directory |
| 8 | `NEO_CONFIG_CONTENT` |
| 9 | Active Neo Cloud organization config |
...
Global config files load from `${Global.Path.config}`. Project updates prefer existing config files found in ancestor `.neo` or legacy `.neocode` directories, then existing project root config files, then create `.neo/neo.json`. Global indexing settings can carry provider and storage defaults, but global `indexing.enabled` is stripped so project enablement remains local in effective instance config.
...
Runtime config loading is separate from editor-facing JSON Schema publication. Cloud-served schema improves validation and completion for `neo.json` and `neo.jsonc`; it does not load, apply, or override effective runtime config. When adding or changing config key, follow [CLI Config Schema](/docs/contributing/architecture/config-schema) so CLI source and cloud overlay stay aligned.
...
`neo console` starts or reuses daemon, opens `/console`, and prints Console launch URL. Browser launch URL embeds daemon Basic credentials so initial request authenticates.

| Area | Behavior |
|---|---|
| Frontend | Solid/Vite app in `packages/neo-console/` |
...
| SDK | Console calls generated JavaScript SDK through `createNeoClient()` |
...
`packages/neo-indexing/` owns indexing engine. CLI bridge injects indexing plugin by default unless default plugins are disabled, then starts indexing asynchronously per normalized directory during instance bootstrap.

| Area | Behavior |
|---|---|
| Bootstrap | `NeocodeBootstrap` forks indexing initialization so instance startup is not blocked |
...
| Worktrees | Agent Manager `.neo/worktrees/` and legacy `.neocode/worktrees/` paths return disabled status |
| Empty VS Code window | Extension sets `NEO_DISABLE_CODEBASE_INDEXING=vscode-no-workspace`; bridge reports disabled status |
| Embeddings | Supports Neo, OpenAI, Ollama, OpenAI-compatible, Gemini, Mistral, Vercel AI Gateway, Bedrock, OpenRouter, and Voyage configuration |
| Vector stores | Supports Qdrant and LanceDB |

## Source map

Paths below are relative to [`Neopilot-Ai/neocode`](https://github.com/Neopilot-Ai/neocode).

| Concern | Source paths |
|---|---|
| CLI entry points | `packages/opencode/src/cli/cmd/` |
| Daemon | `packages/opencode/src/neocode/daemon/` |
...
| Snapshots | `packages/opencode/src/snapshot/index.ts`{% linebreak /%}`packages/opencode/src/neocode/snapshot/track.ts` |
| SDK | `packages/sdk/js/`{% linebreak /%}`script/generate.ts` |
| Console | `packages/neo-console/`{% linebreak /%}`packages/opencode/src/neocode/console/` |
| Indexing | `packages/neo-indexing/`{% linebreak /%}`packages/opencode/src/neocode/indexing.ts` |
