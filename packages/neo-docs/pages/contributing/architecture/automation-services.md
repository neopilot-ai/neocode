...
description: "Architecture of Neo Cloud automation services that dispatch scoped work"
---
...
This page describes cloud automation boundaries present in `Neopilot-Ai/cloud`. Static source shows supported code paths, Worker bindings, and deployable surfaces. It does not prove live production enablement or rollout policy.
{% /callout %}
...
|---|---|---|---|---|
| Neo Bot | GitHub, Slack, or Linear command ingress | Web control plane bot libraries | Cloud Agent for requested repository work | Command response and coding-session result |
...
| Auto Fix | `neo-auto-fix` label or dispatch rule | `auto-fix-infra` Durable Object per fix ticket | Cloud Agent branch and pull-request work | Pull request and lifecycle status callback |
...
| Webhook Agent Ingest | HTTP webhook or scheduled alarm | `webhook-agent-ingest` queue and `TriggerDO` | Cloud Agent or Neo Chat destination | Destination delivery and queue retry behavior |

## Neo Bot ingress and source-control targets

Neo Bot command ingress and repository target support are separate concerns.

| Concern | Current static-source statement |
|---|---|
| Command ingress | GitHub, Slack, and Linear are current Neo Bot command surfaces |
...
| GitLab command ingress | GitLab repository support does not imply note or comment-triggered Neo Bot commands |

Do not document GitLab issue notes or merge-request comments as current Neo Bot trigger surfaces unless source adds explicit ingress handling.
...
| Output | High-confidence classification applies labels such as `neo-auto-fix` for downstream Auto Fix |
...
| Destination | `neoclaw_chat` | Posts to user-scoped Neo Chat destination through Neo Chat service binding |
...
|---|---|
| Neo Bot | `apps/web/src/lib/bot/`{% linebreak /%}`apps/web/src/lib/bots/`{% linebreak /%}`apps/web/src/lib/slack-bot/` |
