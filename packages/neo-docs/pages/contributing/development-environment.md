...
{% callout type="info" %}
**New versions of the VS Code extension and CLI are being developed in [Neopilot-Ai/neocode](https://github.com/Neopilot-Ai/neocode)** (extension at `packages/neo-vscode`, CLI at `packages/opencode`). For extension and CLI development, please head over to that repository.
{% /callout %}
...
4. **Java 21** - Required only when running JetBrains plugin checks or repo-level checks that include `@neocode/neo-jetbrains`
...
1. **Fork and Clone the Repository**:
   - **Fork the Repository**:
     - Visit the [Neo Code GitHub repository](https://github.com/Neopilot-Ai/neocode)
...
- **`packages/opencode/`** - Neo CLI, agent runtime, local HTTP server, session management, and TUI
- **`packages/neo-vscode/`** - VS Code extension, webview UI, Agent Manager, and extension packaging
- **`packages/sdk/js/`** - Generated TypeScript SDK for the local server API
- **`packages/neo-docs/`** - Documentation site
- **`packages/neo-jetbrains/`** - JetBrains plugin
...
`bun dev` and `bun run dev` are equivalent. Both run the local source in `packages/opencode/`; they do not use a globally installed `neo` binary.

### Backend/API Validation

For backend and API validation, use the root [TESTING.md](https://github.com/Neopilot-Ai/neocode/blob/main/TESTING.md) guide. It covers starting the local backend with:
...
When the Extension Development Host opens, check the Neo Code output channel and the Developer Tools console for startup or webview errors.
...
Neo Code uses several types of tests to ensure quality:
...
Use the root [TESTING.md](https://github.com/Neopilot-Ai/neocode/blob/main/TESTING.md) guide for backend/API checks that require `bun dev serve` and `curl`-based requests.
...
- When editing shared `packages/opencode/` files, keep changes small and mark Neo-only edits with `// neocode_change` for a single line or `// neocode_change start` / `// neocode_change end` for a block. Do not add these markers inside `neocode`-named paths.
...
- Check the Output panel in VSCode (View > Output) and select "Neo Code" from the dropdown
