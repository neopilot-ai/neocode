---
title: "What's New in Neo Code (April 2026)"
description: "The Neo Code extension has been rebuilt from the ground up on the Neo CLI — faster, more flexible, and with access to 500+ models."
---

# What's New in Neo Code

The Neo Code extension has been completely rebuilt on a portable, open-source core shared across VS Code, the CLI, and Cloud Agents. This is the biggest update since launch: faster execution with parallel tool calls and subagents, the new Agent Manager for running multiple agents side by side, inline code review with line-level comments, multi-model comparisons, and access to 500+ models.

Whether you're writing features in VS Code, debugging over SSH, or reviewing code on Slack, Neo now goes with you. Read the [full announcement on the Neo Blog](https://blog.neo.khulnasoft.com/p/new-neo-for-vs-code-is-live) for everything that's new.
...
Code indexing is temporarily unavailable in the new extension. It is actively being worked on and is expected to return soon. Please follow [this issue](https://github.com/Neopilot-Ai/neocode/issues/6144)
...
You can expand or collapse the graph — your preference is saved in the `neo-code.new.showTaskTimeline` setting.

### I like to closely monitor and approve the behavior of the agent. How can I do that better in the new version?

We are working to improve the experience in closely managing an agent. Identified improvements and progress are being tracked in a [GitHub issue](https://github.com/Neopilot-Ai/neocode/issues/8415).

In the meantime we suggest exploring:

- [Auto-approval](https://neo.khulnasoft.com/docs/getting-started/settings/auto-approving-actions) of actions: to control what the agent is allowed to do, and require approval when desired
- [Agents](https://neo.khulnasoft.com/docs/code-with-ai/agents/using-agents) (previously known as Modes): Managing the agent types in the extension, adding new ones, and setting the default models for each.

### How can I control which models each agent/mode uses?

Modes have been renamed to Agents in the new extension. You can set the default model for each agent in `Settings -> Models -> Model per Mode`. For more information please check the [agents documentation](https://neo.khulnasoft.com/docs/code-with-ai/agents/using-agents).
