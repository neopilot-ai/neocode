---
title: "File Encoding"
description: "How Neo handles text file encodings when reading and editing files"
---

# File Encoding

Neo automatically detects the text encoding of each file it reads and preserves that encoding when writing changes back. You can work with source files in any supported encoding without worrying about Neo corrupting them or showing the model garbled text.
...
New files Neo creates are always UTF-8 without a BOM. Encoding detection only runs when Neo reads or overwrites an existing file.
...
If Neo displays a file as garbled text, or writes it back in a different encoding than it was saved in, please open an issue at [github.com/Neopilot-Ai/neocode/issues](https://github.com/Neopilot-Ai/neocode/issues) and include all of the following:
...
- **The model and provider** you were using when the issue occurred, for example `claude-sonnet-4.5` via Neo Gateway.
- **The exact Neo version** you are running. For the CLI, run `neo --version`. For the VS Code extension, open the Extensions view and check the version next to "Neo Code".
