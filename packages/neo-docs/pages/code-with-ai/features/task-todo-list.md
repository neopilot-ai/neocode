...
{% image src="/docs/img/screenshot-tests/neo-vscode/visual-regression/composite-webview/todo-write-docs-overview-chromium-linux.png" alt="Task Todo List overview showing interactive checklist in Neo Code" width="420" /%}
...
- Ask Neo to "use the [`todowrite` tool](/docs/automate/tools)"
- Say "create a todo list"

**The bottom line**: Neo decides what goes in the list, but you can provide feedback during approval dialogs.
...
**Now**: Neo creates structured checklists that update automatically as work progresses. You see exactly where you are and what's coming up.
...
{% image src="/docs/img/screenshot-tests/neo-vscode/visual-regression/chat/task-header-with-todos-chromium-linux.png" alt="Task header summary showing todo list progress" width="420" /%}
...
- Edit descriptions when Neo asks for approval
- Stage changes using the "Edit" button

**3. Environment Details**
Background "REMINDERS" table that keeps Neo informed about current progress
...
When Neo proposes a todo list update, you can edit the list before approving. Use the "Edit" button in the tool block to update item text, add or remove steps, or adjust status. Once approved, Neo continues with the updated list.

## Common questions

**"Can I create my own todo lists?"**
Yes, just ask Neo to use the `todowrite` tool. Neo stays in control of the content and workflow.

**"What about simple tasks?"**
Neo typically skips todo lists for simple tasks. The overhead isn't worth it.

**"Why can't I directly edit the list?"**
Design choice. Neo maintains authority over task management to ensure consistent progress tracking. You provide input, Neo executes.

---

## Settings

You can disable todo lists in Settings -> Advanced -> **Enable todo list tool**. When disabled, Neo won't create or update todo lists, and the REMINDERS table won't appear in Environment Details.
