...
This guide walks you through setting up Mistral's Codestral model for free autocomplete in Neo Code. Mistral offers a free tier that's perfect for getting started with AI-powered code completions.

## Prerequisites

- A [Neo Code account](https://app.neo.khulnasoft.com) (free to create)
...
## Step 4: Add Your Key via BYOK in Neo

1. Log into the [Neo platform](https://app.neo.khulnasoft.com).
2. Navigate to the [Bring Your Own Key (BYOK) page](https://app.neo.khulnasoft.com/byok), available in the sidebar under **Account**.
...
Once your BYOK key is saved, Neo Code's autocomplete will automatically use your Codestral key through the Neo Gateway. No additional configuration is needed in the extension.

1. Open VS Code with the Neo Code extension installed.
...
The autocomplete status bar in VS Code shows the current provider ("Neo Gateway") and tracks cumulative cost. With BYOK, requests are billed directly by Mistral at their rates (Codestral has a free tier) and show as $0.00 on your Neo balance.
...
```
Your Editor → Neo Gateway (with your key) → Mistral
```

- The Neo Gateway detects your BYOK key and routes autocomplete requests using it.
- You are billed directly by Mistral — Neo does not add any markup.
- If your BYOK key is invalid, the request will fail (it does not fall back to Neo's keys).

## Troubleshooting

- **Autocomplete not appearing?** Check that autocomplete is enabled in Neo Code settings (it is on by default). Also verify you are signed into Neo Code in the extension.
...
- **Seeing charges on your Neo balance?** If you haven't configured BYOK, autocomplete defaults to using your Neo credits. Add your Codestral key via BYOK to route requests through your own Mistral account.
