---
title: "Browser Use"
description: "Using Neo Code to interact with web browsers"
---

# Browser Use

Neo Code provides browser automation capabilities that let you interact with websites directly from your coding workflow. This feature supports testing web applications, automating browser tasks, and capturing screenshots without leaving your editor.
...
Browser automation is built into the extension and requires no manual setup. Enable it from **Settings → Browser** and Neo handles the rest automatically.

{% /tab %}
{% tab label="CLI" %}

Neo Code uses [Playwright](https://playwright.dev/) for browser automation. Add it to your `neo.jsonc` configuration:
...
1. Ask Neo to visit a website
2. Neo launches the browser and shows you a screenshot
3. Request additional actions (clicking, typing, scrolling)
4. Neo closes the browser when finished

For example:

- `Open the browser and view our site.`
- `Can you check if my website at https://neocode.ai is displaying correctly?`
...
Neo launches a browser automatically when asked and returns screenshots after each action so you can see what's happening. It can navigate to URLs, click elements, fill in forms, scroll, hover, select from dropdowns, and drag and drop — all driven by natural language instructions in chat.
...
Browser automation is configured in your `neo.jsonc` file. No additional settings are required — Playwright manages the browser lifecycle automatically.
