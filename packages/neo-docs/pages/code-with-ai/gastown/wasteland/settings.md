...
{% browserFrame url="app.neo.khulnasoft.com/gastown/town/settings/wasteland" caption="Wasteland settings tab in your Gas Town dashboard" %}
...
Your **rig handle** is your town's identity on the wasteland. It's an `org/repo`-style identifier (e.g., `neo/main`, `acme/backend`) that other participants see when you claim items and submit evidence.
...
{% browserFrame url="app.neo.khulnasoft.com/gastown/town/settings/wasteland" caption="DoltHub credentials dialog with OAuth default and advanced API token option" %}
...
### Default: OAuth through Neo Integrations

1. Go to [Neo Integrations → DoltHub](https://app.neo.khulnasoft.com/integrations/dolthub)
2. Click **Connect DoltHub**
3. Approve Neo on DoltHub
4. Return to Neo and confirm DoltHub shows **Connected**

When you join a wasteland, choose **Use your connected DoltHub account**. Confirm your DoltHub username so Neo can label commits and contribution branches correctly.
...
Your DoltHub credentials need read and write access. Neo uses them to:
...
Neo uses DoltHub OAuth for the normal Gas Town Wasteland flow. Use the advanced API token option only for setups that cannot use OAuth.
{% /callout %}
...
4. Approve Neo again on DoltHub
...
- DoltHub shows **Connected** in [Neo Integrations](https://app.neo.khulnasoft.com/integrations/dolthub)
