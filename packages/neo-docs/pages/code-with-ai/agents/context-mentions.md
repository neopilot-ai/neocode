...
description: "How to provide context to Neo Code using mentions"
---

# Context Mentions

Providing the right context helps Neo Code understand your project and perform tasks accurately. All platforms support `@`-mentions for referencing files, and the agent can also discover context on its own using built-in tools like `read`, `grep`, and `glob`.
...
Selected code and editor diagnostics (errors/warnings) are not included automatically. However, you can send these to Neo Code through VS Code's Code Actions: select code or hover over an error, then use the lightbulb menu to find context-dependent actions like "Explain with Neo Code" or "Fix with Neo Code."
...
- **Use `neo run -f`** — When using the non-interactive `neo run` command, pass `-f path/to/file.ts` to explicitly include a file's contents in the context.
...
| **Describe the task clearly** | The agent finds context on its own — focus on _what_ you want done rather than _where_ the code is |
| **Mention files when helpful** | If you know the exact file, mention its path to save the agent a search step |
| **Use `neo run -f`** | Pass key files with `-f` when using `neo run` for immediate context |
| **Trust the agent's tools** | The agent can search, read, and explore your codebase — let it do the discovery work |
