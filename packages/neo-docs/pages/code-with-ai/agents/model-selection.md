...
Instead of maintaining a static list that's perpetually behind, we built something better — a real-time leaderboard showing which models Neo Code users are actually having success with right now.

## Check the Live Models List

**[👉 See what's working today at neo.khulnasoft.com/models](https://neo.khulnasoft.com/models)**
...
- Set per-agent defaults and a global default in the **Settings** panel (Models tab), or directly in the `neo.jsonc` config file.
...
- For non-interactive use, pass `--model` flag to `neo run` (e.g., `neo run --model claude-sonnet-4-20250514`).
- Set the global default with the `model` key in `neo.jsonc`, or configure per-agent models in the `agent` section.
...
You don't need a paid API key to use Neo Code productively. For the lowest cost on paid work, [Auto Efficient](/docs/code-with-ai/agents/auto-model#tiers) (`neo-auto/efficient`) routes each request to the cheapest model proven accurate enough for that task. The fastest way to start for free is [Auto Model Free](/docs/code-with-ai/agents/auto-model) (`neo-auto/free`), which routes to the best available free models automatically. See [Using Neo for Free](/docs/getting-started/using-neo-for-free) for the full zero-cost setup.

If you prefer to pick models yourself, type `free` in the model picker to filter by free models, or browse the full list at [neo.khulnasoft.com/models](https://neo.khulnasoft.com/models).

{% callout type="info" %}
Free model availability changes as providers adjust promotional periods. Check [neo.khulnasoft.com/models](https://neo.khulnasoft.com/models) for the live list.
{% /callout %}
...
- **Via config file** — edit `neo.jsonc`:
...
```
vscode://neocode.neo-code/neocode/switch?model=<modelID>
vscode://neocode.neo-code/neocode/switch?agent=<agentName>
vscode://neocode.neo-code/neocode/switch?model=<modelID>&agent=<agentName>
```

Replace `<modelID>` with a Neo Gateway model ID such as `neo-auto/free`. Replace `<agentName>` with a visible primary agent ID such as `code` or `plan`, rather than its display name.

### Example: Auto Free

To open Neo Code and switch to the [Auto Free](/docs/code-with-ai/agents/auto-model) tier (`neo-auto/free`), use:

```
vscode://neocode.neo-code/neocode/switch?model=neo-auto%2Ffree
```

To switch only to Plan and use its normal model selection, specify the agent without a model:

```
vscode://neocode.neo-code/neocode/switch?agent=plan
```

To select both at the same time, include both parameters:

```
vscode://neocode.neo-code/neocode/switch?model=neo-auto%2Ffree&agent=plan
```
...
- **VS Code open**: the Neo sidebar is focused and the linked selection is applied to the active session immediately.
- **VS Code closed**: VS Code launches, then applies the selection once the extension is ready.
- When `model` is provided, it must identify a model in the current Neo Gateway catalog. Invalid or unavailable models cause the deep link to be ignored.
...
```html
<a href="vscode://neocode.neo-code/neocode/switch?model=neo-auto%2Ffree&amp;agent=plan">
  Open Neo Code with Auto Free in Plan
</a>
```
...
The AI model space moves fast. Bookmark [neo.khulnasoft.com/models](https://neo.khulnasoft.com/models) and check back when you're evaluating options. What's best today might not be best next month — and that's actually exciting.
