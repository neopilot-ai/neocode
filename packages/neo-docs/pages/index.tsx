...
const terminalContent = {
  installation: (
    <>
      <span className="terminal-comment"># Install Neo Code VS Code Extension</span>
      {"\n"}
      <span className="terminal-prompt">$</span> code --install-extension neocode.neo-code
      {"\n"}
      {"\n"}
      <span className="terminal-comment"># Or install via CLI</span>
      {"\n"}
      <span className="terminal-prompt">$</span> npm install -g @neocode/cli
    </>
  ),
  gateway: (
    <>
      <span className="terminal-comment"># Call Neo Gateway with a quick curl script</span>
      {"\n"}
      <span className="terminal-prompt">$</span> export NEO_API_KEY="YOUR_API_KEY"
      {"\n"}
      <span className="terminal-prompt">$</span> curl https://api.neo.khulnasoft.com/api/gateway/chat/completions \{"\n"}
      -H "Authorization: Bearer $NEO_API_KEY" \{"\n"}
      -H "Content-Type: application/json" \{"\n"}
      {`  -d '{"model":"anthropic/claude-sonnet-4.5","messages":[{"role":"user","content":"Say hi from Neo Gateway"}]}'`}
    </>
  ),
  firstTask: (
    <>
      <span className="terminal-comment"># Start a new task with Neo Code</span>
      {"\n"}
      <span className="terminal-prompt">$</span> neo "Create a React component for a user profile"
      {"\n"}
      {"\n"}
      <span className="terminal-comment">
        # Or for interactive sessions, just run the Neo CLI in your project folder
      </span>
      {"\n"}
      <span className="terminal-prompt">$</span> neo
      {"\n"}
      {"\n"}
      <span className="terminal-comment"># Run in architect mode for planning</span>
      {"\n"}
      <span className="terminal-prompt">$</span> neo --mode architect "Design a REST API"
    </>
  ),
  customRules: (
    <>
      ...
      {"\n"}
      <span className="terminal-prompt">$</span> touch .neocode/rules.md
      {"\n"}
      {"\n"}
      <span className="terminal-comment"># Or use the CLI to add rules</span>
      {"\n"}
      <span className="terminal-prompt">$</span> neo rules add "Always use TypeScript"
      {"\n"}
      <span className="terminal-prompt">$</span> neo rules add "Follow React best practices"
      {"\n"}
      {"\n"}
      <span className="terminal-comment"># List all active rules</span>
      {"\n"}
      <span className="terminal-prompt">$</span> neo rules list
    </>
  ),
  neoclaw: (
    <>
      <span className="terminal-comment"># Chat with your NeoClaw agent from the CLI</span>
      {"\n"}
      <span className="terminal-prompt">$</span> neo /claw
      {"\n"}
      {"\n"}
      ...
      {"\n"}
      <span className="terminal-prompt">$</span> curl -X POST https://your-instance.neoclaw.ai/webhook \{"\n"}
      -H "Content-Type: application/json" \{"\n"}
...
const categories = [
  {
    title: "Get Started",
    description: "Install Neo Code and get up and running in minutes",
  ...
  {
    title: "Code with AI",
    description: "Learn how to use Neo Code to write, edit, and understand code",
    href: "/code-with-ai",
    icon: (
      <svg className="category-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
  ...
  },
  {
    ...
    links: [
      { title: "Overview", href: "/neoclaw/overview" },
      { title: "Chat Platforms", href: "/neoclaw/chat-platforms" },
      { title: "Development Tools", href: "/neoclaw/development-tools" },
    ],
  },
  {
    title: "Collaborate",
    ...
    links: [
      ...
      { title: "Neo for Teams", href: "/collaborate" },
  ...
  {
    title: "Contributing",
    description: "Help improve Neo Code and learn about its architecture",
    href: "/contributing",
    icon: (
      <svg className="category-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path
...
export default function HomePage() {
  const [activeTab, setActiveTab] = useState<"installation" | "firstTask" | "customRules" | "gateway" | "neoclaw">(
    "installation",
  )
  ...
  return (
    <div className="homepage">
      <Head>
        <title>Neo Code Docs: Setup, Models, MCP, Custom Modes & CLI</title>
      </Head>
      ...
      <section className="hero">
        <div className="hero-content">
          <h1 className="hero-title">Neo Documentation</h1>
          <p className="hero-subtitle">
            Explore guides and examples for the Neo platform — from coding agents and AI-powered development to hosted
            agentic infrastructure with NeoClaw.
          </p>
          <div className="hero-buttons">
            <Link href="/getting-started" className="btn btn-primary">
              Get started with Neo Code →
            </Link>
            <Link href="/neoclaw/overview" className="btn btn-secondary">
              Explore NeoClaw
            </Link>
          </div>
        </div>
        ...
        <div className="quick-panel">
          <div className="quick-section">
            <h3 className="quick-title">NEO CODE</h3>
          ...
          <div className="quick-section">
            <h3 className="quick-title">NEOCLAW</h3>
            <div className="quick-links">
              <Link href="/neoclaw/overview" className="quick-link">
                Overview
              </Link>
              <Link href="/neoclaw/chat-platforms" className="quick-link">
                Chat Platforms
              </Link>
              <Link href="/neoclaw/development-tools" className="quick-link">
                Development Tools
              </Link>
            </div>
          </div>
      ...
      <section className="terminal-section">
        <div className="terminal-intro">
          ...
          <p className="terminal-description">Get started quickly with Neo Code, NeoClaw, and Neo Gateway</p>
        </div>
        <div className="terminal-container">
          <div className="terminal-tabs">
            ...
            <button
              className={`terminal-tab ${activeTab === "neoclaw" ? "active" : ""}`}
              onClick={() => setActiveTab("neoclaw")}
            >
              NeoClaw
            </button>
            <button
        ...
        <div className="footer-grid">
          <div className="footer-item">
            ...
            <div>
              ...
              <Link href="https://neo.khulnasoft.com/discord" className="footer-link">
                Join our Discord
              </Link>
            </div>
          </div>
          <div className="footer-item">
            ...
            <div>
              ...
              <Link href="https://github.com/Neopilot-Ai/neocode/releases" className="footer-link">
                Changelog
              </Link>
            </div>
          </div>
          <div className="footer-item">
            ...
            <div>
              ...
              <Link href="https://github.com/Neopilot-Ai/neocode/issues" className="footer-link">
                Report an issue
              </Link>
            </div>
          </div>
