---
title: "Migration"
description: "Migrate your team to Neo Code Enterprise"
---

# Migration

Switch to **Neo Teams** or **Neo Enterprise** from other AI coding tools and experience transparent pricing, no vendor lock-in, and superior team management capabilities.

## Why Teams Switch to Neo
...
**Neo Teams** and **Neo Enterprise** show you exactly what each AI request costs - no markup, no hidden fees, complete transparency.

### No Rate Limiting

**Other tools** slow you down with rate limits and model switching when you need AI most.

**Neo Teams** and **Neo Enterprise** never limit your usage - pay for what you use, use what you need.

### True Team Management

**Other solutions** offer basic user management with limited visibility.

**Neo Teams** provides comprehensive team analytics, role-based permissions, and detailed usage insights, while **Neo Enterprise** adds advanced governance, audit logging, and enterprise-level security controls.
...
### What You Gain with Neo Teams or Neo Enterprise
...
3. **Calculate current costs** (if visible) vs. Neo pricing
4. **Plan migration timeline** to minimize disruption

**Step 2: Neo Setup**

1. **Create organization** at [app.neo.khulnasoft.com](https://app.neo.khulnasoft.com)
2. **Subscribe to Teams ($15/user/month)** or **Enterprise ([Contact Sales](https://neo.khulnasoft.com/contact-sales))**
3. **Configure team settings** and usage policies
4. **Purchase initial AI credits** based on usage estimates

**Step 3: Team Migration**

1. **Invite team members** to Neo
2. **Install Neo Code extension** alongside Cursor initially
3. **Migrate projects gradually** starting with non-critical work
4. **Train team** on Neo Code features and workflows

**Step 4: Full Transition**

1. **Monitor usage patterns** in Neo dashboard
...
2. **Install Neo Code** extension for team members
3. **Start with simple tasks** in Neo Code
4. **Compare results** and team satisfaction

**Phase 2: Gradual Transition (Week 3-4)**

1. **Use Neo Code** for new projects
...
Neo Code uses a compatible rules system that supports Cursor and Windsurf patterns. Migrating your custom rules and configurations is straightforward and typically takes 5-10 minutes per project.

**Quick Overview:**

- **Project rules**: `.cursor/rules/*.mdc` → `.neocode/rules/*.md` (remove YAML frontmatter, keep Markdown content)
- **Legacy rules**: `.cursorrules` → `.neocode/rules/legacy-rules.md`
- **AGENTS.md**: Works identically in Neo Code (no conversion needed)
- **Global rules**: Recreate in `~/.neocode/rules/*.md` directory

Neo Code also supports mode-specific rules (`.neocode/rules-{mode}/`), which Cursor and Windsurf don't have. This allows different rules for different workflows (e.g., Code mode vs Debug mode).
...
- Neo Code extension installation and setup
...
- **Email:** migrations@neo.khulnasoft.com
...
Ready to make the switch? Contact our migration team at migrations@neo.khulnasoft.com to plan your transition to transparent AI coding.
