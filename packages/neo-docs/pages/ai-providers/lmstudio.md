---
title: "Using LM Studio with Neo Code | Local LLMs"
description: "Run local LLMs in Neo Code by connecting to LM Studio's OpenAI-compatible server. Setup guide for VS Code and the CLI."
sidebar_label: LM Studio
---

# Using LM Studio With Neo Code

Neo Code supports running models locally using LM Studio. LM Studio provides a user-friendly interface for downloading, configuring, and running local language models. It also includes a built-in local inference server that emulates the OpenAI API, making it easy to integrate with Neo Code.
...
2.  **Download a Model:** Use the LM Studio interface to search for and download a model. Some recommended models include:
    ...
    - Any other model that is supported by Neo Code, or for which you can set the context window.
...
If the model you loaded in LM Studio doesn't appear in the Neo model picker, you can register it as a custom model in your config file:
...
- **Local Server:** The LM Studio local server must be running for Neo Code to connect to it.
