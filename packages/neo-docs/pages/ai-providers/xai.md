---
title: "Using xAI Grok with Neo Code"
description: "Connect xAI's Grok models to Neo Code. Use a SuperGrok or X Premium subscription via OAuth or a paid API key. Guide to setup in VS Code and the CLI."
sidebar_label: xAI (Grok)
---

# Using xAI (Grok) With Neo Code
...
Neo Code supports two ways to connect xAI:
...
- **OAuth login — no API keys:** Sign in through your browser and Neo Code handles token management automatically.
- **Automatic token refresh:** Neo Code refreshes your access token in the background so long-running sessions stay authenticated.

{% callout type="note" %}
SuperGrok and X Premium subscription access works with Neo Code's core functionality (VS Code extension and CLI). For cloud features such as Cloud Agents or NeoClaw, use the [Neo Gateway](/docs/gateway) — the Gateway supports xAI via [BYOK](/docs/getting-started/byok) with an API key (OAuth and subscription-based access are not supported through the Gateway).
{% /callout %}
...
```bash
neo auth login --provider xai
```

Neo Code offers three methods at the prompt:
...
- **Sign out:** To disconnect in VS Code, use the "Disconnect" button in the provider settings. In the CLI, run `neo auth logout` and choose xAI.
...
- **Token rotation:** xAI rotates refresh tokens on each use. Neo Code persists the latest tokens automatically. If you run Neo Code from multiple processes simultaneously, the first refresh can invalidate the other process's token — re-run `neo auth login --provider xai` to restore the session.
...
3. **Create a Key:** Click to create a new API key. Give your key a descriptive name (e.g., "Neo Code").
