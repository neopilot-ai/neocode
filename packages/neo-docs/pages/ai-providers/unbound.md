---
title: "Using Unbound with Neo Code"
description: "Access Claude, GPT, and other leading models through the Unbound proxy in Neo Code. Setup guide for VS Code and the CLI."
sidebar_label: Unbound
---

# Using Unbound With Neo Code

Neo Code supports accessing models through [Unbound](https://getunbound.ai/), a platform that focuses on providing secure and reliable access to a variety of large language models (LLMs). Unbound acts as a gateway, allowing you to use models from providers like Anthropic and OpenAI without needing to manage multiple API keys and configurations directly. They emphasize security and compliance features for enterprise use.
...
2.  **Create an Application:** Go to the [Connect](https://gateway.getunbound.ai/connect) page and select "Neo Code".
3.  **Copy the API Key:** Copy the API key to your clipboard.

## Supported Models

Unbound allows you configure a list of supported models in your application, and Neo Code will automatically fetch the list of available models from the Unbound API.
...
Unbound is not yet available as a CLI provider. Check the [Neo Code releases](https://github.com/Neopilot-Ai/neocode/releases) for updates on provider support.
{% /callout %}
