---
title: "Slack"
description: "Using NeoClaw with Slack"
---

# Slack

This page covers everything you need to use NeoClaw with Slack: connecting your bot, controlling who can DM it, and adding it to channels.

## Connecting NeoClaw to Slack

{% youtube url="https://youtu.be/Q5bt-qH-_pY" title="Slack Setup Guide" caption="How to connect your NeoClaw agent to Slack" /%}

Create a Slack app from the OpenClaw manifest and link it to your NeoClaw dashboard.
...
4. Customize the manifest before creating:
   ...
   - Update the slash command if desired (e.g., `/neoclaw`)
5. Click **Create**
...
### Step 3: Connect Slack to NeoClaw

1. In the [NeoClaw UI](https://app.neo.khulnasoft.com/claw), find the Slack integration section (may show "not configured")
2. Enter both tokens:
   - The `xapp-` app-level token
   - The `xoxb-` bot user OAuth token
3. Click **Save**
4. Scroll to the top of the NeoClaw UI and click **Redeploy**. Wait for the instance to come back up

### Step 4: Pair Slack with NeoClaw

1. In Slack, DM the app and send any message — this triggers the pairing flow
2. The app will return a pairing code
3. Return to [app.neo.khulnasoft.com/claw](https://app.neo.khulnasoft.com/claw) and confirm the pairing code and approve
...
By default, NeoClaw can respond to any DMs and will not respond in Slack channels, even if added.

## Making NeoClaw DM-Only (from you)

By default, NeoClaw will respond to DMs from any user in Slack.
...
## Adding NeoClaw to a Slack Channel

By default, NeoClaw will not respond in Slack channels, even if added. To have NeoClaw participate in a Slack channel:
...
Tell your NeoClaw agent (via DM):
