...
description: "Answers to common NeoClaw troubleshooting questions"
---
...
2. Click **OpenClaw Doctor** on your [dashboard](/docs/neoclaw/dashboard)
...
1. Check that the channel token is configured in [Settings](/docs/neoclaw/dashboard#channels)
...
You can directly access the files in `/root/.openclaw/` on the [NeoClaw Dashboard](https://app.neo.khulnasoft.com/claw) using the file browser of the edit files dialog. This can be a useful way to examine or update the config files (especially `openclaw.json`) if you run into an issue. There may also be backups in the form of `openclaw.bak` files that you can manually restore from if needed.
...
3. If the issue persists, join the [Neo Discord](https://neo.khulnasoft.com/discord) and share details in the NeoClaw channel
