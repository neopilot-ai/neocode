...
Neo's Code Reviews integrate with GitLab to automatically review merge requests with AI. When an MR is opened, updated, or reopened, the Review Agent analyzes the changes and posts feedback directly on the merge request — as summary notes and inline discussion comments.

Both **GitLab.com** and **self-hosted GitLab instances** are supported.

## Prerequisites

- A Neo Code account at [app.neo.khulnasoft.com](https://app.neo.khulnasoft.com)
- A GitLab account with **Maintainer** role (or higher) on the projects you want to review
- Neo Code credits for AI model usage

> **Why Maintainer role?** Neo creates a bot account (Project Access Token) on each project so that review comments appear from a bot, not your personal account. This requires Maintainer access.
...
1. Go to **Code Reviews**:
   - **Personal**: [app.neo.khulnasoft.com/code-reviews](https://app.neo.khulnasoft.com/code-reviews)
...
When you select repositories, Neo **automatically creates webhooks** on each project.
...
1. A 👀 reaction appears on the MR — this means Neo is reviewing
...
Review comments are posted by a **Neo Code Review Bot** — not by your personal GitLab account. This bot is created automatically as a Project Access Token on each project.

- Created automatically the first time a project is reviewed
- Valid for 365 days and rotated automatically before expiry
- If you manually revoke the bot token in GitLab, Neo creates a new one on the next review
- Requires **Maintainer role** on the project

## Webhooks

Neo manages webhooks automatically:
...
- **URL**: `https://app.neo.khulnasoft.com/api/webhooks/gitlab`
...
- Ensure you have sufficient Neo Code credits
