...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Neo Code and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Neo Code) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Neo Code
...
- Single-client scenarios (one Neo Code instance per server)
...
1. The client (Neo Code) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Neo Code
...
STDIO servers run locally on the same machine as Neo Code, which has several important implications:
...
- **Execution**: Starts and stops with Neo Code (child process lifecycle)
...
- Start when needed by Neo Code
- Not require network configuration
- Need to be installed alongside Neo Code or via a package manager
...
## Configuring Transports in Neo Code

For detailed information on configuring STDIO and SSE transports in Neo Code, including example configurations, see the [Understanding Transport Types](using-in-neo-code#understanding-transport-types) section in the Using MCP in Neo Code guide.
