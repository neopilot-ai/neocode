---
title: "Using MCP in Neo Code"
description: "How to use MCP servers in Neo Code"
---

# Using MCP in Neo Code

Model Context Protocol (MCP) extends Neo Code's capabilities by connecting to external tools and services. This guide covers everything you need to know about using MCP with Neo Code.
...
MCP server configurations are stored inside the main Neo config file. There are two levels:

1. **Global Configuration**: `~/.config/neo/neo.jsonc` — applies to all projects.
2. **Project-level Configuration**: `neo.jsonc` in your project root, or `.neo/neo.jsonc` for a cleaner setup.

**Precedence**: Project-level configuration takes precedence over global configuration.

### Editing MCP Settings

You can edit MCP settings from the Neo Code settings UI:
...
If the UI cannot add a server, edit a Neo config file directly and add the server under the top-level `mcp` key. For project-specific servers, edit `./neo.json` or `./neo.jsonc` if your project already has one; otherwise use `./.neo/neo.json` or `./.neo/neo.jsonc` for a cleaner setup. For servers you want in every workspace, use `~/.config/neo/neo.json` or `~/.config/neo/neo.jsonc`.

### Config Format

MCP servers are configured under the `mcp` key in `neo.jsonc`:
...
Remote servers support OAuth 2.0 authentication. If the server supports it, Neo Code will automatically start the OAuth flow when you connect. You can also disable OAuth with `"oauth": false`.

{% /tab %}
{% tab label="CLI" %}

The CLI accepts several config filenames. The recommended file is `neo.json`:

| Scope | Recommended Path | Also supported |
|---|---|---|
| **Global** | `~/.config/neo/neo.json` | `neo.jsonc`, `opencode.json`, `opencode.jsonc`, `config.json` |
| **Project** | `./neo.json` or `./.neo/neo.json` | `neo.jsonc`, `opencode.jsonc`, `opencode.json` |
...
- **Remote (HTTP/SSE)**: Servers hosted over HTTP/HTTPS. Neo Code tries `StreamableHTTP` first, then falls back to `SSE` automatically.
...
| Command | Description |
|---|---|
| `neo mcp list` | List all configured MCP servers |
| `neo mcp add` | Add an MCP server |
| `neo mcp auth` | Authenticate with an MCP server |
| `neo mcp logout` | Log out from an MCP server |
| `neo mcp debug` | Debug an MCP server connection |
...
Run `neo mcp list` to verify the server status.
...
**In your config file:** Add the tool name (or a wildcard pattern) to the `permission` key in `neo.jsonc`:
...
Neo Code does not come with any pre-installed MCP servers. You'll need to find and install them separately.

- **Neo Marketplace:** Browse community-contributed MCP server configurations and agent skills in the [Neo Marketplace](https://github.com/Neopilot-Ai/neo-marketplace). The marketplace includes ready-to-use configs for popular tools like Figma, Sentry, and more.
- **Community Repositories:** Check for community-maintained lists of MCP servers on GitHub
- **Ask Neo Code:** You can ask Neo Code to help you find or even create MCP servers
- **Build Your Own:** Create custom MCP servers using the SDK to extend Neo Code with your own tools

For full SDK documentation, visit the [MCP GitHub repository](https://github.com/modelcontextprotocol/).

## Using MCP Tools in Your Workflow

After configuring an MCP server, Neo Code will automatically detect available tools and resources. To use them:

1. Type your request in the Neo Code chat interface
2. Neo Code will identify when an MCP tool can help with your task
...
- **Server Not Responding:** Check if the server process is running. Use `neo mcp debug <server-name>` to inspect the connection.
- **Permission Errors:** Ensure API keys and credentials are set in your `neo.jsonc` config or via `{env:VARIABLE_NAME}` references.
