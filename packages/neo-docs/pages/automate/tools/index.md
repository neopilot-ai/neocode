...
description: Learn how Neo Code's tools automate your development workflow
---

# Tool Use Overview

Neo Code implements a sophisticated tool system that allows AI models to interact with your development environment in a controlled and secure manner. This document explains how tools work, when they're called, and how they're managed.
...
| **Browser Group** | Web browser automation | `neo-playwright_*` (via built-in Playwright MCP) | Browser testing and interaction |
...
These tools help Neo Code understand your code and project:
...
These tools help Neo Code make changes to your code:
...
For multiple replacements in one file, Neo uses repeated `edit` calls or a patch-style edit when the model supports it.

### Execute Tools

These tools help Neo Code run commands:

- `bash` - Runs shell commands with configurable timeout and working directory

### Web Tools

These tools help Neo Code access web content:

- `webfetch` - Fetches a URL and returns the content
- `websearch` - Searches the web (available to Neo/OpenRouter users)

### Browser Tools

The VS Code extension has a built-in browser automation tool powered by [Playwright MCP](https://www.npmjs.com/package/@playwright/mcp). Enable it in Settings → Browser Automation. When enabled, it registers an MCP server named `neo-playwright` and exposes tools such as:

- `neo-playwright_browser_navigate` - Navigate to a URL
- `neo-playwright_browser_click` - Click an element
- `neo-playwright_browser_type` - Type text into an element
- `neo-playwright_browser_screenshot` - Capture a screenshot
- `neo-playwright_browser_snapshot` - Capture an accessibility snapshot
