...
How much you spend with Neo Code is shaped by several factors working together:
...
This page covers the controls currently available in Neo Code. For a direct overview of Auto Model tiers and token optimization tips, see [Cost Efficiency & Model Selection](/docs/getting-started/rate-limits-and-costs).
...
When the agent enters a repeated failure cycle — attempting the same action multiple times without making progress — Neo pauses and asks for permission before continuing. This is controlled by the `doom_loop` permission, which defaults to `ask`.

**Where to configure:** Settings → Auto Approve (VS Code) or the `permission.doom_loop` key in `neo.jsonc` (CLI).
...
Every action Neo takes — reading files, editing code, running shell commands, launching sub-agents — is governed by the permission system. Each tool can be set to `allow`, `ask`, or `deny`. When set to `ask`, Neo pauses before executing and you can approve or reject that specific action.

**Where to configure:** Settings → Auto Approve (VS Code) or the `permission` section in `neo.jsonc` (CLI). See [Auto-Approving Actions](/docs/getting-started/settings/auto-approving-actions) for the full list of available permissions.
...
A shield button in the prompt controls lets you toggle auto-approve on and off at runtime without opening Settings. When enabled, pending permission prompts are approved automatically. The state stays synced across the sidebar and open Neo tabs.
...
**Where to configure:** Organization spending limits are managed in the organization dashboard at [app.neo.khulnasoft.com](https://app.neo.khulnasoft.com). Individual credit top-up is at Settings → Adding Credits.

### Free model rate limits

Requests to free models (`neo-auto/free` and other free-tier models) are rate-limited to **200 requests per hour**. If you exceed this, requests return HTTP 429 and you must wait before continuing.
...
Neo automatically skips a set of directories including `node_modules`, `dist`, `build`, `.git`, `__pycache__`, `.cache`, and `vendor`. You can add additional paths using **permission deny rules** in `neo.jsonc` or using a `.neocodeignore` file at your workspace root.
...
**Where to configure:** `neo.jsonc` (VS Code / CLI). See [.neocodeignore](/docs/customize/context/neocodeignore) for full details.

### Compact long conversations

When a conversation grows long, use `/compact` in the chat (also searchable as `smol` or `condense`) to summarize the history and free up context space. Neo replaces older conversation turns with an anchored summary that captures your goal, constraints, progress, and next steps.

Auto-compaction is **enabled by default** — Neo automatically compacts when approaching the context window limit so you do not need to intervene manually.

**Where to configure:** Toggle auto-compaction in **Settings → Context** (VS Code) or set `compaction.auto` in `neo.jsonc`. Configure the trigger threshold with `compaction.threshold_percent` (e.g. `80` to compact at 80% of the model's context window).
...
**Where to configure:** Model settings in the Neo Code UI, or the `limit.output` key in custom model configuration.
...
Neo automatically applies prompt caching on supported providers. Repeated context — your system prompt, stable file contents, and tool definitions — is reused from cache at a discounted rate. No configuration is required to benefit from this.
...
Neo has auto-models that can help you control costs; more information is available in [Auto Model](/docs/code-with-ai/agents/auto-model).

### Practical examples by task type

| Task type | Suggested approach |
|---|---|
| Quick questions, syntax lookups, simple formatting | `neo-auto/efficient` or a lightweight model |
| Routine edits, test generation, straightforward refactors | `neo-auto/efficient` or a mid-tier model |
| Complex debugging, tracing unexpected behavior | `neo-auto/frontier` or a strong reasoning model; Debug mode |
| Architecture planning, design decisions | `neo-auto/frontier`; Architect mode |
| Repository-wide analysis or search | A model with a large context window (256K+); Architect mode |
| Code review and summarization | `neo-auto/efficient` or a cost-effective model |
| Automated background tasks (CI, scripting) | `neo-auto/efficient` or `neo-auto/free` |

### Manually selecting a model

Use the **model selector dropdown** in the Neo Code chat interface to switch models for the current session. In the CLI, pass the `--model` flag to `neo run` or use the model picker in the TUI (`Ctrl+X m` or `/models`).
...
- **VS Code:** Settings → Models → Model per Mode, or edit `neo.jsonc` directly.
- **CLI:** Set `agent.<name>.model` in `neo.jsonc`.

```jsonc
{
  "agent": {
    "code": {
      "model": "neo-auto/efficient"
    },
    "architect": {
      "model": "neo-auto/frontier"
    }
  }
}
```
...
**Where to access:** [app.neo.khulnasoft.com](https://app.neo.khulnasoft.com) → Usage tab. 
...
- Use `neo-auto/efficient` as the default model
- Switch to `neo-auto/free` for low-stakes questions and exploration
...
- Assign `neo-auto/efficient` to Code and Debug agents for everyday work
- Assign `neo-auto/frontier` to Architect (or Plan) agent for planning tasks
- Set `neo-auto/efficient` as the compaction model for all agents
- If on an Enterprise plan, use Providers & Models to block high-cost models that are not needed for your team's typical work

```jsonc
{
  "agent": {
    "code": { "model": "neo-auto/efficient" },
    "debug": { "model": "neo-auto/efficient" },
    "architect": { "model": "neo-auto/frontier" },
...
The snippet below is a ready-to-copy `neo.jsonc` that turns on every available cost-control knob at its most restrictive setting. Drop it into your project root (or your global `~/.config/neo/neo.jsonc`) and adjust individual values upward as you get comfortable with how each one behaves.

{% callout type="tip" %}
This configuration uses only `neo-auto/efficient`.
{% /callout %}
...
```jsonc
{
  ...
  // ── Model selection ──────────────────────────────────────────────────────
  // Route all requests through the two lowest-cost Neo Auto tiers.
  // neo-auto/efficient: lowest-cost paid tier (classifies each request by
  //   difficulty and routes to the cheapest benchmark-proven model).
  "model": "neo-auto/efficient",
  "subagent_model": "neo-auto/efficient", // default model for Task-tool subagents
  ...
  "agent": {
    ...
    "compaction": { "model": "neo-auto/free" },
    "title":      { "model": "neo-auto/free" },
    "summary":    { "model": "neo-auto/free" }
  },
  ...
  // "ask" means Neo pauses and requires your approval before executing.
...
- **Check your usage dashboard** at [app.neo.khulnasoft.com/usage](https://app.neo.khulnasoft.com/usage) for a breakdown by day, model, and project
...
For further reading: [4 Levers to Take Control of Your AI Spend](https://blog.neo.khulnasoft.com/p/4-spend-levers)
