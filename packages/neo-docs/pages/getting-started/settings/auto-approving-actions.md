...
description: "Configure automatic approval settings for Neo Code operations"
---

# Auto-Approving Actions

{% callout type="danger" %}
**Security Warning:** Auto-approve settings bypass confirmation prompts, giving Neo Code direct access to your system. This can result in data loss, file corruption, or worse. Command line access is particularly dangerous, as it can potentially execute harmful operations that could damage your system or compromise security. Only enable auto-approval for actions you fully trust.
{% /callout %}

Auto-approve settings speed up your workflow by eliminating repetitive confirmation prompts, but they significantly increase security risks. The VS Code extension and CLI share the same permission model; choose the tab that matches how you configure Neo Code.
...
The UI reads and writes to the same `neo.jsonc` config files used by the CLI, so changes made in either place are reflected in both.
...
| `"ask"` | Neo pauses and asks for approval before running the tool |
...
When a tool is set to `"ask"`, Neo pauses and displays a permission prompt with two options:
...
For full details and examples, see [MCP Tool Permissions](/docs/automate/mcp/using-in-neo-code#auto-approve-tools).
...
The CLI uses a granular, per-tool permission system configured in `neo.jsonc`. Instead of broad categories like "read" or "write," each tool has its own permission level with glob-pattern rules for fine-grained control.
...
Permissions are configured under the `permission` key in `neo.jsonc`. The following tool-specific permission levels are available:
...
If you define agents in Markdown files, the `permission` frontmatter uses the same `allow` / `ask` / `deny` values and glob patterns as `neo.jsonc`:
...
When a tool is set to `"ask"`, Neo pauses and displays a permission prompt. You have three options:
