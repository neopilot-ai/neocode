...
The sandbox adds an operating-system boundary around agent tools. It limits where tools can write and, by default, blocks outbound network access from model-originated commands. This boundary applies even when a tool passes Neo's permission checks.
...
Sandboxing is not available on Windows. If an enabled macOS or Linux sandbox policy cannot be enforced, Neo reports the reason and refuses to run the affected tool. It does not silently fall back to unrestricted execution.
{% /callout %}
...
1. Open Neo Code Settings using the gear icon ({% codicon name="gear" /%}).
2. Select **Sandboxing**.
3. Turn on **Sandbox**.
...
You can also configure the default in the global `neo.jsonc` file:
...
|---|---|---|
| Permissions | Whether Neo allows, asks about, or denies a matching tool invocation | Prompting for sensitive file reads, blocking specific commands or tools, reviewing consequential actions, and limiting MCP tool or subagent invocation |
...
- Neo's runtime directories listed below
- Paths listed in `sandbox.writable_paths`

| Writable Neo path | Purpose |
|---|---|
| `$XDG_DATA_HOME/neo` (normally `~/.local/share/neo`) | Session data, logs, and Neo's managed repository cache under `repos/` |
| `$XDG_CACHE_HOME/neo` (normally `~/.cache/neo`) | Cached data and downloaded binaries |
| `$XDG_STATE_HOME/neo` (normally `~/.local/state/neo`) | Runtime state |
| `$TMPDIR/neo` | Temporary files; on macOS this is commonly under `/var/folders/.../T/neo` |
...
- Neo's stored sandbox policy and preference files are read-only.
- Neo's global config root is read-only so a sandboxed command cannot widen network or write authority for a later session.
...
Shell commands and their child processes inherit the same restrictions. Neo's file tools perform mutations through a sandboxed worker. Writable file handles are unavailable, so a tool that requires an open read-write handle may fail even for an allowed path.

Direct filesystem access inside trusted integrations is confined only when the integration uses Neo's sandbox-aware filesystem service. Interactive terminals, notebook execution, and starting or restarting a background process are unavailable while sandboxing is active because those processes do not yet run inside the same boundary.
...
When network restriction is on, Neo blocks:
...
- Local and remote MCP tool calls and typed resource reads, plus custom or plugin tools that Neo cannot prove will remain offline
...
On every platform, Neo reports an error and refuses to run the affected tool if the configured confinement or destination proxy cannot be established. It does not fall back to unrestricted execution.
