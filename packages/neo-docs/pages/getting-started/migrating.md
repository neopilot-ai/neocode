...
description: "Guide for migrating to Neo Code from other AI coding tools"
---

# Migrating from Cursor or Windsurf

Quickly migrate your custom rules from Cursor or Windsurf to Neo Code. The process typically takes just a few minutes per project.

{% callout type="info" title="Two Workflow Approaches"%}

Neo Code supports **two complementary workflows**—choose the one that fits your style, or use both:
...
## Why Neo Code's Rules System?

Neo Code simplifies AI configuration while adding powerful new capabilities:
...
### What's Different in Neo Code

| Cursor | Neo Code | Key Difference |
|---|---|---|
| `.cursor/rules/*.mdc` with YAML frontmatter | `.neocode/rules/*.md` plain Markdown | No YAML metadata required |
...
| Global rules in UI settings | `~/.neocode/rules/*.md` files | Global rules stored as files in home folder |
...
**2. Create Neo Code directory:**

```bash
mkdir -p .neocode/rules
```
...
for file in .cursor/rules/*.mdc; do
  basename="${file##*/}"
  cp "$file" ".neocode/rules/${basename%.mdc}.md"
done
...
- Save to `~/.neocode/rules/cursor-global.md`

**6. Handle legacy `.cursorrules`:**

```bash
cp .cursorrules .neocode/rules/legacy-rules.md
```

### Converting Cursor's `globs` Patterns

Cursor's `globs` field specifies which files a rule applies to. Neo Code handles this through **mode-specific directories** instead.
...
**Neo Code approach (Option 1 - Mode-specific directory):**

```bash
mkdir -p .neocode/rules-code
# Save TypeScript-specific rules here
```

**Neo Code approach (Option 2 - Custom mode):**

```yaml
# .neocodemodes (at project root)
- slug: typescript
  name: TypeScript
...
Then place rules in `.neocode/rules-typescript/`

### Flattening Nested Cursor Rules

Cursor supports nested `.cursor/rules/` directories. Neo Code uses flat structure with descriptive names:

```bash
# Cursor: .cursor/rules/backend/server/api-rules.mdc
# Neo Code: .neocode/rules/backend-server-api-rules.md
```
...
| Windsurf | Neo Code | Key Difference |
|---|---|---|
| `.windsurf/rules/*.md` | `.neocode/rules/*.md` | Same Markdown format |
| GUI configuration for activation modes | File location determines scope | Scope controlled by directory structure |
| "Always On" mode (GUI) | Place in `.neocode/rules/` | Rules stored as files, not GUI settings |
...
| Global rules in `~/.codeium/windsurf/memories/global_rules.md` | `~/.neocode/rules/*.md` | Global rules in home folder, multiple files |
...
```bash
cp .windsurf/rules/*.md .neocode/rules/
```

**4. Migrate global rules:**

```bash
cp ~/.codeium/windsurf/memories/global_rules.md ~/.neocode/rules/global-rules.md
```

**5. Handle legacy `.windsurfrules`:**

```bash
cp .windsurfrules .neocode/rules/legacy-rules.md
```
...
# .neocode/rules/api-conventions.md
# .neocode/rules/testing-standards.md
# .neocode/rules/code-style.md
```

### Converting Windsurf's Activation Modes

Windsurf configures activation through the GUI. In Neo Code, file organization replaces GUI configuration:

| Windsurf GUI Mode | Neo Code Equivalent |
|---|---|
| **Always On** | Place in `.neocode/rules/` (default) |
...
Then place the rule in `.neocode/rules-test/`

## AGENTS.md Support

All three tools support the `AGENTS.md` standard. If you have one, it works in Neo Code automatically:

```bash
# Verify it exists
ls -la AGENTS.md

# That's it - Neo Code loads it automatically (enabled by default)
```

**Important:** Use uppercase `AGENTS.md` (not `agents.md`). Neo Code also accepts `AGENT.md` (singular) as a fallback.

**Note:** Both `AGENTS.md` and `AGENT.md` are write-protected files in Neo Code and require user approval to modify.

## Understanding Mode-Specific Rules

This is Neo Code's unique feature that replaces both Cursor's `globs` and Windsurf's activation modes.

### Directory Structure

```bash
.neocode/rules/              # Apply to ALL modes
.neocode/rules-code/         # Only in Code mode
.neocode/rules-debug/        # Only in Debug mode
.neocode/rules-ask/          # Only in Ask mode
.neocode/rules-{custom}/     # Only in your custom mode
```
...
mkdir -p .neocode/rules-test

# 2. Save rule as plain Markdown
cat > .neocode/rules-test/testing-standards.md << 'EOF'
# Testing Rules
- Write tests for all features
- Maintain >80% coverage
EOF
...
- [ ] **Verify rules loaded:** Click law icon (⚖️) in Neo Code panel
- [ ] **Test rule application:** Ask Neo Code to perform tasks following your rules
- [ ] **Organize rules:** Split large files, use clear names
- [ ] **Set up mode-specific rules:** Create directories for specialized workflows
- [ ] **Update team docs:** Document new `.neocode/rules/` location
- [ ] **Commit to version control:** `git add .neocode/`
...
```bash
ls -la .neocode/rules/      # Project rules
ls -la ~/.neocode/rules/    # Global rules
```
...
- **For always-on rules:** Place in `.neocode/rules/`
...
- **After migrating:** Organize files accordingly in Neo Code

### Nested Rules Flattened

Cursor's nested directories don't map to Neo Code. Flatten with descriptive names:

```bash
# Bad: .cursor/rules/backend/api/rules.mdc
# Good: .neocode/rules/backend-api-rules.md
```
...
- **Check setting:** Verify "Use Agent Rules" is enabled in Neo Code settings (enabled by default)
- **Reload:** Restart VS Code if needed

### Choosing Your Workflow

Neo Code supports **both autocomplete and chat-driven workflows**. Choose the approach that fits your coding style, or combine them:
...
```bash
mkdir -p .neocode/rules-review
mkdir -p .neocode/rules-docs
```

**Note:** `.neocodemodes` can be in YAML (preferred) or JSON format. For global modes, edit the `custom_modes.yaml` file via Settings > Edit Global Modes.
...
- [Join our Discord](https://neo.khulnasoft.com/discord) for migration support
