---
title: "Using Neo for Free"
description: "How to use Neo Code for free — Auto Free, finding free models, free autocomplete, and free background tasks"
---

# Using Neo for Free

Neo Code can be used completely free of charge. There are three places where Neo uses AI model inference, and each can be configured to use free models.

## Where Neo Uses Models
...
Each of these consumes credits by default. **To use Neo entirely for free, configure all three to use free models.**

## Free Agentic Usage

Neo provides free models for coding tasks through the Neo Gateway and partner providers.

### Auto Free

The easiest way to get started is [**Auto Free**](/docs/code-with-ai/agents/auto-model) (`neo-auto/free`). This is a Neo-provided model tier that automatically routes your requests to the best available free models — no configuration needed.
...
Neo's cloud services — App Builder, Code Reviewer, and others — also support free models. Select any model labeled "(free)" in the model dropdown when configuring a cloud task.

{% callout type="tip" %}
Available free models change over time as Neo partners with different inference providers. Subscribe to our blog or join our [Discord](https://neo.khulnasoft.com/discord) for updates.
{% /callout %}

## Free Autocomplete

Neo's autocomplete feature provides AI-powered code completions as you type in IDE extensions.

By default, autocomplete routes through the Neo provider and uses credits. If you run out of credits without a free alternative configured, autocomplete stops working — but your main coding workflow is unaffected.

### How to Get It Free

Add your own Mistral AI API key via **BYOK (Bring Your Own Key)** on the Neo Gateway. Mistral offers a free tier for Codestral. When you configure a BYOK key, autocomplete requests use your key directly — at no cost on your Neo balance.
...
Neo uses a small model in the background for tasks like session titling. By default this is Auto Small, which consumes credits. If the small model is unavailable, Neo falls back to your primary model — which may also consume credits if it's a paid model.
...
**In the CLI:** Set the `small_model` parameter in `~/.config/neo/config.json`:
