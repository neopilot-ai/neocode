...
description: "Learn how to authenticate with the Neo AI Gateway using API keys, session tokens, and Bring Your Own Key (BYOK)."
---

# Authentication

The Neo AI Gateway supports multiple authentication methods depending on your use case.
...
API keys are JWT tokens tied to your Neo account. See [how to get your API key](/docs/getting-started/setup-authentication#neo-gateway-api-key) for step-by-step instructions.
...
```
X-neocode-OrganizationId: your_org_id
```
...
BYOK lets you use your own provider API keys with the Neo AI Gateway. When a BYOK key is configured, requests are sent to the provider using your key. You are billed directly by the provider -- Neo does not add any markup.
...
1. Add your provider API key in the [Neo dashboard](https://app.neo.khulnasoft.com) or through your Neo Code extension settings
...
4. Usage is tracked but not billed to your Neo balance (cost is set to $0)
5. If your BYOK key fails, the request will not automatically fall back to Neo's keys
...
| `X-neocode-OrganizationId` | No | Organization context for org-scoped requests |
| `X-neocode-TaskId` | No | Task identifier for prompt cache keying |
| `X-neocode-Version` | No | Client version string |
| `x-neocode-mode` | No | Mode hint for `neo-auto` model routing |
