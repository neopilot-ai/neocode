...
description: "Learn about the AI models available through the Neo AI Gateway, including model IDs and how to use them."
---

# Models & Providers

The Neo AI Gateway provides access to hundreds of AI models through a single unified API. You can switch between models by changing the model ID string -- no code changes required.
...
const result = streamText({
  model: neo.chat("anthropic/claude-sonnet-4.6"),
  prompt: "Hello!",
})
```
...
```
GET https://api.neo.khulnasoft.com/api/gateway/models
```
...
Auto virtual models select an underlying model using tier-specific routing. Frontier uses the `x-neocode-mode` request header. Balanced uses the API interface, Free uses deterministic affinity across available candidates, and Small uses account balance.

{% callout type="info" title="Underlying models can change" %}
The mappings below reflect the current routing. The underlying models behind each `neo-auto/*` tier are updated server-side as better options become available or as providers change pricing and availability — the tier IDs themselves remain stable.
{% /callout %}
...
curl -X POST "https://api.neo.khulnasoft.com/api/gateway/chat/completions" \
  ...
  -H "x-neocode-mode: plan" \
  -H "Content-Type: application/json" \
  -d '{"model": "neo-auto/balanced", "messages": [{"role": "user", "content": "Design a database schema"}]}'
```
