// neocode_change start - Tests for NEO_CONFIG_CONTENT merging
import { describe, test, expect, beforeEach, afterEach } from "bun:test"
import { buildConfigEnv } from "../src/server"

describe("buildConfigEnv", () => {
  const originalEnv = process.env.NEO_CONFIG_CONTENT

  beforeEach(() => {
    delete process.env.NEO_CONFIG_CONTENT
  })
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.NEO_CONFIG_CONTENT = originalEnv
    }
  })
  ...
  test("preserves existing NEO_CONFIG_CONTENT when spawning with new config", () => {
    // Simulate Neocode having injected modes via NEO_CONFIG_CONTENT
    process.env.NEO_CONFIG_CONTENT = JSON.stringify({
      agent: {
        translate: { mode: "primary", prompt: "You are a translator" },
      },
      instructions: [".neo/rules/main.md"],
    })
    ...
    expect(parsed.instructions).toContain(".neo/rules/main.md")
  ...
  test("handles invalid JSON in existing NEO_CONFIG_CONTENT gracefully", () => {
    process.env.NEO_CONFIG_CONTENT = "invalid json {"
