...
function rewrite(request: Request, directory?: string) {
  ...
  const value = pick(request.headers.get("x-neo-directory"), directory)
  ...
  const next = new Request(url.href, request) // neocode_change
  next.headers.delete("x-neo-directory")
  return next
}

export function createNeoClient(config?: Config & { directory?: string }) {
  if (!config?.fetch) {
