...
// neocode_change start - Merge existing NEO_CONFIG_CONTENT with new config
// This preserves Neocode-injected modes when spawning nested CLI instances
...
function parseExistingConfig(): Config | undefined {
  const content = process.env.NEO_CONFIG_CONTENT
...
export async function createNeoServer(options?: ServerOptions) {
  ...
  const proc = launch(`neo`, args, {
    // neocode_change end
    env: {
      ...process.env,
      NEO_CONFIG_CONTENT: buildConfigEnv(options.config), // neocode_change
    },
  })
  let clear = () => {}
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    proc.stdout?.on("data", (chunk) => {
      ...
      for (const line of lines) {
        // neocode_change start
        if (line.startsWith("neo server listening")) {
          // neocode_change end
...
export function createNeoTui(options?: TuiOptions) {
  const args = []
