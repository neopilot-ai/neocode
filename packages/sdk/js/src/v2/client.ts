...
import { NeoClient } from "./gen/sdk.gen.js"
import { wrapClientError } from "../error-interceptor.js"
export { type Config as NeoClientConfig, NeoClient }
...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-neo-directory", "directory"],
    ["x-neo-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-neo-directory")
  next.headers.delete("x-neo-workspace")
  return next
}

export function createNeoClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  if (!config?.fetch) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-neo-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-neo-workspace": config.experimental_workspaceID,
    }
  }
