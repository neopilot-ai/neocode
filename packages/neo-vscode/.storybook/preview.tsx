...
// Reference neo-ui stories helpers directly — not exported via package.json
import { applyNeoTheme, applyVscodeTheme, clearVscodeTheme } from "../../neo-ui/src/stories/theme-decorator"
import "../../neo-ui/.storybook/fonts.css"
import "@neocode/neo-ui/styles"
import "../webview-ui/src/styles/chat.css"

// Make the Neo logo available in Storybook (normally injected by the extension host)
;(window as { ICONS_BASE_URI?: string }).ICONS_BASE_URI = "/icons"

const themeDecorator: DecoratorFunction<SolidRenderer> = (Story, context) => {
  const themeId = (context.globals["theme"] as string) ?? "neo-vscode"
  const vscodeThemeId = (context.globals["vscodeTheme"] as string) ?? "dark-modern"

  const colorScheme = (() => {
    if (themeId === "neo-vscode") return applyVscodeTheme(vscodeThemeId)
    clearVscodeTheme()
    return (context.globals["colorScheme"] as "light" | "dark") ?? "dark"
  })()

  applyNeoTheme(themeId, colorScheme)
  ...
  globalTypes: {
    theme: {
      ...
      toolbar: {
        ...
        icon: "paintbrush",
        items: [
          { value: "neo-vscode", title: "Neo VSCode" },
          { value: "neo", title: "Neo" },
        ],
        dynamicTitle: true,
      },
