# Feature Parity Plan — Neo Code VS Code Extension (Rebuild)

## Overview

This extension is a **ground-up rebuild** of the [old Neo Code extension](https://github.com/Neopilot-Ai/neocode-legacy) using Neo CLI as the backend. Rather than migrating the old extension's codebase, we started fresh with a Solid.js webview, a CLI server manager, and a message-based protocol between extension host and webview. This new extension lives in the [neocode monorepo](https://github.com/Neopilot-Ai/neocode/tree/main/packages/neo-vscode).

This document tracks remaining work needed for feature parity with the old extension. Each feature links to its detailed parity requirement doc. Features sourced from the [GitHub project board](https://github.com/orgs/Neopilot-Ai/projects/25/views/1) include issue links.
...
| [Cloud Task Support](non-agent-features/cloud-task-support.md) | Upload local sessions to cloud, real-time sync, conflict resolution | Neo cloud API + CLI; extension provides UI | P2 |
...
Open issues from the [GitHub project board](https://github.com/orgs/Neopilot-Ai/projects/25/views/1) not covered by the feature docs above. Each item has its own detailed doc.
...
| [Dedicated Output Channel](infrastructure/dedicated-output-channel.md) | General "Neo Code" output channel and centralized logging utility | P2 |
...
- [ ] **Review and tighten CSP** — The current policy in [`NeoProvider._getHtmlForWebview()`](../src/NeoProvider.ts:829) has several areas to audit:
  - `style-src 'unsafe-inline'` is broadly permissive — investigate whether nonce-based style loading is feasible now that neo-ui styles are bundled
...
- [ ] **Validate `openExternal` URLs** — The [`openExternal` handler](../src/NeoProvider.ts:186) passes any URL from the webview directly to `vscode.env.openExternal()` with no allowlist or scheme check. Restrict to `https:` (and possibly `vscode:`) schemes, or allowlist specific hosts
...
- [ ] **Bundle size audit** — With neo-ui and its transitive dependencies (shiki, marked, katex, dompurify, etc.) now bundled, measure `dist/webview.js` size and verify the total `.vsix` package size is acceptable
...
- [ ] **Dedicated output channel** — All logging currently goes to `console.log` mixed with other extensions ([details](infrastructure/dedicated-output-channel.md)). Create a dedicated "Neo Code" output channel before production
- [ ] **Remove or guard verbose logging** — Many `console.log` calls with emojis and debug detail exist in [`NeoProvider.ts`](../src/NeoProvider.ts). Gate behind a debug flag or move to the output channel at appropriate log levels
...
### neo-ui Shared Library

- **neo-ui shared library**: The webview now heavily uses `@neocode/neo-ui` for UI components. A `DataBridge` component in App.tsx adapts the session store to neo-ui's `DataProvider` expected shape, enabling shared components like `<NeoMessage>` to work with the extension's data model.
...
- Permissions flow through CLI's ask/reply model, not extension-side approval queues. Permissions are rendered through neo-ui's DataProvider pattern, not a standalone PermissionDialog.
