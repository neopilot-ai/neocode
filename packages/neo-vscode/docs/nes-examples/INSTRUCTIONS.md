...
1. **`bun run watch`** is already running for the neocode extension.
2. In the neocode VSCode window, press **F5** → opens the **Extension Development Host**.
3. In the Dev Host: `File → Open Folder…` → `packages/neo-vscode/docs/nes-examples/` inside this repo.
4. Open Settings (`Cmd+,`), confirm:
   - `neo-code.new.autocomplete.enableAutoTrigger` → ✓ (default true)
   - `neo-code.new.autocomplete.model` → **Mercury Next Edit (Inception)** ← *NOT* "Mercury Edit 2", which is the classic FIM option
   - `neo-code.new.autocomplete.nextEdit.apiKey` → your `sk_...` key
   - VSCode global `editor.inlineSuggest.enabled` → ✓
5. To watch the pipeline live: `View → Output` → pick the **"Neo Code · Next Edit"** channel.
...
- **No log lines appearing**: confirm the output channel is "Neo Code · Next Edit". Also confirm you reloaded the Dev Host after rebuilding.
