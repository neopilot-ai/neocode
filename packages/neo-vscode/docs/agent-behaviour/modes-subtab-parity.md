...
| Custom Instructions (per-mode) | `<TextArea>` (10 rows) + link to `.neocode/rules-{slug}/rules.md` | Not available as separate field (merged into system prompt) |
...
| Override System Prompt | Advanced disclosure with link to `.neocode/system-prompt-{slug}` | Not available |
...
| Link to global rules file | Link to `.neocode/rules/rules.md` | Not available |

### Organization Features

| Feature | Legacy | New |
|---|---|---|
| Share modes banner | `NeoShareModesBanner` component | Not available |
