...
import type {
  ...
  FilePartInput,
  Config,
} from "@neocode/sdk/v2/client"
import { MaxCostNudge, type MaxCostChoice } from "@opencode-ai/core/neocode/cost/max-cost-nudge"
import { type NeoConnectionService, ServerStartupError } from "./services/cli-backend"
...
import { saveImage } from "./neo-provider/save-image"
import { handleEditorAction } from "./neo-provider/editor-actions"
import { exportTranscript } from "./neo-provider/export-transcript"
import {
  TelemetryProxy,
...
import { createMarketplaceRemover, removeMcp } from "./neo-provider/remove-config-item"
import { AgentRequirementsController } from "./neo-provider/agent-requirements-controller"
...
import { slimInfo, slimPart, slimParts } from "./neo-provider/slim-metadata"
import { handleSidebarWorktreeMessage } from "./neo-provider/sidebar-worktree"
import { parseMessageFiles, type MessageFile } from "./neo-provider/message-files"
import { renameSession } from "./neo-provider/rename-session"
import { handleFileSearch } from "./neo-provider/file-search"
import { handleFilePicker } from "./neo-provider/file-picker"
import { watchFontSizeConfig } from "./neo-provider/font-size"
import { getTerminalContents } from "./services/terminal/context"
import { disposeGitChangesTarget } from "./neo-provider/git-changes-target"
import { interceptMessage } from "./neo-provider/git-changes-request"
import { matchFollowup, recordFollowup, type Followup } from "./neo-provider/followup-session"
import { clearCommandsCache, loadCommands } from "./neo-provider/commands"
import { fetchMessagePage, MESSAGE_PAGE_LIMIT } from "./neo-provider/message-page"
import {
  dismissNotification,
...
} from "./neo-provider/notifications"
import { childID } from "./neo-provider/task-session"
import { VisibleTaskStreams } from "./neo-provider/visible-task-streams"
import { handleNetworkEvent, clearNetworkWaits } from "./neo-provider/network"
import { SessionAbort } from "./neo-provider/abort"
import {
  buildAutocompleteSettingsMessage,
  validAutocompleteSetting,
  watchAutocompleteConfig,
} from "./services/autocomplete/settings"
import { routeEarlyMessage } from "./neo-provider/early-message"
import * as ModelState from "./neo-provider/model-state"
import { handleForkSession } from "./neo-provider/fork-session"
import { openConfig } from "./neo-provider/open-config"
import {
  getWorkStylePayload,
  handleWorkStyleMessage,
  isWorkStyleSetting,
  watchWorkStyleConfig,
} from "./neo-provider/work-style"
import * as McpOAuth from "./neo-provider/mcp-oauth"
import { retryable, backoff, MAX_RETRIES } from "./util/retry"
import { hasGit } from "./neo-provider/git-status"
...
} from "./neo-provider/handlers/migration"
...
import {
  ...
  type AuthContext,
} from "./neo-provider/handlers/auth"
import {
  handleRequestCloudSessions,
  handleRequestCloudSessionData,
  handleImportAndSend,
  type CloudSessionContext,
} from "./neo-provider/handlers/cloud-session"
import {
  handlePermissionResponse,
  fetchAndSendPendingPermissions,
  type PermissionContext,
} from "./neo-provider/handlers/permission-handler"
import {
  handleQuestionReply,
  handleQuestionReject,
  fetchAndSendPendingQuestions,
} from "./neo-provider/handlers/question"
import { fetchAndSendPendingSuggestions } from "./neo-provider/handlers/suggestion"
import { nativeTitle } from "./neo-provider/native-tab-title"
import { parseReview, reviewMetadata, type ReviewMessageData } from "./shared/review-comments"
import { completesWithoutStatus } from "./neo-provider/command-completion"
import { NeoProviderMemory } from "./neo-provider/memory"
...
import type { Agent } from "@neocode/sdk/v2/client"
import { configFeatures } from "./features"
import { createAutoApproveBridge } from "./neo-provider/auto-approve"
import type { NeoProviderOptions } from "./neo-provider/options"
import { fetchNeoEmbeddingModelCatalog } from "@neocode/neo-gateway"
import { fetchImageModels } from "./image-generation/models"
import { stopSessionProcesses } from "./neo-provider/background-process"
import { sandboxDefault, sandboxSessionMetadata } from "./shared/sandbox-session"
import {
  buildIndexingSettingsMessage,
  validIndexingSetting,
  watchIndexingConfig,
} from "./neo-provider/indexing-settings"
...
function sandboxClient(client: NeoClient | null) {
...
export class NeoProvider implements vscode.WebviewViewProvider, TelemetryPropertiesProvider {
  public static readonly viewType = "neo-code.SidebarProvider"
  ...
  private readonly extensionVersion =
    vscode.extensions.getExtension("neocode.neo-code")?.packageJSON?.version ?? "unknown"
  ...
  /** Cached neoEmbeddingModelsLoaded payload so requestNeoEmbeddingModels is resilient offline. */
  private cachedNeoEmbeddingModelsMessage: unknown = null
  ...
  private pendingNeoModel: { modelID?: string; agent?: string } | null = null
  ...
  private readonly memory = new NeoProviderMemory({
  ...
  constructor(
    ...
    private readonly connectionService: NeoConnectionService,
    private readonly extensionContext?: vscode.ExtensionContext,
    private readonly opts: NeoProviderOptions = {},
  ) {
  ...
  private checkpoint(sid: string, run: () => Promise<void>): void {
    ...
    void pending.then(cleanup, (error) => {
      console.error("[Neo New] checkpoint mutation failed:", error)
      cleanup()
    })
  }
  ...
  /**
   * Convenience getter that returns the shared SDK NeoClient or null if not yet connected.
   * Preserves the existing null-check pattern used throughout handler methods.
   */
  private get client(): NeoClient | null {
    try {
  ...
  private openMarketplacePanel(directory: unknown): void {
    if (typeof directory === "string" && directory) {
      vscode.commands.executeCommand("neo-code.new.marketplaceButtonClicked", directory)
      return
    }
    vscode.commands.executeCommand("neo-code.new.marketplaceButtonClicked", this.projectDirectory)
  }

  // Strip metadata unused by the webview to keep session switches fast.
  // Logic in neo-provider/slim-metadata.ts.
  ...
  private async syncWebviewState(reason: string): Promise<void> {
    ...
    console.log("[Neo New] NeoProvider: 🔄 syncWebviewState()", {
      reason,
    ...
    if (!this.isWebviewReady) {
      console.log("[Neo New] NeoProvider: ⏭️ syncWebviewState skipped (webview not ready)")
      return
    }
    ...
    if (serverInfo) {
      const langConfig = vscode.workspace.getConfiguration("neo-code.new")
      this.postMessage({
        type: "ready",
        serverInfo,
    ...
    // Profile returns 401 when user isn't logged into Neo Gateway — that's expected.
    ...
    if (this.connectionState === "connected" && this.client) {
      console.log("[Neo New] NeoProvider: 👤 syncWebviewState fetching profile...")
      const profileResult = await retry(() => this.client!.neo.profile())
      const profileData = profileResult.data ?? null
      console.log("[Neo New] NeoProvider: 👤 syncWebviewState profile:", profileData ? "received" : "null")
      this.postMessage({
        type: "profileData",
        data: profileData,
      })
  ...
  private setSidebarVisible(visible: boolean): void {
    ...
    vscode.commands.executeCommand("setContext", "neo-code.new.sidebarVisible", visible)
  }

  /** Resolve a WebviewPanel for displaying Neo in an editor tab. */
  ...
  public async getSessionInfo(sessionId: string): Promise<Session | undefined> {
    ...
    return retry(() => client.session.get({ sessionID: sessionId, directory }, { throwOnError: true }))
      ...
      .catch((error: unknown) => {
        console.warn("[Neo New] NeoProvider: Failed to resolve managed session:", error)
        return undefined
      })
  }
  ...
  public selectNeoModel(modelID?: string, agent?: string): void {
    if (!modelID && !agent) return
    this.pendingNeoModel = { ...(modelID && { modelID }), ...(agent && { agent }) }
    this.flushPendingNeoModel()
  }
    ...
    this.webviewMessageDisposable = webview.onDidReceiveMessage(async (message) => {
      ...
      if (
        await handleSidebarWorktreeMessage(message, {
          post: (msg) => this.postMessage(msg),
          openAgentManager: () => vscode.commands.executeCommand("neo-code.new.agentManagerOpen"),
          openAdvancedWorktree: () => vscode.commands.executeCommand("neo-code.new.agentManager.advancedWorktree"),
          openChanges: (sessionId?: string, turnId?: string) =>
            vscode.commands.executeCommand("neo-code.new.showChanges", { sessionId, turnId }),
      ...
      switch (message.type) {
        case "webviewReady":
          console.log("[Neo New] NeoProvider: ✅ webviewReady received")
        ...
        case "syncSession":
          this.handleSyncSession(message.sessionID, message.parentSessionID).catch((e) =>
            console.error("[Neo New] handleSyncSession failed:", e),
          )
          break
        case "loadSessions":
          this.handleLoadSessions().catch((e) => console.error("[Neo New] handleLoadSessions failed:", e))
          break
        ...
        case "openSettingsPanel":
          vscode.commands.executeCommand("neo-code.new.settingsButtonClicked", message.tab)
          break
        case "openNeoClaw":
          vscode.commands.executeCommand("neo-code.new.neoClawOpen")
          break
        ...
        case "forkSession":
          handleForkSession(this.forkCtx, message.sessionId, message.messageId).catch((e) =>
            console.error("[Neo New] handleForkSession failed:", e),
          )
          break
        case "retryConnection":
          console.log("[Neo New] NeoProvider: 🔄 Retrying connection...")
          this.initializeConnection().catch((e) =>
            console.error("[Neo New] NeoProvider: ❌ Retry connection failed:", e),
          )
          break
        case "reload":
          this.handleReload().catch((e) => console.error("[Neo New] NeoProvider: Reload failed:", e))
          break
        case "openSubAgentViewer":
          vscode.commands.executeCommand("neo-code.new.openSubAgentViewer", message.sessionID, message.title)
          break
        case "saveImage":
          return saveImage(this.getWorkspaceDirectory(this.currentSession?.id), message)
        case "requestProviders":
          this.fetchAndSendProviders().catch((e) => console.error("[Neo New] fetchAndSendProviders failed:", e))
          break
        ...
        case "fetchCustomProviderModels":
          this.handleFetchCustomProviderModels(message).catch((e) =>
            console.error("[Neo New] fetchCustomProviderModels failed:", e),
          )
          break
        case "compact":
          ...
          break
        case "requestAgents":
          this.fetchAndSendAgents().catch((e) => console.error("[Neo New] fetchAndSendAgents failed:", e))
          break
        case "requestSkills":
          this.fetchAndSendSkills().catch((e) => console.error("[Neo New] fetchAndSendSkills failed:", e))
          break
        case "requestAgentRequirements":
          this.requirements
            ...
            .catch((e) => console.error("[Neo New] fetchAndSendAgentRequirements failed:", e))
          break
        case "requestCommands":
          this.fetchAndSendCommands().catch((e) => console.error("[Neo New] fetchAndSendCommands failed:", e))
          break
        case "removeSkill":
          this.removeSkillViaCli(message.location).catch((e: unknown) =>
            console.error("[Neo New] removeSkill failed:", e),
          )
          break
        case "removeAgent":
          this.handleRemoveAgent(message.name).catch((e) => console.error("[Neo New] handleRemoveAgent failed:", e))
          break
        case "removeMcp":
          this.handleRemoveMcp(message.name).catch((e) => console.error("[Neo New] handleRemoveMcp failed:", e))
          break
        case "requestMcpStatus":
          this.fetchAndSendMcpStatus().catch((e) => console.error("[Neo New] fetchAndSendMcpStatus failed:", e))
          break
        case "connectMcp": {
          ...
          if (c1) {
            ...
            ).catch((e) => console.error("[Neo New] connectMcpServer failed:", e))
          }
          break
        }
        case "disconnectMcp": {
          ...
          if (c2) {
            void McpOAuth.disconnectMcpServer(c2, message.name, this.getWorkspaceDirectory(), () =>
              this.refreshMcpStatus(),
            ).catch((e) => console.error("[Neo New] disconnectMcpServer failed:", e))
          }
          break
        }
        case "authenticateMcp": {
          ...
          if (c) {
            void McpOAuth.authenticateMcpServer(c, message.name, this.getWorkspaceDirectory(), () =>
              this.refreshMcpStatus(),
            ).catch((e) => console.error("[Neo New] authenticateMcpServer failed:", e))
          }
          break
        }
        ...
        case "toggleSandbox":
          ...
          break
        case "requestConfig":
          this.fetchAndSendConfig().catch((e) => console.error("[Neo New] fetchAndSendConfig failed:", e))
          break
        case "requestGlobalConfig":
          this.fetchAndSendGlobalConfig().catch((e) => console.error("[Neo New] fetchAndSendGlobalConfig failed:", e))
          break
        case "requestIndexingStatus":
          this.fetchAndSendIndexingStatus().catch((e) =>
            console.error("[Neo New] fetchAndSendIndexingStatus failed:", e),
          )
          break
        case "requestIndexingSettings":
          this.postMessage(buildIndexingSettingsMessage())
          break
        case "requestNeoEmbeddingModels":
          this.fetchAndSendNeoEmbeddingModels().catch((e) =>
            console.error("[Neo New] fetchAndSendNeoEmbeddingModels failed:", e),
          )
          break
        case "requestImageModels":
          this.fetchAndSendImageModels().catch((e) => console.error("[Neo New] fetchAndSendImageModels failed:", e))
          break
        case "updateConfig":
        ...
        case "openSettingsTab":
          if (message.tab === "indexing") {
            await vscode.commands.executeCommand("neo-code.new.openIndexingSettings")
          }
          break
        case "setLanguage":
          await vscode.workspace
            .getConfiguration("neo-code.new")
        ...
        case "requestRemoteStatus":
          this.remoteService
            ...
            .catch((err) => console.error("[Neo New] remote message failed:", err))
          break
        case "deleteSession":
        ...
        case "enhancePrompt": {
          ...
          void sdkClient.enhancePrompt
            ...
            .catch((err: unknown) => {
              ...
              console.error("[Neo New] NeoProvider: Failed to enhance prompt:", err)
   ...
   * Subscribes to the shared NeoConnectionService.
   */
  ...
  private async doInitializeConnection(): Promise<void> {
    console.log("[Neo New] NeoProvider: 🔧 Starting initializeConnection...")
    ...
    try {
      ...
      this.unsubscribeEvent = this.connectionService.onEventFiltered(
        (payload, directory) => {
          ...
          if (event.type === "neo-sessions.remote-status-changed") return true
          ...
          // NeoProvider instance. The Settings panel is a separate provider with no tracked
      ...
      this.unsubscribeState = this.connectionService.onStateChange(async (state, error) => {
        ...
        if (state === "connected") {
          ...
          try {
            ...
            const sdkClient = this.client
            if (sdkClient) {
              const profileResult = await sdkClient.neo.profile()
            ...
            console.error("[Neo New] NeoProvider: ❌ Failed during connected state handling:", error)
            this.postMessage({
              type: "error",
      ...
      // Subscribe to notification dismiss broadcast from other NeoProvider instances
      this.unsubscribeNotificationDismiss = this.connectionService.onNotificationDismissed(() => {
        this.fetchAndSendNotifications()
      })

      // Subscribe to language change broadcast from other NeoProvider instances
      ...
      // Subscribe to profile change broadcast from other NeoProvider instances
      this.unsubscribeProfileChange = this.connectionService.onProfileChanged((data) => {
        this.postMessage({ type: "profileData", data })
      })

      // Subscribe to favorites change broadcast from other NeoProvider instances
      this.unsubscribeFavoritesChange = this.connectionService.onFavoritesChanged((favorites) => {
        this.postMessage({ type: "favoritesLoaded", favorites })
      })

      // Subscribe to model-selector expand/collapse broadcast from other NeoProvider instances
      this.unsubscribeModelSelectorExpanded = this.connectionService.onModelSelectorExpandedChanged((value) => {
        this.postMessage({ type: "modelSelectorExpandedLoaded", value })
      })

      // legacy-migration start
      // Subscribe to migration-complete broadcast from any NeoProvider instance
      ...
      console.log("[Neo New] NeoProvider: ✅ initializeConnection completed successfully")
    } catch (error) {
      console.error("[Neo New] NeoProvider: ❌ Failed to initialize connection:", error)
      this.connectionState = "error"
      this.postMessage({
        type: "connectionState",
        state: "error",
  ...
  private async handleCreateSession(): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to create session:", error)
      this.postMessage({
        type: "error",
  ...
  private refreshSessionDetails(sessionID: string, dir: string, signal?: AbortSignal): void {
    ...
    this.client.session
      ...
      .catch((e: unknown) => console.warn("[Neo New] NeoProvider: getSession failed (non-critical):", e))
    ...
    this.client.session
      ...
      .catch((e: unknown) => console.error("[Neo New] NeoProvider: Failed to fetch session statuses:", e))
  }
  ...
  private fetchAndSendSessionModelUsage(sessionID: string, requestID: string): Promise<void> {
    ...
    return this.connectionService
      ...
      .then((client) => client.neocode.sessionModelUsage({ sessionID, directory }, { throwOnError: true }))
      .then((response) => {
      ...
      .catch((error: unknown) => {
        console.warn("[Neo New] NeoProvider: Failed to load session model usage:", error)
  ...
  private async handleLoadMessages(
    ...
    try {
      ...
      if (abort?.signal.aborted) return
      console.error("[Neo New] NeoProvider: Failed to load messages:", error)
  ...
  private async handleSyncSession(sessionID: string, parentSessionID?: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to sync child session:", err)
    }
  }
  ...
  private async flushPendingSessionRefresh(reason: string): Promise<void> {
    ...
    console.log("[Neo New] NeoProvider: 🔄 Flushing deferred sessions refresh", { reason })
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to flush session refresh:", error)
    }
  ...
  private async handleLoadSessions(): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to load sessions:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleTerminalContext(requestId: string): Promise<void> {
    try {
      ...
      console.error("[Neo New] Failed to capture terminal context:", error)
      this.postMessage({
  ...
  private async handleDeleteSession(sessionID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to delete session:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleRenameSession(sessionID: string, title: string): Promise<void> {
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to rename session:", error)
  ...
  private async handleExportSessionTranscript(sessionID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to export session transcript:", error)
      this.postMessage({
        type: "error",
    ...
    const task = (async () => {
      ...
      while (true) {
        ...
        try {
          ...
          const settings = vscode.workspace.getConfiguration("neo-code.new.model")
          const message = {
          ...
          console.error("[Neo New] NeoProvider: Failed to fetch providers:", error)
        }
        if (!this.providersQueued) return
        generation = this.providersGeneration
      }
  ...
  private async fetchAndSendAgents(): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to fetch agents:", error)
    }
  }
  ...
  private async fetchAndSendSkills(): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to fetch skills:", error)
    }
  }
  ...
  private async fetchAndSendCommands(): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to fetch commands:", error)
    }
  }
  ...
  private async removeSkillViaCli(location: string): Promise<boolean> {
    ...
    try {
      const dir = this.getWorkspaceDirectory()
      const result = await this.client.neocode.removeSkill({ location, directory: dir })
      if (result.error) {
        console.error("[Neo New] removeSkill returned error:", result.error)
      ...
      console.error("[Neo New] Failed to remove skill:", error)
      this.cachedSkillsMessage = null
      this.cachedCommandsMessage = null
      await Promise.all([this.fetchAndSendSkills(), this.fetchAndSendCommands()])
      return false
  ...
  private async handleRemoveAgent(name: string): Promise<void> {
    if (!this.client) return
    try {
      const result = await this.client.neocode.removeAgent({ name, directory: this.getWorkspaceDirectory() })
      if (result.error) {
        console.error("[Neo New] removeAgent returned error:", result.error)
      }
    } catch (err) {
      console.error("[Neo New] Failed to remove agent:", err)
    }
    this.cachedAgentsMessage = null
  ...
  private async handleRemoveMcp(name: string): Promise<void> {
    ...
    if (!removed) {
      console.error("[Neo New] NeoProvider: Failed to remove MCP server:", name)
    }
  }
  ...
  private async fetchAndSendMcpStatus(): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to fetch MCP status:", error)
    }
  }
  ...
  private async fetchAndSendConfig(): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to fetch config:", error)
    }
  }
  ...
  private async fetchAndSendGlobalConfig(): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to fetch global config:", error)
    }
  }
  ...
  private async fetchAndSendIndexingStatus(): Promise<void> {
    ...
    try {
      ...
      const auth = Buffer.from(`neo:${config.password}`).toString("base64")
      const res = await fetch(`${config.baseUrl}/indexing/status`, {
        headers: {
          Authorization: `Basic ${auth}`,
          ...(dir ? { "x-neo-directory": dir } : {}),
        },
      })
      ...
      console.error("[Neo New] NeoProvider: Failed to fetch indexing status:", error)
    }
  }

  private async fetchAndSendNeoEmbeddingModels(): Promise<void> {
    const catalog = await fetchNeoEmbeddingModelCatalog()
    const message = { type: "neoEmbeddingModelsLoaded", catalog }
    this.cachedNeoEmbeddingModelsMessage = message
    this.postMessage(message)
  }
  ...
  private async fetchAndSendConfigUpdated(): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to fetch config after update:", error)
    }
  }
  ...
  private async checkConfigWarnings(from: string): Promise<void> {
    if (this.configWarningsShown) {
      console.log("[Neo New] NeoProvider: config warnings already shown", { from })
      return
    }
    if (!this.client) {
      console.log("[Neo New] NeoProvider: config warnings skipped (no client)", { from })
      return
    }
    try {
      const dir = this.getWorkspaceDirectory()
      console.log("[Neo New] NeoProvider: checking config warnings", { from, dir })
      const result = await this.client.config.warnings({ directory: dir })
      const list = result?.data ?? []
      console.log("[Neo New] NeoProvider: config warnings fetched", { from, count: list.length })
      if (list.length === 0) return
      this.configWarningsShown = true

      const first = list[0]!
      const summary = list.length === 1 ? first.message : `${first.message} (and ${list.length - 1} more)`
      console.warn("[Neo New] NeoProvider: showing config warnings", { from, count: list.length, path: first.path })
      ...
      if (action === "Show Details") {
        ...
        const channel = vscode.window.createOutputChannel("Neo Config Warnings")
        channel.clear()
        channel.appendLine(lines.join("\n\n"))
        channel.show()
      }
    } catch (err) {
      console.warn("[Neo New] NeoProvider: checkConfigWarnings failed:", { from, err })
    }
  }
  ...
  // Cloud session methods extracted to neo-provider/handlers/cloud-session.ts
  ...
  private sendNotificationSettings(): void {
    const attention = vscode.workspace.getConfiguration("neo-code.new.attention")
    this.postMessage({
  ...
  private sendTimelineSetting(): void {
    const config = vscode.workspace.getConfiguration("neo-code.new")
    this.postMessage({
  ...
  private async runToggleSandbox(
    ...
    try {
      ...
      const remembered = await this.connectionService.sandboxPreference
        ...
        .then(() => true)
        .catch((error) => {
          console.error("[Neo New] Failed to persist sandbox default:", error)
          return false
        })
      this.postMessage({
        type: "sandboxStatus",
  ...
  private async handleUpdateConfig(
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Config write succeeded but post-write refresh failed:", error)
      const patch =
  ...
  private postConfigFailure(error: unknown): void {
    console.error("[Neo New] NeoProvider: Failed to update config:", error)
    this.postMessage({
  ...
  private async withRetry(
    ...
    try {
      for (let attempt = 1; ; attempt++) {
        ...
        console.log(`[Neo New] NeoProvider: Retry on ${status}, attempt ${attempt}/${MAX_RETRIES}, delay ${delay}ms`)
  ...
  private maxCostSetting(): number {
    return this.setMaxCost(vscode.workspace.getConfiguration("neo-code.new").get<number>("maxCost", 0))
  }

  private commitMessageLanguageSetting(): string {
    return vscode.workspace.getConfiguration("neo-code.new").get<string>("languageCommitMessage", "sync")
  }
  ...
  private async handleSendMessage(
    ...
    try {
      ...
      await runWithMessageConfirmation(this.confirmations, messageID, "NeoProvider: Message request", () =>
        this.withRetry(
          () =>
      ...
      console.error("[Neo New] NeoProvider: Failed to send message:", error)
      this.postMessage({
        type: "sendMessageFailed",
  ...
  private async handleSendCommand(
    ...
    try {
      ...
      await this.requirements.assertAgentRequirements(agent, dir)
      await this.checkpoints.get(sid)
      await runWithMessageConfirmation(this.confirmations, messageID, "NeoProvider: Command request", () =>
        this.withRetry(
          () =>
      ...
      console.error("[Neo New] NeoProvider: Failed to send command:", error)
      this.postMessage({
        type: "sendMessageFailed",
  ...
  private async handleRevertSession(sessionID: string, messageID: string, partID?: string): Promise<void> {
    ...
    if (error) {
      console.error("[Neo New] NeoProvider: Failed to revert session:", error)
  ...
  private async handleUnrevertSession(sessionID: string): Promise<void> {
    ...
    if (error) {
      console.error("[Neo New] NeoProvider: Failed to unrevert session:", error)
  ...
  private async handleCompact(sessionID?: string, providerID?: string, modelID?: string): Promise<void> {
    ...
    if (!target) {
      console.error("[Neo New] NeoProvider: No sessionID for compact")
      return
    }

    if (!providerID || !modelID) {
      console.error("[Neo New] NeoProvider: No model selected for compact")
      this.postMessage({
        type: "error",
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to compact session:", error)
      this.postMessage({
        type: "error",
  ...
  // Permission + question handlers extracted to neo-provider/handlers/permission.ts and question.ts
  ...
  // Cloud session handlers extracted to neo-provider/handlers/cloud-session.ts
  ...
  // Auth handlers extracted to neo-provider/handlers/auth.ts
  ...
  private async disposeGlobal(): Promise<void> {
    ...
    await this.client.global
      .dispose()
      .catch((e: unknown) => console.warn("[Neo New] NeoProvider: global.dispose() after org switch failed:", e))

    // Org switch succeeded — refresh profile and providers independently (best-effort)
    try {
      const profileResult = await this.client!.neo.profile()
      ...
      console.error("[Neo New] NeoProvider: Failed to refresh profile after org switch:", error)
    }
    try {
      await this.fetchAndSendProviders()
    } catch (error) {
      console.error("[Neo New] NeoProvider: Failed to refresh providers after org switch:", error)
    }
  }

  /**
   * Handle a generic setting update from the webview.
   * The key uses dot notation relative to `neo-code.new` (e.g. "browserAutomation.enabled").
   */
  private async handleUpdateSetting(key: string, value: unknown): Promise<void> {
    ...
    const config = vscode.workspace.getConfiguration(`neo-code.new${section ? `.${section}` : ""}`)
  ...
  /**
   * Reset all "neo-code.new.*" extension settings to their defaults by reading
   * contributes.configuration from the extension's package.json at runtime.
   * Only resets settings under the "neo-code.new." namespace to avoid touching
   * settings from the previous version of the extension which shares the same
   * extension ID and "neo-code.*" namespace.
   */
  private async handleResetAllSettings(): Promise<void> {
    const confirmed = await vscode.window.showWarningMessage(
      "Reset all Neo Code extension settings to defaults?",
      { modal: true },
      "Reset",
    )
    if (confirmed !== "Reset") return

    const prefix = "neo-code.new."
    const ext = vscode.extensions.getExtension("neocode.neo-code")
    ...
    await this.extensionContext?.globalState.update("neo.dismissedNotificationIds", undefined)
    await this.extensionContext?.globalState.update("neo.agentMigrationBannerDismissed", undefined)
    await this.extensionContext?.globalState.update("neo.marketplace.dismissedSuggestions", undefined)
    ...
    vscode.window.showInformationMessage("Neo Code settings have been reset to defaults.")
  }
  ...
  private sendBrowserSettings(): void {
    const config = vscode.workspace.getConfiguration("neo-code.new.browserAutomation")
    this.postMessage({
  ...
  private sendClaudeCompatSetting(): void {
    const enabled = vscode.workspace.getConfiguration("neo-code.new").get<boolean>("claudeCodeCompat", false)
    this.postMessage({
  ...
  private async handleReload(): Promise<void> {
    if (!this.client) {
      console.warn("[Neo New] handleReload: no client connection")
      return
    }
    const dir = this.getWorkspaceDirectory(this.currentSession?.id)
    try {
      ...
      if (status === 409) {
        ...
        console.error("[Neo New] handleReload: reload endpoint failed:", err)
  ...
  private handleEvent(event: ProviderEvent, directory?: string): void {
    if (event.type === "neo-sessions.remote-status-changed") {
    ...
    // so this runs on every NeoProvider instance — including the Settings panel which has no
    ...
    if (event.type === "message.part.updated") {
      ...
      if (childId && !this.trackedSessionIds.has(childId)) {
        console.log("[Neo New] NeoProvider: 🔗 Auto-adopting child session from task tool", { childId })
    ...
    if (!this.webview) {
      const type =
        ...
        typeof (message as { type?: unknown }).type === "string"
          ...
          : "<unknown>"
      console.warn("[Neo New] NeoProvider: ⚠️ postMessage dropped (no webview)", { type })
      return
    }

    void this.webview.postMessage(message).then(undefined, (error) => {
      console.error("[Neo New] NeoProvider: ❌ postMessage failed", error)
    })
  }

  private flushPendingNeoModel(): void {
    if (!this.webview || !this.isWebviewReady || !this.client || !this.pendingNeoModel) return

    const pending = this.pendingNeoModel
    this.pendingNeoModel = null
    this.postMessage({ type: "selectNeoModel", ...pending })
  }
  ...
  public async appendReviewComments(comments: unknown[], autoSend = false): Promise<void> {
    ...
    if (!this.webview) {
      await vscode.commands.executeCommand(`${NeoProvider.viewType}.focus`)
    }
  ...
  public async toggleMemory(sessionID?: string): Promise<void> {
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to toggle memory:", error)
  ...
  private async getGitRemoteUrl(): Promise<string | undefined> {
    try {
      ...
      console.warn("[Neo New] NeoProvider: Failed to get git remote URL:", error)
      return undefined
    }
  }
   ...
   * given directory, filtered through .neocodeignore.
   */
  ...
  private async gatherEditorContext(dir?: string): Promise<EditorContext> {
    ...
    // Visible files (capped to avoid bloating context, filtered through .neocodeignore)
    const visibleFiles = [
      ...new Set(
        [
    ...
    // Active file (also filtered through .neocodeignore)
  ...
  // legacy-migration start -------------------------------------------------------
  // Migration handlers extracted to neo-provider/handlers/migration.ts
