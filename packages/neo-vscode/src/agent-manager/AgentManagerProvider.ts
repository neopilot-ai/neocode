...
export class AgentManagerProvider implements Disposable {
  public static readonly viewType = "neo-code.new.AgentManagerPanel"
  ...
  constructor(
    ...
    this.outputChannel = host.createOutput("Neo Agent Manager")
    ...
    this.unsubTool = this.connectionService.onEventFiltered(
      (event) => (event as { type?: string }).type === "neocode.agent_manager.start",
  ...
  private async initializeState(): Promise<void> {
    ...
    // When the .neocode → .neo migration rewrote git worktree refs, nudge
  ...
  private openWorktreeFile(sessionId: string, filePath: string, line?: number, column?: number): void {
    ...
    try {
      ...
      console.error("[Neo New] AgentManagerProvider: Cannot resolve file path:", err)
      return
    }
   ...
   * Called from NeoProvider when the sidebar sends "continueInWorktree".
   */
