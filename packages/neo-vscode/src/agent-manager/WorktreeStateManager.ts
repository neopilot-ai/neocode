 ...
 *
 * Persists to `.neo/agent-manager.json`. Decouples worktrees from sessions
...
import { NEO_DIR, migrateAgentManagerData, type MigrationResult } from "./constants"
...
export class WorktreeStateManager {
  ...
  constructor(root: string, log: (msg: string) => void) {
    this.root = root
    this.file = path.join(root, NEO_DIR, STATE_FILE)
    this.log = log
  }
  ...
  async load(): Promise<StateLoadResult> {
    // Migrate Agent Manager data from .neocode → .neo before first read
  ...
  private apply(content: string): void {
    ...
    for (const [id, wt] of Object.entries(data.worktrees ?? {})) {
      // Rewrite stale .neocode paths while preserving the separator style already stored.
      const fixed =
        wt.path?.replace(/([/\\])\.neocode([/\\])/g, (_match, leadingSep, trailingSep) => {
          return `${leadingSep}.neo${trailingSep}`
        }) ?? wt.path
