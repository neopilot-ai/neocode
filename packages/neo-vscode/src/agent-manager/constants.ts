...
// TODO: Remove the legacy .neocode -> .neo migration helpers below after the
// GA release cleanup tracked in https://github.com/Neopilot-Ai/neocode/issues/6986.
...
/** Neo config directory name (project-level and inside worktrees). */
export const NEO_DIR = ".neo"

/** Legacy config directory name for backward compatibility reads. */
export const LEGACY_DIR = ".neocode"

/** Agent Manager files that should be migrated from .neocode/ to .neo/. */
...
export interface MigrationResult {
  /** Number of git worktree refs that were rewritten from .neocode → .neo. */
  refsFixed: number
}

/**
 * Migrate Agent Manager data from .neocode/ to .neo/.
 *
 * Moves individual Agent Manager files/directories (worktrees, state,
 * setup scripts) from the legacy .neocode/ into .neo/. Skips items
 * that already exist in .neo/ (the new location wins). This is safe
 * because Agent Manager exclusively owns these files.
 *
 * Fixes git worktree internal references (.git/worktrees/{name}/gitdir)
 * whenever .neo/worktrees/ exists so partially migrated repos recover too.
 *
...
export async function migrateAgentManagerData(root: string, log: (msg: string) => void): Promise<MigrationResult> {
  ...
  const target = path.join(root, NEO_DIR)
  ...
  if (await isDirectory(legacy)) {
    ...
    for (const item of AGENT_MANAGER_ITEMS) {
      ...
      if (await exists(dst)) {
        log(`Skipping ${item}: already exists in ${NEO_DIR}`)
        continue
      }

      try {
        await fs.promises.rename(src, dst)
        log(`Migrated ${item} from ${LEGACY_DIR} to ${NEO_DIR}`)
      } catch (err) {
...
/**
 * After moving worktrees from .neocode/ to .neo/, fix git internal refs.
 *
 ...
 * gitdir files that reference the old .neocode path.
 *
...
async function fixGitWorktreeRefs(root: string, log: (msg: string) => void): Promise<number> {
  ...
  const newSegment = path.join(root, NEO_DIR) + path.sep
  let fixed = 0
