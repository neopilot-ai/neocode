 ...
 *
 * Ported from neocode/src/core/neocode/agent-manager/WorktreeManager.ts.
 * Handles creation, discovery, and cleanup of worktrees stored in
 * {projectRoot}/.neo/worktrees/
 */
...
import { type GitOps, isNeoOwnedSshCommand, nonInteractiveEnv } from "./GitOps"
...
import { NEO_DIR, LEGACY_DIR, migrateAgentManagerData } from "./constants"

const SESSION_ID_FILE = "session-id"
const METADATA_FILE = "metadata.json"
const GIT_METADATA_FILE = "neo-agent-manager-metadata.json"
...
export class WorktreeManager {
  ...
  constructor(root: string, log: (msg: string) => void, ops?: GitOps) {
    this.root = root
    this.dir = path.join(root, NEO_DIR, "worktrees")
    this.git = simpleGit(root)
    this.ops = ops
    this.log = log
  }

  /** Run once before first read/write to migrate Agent Manager data from .neocode → .neo. */
  ...
  private async removeWorktreeImpl(worktreePath: string, branch?: string): Promise<void> {
    ...
    const temp = path.join(path.dirname(worktreePath), `.neo-delete-${randomUUID()}`)
    try {
  ...
  /** Remove orphaned .neo-delete-* temp dirs left by interrupted deletions. */
  ...
  async readMetadata(
    ...
    // Check .neo/ first, then legacy .neocode/
    for (const dirName of [NEO_DIR, LEGACY_DIR]) {
  ...
  async ensureGitExclude(): Promise<void> {
    ...
    const items = [
      [".neo/worktrees/", "Neo Code agent worktrees"],
      [".neo/agent-manager.json", "Neo Agent Manager state"],
      [".neo/setup-script", "Neo Code worktree setup script"],
      [".neo/setup-script.sh", "Neo Code worktree setup script"],
      [".neo/setup-script.ps1", "Neo Code worktree setup script"],
      [".neo/setup-script.cmd", "Neo Code worktree setup script"],
      [".neo/setup-script.bat", "Neo Code worktree setup script"],
      [".neocode/worktrees/", "Neo Code legacy agent worktrees"],
      [".neocode/agent-manager.json", "Neo Agent Manager legacy state"],
      [".neocode/setup-script", "Neo Code legacy worktree setup script"],
      [".neocode/setup-script.sh", "Neo Code legacy worktree setup script"],
      [".neocode/setup-script.ps1", "Neo Code legacy worktree setup script"],
      [".neocode/setup-script.cmd", "Neo Code legacy worktree setup script"],
      [".neocode/setup-script.bat", "Neo Code legacy worktree setup script"],
    ] as const
  ...
  async resolveStartPoint(
    ...
    if (remote) {
      ...
      try {
        ...
        // is the fixed value Neo injects — never for an inherited one, which
        // could be attacker-controlled.
        const env = nonInteractiveEnv()
        await simpleGit(this.root, { unsafe: { allowUnsafeSshCommand: isNeoOwnedSshCommand(env) } })
          .env(env)
