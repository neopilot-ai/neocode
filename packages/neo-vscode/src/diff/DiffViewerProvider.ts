...
export class DiffViewerProvider implements vscode.Disposable {
  public static readonly viewType = "neo-code.new.DiffViewerPanel"
  ...
  constructor(
    ...
    this.output = vscode.window.createOutputChannel("Neo Diff Panel")
  }
  ...
  /**
   * Entry point for the `neo-code.new.showChanges` command. Composes the
  ...
  private onWebviewReady(): void {
    ...
    void this.panel.webview.postMessage({
      ...
      languageOverride: vscode.workspace.getConfiguration("neo-code.new").get<string>("language"),
