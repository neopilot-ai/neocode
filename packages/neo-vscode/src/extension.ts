...
import { NeoClawProvider } from "./neoclaw/NeoClawProvider"
...
import { NeoConnectionService } from "./services/cli-backend"
...
import { registerCodeActions, registerTerminalActions, neocodeActionProvider } from "./services/code-actions"
...
const RESTORE_KEY = "neo.workbench.restore"
...
// without requiring the user to open a Neo sidebar or panel first. The CLI backend is NOT spawned here;
// it starts lazily when a webview connects or when ensureBackendForAutocomplete() triggers it.
export function activate(context: vscode.ExtensionContext) {
  console.log("Neo Code extension is now active")
  shuttingDown = false

  const telemetry = TelemetryProxy.getInstance()

  // Create shared connection service (one server for all webviews)
  const connectionService = new NeoConnectionService(context)
  ...
  const unsubscribeStateChange = connectionService.onStateChange((state) => {
    if (state === "connected") {
      ...
      if (config) {
        ...
        // CLI reads NEO_TELEMETRY_LEVEL once at spawn, so without this call
      ...
      try {
        ...
        console.log("[Neo New] CLI connected, calling remoteService.refresh()")
        remoteService.refresh().catch((err) => console.warn("[Neo New] initial remote refresh failed:", err))
      } catch {
  ...
  for (const folder of vscode.workspace.workspaceFolders ?? []) {
    void markWorkspace(folder.uri.fsPath, (msg) => console.warn(`[Neo New] ${msg}`))
  }
  ...
  const tabPanels = new Map<vscode.WebviewPanel, NeoProvider>()
  ...
  const provider = new NeoProvider(context.extensionUri, connectionService, context)
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(NeoProvider.viewType, provider, {
  ...
  // commands by default.
  const skip = ["neo-code.new.agentManagerOpen", "neo-code.new.agentManager.showTerminal"]
  if (process.platform === "darwin") skip.push("neo-code.new.agentManager.runScript")
  ensureCommandsSkipShell(skip)

  // Create NeoClaw chat provider for editor panel
  const neoClawProvider = new NeoClawProvider(context.extensionUri, connectionService)
  context.subscriptions.push(neoClawProvider)
  ...
  // Register serializer so NeoClaw panel restores when VS Code restarts
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer(NeoClawProvider.viewType, {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
        neoClawProvider.restorePanel(panel)
        return Promise.resolve()
      },
    }),
  )
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("neo-code.new.TabPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
        const tabProvider = new NeoProvider(context.extensionUri, connectionService, context, {
        ...
        panel.onDidDispose(
          () => {
            console.log("[Neo New] Tab panel restored from restart disposed")
  ...
  for (const suffix of settingsViews) {
    context.subscriptions.push(
      vscode.window.registerWebviewPanelSerializer(`neo-code.new.${suffix}`, {
        deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("neo-code.new.SubAgentViewerPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("neo-code.new.sidebarTitle.plusButtonClicked", () => {
      track("new_task", "neo-code.new.plusButtonClicked")
    }),
    vscode.commands.registerCommand("neo-code.new.sidebarTitle.historyButtonClicked", () => {
      track("history", "neo-code.new.historyButtonClicked")
    }),
    vscode.commands.registerCommand("neo-code.new.sidebarTitle.agentManagerOpen", () => {
      track("agent_manager", "neo-code.new.agentManagerOpen")
    }),
    vscode.commands.registerCommand("neo-code.new.sidebarTitle.neoClawOpen", () => {
      track("neoclaw", "neo-code.new.neoClawOpen")
    }),
    vscode.commands.registerCommand("neo-code.new.sidebarTitle.marketplaceButtonClicked", () => {
      track("marketplace", "neo-code.new.marketplaceButtonClicked")
    }),
    vscode.commands.registerCommand("neo-code.new.sidebarTitle.profileButtonClicked", () => {
      track("profile", "neo-code.new.profileButtonClicked")
    }),
    vscode.commands.registerCommand("neo-code.new.sidebarTitle.settingsButtonClicked", () => {
      track("settings", "neo-code.new.settingsButtonClicked")
    }),
    vscode.commands.registerCommand("neo-code.new.plusButtonClicked", () => {
    ...
    vscode.commands.registerCommand("neo-code.new.agentManagerOpen", () => {
      agentManagerProvider.openPanel()
    }),
    vscode.commands.registerCommand("neo-code.new.marketplaceButtonClicked", (directory?: string | null) => {
      marketplacePanelProvider.openPanel(directory)
    }),
    vscode.commands.registerCommand("neo-code.new.neoClawOpen", () => {
      neoClawProvider.openPanel()
    }),
    vscode.commands.registerCommand("neo-code.new.historyButtonClicked", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("neo-code.new.cycleAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("neo-code.new.cyclePreviousAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("neo-code.new.profileButtonClicked", () => {
      settingsEditorProvider.openPanel("profile")
    }),
    vscode.commands.registerCommand("neo-code.new.settingsButtonClicked", (tab?: string) => {
      settingsEditorProvider.openPanel("settings", tab)
    }),
    vscode.commands.registerCommand("neo-code.new.openIndexingSettings", () => {
      settingsEditorProvider.openPanel("settings", "indexing")
    }),
    vscode.commands.registerCommand("neo-code.new.showMemory", async () => {
      if (agentManagerProvider.isActive()) {
        await agentManagerProvider.showMemory()
        return
      }
      const target = activeTabProvider() ?? provider
      if (target === provider) await vscode.commands.executeCommand("neo-code.SidebarProvider.focus")
      await target.waitForReady()
      await target.showMemory()
    }),
    vscode.commands.registerCommand("neo-code.new.toggleMemory", async () => {
      if (agentManagerProvider.isActive()) {
    ...
    vscode.commands.registerCommand("neo-code.new.openMigrationWizard", () => {
      provider.postMessage({ type: "migrationState", needed: true, source: "legacy" })
    }),
    // legacy-migration end
    vscode.commands.registerCommand("neo-code.new.generateTerminalCommand", async () => {
      ...
      if (!input) return
      await vscode.commands.executeCommand("neo-code.SidebarProvider.focus")
      await provider.waitForReady()
      provider.postMessage({ type: "triggerTask", text: `Generate a terminal command: ${input}` })
    }),
    vscode.commands.registerCommand("neo-code.new.toggleRemote", () => {
      remoteService.toggle().catch((err) => console.error("[Neo New] toggleRemote command failed:", err))
    }),
    vscode.commands.registerCommand("neo-code.new.openInTab", () => {
      return openNeoInNewTab(
        context,
        connectionService,
    ...
    vscode.commands.registerCommand(
      "neo-code.new.showChanges",
      (arg?: { sessionId?: string; turnId?: string; initialSourceId?: string }) => {
        diffViewerProvider.openFromCommand(arg)
      },
    ),
    vscode.commands.registerCommand("neo-code.new.openSubAgentViewer", (sessionID: string, title?: string) => {
      subAgentViewerProvider.openPanel(sessionID, title)
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.previousSession", () => {
      agentManagerProvider.postMessage({ type: "action", action: "sessionPrevious" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.nextSession", () => {
      agentManagerProvider.postMessage({ type: "action", action: "sessionNext" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.previousTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "tabPrevious" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.nextTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "tabNext" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.search", () => {
      agentManagerProvider.postMessage({ type: "action", action: "search" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.showTerminal", () => {
    ...
    vscode.commands.registerCommand("neo-code.new.agentManager.runScript", () => {
      agentManagerProvider.postMessage({ type: "action", action: "runScript" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.toggleDiff", () => {
      agentManagerProvider.postMessage({ type: "action", action: "toggleDiff" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.showShortcuts", () => {
      agentManagerProvider.postMessage({ type: "action", action: "showShortcuts" })
    }),

    vscode.commands.registerCommand("neo-code.new.agentManager.newTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newTab" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.newTerminal", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newTerminal" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.closeTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "closeTab" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.newWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newWorktree" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.quickWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "quickWorktree" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.openWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "openWorktree" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.openPR", () => {
      agentManagerProvider.postMessage({ type: "action", action: "openPR" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.closeWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "closeWorktree" })
    }),
    vscode.commands.registerCommand("neo-code.new.agentManager.advancedWorktree", () =>
      agentManagerProvider.openAdvancedWorktree(),
    ),
    ...Array.from({ length: 9 }, (_, i) =>
      vscode.commands.registerCommand(`neo-code.new.agentManager.jumpTo${i + 1}`, () => {
        agentManagerProvider.postMessage({ type: "action", action: `jumpTo${i + 1}` })
      }),
    ),
  )

  // Register URI handler for extension deep links (vscode://neocode.neo-code/neocode/...)
  context.subscriptions.push(
    vscode.window.registerUriHandler({
      async handleUri(uri: vscode.Uri) {
        const sessionMatch = uri.path.match(/^\/neocode\/s\/([a-zA-Z0-9_-]+)$/)
        const sessionId = sessionMatch?.[1]
        if (sessionId) {
          console.log("[Neo New] URI handler: opening cloud session:", sessionId)
          await vscode.commands.executeCommand(`${NeoProvider.viewType}.focus`)
          provider.openCloudSession(sessionId)
          return
        }

        if (uri.path !== "/neocode/switch" && uri.path !== "/neocode/model") return
        const params = new URLSearchParams(uri.query)
        const modelID = params.get("model") || undefined
        const agent = params.get("agent") || undefined
        if (!modelID && !agent) return
        console.log("[Neo New] URI handler: applying linked Neo selection:", { modelID, agent })
        await vscode.commands.executeCommand(`${NeoProvider.viewType}.focus`)
        provider.selectNeoModel(modelID, agent)
      },
    }),
  )
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("neo-code.new.reload", () => {
      provider.reload().catch((e) => console.error("[Neo New] reload command failed:", e))
    }),
  )
  ...
  context.subscriptions.push(
    vscode.languages.registerCodeActionsProvider(
      { scheme: "file" },
      new neocodeActionProvider(),
      neocodeActionProvider.metadata,
    ),
  )
...
async function openNeoInNewTab(
  context: vscode.ExtensionContext,
  connectionService: NeoConnectionService,
  agentManagerProvider: AgentManagerProvider,
  tabPanels: Map<vscode.WebviewPanel, NeoProvider>,
  ...
  const panel = vscode.window.createWebviewPanel("neo-code.new.TabPanel", EXTENSION_DISPLAY_NAME, targetCol, {
    enableScripts: true,
  ...
  panel.iconPath = {
    light: vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "neo-light.svg"),
    dark: vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "neo-dark.svg"),
  }
  ...
  panel.onDidDispose(
    () => {
      console.log("[Neo New] Tab panel disposed")
      tabPanels.delete(panel)
      tabProvider.dispose()
    },
    null,
