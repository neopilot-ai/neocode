...
export async function startSpeechCapture(input: Input): Promise<boolean> {
  ...
  try {
    ...
    const file = path.join(os.tmpdir(), `neo-stt-${process.pid}-${Date.now()}.wav`)
...
export async function stopSpeechCapture(requestId: string): Promise<Audio> {
  ...
  const size = await stat(state.file)
    .then((info) => info.size)
    .catch((err: unknown) => {
      console.warn("[Neo New] Failed to stat speech recording", err)
      return 0
    })
...
async function findFFmpeg(): Promise<string> {
  ...
  for (const bin of paths) {
    ...
    try {
      ...
      console.warn(`[Neo New] FFmpeg candidate failed: ${bin}`, err)
    }
  }

  throw new Error("Speech input needs the bundled FFmpeg helper, but it was not found. Rebuild or reinstall Neo Code.")
}
...
async function inputArgSets(bin: string): Promise<Args[]> {
  ...
  if (process.platform === "linux") {
    const device = process.env.NEO_FFMPEG_AUDIO_DEVICE
    if (device)
      return [
...
async function windowsInputArgSets(bin: string): Promise<Args[]> {
  const configured = process.env.NEO_FFMPEG_AUDIO_DEVICE
...
async function removeFile(file: string): Promise<void> {
  await unlink(file).catch((err: unknown) => {
    console.warn("[Neo New] Failed to remove speech recording", err)
  })
}
