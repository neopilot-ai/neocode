...
export class RemoteStatusService implements vscode.Disposable {
  ...
  constructor() {
    ...
    this.bar.command = "neo-code.new.toggleRemote"
    this.sync()
  }

  setClient(c: NeoClient | null): void {
    this.client = c
  }
  ...
  async refresh(): Promise<void> {
    ...
    const res = await this.client.remote.status().catch((err: unknown) => {
      console.warn("[Neo] remote status refresh failed:", err)
      return undefined
    })
    if (!res?.data) return
  ...
  private sync(): void {
    ...
    if (this.state.connected) {
      this.bar.text = "$(radio-tower) Neo Remote"
