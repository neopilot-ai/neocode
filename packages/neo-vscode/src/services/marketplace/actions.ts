...
export interface MarketplaceActionContext {
  connection: NeoConnectionService
...
export async function removeMarketplaceItemFromAllScopes(
  ...
  try {
    ...
    console.warn("[Neo New] Marketplace removal failed:", err)
    return false
  }
}
...
async function fetchSkills(ctx: MarketplaceActionContext, dir: string) {
  try {
    ...
    console.warn("[Neo New] Failed to fetch CLI skills for marketplace:", err)
    return undefined
  }
}

async function invalidate(
  ctx: { connection: NeoConnectionService },
  scope: "project" | "global",
  dir: string,
): Promise<void> {
  const client = await ctx.connection.getClientAsync(dir).catch((err: unknown) => {
    console.warn("[Neo New] Marketplace CLI invalidation deferred:", err)
    return null
  })
  if (!client) return
  ...
  if (scope === "global") {
    await client.global.config.update({ config: {} }).catch((err: unknown) => {
      console.warn("[Neo New] global.config.update after marketplace change failed:", err)
    })
  }
  await client.instance.dispose({ directory: dir }).catch((err: unknown) => {
    console.warn("[Neo New] instance.dispose() after marketplace change failed:", err)
  })
}
...
async function removeLegacyMcp(
  ...
  if (project && scope !== "global") {
    files.push(vscode.Uri.file(path.join(project, ".neo", "mcp.json")))
    files.push(vscode.Uri.file(path.join(project, ".neocode", "mcp.json")))
  }
  ...
  for (const uri of files) {
    ...
    try {
      ...
      console.warn("[Neo New] Failed to remove legacy MCP from", uri.fsPath, err)
    }
  }
  return removed
}
