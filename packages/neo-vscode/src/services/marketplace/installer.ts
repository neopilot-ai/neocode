...
export class MarketplaceInstaller {
  ...
  async installAgent(
    ...
    // Migration: remove stale neo.json agent entry with same id if present
    const config = await this.readConfig(scope, workspace)
  ...
  async removeAgent(
    ...
    // Also clean up any stale neo.json agent entry
    const config = await this.readConfig(scope, workspace)
    if (config.agent?.[item.id]) {
      delete (config.agent as Record<string, unknown>)[item.id]
      if (Object.keys(config.agent as object).length === 0) delete config.agent
  ...
  async installSkill(
    ...
    const tarball = path.join(os.tmpdir(), `neo-skill-${item.id}-${randomUUID()}.tar.gz`)
...
/**
 * Normalize a marketplace MCP entry from the old Neocode format to the CLI's expected format.
 *
