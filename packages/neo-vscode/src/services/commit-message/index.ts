...
import type { NeoConnectionService } from "../cli-backend/connection-service"
...
export function registerCommitMessageService(
  ...
  const command = vscode.commands.registerCommand(
    "neo-code.new.generateCommitMessage",
    async (arg?: vscode.SourceControl) => {
      ...
      try {
        ...
        console.error("[Neo New] Failed to connect to Neo backend:", err)
        vscode.window.showErrorMessage("Failed to connect to Neo backend. Please try again.")
        return
      }
      ...
      await vscode.window
        .withProgress(
          ...
          async (_progress, token) => {
            ...
            try {
              ...
              console.log("[Neo New] Commit message generated successfully")
            } finally {
              clearTimeout(timer)
            }
          },
        ...
        .then(undefined, (error: unknown) => {
          if (userCancelled) {
            console.log("[Neo New] Commit message generation was cancelled by user")
            return
          }
          if (timedOut) {
            console.log("[Neo New] Commit message generation timed out")
            vscode.window.showErrorMessage("Commit message generation timed out. Please try again.")
            return
          }
          const msg = getErrorMessage(error)
          console.error("[Neo New] Failed to generate commit message:", msg)
