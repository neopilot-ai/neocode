...
import { createNeoClient, type NeoClient } from "@neocode/sdk/v2/client"
...
async function parallel(items: string[], fn: (item: string) => Promise<void>): Promise<void> {
  ...
  for (const [index, error] of failures.slice(1)) {
    console.warn(`[Neo New] ConnectionService: Additional prompt drain failed for ${items[index]}:`, error)
  }
  throw failures[0]![1]
}
...
async function drainNetworkWaits(client: NeoClient, dir: string) {
...
/**
 * Shared connection service that owns the single ServerManager, NeoClient (SDK), and SdkSSEAdapter.
 * Multiple NeoProvider instances subscribe to it for SSE events and state changes.
 */
export class NeoConnectionService {
  readonly sandboxPreference: SandboxPreference
  private readonly serverManager: ServerManager
  private client: NeoClient | null = null
   ...
   */
  async getClientAsync(dir?: string): Promise<NeoClient> {
  ...
  /**
   * Subscribe to notification dismiss events broadcast from any NeoProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a notification dismiss event to all subscribed NeoProvider instances.
   */
  ...
  /**
   * Subscribe to language change events broadcast from any NeoProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a language change event to all subscribed NeoProvider instances.
   */
  ...
  /**
   * Subscribe to profile change events broadcast from any NeoProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a profile change event to all subscribed NeoProvider instances.
   */
  ...
  /**
   * Subscribe to migration-complete events broadcast from any NeoProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a migration-complete event to all subscribed NeoProvider instances.
   */
  ...
  /**
   * Subscribe to favorites change events broadcast from any NeoProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a favorites change event to all subscribed NeoProvider instances.
   */
  ...
  /**
   * Subscribe to model-selector expand/collapse changes broadcast from any NeoProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a model-selector expand/collapse change to all subscribed NeoProvider instances.
   */
   ...
   * NeoProvider (root + worktree dirs). Used by drainPendingPrompts() to
   ...
   * directory known to any currently-mounted NeoProvider.
   *
  ...
  private sendViewed(): void {
    ...
    void this.client.session
      ...
      .catch((err) => console.warn("[Neo New] ConnectionService: viewed flush failed:", err))
      .finally(() => {
  ...
  private startHealthPoll(baseUrl: string, password: string): void {
    ...
    this.healthPollTimer = setInterval(async () => {
      ...
      if (!healthy && this.state === "connected") {
        console.warn("[Neo New] ConnectionService: ❤️‍🩹 Health check failed — forcing SSE reconnect")
  ...
  private async checkHealth(baseUrl: string, password: string): Promise<boolean> {
    try {
      ...
      const res = await fetch(`${baseUrl}/global/health`, {
        headers: { Authorization: `Basic ${Buffer.from(`neo:${password}`).toString("base64")}` },
  ...
  private handleServerExit(code: number | null): void {
    console.warn("[Neo New] ConnectionService: CLI background process exited:", code)
  ...
  private async doConnect(workspaceDir: string): Promise<void> {
    ...
    const authHeader = `Basic ${Buffer.from(`neo:${server.password}`).toString("base64")}`
...
async function drainSuggestions(client: NeoClient, directory: string): Promise<void> {
