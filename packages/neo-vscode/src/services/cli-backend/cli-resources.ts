...
const neoSandboxWorker = "neo-sandbox-mutation-worker.js"
const bwrap = "bwrap"
const bwrapLicense = path.join("licenses", "bubblewrap")
const bwrapLicenseFiles = ["NOTICE", "COPYING", "MUSL-COPYRIGHT", "build.ts"]
const sandboxNetworkFiles = ["neo-sandbox-network-relay.js", "neo-sandbox-seccomp"]
...
export function resolveTreeSitterEnv(root: string): Record<string, string> {
  return { NEO_TREE_SITTER_WASM_DIR: treeSitterDirForExtension(root) }
}
...
export function neoSandboxWorkerForBinary(file: string): string {
  const p = paths(file)
  return p.join(p.dirname(file), neoSandboxWorker)
}

export function hasNeoSandboxWorker(file: string): boolean {
  return fs.existsSync(neoSandboxWorkerForBinary(file))
}
...
export async function copySandboxResources(source: string, target: string): Promise<void> {
  ...
  for (const file of sandboxNetworkFiles) {
    ...
    if (file === "neo-sandbox-seccomp") await fs.promises.chmod(output, 0o755)
  }
...
export async function copyNeoSandboxWorker(source: string, target: string): Promise<void> {
  const from = neoSandboxWorkerForBinary(source)
  const to = neoSandboxWorkerForBinary(target)
  if (!fs.existsSync(from)) throw new Error(`Neo sandbox mutation worker not found at ${from}`)
  await fs.promises.copyFile(from, to)
}

function cacheRoot() {
  const root = process.env.XDG_CACHE_HOME ?? path.join(os.homedir(), ".cache")
  return path.join(root, "neo-vscode", "bwrap")
}
...
export function resolveLocalBwrapEnv(
  ...
  return { NEO_BWRAP_PATH: executable }
}
