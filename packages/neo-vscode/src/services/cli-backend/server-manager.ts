...
export function resolveIndexingEnv(folders: readonly WorkspaceFolderLike[] | undefined): Record<string, string> {
  ...
  return { NEO_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" }
}

export function resolveManagedServerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  return { ...env, NEO_DISABLE_CHANNEL_DB: "true" }
}
...
export class ServerManager {
  ...
  async getServer(): Promise<ServerInstance> {
    console.log("[Neo New] ServerManager: 🔍 getServer called")
    if (this.instance) {
      console.log("[Neo New] ServerManager: ♻️ Returning existing instance:", { port: this.instance.port })
      return this.instance
    }

    if (this.startupPromise) {
      console.log("[Neo New] ServerManager: ⏳ Startup already in progress, waiting...")
      return this.startupPromise
    }

    console.log("[Neo New] ServerManager: 🚀 Starting new server instance...")
    this.startupPromise = this.startServer()
    try {
      this.instance = await this.startupPromise
      console.log("[Neo New] ServerManager: ✅ Server started successfully:", { port: this.instance.port })
      return this.instance
    } finally {
  ...
  private async startServer(): Promise<ServerInstance> {
    ...
    console.log("[Neo New] ServerManager: 📍 CLI path:", cliPath)
    console.log("[Neo New] ServerManager: 🔐 Generated password (length):", password.length)
    ...
    console.log("[Neo New] ServerManager: 📄 CLI isFile:", stat.isFile())
    console.log("[Neo New] ServerManager: 📄 CLI mode (octal):", (stat.mode & 0o777).toString(8))

    return new Promise((resolve, reject) => {
      console.log("[Neo New] ServerManager: 🎬 Spawning CLI process:", cliPath, ["serve", "--port", "0"])
      const cfg = vscode.workspace.getConfiguration("neo-code.new")
      ...
      const bwrapEnv = process.env.NEO_BWRAP_PATH ? {} : resolveLocalBwrapEnv(this.context.extensionPath, localCli)
      ...
      //     `neo-code.new.extraCaCerts` (NODE_EXTRA_CA_CERTS).
      ...
      const serverProcess = spawn(cliPath, ["serve", "--port", "0"], {
        ...
        env: {
          ...
          NEO_PARENT_PID: String(process.pid),
          NEO_CLIENT: "vscode",
          NEO_ENABLE_QUESTION_TOOL: "true",
          NEOCODE_FEATURE: "vscode-extension",
          ...indexingEnv,
          NEO_TELEMETRY_LEVEL: vscode.env.isTelemetryEnabled ? "all" : "off",
          NEO_APP_NAME: "neo-code",
          NEO_EDITOR_NAME: vscode.env.appName,
          NEO_PLATFORM: "vscode",
          NEO_MACHINE_ID: vscode.env.machineId,
          NEO_APP_VERSION: this.context.extension.packageJSON.version,
          NEO_VSCODE_VERSION: vscode.version,
          NEOCODE_VERSION: this.context.extension.packageJSON.version,
          NEOCODE_EDITOR_NAME: `${vscode.env.appName} ${vscode.version}`,
          ...(!claudeCompat && { NEO_DISABLE_CLAUDE_CODE: "true" }),
      ...
      console.log("[Neo New] ServerManager: 📦 Process spawned with PID:", serverProcess.pid)
      ...
      serverProcess.stdout?.on("data", (data: Buffer) => {
        ...
        console.log("[Neo New] ServerManager: 📥 CLI Server stdout:", output)

        const port = parseServerPort(output)
        if (port !== null && !resolved) {
          resolved = true
          console.log("[Neo New] ServerManager: 🎯 Port detected:", port)
      ...
      serverProcess.stderr?.on("data", (data: Buffer) => {
        ...
        console.error("[Neo New] ServerManager: ⚠️ CLI Server stderr:", errorOutput)
        stderrLines.push(errorOutput)
      })

      serverProcess.on("error", (error) => {
        console.error("[Neo New] ServerManager: ❌ Process error:", error)
        if (!resolved) {
          reject(error)
        }
      })
      ...
      serverProcess.on("exit", (code) => {
        console.log("[Neo New] ServerManager: 🛑 Process exited with code:", code)
      ...
      setTimeout(() => {
        if (!resolved) {
          console.error(`[Neo New] ServerManager: ⏰ Server startup timeout (${STARTUP_TIMEOUT_SECONDS}s)`)
  ...
  private getCliPath(): string {
    ...
    console.log("[Neo New] ServerManager: 📦 Using CLI path:", cliPath)
    return cliPath
  }
  ...
  dispose(): void {
    ...
    console.log("[Neo New] ServerManager: 🔴 Disposing — sending SIGTERM to process group, PID:", proc.pid)
    ...
    const timer = setTimeout(() => {
      if (proc.exitCode === null) {
        console.warn("[Neo New] ServerManager: ⚠️ Process did not exit after SIGTERM, sending SIGKILL")
