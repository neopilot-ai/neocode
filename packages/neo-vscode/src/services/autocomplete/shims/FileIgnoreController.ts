...
const NEOCODEIGNORE = ".neocodeignore"
const GITIGNORE = ".gitignore"

/**
 * Patterns for sensitive environment files, applied only when no .neocodeignore exists.
 */
...
export class FileIgnoreController {
  ...
  async initialize(): Promise<void> {
    ...
    // Try .neocodeignore first — if it exists, use only that.
    ...
    const neocodeignorePath = path.join(this.workspacePath, NEOCODEIGNORE)
    if (fs.existsSync(neocodeignorePath)) {
      const neocodeignoreContent = fs.readFileSync(neocodeignorePath, "utf-8")
      if (neocodeignoreContent.trim()) {
        this.ignoreInstance.add(neocodeignoreContent)
        this.ignoreInstance.add(NEOCODEIGNORE)
        return
      }
    }
