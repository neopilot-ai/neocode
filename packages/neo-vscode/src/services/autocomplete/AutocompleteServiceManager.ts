...
const CONFIG_SECTION = "neo-code.new.autocomplete"
...
export class AutocompleteServiceManager {
  ...
  constructor(context: vscode.ExtensionContext, connectionService: NeoConnectionService) {
    ...
    this.config = vscode.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("neo-code.new.language")) {
  ...
  private async updateGlobalContext() {
    await vscode.commands.executeCommand(
      "setContext",
      "neocode.autocomplete.enableSmartInlineTaskKeybinding",
  ...
  private handleFatalAutocompleteError(status: number | null): void {
    ...
    if (status === 402) {
      vscode.window.showWarningMessage(msg, t("neocode:autocomplete.creditsExhausted.addCredits")).then((choice) => {
        if (choice === t("neocode:autocomplete.creditsExhausted.addCredits")) {
          vscode.env.openExternal(vscode.Uri.parse("https://app.neo.khulnasoft.com/credits"))
        }
      })
    } else {
  ...
  public async showIncompatibilityExtensionPopup() {
    const message = t("neocode:autocomplete.incompatibilityExtensionPopup.message")
    const disableCopilot = t("neocode:autocomplete.incompatibilityExtensionPopup.disableCopilot")
    const disableInlineAssist = t("neocode:autocomplete.incompatibilityExtensionPopup.disableInlineAssist")
    ...
    if (response === disableCopilot) {
      ...
      await vscode.commands.executeCommand("neo-code.new.autocomplete.disable")
    }
  }
