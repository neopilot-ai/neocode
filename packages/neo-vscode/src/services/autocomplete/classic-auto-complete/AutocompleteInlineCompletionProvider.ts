 ...
 */
export const INLINE_COMPLETION_ACCEPTED_COMMAND = "neocode.autocomplete.inline-completion.accepted"
...
export class AutocompleteInlineCompletionProvider implements vscode.InlineCompletionItemProvider {
  ...
  private connectionService: NeoConnectionService
  ...
  public async provideInlineCompletionItems_Internal(
    ...
    try {
      ...
      if (matchingResult !== null) {
        ...
        vscode.commands.executeCommand("setContext", "neo-code.new.autocomplete.hasSuggestions", true)
  ...
  private async hasBalance(): Promise<boolean> {
    try {
      ...
      const result = await client.neo.profile().catch(() => null)
