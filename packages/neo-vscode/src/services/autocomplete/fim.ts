 ...
 * Uses the SDK's neo.fim() SSE endpoint which handles auth and streaming.
 *
...
export async function generateFim(
  ...
  console.info(`[FIM] request provider=${info.providerID} model=${info.requestModel} url=/neo/fim`)

  const { stream } = await client.neo.fim(
    {
      prefix,
      suffix,
 ...
 */
export function hasValidCredentials(connectionService: NeoConnectionService): boolean {
