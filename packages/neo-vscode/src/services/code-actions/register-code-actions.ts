...
import type { NeoProvider } from "../../NeoProvider"
...
export function registerCodeActions(
  ...
  activeTabProvider?: () => NeoProvider | undefined,
): void {
  ...
  // settles.
  const revealTarget = async (view: NeoProvider | AgentManagerProvider): Promise<boolean> => {
    if (view === provider) {
      await reveal()
      return true
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("neo-code.new.explainCode", async () => {
    ...
    vscode.commands.registerCommand("neo-code.new.fixCode", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("neo-code.new.improveCode", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("neo-code.new.addToContext", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("neo-code.new.focusChatInput", async () => {
      const view = target()
      if (!(await revealTarget(view))) return
    ...
    vscode.commands.registerCommand("neo-code.new.toggleChatSearch", async () => {
      const view = target()
      if (!(await revealTarget(view))) return
