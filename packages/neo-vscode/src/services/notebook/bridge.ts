...
import type {
  EventNeocodeNotebookCancelled,
  EventNeocodeNotebookRequested,
  NeoClient,
  NotebookFailure,
  NotebookRequest,
  NotebookResult,
...
import type { ConnectionState, NeoConnectionService } from "../cli-backend/connection-service"
...
export class NotebookBridge {
  ...
  private revision = 0
  private backend: NeoClient | undefined
  ...
  constructor(
    ...
    this.unsubscribeState = connection.onStateChange((state) => {
      ...
      void this.recover(revision).catch((error: unknown) => {
        console.error("[Neo New] NotebookBridge: pending request recovery failed:", error)
      })
    })
  }
  ...
  dispose(): void {
    ...
    for (const context of this.contexts.values()) {
      void context
        ...
        .catch((error: unknown) => console.error("[Neo New] NotebookBridge: context disposal failed:", error))
    }
    this.contexts.clear()
    this.origins.clear()
    this.outcomes.clear()
  ...
  private event(event: SSEPayload, directory?: string): void {
    if (event.type === "neocode.notebook.requested") {
      this.request(event as EventNeocodeNotebookRequested, directory)
      return
    }
    if (event.type === "neocode.notebook.cancelled") {
      this.cancel(event as EventNeocodeNotebookCancelled, directory)
    }
  }

  private request(event: EventNeocodeNotebookRequested, directory?: string): void {
  ...
  private start(request: NotebookRequest, origin: RequestOrigin): void {
    ...
    void this.run(request, origin, active).catch((error: unknown) => {
      console.error(`[Neo New] NotebookBridge: request ${request.id} failed:`, error)
    })
  }

  private cancel(event: EventNeocodeNotebookCancelled, directory?: string): void {
  ...
  private async reply(requestID: string, directory: string, result: NotebookResult): Promise<boolean> {
    try {
      const response = await this.connection.getClient().neocode.notebook.reply({ requestID, directory, result })
      if (!response.error) return true
      console.error(`[Neo New] NotebookBridge: reply ${requestID} failed:`, response.error)
      return false
    } catch (error) {
      console.error(`[Neo New] NotebookBridge: reply ${requestID} failed:`, error)
      return false
    }
  }
  ...
  private async reject(requestID: string, directory: string, error: NotebookFailure): Promise<boolean> {
    try {
      const response = await this.connection.getClient().neocode.notebook.reject({ requestID, directory, error })
      if (!response.error) return true
      console.error(`[Neo New] NotebookBridge: rejection ${requestID} failed:`, response.error)
      return false
    } catch (cause) {
      console.error(`[Neo New] NotebookBridge: rejection ${requestID} failed:`, cause)
      return false
    }
  }
  ...
  private async recover(revision: number): Promise<void> {
    ...
    for (const directory of this.connection.getKnownDirectories()) {
      try {
        const response = await client.neocode.notebook.list({ directory })
        if (this.disposed || revision !== this.revision) return
        if (response.error) {
          console.error(`[Neo New] NotebookBridge: could not list requests for ${directory}:`, response.error)
          continue
        }
        for (const request of response.data ?? []) {
          this.request({ id: request.id, type: "neocode.notebook.requested", properties: request }, directory)
        }
      } catch (error) {
        console.error(`[Neo New] NotebookBridge: could not list requests for ${directory}:`, error)
      }
    }
  }
...
export function createNotebookBridge(connection: NeoConnectionService): NotebookBridge {
