...
export class BrowserAutomationService implements vscode.Disposable {
  ...
  private static readonly MCP_SERVER_NAME = "neo-playwright"

  constructor(private readonly connectionService: NeoConnectionService) {
    // Listen for settings changes
    this.disposables.push(
      vscode.workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration("neo-code.new.browserAutomation")) {
  ...
  private async register(): Promise<void> {
    ...
    if (!client) {
      console.error("[Neo New] BrowserAutomationService: No SDK client available")
    ...
    try {
      ...
      if (serverStatus?.status === "connected") {
        ...
        console.error(
          "[Neo New] BrowserAutomationService: MCP server failed:",
      ...
      console.error("[Neo New] BrowserAutomationService: Failed to register MCP server:", error)
      this.setState("failed")
    }
  }
  ...
  private async unregister(): Promise<void> {
    ...
    if (client) {
      try {
        ...
        console.error("[Neo New] BrowserAutomationService: Failed to disconnect MCP server:", error)
      }
    }
  ...
  private getClient(): NeoClient | null {
    try {
  ...
  private setState(state: BrowserAutomationState): void {
    ...
    console.log(`[Neo New] BrowserAutomationService: State ${this.state} → ${state}`)
    this.state = state
  }
