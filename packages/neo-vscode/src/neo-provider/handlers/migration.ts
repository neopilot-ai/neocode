/**
 * Legacy migration handlers — extracted from NeoProvider.
 *
 * Manages the migration wizard for users upgrading from Neo Code v5.x.
...
export async function checkAndShowMigrationWizard(ctx: MigrationContext): Promise<void> {
  ...
  console.log("[Neo New] NeoProvider: 🔄 Legacy data detected, showing migration wizard")
...
async function startLegacyMigration(
  ...
  try {
    ...
    console.error("[Neo New] NeoProvider: ❌ Migration failed", error)
    ctx.postMessage({
      type: "migrationComplete",
      source: "legacy",
      operationId,
