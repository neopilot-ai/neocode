/**
 * Cloud session handlers — extracted from NeoProvider.
 *
...
import type { NeoClient, Session, TextPartInput, FilePartInput } from "@neocode/sdk/v2/client"
import type { CloudSessionData, EditorContext } from "../../services/cli-backend/types"
import { getErrorMessage, sessionToWebview, mapCloudSessionMessageToWebviewMessage } from "../../neo-provider-utils"
...
export async function handleRequestCloudSessions(
  ...
  try {
    const result = await ctx.client.neo.cloudSessions({
    ...
    console.error("[Neo New] NeoProvider: Failed to fetch cloud sessions:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleRequestCloudSessionData(ctx: CloudSessionContext, sessionId: string): Promise<void> {
  ...
  try {
    const result = await ctx.client.neo.cloud.session.get({ id: sessionId }, { signal: AbortSignal.timeout(TIMEOUT) })
    ...
    console.error("[Neo New] Failed to load cloud session data:", err)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId: sessionId,
...
export async function handleImportAndSend(
  ...
  try {
    const result = await ctx.client.neo.cloud.session.import(
      {
    ...
    console.error("[Neo New] NeoProvider: ❌ Cloud session import failed:", error)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId,
  ...
  try {
    ...
    console.error("[Neo New] Failed to send message after cloud import:", err)
    ctx.postMessage({
