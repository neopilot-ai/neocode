/**
 * Suggestion handlers — extracted from NeoProvider.
 *
 * Manages suggestion accept and dismiss flows plus recovery after SSE reconnects.
 * No vscode dependency.
 */

import type { NeoClient, SuggestionRequest } from "@neocode/sdk/v2/client"
...
export async function handleSuggestionAccept(
  ...
  try {
    ...
    console.error("[Neo New] NeoProvider: Failed to accept suggestion:", error)
    ctx.postMessage({ type: "suggestionError", requestID })
  }
}
...
export async function handleSuggestionDismiss(
  ...
  try {
    ...
    console.error("[Neo New] NeoProvider: Failed to dismiss suggestion:", error)
    ctx.postMessage({ type: "suggestionError", requestID })
  }
}
...
export async function fetchAndSendPendingSuggestions(ctx: SuggestionContext): Promise<void> {
  ...
  try {
    ...
    console.error("[Neo New] NeoProvider: Failed to fetch pending suggestions:", error)
  }
}
