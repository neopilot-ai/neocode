/**
 * Authentication handlers — extracted from NeoProvider.
 *
...
export interface AuthContext {
  readonly client: NeoClient | null
...
export async function handleLogin(ctx: AuthContext, attempt: number, getAttempt: () => number): Promise<void> {
  ...
  console.log("[Neo New] NeoProvider: 🔐 Starting login flow...")
  ...
  try {
    ...
    const { data: auth } = await ctx.client.provider.oauth.authorize(
      { providerID: "neo", method: 0, directory: dir },
      { throwOnError: true },
    )
    console.log("[Neo New] NeoProvider: 🔐 Got auth URL:", auth.url)
    ...
    await ctx.client.provider.oauth.callback({ providerID: "neo", method: 0, directory: dir }, { throwOnError: true })

    // Check if this attempt was cancelled
    if (attempt !== getAttempt()) return

    console.log("[Neo New] NeoProvider: 🔐 Login successful")

    await ctx.disposeGlobal()

    // Step 3: Fetch profile and push to webview
    const { data: profile } = await ctx.client.neo.profile(undefined, { throwOnError: true })
...
export async function handleLogout(ctx: AuthContext): Promise<void> {
  ...
  try {
    console.log("[Neo New] NeoProvider: 🚪 Logging out...")
    await ctx.client.auth.remove({ providerID: "neo" }, { throwOnError: true })
    console.log("[Neo New] NeoProvider: 🚪 Logged out successfully")
    ...
    console.error("[Neo New] NeoProvider: ❌ Logout failed:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleSetOrganization(ctx: AuthContext, organizationId: string | null): Promise<void> {
  ...
  console.log("[Neo New] NeoProvider: Switching organization:", organizationId ?? "personal")
  try {
    await ctx.client.neo.organization.set({ organizationId }, { throwOnError: true })
  } catch (error) {
    console.error("[Neo New] NeoProvider: Failed to switch organization:", error)
    // Re-fetch current profile to reset webview state — best-effort
    try {
      const result = await ctx.client.neo.profile()
      ctx.postMessage({ type: "profileData", data: result.data ?? null })
    } catch (profileError) {
      console.error("[Neo New] NeoProvider: Failed to refresh profile after org switch error:", profileError)
    }
    return
  }
  ...
  try {
    ...
    console.error("[Neo New] NeoProvider: Failed to refresh agents after org switch:", error)
  }
}
...
export async function handleRefreshProfile(ctx: AuthContext): Promise<void> {
  ...
  console.log("[Neo New] NeoProvider: 🔄 Refreshing profile...")
  const result = await ctx.client.neo.profile().catch(() => ({ data: null }))
  ctx.postMessage({ type: "profileData", data: result.data ?? null })
}
