/**
 * Question handlers — extracted from NeoProvider.
 *
...
import type { NeoClient, QuestionRequest } from "@neocode/sdk/v2/client"
  ...
  try {
    for (;;) {
      ...
      for (const dir of dirs) {
        ...
        if (error) {
          failed.add(dir)
          console.error(`[Neo New] NeoProvider: Failed to fetch pending questions for ${dir}:`, error)
          continue
        }
        scanned.add(dir)
        if (!data) continue
    ...
    console.error("[Neo New] NeoProvider: Failed to fetch pending questions:", error)
  }
}
...
export async function handleQuestionReply(
  ...
  try {
    ...
    console.error("[Neo New] NeoProvider: Failed to reply to question:", error)
    ctx.postMessage({ type: "questionError", requestID })
    return false
  }
}
...
export async function handleQuestionReject(
  ...
  try {
    ...
    if (isNotFoundError(error) && (await recover(ctx, requestID))) return false
    console.error("[Neo New] NeoProvider: Failed to reject question:", error)
    ctx.postMessage({ type: "questionError", requestID })
    return false
  }
}
