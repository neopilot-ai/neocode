...
const KEY = "neo.dismissedNotificationIds"
...
export async function fetchAndSendNotifications(ctx: NotificationsContext): Promise<void> {
  ...
  try {
    const { data: all } = await retry(() => ctx.client!.neo.notifications(undefined, { throwOnError: true }))
    ...
    console.error("[Neo New] NeoProvider: Failed to fetch notifications:", error)
  }
}
