...
async function apply(connection: NeoConnectionService, directory: string, style: WorkStyle) {
  const settings = vscode.workspace.getConfiguration("neo-code.new")
...
export async function handleWorkStyleApplyMessage(input: {
  ...
  if (input.message.style !== "human-in-the-loop" && input.message.style !== "autonomous") {
    console.error("[Neo New] Invalid style in applyWorkStyle message")
