...
import type { NeoClient, McpStatus } from "@neocode/sdk/v2/client"
  ...
  void vscode.env.openExternal(vscode.Uri.parse(url)).then(
    (opened) => {
      ...
      void vscode.window.showErrorMessage(
        "MCP sign-in failed to open the browser. Check the Neo logs for the authentication URL.",
      )
    },
    (error) => {
      console.error("[Neo New] Failed to open MCP OAuth URL:", error)
      void vscode.window.showErrorMessage(
        "MCP sign-in failed to open the browser. Check the Neo logs for the authentication URL.",
      )
    },
...
export async function connectMcpServer(
  ...
  try {
    ...
    console.error("[Neo New] Failed to connect MCP:", name, error)
    await refreshStatus()
  }
}
...
export async function disconnectMcpServer(
  ...
  try {
    ...
    console.error("[Neo New] Failed to disconnect MCP:", name, error)
    await refreshStatus()
  }
}
...
export async function authenticateMcpServer(
  ...
  try {
    ...
    console.error("[Neo New] Failed to authenticate MCP:", name, error)
