import type { NeoClient, SessionStatus } from "@neocode/sdk/v2/client"
...
export class SessionAbort {
  ...
  async stop(client: NeoClient, sessionID: string, fallback: string) {
    ...
    if (failures.length > 0) {
      console.error("[Neo New] NeoProvider: Failed to abort session in one or more directories:", failures)
      return false
    }
...
export async function abortSession(input: { client: NeoClient; sessionID: string; dir: string }) {
