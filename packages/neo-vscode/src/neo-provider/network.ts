 ...
 * user to press Enter; `neo run` auto-retries with backoff. This module implements auto-reply
 ...
 */
export function handleNetworkEvent(type: string, props: Props, client: NeoClient | null, dir: GetDir) {
  ...
  if (type === "session.network.restored" && props.requestID) {
    ...
    if (!entry) return
    console.log("[Neo New] network: auto-replying to restore", props.requestID)
