...
function getConfig() {
  return vscode.workspace.getConfiguration("neo-code.new")
}
...
export function watchWorkStyleConfig(post: (message: unknown) => void, next?: vscode.Disposable) {
  ...
  const watcher = vscode.workspace.onDidChangeConfiguration((event) => {
    if (keys.some((key) => event.affectsConfiguration(`neo-code.new.${key}`))) post(getWorkStylePayload())
  })
...
async function hasAnySession(connection: NeoConnectionService, directory: string): Promise<boolean> {
...
async function initializeWorkStyle(connection: NeoConnectionService, directory: string): Promise<void> {
...
export async function handleWorkStyleMessage(input: {
  ...
  if (input.message.type === "requestWorkStyle") {
    const initialized = await initializeWorkStyle(input.connection, input.directory)
      .then(() => true)
      .catch((err: unknown) => {
        console.error("[Neo New] Failed to initialize work style:", err)
        return false
      })
  ...
  if (!input.message.style) {
    console.error("[Neo New] Missing style in setWorkStyle message")
    return true
  }
