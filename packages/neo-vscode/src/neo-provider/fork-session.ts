import type { Session, SessionStatus } from "@neocode/sdk/v2/client"
...
export async function handleForkSession(ctx: ForkContext, sessionId: string, messageId?: string): Promise<void> {
  const status =
    ...
    (await Promise.resolve()
      ...
      .catch((e) => {
        console.error("[Neo New] refreshForkStatus failed:", e)
  ...
  await forkSession(
    {
      ...
      registerSession: () => {},
      log: (...args) => console.log("[Neo New] NeoProvider:", ...args),
    },
    sessionId,
    undefined,
    messageId,
