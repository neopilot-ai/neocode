...
} from "@neocode/neo-memory/commands"
import { MemorySchema } from "@neocode/neo-memory/schema"
import type { NeoClient, Session } from "@neocode/sdk/v2/client"
import { retry } from "../services/cli-backend/retry"
import { getErrorMessage } from "../neo-provider-utils"

type MemorySourceFile = MemorySchema.Source
type MemoryApi = NeoClient["memory"]
const CACHE_LIMIT = 8
const NO_PROJECT = "No active project for memory. Open a file in the target folder to manage its memory."

export type NeoProviderMemoryMessage = {
...
export type NeoProviderMemoryInput = {
...
function memory(client: NeoClient | undefined): MemoryApi | undefined {
  return (client as { memory?: MemoryApi } | undefined)?.memory
}

function request(input: Record<string, unknown>): { value: NeoProviderMemoryMessage } | { error: string } {
...
export class NeoProviderMemory {
  private readonly cached = new Map<string, unknown>()
  private tail = Promise.resolve()

  constructor(private readonly input: NeoProviderMemoryInput) {}
  ...
  async handle(message: Record<string, unknown>): Promise<boolean> {
    if (message.type === "requestMemory") {
      ...
      ).catch((err: unknown) => console.error("[Neo New] fetchAndSendMemory failed:", err))
      return true
    }
  ...
  private async load(sessionID?: string, includeSources = false): Promise<void> {
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to fetch memory:", err)
      this.input.post({
        type: "memoryLoaded",
        sessionID,
  ...
  private async doShow(sessionID?: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Neo New] NeoProvider: Failed to show memory:", err)
      this.input.post({
        type: "memoryLoaded",
        sessionID,
  ...
  run(message: NeoProviderMemoryMessage): Promise<boolean> {
  ...
  private async execute(message: NeoProviderMemoryMessage): Promise<boolean> {
    const client = this.input.client()
    if (!client) {
      this.input.post({
        type: "memoryOperationResult",
    ...
    try {
      ...
      const refreshed = await Promise.all([
        ...
        console.warn("[Neo New] Memory changed but refresh failed:", err)
        return undefined
      })
      ...
      console.error("[Neo New] NeoProvider: Failed memory operation:", err)
      this.input.post({
        type: "memoryOperationResult",
        operation: message.operation,
        sessionID: message.sessionID,
  ...
  private async action(api: MemoryApi, directory: string, message: NeoProviderMemoryMessage) {
  ...
  private async remember(api: MemoryApi, directory: string, message: NeoProviderMemoryMessage) {
  ...
  private async correct(api: MemoryApi, directory: string, message: NeoProviderMemoryMessage) {
    const text = message.text?.trim()
  ...
  private async forget(api: MemoryApi, directory: string, message: NeoProviderMemoryMessage) {
  ...
  private async purge(api: MemoryApi, directory: string, message: NeoProviderMemoryMessage) {
  ...
  private async auto(api: MemoryApi, directory: string, message: NeoProviderMemoryMessage) {
  ...
  private async verbose(api: MemoryApi, directory: string, message: NeoProviderMemoryMessage) {
    if (message.mode === "on" || message.mode === "off") {
