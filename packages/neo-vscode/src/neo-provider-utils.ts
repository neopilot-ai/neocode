import type { Session, Agent, Event, ProviderListResponse } from "@neocode/sdk/v2/client"
...
import type { PartBatch, PartUpdate } from "./neo-provider/session-stream-scheduler"
import type { PartRemove } from "./shared/stream-messages"
import * as path from "path"

export { SessionStreamScheduler } from "./neo-provider/session-stream-scheduler"
...
function safeStringify(value: unknown): string | undefined {
  try {
    ...
    console.warn("[Neo New] getErrorMessage: JSON.stringify failed", err)
  }
  return undefined
}
...
export async function runWithMessageConfirmation<T>(
  ...
  try {
    ...
    if (await state.wait(id)) {
      console.warn(`[Neo New] ${label} ended after server accepted it; ignoring transport error`, {
...
/**
 * Shared interface for the subset of NeoProvider state needed by session-refresh helpers.
 * Extracted here so the logic can be tested without importing NeoProvider (and vscode).
 */
  ...
  const extra = await Promise.all(
    [...worktreeDirs].map((dir) =>
      list(dir).catch((err: unknown) => {
        console.error(`[Neo] Failed to list sessions for ${dir}:`, err)
        failed.add(dir)
        return [] as Session[]
      }),
    ),
