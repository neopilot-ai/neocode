/**
 * Provider action handlers extracted from NeoProvider to stay under max-lines.
 * These are pure async functions that operate on the SDK client — no vscode dependency.
 */
import type { Config, NeoClient } from "@neocode/sdk/v2"
...
import { isCustomProviderPackage, NEO_AUTO, NEO_PROVIDER_ID, parseModelString } from "./shared/provider-model"
...
export async function fetchProviderData(client: NeoClient, dir: string) {
  const authRequest =
  ...
  delete authStates[NEO_PROVIDER_ID]
  if (neoAuth) authStates[NEO_PROVIDER_ID] = neoAuth
  ...
  return {
    ...
    disposeGlobal: async (reason: string) => {
      ...
      await client.global.dispose().catch((error: unknown) => {
        console.warn(`[Neo New] NeoProvider: global.dispose() after ${reason} failed:`, error)
      })
    },
...
async function removeAuth(ctx: ActionContext, id: string, configured: boolean) {
  try {
    ...
    console.warn(`[Neo New] auth.remove failed for configured provider ${id} (non-fatal):`, err)
  }
}
