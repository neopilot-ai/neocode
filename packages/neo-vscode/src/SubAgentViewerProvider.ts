 ...
 *
 * Uses a full NeoProvider so the viewer has backend connectivity
...
export class SubAgentViewerProvider implements vscode.Disposable {
  ...
  private providers = new Map<string, NeoProvider>()
  ...
  openPanel(sessionID: string, title?: string): void {
    ...
    const panel = vscode.window.createWebviewPanel("neo-code.new.SubAgentViewerPanel", label, vscode.ViewColumn.One, {
      enableScripts: true,
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage((msg) => {
      ...
      try {
        ...
        void client.session
          ...
          .catch((err: unknown) => {
            console.error("[Neo New] SubAgentViewerProvider: Failed to load session metadata:", err)
          })
      } catch (err) {
        console.error("[Neo New] SubAgentViewerProvider: Failed to load session metadata:", err)
      }
    ...
    panel.onDidDispose(() => {
      console.log("[Neo New] Sub-agent viewer panel disposed:", sessionID)
