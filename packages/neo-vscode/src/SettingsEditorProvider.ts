 ...
 *
 * Uses a full NeoProvider under the hood so each panel has
...
export class SettingsEditorProvider implements vscode.Disposable {
  ...
  private providers = new Map<PanelView, NeoProvider>()
  ...
  /** Extract the PanelView from a viewType string like "neo-code.new.settingsPanel". */
  static viewFromType(type: string): PanelView | undefined {
    const match = type.match(/^neo-code\.new\.(\w+)Panel$/)
  ...
  private wirePanel(panel: vscode.WebviewPanel, view: PanelView, projectDirectory: string | null): void {
    ...
    // Create a dedicated NeoProvider for this panel so it has full
    // backend connectivity (config, providers, agents, profile, auth).
    const provider = new NeoProvider(this.extensionUri, this.connectionService, this.context, {
      projectDirectory,
    })
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage((msg) => {
      if (msg.type === "webviewReady") {
        // Small delay to let NeoProvider's own webviewReady handler finish first
        setTimeout(() => {
    ...
    panel.onDidDispose(() => {
      console.log(`[Neo New] ${title} panel disposed`)
