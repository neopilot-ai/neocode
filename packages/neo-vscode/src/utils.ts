...
export function getWebviewFontSize(): number {
  const raw = vscode.workspace.getConfiguration("neo-code.new").get<number>("fontSize", 13)
  return clamp(raw)
}

function fontStyle(): string {
  const base = getWebviewFontSize()
  const vars = SIZES.map((size) => `--neo-font-size-${size}: ${(base * size) / 13}px;`).join("\n      ")
  return `:root {
      ${vars}
      --neo-font-scale: ${base / 13};
      --font-size-x-small: var(--neo-font-size-10);
      --font-size-small: var(--neo-font-size-11);
      --font-size-base: var(--neo-font-size-13);
      --font-size-large: var(--neo-font-size-16);
    }`
}
...
export function buildWebviewHtml(
  ...
  return `<!DOCTYPE html>
<html lang="en" data-theme="neo-vscode">
<head>
  <meta charset="UTF-8">
...
<body>
  <div id="root"></div>
  <script nonce="${nonce}">window.ICONS_BASE_URI = "${opts.iconsBaseUri}"; window.NEO_SHIKI_WORKER_URI = "${opts.workerUri}";</script>
