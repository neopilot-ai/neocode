...
const CONFIG = "neo-code.new.autoApprove"
const KEY = "enabled"
...
export function registerToggleAutoApprove(
      ...
      try {
        ...
        for (const req of pending) {
          ...
          await client.permission
            ...
            .catch((err) => {
              console.error("[Neo New] toggleAutoApprove: failed to drain pending:", err)
            })
        }
      } catch (err) {
        console.error("[Neo New] toggleAutoApprove: failed to list pending permissions:", err)
      }
    }
  ...
  const approve = async (event: Asked, directory?: string) => {
    ...
    return client.permission
      ...
      .then(
        () => true,
        (err) => {
          console.error("[Neo New] toggleAutoApprove: failed to auto-reply:", err)
          return false
        },
      )
  }
  ...
  context.subscriptions.push(vscode.commands.registerCommand("neo-code.new.toggleAutoApprove", toggle))
...
function tryGetClient(connectionService: NeoConnectionService): NeoClient | undefined {
  try {
