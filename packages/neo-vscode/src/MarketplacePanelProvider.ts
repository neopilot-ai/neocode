...
import type { GlobalEvent, SessionStatus } from "@neocode/sdk/v2/client"
import { buildWebviewHtml, getWebviewFontSize } from "./utils"
import { watchFontSizeConfig } from "./neo-provider/font-size"
import { mapSSEEventToWebviewMessage } from "./neo-provider-utils"
...
export class MarketplacePanelProvider implements vscode.Disposable {
  public static readonly viewType = "neo-code.new.marketplacePanel"
  ...
  private attach(panel: vscode.WebviewPanel, project: string | null): void {
    ...
    this.ready = false
    panel.iconPath = {
      light: vscode.Uri.joinPath(this.extensionUri, "assets", "icons", "neo-light.svg"),
      dark: vscode.Uri.joinPath(this.extensionUri, "assets", "icons", "neo-dark.svg"),
    }
  ...
  private async sync(reconcile: boolean): Promise<void> {
    ...
    try {
      ...
      console.warn("[Neo New] Marketplace session status sync failed:", err)
    }
  }
  ...
  private async handle(msg: MarketplaceMessage): Promise<void> {
    switch (msg.type) {
      ...
      case "dismissAgentMigrationBanner":
        await this.context.globalState.update("neo.agentMigrationBannerDismissed", true)
        return
      case "openExternal":
  ...
  private async fetchData(): Promise<void> {
    ...
    try {
      ...
      const dismissed = this.context.globalState.get<boolean>("neo.agentMigrationBannerDismissed") ?? false
      this.post({ type: "marketplaceData", ...data, showAgentMigrationBanner: !dismissed })
    } catch (err) {
      if (generation !== this.generation) return
      const error = err instanceof Error ? err.message : String(err)
      console.warn("[Neo New] Marketplace data fetch failed:", err)
      this.post({
  ...
  private post(msg: unknown): void {
    ...
    void this.panel.webview.postMessage(msg).then(undefined, (err) => {
      console.warn("[Neo New] Marketplace panel postMessage failed:", err)
    })
  }
