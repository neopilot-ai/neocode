/**
 * NeoClaw panel provider for the VS Code extension.
 *
 * Owns the Neo Chat HTTP client + event-service WebSocket connection
...
import type { NeoConnectionService } from "../services/cli-backend"
import type { NeoClient } from "@neocode/sdk/v2/client"
import { buildWebviewHtml } from "../utils"
import { watchFontSizeConfig } from "../neo-provider/font-size"
import { TokenManager } from "./token-manager"
import { NeoChatApiError, NeoChatClient } from "./neo-chat-client"
...
export class NeoClawProvider implements vscode.Disposable {
  static readonly viewType = "neo-code.new.NeoClawPanel"
  ...
  private chat: NeoChatClient | null = null
  ...
  constructor(
    ...
    private readonly connection: NeoConnectionService,
  ) {}
  ...
  openPanel(): void {
    ...
    const panel = vscode.window.createWebviewPanel(NeoClawProvider.viewType, "NeoClaw", vscode.ViewColumn.One, {
      enableScripts: true,
  ...
  private attach(panel: vscode.WebviewPanel): void {
    ...
    panel.iconPath = {
      light: vscode.Uri.joinPath(this.uri, "assets", "icons", "neo-light.svg"),
      dark: vscode.Uri.joinPath(this.uri, "assets", "icons", "neo-dark.svg"),
    }
    ...
    panel.webview.html = buildWebviewHtml(panel.webview, {
      scriptUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "dist", "neoclaw.js")),
      styleUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "dist", "neoclaw.css")),
    ...
    const msgSub = panel.webview.onDidReceiveMessage((msg: NeoClawInMessage) => this.onMessage(msg))
    ...
    const unsub = this.connection.onLanguageChanged((locale) => {
      this.post({ type: "neoclaw.locale", locale })
    })
  ...
  private post(msg: NeoClawOutMessage): void {
    this.panel?.webview.postMessage(msg)
  }

  private async onMessage(msg: NeoClawInMessage): Promise<void> {
    switch (msg.type) {
      case "neoclaw.ready":
        await this.init()
        return
      case "neoclaw.openExternal": {
  ...
  private get locale(): string {
    const override = vscode.workspace.getConfiguration("neo-code.new").get<string>("language")
  ...
  private async init(): Promise<void> {
    ...
    try {
      this.post({ type: "neoclaw.state", state: { phase: "loading", locale: this.locale } })
      ...
      this.post({ type: "neoclaw.state", state })
      this.startPolling()
      this.startBotNudge()
    } finally {
   ...
   */
  private async bootstrap(client: NeoClient, gen: number): Promise<boolean> {
    ...
    if (!this.sandboxId) {
      this.post({ type: "neoclaw.state", state: { phase: "noInstance", locale: this.locale } })
      return false
    }
  ...
  private async resolveStatus(client: NeoClient, gen: number): Promise<boolean> {
    const statusRes = await client.neo.claw.status().catch(() => null)
    if (this.stale(gen)) return false
  ...
  private async fetchChatToken(gen: number): Promise<ChatToken | null> {
    ...
    try {
      ...
      console.error("[Neo New] NeoClaw chat token fetch failed:", message)
      // Token fetch typically fails when the instance hasn't been upgraded
      // to support neo-chat — surface that as the upgrade prompt.
      this.post({ type: "neoclaw.state", state: { phase: "needsUpgrade", locale: this.locale } })
      return null
    }
  }
  ...
  private async openChatStream(envelope: ChatToken, gen: number): Promise<boolean> {
    ...
    const events = new EventServiceClient({
      ...
      onUnauthorized: () => {
        tokens.clear()
        this.post({ type: "neoclaw.error", error: "Authentication expired" })
      },
    })
    this.events = events

    const chat = new NeoChatClient({
      baseUrl: envelope.neoChatUrl,
      getToken: () => tokens.get(),
      onUnauthorized: () => {
        tokens.clear()
        this.post({ type: "neoclaw.error", error: "Authentication expired" })
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.error("[Neo New] NeoClaw event-service connect failed:", message)
      this.post({
        type: "neoclaw.state",
  ...
  private async loadInitialSnapshots(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Neo New] NeoClaw listConversations failed:", message)
    }
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Neo New] NeoClaw getBotStatus failed:", message)
    }
    ...
    if (!this.activeConversationId && this.conversations.length > 0) {
      ...
      try {
        ...
        const message = err instanceof Error ? err.message : String(err)
        console.warn("[Neo New] NeoClaw getConversationStatus failed:", message)
      }
  ...
  private async resolveClient(): Promise<NeoClient | null> {
    if (this.connection.getConnectionState() !== "connected") {
      try {
        ...
        console.debug("[Neo New] NeoClaw connect deferred:", (err as Error)?.message ?? err)
        return null
      }
    }
    try {
      ...
      console.debug("[Neo New] NeoClaw getClient deferred:", (err as Error)?.message ?? err)
      return null
    }
  }
  ...
  private subscribeSandboxContext(): void {
    ...
    const ctx = `/neoclaw/${this.sandboxId}`
  ...
  private subscribeConversationContext(conversationId: string): void {
    ...
    const ctx = `/neoclaw/${this.sandboxId}/${conversationId}`
    this.events.subscribe([ctx])
  ...
  private attachEventHandlers(events: EventServiceClient): void {
    ...
    this.chatSubs.push(
      events.on("conversation.created", (ctx, e: ConversationCreatedEvent) => {
        if (!this.sandboxId || ctx !== `/neoclaw/${this.sandboxId}`) return
    ...
    this.chatSubs.push(
      events.on("conversation.left", (ctx, e: ConversationLeftEvent) => {
        ...
        if (this.activeConversationId === e.conversationId) {
          ...
          this.post({ type: "neoclaw.activeConversation", conversationId: null })
          this.post({
            type: "neoclaw.messages",
            conversationId: e.conversationId,
            messages: [],
            hasMore: false,
            replace: true,
          })
          this.post({ type: "neoclaw.conversationStatus", status: null })
        }
        this.broadcastConversations({ replace: true })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("bot.status", (ctx, e: BotStatusEvent) => {
        ...
        this.post({ type: "neoclaw.botStatus", status: this.botStatus })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("action.delivery_failed", (ctx, e: ActionDeliveryFailedEvent) => {
        ...
        this.broadcastMessages({ replace: true })
        this.post({ type: "neoclaw.error", error: "Couldn't reach the bot — please try again" })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("conversation.status", (ctx, e: ConversationStatusEvent) => {
        ...
        this.post({ type: "neoclaw.conversationStatus", status: this.conversationStatus })
      }),
    )
  }
  ...
  private async selectConversation(conversationId: string): Promise<void> {
    ...
    this.post({ type: "neoclaw.activeConversation", conversationId })
    this.typingMembers = []
  ...
  private async createConversation(title?: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "neoclaw.error", error: this.formatError(err, "Failed to create conversation") })
    }
  }
  ...
  private async renameConversation(conversationId: string, title: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "neoclaw.error", error: this.formatError(err, "Failed to rename conversation") })
      void this.refreshConversations()
    }
  }
  ...
  private async leaveConversation(conversationId: string): Promise<void> {
    ...
    try {
      ...
      if (this.activeConversationId === conversationId) {
        ...
        this.messages = []
        this.post({ type: "neoclaw.activeConversation", conversationId: null })
        this.post({ type: "neoclaw.messages", conversationId, messages: [], hasMore: false, replace: true })
      }
      this.broadcastConversations({ replace: true })
    } catch (err) {
      this.post({ type: "neoclaw.error", error: this.formatError(err, "Failed to leave conversation") })
    }
  }
  ...
  private async loadMoreConversations(): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "neoclaw.error", error: this.formatError(err, "Failed to load conversations") })
    }
  }
  ...
  private async sendMessage(
    ...
    // neo-chat validates clientId as a ULID (Crockford Base32); generate
    ...
    if (conversationId === this.activeConversationId) {
      ...
      this.post({ type: "neoclaw.messageOptimistic", conversationId, message: optimistic })
    }
    ...
    try {
      ...
      console.error("[Neo New] NeoClaw sendMessage failed:", err instanceof Error ? err.message : err)
      this.post({ type: "neoclaw.error", error: this.formatError(err, "Failed to send message") })
      if (conversationId === this.activeConversationId) {
        this.messages = this.messages.filter((m) => m.id !== pendingId)
        this.post({ type: "neoclaw.messageRemoved", conversationId, messageId: pendingId })
      }
    }
  }
  ...
  private async editMessage(conversationId: string, messageId: string, content: ContentBlock[]): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "neoclaw.error", error: this.formatError(err, "Failed to edit message") })
      if (snapshot && conversationId === this.activeConversationId) {
  ...
  private async deleteMessage(conversationId: string, messageId: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "neoclaw.error", error: this.formatError(err, "Failed to delete message") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async loadMoreMessages(conversationId: string, before: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "neoclaw.error", error: this.formatError(err, "Failed to load messages") })
    }
  }
  ...
  private async addReaction(conversationId: string, messageId: string, emoji: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "neoclaw.error", error: this.formatError(err, "Failed to add reaction") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async removeReaction(conversationId: string, messageId: string, emoji: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "neoclaw.error", error: this.formatError(err, "Failed to remove reaction") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async executeAction(
    ...
    try {
      ...
      this.post({ type: "neoclaw.error", error: this.formatError(err, "Failed to execute action") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async refreshConversations(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Neo New] NeoClaw refreshConversations failed:", message)
    }
  }
  ...
  private async refreshActiveMessages(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Neo New] NeoClaw refreshActiveMessages failed:", message)
    }
  }
  ...
  private upsertTypingMember(memberId: string): void {
    ...
    if (this.activeConversationId) {
      this.post({ type: "neoclaw.typing", conversationId: this.activeConversationId, memberId })
    }
  ...
  private removeTypingMember(memberId: string): void {
    ...
    if (this.activeConversationId) {
      this.post({ type: "neoclaw.typingStop", conversationId: this.activeConversationId, memberId })
    }
  }
  ...
  private formatError(err: unknown, fallback: string): string {
    if (err instanceof NeoChatApiError) {
  ...
  private startBotNudge(): void {
    ...
    this.botNudge = setInterval(() => {
      ...
      this.chat.requestBotStatus(this.sandboxId).catch((err) => {
        console.debug("[Neo New] NeoClaw requestBotStatus failed:", (err as Error)?.message ?? err)
      })
    }, BOT_STATUS_NUDGE_MS)
  }
  ...
  private async poll(): Promise<void> {
    try {
      ...
      const res = await client.neo.claw.status()
      if (res?.data) {
        this.status = res.data as ClawStatus
        this.post({ type: "neoclaw.status", data: this.status })
      }
    } catch (err) {
      console.debug("[Neo New] NeoClaw poll failed:", (err as Error)?.message ?? err)
    }
  }
