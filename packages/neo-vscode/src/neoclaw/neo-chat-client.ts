/**
 * HTTP client for the neo-chat Cloudflare Worker.
 *
 * Minimal inline port of `@neocode/neo-chat/client` (cloud monorepo) tailored
 ...
 * the neo-chat worker is the source of truth and validates at its edge.
 */
...
export type NeoChatClientConfig = {
  baseUrl: string
  getToken: () => Promise<string>
  onUnauthorized?: () => void
}

export class NeoChatApiError extends Error {
  constructor(
    public readonly status: number,
    public readonly body: unknown,
  ) {
    super(`NeoChat request failed: ${status}${formatBodyDetail(body)}`)
    this.name = "NeoChatApiError"
  }
}
...
export class NeoChatClient {
  ...
  constructor(config: NeoChatClientConfig) {
  ...
  private async request<T>(path: string, opts: HttpOpts = {}): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new NeoChatApiError(res.status, body)
    }
