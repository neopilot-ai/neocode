/**
 * NeoClaw VS Code extension message types.
 *
 * Defines the postMessage protocol between the extension host (Node.js)
 * and the NeoClaw webview (SolidJS). The extension host owns all network
 * connections (Neo Chat HTTP + event-service WebSocket) and relays data
 * to the webview.
 *
 * SYNC: Shared types are mirrored in webview-ui/neoclaw/lib/types.ts —
 * keep both in sync.
 */

// ── Instance status (NeoClaw worker) ───────────────────────────────
...
export type ClawStatus = {
  ...
  // restore (cloud: `services/neoclaw/src/index.ts`).
  status:
    | "provisioned"
    | "starting"
    | "restarting"
...
// ── Neo Chat token envelope (gateway response) ─────────────────────
...
// ── Neo Chat content blocks ────────────────────────────────────────
// Mirrors `@neocode/neo-chat` schemas. See cloud/packages/neo-chat/src/schemas.ts.
...
// ── Typed Neo Chat events (server → client) ───────────────────────
// Event names mirror `@neocode/neo-chat/events`.
...
export type NeoChatEventName = keyof NeoChatEventMap
...
export type NeoClawInMessage =
  | { type: "neoclaw.ready" }
  | { type: "neoclaw.openExternal"; url: string }
  | { type: "neoclaw.selectConversation"; conversationId: string }
  | { type: "neoclaw.createConversation"; title?: string }
  | { type: "neoclaw.renameConversation"; conversationId: string; title: string }
  | { type: "neoclaw.leaveConversation"; conversationId: string }
  | { type: "neoclaw.loadMoreConversations" }
  | {
  ...
  | { type: "neoclaw.editMessage"; conversationId: string; messageId: string; content: ContentBlock[] }
  | { type: "neoclaw.deleteMessage"; conversationId: string; messageId: string }
  | { type: "neoclaw.loadMoreMessages"; conversationId: string; before: string }
  | { type: "neoclaw.addReaction"; conversationId: string; messageId: string; emoji: string }
  | { type: "neoclaw.removeReaction"; conversationId: string; messageId: string; emoji: string }
  | {
      type: "neoclaw.executeAction"
      conversationId: string
      messageId: string
      groupId: string
      value: ExecApprovalDecision
    }
  | { type: "neoclaw.sendTyping"; conversationId: string }
  | { type: "neoclaw.sendTypingStop"; conversationId: string }
  | { type: "neoclaw.markRead"; conversationId: string }

// ── Messages: Extension Host → Webview ──────────────────────────────

export type NeoClawOutMessage =
  | { type: "neoclaw.state"; state: NeoClawState }
  | { type: "neoclaw.status"; data: ClawStatus | null }
  | { type: "neoclaw.locale"; locale: string }
  | { type: "neoclaw.error"; error: string }
  | { type: "neoclaw.conversations"; conversations: ConversationListItem[]; hasMore: boolean; replace: boolean }
  | { type: "neoclaw.activeConversation"; conversationId: string | null }
  | { type: "neoclaw.messages"; conversationId: string; messages: Message[]; hasMore: boolean; replace: boolean }
  | { type: "neoclaw.messageOptimistic"; conversationId: string; message: Message }
  | { type: "neoclaw.messageReplaced"; conversationId: string; pendingId: string; message: Message }
  | { type: "neoclaw.messageRemoved"; conversationId: string; messageId: string }
  | { type: "neoclaw.botStatus"; status: BotStatusRecord | null }
  | { type: "neoclaw.conversationStatus"; status: ConversationStatusRecord | null }
  | { type: "neoclaw.typing"; conversationId: string; memberId: string }
  | { type: "neoclaw.typingStop"; conversationId: string; memberId: string }
