 ...
 *
 * Minimal inline port of `@neocode/event-service` (cloud monorepo). Connects
 * to the neo events Cloudflare Worker using a two-step ticket flow:
...
import type { NeoChatEventMap, NeoChatEventName } from "./types"

const WS_SUBPROTOCOL = "neo.events.v1"
...
export class EventServiceClient {
  ...
  on<N extends NeoChatEventName>(event: N, handler: (ctx: string, payload: NeoChatEventMap[N]) => void): () => void {
    ...
    // neo-chat worker edge before broadcast.
    const wrapped: EventHandler = (ctx, payload) => handler(ctx, payload as NeoChatEventMap[N])
    set.add(wrapped)
  ...
  private handleMessage(data: string): void {
    ...
    if (m.type === "error") {
      console.warn("[Neo New] event-service server error", m)
    }
  }
