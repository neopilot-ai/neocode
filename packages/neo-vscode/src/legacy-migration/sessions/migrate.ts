...
export async function migrate(
  ...
  try {
    ...
    const project = await client.neocode.sessionImport.project(payload.project, { throwOnError: true })
    const projectID = project.data?.id ?? payload.project.id
    const session = await client.neocode.sessionImport.session(
      {
        ...payload.session,
        projectID,
    ...
    for (const msg of payload.messages) {
      await client.neocode.sessionImport.message(msg, { throwOnError: true })
    }

    for (const part of payload.parts) {
      await client.neocode.sessionImport.part(part, { throwOnError: true })
    }
