 ...
 *
 * Reads legacy Neo Code v5.x data from VS Code SecretStorage and the extension's
...
const MIGRATION_STATUS_KEY = "neo.legacyMigrationStatus"
...
export async function clearLegacyData(context: vscode.ExtensionContext): Promise<void> {
  ...
  const legacyStateKeys = [
    "neo-code.allowedCommands",
    "neo-code.deniedCommands",
    "neo-code.autoApprovalEnabled",
    "neo-code.fuzzyMatchThreshold",
    "neo-code.diffEnabled",
    "neo-code.language",
    "neo-code.customModes",
    "neo-code.firstInstallCompleted",
    "neo-code.telemetrySetting",
  ...
  // Clear legacy VS Code settings registered under the "neo-code" configuration scope.
  ...
  const cfg = vscode.workspace.getConfiguration("neo-code")
...
async function migrateProvider(
  ...
  // The profile endpoint requires type:"oauth". The legacy extension stored the same Neo
  ...
  // For providers that support an organization ID (e.g. Neo Gateway), migrate using OAuth
...
async function migrateDefaultModel(settings: LegacyProviderSettings, client: NeoClient): Promise<MigrationResultItem> {
  const provider = settings.apiProvider
  if (!provider) {
...
async function migrateAutocomplete(settings: LegacyAutocompleteSettings): Promise<MigrationResultItem> {
  try {
    const config = vscode.workspace.getConfiguration("neo-code.new.autocomplete")
...
function readLegacySettings(context: vscode.ExtensionContext): LegacySettings {
  ...
  return {
    autoApprovalEnabled: context.globalState.get<boolean>("neo-code.autoApprovalEnabled"),
    allowedCommands: context.globalState.get<string[]>("neo-code.allowedCommands"),
    deniedCommands: context.globalState.get<string[]>("neo-code.deniedCommands"),
    ...
    language: context.globalState.get<string>("neo-code.language"),
