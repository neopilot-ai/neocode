...
const { neocodeActionProvider } = await import("../../src/services/code-actions/code-action-provider")

const provider = new neocodeActionProvider()
...
describe("neocodeActionProvider", () => {
  describe("provideCodeActions", () => {
    ...
    describe("non-empty range, no diagnostics", () => {
      it("returns Add, Explain, Improve actions", () => {
        ...
        expect(titles).toContain("Add to Neo Code")
        expect(titles).toContain("Explain with Neo Code")
        expect(titles).toContain("Improve with Neo Code")
      })

      it("does not include Fix action", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(0) as never)
        expect(result.map((a) => a.title)).not.toContain("Fix with Neo Code")
      })
      ...
      it("uses correct command IDs", () => {
        ...
        expect(commands).toContain("neo-code.new.addToContext")
        expect(commands).toContain("neo-code.new.explainCode")
        expect(commands).toContain("neo-code.new.improveCode")
      })
    ...
    describe("non-empty range, with diagnostics", () => {
      it("returns Add and Fix actions", () => {
        ...
        const titles = result.map((a) => a.title)
        expect(titles).toContain("Add to Neo Code")
        expect(titles).toContain("Fix with Neo Code")
      })
      ...
      it("does not include Explain or Improve actions", () => {
        ...
        const titles = result.map((a) => a.title)
        expect(titles).not.toContain("Explain with Neo Code")
        expect(titles).not.toContain("Improve with Neo Code")
      })
      ...
      it("Fix action is preferred", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(1) as never)
        const fix = result.find((a) => a.title === "Fix with Neo Code")
      ...
      it("uses correct Fix command ID", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(1) as never)
        const fix = result.find((a) => a.title === "Fix with Neo Code")
        expect(fix?.command?.command).toBe("neo-code.new.fixCode")
      })
    })
  })
})
