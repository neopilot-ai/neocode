 ...
 *
 * Static analysis — reads NeoProvider.ts, AgentManagerProvider.ts,
 ...
 *
 * Protects against accidental removal during Neo development.
 */
...
const NEOPROVIDER_FILE = path.join(ROOT, "src/NeoProvider.ts")
...
describe("NeoProvider editor-panel visible registration contract", () => {
  const source = readFile(NEOPROVIDER_FILE)
...
describe("NeoProvider disableViewedRegistration contract", () => {
  const neoProvider = readFile(NEOPROVIDER_FILE)
  const vscodeHost = readFile(VSCODE_HOST_FILE)

  it("registerPresence skips viewed registration when the option is set", () => {
    const match = neoProvider.match(/private registerPresence\(\): void \{([\s\S]*?)\n {2}\}/)
    expect(match).not.toBeNull()
    const body = match![1]
  ...
  it("focusSession and trackOpenSessions report through registerPresence", () => {
    ...
    const focus = neoProvider.match(/private focusSession\(id\?: string\): void \{([\s\S]*?)\n {2}\}/)
    expect(focus).not.toBeNull()
    expect(focus![1]).toContain("this.registerPresence()")
    const track = neoProvider.match(/private trackOpenSessions\(ids: string\[\]\): void \{([\s\S]*?)\n {2}\}/)
  ...
  it("embedded Agent Manager providers disable generic viewed registration", () => {
    // Each Agent Manager panel hosts a full NeoProvider; the "agent-manager"
...
describe("NeoConnectionService connection snapshot contract", () => {
