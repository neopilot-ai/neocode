...
const NEO_AUTO: ModelSelection = { providerID: "neo", modelID: "neo-auto/free" }

const providers: Record<string, Provider> = {
  neo: makeProvider("neo", ["neo-auto/free"]),
...
function env(): ResolveEnv {
  return {
    providers,
    connected: ["neo", "anthropic", "openai"],
    fallback: NEO_AUTO,
    getModeModel: () => null,
    getGlobalModel: () => null,
  }
...
describe("per-session model selection", () => {
  it("selecting a model in session A does not write per-mode globally", () => {
    ...
    expect(sessionB).toEqual(NEO_AUTO)
  })
...
describe("per-mode model memory", () => {
  ...
  it("switching modes falls back to default after session override is cleared", () => {
    ...
    expect(getSelected(cleared, e, "session-a", "code")).toEqual(NEO_AUTO)
  })
