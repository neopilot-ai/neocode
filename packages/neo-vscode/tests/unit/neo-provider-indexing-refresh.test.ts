...
import type { Config } from "@neocode/sdk/v2/client"

// vscode mock is provided by the shared preload (tests/setup/vscode-mock.ts)
const { NeoProvider } = await import("../../src/NeoProvider")
...
describe("NeoProvider indexing refresh", () => {
  it("reloadAfterAuthChange fetches config first, then indexing status", async () => {
    const provider = new NeoProvider({} as never, {} as never)
  ...
  it("handleUpdateConfig no longer eagerly fetches indexing status", async () => {
    ...
    const provider = new NeoProvider({} as never, conn.service as never)
    const internal = provider as unknown as Internals
  ...
  it("fetchAndSendIndexingStatus uses current session directory header", async () => {
    const worktree = "/repo/.neo/.neocode/worktrees/feature"
    ...
    try {
      const provider = new NeoProvider(
        {} as never,
        {
          getClient: () => ({}) as never,
      ...
      const auth = Buffer.from("neo:secret").toString("base64")
      expect(headers.get("Authorization")).toBe(`Basic ${auth}`)
      expect(headers.get("x-neo-directory")).toBe(worktree)
    } finally {
