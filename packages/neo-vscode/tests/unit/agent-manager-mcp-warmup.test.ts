...
describe("Agent Manager MCP warmup", () => {
  it("starts MCP status for the worktree directory before session creation", async () => {
    ...
    expect(calls[0]).toEqual(["warm", { directory: "/repo/.neo/worktrees/feature" }, { throwOnError: true }])
  ...
  it("logs and contains MCP warmup failures", async () => {
    ...
    expect(result).toBe("created")
    expect(logs[0]).toEqual(["[MCPWarmup] Starting for /repo/.neo/worktrees/feature"])
    expect(logs[1]?.[0]).toBe("[MCPWarmup] Failed for /repo/.neo/worktrees/feature:")
