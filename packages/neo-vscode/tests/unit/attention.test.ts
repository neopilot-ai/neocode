...
import type { TuiAttentionSoundName } from "@neocode/plugin/tui"
...
describe("attention defaults", () => {
  it("keeps attention sounds opt-in", async () => {
    ...
    expect(properties["neo-code.new.attention.enabled"]?.default).toBe(false)
    expect(properties["neo-code.new.attention.sound"]?.default).toBe("default")
    expect(properties["neo-code.new.attention.sound"]?.enum).toEqual(["default", "system", ...CustomSoundIDs])
    expect(properties["neo-code.new.sounds.agentEnabled"]).toBeUndefined()
    expect(properties["neo-code.new.sounds.permissionsEnabled"]).toBeUndefined()
    expect(properties["neo-code.new.sounds.errorsEnabled"]).toBeUndefined()
  })
