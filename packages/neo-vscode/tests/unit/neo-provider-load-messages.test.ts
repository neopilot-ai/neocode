...
const { NeoProvider, unwrapSyncEvent } = await import("../../src/NeoProvider")
...
function makeProvider(client: ReturnType<typeof createClient>) {
  ...
  const provider = new NeoProvider({} as never, connection as never)
...
describe("NeoProvider.handleAbort", () => {
...
describe("NeoProvider sandbox status", () => {
...
describe("NeoProvider sandbox toggle", () => {
  ...
  it("resolves a blank worktree default against the routed directory", async () => {
    ...
    await internal.fetchAndSendSandboxDefault("/repo/.neo/worktrees/wt-1")

    expect(client.configReads).toEqual([{ directory: "/repo/.neo/worktrees/wt-1" }])
    expect(client.sandboxSupport).toEqual([{ directory: "/repo/.neo/worktrees/wt-1" }])
  })
  ...
  it("waits for a blank toggle before creating the first prompt session", async () => {
    ...
    expect(client.created).toEqual([
      expect.objectContaining({ metadata: { "neocode.sandbox": { enabled: true, version: 0 } } }),
    ])
  ...
  it("snapshots the remembered default before sending the first prompt", async () => {
    ...
    expect(client.created).toEqual([
      expect.objectContaining({
        directory: "/repo",
        metadata: { "neocode.sandbox": { enabled: true, version: 0 } },
      }),
    ])
    expect(client.sandboxed).toHaveLength(0)
    expect(client.prompted).toHaveLength(1)
...
describe("NeoProvider sidebar tabs", () => {
...
describe("NeoProvider revert ordering", () => {
...
describe("NeoProvider.handleLoadMessages / focus mode freshness", () => {
...
describe("NeoProvider.handleDeleteSession / background processes", () => {
...
describe("NeoProvider.handleLoadMessages / slim payload", () => {
...
describe("NeoProvider.loadMessages / sub-agent viewer", () => {
...
describe("NeoProvider.handleLoadMessages / prepend into deleted session", () => {
