...
async function createTempWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "neo-ignore-"))
  tempDirs.push(dir)
  return dir
}

describe("FileIgnoreController", () => {
  describe("when .neocodeignore exists", () => {
    it("applies only .neocodeignore patterns", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".neocodeignore"), "secret/**\n*.snap\n")
    ...
    it("does NOT block .env files unless explicitly listed", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".neocodeignore"), "dist/\n")
    ...
    it("does NOT apply .gitignore patterns", async () => {
      ...
      // .neocodeignore pattern should apply
      expect(controller.validateAccess(path.join(workspace, "dist", "bundle.js"))).toBe(false)
    })
  })

  describe("when no .neocodeignore exists (fallback)", () => {
  ...
  describe("Windows cross-drive paths", () => {
    it("does not throw for a Windows-style absolute path from another drive", async () => {
      ...
      const cross = "c:/Users/User/AppData/Roaming/Code/User/globalStorage/neocode.neo-code/settings/mcp_settings.json"
    ...
    it("does not throw for file:// URIs with Windows drive letters", async () => {
      ...
      const uri =
        "file:///c:/Users/User/AppData/Roaming/Code/User/globalStorage/neocode.neo-code/settings/mcp_settings.json"
