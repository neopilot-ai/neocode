...
const neo = fs.readFileSync(path.join(root, "src/NeoProvider.ts"), "utf-8")
const panel = fs.readFileSync(path.join(root, "src/MarketplacePanelProvider.ts"), "utf-8")
const remove = fs.readFileSync(path.join(root, "src/neo-provider/remove-config-item.ts"), "utf-8")

describe("standalone Marketplace architecture", () => {
  it("keeps Marketplace webview cases out of NeoProvider", () => {
    for (const type of [
      ...
      expect(neo).not.toContain(`case \"${type}\"`)
  ...
  it("keeps sidebar removal behind a narrow adapter", () => {
    expect(neo).toContain("removeMcp(this.removeConfigItemCtx, name)")
