...
describe("WorktreeStateManager", () => {
  ...
  beforeEach(() => {
    ...
    // Pre-create .neo dir so fire-and-forget saves don't race on mkdir
    fs.mkdirSync(path.join(root, ".neo"), { recursive: true })
    logs.length = 0
  ...
  describe("session CRUD", () => {
    ...
    it("drops obsolete session prefs while loading state", async () => {
      const file = path.join(root, ".neo", "agent-manager.json")
      fs.writeFileSync(
        file,
        JSON.stringify({
          worktrees: {},
  ...
  describe("persistence", () => {
    ...
    it("creates .neo directory if missing", async () => {
      ...
      expect(fs.existsSync(path.join(fresh, ".neo", "agent-manager.json"))).toBe(true)
    })
    ...
    it("backs up a corrupt state file before recovery saves", async () => {
      ...
      const files = fs.readdirSync(path.join(root, ".neo"))
      expect(result.status).toBe("failed")
  ...
  describe("tab order", () => {
    ...
    it("does not persist empty tab order", async () => {
      ...
      const content = fs.readFileSync(path.join(root, ".neo", "agent-manager.json"), "utf-8")
  ...
  describe("validate", () => {
    ...
    it("resolves relative paths against root", async () => {
      const relative = ".neo/worktrees/test-branch"
  ...
  describe("legacy .neocode migration", () => {
    it("migrates and loads state from .neocode when .neo is absent", async () => {
      // Remove the .neo dir created in beforeEach
      fs.rmSync(path.join(root, ".neo"), { recursive: true, force: true })

      // Write state to legacy .neocode dir (migration will move it to .neo)
      const legacyDir = path.join(root, ".neocode")
    ...
    it("skips migration when .neo state already exists", async () => {
      // .neo state already present — migration should skip agent-manager.json
      fs.writeFileSync(
        path.join(root, ".neo", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-new": {
              branch: "new-branch",
    ...
    it("rewrites stale .neocode paths in worktree entries (unix)", async () => {
      fs.writeFileSync(
        path.join(root, ".neo", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-stale": {
              branch: "fix",
              path: "/repo/.neocode/worktrees/fix",
              parentBranch: "main",
              createdAt: new Date().toISOString(),
            },
          },
      ...
      expect(manager.getWorktrees()[0].path).toBe(`/repo/.neo/worktrees/fix`)
    })

    it("rewrites stale .neocode paths with backslashes (windows)", async () => {
      fs.writeFileSync(
        path.join(root, ".neo", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
      ...
      expect(manager.getWorktrees()[0].path).toBe("C:\\.neo\\worktrees\\fix")
    })
  })
})
