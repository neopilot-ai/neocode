...
describe("extension host i18n", () => {
  it("returns translated string for known key", () => {
    const result = t("neocode:autocomplete.statusBar.enabled")
    expect(typeof result).toBe("string")
    expect(result.length).toBeGreaterThan(0)
    expect(result).not.toBe("neocode:autocomplete.statusBar.enabled")
  })
  ...
  it("interpolates a single variable", () => {
    const result = t("neocode:autocomplete.statusBar.tooltip.noUsableProvider", {
      providers: "OpenAI, Anthropic",
      command: "command:neo-code.new.settingsButtonClicked",
    })
  ...
  it("interpolates multiple variables", () => {
    const result = t("neocode:autocomplete.statusBar.tooltip.completionSummary", {
      count: "5",
      startTime: "10:00",
      endTime: "11:00",
      cost: "$0.05",
  ...
  it("leaves unreferenced vars intact in template", () => {
    const key = "neocode:autocomplete.statusBar.tooltip.noUsableProvider"
  ...
  it("prefers Neo new language setting over VS Code language", () => {
    const vscode = {
      env: { language: "en" },
      workspace: {
        getConfiguration: (section: string) => ({
          get: () => (section === "neo-code.new" ? "de" : undefined),
        }),
      },
  ...
  it("uses VS Code language when Neo language setting is automatic", () => {
    const vscode = {
      env: { language: "nl" },
      workspace: {
        getConfiguration: () => ({
  ...
  it("translates status bar tooltip copy for German", () => {
    const text = translate("de", "neocode:autocomplete.statusBar.tooltip.completionSummary", {
      count: 1,
      startTime: "12:25:24",
      endTime: "12:25:26",
      cost: "$0.00",
