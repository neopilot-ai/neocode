 ...
 *
 * Neo's VS Code webviews use the `neo-code.new.fontSize` setting, not
 * VS Code editor font-size or raw pixel declarations. This keeps the Neo UI
 * independently scalable across sidebar, settings, Agent Manager, NeoClaw,
 * diff viewers, code blocks, and shared neo-ui controls.
 */
...
describe("webview font-size architecture", () => {
  it("does not use raw font-size pixels or VS Code editor font-size variables in runtime webview UI", () => {
    ...
    expect(
      violations,
      `Use the Neo webview font-size tokens instead of raw pixels or VS Code editor font-size variables.\n` +
        `Preferred tokens: var(--font-size-base), var(--font-size-small), or var(--neo-font-size-N).\n\n` +
  ...
  it("uses scalable line heights in polished tool previews", () => {
    const files = [
      path.join(REPO, "packages/neo-ui/src/components/basic-tool.css"),
      path.join(REPO, "packages/neo-ui/src/components/message-part.css"),
    ]
  ...
  it("injects and live-broadcasts the webview font-size setting to all webview providers", () => {
    ...
    expect(util, "buildWebviewHtml must define scaled Neo font tokens").toContain("--neo-font-size-")

    const missing = WATCHED_PROVIDERS.filter((file) => !fs.readFileSync(file, "utf-8").includes("watchFontSizeConfig"))
    expect(
      missing.map(rel),
      `Webview providers that stay open must broadcast fontSizeChanged when neo-code.new.fontSize changes.\n` +
