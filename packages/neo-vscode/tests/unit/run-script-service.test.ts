...
describe("RunScriptService", () => {
  ...
  it("builds commands for shell, PowerShell, and CMD scripts", () => {
    ...
    expect(buildRunTaskCommand({ path: "C:\\repo\\.neo\\run-script.ps1", kind: "powershell" })).toEqual({
      command: "powershell.exe",
      args: ["-NoLogo", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "C:\\repo\\.neo\\run-script.ps1"],
    })
    expect(buildRunTaskCommand({ path: "C:\\repo path\\.neo\\run-script.cmd", kind: "cmd" })).toEqual({
      command: "cmd.exe",
      args: ["/d", "/s", "/c", '"C:\\repo path\\.neo\\run-script.cmd"'],
    })
  })
  ...
  it("rejects symlinks pointing outside the .neo directory", () => {
  ...
  it("accepts symlinks pointing inside the .neo directory", () => {
