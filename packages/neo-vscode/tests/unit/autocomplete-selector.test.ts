...
describe("autocomplete document selector", () => {
  ...
  it("uses the matching FIM model for notebook fallback", () => {
    expect(notebookModel("neo", "inception/mercury-next-edit").id).toBe("neo/inception/mercury-edit-2")
    expect(notebookModel("inception", "mercury-next-edit").id).toBe("inception/mercury-edit-2")
    expect(notebookModel("neo", "mistralai/codestral-2508").id).toBe("neo/mistralai/codestral-2508")
  })
})
