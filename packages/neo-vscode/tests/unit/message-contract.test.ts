 ...
 * 2. Every WebviewMessage type handled in NeoProvider has a corresponding member in the WebviewMessage union
...
const NEO_PROVIDER_UTILS_FILE = path.join(ROOT, "src/neo-provider-utils.ts")
...
describe("NeoProvider message handler coverage", () => {
  it("all WebviewMessage switch cases in NeoProvider exist in WebviewMessage union", () => {
    const providerContent = readFile(NEO_PROVIDER_FILE)
    ...
    expect(
      unrecognized,
      `NeoProvider switch cases not found in any message type definition: ${unrecognized.join(", ")}`,
    ).toEqual([])
  })
})
...
describe("mapSSEEventToWebviewMessage output types", () => {
  it("all output types from mapSSEEventToWebviewMessage exist in ExtensionMessage", () => {
    const utilsContent = readFile(NEO_PROVIDER_UTILS_FILE)
    const messagesContent = readMessageTypeSources()
