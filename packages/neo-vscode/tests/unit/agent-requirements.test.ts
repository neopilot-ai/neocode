...
} from "../../src/neo-provider/agent-requirements"
import { AgentRequirementsController } from "../../src/neo-provider/agent-requirements-controller"
...
describe("agent requirement helpers", () => {
  ...
  it("scopes requests to active workspace, session, and worktree directories", () => {
    const sessions = new Map([["session-1", "/repo/.neo/worktrees/one"]])
    const base = {
      workspaceDirectory: root,
      workspaceDirectories: ["/workspace"],
      projectDirectory: root,
      sessionDirectories: sessions,
      worktreeDirectories: () => ["/repo/.neo/worktrees/two"],
    }

    expect(requirementDirectory({ ...base, requested: "/repo" })).toBe(root)
    expect(requirementDirectory({ ...base, requested: "/repo/.neo/worktrees/one", sessionID: "session-1" })).toBe(
      "/repo/.neo/worktrees/one",
    )
    expect(requirementDirectory({ ...base, requested: "/repo/.neo/worktrees/two" })).toBe("/repo/.neo/worktrees/two")
