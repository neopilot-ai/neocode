...
import { NeoProviderMemory } from "../../src/neo-provider/memory"

function subject(client: NeoClient | undefined) {
  const posts: unknown[] = []
  const memory = new NeoProviderMemory({
    client: () => client,
    session: () => undefined,
    dir: () => "/repo",
...
describe("NeoProviderMemory", () => {
  ...
  it("evicts older cached memory payloads", async () => {
    let client: NeoClient | undefined
    const posts: unknown[] = []
    const item = new NeoProviderMemory({
      client: () => client,
      session: () => undefined,
    ...
    expect(posts[1]).toMatchObject({
      type: "memoryLoaded",
      sessionID: "ses_8",
      show: { root: "/repo/ses_8/.neo/memory" },
    })
  })
    ...
    const item = subject({
      memory: {
        configure: async (input: unknown) => {
          ...
          return { data: { root: "/repo/.neo/memory", state: state.state } }
        },
        purge: async (input: unknown) => {
          calls.push(["purge", input])
          return { data: { root: "/repo/.neo/memory", purged: true } }
        },
        status: async () => ({ data: state }),
        show: async () => ({ data: view }),
      },
