...
const NEO_PROVIDER_FILE = path.join(ROOT, "src/NeoProvider.ts")
...
describe("Extension — package.json command sync", () => {
  ...
  it("every contributes.commands entry has a registerCommand() call", () => {
    ...
    const dynamic: Record<string, string> = {
      "neo-code.new.agentManager.jumpTo": "registerCommand(`neo-code.new.agentManager.jumpTo${",
    }
  ...
  /**
   * All declared commands must use the neo-code.new. prefix.
   * The legacy neo-code.* namespace (without .new.) belongs to the old
   * extension and must not be reintroduced.
   */
  it("all declared commands use the neo-code.new. prefix", () => {
    const bad = declared.filter((cmd) => !cmd.startsWith("neo-code.new."))
    expect(
      bad,
      `Commands without "neo-code.new." prefix — use the namespaced form:\n` + bad.map((b) => `  - ${b}`).join("\n"),
    ).toEqual([])
  })
  ...
  it("scopes Agent Manager search to the panel and leaves the integrated terminal alone", () => {
    const binding = pkg.contributes?.keybindings?.find(
      (item: { command: string }) => item.command === "neo-code.new.agentManager.search",
    )
    expect(binding).toMatchObject({
      key: "ctrl+f",
      mac: "cmd+f",
      when: "activeWebviewPanelId == 'neo-code.new.AgentManagerPanel' && !terminalFocus",
    })
  })

  it("scopes the open PR shortcut to Agent Manager", () => {
    const binding = pkg.contributes?.keybindings?.find(
      (item: { command: string }) => item.command === "neo-code.new.agentManager.openPR",
    )
    expect(binding).toMatchObject({
      key: "ctrl+shift+r",
      mac: "cmd+shift+r",
      when: "activeWebviewPanelId == 'neo-code.new.AgentManagerPanel'",
    })
  })
})

// ---------------------------------------------------------------------------
// NeoProvider handler wiring — every new NeoProvider() must get
// setContinueInWorktreeHandler() called before resolving its webview.
//
// Regression: tab panels created via openNeoInNewTab() and the TabPanel
...
describe("Extension — NeoProvider handler wiring", () => {
  const ext = fs.readFileSync(EXTENSION_FILE, "utf-8")

  /**
   * Every `new NeoProvider(` in extension.ts must be followed (before the
   * next `new NeoProvider(`) by a `setContinueInWorktreeHandler` call.
   * This prevents future tab/panel additions from silently missing the handler.
   */
  it("every NeoProvider instance gets setContinueInWorktreeHandler wired", () => {
    const pattern = /new NeoProvider\(/g
    ...
    expect(
      instances.length,
      "expected at least 3 NeoProvider instances (sidebar, tab, deserializer)",
    ...
    for (let i = 0; i < instances.length; i++) {
      ...
      if (!region.includes("setContinueInWorktreeHandler")) {
        ...
        missing.push(`NeoProvider at line ${line}`)
      }
    }

    expect(
      missing,
      `These NeoProvider instances are missing setContinueInWorktreeHandler.\n` +
  ...
  it("openNeoInNewTab wires setContinueInWorktreeHandler before resolveWebviewPanel", () => {
    const fn = ext.indexOf("function openNeoInNewTab")
    expect(fn, "openNeoInNewTab must exist").toBeGreaterThan(-1)
  ...
  it("TabPanel deserializer wires setContinueInWorktreeHandler before resolveWebviewPanel", () => {
    const serializer = ext.indexOf('"neo-code.new.TabPanel"')
...
// ---------------------------------------------------------------------------
// NeoProvider — continueInWorktree error fallback
//
...
describe("NeoProvider — remote focus lifecycle", () => {
  const provider = fs.readFileSync(NEO_PROVIDER_FILE, "utf-8")
...
describe("NeoProvider — continueInWorktree error fallback", () => {
  const helper = fs.readFileSync(path.join(ROOT, "src/neo-provider/continue-worktree.ts"), "utf-8")
