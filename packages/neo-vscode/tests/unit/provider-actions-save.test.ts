...
describe("saveCustomProvider", () => {
  ...
  // Regression tests for https://github.com/Neopilot-Ai/neocode/issues/9186
  //
...
describe("fetchProviderData", () => {
  ...
  it("uses local Neo auth status instead of profile availability", async () => {
    const client = {
      provider: {
        list: async () => ({
          data: {
            all: [{ id: "neo", name: "Neo Gateway", source: "custom", env: [], models: {} }],
            connected: ["neo"],
            default: { neo: "neo-auto/frontier" },
          },
        }),
        auth: async () => ({ data: {} }),
      },
    ...
    expect(result.authStates).toEqual({ neo: "oauth" })
  })

  it("does not infer Neo speech access without stored Gateway auth", async () => {
    const client = {
      provider: {
        list: async () => ({
          data: {
            all: [{ id: "neo", name: "Neo Gateway", source: "config", key: "configured", env: [], models: {} }],
            connected: ["neo"],
            default: { neo: "neo-auto/frontier" },
          },
        }),
