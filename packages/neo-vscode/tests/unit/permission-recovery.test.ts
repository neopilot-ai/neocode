...
} from "../../src/neo-provider/handlers/permission-handler"
...
describe("recoveryDirs", () => {
  ...
  it("returns workspace root plus each unique worktree directory", () => {
    const dirs = new Map([
      ["s1", "/workspace/.neo/worktrees/alpha"],
      ["s2", "/workspace/.neo/worktrees/beta"],
      ["s3", "/workspace/.neo/worktrees/alpha"],
    ])
    expect(recoveryDirs("/workspace", dirs)).toEqual([
      "/workspace",
      "/workspace/.neo/worktrees/alpha",
      "/workspace/.neo/worktrees/beta",
    ])
  })

  it("includes extra worktree directories", () => {
    const dirs = new Map([["s1", "/workspace/.neo/worktrees/alpha"]])
    expect(recoveryDirs("/workspace", dirs, ["/workspace/.neo/worktrees/beta", "/workspace"])).toEqual([
      "/workspace",
      "/workspace/.neo/worktrees/alpha",
      "/workspace/.neo/worktrees/beta",
    ])
...
describe("handlePermissionResponse", () => {
  it("uses the recorded SSE directory instead of a stale session fallback", async () => {
    ...
    permDirs.set("p1", "/workspace/.neo/worktrees/feature")

    await handlePermissionResponse(fake, "p1", "s1", "once", [], [])

    expect(replies).toEqual([{ requestID: "p1", reply: "once", directory: "/workspace/.neo/worktrees/feature" }])
  })
  ...
  it("saves selected rules and replies in the recorded SSE directory", async () => {
    ...
    expect(saves).toEqual([
      {
        requestID: "p1",
        directory: "/workspace/.neo/worktrees/feature",
        approvedAlways: ["bun *"],
        deniedAlways: ["rm *"],
      },
    ])
    expect(replies).toEqual([{ requestID: "p1", reply: "reject", directory: "/workspace/.neo/worktrees/feature" }])
  })
...
describe("fetchAndSendPendingPermissions", () => {
  ...
  it("queries workspace root plus each unique worktree directory", async () => {
    ...
    expect(queries).toContain("/workspace/.neo/worktrees/alpha")
    expect(queries).toContain("/workspace/.neo/worktrees/beta")
  ...
  it("queries extra Agent Manager worktree directories", async () => {
    const { fake, queries, permDirs } = ctx({
      tracked: ["s1"],
      extra: ["/workspace/.neo/worktrees/late"],
      permsPerDir: { "/workspace/.neo/worktrees/late": [pending("p1", "s1")] },
    })
    await fetchAndSendPendingPermissions(fake)
    expect(queries).toEqual(["/workspace", "/workspace/.neo/worktrees/late"])
    expect(permDirs.get("p1")).toBe("/workspace/.neo/worktrees/late")
  })

  it("preserves cached routes for directories that fail to list", async () => {
    const dirs = new Map([["s1", "/workspace/.neo/worktrees/failing"]])
    const error = new Error("temporary failure")
    const { fake, permDirs } = ctx({
      tracked: ["s1"],
      dirs,
      errors: { list: { "/workspace/.neo/worktrees/failing": error } },
    })
    const spy = spyOn(console, "error").mockImplementation(() => {})
    permDirs.set("workspace-stale", "/workspace")
    permDirs.set("worktree-pending", "/workspace/.neo/worktrees/failing")

    await fetchAndSendPendingPermissions(fake)
    spy.mockRestore()

    expect(permDirs.has("workspace-stale")).toBe(false)
    expect(permDirs.get("worktree-pending")).toBe("/workspace/.neo/worktrees/failing")
  })

  it("deduplicates directories", async () => {
    const dirs = new Map([
      ["s1", "/workspace/.neo/worktrees/alpha"],
      ["s2", "/workspace/.neo/worktrees/alpha"],
    ])
    const { fake, queries } = ctx({ tracked: ["s1", "s2"], dirs })
    await fetchAndSendPendingPermissions(fake)
    expect(queries.filter((d) => d === "/workspace/.neo/worktrees/alpha")).toHaveLength(1)
  })
