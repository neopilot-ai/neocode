...
import type { NotebookRequest } from "@neocode/sdk/v2/client"
import * as vscode from "vscode"
import { NeoConnectionService } from "../../src/services/cli-backend/connection-service"
...
function harness(context: NotebookBridgeContext, dirs = ["/repo"]) {
  ...
  const request = (value: NotebookRequest = read, directory = "/repo") =>
    handlers.event?.(
      { id: `event-${value.id}`, type: "neocode.notebook.requested", properties: value } as SSEPayload,
      directory,
    )
  const cancel = (id = read.id, directory = "/repo") =>
    handlers.event?.(
      {
        id: `cancel-${id}`,
        type: "neocode.notebook.cancelled",
...
describe("NeoConnectionService notebook directories", () => {
  it("tracks the workspace root, current request directory, and provider directories", async () => {
    ...
    const service = new NeoConnectionService({} as vscode.ExtensionContext)
