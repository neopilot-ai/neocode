...
describe("parseServerPort", () => {
  it("parses port from standard CLI startup message", () => {
    expect(parseServerPort("neo server listening on http://127.0.0.1:12345")).toBe(12345)
  })
  ...
  it("parses port when embedded in longer output", () => {
    const output = "[INFO] 2024-01-01 neo server listening on http://127.0.0.1:54321\n[INFO] ready"
...
describe("cli tree-sitter resources", () => {
  it("resolves resources next to the VS Code bundled CLI", () => {
    const root = "/Users/test/.vscode/extensions/neocode.neo-code-7.2.50-darwin-arm64"
    const bin = `${root}/bin/neo`
    ...
    expect(neoSandboxWorkerForBinary(bin)).toBe(`${root}/bin/neo-sandbox-mutation-worker.js`)
    expect(resolveTreeSitterEnv(root)).toEqual({ NEO_TREE_SITTER_WASM_DIR: `${root}/bin/tree-sitter` })
  })

  it("resolves resources next to a Windows packaged CLI", () => {
    const root = String.raw`C:\Users\test\.vscode\extensions\neocode.neo-code-7.2.50-win32-x64`
    const bin = String.raw`${root}\bin\neo.exe`
    ...
    expect(neoSandboxWorkerForBinary(bin)).toBe(String.raw`${root}\bin\neo-sandbox-mutation-worker.js`)
    expect(resolveTreeSitterEnv(root)).toEqual({
      NEO_TREE_SITTER_WASM_DIR: String.raw`${root}\bin\tree-sitter`,
    })
  })

  it("copies the Neo sandbox worker with the packaged CLI binary", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "neo-vscode-sandbox-worker-"))
    try {
      const source = path.join(root, "dist", "@neocode", "cli-darwin-arm64", "bin", "neo")
      const target = path.join(root, "extension", "bin", "neo")
      await fs.mkdir(path.dirname(source), { recursive: true })
      await fs.mkdir(path.dirname(target), { recursive: true })
      await fs.writeFile(neoSandboxWorkerForBinary(source), "worker")

      await copyNeoSandboxWorker(source, target)

      expect(await fs.readFile(neoSandboxWorkerForBinary(target), "utf8")).toBe("worker")
    } finally {
  ...
  it("copies runtime and language WASMs with the packaged CLI binary", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "neo-vscode-tree-sitter-"))
    try {
      const source = path.join(root, "dist", "@neocode", "cli-darwin-arm64", "bin", "neo")
      const target = path.join(root, "extension", "bin", "neo")
  ...
  it("copies the Linux sandbox helper and license resources", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "neo-vscode-sandbox-"))
    try {
      const source = path.join(root, "dist", "bin", "neo")
      const target = path.join(root, "extension", "bin", "neo")
      ...
      const relay = path.join(path.dirname(source), "neo-sandbox-network-relay.js")
      const seccomp = path.join(path.dirname(source), "neo-sandbox-seccomp")
      ...
      expect(await fs.readFile(path.join(path.dirname(target), "neo-sandbox-network-relay.js"), "utf8")).toBe("relay")
      const copiedSeccomp = path.join(path.dirname(target), "neo-sandbox-seccomp")
  ...
  it("copies seccomp licensing when bundled Bubblewrap is omitted", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "neo-vscode-seccomp-license-"))
    try {
      const source = path.join(root, "dist", "bin", "neo")
      const target = path.join(root, "extension", "bin", "neo")
      const relay = path.join(path.dirname(source), "neo-sandbox-network-relay.js")
      ...
      expect(await fs.readFile(path.join(path.dirname(target), "neo-sandbox-network-relay.js"), "utf8")).toBe("relay")
      expect(await fs.readFile(path.join(path.dirname(target), "neo-sandbox-seccomp"), "utf8")).toBe("seccomp")
      expect(await fs.readFile(path.join(path.dirname(target), "licenses", "sandbox-runtime", "LICENSE"), "utf8")).toBe(
        "Apache-2.0",
      )
    } finally {
  ...
  it("removes stale sandbox resources when the source has none", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "neo-vscode-sandbox-stale-"))
    try {
      const source = path.join(root, "dist", "bin", "neo")
      const target = path.join(root, "extension", "bin", "neo")
...
describe("toErrorMessage", () => {
  ...
  it("strips ANSI codes before matching Error:", () => {
    const ansiError = "\x1b[91m\x1b[1mError: \x1b[0mConfig file at /path/neo.json is not valid JSON(C):"
    const result = toErrorMessage("startup failed", [ansiError])
    expect(result.userMessage).toBe("Config file at /path/neo.json is not valid JSON(C):")
  })
  ...
  it("includes CLI path in userDetails when provided", () => {
    const result = toErrorMessage("startup failed", [], "/usr/local/bin/neo")
    expect(result.userDetails).toContain("CLI path: /usr/local/bin/neo")
  })
...
describe("server workspace helpers", () => {
  ...
  it("disables codebase indexing only when no workspace folder is open", () => {
    expect(resolveIndexingEnv(undefined)).toEqual({ NEO_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" })
    expect(resolveIndexingEnv([])).toEqual({ NEO_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" })
  ...
  it("uses the shared database for the managed backend while preserving the environment", () => {
    expect(resolveManagedServerEnv({ PATH: "/usr/bin", NEO_DISABLE_CHANNEL_DB: "false" })).toEqual({
      PATH: "/usr/bin",
      NEO_DISABLE_CHANNEL_DB: "true",
    })
  })
})
