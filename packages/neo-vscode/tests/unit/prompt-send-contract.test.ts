...
const CLOUD_SESSION_FILE = path.join(ROOT, "src/neo-provider/handlers/cloud-session.ts")
...
describe("static command completion contract", () => {
  ...
  it("acknowledges aliases after direct and cloud command confirmation", () => {
    const provider = readFile(NEOPROVIDER_FILE)
...
describe("NeoProvider pruneDeletedSession contract", () => {
...
describe("NeoConnectionService pruneSession contract", () => {
  const source = readFile(CONNECTION_SERVICE_FILE)

  it("drops the deleted session from attached and visible Maps", () => {
    // NeoProvider's pruneDeletedSession calls connectionService.pruneSession.
