...
import { globalFiles, localFiles } from "../../src/neo-provider/config-file"
import { openConfig } from "../../src/neo-provider/open-config"
...
const labels = {
  ...
  sourceHomeNeo: "Home Neo",
  sourceHomeNeocode: "Home Neocode",
  sourceHomeOpencode: "Home Opencode",
  sourceProjectNeo: "Project Neo",
  sourceProjectNeocode: "Project Neocode",
...
async function temp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "neo-config-"))
  dirs.push(dir)
  return dir
}
...
describe("config file discovery", () => {
  it("discovers global, env, legacy, and virtual config sources", async () => {
    ...
    process.env.NEO_CONFIG_DIR = extra
    process.env.NEO_CONFIG_CONTENT = "{}"
    await file(path.join(xdg, "neo", "neo.json"))
    await file(path.join(home, ".neocode", "opencode.json"))
    await file(path.join(home, ".opencode", "neo.jsonc"))
    await file(envfile)
    ...
    expect(sources).toContain("sourceHomeNeocode")
  ...
  it("marks project files unloaded when project config is disabled", async () => {
    ...
    await file(path.join(root, "neo.json"))
    ...
    expect(list.find((item) => item.recommended)?.file).toBe(path.join(root, ".neo", "neo.jsonc"))
  })
})
...
describe("openConfig", () => {
  ...
  it("opens the only editable config without showing the picker", async () => {
    reset()
    const root = await temp()
    const cfg = path.join(root, ".neo", "neo.jsonc")
    await file(cfg)
