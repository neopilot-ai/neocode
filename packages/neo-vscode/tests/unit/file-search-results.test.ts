...
import { mergeFileSearchResults } from "../../src/neo-provider/file-search-results"
...
describe("mergeFileSearchResults", () => {
  ...
  it("preserves camel-case scoring for acronym-style queries", () => {
    const result = mergeFileSearchResults({
      query: "amprov",
      backend: [
        "packages/neo-docs/pages/contributing/architecture/onboarding-improvements.md",
        "packages/neo-vscode/src/agent-manager/AgentManagerProvider.ts",
      ],
      open: new Set(),
    })
    expect(result[0]).toBe("packages/neo-vscode/src/agent-manager/AgentManagerProvider.ts")
  })
})
