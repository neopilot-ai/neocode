...
import type { QuestionRequest } from "@neocode/sdk/v2/client"
import {
  fetchAndSendPendingQuestions,
  handleQuestionReject,
  handleQuestionReply,
  type QuestionContext,
} from "../../src/neo-provider/handlers/question"
...
describe("question handlers", () => {
  it("routes replies through the recorded request directory", async () => {
    const { fake, replies, questionDirs } = ctx({
      ...
      dirs: new Map([["ses-worktree", "/workspace/.neo/worktrees/current"]]),
    })
    questionDirs.set("req-1", "/workspace/.neo/worktrees/origin")
    ...
    expect(replies).toEqual([
      {
        requestID: "req-1",
        answers: [["Continue"]],
        directory: "/workspace/.neo/worktrees/origin",
      },
    ])
  ...
  it("routes rejects through the recorded request directory", async () => {
    ...
    questionDirs.set("req-2", "/workspace/.neo/worktrees/origin")

    const ok = await handleQuestionReject(fake, "req-2", "ses-worktree")

    expect(ok).toBe(true)
    expect(rejects).toEqual([{ requestID: "req-2", directory: "/workspace/.neo/worktrees/origin" }])
    ...
    expect(replies).toEqual([
      {
        requestID: "req-3",
        answers: [["Continue"]],
        directory: "/workspace/.neo/worktrees/current",
      },
    ])
  })
  ...
  it("removes a stale question when the backend reports it missing", async () => {
    ...
    questionDirs.set("req-stale", "/workspace/.neo/worktrees/origin")
  ...
  it("keeps fallback-directory 404s retryable while recovering the request route", async () => {
    ...
    const dir = "/workspace/.neo/worktrees/origin"
  ...
  it("keeps fallback questions retryable when recovery is incomplete", async () => {
    ...
    const dir = "/workspace/.neo/worktrees/failing"
  ...
  it("keeps non-404 failures retryable", async () => {
    ...
    const spy = spyOn(console, "error").mockImplementation(() => {})
    questionDirs.set("req-error", "/workspace/.neo/worktrees/origin")

    const ok = await handleQuestionReject(fake, "req-error", "ses-root")
    spy.mockRestore()

    expect(ok).toBe(false)
    expect(questionDirs.get("req-error")).toBe("/workspace/.neo/worktrees/origin")
...
describe("question recovery", () => {
  it("records the directory that owns each recovered question", async () => {
    const dir = "/workspace/.neo/worktrees/late"
  ...
  it("deduplicates recovered questions across directories", async () => {
    ...
    const dir = "/workspace/.neo/worktrees/feature"
    const { fake, messages } = ctx({
      tracked: ["ses-worktree"],
