/**
 * Runtime contract tests for neo-vscode's dependencies on @neocode/neo-ui.
 *
 * These tests import the upstream UI modules directly and verify at runtime
 * that the exports neo-vscode depends on still exist with the expected shape.
 *
 * Because the upstream modules use SolidJS JSX (jsxImportSource: "solid-js"),
 * they must be loaded from within packages/neo-ui/ where bun picks up the
...
const NEO_UI_DIR = path.join(MONOREPO_ROOT, "packages/neo-ui")
...
const NEO_MESSAGE_PART_FILE = path.join(MONOREPO_ROOT, "packages/neo-ui/src/components/message-part.tsx")
const NEO_MESSAGE_HIGHLIGHT_FILE = path.join(MONOREPO_ROOT, "packages/neo-ui/src/components/message-highlight.ts")
const NEO_MESSAGE_PART_CSS_FILE = path.join(MONOREPO_ROOT, "packages/neo-ui/src/components/message-part.css")
const SHELL_ROLLING_FILE = path.join(MONOREPO_ROOT, "packages/neo-ui/src/components/shell-rolling-results.tsx")
const ASSISTANT_MESSAGE_FILE = path.join(
  MONOREPO_ROOT,
  "packages/neo-vscode/webview-ui/src/components/chat/AssistantMessage.tsx",
)
const TRANSCRIPT_PARTS_FILE = path.join(MONOREPO_ROOT, "packages/neo-vscode/webview-ui/src/utils/transcript-parts.ts")
const CHAT_LAYOUT_FILE = path.join(MONOREPO_ROOT, "packages/neo-vscode/webview-ui/src/styles/chat-layout.css")
...
/**
 * Tool names that neo-vscode overrides or uses directly.
 * Sources:
...
describe("ToolRegistry tool name contract (runtime)", () => {
  it("all tools used by neo-vscode are registered in ToolRegistry", () => {
...
describe("DataProvider contract (runtime)", () => {
  ...
  it("DataProvider accepts onOpenFile prop and exports OpenFileFn (source)", () => {
    // onOpenFile and OpenFileFn are `neocode_change` additions — TypeScript types
  ...
  it("DataProvider accepts onOpenDiff prop and exports OpenDiffFn (source)", () => {
    // onOpenDiff and OpenDiffFn are `neocode_change` additions — TypeScript types
    // erased at runtime, so we verify via source analysis
    const src = fs.readFileSync(DATA_CONTEXT_FILE, "utf-8")
...
describe("Assistant Markdown streaming contract (source)", () => {
  const src = fs.readFileSync(NEO_MESSAGE_PART_FILE, "utf-8")
  const block =
    src.match(
...
describe("Expanded tool motion and typography (source)", () => {
  ...
  it("uses the assistant markdown line-height ratio for reasoning output", () => {
    const css = fs.readFileSync(NEO_MESSAGE_PART_CSS_FILE, "utf-8")
    const block = css.match(
      /html\[data-theme="neo-vscode"\] \[data-component="reasoning-part"\][\s\S]*?(?=@keyframes reasoning-pulse)/,
    )?.[0]
...
describe("HighlightedText @mention regex fallback and click handler (source)", () => {
  const src = fs.readFileSync(NEO_MESSAGE_PART_FILE, "utf-8")
  const helper = fs.readFileSync(NEO_MESSAGE_HIGHLIGHT_FILE, "utf-8")
...
describe("Collapsed deferred tool details contract (source)", () => {
  ...
  const message = fs.readFileSync(NEO_MESSAGE_PART_FILE, "utf-8")
