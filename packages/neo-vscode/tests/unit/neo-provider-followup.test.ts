...
import type { Event, Session } from "@neocode/sdk/v2/client"
...
describe("NeoProvider follow-up sessions", () => {
  it("ignores subagents before adopting pending follow-up sessions", async () => {
    const service = connection()
    const provider = new NeoProvider({} as never, service as never)
  ...
  it("calls onFollowupAdopted listeners with session and directory", async () => {
    ...
    internal.pendingFollowup = { dir: "/repo/.neo/worktrees/feat", time: Date.now() }
    service.emit(created({ id: "ses-wt", directory: "/repo/.neo/worktrees/feat" }))
    await Promise.resolve()

    expect(adopted).toEqual([{ id: "ses-wt", dir: "/repo/.neo/worktrees/feat" }])
  })
})
