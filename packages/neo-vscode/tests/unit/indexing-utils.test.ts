...
import { mapSSEEventToWebviewMessage } from "../../src/neo-provider-utils"
import { configFeatures } from "../../src/features"
import type { EventIndexingStatus, IndexingStatus } from "@neocode/sdk/v2/client"
...
describe("indexing feature detection", () => {
  it("enables indexing settings when the indexing plugin is present", () => {
    expect(configFeatures({ plugin: ["neo-indexing"] }).indexing).toBe(true)
  })

  it("detects supported indexing plugin specifiers", () => {
    expect(configFeatures({ plugin: ["neo-indexing"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["neo-indexing@1.2.3"] }).indexing).toBe(true)
  ...
  it("ignores unrelated plugin lists", () => {
    expect(configFeatures({ plugin: ["@neocode/neo-gateway"] }).indexing).toBe(false)
