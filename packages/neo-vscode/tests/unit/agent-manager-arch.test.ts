...
describe("Agent Manager CSS Prefix", () => {
  it("all class selectors should use am- prefix", () => {
    ...
    // - `neo-diff-theme` is the shared Pierre diff theme utility defined
    //   in webview-ui/src/styles/diff.css and reused across webviews.
    // - `css` is matched from `@import "./diff.css"` file extension, not a
    //   class selector.
    const host = new Set(["vscode-high-contrast", "vscode-high-contrast-light", "neo-diff-theme", "css"])
  ...
  it("all CSS custom properties should use am- prefix", () => {
    ...
    // Allow neo-ui design tokens, vscode theme variables, and third-party
    // library tokens (@pierre/diffs, neo-ui sticky-accordion) used as fallbacks
...
describe("Agent Manager Worktree Actions", () => {
  ...
  it("registers distinct dialog and quick-create worktree shortcuts", () => {
    ...
    const dialog = manifest.contributes.keybindings.find(
      (item) => item.command === "neo-code.new.agentManager.newWorktree",
    )
    const quick = manifest.contributes.keybindings.find(
      (item) => item.command === "neo-code.new.agentManager.quickWorktree",
    )
  ...
  it("forwards the quick-worktree command to immediate creation", () => {
    ...
    const start = source.indexOf('vscode.commands.registerCommand("neo-code.new.agentManager.quickWorktree"')
    const end = source.indexOf('vscode.commands.registerCommand("neo-code.new.agentManager.openWorktree"', start)
...
describe("Agent Manager Webview — non-git sessionsLoaded fix", () => {
  ...
  /**
   * Regression: when isGitRepo is false, the Neo server never sends a
...
// ---------------------------------------------------------------------------
// NeoProvider — pendingSessionRefresh race condition fix
// ---------------------------------------------------------------------------

describe("NeoProvider — pending session refresh on reconnect", () => {
  const provider = fs.readFileSync(NEO_PROVIDER_FILE, "utf-8")
  const utils = fs.readFileSync(path.join(ROOT, "src/neo-provider-utils.ts"), "utf-8")
   ...
   * neo-provider-utils) sets it when httpClient is unavailable, and
  ...
  it("loadSessions sets pendingSessionRefresh when client is null", () => {
    ...
    expect(start, "loadSessions must exist in neo-provider-utils").toBeGreaterThan(-1)
...
//
// Regression: PR #7473 moved NeoNotifications into MessageList. It calls
