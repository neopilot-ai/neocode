...
describe("buildFeedbackProperties — non-Neo providers", () => {
...
describe("buildFeedbackProperties — Neo Gateway", () => {
  it("includes sessionID, messageID, parentMessageID", () => {
    const props = buildFeedbackProperties({ ...baseInput, providerID: "neo", next: "up" })
    expect(props).toEqual({
      providerID: "neo",
      modelID: "claude-sonnet-4-5",
      rating: "up",
