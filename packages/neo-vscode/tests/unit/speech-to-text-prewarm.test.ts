...
const SCRIPT = `
  ...
  const [config, setConfig] = createSignal({ disabled_providers: ["neo"] })
  ...
  if (sent.length !== 0) fail("prewarmed without Neo access")
  setAuth({ neo: "api" })
  if (sent.length !== 0) fail("prewarmed while Neo was disabled")
  setConfig({})
  if (sent.length !== 1 || sent[0]?.type !== "speechToTextPrewarm") {
    fail("did not prewarm after Neo access became available")
  }
  setAuth({ neo: "oauth" })
...
describe("speech-to-text prewarm", () => {
  it("starts only after Neo speech access becomes available", () => {
    const attempts = 3
