...
describe("providerSortKey", () => {
  it("returns 0 for neo gateway", () => {
    expect(providerSortKey(NEO_GATEWAY_ID)).toBe(0)
  })
  ...
  it("sorts providers correctly when used with sort", () => {
    const ids = ["google", "anthropic", "neo", "openai", "deepseek"]
    const sorted = ids.slice().sort((a, b) => providerSortKey(a) - providerSortKey(b))
    expect(sorted).toEqual(["neo", "anthropic", "deepseek", "openai", "google"])
  })
})
...
describe("stripSubProviderPrefix", () => {
  ...
  it("does not strip 'Neo: ' prefix", () => {
    expect(stripSubProviderPrefix("Neo: Auto")).toBe("Neo: Auto")
    expect(stripSubProviderPrefix("neo: Auto")).toBe("neo: Auto")
  })
})
...
describe("sanitizeName", () => {
  ...
  it("preserves bare trailing Free in names like 'Neo Auto Free'", () => {
    expect(sanitizeName("Neo Auto Free")).toBe("Neo Auto Free")
...
describe("isAuto", () => {
  it("matches only Neo Auto model ids", () => {
    expect(isAuto({ providerID: NEO_GATEWAY_ID, id: "neo-auto/efficient" })).toBe(true)
    expect(isAuto({ providerID: NEO_GATEWAY_ID, id: "auto-small" })).toBe(true)
    expect(isAuto({ providerID: "anthropic", id: "neo-auto/efficient" })).toBe(false)
    expect(isAuto({ providerID: NEO_GATEWAY_ID, id: "anthropic/claude-sonnet" })).toBe(false)
  })
})
...
describe("autoChoices", () => {
  ...
  it("ignores missing routes and non-efficient Auto models", () => {
    expect(autoChoices({ providerID: NEO_GATEWAY_ID, id: "neo-auto/efficient" })).toEqual([])
    expect(
      autoChoices({
        providerID: NEO_GATEWAY_ID,
        id: "neo-auto/frontier",
...
describe("buildTriggerLabel", () => {
  it("returns resolved model name for non-neo provider unchanged", () => {
    expect(buildTriggerLabel("GPT-4o", "openai", undefined, null, false, "", true, labels)).toBe("GPT-4o")
  })

  it("strips sub-provider prefix from resolved name for neo gateway models", () => {
    expect(
      buildTriggerLabel("Anthropic: Claude Sonnet", NEO_GATEWAY_ID, undefined, null, false, "", true, labels),
    ).toBe("Claude Sonnet")
  })

  it("does not strip prefix for non-neo provider even if name contains ': '", () => {
  ...
  it("returns providerName / resolvedName for non-neo provider with providerName", () => {
    expect(buildTriggerLabel("GPT-4o", "openai", "OpenAI", null, false, "", true, labels)).toBe("OpenAI / GPT-4o")
  })

  it("returns modelID for neo gateway raw selection", () => {
    const raw = { providerID: "neo", modelID: "neo-auto/frontier" }
    expect(buildTriggerLabel(undefined, undefined, undefined, raw, false, "", true, labels)).toBe("neo-auto/frontier")
  })

  it("returns providerID / modelID for non-neo raw selection", () => {
