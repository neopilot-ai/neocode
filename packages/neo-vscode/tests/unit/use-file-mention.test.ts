  ...
  it("keeps previous file results visible while the next search is pending", async () => {
    ...
    for (const handler of handlers) {
      handler({
        ...
        dir: "/repo",
        paths: ["packages/neo-vscode/src/extension.ts"],
        items: [{ path: "packages/neo-vscode/src/extension.ts", type: "opened-file" }],
      })
    }

    expect(mention.mentionResults()).toEqual([
      { type: "opened-file", value: "packages/neo-vscode/src/extension.ts" },
      FILE_PICKER_RESULT,
    ])
