...
import { createAutoApproveBridge } from "../../src/neo-provider/auto-approve"
import type { Event, NeoClient } from "@neocode/sdk/v2/client"
import type { NeoConnectionService } from "../../src/services/cli-backend/connection-service"
...
function config(initial: boolean, info: Record<string, unknown> = {}) {
  ...
  return {
    ...
    emit(key = "neo-code.new.autoApprove.enabled") {
...
function connection(client: NeoClient | null, dirs = new Map<string, string>()) {
  const svc = {
    getClient: () => {
  ...
  } as unknown as NeoConnectionService
...
describe("registerToggleAutoApprove", () => {
  ...
  it("uses the SSE directory for worktree permissions before session mappings are available", async () => {
    ...
    await ctrl.approve(asked("perm_worktree", "ses_worktree"), "/workspace/.neo/worktrees/feature")
    await ctrl.approve(asked("perm_child", "ses_child"), "/workspace/.neo/worktrees/feature")

    expect(replies).toEqual([
      { requestID: "perm_worktree", directory: "/workspace/.neo/worktrees/feature", reply: "once" },
      { requestID: "perm_child", directory: "/workspace/.neo/worktrees/feature", reply: "once" },
    ])
  })
  ...
  it("uses the shared permission directory before falling back to session mappings", async () => {
    ...
    const conn = connection(
      ...
      new Map([["perm_shared", "/workspace/.neo/worktrees/shared"]]),
    )
    const ctrl = registerToggleAutoApprove(
      context(),
      conn.svc,
    ...
    expect(replies).toEqual([
      { requestID: "perm_shared", directory: "/workspace/.neo/worktrees/shared", reply: "once" },
    ])
  })
