...
import { NEO_AUTO, parseModelString } from "../../src/shared/provider-model"
...
const providers = {
  neo: makeProvider("neo", "Neo Gateway", ["neo-auto/free"]),
...
describe("parseModelString", () => {
  ...
  it("keeps slashes inside neo model ids", () => {
    expect(parseModelString("neo/neo-auto/free")).toEqual({
      providerID: "neo",
      modelID: "neo-auto/free",
    })
  })
...
describe("resolveModelSelection", () => {
  ...
  it("uses neo auto as the explicit final fallback", () => {
    const result = resolveModelSelection({
      providers,
      connected: [],
      fallback: NEO_AUTO,
    })
    expect(result).toEqual(NEO_AUTO)
  })

  it("keeps the explicit fallback even when neo is missing from the loaded catalog", () => {
    const result = resolveModelSelection({
