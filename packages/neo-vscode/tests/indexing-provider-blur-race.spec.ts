...
const GLOBALS = "colorScheme:dark;theme:neo-vscode;vscodeTheme:dark-modern"
const STORY_ID = "settings--indexing-provider-blur-race"
const NEO_STORY_ID = "settings--indexing-neo-model-preset"
const NEO_LOADING_STORY_ID = "settings--indexing-neo-catalog-loading"
...
test("Neo exposes only supported embedding model presets", async ({ page }) => {
  await page.setViewportSize({ width: 420, height: 720 })
  await page.goto(storyUrl(NEO_STORY_ID), { waitUntil: "load" })
  await disableAnimations(page)
  await page.waitForSelector("#storybook-root *", { state: "attached" })

  await expect(page.getByText("Neo model preset", { exact: true })).toBeVisible()
...
test("enabling Neo before its catalog loads does not store an empty model", async ({ page }) => {
  const saved = page.getByTestId("indexing-neo-loading-save")
  const cfg = async () => JSON.parse(((await saved.textContent()) ?? "{}").trim()) as Saved
  const verify = async () => {
    await expect.poll(async () => (await cfg()).provider).toBe("neo")
    expect((await cfg()).model).toBeNull()
    expect((await cfg()).dimension).toBeNull()
  }

  await page.setViewportSize({ width: 420, height: 720 })
  await page.goto(storyUrl(NEO_LOADING_STORY_ID), { waitUntil: "load" })
  await disableAnimations(page)
  await page.waitForSelector("#storybook-root *", { state: "attached" })
  await page.getByRole("button", { name: "Local", exact: true }).click()
  await page
