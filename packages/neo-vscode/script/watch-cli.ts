 ...
 * then copies it into packages/neo-vscode/bin/neo.
 *
...
const targetBinPath = join(targetBinDir, "neo")
...
function sourceBinaryPath(): string {
  return join(opencodeDir, "dist", `@neocode/cli-${process.platform}-${process.arch}`, "bin", "neo")
}
...
async function rebuild() {
  ...
  try {
    ...
    log(`Binary updated (${elapsed}s): ${relative(packagesDir, source)} -> bin/neo`)
  } catch (err) {
...
// Watch for changes
log(`Watching ${relative(neoVscodeDir, opencodeSrcDir)}/ for changes...`)
