...
const version = process.env.NEO_VERSION ? process.env.NEO_VERSION : packageJson.version
const prerelease = process.env.NEO_PRE_RELEASE === "true"
...
const targets = [
  { target: "linux-x64", cliDir: "@neocode/cli-linux-x64", binary: "neo" },
  { target: "linux-arm64", cliDir: "@neocode/cli-linux-arm64", binary: "neo" },
  { target: "alpine-x64", cliDir: "@neocode/cli-linux-x64-musl", binary: "neo" },
  { target: "alpine-arm64", cliDir: "@neocode/cli-linux-arm64-musl", binary: "neo" },
  { target: "darwin-x64", cliDir: "@neocode/cli-darwin-x64", binary: "neo" },
  { target: "darwin-arm64", cliDir: "@neocode/cli-darwin-arm64", binary: "neo" },
  { target: "win32-x64", cliDir: "@neocode/cli-windows-x64", binary: "neo.exe" },
  { target: "win32-arm64", cliDir: "@neocode/cli-windows-arm64", binary: "neo.exe" },
]
...
for (const config of targets) {
  ...
  await copyNeoSandboxWorker(sourceBinary, targetBinary)

  if (config.binary !== "neo.exe") {
  ...
  const vsixPath = join(outDir, `neo-vscode-${config.target}.vsix`)
