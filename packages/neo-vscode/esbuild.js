...
/**
 * Force all solid-js imports (from neo-ui and the webview) to resolve to
...
const solidDedupePlugin = {
  name: "solid-dedupe",
  setup(build) {
    // Resolve these bare specifiers to the neo-vscode-local copy
 ...
 * variants) to the Neo implementation in `webview-ui/pierre-worker.ts`.
 *
 * The upstream module loads Pierre's Shiki worker via a Vite-only
 * `?worker&url` import that esbuild can't resolve. The Neo replacement loads
...
/**
 * Resolve the synthetic `neo-shiki-worker` entry point to Pierre's Shiki worker
...
const shikiWorkerEntryPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /^neo-shiki-worker$/ }, async () => {
    ...
    build.onLoad({ filter: /sprite\.svg$/ }, (args) => {
      ...
      return {
        contents: `
          ...
          const inject = () => {
            if (!document.getElementById("neo-sprite")) {
...
async function main() {
  ...
  // Build NeoClaw webview (SolidJS, standalone chat panel)
  const neoClawCtx = await createBrowserWebviewContext("webview-ui/neoclaw/index.tsx", "dist/neoclaw.js")
