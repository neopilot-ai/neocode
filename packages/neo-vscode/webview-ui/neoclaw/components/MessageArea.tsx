// NeoClaw active-conversation view — message list + composer.
// Mirrors cloud/apps/web/src/app/(app)/claw/neo-chat/components/MessageArea.tsx

import { For, Show, createEffect, createMemo, createSignal, on, onCleanup } from "solid-js"
import { Button } from "@neocode/neo-ui/button"
import { Spinner } from "@neocode/neo-ui/spinner"
import { createAutoScroll } from "@neocode/neo-ui/hooks"
...
export function MessageArea() {
  ...
  //
  // Uses @neocode/neo-ui's createAutoScroll (the same one MessageList.tsx
  ...
  const sendDisabledReason = createMemo(() => {
    ...
    const st = botDisplay().state
    if (st === "unknown") return t("neoClaw.chat.waitingBotStatus")
    return t("neoClaw.chat.botOffline")
  })

  // Render the typing banner with friendly names. Bot members come in as
  // `bot:neoclaw:{sandboxId}` — the bot is the only non-self member in
  // 1:1 chats, so we resolve bot ids to the user-configured `botName`
  // (falls back to the default "NeoClaw" label) and render any human
  // collaborators by their raw memberId.
  const typingNames = createMemo(() => {
    const activeId = claw.activeConversationId()
    if (!activeId) return []
    const assistant = claw.assistantName() ?? t("neoClaw.message.bot")
  ...
  return (
    <Show
      ...
      fallback={
        <div class="neoclaw-empty-area">
          <p>{t("neoClaw.conversations.selectOne")}</p>
        </div>
      }
    >
      <div class="neoclaw-area">
        {/* Messages */}
        <div class="neoclaw-area-messages-wrap">
          <div
            class="neoclaw-area-messages"
            ref={(el) => {
              scrollEl = el
              auto.scrollRef(el)
            }}
          ...
          >
            <div ref={auto.contentRef}>
              <Show when={claw.messages().length === 0}>
                <div class="neoclaw-empty">
                  {claw.assistantName()
                    ? t("neoClaw.chat.emptyWithBot").replace("{bot}", claw.assistantName()!)
                    : t("neoClaw.chat.empty")}
                </div>
              </Show>
        ...
        <Show when={typingNames().length > 0}>
          <div class="neoclaw-typing">
            <Spinner />
            <span>
              {typingNames().length === 1
                ? t("neoClaw.typing.one").replace("{name}", typingNames()[0])
                : t("neoClaw.typing.many").replace("{count}", String(typingNames().length))}
            </span>
          </div>
        </Show>
        ...
        <Show when={replyingTo()}>
          {(r) => (
            <div class="neoclaw-reply-preview">
              <span class="neoclaw-reply-preview-label">{t("neoClaw.message.replyTo")}</span>
              <span class="neoclaw-reply-preview-text">{replyText(r())}</span>
              <button
                type="button"
                class="neoclaw-iconbtn-sm"
                onClick={() => setReplyingTo(null)}
                aria-label={t("neoClaw.message.cancelReply")}
              >
                ×
              </button>
            </div>
        ...
        <div class="neoclaw-input-wrap">
          <textarea
            ref={input}
            class="neoclaw-input"
            placeholder={sendDisabledReason() ?? t("neoClaw.chat.placeholder")}
            disabled={!canSend()}
            value={text()}
            onInput={onInput}
            onKeyDown={onKeyDown}
            rows={1}
            aria-label={t("neoClaw.chat.placeholder")}
          />
