...
import { DataProvider, type OpenDiffFn, type OpenFileFn } from "@neocode/neo-ui/context/data"
...
const dict: Record<string, string> = { ...appEn, ...amEn, ...uiEn, ...neoEn }
...
// ---------------------------------------------------------------------------
// Mock providers — pre-loaded Neo Gateway model for stories
// ---------------------------------------------------------------------------
...
const MockProviderProvider: ParentComponent<{ neoAuth?: boolean }> = (props) => {
  const value = {
    providers: () => MOCK_PROVIDERS as any,
    connected: () => ["neo"],
    defaults: () => ({}),
    defaultSelection: () => ({ providerID: "neo", modelID: "anthropic/claude-sonnet-4-6" }),
    models: () => MOCK_MODELS,
    findModel: (sel: any) => _findModel(MOCK_MODELS, sel),
    authMethods: () => ({}),
    authStates: () => (props.neoAuth ? { neo: "oauth" } : {}) as Record<string, ProviderAuthState>,
    isModelValid: () => true,
  }
...
function mockNotificationsValue(items: NeocodeNotification[] = []) {
  return {
    notifications: () => items,
...
export function mockSessionValue(overrides?: {
  ...
  return {
    ...
    selected: () => ({ providerID: "neo", modelID: "anthropic/claude-sonnet-4-6" }),
    selectModel: noop,
    ...
    getSessionModel: () => ({ providerID: "neo", modelID: "anthropic/claude-sonnet-4-6" }),
...
interface StoryProvidersProps {
  ...
  questions?: QuestionRequest[]
  suggestions?: SuggestionRequest[]
  notifications?: NeocodeNotification[]
    ...
    <VSCodeProvider>
      <ServerProvider>
        <FeedbackProvider>
          ...
          >
            <DisplayProvider>
              <MockProviderProvider neoAuth={props.neoAuth}>
                <DialogProvider>
