...
import { NeoEmbeddingModelsContext } from "../context/neo-embedding-models"
...
export const ModelsSpeechToText: Story = {
  ...
  render: () => (
    <StoryProviders neoAuth config={{ experimental: { speech_to_text_model: "google/chirp-3" } } as any}>
      <div style={{ "max-height": "700px", overflow: "auto" }}>
        <ModelsTab />
      </div>
    </StoryProviders>
...
export const IndexingNeoModelPreset: Story = {
  name: "IndexingTab - Neo stale custom model fallback",
  render: () => {
    const cfg: Config = {
      indexing: {
        provider: "neo",
    ...
    return (
      <StoryProviders config={cfg}>
        <NeoEmbeddingModelsContext.Provider value={{ catalog: () => catalog }}>
          <div style={{ "max-height": "700px", overflow: "auto" }}>
            <IndexingTab />
          </div>
        </NeoEmbeddingModelsContext.Provider>
      </StoryProviders>
    )
  },
}

export const IndexingNeoCatalogLoading: Story = {
  name: "IndexingTab - Neo catalog loading",
  render: () => {
    const [saved, setSaved] = createSignal<Record<string, unknown>>({})
    const cfg: Config = {
      indexing: {},
    ...
    return (
      <>
        ...
        <pre data-testid="indexing-neo-loading-save">{JSON.stringify(saved(), null, 2)}</pre>
      </>
    )
  },
}
