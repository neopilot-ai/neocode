 ...
 * Neo gateway models remain usable whenever the provider catalog exposes them.
 */
export function isModelValid(
  ...
  if (selection.providerID !== "neo" && !connected.includes(selection.providerID)) return false
