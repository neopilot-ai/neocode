...
import { NEO_AUTO, NEO_PROVIDER_ID, parseModelString } from "../../../src/shared/provider-model"
...
export const SessionProvider: ParentComponent = (props) => {
  ...
  const [pendingNeoModel, setPendingNeoModel] = createSignal<{
    modelID?: string
    agent?: string
    after: number
  } | null>(null)
  ...
  function selectNeoModel(modelID?: string, agent?: string) {
    if (!modelID && !agent) return
    setPendingNeoModel({ ...(modelID && { modelID }), ...(agent && { agent }), after: catalog() })
    if (modelID) vscode.postMessage({ type: "requestProviders" })
  }

  const unsubNeoModel = vscode.onMessage((message: ExtensionMessage) => {
    if (message.type === "providersLoaded") {
      setCatalog((value) => value + 1)
      return
    }
    if (message.type === "selectNeoModel") selectNeoModel(message.modelID, message.agent)
  })
  onCleanup(unsubNeoModel)

  createEffect(() => {
    const pending = pendingNeoModel()
    if (!pending || agents().length === 0 || (pending.modelID && catalog() <= pending.after)) return
    setPendingNeoModel(null)
    if (pending.modelID && !provider.providers()[NEO_PROVIDER_ID]?.models[pending.modelID]) {
      console.warn("[Neo New] Ignoring unavailable Neo catalog model:", pending.modelID)
      return
    }
    if (pending.agent && !agentNames().has(pending.agent)) {
      console.warn("[Neo New] Ignoring unavailable Neo agent:", pending.agent)
      return
    }
    if (pending.agent) selectAgent(pending.agent)
    if (pending.modelID) selectModel(NEO_PROVIDER_ID, pending.modelID)
  })
  ...
  function handleExtensionMessage(message: ExtensionMessage): void {
    ...
    switch (message.type) {
      ...
      case "cloudSessionImportFailed": {
        ...
        console.error("[Neo New] Cloud session import failed:", message.error)
        break
      }
  ...
  function handlePartUpdated(
    ...
    if (!effectiveMessageID) {
      console.warn("[Neo New] Part updated without messageID:", part.id, part.type)
      return
    }
  ...
  function sendMessage(
    ...
    if (!server.isConnected()) {
      console.warn("[Neo New] Cannot send message: not connected")
      return
    }
  ...
  function sendCommand(
    ...
    if (!server.isConnected()) {
      console.warn("[Neo New] Cannot send command: not connected")
      return
    }
  ...
  function abort() {
    ...
    if (!scope) {
      console.warn("[Neo New] Cannot abort: no current or pending session")
      return
    }
  ...
  function compact() {
    if (!server.isConnected()) {
      console.warn("[Neo New] Cannot compact: not connected")
      return
    }

    const sessionID = currentSessionID()
    if (!sessionID) {
      console.warn("[Neo New] Cannot compact: no current session")
      return
    }
  ...
  function createSession() {
    if (!server.isConnected()) {
      console.warn("[Neo New] Cannot create session: not connected")
      return
    }
  ...
  function loadSessions() {
    if (!server.isConnected()) {
      console.warn("[Neo New] Cannot load sessions: not connected")
      return
    }
  ...
  function selectSession(id: string) {
    ...
    if (id.startsWith("cloud:")) {
      console.warn("[Neo New] Cannot select cloud preview session via selectSession")
      return
    }
  ...
  function selectCloudSession(cloudSessionId: string) {
    if (!server.isConnected()) {
      console.warn("[Neo New] Cannot select cloud session: not connected")
      return
    }
    const key = `cloud:${cloudSessionId}`
    agentDrafts.prune(draftSessionID())
  ...
  function deleteSession(id: string) {
    if (!server.isConnected()) {
      console.warn("[Neo New] Cannot delete session: not connected")
      return
    }
  ...
  function renameSession(id: string, title: string) {
    if (!server.isConnected()) {
      console.warn("[Neo New] Cannot rename session: not connected")
      return
    }
  ...
  function exportSessionTranscript(id: string) {
    if (!server.isConnected()) {
      console.warn("[Neo New] Cannot export session transcript: not connected")
      return
    }
    if (id.startsWith("cloud:")) {
      console.warn("[Neo New] Cannot export cloud session transcript")
      return
    }
