...
export const NeoNotifications: Component<{ sessionID?: Accessor<string | undefined> }> = (props) => {
  ...
  /**
   * Resolve suggestModelId to a neo-provider model selection.
   * Only the neo provider is supported — the model must be present in the
  ...
  const suggestedModel = createMemo(() => {
    ...
    const sel = { providerID: NEO_PROVIDER_ID, modelID: id }
  ...
  return (
    <Show when={total() > 0}>
      <div class="neo-notifications">
        <div class="neo-notifications-card">
          <div class="neo-notifications-header">
            <span class="neo-notifications-title">{current()?.title}</span>
            <Show when={total() > 1}>
              <span class="neo-notifications-nav-count">
                {safeIndex() + 1} / {total()}
              </span>
            </Show>
          </div>
          <p class="neo-notifications-message">{current()?.message}</p>
          <div class="neo-notifications-footer">
            <div class="neo-notifications-cta-group">
              <Show when={canSwitchModel()}>
                <button class="neo-notifications-action-btn" onClick={handleTryModel}>
                  {suggestedName()
              ...
              <Show when={current()?.action}>
                {(action) => (
                  <button class="neo-notifications-action-btn" onClick={() => handleAction(action().actionURL)}>
                    {action().actionText}
                  </button>
                )}
              </Show>
            </div>
            <div class="neo-notifications-next-group">
              <Show when={safeIndex() > 0}>
                <button class="neo-notifications-back-link" onClick={() => setIndex(safeIndex() - 1)}>
                  {language.t("common.goBack")}
                </button>
              </Show>
              <button class="neo-notifications-primary-btn" onClick={handleNext}>
