...
import { useI18n } from "@neocode/neo-ui/context/i18n"
...
import { NeoNotifications } from "./NeoNotifications"
...
import { useData } from "@neocode/neo-ui/context/data"
...
export const MessageList: Component<MessageListProps> = (props) => {
  ...
  // back to neo-ui's default hideDetails renderer, which never shows a
  ...
  // Extracts only the text neo-ui's tool renderers actually put on screen —
  ...
  //
  // read/glob/grep/list are the one confirmed exception: neo-ui always
  ...
  // see packages/opencode/src/neocode/todo-view.ts). Recursively collecting
  ...
  // registers for "task", overriding neo-ui's default) exactly: title is
  ...
  function taskText(part: Part & { type: "tool" }, state: ToolState): string[] {
    ...
    // neo-ui's default hideDetails task card, which never shows result
  ...
  function questionText(part: Part & { type: "tool" }, state: ToolState): string[] {
    ...
    questions.forEach((q, i) => {
      ...
      // values. Once completed, neo-ui's question renderer instead shows
    ...
    <div class="message-list-container">
      ...
      <Show when={isEmpty()}>
        <div class="welcome-header">
          ...
          <NeoNotifications sessionID={props.sessionID} />
        </div>
      </Show>
      <div
        ref={setScrollRef}
        ...
        <div ref={autoScroll.contentRef} class={isEmpty() ? "message-list-content-empty" : "message-list-content"}>
          ...
          <Show when={isEmpty() && props.readonly}>
            <div class="message-list-empty">
              <p class="neo-about-text">{language.t("session.messages.initializing")}</p>
            </div>
          </Show>
          <Show when={isEmpty() && !props.readonly}>
            {props.emptyState ? (
