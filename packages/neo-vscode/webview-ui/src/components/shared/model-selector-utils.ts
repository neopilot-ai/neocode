...
import {
  NEO_PROVIDER_ID as NEO_GATEWAY_ID,
  PROVIDER_PRIORITY as PROVIDER_ORDER,
  providerOrderIndex,
} from "../../../../src/shared/provider-model"

export { NEO_GATEWAY_ID, PROVIDER_ORDER }

export const NEO_AUTO_SMALL_IDS = new Set(["neo-auto/small", "auto-small"])
export const NEO_AUTO_EFFICIENT_ID = "neo-auto/efficient"
...
export function isAutoEfficient(model: Pick<EnrichedModel, "providerID" | "id">): boolean {
  return model.providerID === NEO_GATEWAY_ID && model.id === NEO_AUTO_EFFICIENT_ID
}
...
export function isSmall(model: Pick<EnrichedModel, "providerID" | "id">): boolean {
  return model.providerID === NEO_GATEWAY_ID && NEO_AUTO_SMALL_IDS.has(model.id)
}
...
// elsewhere, so preserve bare trailing "Free" words (e.g. "Neo Auto Free").
...
export function stripSubProviderPrefix(name: string): string {
  ...
  if (prefix.toLowerCase() === NEO_GATEWAY_ID) return name
...
export function buildTriggerLabel(
  ...
  if (resolvedName) {
    if (providerID === NEO_GATEWAY_ID) return stripSubProviderPrefix(resolvedName)
    if (providerName) return `${providerName} / ${resolvedName}`
    return resolvedName
  }
  if (raw?.providerID && raw?.modelID) {
    return raw.providerID === NEO_GATEWAY_ID ? raw.modelID : `${raw.providerID} / ${raw.modelID}`
  }
