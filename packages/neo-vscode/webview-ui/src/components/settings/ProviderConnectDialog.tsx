...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@neocode/sdk/v2/client"
    ...
    return (
      ...
      >
              ...
              <Match when={prompt.type === "select"}>
                <div style={{ display: "flex", "flex-direction": "column", gap: "4px" }}>
                  ...
                  <Show when={state.field === prompt.key && state.error}>
                    <span style={{ "font-size": "var(--neo-font-size-12)", color: "var(--vscode-errorForeground)" }}>
                      {state.error}
                    </span>
                  </Show>
                </div>
        ...
        <Show when={state.error && !state.field}>
          <div style={{ color: "var(--vscode-errorForeground)", "font-size": "var(--neo-font-size-13)" }}>
            {state.error}
          </div>
        </Show>
        <div class="dialog-confirm-actions provider-connect-actions">
          <div class="provider-connect-byok">
            {language.t("provider.connect.neoGateway.byok.prefix")}
            <a
              href="https://blog.neo.khulnasoft.com/p/neo-gateway-now-supports-byok-20-providers"
              onClick={(e) => {
                e.preventDefault()
                openExternal("https://blog.neo.khulnasoft.com/p/neo-gateway-now-supports-byok-20-providers")
              }}
              class="provider-connect-byok-link"
            >
              {language.t("provider.connect.neoGateway.byok.link")}
            </a>
            {language.t("provider.connect.neoGateway.byok.suffix")}
          </div>
