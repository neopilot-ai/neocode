...
export function providerIcon(provider: Provider | string): IconName {
  ...
  if (icon) return icon
  if (providerID === NEO_PROVIDER_ID) return validIcon("neo") ?? "synthetic"
  const fallback = validIcon(providerID)
  if (fallback) return fallback
  return "synthetic"
}

export function neoFallbackProvider(): Provider {
  return createNeoFallbackProvider()
}
...
export function providerNoteKey(provider: Provider | string) {
  ...
  if (provider === NEO_PROVIDER_ID) return "settings.providers.note.neo"
  return undefined
}
