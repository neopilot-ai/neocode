...
import { Collapsible } from "@neocode/neo-ui/collapsible"
import { useDialog } from "@neocode/neo-ui/context/dialog"
import { Icon } from "@neocode/neo-ui/icon"
import { ProviderIcon } from "@neocode/neo-ui/provider-icon"
import { Select } from "@neocode/neo-ui/select"
import { Tag } from "@neocode/neo-ui/tag"
...
import { disabledProviderOptions, providersWithNeoFallback, visibleConnectedIds } from "./provider-visibility"
import { isCustomProviderPackage, NEO_PROVIDER_ID } from "../../../../src/shared/provider-model"
...
const ProvidersTab: Component = () => {
  ...
  const popularProviders = createMemo(() => {
    ...
    return sortProviders(
      all.filter(
        (item) =>
          item.id !== NEO_PROVIDER_ID && isPopularProvider(item) && !connected.has(item.id) && !disabled.has(item.id),
      ),
    )
  })
  ...
  const providers = createMemo(() => providersWithNeoFallback(provider.providers()))
  ...
  function connectProvider(item: Provider) {
    if (item.id === NEO_PROVIDER_ID) {
      // Route Neo Gateway sign-in through the Profile view so the user sees
  ...
  return (
    <div>
      <Show when={!disabledIds().has(NEO_PROVIDER_ID)}>
        {/* Neo Gateway — always at the top, not editable */}
        <Card>
          <div
            style={{
              display: "flex",
          ...
          >
            <ProviderIcon id={providerIcon(NEO_PROVIDER_ID)} width={20} height={20} />
            <span
              style={{
                "font-size": "var(--neo-font-size-14)",
                "font-weight": "500",
      ...
      {/* Connected providers (excluding Neo) */}
