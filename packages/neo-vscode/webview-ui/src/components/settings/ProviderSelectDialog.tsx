...
import { List } from "@neocode/neo-ui/list"
...
const ProviderSelectDialog = () => {
  ...
  const items = createMemo<ProviderItem[]>(() => {
    ...
    const withNeo = all.some((item) => item.id === NEO_PROVIDER_ID) ? all : [neoFallbackProvider(), ...all]
    const available = withNeo.filter((item) => !disabled.has(item.id) && !connected.has(item.id))
            ...
            >
              <span
                style={{
                  ...
                  "line-height": "var(--neo-font-size-20)",
                  color: "var(--vscode-foreground)",
                }}
              >
                {item.name}
              </span>
              <Show when={item.id === NEO_PROVIDER_ID}>
