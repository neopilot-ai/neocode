...
export function hasSpeechToTextAccess(cfg: Cfg, auth: Readonly<Record<string, AuthState>>): boolean {
  const enabled = !cfg.enabled_providers || cfg.enabled_providers.includes(NEO_PROVIDER_ID)
  const type = auth[NEO_PROVIDER_ID]
  return enabled && !cfg.disabled_providers?.includes(NEO_PROVIDER_ID) && (type === "api" || type === "oauth")
}
