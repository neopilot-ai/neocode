...
const MARKETPLACE_DOCS = "https://neo.khulnasoft.com/docs/customize/marketplace"
const MCP_DOCS = "https://neo.khulnasoft.com/docs/automate/mcp/what-is-mcp"
...
export const InstallModal = (props: Props) => {
  ...
  const destination = (target = scope().value) => {
    const base = target === "project" ? ".neo" : "~/.config/neo"
    if (props.item.type === "mcp") return `${base}/neo.json`
    if (props.item.type === "agent") return `${base}/agents/${props.item.id}.md`
    if (target === "project") return `.neo/skills/${props.item.id}/`
    return `~/.neo/skills/${props.item.id}/`
  }
