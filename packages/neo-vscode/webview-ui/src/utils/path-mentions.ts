...
export function convertToMentionPath(path: string, cwd: string): string {
  ...
  try {
    ...
    console.error("[Neo New] Failed to decode dropped URI:", err, cleaned)
  }
 ...
 */
export const NEO_FILE_PATH_MIME = "application/x-neo-file-path"

/**
 * Extract file paths from a drop's DataTransfer.
 * Checks (in order):
 * 1. Internal relative-path drag (application/x-neo-file-path)
...
export function extractDropPaths(dt: DataTransfer): string[] | null {
  ...
  const neo = dt.getData(NEO_FILE_PATH_MIME)
  if (neo) {
    const paths = neo.split(/\r?\n/).filter((line) => line.trim() !== "")
