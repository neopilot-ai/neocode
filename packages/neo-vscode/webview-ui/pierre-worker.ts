// Neo-specific replacement for the shared `@opencode-ai/ui/pierre/worker`
...
import { LINE_DIFF_TYPE } from "@neocode/neo-ui/pierre"
import { WorkerPoolManager } from "@pierre/diffs/worker"
import { ensureNeoDiffTheme, NEO_DIFF_THEME } from "@opencode-ai/ui/pierre/neo-diff-theme"

// Register the "Neo" theme before any pool initializes. resolveThemes([theme])
// runs on the main thread during initialize() and throws "resolveTheme: No valid
// loader for Neo" if the theme name was never registered. Registering here makes
...
function uri(): string | undefined {
  ...
  return (window as { NEO_SHIKI_WORKER_URI?: string }).NEO_SHIKI_WORKER_URI
}

export function workerFactory(): Worker {
  const url = uri()
  if (!url) throw new Error("NEO_SHIKI_WORKER_URI is not set")
...
function createPool() {
  const pool = new WorkerPoolManager(
    ...
    { theme: NEO_DIFF_THEME, lineDiffType: LINE_DIFF_TYPE, preferredHighlighter: ENGINE },
  )
  void pool.initialize().catch((err) => console.warn("[Neo New] Failed to initialize Pierre worker pool", err))
  return pool
}
