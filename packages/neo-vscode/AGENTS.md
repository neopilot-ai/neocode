...
Neo Code is an open source AI coding agent platform. It ships as a CLI and editor clients that all build on the same backend. This package (`packages/neo-vscode/`) is the **VS Code extension**.

### Products and How They Relate

All products are thin clients over the **CLI** (`packages/opencode/`, published as `@neocode/cli`). The CLI is a fork of upstream [OpenCode](https://github.com/anomalyco/opencode) with Neo-specific additions (gateway auth, telemetry, migration, code review, branding). It contains the full AI agent runtime, tool execution, session management, provider integrations (500+ models), and an HTTP API server.

Every client spawns or connects to a `neo serve` process and communicates via HTTP REST + SSE using the auto-generated `@neocode/sdk`.

```
                        @neocode/cli  (packages/opencode/)
                     ┌────────────────────────────────┐
...
|---|---|---|---|
| Neo CLI (TUI) | `packages/opencode/` | Interactive terminal UI (SolidJS + OpenTUI) | In-process — TUI and server run together |
| Neo CLI (`neo run`) | `packages/opencode/` | Non-interactive headless mode for scripting | In-process — no network socket |
| **Neo VS Code Extension** | **`packages/neo-vscode/`** | VS Code extension with sidebar chat + Agent Manager | Bundles CLI binary, spawns `neo serve --port 0` as child process |

### Neo-Domain Packages

| Package | Name | Role |
|---|---|---|
| `packages/neo-vscode/` | `neo-code` | **This package.** VS Code extension. |
| `packages/neo-gateway/` | `@neocode/neo-gateway` | Auth (device flow), AI provider routing (OpenRouter), Neo API integration (profile, balance, teams) |
| `packages/neo-ui/` | `@neocode/neo-ui` | SolidJS component library (40+ components, built on `@kobalte/core`). Shared by this extension's webview and docs screenshot stories |
| `packages/neo-telemetry/` | `@neocode/neo-telemetry` | PostHog analytics + OpenTelemetry tracing for the CLI |
| `packages/neo-i18n/` | `@neocode/neo-i18n` | Translation strings (16 languages) |
| `packages/neo-docs/` | `@neocode/neo-docs` | Documentation site (Next.js + Markdoc) |

### Upstream OpenCode Packages (not Neo-specific)

| Package | Name | Role |
|---|---|---|
| `packages/opencode/` | `@neocode/cli` | Core CLI — forked from upstream OpenCode. AI agents, tools, sessions, server. |
...
The extension bundles its own CLI binary at `bin/neo` — it does NOT use a system-installed CLI. To build it:
...
The script checks for a prebuilt binary in `packages/opencode/dist/`, builds the CLI if needed, and copies it to `bin/neo`.

## Architecture

### Extension ↔ CLI Backend

The extension is a client of the CLI. Activation creates one shared `NeoConnectionService`; on its first connection, which autocomplete may prewarm, `ServerManager` spawns `bin/neo serve --port 0`, captures the dynamically assigned port from stdout, and communicates over HTTP + SSE. The current child process is reused unless it exits. A random password is generated and passed via `NEO_SERVER_PASSWORD` env var for basic auth.

```
Extension (Node.js)                          CLI Backend (child process)
┌──────────────────────────┐                ┌──────────────────────┐
│ NeoConnectionService    │── HTTP/SSE ──> │ neo serve --port 0  │
│   ├── ServerManager      │                │   Hono REST API      │
│   ├── HttpClient         │                │   SSE event stream   │
│   └── SSEClient          │                │   Session management │
│                          │                │   AI agent runtime   │
│ NeoProvider (sidebar)   │                └──────────────────────┘
│ NeoProvider (agent mgr) │
│ NeoProvider (open tabs) │
└──────────────────────────┘
```

- **`NeoConnectionService`** (`src/services/cli-backend/connection-service.ts`) is created once during extension activation and shared across the sidebar, Neo editor tabs, and Agent Manager. It owns the current server process, HTTP client, and SSE connection.
...
- The sidebar, every **Open in Tab** Neo panel, and the Agent Manager chat provider reuse this connection. Multiple **`NeoProvider`** instances subscribe to it, with SSE events filtered per-webview via a `trackedSessionIds` Set. Agent Manager terminals may use additional PTY/WebSocket channels to the same backend, not separate `neo serve` processes.
...
- CSP requires nonce for scripts and `font-src` for bundled fonts — see [`NeoProvider.ts`](src/NeoProvider.ts:777)
- HTML root has `data-theme="neo-vscode"` to activate neo-ui's VS Code theme bridge
...
3. **NeoProvider** (`src/NeoProvider.ts`): Add a `fetchAndSend*()` method using the cached message pattern, and handle the corresponding `request*` message from the webview in `handleWebviewMessage()`
...
- **Cached messages** (e.g. `cachedProvidersMessage`, `cachedAgentsMessage` in NeoProvider): Ensures webview refreshes get data immediately without waiting for a new HTTP round-trip
...
| State | No dedicated state file | `.neo/agent-manager.json` |
| Terminals | None | Dedicated VS Code terminal per session |
| Setup scripts | None | Configurable `.neo/setup-script` runs per worktree |
| Multi-version | Not supported | Up to 4 parallel worktrees with the same prompt |

### Architecture

Agent Manager local worktree sessions use the current shared `neo serve` process owned by `NeoConnectionService`; no session starts its own backend. Their CLI requests pass the worktree path as `directory`, which resolves directory-scoped backend state. Setup scripts, terminal PTYs, git subprocesses, and a separately opened VS Code window are separate process or extension-host boundaries, not per-worktree `neo serve` instances.
...
New webview features must use **`@neocode/neo-ui`** components instead of raw HTML elements with inline styles. This is a Solid.js component library built on `@kobalte/core`.

- Import via deep subpaths: `import { Button } from "@neocode/neo-ui/button"`
...
- Global styles imported via `import "@neocode/neo-ui/styles"` in [`index.tsx`](webview-ui/src/index.tsx:2)
- [`chat.css`](webview-ui/src/styles/chat.css) is being progressively migrated — when replacing a component with neo-ui, remove the corresponding CSS rules from it
- New CSS for components not yet in neo-ui goes into `chat.css` grouped by comment-delimited sections (`/* Component Name */`). Once a neo-ui equivalent exists, remove the section.
- **Check existing webview usages first**: `webview-ui/src/` and `packages/neo-ui/src/stories/` show how neo-ui components are composed. Do not rely only on the component API in isolation.
- **`data-component` and `data-slot` attributes carry CSS styling** — neo-ui uses `[data-component]` and `[data-slot]` attribute selectors, not class names. Reuse existing component slots where available so shared styles apply consistently.
- **Prefer neo-ui styles**: Always reuse existing neo-ui CSS variables, tokens, and component styles instead of writing custom CSS. If a style doesn't exist in neo-ui yet, add it there and reuse it rather than inlining or duplicating styles in the webview.
- **Icons**: neo-ui has 75+ custom SVG icons in [`packages/ui/src/components/icon.tsx`](../../packages/ui/src/components/icon.tsx). To list all available icon names: `node -e "const c=require('fs').readFileSync('../../packages/ui/src/components/icon.tsx','utf8');[...c.matchAll(/^\\s{2}[\"']?([\\w-]+)[\"']?:\\s*\x60/gm)].map(m=>m[1]).sort().forEach(n=>console.log(n))"`. Icon names use both hyphenated (`arrow-left`) and bare-word (`brain`, `console`, `providers`) keys.
...
Generated screenshot baselines live under `packages/neo-docs/public/img/screenshot-tests/` and are referenced from docs as `/docs/img/screenshot-tests/...`. If a generated VS Code visual-regression screenshot is used in docs, add the docs usage to the `DOCS` map in `tests/visual-regression.spec.ts` and keep `tests/visual-regression.spec.mts` in sync while that file exists.
...
- All debug output must be prepended with `[Neo New]` for easy filtering

## Naming Conventions

- All VSCode commands must use `neo-code.new.` prefix (not `neo-code.`)
- All view IDs must use `neo-code.new.` prefix, **except** the sidebar view which uses `neo-code.SidebarProvider` to preserve user sidebar position when upgrading from the legacy extension

## Neocode Change Markers

This package is entirely Neo-specific — `neocode_change` markers are NOT needed in any files under `packages/neo-vscode/`. The markers are only necessary when modifying shared upstream opencode files.
...
The `spawn` wrapper covers long-lived processes (e.g. `neo serve`). The `exec` wrapper covers short commands (e.g. `git`, `tar`). If you need the raw callback form of `execFile` for some reason, pass `windowsHide: true` explicitly in the options object.
