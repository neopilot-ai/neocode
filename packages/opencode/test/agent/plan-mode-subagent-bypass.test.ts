...
import { NeoTask } from "../../src/neocode/tool/task" // neocode_change
...
// neocode_change start - preserve Plan mutation ceilings across Neo task delegation
it.instance("Plan delegation preserves notebook and process mutation ceilings", () =>
  Effect.gen(function* () {
    const caller = yield* Agent.use.get("plan")
    expect(caller).toBeDefined()
    const rules = NeoTask.inherited({
      caller: caller!,
      session: { permission: [] } as unknown as Parameters<typeof NeoTask.inherited>[0]["session"],
      mcp: {},
    })
