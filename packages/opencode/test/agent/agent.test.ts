...
import { MCP } from "../../src/mcp" // neocode_change
...
const agentLayer = (flags: Partial<RuntimeFlags.Info> = {}) =>
  Agent.layer.pipe(
    ...
    Layer.provide(Layer.mock(MCP.Service)({})), // neocode_change
    Layer.provide(LocationServiceMap.layer), // neocode_change
    Layer.provide(RuntimeFlags.layer(flags)),
  )

const it = testEffect(agentLayer())
const scout = testEffect(agentLayer({ experimentalScout: true })) // neocode_change
...
it.instance("returns default native agents when no config", () =>
  Effect.gen(function* () {
    ...
    expect(names).not.toContain("scout") // neocode_change
...
it.instance("build agent has correct default properties", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(build, "interactive_terminal")).toBe("allow") // neocode_change
  }),
)
...
it.instance("plan agent denies edits except .opencode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(plan, "interactive_terminal")).toBe("deny") // neocode_change
...
it.instance("explore agent denies edit and write", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(explore, "interactive_terminal")).toBe("deny") // neocode_change
  }),
)
...
// neocode_change start - Scout is opt-in and owns repository research permissions
...
scout.instance(
  ...
  {
    config: {
      // neocode_change - Scout-backed Neo agents use the supported references config
      references: {
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".neo", "skill", "perm-skill") // neocode_change
