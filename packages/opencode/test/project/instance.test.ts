...
import { capture } from "../../src/neocode/instance" // neocode_change
...
describe("InstanceStore", () => {
  it.live("loads instance context", () =>
    Effect.gen(function* () {
      ...
      // neocode_change start - capture prefers legacy ALS, then falls back to the Effect fiber reference
  ...
  it.live("runs bootstrap with InstanceRef provided", () =>
    Effect.gen(function* () {
      ...
      expect(capture()).toBeUndefined() // neocode_change - bootstrap legacy ALS does not leak into the caller
    }),
  )
  ...
  // neocode_change start - reload disposers retain legacy instance context
  ...
  // neocode_change - InstanceStore.boot provides InstanceRef to bootstrap.run so
  // NeocodeBootstrap (and anything it forkDetaches, e.g. NeoIndexing.init) can read
  // the current directory. This regression test pins the Neo contract.
  it.live("provides InstanceRef during bootstrap for Neo bootstrap compatibility", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const store = yield* InstanceStore.Service
