...
const paid = (providers: Record<string, { models: Record<string, { cost: { input: number } }> }>) => {
  ...
  if (!item) return 0 // neocode_change - Neo drops opencode provider without apiKey/auth
...
it.effect("plugin config providers persist after instance dispose", () =>
  Effect.gen(function* () {
    ...
    const configDir = path.join(dir, ".neo") // neocode_change
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const configDir = path.join(instance.directory, ".neo") // neocode_change
    const root = path.join(configDir, "plugin")
    yield* Effect.promise(() => mkdir(root, { recursive: true }))
    yield* Effect.promise(() => markPluginDependenciesReady(configDir))
    yield* Effect.promise(() => markPluginDependenciesReady(Global.Path.config)) // neocode_change
    yield* Effect.promise(() =>
      Bun.write(
