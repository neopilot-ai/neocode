...
describe("ProviderTransform.schema - gemini non-object properties removal", () => {
  ...
  test("sanitizes gemini schemas routed through the Neo Gateway", () => {
...
describe("ProviderTransform.variants", () => {
  ...
  test("minimax via neo gateway returns instant/thinking toggle", () => {
    const model = createMockModel({
      id: "neo/minimax/minimax-m3",
      providerID: "neo",
      api: {
        id: "minimax/minimax-m3",
        url: "https://gateway.neo.khulnasoft.com",
  ...
  // neocode_change start
  describe("@neocode/neo-gateway", () => {
    test("claude models return empty variants (reasoning disabled)", () => {
      const model = createMockModel({
        id: "neo/anthropic/claude-sonnet-4",
        providerID: "neo",
        capabilities: { reasoning: false },
        api: {
          id: "anthropic/claude-sonnet-4",
    ...
    test("anthropic models in api.id return empty variants (reasoning disabled)", () => {
      const model = createMockModel({
        id: "neo/anthropic/claude-opus-4",
        providerID: "neo",
        capabilities: { reasoning: false },
        api: {
    ...
    // neocode_change start
    test("mercury-2 uses server-provided variants from neo gateway", () => {
  ...
  describe("@ai-sdk/azure", () => {
    ...
    for (const testCase of [
      { id: "o3-deep-research", efforts: ["medium"] }, // neocode_change - preserve helper exclusions on Azure
      { id: "gpt-5-pro", efforts: ["high"] }, // neocode_change - preserve helper exclusions on Azure
  ...
  describe("ProviderTransform.smallOptions", () => {
    describe("@neocode/neo-gateway", () => {
      ...
      test("google models use their default reasoning effort", () => {
        const model = createMockModel({
          id: "neo/google/gemini-2.0-flash",
          providerID: "neo",
          api: {
...
// neocode_change start - tests for reasoningSummary guard
describe("ProviderTransform.options - OpenAI Responses API params guard", () => {
  ...
  test("includes reasoningSummary for @neocode/neo-gateway", () => {
    const result = ProviderTransform.options({
      model: gpt5Model("@neocode/neo-gateway", "neo"),
      sessionID,
    })
    expect(result.reasoningSummary).toBe("auto")
  })
