...
import { Effect, Exit, Layer, Result, Schema } from "effect" // neocode_change
...
import { run as runSandbox, type Profile } from "@neocode/sandbox" // neocode_change
import { ProviderV2 } from "@opencode-ai/core/provider"
import { ModelV2 } from "@opencode-ai/core/model"

const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".neo")])), // neocode_change
})
...
const scout = testEffect(registryLayer({ flags: { experimentalScout: true } })) // neocode_change
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .neo/tool (singular)" /* neocode_change */, () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const opencode = path.join(test.directory, ".neo") // neocode_change
  ...
  it.instance("ignores non-tool exports in .neo/tool files" /* neocode_change */, () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".neo", "tool") // neocode_change
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools from .neo/tools (plural)" /* neocode_change */, () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const opencode = path.join(test.directory, ".neo") // neocode_change
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".neo", "tools") // neocode_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const plugin = path.join(opencode, "node_modules", "@neocode", "plugin") // neocode_change
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@neocode/plugin", type: "module", exports: { ".": "./dist/index.js" } }), // neocode_change
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@neocode/plugin"', // neocode_change
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".neo", "tools") // neocode_change
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
