...
import { Cause, Deferred, Effect, Exit, Fiber, Layer } from "effect" // neocode_change - Cause/Deferred for resume-hint coverage
...
import { MessageV2 } from "@/session/message-v2" // neocode_change
import type { SessionPrompt } from "../../src/session/prompt"
import { MessageID, PartID, SessionID } from "../../src/session/schema" // neocode_change - SessionID used by cost propagation tests
import { SessionRunState } from "@/session/run-state"
import { SessionStatus } from "@/session/status"
import { Provider } from "../../src/provider/provider" // neocode_change
...
const layer = (flags: Partial<RuntimeFlags.Info> = {}) =>
  Layer.mergeAll(
    ...
    Provider.defaultLayer, // neocode_change
...
// neocode_change start - stub signature + prompt body extended to persist assistant cost for propagation tests
...
describe("tool.task", () => {
  ...
  // neocode_change start - resumed children rebuild parent platform attribution after restart
  it.instance("execute preserves platform attribution when resuming a task", () =>
    Effect.gen(function* () {
      const sessions = yield* Session.Service
      const { chat, assistant } = yield* seed()
      NeoSession.setPlatformOverride(chat.id, "agent-manager")
      const child = yield* sessions.create({ parentID: chat.id, title: "Existing child" })
      NeoSession.clearPlatformOverride(child.id)
      const tool = yield* TaskTool
      const def = yield* tool.init()
      ...
      expect(NeoSession.resolvePlatform(child.id)).toBe("agent-manager")
      expect(NeoSession.resolveRoot(child.id)).toBe(chat.id)
    }),
  )
  // neocode_change end
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // neocode_change start — use arrayContaining: Neo appends inherited caller restrictions
        ...
        expect(seen?.tools).toEqual({
          ...
          todowrite: false,
          task: false, // neocode_change - Neo disallows nested subagents
          bash: false,
          read: false,
        })
      }),
    {
      config: {
        ...
        experimental: {
          ...
          openTelemetry: true, // neocode_change
        },
      },
    },
  )

  // neocode_change start - terminal child assistant errors fail the task tool boundary
  ...
  // neocode_change start - background subagent failures also surface the resumable task_id (#11620)
  ...
  // neocode_change start - completed background tasks propagate their invocation cost delta
  ...
  // neocode_change start - the background.extend() path must also propagate its run's cost delta (regression)
...
// neocode_change start - subagent cost propagation coverage (#6321)
