...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.NEO_WEBSEARCH_PROVIDER

    try {
      process.env.NEO_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.NEO_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.NEO_WEBSEARCH_PROVIDER
      else process.env.NEO_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for neo or explicit websearch provider flags", () => {
    // neocode_change
    expect(webSearchEnabled(ProviderV2.ID.neo, { exa: false, parallel: false })).toBe(true) // neocode_change
    expect(webSearchEnabled(ProviderV2.ID.opencode, { exa: false, parallel: false })).toBe(false) // neocode_change
