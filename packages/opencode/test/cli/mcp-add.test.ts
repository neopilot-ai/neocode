...
import { mkdir } from "node:fs/promises" // neocode_change
...
describe("opencode mcp add (non-interactive subprocess)", () => {
    ...
    ({ home, opencode }) =>
      Effect.gen(function* () {
        ...
        const config = yield* Effect.promise(() =>
          Bun.file(path.join(home, ".config", "neo", "neo.json")).json(), // neocode_change
        )
  ...
  cliIt.concurrent(
    "writes to NEO_CONFIG_DIR without touching the default profile",
    ({ home, opencode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* opencode.spawn(
          ...
          { env: { NEO_CONFIG_DIR: profile } },
        )
        opencode.expectExit(result, 0)

        const config = yield* Effect.promise(() => Bun.file(path.join(profile, "neo.json")).json())
        expect(config.mcp.profile).toEqual({ type: "remote", url: "https://example.com/profile" })
        expect(yield* Effect.promise(() => Bun.file(path.join(home, ".config", "neo", "neo.json")).exists())).toBe(
          false,
        )
      }),
    60_000,
