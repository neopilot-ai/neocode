  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.agentInfo?.name).toBe("Neo") // neocode_change
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.authMethods?.[0]?.id).toBe("neo-login") // neocode_change
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "neo-login" })).toMatchObject({
          // neocode_change
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.NEO_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("neo-login") // neocode_change
