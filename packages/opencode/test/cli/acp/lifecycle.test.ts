  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        const code = yield* Effect.promise(() => acp.exited).pipe(Effect.timeout(Duration.seconds(15))) // neocode_change
        expect(code).toBe(0)
      }),
    60_000,
  )
    ...
    ({ home, llm, opencode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient(
          { opencode },
          { NEO_CONFIG_CONTENT: JSON.stringify(verifierConfig(llm.url)) },
        )
