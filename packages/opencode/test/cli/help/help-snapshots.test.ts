...
// neocode_change start - describe Neo's branded CLI
// terminal sizes. The default neo TUI command is excluded —
// `neo --help` includes an ASCII banner that pulls in the install
// neocode_change end
...
function normalize(text: string): string {
  // neocode_change start - snapshot Neo help independently of lifecycle logs
  const help = text.slice(text.indexOf("neo "))
  const output = help
    .replace(/(?=INFO  \d{4}-\d{2}-\d{2}).*$/s, "")
    .replace(/ {4}(?=\[aliases: ls\])/g, "")
    .replace(/ {4}(?=\[string\] \[default: "neo\.local"\])/g, "")
  // neocode_change end
  // neocode_change start - normalize the branded help output
...
// neocode_change start - describe Neo's command list
// Top-level commands. Order matches what `neo --help` prints today;
...
// top-level help on `--help` and exits 1; not a real neo command.
// neocode_change end
const TOP_LEVEL = [
  "acp",
  "mcp",
...
// neocode_change start - name snapshots after the shipped CLI
describe("Neo CLI help-text snapshots", () => {
  // neocode_change end
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
          ...
          (argv) =>
            Effect.gen(function* () {
              ...
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`neo ${argv.join(" ")}: exit ${result.exitCode}`) // neocode_change
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`neo ${argv.join(" ")} --help`) // neocode_change
        }
