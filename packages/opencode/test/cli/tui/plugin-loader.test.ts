...
import type { TuiAttentionSoundPack } from "@neocode/plugin/tui"
...
async function load(): Promise<Data> {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const localDest = path.join(dir, ".neo", "themes", localThemeFile) // neocode_change
      const globalDest = path.join(Global.Path.config, "themes", globalThemeFile)
      const preloadedDest = path.join(dir, ".neo", "themes", preloadedThemeFile) // neocode_change
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ".neo", "themes", tmp.extra.globalThemeFile)) // neocode_change
      .then(() => true)
      .catch(() => false)
...
`,
  ...
  process.env.NEO_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
  ...
  try {
    ...
    cwd.mockRestore()
    wait.mockRestore()
    delete process.env.NEO_PLUGIN_META_FILE
  }
})
...
test("updates installed theme when plugin metadata changes", async () => {
  await using tmp = await tmpdir<{
    ...
    init: async (dir) => {
      ...
      const dest = path.join(dir, ".neo", "themes", themeFile) // neocode_change
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.NEO_PLUGIN_META_FILE!,
    )
