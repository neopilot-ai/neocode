...
`,
  ...
  const pure = process.env.NEO_PURE
  const meta = process.env.NEO_PLUGIN_META_FILE
  process.env.NEO_PURE = "1"
  process.env.NEO_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.NEO_PURE
    } else {
      process.env.NEO_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.NEO_PLUGIN_META_FILE
    } else {
      process.env.NEO_PLUGIN_META_FILE = meta
    }
  }
})
