...
describe("createTuiAttention", () => {
  ...
  test("supports focused-only requests", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "Neo", message: "focused" }]) // neocode_change
  })
  ...
  test("notification can deliver while focused when requested", async () => {
    ...
    expect(audio.playCalls).toBe(1)
    expect(renderer.notifications).toEqual([{ title: "Neo", message: "hello" }]) // neocode_change
  })
  ...
  test("notifies while blurred", async () => {
    ...
    expect(await attention.notify({ title: "Neo", message: "hello", sound: false })).toEqual({
      // neocode_change
      ok: true,
      notification: true,
      sound: false,
  ...
  test("can play sound always while notification is blurred-only", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "Neo", message: "hello again" }]) // neocode_change
  })
  ...
  test("plays named sounds from the active sound pack", async () => {
    ...
    dispose()
    expect(attention.soundboard.current()).toBe("neo.default") // neocode_change
  })
