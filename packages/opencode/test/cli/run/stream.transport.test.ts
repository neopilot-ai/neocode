...
import { NeoClient, type GlobalEvent } from "@neocode/sdk/v2"
...
type GlobalEventStream = Awaited<ReturnType<NeoClient["global"]["event"]>>["stream"]
type SessionMessage = NonNullable<Awaited<ReturnType<NeoClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<NeoClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<NeoClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    globalEvent?: NeoClient["global"]["event"]
    promptAsync?: NeoClient["session"]["promptAsync"]
    status?: NeoClient["session"]["status"]
    messages?: NeoClient["session"]["messages"]
    children?: NeoClient["session"]["children"]
    permissions?: NeoClient["permission"]["list"]
    questions?: NeoClient["question"]["list"]
  } = {},
) {
  const client = new NeoClient()

  const globalEvent: NeoClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: NeoClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: NeoClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: NeoClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: NeoClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: NeoClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: NeoClient["question"]["list"] = input.questions ?? (() => ok([]))
