...
import { Git } from "../../src/git" // neocode_change
import {
...
import { isIndexingPlugin } from "@neocode/neo-indexing/detect" // neocode_change
import { isAtomicChatPlugin } from "@/neocode/atomic-chat-feature" // neocode_change
...
const schemaConfig = (config: object) => ({ $schema: "https://app.neo.khulnasoft.com/config.json", ...config }) // neocode_change
...
const managedConfigDir = process.env.NEO_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.NEO_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.NEO_CONSOLE_TOKEN
  else process.env.NEO_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})

const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  FSUtil.use.writeWithDirs(path.join(managedConfigDir, filename ?? "neo.json"), JSON.stringify(settings)) // neocode_change
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".neo"), schemaConfig(input.local)) : undefined, // neocode_change
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* FSUtil.use.readFileString(path.join(dir, "neo.jsonc")) // neocode_change
      expect(content).toContain('"$schema": "https://app.neo.khulnasoft.com/config.json"') // neocode_change
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
  ),
)

it.effect("does not create global config when NEO_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
...
// neocode_change start
it.instance("preserves Neo provider free model metadata", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.neo.khulnasoft.com/config.json",
    ...
    const config = yield* Config.use.get()
    const model = config.provider?.neo?.models?.["free-e2e"]
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    // neocode_change - upstream hardcodes project config to config.json; Neo writes to neo.json
    ...
    const writtenConfig = yield* FSUtil.use.readJson(path.join(test.directory, "neo.json")) // neocode_change
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* FSUtil.use.readJson(path.join(dir, "neo.json")) // neocode_change
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* FSUtil.use.writeWithDirs(
      // neocode_change start
      path.join(test.directory, "neo.jsonc"),
      `{
        // This is a comment
        "$schema": "https://app.neo.khulnasoft.com/config.json",
        "model": "test/model",
...
it.instance("jsonc overrides json in the same directory", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      {
        $schema: "https://app.neo.khulnasoft.com/config.json", // neocode_change
        model: "base",
        username: "base",
      },
      "neo.jsonc", // neocode_change
    )
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.neo.khulnasoft.com/config.json", // neocode_change
      model: "override",
...
// neocode_change start
it.instance("prefers .neo directory config over legacy .neocode", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(path.join(test.directory, ".neocode"), {
      $schema: "https://app.neo.khulnasoft.com/config.json",
      model: "legacy/model",
    })
    yield* writeConfigEffect(path.join(test.directory, ".neo"), {
      $schema: "https://app.neo.khulnasoft.com/config.json",
      model: "new/model",
    })
...
// neocode_change start - project config is untrusted: {env:} rejected; {file:} confined to the project root
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:NEO_CONSOLE_TOKEN}" } } },
        }),
      ),
...
it.instance("reports warning for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(path.join(test.directory, "neo.json"), "{ invalid json }")
    yield* Config.use.get()
    const issues = yield* Config.Service.use((svc) => svc.warnings())
    expect(issues.length).toBeGreaterThan(0)
  }),
...
// neocode_change start
it.instance("loads config from .neo directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".neo", "agent", "test.md"), // neocode_change
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".neo", "agent", "ordered.md"), // neocode_change
      `---
permission:
  bash: allow
  "*": deny
...
// neocode_change start
it.instance("loads agents from .neo/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".neo", "agents", "helper.md"), // neocode_change
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".neo", "agents", "nested", "child.md"), // neocode_change
      `---
model: test/model
mode: subagent
---
...
// neocode_change start
it.instance("loads commands from .neo/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".neo", "command", "hello.md"), // neocode_change
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".neo", "command", "nested", "child.md"), // neocode_change
      `---
...
// neocode_change start
it.instance("loads commands from .neo/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".neo", "commands", "hello.md"), // neocode_change
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".neo", "commands", "nested", "child.md"), // neocode_change
      `---
description: Nested command
---
Nested command template`,
...
// neocode_change start
it.instance("prefers .neo commands over legacy .neocode commands", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".neocode", "command", "hello.md"),
      `---
...
Hello from legacy command`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".neo", "command", "hello.md"),
      `---
...
it.instance("updates config and writes to file", () =>
  Effect.gen(function* () {
    ...
    const writtenConfig = yield* FSUtil.use.readJson(
      path.join(test.directory, ".neo", "neo.jsonc"), // neocode_change
    )
...
it.effect("does not try to install dependencies in read-only NEO_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("NEO_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
)

it.effect("installs dependencies in writable NEO_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".neo", "agent", "helper.md"), // neocode_change
      `---
model: test/model
mode: subagent
---
...
it.effect("keeps plugin origins aligned with merged plugin list", () =>
  withConfigTree(
    ...
    Effect.gen(function* () {
      ...
      // neocode_change start - bundled plugins intentionally have no external plugin origins
...
// neocode_change - Note: preload.ts sets NEO_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("permission config preserves user key order", () =>
  ...
  Effect.gen(function* () {
    const test = yield* TestInstance
    // neocode_change start — isolate from global config to prevent cross-test contamination
...
// neocode_change start - regression for `env` alias on local MCP entries
...
it.instance("project config can override MCP server enabled status", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    // neocode_change - base config in .json, override in .jsonc (jsonc loads second and wins)
...
it.instance("MCP config deep merges preserving base config properties", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    // neocode_change - base config in .json, override in .jsonc (jsonc loads second and wins)
    // neocode_change - Base config with full MCP definition
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.neo.khulnasoft.com/config.json", // neocode_change
      mcp: {
        myserver: {
    ...
    // neocode_change - Override just enables it, should preserve other properties
    yield* writeConfigEffect(
      test.directory,
      {
        $schema: "https://app.neo.khulnasoft.com/config.json", // neocode_change
...
// neocode_change start
it.instance("local .neo config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.neo.khulnasoft.com/config.json", // neocode_change
    ...
    yield* writeConfigEffect(
      path.join(test.directory, ".neo"), // neocode_change
      {
        $schema: "https://app.neo.khulnasoft.com/config.json", // neocode_change
        mcp: {
          docs: {
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    const file = path.join(tmp.path, "neo.json") // neocode_change
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.neo/plugin/oh-my-opencode.js"] // neocode_change
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.neo/plugin/demo.ts", "file:///project/.neo/plugin/demo.ts"] // neocode_change

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.neo/plugin/demo.ts"]) // neocode_change
  })
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".neo", "plugin", "my-plugin.js"), // neocode_change
          "export default {}",
        )
...
describe("NEO_DISABLE_PROJECT_CONFIG", () => {
  // neocode_change start
  ...
  it.instance("skips project .neo directory when flag is set", () =>
    withProcessEnv(
      "NEO_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".neo", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { NEO_CONFIG_DIR: undefined, NEO_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "NEO_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        ...
        yield* withProcessEnvs(
          { NEO_DISABLE_PROJECT_CONFIG: "true", NEO_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed NEO_PERMISSION JSON used to crash
...
describe("NEO_PERMISSION env var", () => {
  it.instance("does not crash when NEO_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "NEO_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("NEO_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in NEO_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
  ...
  it.instance("substitutes {file:} tokens in NEO_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
