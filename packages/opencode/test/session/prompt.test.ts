...
import { Bus } from "@/bus" // neocode_change - ToolRegistry retains the Neo bus dependency
import { FetchHttpClient } from "effect/unstable/http"
// neocode_change start
import { expect, spyOn } from "bun:test"
import { Telemetry } from "@neocode/neo-telemetry"
import { legacyReviewMessage } from "../../src/neocode/review/command"
// neocode_change end
...
import { Auth } from "../../src/auth" // neocode_change
...
import { NeoSession } from "../../src/neocode/session" // neocode_change
import { Suggestion } from "../../src/neocode/suggestion" // neocode_change - accept suggestion in telemetry test
...
function makePrompt(input?: { processor?: "blocking" }) {
  const deps = Layer.mergeAll(
    ...
    LLM.defaultLayer,
    Env.defaultLayer,
    input?.processor === "blocking" ? fastAgents : AgentSvc.defaultLayer, // neocode_change
    ...
    Bus.layer, // neocode_change - satisfy the Neo ToolRegistry dependency
  ...
  const registry = ToolRegistry.layer.pipe(
    ...
    Layer.provide(RepositoryCache.defaultLayer), // neocode_change - RepoCloneTool dependency
    Layer.provide(Format.defaultLayer),
    Layer.provide(RuntimeFlags.layer({ experimentalEventSystem: true })),
    Layer.provide(Auth.defaultLayer), // neocode_change
  ...
  return SessionPrompt.layer.pipe(
    ...
    Layer.provideMerge(question), // neocode_change - SessionPrompt dismisses pending questions
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://app.neo.khulnasoft.com/config.json", ...config }), // neocode_change
  )
})
...
// neocode_change start - wait for the runner state that cancel observes instead of session status
...
// neocode_change start - replacement prompts unblock pending Question service requests
noLLMServer.instance(
...
// neocode_change start - cover user image normalization before persistence
noLLMServer.instance(
...
noLLMServer.instance("prompt tools replace matching rules and preserve existing restrictions", () =>
  Effect.gen(function* () {
    ...
    // neocode_change start - Neo preserves existing restrictions that the new prompt does not override
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const tool = yield* pollWithTimeout(
        Effect.gen(function* () {
          const msgs = yield* MessageV2.filterCompactedEffect(chat.id)
          const assistant = msgs.findLast((item) => item.info.role === "assistant" && item.info.agent === "code") // neocode_change
        ...
        "10 seconds", // neocode_change - allow loaded Darwin runners to persist the tool transition
      )
  ...
  20_000, // neocode_change
)

// neocode_change start - child task failures stay tool errors so the parent can recover
it.instance(
...
it.instance(
  ...
  () =>
    // neocode_change start - hold the model response instead of cancelling an infinite stream
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig(providerCfg)
...
it.instance(
  ...
  10_000, // neocode_change - Windows CI can take longer to cancel the live loop
)
...
unix(
  ...
  10_000, // neocode_change - upstream's 3s deadline flakes under CI shard load (observed 3048ms on macOS)
)
...
raceNoLLMServer.instance(
  ...
  { config: cfg },
  10_000, // neocode_change - cancellation tree cleanup can exceed 3s under macOS CI shard load
)
...
// neocode_change start - handleSubtask propagates child session cost to wrapper (#6321)
it.instance(
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.yieldNow // neocode_change - let the queued caller join without a wall-clock race
  ...
  { git: true },
  10_000, // neocode_change - Windows CI can take longer to cancel queued live loops
)
...
it.instance(
  ...
  () =>
    // neocode_change start - gate the failing stream so both callers join the same run
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig(providerCfg)
      const gate = yield* Deferred.make<void>()
      const prompt = yield* SessionPrompt.Service
...
it.instance(
  ...
  10_000, // neocode_change - loaded CI runners can exceed 3s for two prompt turns
)
...
it.instance(
  ...
  10_000, // neocode_change - Windows CI can take longer to enter and cancel the live loop
)
...
// neocode_change start - verify shell v2 events correlate with the persisted tool part
unixNoLLMServer(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const messages = yield* SessionV2.Service.use((session) => session.messages({ sessionID: chat.id })).pipe(
        Effect.provide(SessionV2.defaultLayer), // neocode_change - use the complete upstream v2 session layer
      )
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.yieldNow // neocode_change - give the queued loop a scheduler turn instead of a wall-clock window
  ...
  { git: true },
  30_000, // neocode_change - Windows CI process startup can exceed 3s
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const a = yield* prompt.loop({ sessionID: chat.id }).pipe(Effect.forkChild)
      const b = yield* prompt.loop({ sessionID: chat.id }).pipe(Effect.forkChild)
      yield* Effect.yieldNow // neocode_change - give the queued loops a scheduler turn instead of a wall-clock window
...
unixNoLLMServer(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* waitForBusy(chat.id)
      // neocode_change start - busy is set before shell persistence completes
      yield* pollWithTimeout(
        sessions
          .messages({ sessionID: chat.id })
          .pipe(
      ...
      // neocode_change start - wait until the loop reaches the queued-run handoff
      const opened = yield* Deferred.make<void>()
      yield* Effect.acquireRelease(
        Effect.sync(() =>
          Bus.subscribe(NeoSession.Event.TurnOpen, (event) => {
...
// neocode_change start - expand configured Neo references once per prompt
noLLMServer.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(agents.map((agent) => agent.name)).toEqual(["code"]) // neocode_change
    }),
  {
    config: {
      ...cfg,
...
// neocode_change start - deduplicate configured Neo references by source identity
noLLMServer.instance(
...
noLLMServer.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(synthetic.some((part) => part.text.includes("Called the Read tool"))).toBe(true) // neocode_change
      expect(files).toHaveLength(1) // neocode_change - directory attachment is expanded, not denied
...
// neocode_change start - Agent Manager records a model-less synthetic prompt after forking
noLLMServer.instance(
...
// neocode_change start - Neo review command behavior
noLLMServer.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit)) {
        ...
        expect(NamedError.Unknown.isInstance(err)).toBe(true)
        if (NamedError.Unknown.isInstance(err)) {
          expect(err.data.message).toContain("code") // neocode_change - "build" renamed to "code"
        }
      }
    }),
  30_000,
