...
import { USER_AGENT } from "../../src/installation" // neocode_change
import { ProviderV2 } from "@opencode-ai/core/provider"
import { ModelV2 } from "@opencode-ai/core/model"

type ConfigModel = NonNullable<NonNullable<NonNullable<ConfigV1.Info["provider"]>[string]>["models"]>[string] // neocode_change
...
describe("session.llm.ai-sdk adapter", () => {
  ...
  // neocode_change start - preserve AI SDK raw usage for Neo provider billing
  ...
  test("preserves providerMetadata on step-finish so Anthropic cache writes survive getUsage", async () => {
    ...
    expect(stepFinish.providerMetadata).toEqual({
      ...
      neocode: { routedModelID: "claude-3-5-sonnet" },
    })
    // neocode_change end
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(headers.get("User-Agent") ?? "").toMatch(/^Neo-Code\//) // neocode_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const capture = yield* Effect.promise(() => request)
        const body = capture.body
        const headers = capture.headers // neocode_change

        expect(capture.url.pathname.endsWith("/messages")).toBe(true)
        expect(headers.get("User-Agent")?.split(" ")[0]).toBe(USER_AGENT) // neocode_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // neocode_change start - auth keys use the same Google API key header as Standard keys
