...
function fake(
  ...
  return {
    ...
    metadata: Effect.fn("TestSessionProcessor.metadata")(() => Effect.void), // neocode_change
...
type CompactionProcessOptions = {
  ...
  flags?: Partial<RuntimeFlags.Info> // neocode_change
  snapshot?: Layer.Layer<Snapshot.Service> // neocode_change
}
...
function compactionProcessLayer(options?: CompactionProcessOptions) {
  ...
  return Layer.mergeAll(SessionCompaction.layer.pipe(Layer.provide(processor)), processor, events, status).pipe(
    ...
    Layer.provide(options?.snapshot ?? Snapshot.defaultLayer), // neocode_change
    ...
    Layer.provide(RuntimeFlags.layer({ experimentalEventSystem: true, ...options?.flags })), // neocode_change
    Layer.provide(EventV2Bridge.defaultLayer),
  )
}

// neocode_change start - keep retry-backoff cancellation tests independent of git snapshot cleanup latency
...
describe("session.compaction.process", () => {
  ...
  // neocode_change start - configured output ceiling controls automatic tail budgeting
  itCompaction.instance(
  ...
  itCompaction.instance(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        // neocode_change start - Effect 4.x timeout throws TimeoutError on the error
      ...
      }).pipe(withCompaction({ llm: stub.layer, snapshot: snap })) // neocode_change
    },
    {}, // neocode_change - isolate cancellation from git setup
  )
  ...
  itCompaction.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        }).pipe(withCompaction({ plugin: plugin(ready), snapshot: snap })) // neocode_change - avoid git snapshot startup
      }),
    {}, // neocode_change - isolate cancellation from git setup
  )
...
describe("SessionNs.getUsage", () => {
  ...
  // neocode_change start - Test for OpenRouter provider cost
  ...
  test("uses upstreamInferenceCost for Neo provider", () => {
    const model = createModel({
      context: 100_000,
      output: 32_000,
      cost: {
    ...
    const provider = { id: "neo" } as Provider.Info
    const result = SessionNs.getUsage({
      model,
      provider,
      usage: usage({ inputTokens: 1_000_000, outputTokens: 100_000, totalTokens: 1_100_000 }),
    ...
    // Should use upstreamInferenceCost for Neo provider (BYOK)
  ...
  test("uses regular cost when upstreamInferenceCost is missing for Neo", () => {
    const model = createModel({
      context: 100_000,
      output: 32_000,
      cost: {
    ...
    // When upstream cost is missing for Neo, fall back to regular cost field
    expect(result.cost).toBe(0.01)
  })

  // Tests for Anthropic Messages / OpenAI Responses / Vercel AI Gateway cost extraction
  // live in test/neocode/provider-cost.test.ts (neocode_change).
  // neocode_change end
