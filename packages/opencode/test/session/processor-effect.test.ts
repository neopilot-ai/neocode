...
import { SessionNetwork } from "../../src/session/network" // neocode_change
import { Bus } from "../../src/bus" // neocode_change
...
// neocode_change start - exercise non-default output token ceilings in the processor
...
// neocode_change start - configured output ceiling must reach finish-step overflow accounting
...
it.live("session.processor effect tests reset reasoning state across retries", () =>
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        const { processors, session, provider } = yield* boot()
        // neocode_change start — auto-reply to network reconnection prompts triggered by reset()
        ...
        offAsk() // neocode_change — cleanup subscriber
      }),
    { config: (url) => providerCfg(url) },
  ),
)
