...
const NEO_FIXTURES_DIR = path.join(FIXTURES_DIR, "neocode") // neocode_change

const zenURL = (connection: string) => `https://console.opencode.ai/proxy/connections/${connection}/v1` // neocode_change
...
const cloneModel = (model: ModelsDev.Provider["models"][string]) => {
  ...
  if (typeof experimental === "boolean") {
    ...
    return cloned as NonNullable<NonNullable<NonNullable<ConfigV1.Info["provider"]>[string]>["models"]>[string] // neocode_change
  }
  ...
  return rest as NonNullable<NonNullable<NonNullable<ConfigV1.Info["provider"]>[string]>["models"]>[string] // neocode_change
}
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.NEO_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    canRecord: () => Boolean(envValue("NEO_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("NEO_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  {
    id: "opencode-proxy",
    name: "OpenCode proxy", // neocode_change
    providerID: ProviderV2.ID.opencode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["opencode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.NEO_RECORD_CONSOLE_TOKEN && process.env.NEO_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.opencode,
        name: "OpenCode Zen",
        env: ["NEO_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.NEO_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.NEO_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.NEO_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["opencode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("NEO_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("NEO_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("NEO_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const canRun = (scenario: RecordedScenario) =>
  ...
  shouldRecord
    ? scenario.canRecord()
    : HttpRecorderInternal.hasCassetteSync(scenario.cassette, { directory: NEO_FIXTURES_DIR })
  // neocode_change end

const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set NEO_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  const recordedHttp = shouldRecord
    ? HttpRecorderInternal.cassetteLayer(scenario.cassette, {
        directory: NEO_FIXTURES_DIR, // neocode_change
        mode: "record",
        metadata,
        redactor: HttpRecorderInternal.Redactor.make(redact),
      })
    : HttpRecorder.http(scenario.cassette, { directory: NEO_FIXTURES_DIR, metadata, redact }) // neocode_change
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "https://app.neo.khulnasoft.com/config.json", ...scenario.config(model) }), // neocode_change
    ),
  )
