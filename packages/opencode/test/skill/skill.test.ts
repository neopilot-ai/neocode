...
const itWithoutClaudeCodeSkills = testEffect(Layer.mergeAll(skills(false, true), node, testInstanceStoreLayer)) // neocode_change
...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.NEO_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.NEO_TEST_HOME = prev
      }),
  )

const discovered = <T extends { location: string }>(list: readonly T[]) =>
  list.filter((skill) => ![Skill.BUILTIN_LOCATION, "<built-in>"].includes(skill.location)) // neocode_change

describe("skill", () => {
  // neocode_change start
  it.live("discovers skills from .neo/skill/ directory", () =>
    // neocode_change end
      ...
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".neo", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
...
`,
          ...
          const list = discovered(yield* skill.all()) // neocode_change
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".neo", "skill", "dir-skill", "SKILL.md"), // neocode_change: .neo is primary
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".neo", "skill", "dir-skill")) // neocode_change: .neo is primary
  ...
  // neocode_change start
  it.live("discovers multiple skills from .neo/skill/ directory", () =>
    // neocode_change end
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".neo", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".neo", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".neo", "skill", "no-frontmatter", "SKILL.md"), // neocode_change: .neo is primary
              `# No Frontmatter
...
`,
          ...
          const skill = yield* Skill.Service
          expect(discovered(yield* skill.all())).toEqual([]) // neocode_change
        }),
      { git: true },
    ),
  )
  ...
  it.live("discovers skills without descriptions", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          // neocode_change start - load .neo skills without falling back to .opencode
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".neo", "skill", "manual-skill", "SKILL.md"),
                `---
name: manual-skill
---
...
`,
              ...
              // neocode_change start
              Bun.write(
                path.join(dir, ".neo", "skill", "opencode-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .neo/skill directory.
---
...
`,
              ...
              // neocode_change start
              Bun.write(
                path.join(dir, ".neo", "skill", "agent-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .neo/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".neo", "skills", "agent-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .neo/skills directory.
---
