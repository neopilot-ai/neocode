// Unit tests for NeoSessionPrompt history-trim / media-strip helpers.
...
import { NeoSessionMessageOrder } from "../../src/neocode/session/message-order"
...
describe("NeoSessionPrompt.hasCompletedSummary", () => {
  test("returns false for empty array", () => {
    expect(NeoSessionPrompt.hasCompletedSummary([])).toBe(false)
  })
  ...
  test("returns false when only summary has an error", () => {
    ...
    expect(NeoSessionPrompt.hasCompletedSummary(msgs)).toBe(false)
  })
  ...
  test("returns true when summary has finish and no error", () => {
    ...
    expect(NeoSessionPrompt.hasCompletedSummary(msgs)).toBe(true)
  })
})
...
describe("MessageV2.latest", () => {
  test("selects chronological state after retained pre-compaction tail", () => {
    ...
    const state = NeoSessionMessageOrder.latest(msgs)
  ...
  test("keeps queued subtasks moved after a chronologically later assistant", () => {
    ...
    NeoSessionMessageOrder.annotate([active, queued, done])

    const state = NeoSessionMessageOrder.latest([active, done, queued])
...
describe("NeoSessionPrompt.trimBeforeLastSummary", () => {
  ...
  test("trims to summary's text-user parent (reference-session bug)", () => {
    ...
    const result = NeoSessionPrompt.trimBeforeLastSummary(filtered)
  ...
  test("keeps the newest summary when retained history contains an older summary", () => {
    ...
    expect(NeoSessionPrompt.trimBeforeLastSummary(filtered)).toBe(filtered)
  })
  ...
  test("is idempotent", () => {
    ...
    const first = NeoSessionPrompt.trimBeforeLastSummary(msgs)
    const second = NeoSessionPrompt.trimBeforeLastSummary(first)
...
describe("NeoSessionPrompt.stripHistoricalMedia", () => {
  test("replaces image file part in historical user message with placeholder text", () => {
    ...
    const result = NeoSessionPrompt.stripHistoricalMedia(msgs)
  ...
  test("does not mutate the input", () => {
    ...
    const result = NeoSessionPrompt.stripHistoricalMedia(input)
...
describe("NeoSessionPrompt.maybeStripHistoricalMedia", () => {
  test("returns input unchanged when no completed summary exists", () => {
    const msgs = [
      user("msg_hist", [filePart("msg_hist", "image/png", "hist.png")]),
      user("msg_last", [textPart("msg_last", "follow-up")]),
    ]
    const result = NeoSessionPrompt.maybeStripHistoricalMedia(msgs)
    expect(result).toBe(msgs)
  })
