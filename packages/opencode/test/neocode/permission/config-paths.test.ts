...
import { ConfigProtection } from "../../../src/neocode/permission/config-paths"
...
describe("ConfigProtection.isRequest", () => {
  ...
  const legacy = NeocodePaths.globalDirs().map((d) => path.resolve(d))
  ...
  test("returns false for file-tool external_directory targeting global config", () => {
    const result = ConfigProtection.isRequest({
      permission: "external_directory",
      patterns: [config + "/*"],
      metadata: { filepath: path.join(config, "neo.json"), parentDir: config },
    })
  ...
  test("returns true for edit targeting root config files", () => {
    for (const file of ["neo.json", "neo.jsonc", "AGENTS.md"]) {
      const result = ConfigProtection.isRequest({
        permission: "edit",
        patterns: [file],
      })
  ...
  test("protects package lock files in project config directories", () => {
    for (const file of [".neo/package-lock.json", ".neocode/package-lock.json"]) {
  ...
  test("protects a combined source and config lockfile edit", () => {
    expect(
      ConfigProtection.isRequest({
        permission: "edit",
        patterns: ["src/app/layout.tsx", ".neo/package-lock.json", ".neocode/package-lock.json"],
        metadata: {
          filepath: "src/app/layout.tsx, .neo/package-lock.json, .neocode/package-lock.json",
        },
      }),
    ).toBe(true)
  })
...
describe("ConfigProtection.isGlobalSkillRequest", () => {
  const roots = [Global.Path.config, ...NeocodePaths.globalDirs()]
  ...
  test("rejects glob characters in the canonical rule", async () => {
    ...
    const skill = path.join(root, "neo", "skills", "unsafe-root")
