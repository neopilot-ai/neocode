...
afterAll(async () => {
  ...
  for (const file of ["neo.jsonc", "neo.json", "config.json", "opencode.json", "opencode.jsonc"]) {
    ...
    withDir({ git: true }, () =>
      Effect.gen(function* () {
        const fiber = yield* ask({
          ...
          permission: "bash",
          patterns: ["neo-permission-8353 test"],
          metadata: { rules: ["neo-permission-8353 *", "neo-permission-8353 test"] },
          always: ["neo-permission-8353 *", "neo-permission-8353 test"],
          ruleset: [],
        }).pipe(Effect.forkScoped)
        ...
        yield* saveAlwaysRules({
          ...
          approvedAlways: ["neo-permission-8353 test"],
        })
        ...
        expect(cfg.permission?.bash).toMatchObject({ "neo-permission-8353 test": "allow" })
        expect(cfg.permission?.bash).not.toMatchObject({ "neo-permission-8353 *": "allow" })

        const broad = yield* ask({
          id: PermissionV1.ID.make("permission_saved_always_broad"),
          sessionID: SessionID.make("session_saved_always"),
          permission: "bash",
          patterns: ["neo-permission-8353 install"],
          metadata: {},
          always: [],
          ruleset: [],
        }).pipe(Effect.forkScoped)
