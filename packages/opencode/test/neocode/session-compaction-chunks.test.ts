...
import { NeoCompactionChunks } from "../../src/neocode/session/compaction-chunks"
import { NeoSessionCompaction } from "../../src/neocode/session/compaction"
...
const dbfile = path.join(os.tmpdir(), `neo-compaction-chunks-${process.pid}-${crypto.randomUUID()}.db`)
...
describe("NeoCompactionChunks", () => {
  test("splits oversized history into chronological chunks", async () => {
    ...
    const chunks = await Effect.runPromise(NeoCompactionChunks.split({ messages, model, size: 2_000 }))
  ...
  test("uses runtime output cap for fallback selection and chunk budget", () => {
    ...
    expect(NeoCompactionChunks.needed({ cfg, model, tokens: 5_000, outputTokenMax })).toBe(false)
    expect(NeoCompactionChunks.budget({ cfg, model, outputTokenMax })).toBe(5_692)
  })
