...
import { assertNetwork, assertWrite, enabled as sandboxed } from "@neocode/sandbox"
import { Bus } from "@/bus"
import { Config } from "@/config/config"
import * as Network from "@/neocode/sandbox/network"
import * as SandboxPolicy from "@/neocode/sandbox/policy"
import { SandboxStore } from "@/neocode/sandbox/store"
...
test("restores the session snapshot after a backend restart", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "neo-sandbox-restart-"))
  ...
  const env = {
    ...process.env,
    NEO_TEST_HOME: path.join(root, "home"),
...
posix("canonicalizes a symlinked policy state root", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "neo-sandbox-state-link-"))
  const target = path.join(root, "real-state")
  const link = path.join(root, "state")
  await fs.mkdir(target)
  await fs.symlink(target, link)
  const script = 'import { SandboxStore } from "@/neocode/sandbox/store"; console.log(SandboxStore.root)'
  ...
  try {
    ...
    expect(result.stdout.toString().trim().split("\n").at(-1)).toBe(
      path.join(await fs.realpath(target), "neo-sandbox-policy"),
    )
  } finally {
    await fs.rm(root, { recursive: true, force: true })
  }
...
linux("reports configured network namespace availability", async () => {
  const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "neo-sandbox-status-"))
  ...
  try {
    const result = Bun.spawnSync([process.execPath, "-e", script], {
      cwd: import.meta.dir,
      env: { ...process.env, NEO_BWRAP_PATH: helper, NEO_SERVER_PASSWORD: "sandbox-test" },
      stdout: "pipe",
      stderr: "pipe",
      windowsHide: true,
    })
...
it.instance("snapshots the primary neo config for the session lifetime", () =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const password = Flag.NEO_SERVER_PASSWORD
      Flag.NEO_SERVER_PASSWORD = "sandbox-test"
      return password
    }),
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const file = path.join(test.directory, "neo.json")
    ...
    (password) => Effect.sync(() => (Flag.NEO_SERVER_PASSWORD = password)),
  ),
)
