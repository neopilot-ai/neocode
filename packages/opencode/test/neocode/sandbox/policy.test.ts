...
import { assertWrite, run as runSandbox } from "@neocode/sandbox"
import { Effect, Exit } from "effect"
import { profile } from "@/neocode/sandbox/policy"
import { SandboxPreference } from "@/neocode/sandbox/preference"
...
function fixture() {
  return tmpdir<Dirs>({
    init: async (root) => {
      ...
      const a = path.join(main, ".neo", "worktrees", "a")
      const b = path.join(main, ".neo", "worktrees", "b")
...
function expected(...directories: string[]) {
  return new Set([...directories, ...neo])
}
...
describe("sandbox policy", () => {
  ...
  test("keeps Neo state writable without exposing sandbox policy state", async () => {
    await using tmp = await fixture()
    const dirs = tmp.extra
