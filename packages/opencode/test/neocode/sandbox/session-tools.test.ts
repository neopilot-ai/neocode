...
import * as ToolNetwork from "@/neocode/sandbox/network"
import { MCP } from "@/mcp"
...
const sessions = Layer.mock(Session.Service)({
  get: () => Effect.succeed(session("/workspace/project/.neo/worktrees/a")),
})
...
mac("confines model-originated file mutations to the active worktree", () =>
  Effect.gen(function* () {
    ...
    const neo = yield* call(write, { filePath: state, content: "state" }, "call-state").pipe(Effect.exit)
    ...
    expect(Exit.isSuccess(neo)).toBe(true)
