...
import * as SandboxActivation from "@/neocode/sandbox/activation"
...
describe("sandbox session cleanup", () => {
  ...
  it.live("creates honor the neocode.sandbox metadata over the config default", () =>
    Effect.gen(function* () {
      const sessions = yield* Session.Service
      ...
      const session = yield* provideInstance(dir)(
        sessions.create({ title: "sandbox-explicit", metadata: { "neocode.sandbox": { enabled: true, version: 0 } } }),
      )
