...
import { AgentManagerTool } from "../../src/neocode/tool/agent-manager"
import { AgentManagerEvent, type AgentManagerStart } from "../../src/neocode/agent-manager/event"
import { AgentManager } from "../../src/neocode/agent-manager/service"
...
const providers = {
  test: {
    ...
    models: {
      ...
      // "Shared" is also offered by the neo provider, to exercise provider resolution.
  ...
  neo: {
    ...
    name: "Neo Gateway",
    models: {
      "neo/shared": { id: "neo/shared", providerID: "neo", name: "Shared", variants: { low: {} } },
      "neo/only": { id: "neo/only", providerID: "neo", name: "Gateway Only", variants: { low: {} } },
    },
  } as unknown as Provider.Info,
  zeta: {
    id: "zeta",
...
// Default provider is `test`, so resolution should prefer test, then neo, then others.
...
describe("agent_manager tool", () => {
  ...
  test("inherits the latest invoking model and variant when omitted", async () => {
    const task = await publish(runtime, { prompt: "Fix" }, [
      message("msg_current", "neo", "neo/shared", "low", 2),
      message("msg_old", "test", "reasoning/model", "high", 1),
    ])

    expect(String(task?.model?.providerID)).toBe("neo")
    expect(String(task?.model?.modelID)).toBe("neo/shared")
  ...
  test("explicit model and variant override the invoking selection", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: "test/reasoning/model", variant: "high" }, [
      message("msg_current", "neo", "neo/shared", "low"),
    ])
  ...
  test("does not inherit a variant when only the model is overridden", async () => {
    ...
    expect(String(task?.model?.providerID)).toBe("neo")
    expect(String(task?.model?.modelID)).toBe("neo/only")
    expect(task?.variant).toBeUndefined()
  })
  ...
  test("prefers the invoking provider for an explicit model override", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: "Shared", variant: "low" }, [
      message("msg_current", "neo", "neo/only", "low"),
    ])
    expect(String(task?.model?.providerID)).toBe("neo")
    expect(String(task?.model?.modelID)).toBe("neo/shared")
  })
  ...
  test("falls back to the neo gateway when the preferred provider lacks the model", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: "Gateway Only" })
    // Default provider `test` does not offer it; neo is preferred over zeta.
    expect(String(task?.model?.providerID)).toBe("neo")
  })

  test("narrows to a provider that supports the requested variant", async () => {
    const rt = makeRuntime("neo")
    // neo is preferred, but only `test`'s Shared has the `high` variant.
