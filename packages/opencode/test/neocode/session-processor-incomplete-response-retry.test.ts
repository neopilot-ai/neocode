    ...
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          ...
          expect(error.data.message).toBe(NeoSessionProcessor.INCOMPLETE_RESPONSE_MESSAGE)
    ...
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          ...
          yield* ctx.test.reply(
            LLMEvent.stepStart({ index: 0 }),
            LLMEvent.toolCall({ id: "call", name: "web_search", input: { query: "Neo" }, providerExecuted: true }),
            LLMEvent.toolResult({
              id: "call",
              name: "web_search",
