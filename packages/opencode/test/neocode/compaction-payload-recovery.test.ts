...
import { NeoCompactionPayloadRecovery } from "../../src/neocode/session/compaction-payload-recovery"
...
describe("NeoCompactionPayloadRecovery", () => {
  test("detects Neo gateway payload-size errors", () => {
    const error = new MessageV2.ContextOverflowError({
      message: "Request Entity Too Large",
      responseBody: "Request Entity Too Large\n\nFUNCTION_PAYLOAD_TOO_LARGE",
    }).toObject()

    expect(NeoCompactionPayloadRecovery.matches(error)).toBe(true)
  })
  ...
  test("strips media and marks completed tool outputs compacted", async () => {
    ...
    await Effect.runPromise(
      NeoCompactionPayloadRecovery.strip({
  ...
  test("retries compaction without media and tool outputs after payload-size failure", async () => {
    ...
    stub.push((input) => {
      ...
      return Stream.fail(
        new APICallError({
          message: "Request Entity Too Large",
          url: "https://api.neo.khulnasoft.com/api/openrouter/responses",
          requestBodyValues: {},
          statusCode: 413,
