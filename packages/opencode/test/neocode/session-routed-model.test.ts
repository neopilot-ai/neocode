...
import type { Part, StepFinishPart } from "@neocode/sdk/v2"
import { RoutedModelMeta } from "../../src/neocode/cli/cmd/tui/routes/session/routed-model-meta"
import { NeoRoutedModel } from "../../src/neocode/session/routed-model"
...
describe("session routed model", () => {
  ...
  test("preserves finish-step response model in provider metadata", async () => {
    const events = await adapt([
      unchecked({
        ...
        usage: {},
        providerMetadata: { openrouter: { routed: true }, neocode: { existing: true } },
      }),
    ])
    ...
    expect(event.providerMetadata).toEqual({
      ...
      neocode: { existing: true, routedModelID: "openai/gpt-5.5-20260423" },
    })
  })
  ...
  test("shortens date-suffixed routed model ids for display", () => {
    expect(NeoRoutedModel.display("moonshotai/kimi-k2.7-code-20260612")).toBe("moonshotai/kimi-k2.7-code")
    expect(NeoRoutedModel.display("openai/gpt-5.5-2026-04-23")).toBe("openai/gpt-5.5")
    expect(NeoRoutedModel.display("openai/gpt-5.5")).toBe("openai/gpt-5.5")
  })

  test("formats routed model names for compact display", () => {
    expect(NeoRoutedModel.displayName("Qwen: Qwen3.7 Plus (20% off)")).toBe("Qwen 3.7 Plus")
    expect(NeoRoutedModel.displayName("anthropic.claude-opus-4-5-20251101-v1:0")).toBe(
      "anthropic.claude-opus-4-5-20251101-v1:0",
    )
    expect(NeoRoutedModel.displayName("moonshotai/kimi-k2.7-code")).toBe("kimi-k2.7-code")
    expect(NeoRoutedModel.displayName("o3")).toBe("o3")
  })

  test("shows compact labels only for Neo auto selections", () => {
    ...
    const same = RoutedModelMeta.info(
      undefined,
      [reason, finish({ providerID: "neo", modelID: "neo-auto/efficient" })],
      false,
      {
        providerID: "neo",
        modelID: "neo-auto/efficient",
  ...
  test("reads routed model only for selected Neo auto models", () => {
    const meta = { neocode: { routedModelID: "openai/gpt-5.5-20260423" } }

    expect(
      NeoRoutedModel.readAuto(meta, {
        providerID: ProviderV2.ID.neo,
        modelID: "neo-auto/efficient",
      }),
    ).toEqual({
      providerID: ProviderV2.ID.neo,
