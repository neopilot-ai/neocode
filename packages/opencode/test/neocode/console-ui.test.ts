...
import { ConsoleAssets } from "../../src/neocode/console/assets"

const original = process.env.NEO_CONSOLE_ASSET_DIR

afterEach(() => {
  if (original === undefined) delete process.env.NEO_CONSOLE_ASSET_DIR
  else process.env.NEO_CONSOLE_ASSET_DIR = original
})
...
async function assets(dir: string) {
  ...
  await Bun.write(path.join(dir, "assets", "app.js"), "console.log('neo')")
}

describe("Neo Console UI routes", () => {
  test("serves the console index for /console and SPA routes", async () => {
    await using tmp = await tmpdir()
    process.env.NEO_CONSOLE_ASSET_DIR = tmp.path
  ...
  test("serves console assets without falling back on missing files", async () => {
    ...
    expect(await Bun.file(asset.file).text()).toContain("neo")
