...
import type { Config, IndexingConfig } from "@neocode/sdk/v2"
import {
  ...
  mergeIndexingConfig,
  type IndexingScope,
} from "../../src/neocode/components/indexing-dialog-state"

describe("indexing dialog state", () => {
  test.serial("loads Neo models directly from the public catalog", async () => {
    ...
    global.fetch = (async (input) => {
      ...
      return new Response(
        JSON.stringify({
          defaultModel: "neo/default",
          models: [{ id: "neo/default", name: "Default", dimension: 1024, scoreThreshold: 0.35 }],
          aliases: {},
        }),
    ...
    try {
      const catalog = await loadNeoEmbeddingModels()

      expect(catalog.models).toHaveLength(1)
      expect(catalog.defaultModel).toBe("neo/default")
  ...
  test("builds stable loading, empty, and loaded model options", () => {
    expect(neoModelOptions()).toEqual([{ value: "", title: "Loading supported models..." }])
    expect(neoModelOptions({ defaultModel: "", models: [], aliases: {} })).toEqual([
    ...
    expect(neoModelOptions(catalog)).toEqual([
      { value: "provider/default", title: "Default (1024d)" },
      { value: "provider/code", title: "Code (code, 1536d)" },
    ])
    expect(currentNeoModel(catalog, "code")).toBe("provider/code")
    expect(currentNeoModel(catalog, "missing")).toBe("provider/default")
  })
  ...
  test("isolates global auth config from project indexing values", () => {
    const project: IndexingConfig = { neo: { apiKey: "project-key", baseUrl: "https://project.test" } }
    const inherited: IndexingConfig = { enabled: true }
    const effective: Config = {
      provider: { neo: { options: { apiKey: "provider-key" } } },
      indexing: project,
    }
