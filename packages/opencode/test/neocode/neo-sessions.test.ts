...
import { clearInFlightCache } from "../../src/neo-sessions/inflight-cache"
...
function reset(...tokens: string[]) {
  clearInFlightCache("neo-sessions:token")
  clearInFlightCache("neo-sessions:client")
  for (const token of tokens) clearInFlightCache(`neo-sessions:token-valid:${token}`)
}
...
it.instance("initializes once per instance through Config.Service", () => {
  ...
  return Effect.gen(function* () {
    const sessions = yield* NeoSessions.Service
    yield* sessions.init()
    yield* sessions.init()
    expect(reads).toBe(1)
  }).pipe(
...
it.instance("bootstraps session ingest from NEO_API_KEY without stored auth", () => {
  const original = process.env.NEO_API_KEY
  const calls: string[] = []
  ...
  process.env.NEO_API_KEY = "env-token"
  reset("env-token")

  return Effect.promise(() => NeoSessions.bootstrap("session-env")).pipe(
    Effect.andThen(() => Effect.sync(() => expect(calls).toEqual(["Bearer env-token", "Bearer env-token"]))),
    Effect.ensuring(
      Effect.sync(() => {
        if (original === undefined) delete process.env.NEO_API_KEY
        else process.env.NEO_API_KEY = original
        reset("env-token")
        request.mockRestore()
      }),
    ),
...
it.instance("prefers stored auth over NEO_API_KEY for session ingest", () => {
  const original = process.env.NEO_API_KEY
  const calls: string[] = []
  const fetch: typeof globalThis.fetch = Object.assign(
    async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  return Effect.gen(function* () {
    ...
    yield* auth.set("neo", { type: "api", key: "stored-token" })
    yield* Effect.promise(() => NeoSessions.bootstrap("session-auth"))
    expect(calls).toEqual(["Bearer stored-token", "Bearer stored-token"])
  }).pipe(
    Effect.ensuring(
      Effect.gen(function* () {
        const auth = yield* Auth.Service
        yield* auth.remove("neo").pipe(Effect.orDie)
        if (original === undefined) delete process.env.NEO_API_KEY
        else process.env.NEO_API_KEY = original
        reset("env-token", "stored-token")
        request.mockRestore()
...
it.instance("does not duplicate created-session subscribers when init is repeated", () => {
  ...
  return Effect.gen(function* () {
    ...
    const sessions = yield* NeoSessions.Service
    yield* auth.set("neo", { type: "api", key: "test-token" })
    yield* sessions.init()
    yield* sessions.init()
