...
const neo = { id: "neo" } as Provider.Info

// Calculated cost for the `model()` + `baseUsage` pair: 1M input * $3 + 100k output * $15 = 3 + 1.5
const fallback = 3 + 1.5

describe("NeoSession.providerCost — Anthropic Messages / OpenAI Responses", () => {
...
describe("NeoSession.providerCost — Vercel AI Gateway", () => {
    ...
    const result = SessionNs.getUsage({
      ...
      metadata: {
        gateway: {
          ...
          // which Neo doesn't pass on to end users — must be ignored.
          cost: "0",
...
describe("NeoSession.providerCost — fallback", () => {
