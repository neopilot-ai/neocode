...
describe("config resilience", () => {
  ...
  test("skips project markdown that references environment or out-of-project files", async () => {
    const name = "NEO_CONFIG_MARKDOWN_PROJECT_SECRET"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          await Filesystem.write(path.join(project, ".neo", "agent", "unsafe.md"), prompt)
          await Filesystem.write(path.join(project, ".neo", "command", "unsafe.md"), prompt)
  ...
  test("skips project markdown symlinks that escape the project root", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const project = path.join(dir, "project")
        const item = path.join(project, ".neo", "agent", "unsafe.md")
  ...
  test("skips invalid agent markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".neo", "agent", "skip.md"),
          `---
mode: "banana"
---
Broken agent prompt`,
        ...
        await Filesystem.write(
          path.join(dir, ".neo", "agent", "keep.md"),
          `---
model: test/model
---
Valid agent prompt`,
...
Broken agent prompt`,
  ...
  test("skips invalid command markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".neo", "command", "skip.md"),
          `---
subtask: "banana"
---
Broken command template`,
        ...
        await Filesystem.write(
          path.join(dir, ".neo", "command", "keep.md"),
          `---
...
Broken command template`,
  ...
  test("collects warnings for invalid agent markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".neo", "agent", "broken.md"),
          `---
mode: "banana"
---
Broken agent`,
  ...
  test("collects warnings for invalid command markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".neo", "command", "broken.md"),
          `---
subtask: "banana"
---
Broken command`,
  ...
  test("collects warnings for invalid JSON in .neo directory config", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, ".neo", "neo.json"), "{ not valid json !!!")
      },
    })
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(warns.some((w) => w.path.includes("neo.json") && w.message.includes("not valid JSON"))).toBe(true)
      },
    })
  })

  test("collects warnings for invalid schema in .neo directory config", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, ".neo", "neo.json"), JSON.stringify({ unknownField: true }))
      },
    })
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(cfg).toBeDefined()
        expect(warns.some((w) => w.path.includes("neo.json") && w.message.includes("invalid"))).toBe(true)
      },
    })
  })
