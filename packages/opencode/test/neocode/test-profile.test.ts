...
import { TestProfile } from "../../script/neocode/test-profile"
...
describe("test profiles", () => {
  test("darwin profile contains valid test files", () => {
    ...
    expect(result.files).toContain("neocode/cli/install-artifact.test.ts")
    expect(result.files).toContain("neocode/sandbox/macos-confinement.test.ts")
    expect(result.files).toContain("neocode/core-watcher.test.ts")
    expect(result.files).toContain("neocode/tool/repo_clone.test.ts")
    expect(result.files).toContain("filesystem/filesystem.test.ts")
    expect(result.files).toContain("neocode/interactive-terminal.test.ts")
    const sandbox = all.filter((file) => file.startsWith("neocode/sandbox/"))
    expect(result.files.filter((file) => file.startsWith("neocode/sandbox/"))).toEqual(sandbox)
    ...
    expect(result.files).not.toContain("neocode/sessions/remote-ws.test.ts")
