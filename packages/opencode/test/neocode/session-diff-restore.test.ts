...
import { appendSessionDiffs, mergeSessionDiffs } from "../../src/neocode/session-portability/cumulative-diff"
import { extractSessionDiffs, restoreSessionDiffs } from "../../src/neocode/session-portability/session-diff-restore"

const dirs: string[] = []

function tmp() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "neo-session-diff-"))
  dirs.push(dir)
  return dir
}
