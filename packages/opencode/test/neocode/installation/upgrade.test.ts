...
describe("Neo installation upgrade", () => {
  ...
  ).effect("reads fallback versions from the Neo npm registry", () =>
    Effect.gen(function* () {
      const result = yield* Installation.Service.use((svc) => svc.latest("unknown"))
      expect(result).toBe("8.8.8")
      expect(release).toContain(`https://registry.npmjs.org/@neocode%2fcli/${InstallationChannel}`)
    }),
  )
  ...
  ).effect("reads yarn versions from the Neo package registry", () =>
    Effect.gen(function* () {
      const result = yield* Installation.Service.use((svc) => svc.latest("yarn"))
      expect(result).toBe("8.8.8")
      expect(urls).toContain(`https://registry.npmjs.org/@neocode%2fcli/${InstallationChannel}`)
    }),
  )

  testEffect(
    layer((cmd, args) => {
      if (cmd === "npm" && args.includes("list")) return "@neocode/cli@7.3.45"
      return ""
    }),
  ).effect("detects npm installs from the Neo package", () =>
    Effect.gen(function* () {
  ...
  ).effect("reads choco versions from the Neo package", () =>
    Effect.gen(function* () {
      const result = yield* Installation.Service.use((svc) => svc.latest("choco"))
      expect(result).toBe("8.8.8")
      expect(choco).toContain(
        "https://community.chocolatey.org/api/v2/Packages?$filter=Id%20eq%20%27neo%27%20and%20IsLatestVersion&$select=Version",
      )
    }),
  )
  ...
  ).effect("reads scoop versions from the Neo manifest", () =>
    Effect.gen(function* () {
      const result = yield* Installation.Service.use((svc) => svc.latest("scoop"))
      expect(result).toBe("8.8.8")
      expect(scoop).toContain("https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/neo.json")
    }),
  )
  ...
  testEffect(upgrade).effect("installs the Neo npm package", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("npm", "9.9.9"))
      expect(calls).toContain("npm install -g @neocode/cli@9.9.9")
    }),
  )

  testEffect(upgrade).effect("installs the Neo yarn package", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("yarn", "9.9.9"))
      expect(calls).toContain("yarn global add @neocode/cli@9.9.9")
    }),
  )

  testEffect(upgrade).effect("installs the Neo pnpm package", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("pnpm", "9.9.9"))
      expect(calls).toContain("pnpm install -g @neocode/cli@9.9.9")
    }),
  )

  testEffect(upgrade).effect("installs the Neo bun package", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("bun", "9.9.9"))
      expect(calls).toContain("bun install -g @neocode/cli@9.9.9")
    }),
  )
  ...
  const brewer = layer((cmd, args) => {
    ...
    if (cmd === "brew" && args.includes("list")) return "neo"
    if (cmd === "brew" && args.includes("--repo")) return "/tmp/neo-homebrew-tap"
    return ""
  })

  testEffect(brewer).effect("upgrades the Neo brew formula", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("brew", "9.9.9"))
      expect(brew).toContain("brew tap Neopilot-Ai/tap")
      expect(brew).toContain("brew upgrade Neopilot-Ai/tap/neo")
    }),
  )

  testEffect(upgrade).effect("upgrades the Neo choco package", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("choco", "9.9.9"))
      expect(calls).toContain("choco upgrade neo --version=9.9.9 -y")
    }),
  )

  testEffect(upgrade).effect("installs the Neo scoop package", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("scoop", "9.9.9"))
      expect(calls).toContain("scoop install neo@9.9.9")
    }),
  )
