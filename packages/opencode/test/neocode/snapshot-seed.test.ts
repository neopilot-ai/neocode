...
import { Instance } from "../../src/neocode/instance"
import { Filesystem } from "../../src/util/filesystem"
import { NeoSnapshotMaterialize } from "../../src/neocode/snapshot/materialize"
import { NeoSnapshotSeed } from "../../src/neocode/snapshot/seed"
...
test(
  ...
  async () => {
    ...
    expect(
      (
        await $`git --git-dir=${common} rev-parse --verify --quiet ${NeoSnapshotMaterialize.ref(fast.gitdir)}`
          .nothrow()
          .text()
      ).trim(),
    ).toBe("")
...
test(
  ...
  async () => {
    ...
    const sourceRef = NeoSnapshotMaterialize.ref(result.gitdir)
    ...
    const expired = `refs/neo/snapshots/1/${result.value!}`
    ...
    expect((await $`git --git-dir=${result.gitdir} for-each-ref refs/neo/snapshots`.text()).trim()).toContain(
      result.value!,
    )

    const locked = `refs/neo/snapshots/2/${result.value!}`
...
test("interrupted seed removes borrowed state after source gc", async () => {
  ...
  const original = await fs.readFile(index)
  const ref = NeoSnapshotMaterialize.ref(gitdir)
  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      const fiber = yield* NeoSnapshotSeed.seed({
        dir: source.path,
