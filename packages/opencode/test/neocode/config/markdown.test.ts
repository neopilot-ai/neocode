...
import { NeocodeMarkdown } from "@/neocode/config/markdown"
import { tmpdir } from "../../fixture/fixture"

test("confines project markdown substitutions while preserving trusted substitutions", async () => {
  const name = "NEO_MARKDOWN_SUBSTITUTE_TEST_SECRET"
  ...
  try {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        const item = path.join(project, ".neo", "agents", "unsafe.md")
    ...
    const file = await NeocodeMarkdown.substitute(tmp.extra.file, tmp.extra.item, {
      trusted: false,
    ...
    const env = await NeocodeMarkdown.substitute(tmp.extra.env, tmp.extra.item, {
      trusted: false,
      fileScope: { root: tmp.extra.project, source: tmp.extra.item },
    }).then(
      () => false,
    ...
    expect(
      await NeocodeMarkdown.substitute("{file:../../allowed.txt}", tmp.extra.item, {
        trusted: false,
        fileScope: { root: tmp.extra.project, source: tmp.extra.item },
      }),
    ).toBe("project content")

    const trusted = await NeocodeMarkdown.substitute(tmp.extra.text, tmp.extra.global, { trusted: true })
