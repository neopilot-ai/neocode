...
import { NeocodeDefaultPlugins } from "../../../src/neocode/config/default-plugins"
import { INDEXING_PLUGIN } from "../../../src/neocode/indexing-feature"
...
describe("neocode default indexing plugin", () => {
  afterEach(async () => {
  ...
  test("injects indexing without registering an external plugin origin", () => {
    ...
    NeocodeDefaultPlugins.apply(config, { disabled: false })
  ...
  test("removes a persisted indexing marker from external plugin origins", () => {
    ...
    NeocodeDefaultPlugins.apply(config, { disabled: true })
  ...
  test("does not hard-enable indexing plugin when default plugins are disabled", async () => {
    const original = Flag.NEO_DISABLE_DEFAULT_PLUGINS
    ...
    try {
      ...
      Flag.NEO_DISABLE_DEFAULT_PLUGINS = original
    }
  })
})
