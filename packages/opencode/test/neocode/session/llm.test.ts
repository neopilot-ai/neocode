...
describe("neocode.session.llm.timeout", () => {
  test("uses prepared options before the provider fallback", () => {
    const result = NeoLLM.timeout({
  ...
  test("omits the timeout when it is not configured", () => {
    expect(NeoLLM.timeout({ options: {} })).toEqual({})
  })
})

describe("neocode.session.llm.text", () => {
  ...
  test("fails on stream interruption", async () => {
    const text = NeoLLM.text(Stream.fail(new DOMException("Aborted", "AbortError")))
