...
import { NeoSession } from "../../../src/neocode/session"
  ...
  test("child sessions inherit the root platform override", async () => {
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        const attr = NeoSession.attribution(child.id)

        expect(NeoSession.getPlatformOverride(root.id)).toBe("agent-manager")
        expect(NeoSession.getPlatformOverride(child.id)).toBe("agent-manager")
  ...
  test("child sessions expose parent and root lineage", async () => {
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(NeoSession.resolveParent(root.id)).toBeUndefined()
        expect(NeoSession.resolveParent(child.id)).toBe(root.id)
        expect(NeoSession.resolveParent(leaf.id)).toBe(child.id)
        expect(NeoSession.resolveRoot(leaf.id)).toBe(root.id)
  ...
  test("turn close events include persisted parent lineage", async () => {
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(NeoSession.resolveParent(child.id)).toBeUndefined()

        const closed = Promise.withResolvers<SessionID | undefined>()
        const unsubscribe = Bus.subscribe(NeoSession.Event.TurnClose, (event) => {
