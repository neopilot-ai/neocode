...
import type { Event, GlobalEvent, Message, Part, Session } from "@neocode/sdk/v2"
...
import { MemorySidebar } from "@/neocode/cli/cmd/tui/component/memory-status"
...
test("message memory metadata reacts to verbose changes and bounds snippets", async () => {
  ...
  const part = {
    ...
    type: "text",
    text: "",
    metadata: { neoMemory: { type: "recall", count: 3, items: [first, "second", "third"] } },
  } satisfies Part
  setParts([part])
  const app = await testRender(
    () => (
    ...
    setParts([
      {
        ...
        metadata: { neoMemory: { type: "startup", count: 2, tokens: 40 } },
      },
    ])
    await app.renderOnce()
    ...
    setParts([
      {
        ...
        type: "text",
        text: "",
        metadata: { neoMemory: { type: "recall", count: 2 } },
      },
    ])
