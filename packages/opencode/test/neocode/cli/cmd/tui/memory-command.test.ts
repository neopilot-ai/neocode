...
import type { NeoClient } from "@neocode/sdk/v2"
import { memoryRow } from "@/neocode/cli/cmd/tui/component/memory-status"
import { runMemoryCommand } from "@/neocode/cli/cmd/tui/memory-command"
import { MemoryTuiEvents } from "@/neocode/cli/cmd/tui/memory-events"
import { MemoryTuiMeta } from "@/neocode/cli/cmd/tui/memory-meta"
import { MemoryTuiState } from "@/neocode/cli/cmd/tui/memory-state"
...
describe("memory TUI command parser", () => {
  ...
  test("status opens overview dialog", async () => {
    const shown: string[] = []
    const opened: string[] = []
    const client = { memory: {} } as unknown as NeoClient
  ...
  test("on and off call enable and disable endpoints", async () => {
    ...
    const client = {
      memory: {
        enable: async () => {
          calls.push("enable")
          return { data: { root: "/tmp/neo-data/memory/repo-abc123", index: { tokens: 42 } } }
        },
        disable: async () => {
          calls.push("disable")
