...
import { NeoTuiConfig } from "@/neocode/cli/cmd/tui/context/tui-config"
import { NeoTerminalTitle } from "@/neocode/cli/cmd/tui/terminal-title"
...
function resolve(input: TuiConfig.Info): TuiConfig.Resolved {
  ...
  return {
    ...
    attention: {
      ...
      sound_pack: input.attention?.sound_pack ?? "neo.default",
...
describe("NeoTuiConfig.makeStore", () => {
  test("reactive reads update when set() reconciles a new config", () => {
    const store = NeoTuiConfig.makeStore(resolve(cfg({ keybinds: { app_exit: "ctrl+c" }, theme: "neo" })))
    ...
    createRoot((d) => {
      ...
      createEffect(() =>
        titles.push(
          NeoTerminalTitle.format({ base: "Neo CLI", indicator: "working", icon: store.config.title_icon }),
        ),
      )
    })
    ...
    expect(themes).toEqual(["neo"])
    expect(icons).toEqual(["none"])
    expect(titles).toEqual(["Neo CLI"])
    ...
    expect(themes).toEqual(["neo", "nord"])
    expect(icons).toEqual(["none", "emojis"])
    expect(titles).toEqual(["Neo CLI", "💭 Neo CLI"])

    dispose()
  })

  test("set() restores the default title icon when the setting is removed", () => {
    const store = NeoTuiConfig.makeStore(resolve(cfg({ title_icon: "unicode" })))
