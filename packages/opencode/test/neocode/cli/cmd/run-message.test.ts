...
import { NeoRun } from "../../../../src/neocode/cli/cmd/run"
import { buildRunMessage } from "../../../../src/neocode/cli/cmd/run-message"
...
describe("NeoRun", () => {
  test("prefers a configured command over an endpoint-backed built-in", async () => {
    ...
    expect(await NeoRun.resolveBuiltin(sdk as never, "compact", "/tmp/project")).toBeUndefined()
  })
  ...
  test("resolves an endpoint-backed built-in when no configured command matches", async () => {
    ...
    expect(await NeoRun.resolveBuiltin(sdk as never, "compact", "/tmp/project")).toBe("compact")
  })
...
describe("buildRunMessage", () => {
  ...
  test("passes args['--'] through verbatim without wrap-quote (#9622)", () => {
    // `neo run -- "- Who are you?"` - yargs+populate-- captures the leading-dash
