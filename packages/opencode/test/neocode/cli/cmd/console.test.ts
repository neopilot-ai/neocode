...
import { getNetworkIPs, serverUrls } from "../../../../src/neocode/cli/server-urls"
...
describe("console daemon startup", () => {
  test("detects every explicit network option form", () => {
    ...
    expect(explicitNetworkOptions(["neo", "console", "--", "--port=4321"])).toStrictEqual([])
  })
  ...
  test("rejects legacy fixed-password daemon state", () => {
    expect(Daemon.matches({ ...state(), password: "neo" }, opts(), [])).toBe(false)
  })
