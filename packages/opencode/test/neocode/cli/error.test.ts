...
describe("MCP errors", () => {
  test("uses Neo-neutral capability messaging", () => {
...
describe("model not found errors", () => {
  test("indicates when no models are available", () => {
    ...
    expect(named).toContain("neo.json")
...
describe("remote config authentication errors", () => {
  test("uses the Neo login command", () => {
    const error = FormatError({
    ...
    expect(error).toContain("neo auth login https://example.com/config.json")
