...
const wrapper = path.join(root, "bin", "neo")
...
describe("npm install artifact behavior", () => {
  test("keeps the CLI wrapper contract", async () => {
    ...
    expect(text).toContain("const envPath = process.env.NEO_BIN_PATH")
    expect(text).toContain('const base = "@neocode/cli-" + platform + "-" + arch')
  ...
  test("copies cached binary runtime resources during postinstall", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "neo-postinstall-artifact-"))
    try {
      const pkg = path.join(tmp, "node_modules", "@neocode", "cli")
      const native = path.join(tmp, "node_modules", "@neocode", `cli-${process.platform}-${process.arch}`)
      ...
      await Bun.write(
        ...
        JSON.stringify({
          optionalDependencies: {
            [`@neocode/cli-${process.platform}-${process.arch}`]: "1.0.0",
          },
        }),
      )
      await Bun.write(
        path.join(native, "package.json"),
        JSON.stringify({ name: `@neocode/cli-${process.platform}-${process.arch}` }),
      )
      const binary = "#!/bin/sh\n# binary\nexit 0\n"
      await Bun.write(path.join(bin, "neo"), binary)
      await Bun.write(path.join(bin, "neo-sandbox-mutation-worker.js"), "worker")
      ...
      expect(await Bun.file(path.join(pkg, "bin", ".neo")).text()).toBe(binary)
      expect(await Bun.file(path.join(pkg, "bin", "neo-sandbox-mutation-worker.js")).text()).toBe("worker")
  ...
  test("links npm bin commands to the wrapper during local install", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "neo-install-artifact-"))
    try {
      ...
      await fs.copyFile(wrapper, path.join(bin, "neo"))
      await Bun.write(
        path.join(pkg, "package.json"),
        JSON.stringify(
          {
            name: "neo-install-artifact-repro",
            version: "1.0.0",
            bin: {
              neo: "./bin/neo",
              neocode: "./bin/neo",
      ...
      const hidden = path.join(prefix, "node_modules", ".bin", ".neo")
