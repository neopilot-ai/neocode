...
import { NeoTuiThreadDaemon } from "../../../../src/neocode/cli/cmd/tui/thread"
...
describe("neo tui thread", () => {
  ...
  test("uses neo-dev caller directory when running through package cwd", async () => {
    await using root = await tmpdir()
    const pkg = path.join(root.path, "packages", "opencode")
    await fs.mkdir(pkg, { recursive: true })

    const prev = process.env.NEO_DEV_CWD
    process.env.NEO_DEV_CWD = root.path
    try {
      ...
      if (prev === undefined) delete process.env.NEO_DEV_CWD
      else process.env.NEO_DEV_CWD = prev
    }
  })
  ...
  test("validates imported daemon session over HTTP after importing from cloud", async () => {
    ...
    using server = Bun.serve({
      ...
      fetch(request) {
        ...
        calls.push(route)
        if (route === "POST /neo/cloud/session/import") return Response.json({ id: local })
    ...
    const args = { port: 0, hostname: "127.0.0.1", mdns: false, "mdns-domain": "neo.local", cors: [] }
    const start: Parameters<typeof NeoTuiThreadDaemon.attach>[0]["start"] = async (input) => {
      opened.push(input.args.sessionID)
    }

    try {
      await NeoTuiThreadDaemon.attach({
      ...
      expect(calls).toEqual(["POST /neo/cloud/session/import", `GET /session/${local}`])
  ...
  test("imports cloud fork before validating daemon session", async () => {
    ...
    mock.module("@/neocode/daemon/client", () => ({
      DaemonClient: {
    ...
    const mod = await import(`../../../../src/neocode/cli/cmd/tui/thread?${key}`)

    const handled = await mod.NeoTuiThreadDaemon.attach({
