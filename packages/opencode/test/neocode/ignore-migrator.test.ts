...
import { IgnoreMigrator } from "../../src/neocode/ignore-migrator"
...
# Comment 2`
  ...
  *.env  `
  ...
  describe("migrate", () => {
    test("returns empty permission for project without .neocodeignore", async () => {
    ...
    test("loads project .neocodeignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
    ...
    test("handles complex .neocodeignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".neocodeignore"),
            `# Secrets
secrets/
*.env
!.env.example
...
`,
  ...
  describe("loadIgnoreConfig", () => {
    test("returns empty object for project without .neocodeignore", async () => {
      await using tmp = await tmpdir()
    ...
    test("returns permission config for project with .neocodeignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".neocodeignore"), "secrets/")
        },
