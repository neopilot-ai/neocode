...
async function update(target: ReturnType<typeof app>, provider: "neo" | "openrouter") {
...
async function provider(target: ReturnType<typeof app>, directory: string) {
  const response = await target.request("/config", { headers: { "x-neo-directory": directory } })
  return (await response.json()).indexing?.provider as string | undefined
}

async function config(dir: string, value: object) {
  await Bun.write(path.join(dir, "neo.json"), JSON.stringify(value))
}
...
describe("global config refresh", () => {
  ...
  test("update ignores disposal notification failures", async () => {
    ...
    try {
      expect((await update(target, "neo")).status).toBe(200)
    } finally {
