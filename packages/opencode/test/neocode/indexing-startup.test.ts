...
import { CodeIndexManager } from "@neocode/neo-indexing/engine"
import { normalizeIndexingStatus } from "@neocode/neo-indexing/status"
import type { Config } from "../../src/config/config"
import { GlobalBus } from "../../src/bus/global"
import { WorkspaceV2 } from "@opencode-ai/core/workspace"
import { WorkspaceContext } from "../../src/control-plane/workspace-context"
import { NeoIndexing, IndexingModelError } from "../../src/neocode/indexing"
import { indexingWarningKey } from "../../src/neocode/indexing-warning"
import { IndexingWorker } from "../../src/neocode/indexing-worker-client"
...
const staleNeo: Partial<Config.Info> = {
  plugin: ["@neocode/neo-indexing"],
  indexing: {
    enabled: true,
    provider: "neo",
...
const configDir = process.env["NEO_CONFIG_DIR"]
const disabled = process.env["NEO_DISABLE_CODEBASE_INDEXING"]
...
async function wait(read: () => Promise<NeoIndexing.Status>, state: NeoIndexing.Status["state"]) {
...
afterEach(async () => {
  ...
  if (configDir === undefined) delete process.env["NEO_CONFIG_DIR"]
  else process.env["NEO_CONFIG_DIR"] = configDir
  if (disabled === undefined) delete process.env["NEO_DISABLE_CODEBASE_INDEXING"]
  else process.env["NEO_DISABLE_CODEBASE_INDEXING"] = disabled
  global.fetch = fetch
  await disposeAllInstances()
})

describe("indexing model catalog", () => {
  test("ignores a project-scoped Neo origin", async () => {
    await using tmp = await tmpdir({
      git: true,
      init: async (dir) => {
        const global = path.join(dir, "global")
        const project = path.join(dir, "project")
        await fs.mkdir(path.join(project, ".neo"), { recursive: true })
        await fs.mkdir(global, { recursive: true })
        await Bun.write(path.join(global, "neo.jsonc"), "{}")
        await Bun.write(
          path.join(project, ".neo", "neo.jsonc"),
          JSON.stringify({ indexing: { neo: { baseUrl: "http://127.0.0.1:4567" } } }),
        )
        return { global, project }
      },
    })
    process.env["NEO_CONFIG_DIR"] = tmp.extra.global
    const calls: string[] = []
    ...
    const response = await Server.Default().app.request("/indexing/models", {
      headers: { "x-neo-directory": tmp.extra.project },
    })
...
describe("indexing startup degradation", () => {
  test("keeps server routes alive when indexing initialization fails", async () => {
    ...
    process.env["NEO_CONFIG_DIR"] = tmp.path
  ...
  test("retains and deduplicates indexing warnings for TUI replay", async () => {
    ...
    const on = (data: {
      ...
      if (data.payload?.type !== NeoIndexing.Warning.type) return
  ...
  test("coalesces burst indexing progress publications", async () => {
    const complete: NeoIndexing.Status = {
      state: "Complete",
    ...
    await using tmp = await tmpdir({ git: true, config: cfg })
    process.env["NEO_CONFIG_DIR"] = tmp.path
    const events: NeoIndexing.Status[] = []
    const on = (data: {
      directory?: string
      payload?: { type?: string; properties?: { status?: NeoIndexing.Status } }
    }) => {
      if (data.directory !== tmp.path) return
      if (data.payload?.type !== NeoIndexing.Event.type) return
    ...
    try {
      await withTestInstance({
        directory: tmp.path,
        fn: async () => expect(await wait(() => NeoIndexing.current(), "Complete")).toEqual(complete),
      })
  ...
  test("does not publish initialized status after in-flight startup is disposed", async () => {
    ...
    try {
      await provideTestInstance({
        directory: tmp.path,
        init: Effect.promise(() => NeoIndexing.init()),
        fn: async () => {
          await called(init)
          expect((await NeoIndexing.current()).state).toBe("In Progress")
        },
      })
  ...
  test("keeps degraded indexing queryable but releases its failed engine", async () => {
    ...
    try {
      await provideTestInstance({
        ...
        init: Effect.promise(() => NeoIndexing.init()),
        fn: async () => {
          const status = await wait(() => NeoIndexing.current(), "Error")

          expect(status.state).toBe("Error")
          expect(status.message).toContain("Failed to initialize: test indexing initialization failed")
          expect(await NeoIndexing.available()).toBe(false)
          expect(NeoIndexing.ready()).toBe(false)
          expect(await NeoIndexing.search("boot failure")).toEqual([])
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          expect(NeoIndexing.ready()).toBe(false)
          expect(await NeoIndexing.available()).toBe(false)
          const search = NeoIndexing.search("boot failure")
  ...
  test("stays disabled when indexing enablement is unset", async () => {
    ...
    await provideTestInstance({
      directory: tmp.path,
      fn: async () => {
        const status = await wait(() => NeoIndexing.current(), "Disabled")
        ...
        expect(await NeoIndexing.available()).toBe(false)
        expect(NeoIndexing.ready()).toBe(false)
        expect(await NeoIndexing.search("disabled")).toEqual([])
  ...
  test("does not validate the indexing model when indexing is disabled", async () => {
    ...
    const logger = Log.create({ service: "neocode-indexing" })
    const warn = spyOn(logger, "warn")
    const key = process.env.NEO_API_KEY

    const config: Partial<Config.Info> = {
      ...staleNeo,
      indexing: { ...staleNeo.indexing, enabled: false, model: "removed/model" },
    }
    await using tmp = await tmpdir({ git: true, config })
    process.env["NEO_CONFIG_DIR"] = tmp.path
    process.env.NEO_API_KEY = "neo-token"
    ...
    try {
      ...
      if (key === undefined) delete process.env.NEO_API_KEY
      else process.env.NEO_API_KEY = key
      warn.mockRestore()
    }
  })

  test("disabled indexing does not resolve the Neo model catalog", async () => {
  ...
  test("enriches Neo provider config from env auth", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
    ...
    const key = process.env.NEO_API_KEY
    const org = process.env.NEO_ORG_ID

    await using tmp = await tmpdir({ git: true, config: neo })
    process.env["NEO_CONFIG_DIR"] = tmp.path
    process.env.NEO_API_KEY = "neo-token"
    process.env.NEO_ORG_ID = "org_123"
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          expect(init.mock.calls[0]?.[0]).toMatchObject({
            embedderProvider: "neo",
            neoApiKey: "neo-token",
            neoOrganizationId: "org_123",
      ...
      if (org === undefined) delete process.env.NEO_ORG_ID
      else process.env.NEO_ORG_ID = org
      init.mockRestore()
    }
  })

  test("reports an error for an unsupported explicit Neo model instead of falling back", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
    ...
    await using tmp = await tmpdir({ git: true, config: staleNeo })
    process.env["NEO_CONFIG_DIR"] = tmp.path
    process.env.NEO_API_KEY = "neo-token"
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          expect(await NeoIndexing.available()).toBe(false)
          expect(NeoIndexing.ready()).toBe(false)
          expect(await NeoIndexing.search("unsupported model")).toEqual([])
  ...
  test("passes a valid explicit Neo model through without error", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
    ...
    const config: Partial<Config.Info> = {
      ...staleNeo,
      indexing: { ...staleNeo.indexing, model: "mistralai/mistral-embed-2312" },
    }
    await using tmp = await tmpdir({ git: true, config })
    process.env["NEO_CONFIG_DIR"] = tmp.path
    process.env.NEO_API_KEY = "neo-token"
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          const modelErr = warn.mock.calls.find((call) => IndexingModelError.isInstance(call[1]?.err))?.[1]?.err
          expect(modelErr).toBeUndefined()
          expect((await NeoIndexing.current()).state).not.toBe("Error")
        },
      })
    } finally {
      if (key === undefined) delete process.env.NEO_API_KEY
  ...
  test("uses hosted dimensions for supported Neo models", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
  ...
  test("leaves Neo model metadata unset when the hosted catalog is unavailable", async () => {
  ...
  test("does not default to Neo when an existing provider config is present", async () => {
    const init = spyOn(CodeIndexManager.prototype, "initialize").mockResolvedValue({ requiresRestart: false })
    const key = process.env.NEO_API_KEY
  ...
  test("stays disabled when VS Code starts without a workspace folder", async () => {
    await using tmp = await tmpdir({ git: true, config: cfg })
    process.env["NEO_CONFIG_DIR"] = tmp.path
    process.env["NEO_DISABLE_CODEBASE_INDEXING"] = "vscode-no-workspace"
    const init = spyOn(CodeIndexManager.prototype, "initialize")
    ...
    try {
      await provideTestInstance({
        ...
        init: Effect.promise(() => NeoIndexing.init()),
        fn: async () => {
          const status = await NeoIndexing.current()
          ...
          expect(await NeoIndexing.available()).toBe(false)
          expect(NeoIndexing.ready()).toBe(false)
          expect(await NeoIndexing.search("no workspace")).toEqual([])
          expect(init).not.toHaveBeenCalled()
        },
      })
    } finally {
