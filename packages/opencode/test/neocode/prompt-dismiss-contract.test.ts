/**
 * Contract test for prompt.ts Neo-specific invariants.
 *
 * prompt.ts is a shared upstream file. The Neo-specific "new prompt unblocks
 * pending suggestions/questions then enqueues without cancelling the in-flight
 * stream" behaviour lives inside a neocode_change block. An upstream merge
...
describe("prompt.ts Neo-specific invariants", () => {
  test("imports Suggestion from neocode/suggestion", () => {
    const content = fs.readFileSync(PROMPT_FILE, "utf-8")
    expect(content).toMatch(/import\s*\{[^}]*Suggestion[^}]*\}\s*from\s*["']@\/neocode\/suggestion["']/)
  })
  ...
  test("dismissAll for suggestions and questions runs before enqueue, without cancelling the in-flight fiber", () => {
    ...
    // Critically, the block must NOT call state.cancel or NeoSessionPromptQueue.reserve —
    ...
    const block = content.match(
      /neocode_change start[^\n]*unblock tools[\s\S]*?Suggestion\.dismissAll[\s\S]*?question\.dismissAll[\s\S]*?NeoSessionPromptQueue\.enqueue/,
    )
    expect(block).not.toBeNull()
    expect(content).not.toMatch(/state\.cancel\(input\.sessionID\)/)
    expect(content).not.toMatch(/NeoSessionPromptQueue\.reserve/)
  })
  ...
  test("runLoop breaks out between LLM steps when a newer prompt was enqueued", () => {
    ...
    expect(content).toContain("NeoSessionPromptQueue.hasFollowup(sessionID)")
  })
})
