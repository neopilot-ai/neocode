...
import { NeocodeConfigInjector } from "../../src/neocode/config-injector"
import { tmpdir } from "../fixture/fixture"
import path from "path"
import fs from "fs/promises"

describe("NeocodeConfigInjector", () => {
  describe("buildConfig", () => {
    test("returns empty config when no modes exist", async () => {
      await using tmp = await tmpdir()

      const result = await NeocodeConfigInjector.buildConfig({ projectDir: tmp.path, skipGlobalPaths: true })
    ...
    test("includes rules in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          await Bun.write(path.join(dir, ".neocode", "rules", "main.md"), "# Rules")
        },
      })

      const result = await NeocodeConfigInjector.buildConfig({
        projectDir: tmp.path,
  ...
  - slug: translate
    ...
    test("includes ignore patterns in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".neocodeignore"), "secrets/\n*.env")
        },
      })
  ...
  - slug: translate
    ...
    test("handles negation patterns in ignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".neocodeignore"), "*.env\n!.env.example")
        },
      })
  ...
  describe("getEnvVars", () => {
    test("returns empty object for empty config", () => {
      const envVars = NeocodeConfigInjector.getEnvVars("{}")
    ...
    test("returns NEO_CONFIG_CONTENT for non-empty config", () => {
      const config = JSON.stringify({ agent: { test: {} } })
      const envVars = NeocodeConfigInjector.getEnvVars(config)

      expect(envVars).toEqual({
        NEO_CONFIG_CONTENT: config,
      })
    })
  })
})
