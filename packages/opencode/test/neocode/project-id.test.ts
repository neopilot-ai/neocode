...
import { getNeoProjectId } from "../../src/neocode/project-id"
...
describe("project-id", () => {
  describe("normalization", () => {
    test("extracts repo name from HTTPS git URL", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.$`git remote add origin https://github.com/Neopilot-Ai/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from SSH git URL", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.$`git remote add origin git@github.com:Neopilot-Ai/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from HTTPS URL without .git extension", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com/Neopilot-Ai/handbook`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from ssh:// URL", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin ssh://git@github.com/Neopilot-Ai/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("truncates long repo names to 100 characters", async () => {
      ...
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com/Neopilot-Ai/${longName}.git`.cwd(dir).quiet()
        },
      })
  ...
  describe("config file priority", () => {
    test("uses project.id from .neo/config.json", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
    ...
    test("falls back to .neocode/config.json when .neo/config.json is absent", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".neocode"), { recursive: true })
          await Bun.write(
            path.join(dir, ".neocode", "config.json"),
            JSON.stringify({
              project: {
                id: "legacy-project",
              },
    ...
    test("prefers .neo/config.json over .neocode/config.json", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".neo"), { recursive: true })
          await Bun.write(path.join(dir, ".neo", "config.json"), JSON.stringify({ project: { id: "new-project" } }))
          await fs.mkdir(path.join(dir, ".neocode"), { recursive: true })
          await Bun.write(
            path.join(dir, ".neocode", "config.json"),
      ...
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.write(
            ...
            JSON.stringify({
              project: {
                id: "https://github.com/Neopilot-Ai/another-repo.git",
              },
            }),
          )
        },
  ...
  describe("fallback behavior", () => {
    ...
    test("handles malformed JSON in config file gracefully", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".neo"), { recursive: true })
          await Bun.write(path.join(dir, ".neo", "config.json"), "{ invalid json")
  ...
  describe("edge cases", () => {
    test("handles git URLs with port numbers", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com:443/Neopilot-Ai/handbook.git`.cwd(dir).quiet()
        },
      })
