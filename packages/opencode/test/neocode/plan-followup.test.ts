...
import { formatTodos, generateHandover, PlanFollowup, PlanFollowupRuntime } from "../../src/neocode/plan-followup"
import { Instance } from "../../src/neocode/instance"
import * as NeoInstance from "../../src/neocode/instance"
...
async function withInstance(fn: () => Promise<void>) {
  ...
  const provide = spyOn(NeoInstance, "provide").mockImplementation((input) =>
...
describe("plan follow-up", () => {
  ...
  test("ask - retargets prompt queue so injected message is visible in scope", () =>
    withInstance(async () => {
      const { NeoSessionPromptQueue } = await import("../../src/neocode/session/prompt-queue")
      ...
      NeoSessionPromptQueue.retarget(seeded.sessionID, original)
      ...
      const scoped = NeoSessionPromptQueue.scope(seeded.sessionID, all)
