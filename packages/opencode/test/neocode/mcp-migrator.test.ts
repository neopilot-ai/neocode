...
import { McpMigrator } from "../../src/neocode/mcp-migrator"
...
describe("McpMigrator", () => {
  describe("convertServer", () => {
    test("converts local server with command and args", () => {
      const server: McpMigrator.NeocodeMcpServer = {
        command: "npx",
  ...
  describe("migrate", () => {
    ...
    test("migrates servers from project .neo/mcp.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".neo")
          await Bun.write(
    ...
    test("reads from legacy .neocode/mcp.json when .neo/mcp.json is absent", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".neocode")
          await Bun.write(
            path.join(settingsDir, "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    // Regression: malformed .neocode/mcp.json must not prevent .neo/mcp.json from loading
    test("loads .neo/mcp.json even when .neocode/mcp.json is malformed", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".neocode", "mcp.json"), "{ corrupt json !!!")
          await Bun.write(
            path.join(dir, ".neo", "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    test(".neo/mcp.json overrides .neocode/mcp.json for same server name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".neocode", "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    test("does not read project-level .neo/mcp_settings.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".neo")
          await Bun.write(
