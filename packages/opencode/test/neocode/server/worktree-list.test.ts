...
const state = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = Flag.NEO_EXPERIMENTAL_WORKSPACES
    Flag.NEO_EXPERIMENTAL_WORKSPACES = true
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.NEO_EXPERIMENTAL_WORKSPACES = original
        await resetDatabase()
      }),
    )
  }),
...
describe("Neo Console worktree listing", () => {
  run(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const directory = path.join(test.directory, ".neo", "worktrees", "console-list")
