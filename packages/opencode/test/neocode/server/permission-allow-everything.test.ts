...
import { AllowEverythingPermission } from "../../../src/neocode/permission/allow-everything"
...
const original = {
  password: Flag.NEO_SERVER_PASSWORD,
  username: Flag.NEO_SERVER_USERNAME,
  envPassword: process.env.NEO_SERVER_PASSWORD,
  envUsername: process.env.NEO_SERVER_USERNAME,
}

afterEach(() => {
  Flag.NEO_SERVER_PASSWORD = original.password
  Flag.NEO_SERVER_USERNAME = original.username
  if (original.envPassword === undefined) delete process.env.NEO_SERVER_PASSWORD
  else process.env.NEO_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.NEO_SERVER_USERNAME
  else process.env.NEO_SERVER_USERNAME = original.envUsername
})

const auth = () => `Basic ${Buffer.from("neo:secret").toString("base64")}`

const requireAuth = () => {
  Flag.NEO_SERVER_PASSWORD = "secret"
  Flag.NEO_SERVER_USERNAME = undefined
  process.env.NEO_SERVER_PASSWORD = "secret"
  delete process.env.NEO_SERVER_USERNAME
}
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        const blocked = await Server.Default().app.request("/permission/allow-everything", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-neo-directory": tmp.path },
        ...
        const enable = await Server.Default().app.request("/permission/allow-everything", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-neo-directory": tmp.path, authorization: auth() },
          body: JSON.stringify({ enable: true }),
        })
