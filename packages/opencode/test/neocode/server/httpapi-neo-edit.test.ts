...
const env = {
  NEO_AUTH_CONTENT: process.env.NEO_AUTH_CONTENT,
...
async function send(body: Record<string, unknown> = edit, signal?: AbortSignal) {
  ...
  return app().request(NeoGatewayPaths.edit, {
    method: "POST",
...
describe("HttpApi Neo next edit", () => {
  test("requires Neo Gateway authentication for the Neo-backed model", async () => {
    process.env.NEO_AUTH_CONTENT = "{}"
  ...
  test("proxies Neo-backed edits with gateway auth and autocomplete headers", async () => {
    authenticate()
