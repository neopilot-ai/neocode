...
import { NeocodeConfigOverlay } from "../../../src/neocode/config/overlay"
...
async function config(dir: string, value: unknown) {
  await Bun.write(path.join(dir, "neo.json"), JSON.stringify(value, null, 2))
}
...
describe("config overlay routes", () => {
  test("ignores unsafe patch paths", () => {
    const patched = NeocodeConfigOverlay.patch({
      scope: "project",
      unset: [
        ["__proto__", "polluted"],
  ...
  test("prefers .neo over legacy .neocode and ignores .opencode in project overlays", async () => {
    ...
    const entries = [
      ...
      },
      {
        root: ".neocode",
        source: "neocode",
        value: { username: "neocode", model: "test/neocode" },
      },
      {
        root: ".neo",
        source: "neo",
    ...
    for (const item of entries) {
      ...
      await Filesystem.write(path.join(dir, "neo.json"), JSON.stringify(item.value))
    ...
    const body = await NeocodeConfigOverlay.resolve({
    ...
    expect(body.project.username).toBe("neo")
    expect(body.project.model).toBe("test/neocode")
    ...
    expect(body.targets.project).toBe(path.join(project.path, ".neo", "neo.json"))
  })
  ...
  test.serial("tolerates unsafe project config instead of failing the overlay", async () => {
    ...
    await Filesystem.write(
      path.join(project.path, ".neo", "neo.json"),
  ...
  test.serial("writes project indexing overrides to .neo/neo.jsonc", async () => {
    await using global = await tmpdir()
    await using project = await tmpdir()
    ...
    const file = path.join(project.path, ".neo", "neo.jsonc")
    const saved = (await Bun.file(file).json()) as { indexing: Record<string, unknown> }
    const body = await json<Overlay>(await req(project.path, "/config/overlay?scope=project"))

    expect(await Bun.file(path.join(project.path, ".neo", "neo.json")).exists()).toBe(false)
  ...
  test.serial("removes local scalar override and falls back to global", async () => {
    await using global = await tmpdir()
    await using project = await tmpdir({ config: { model: "neo/project-model", username: "alice" } })
    await setGlobal(global.path, { model: "neo/global-model" })
  ...
  test.serial("writes project mcp overrides without copying inherited servers", async () => {
    ...
    const saved = (await Bun.file(path.join(project.path, ".neo", "neo.jsonc")).json()) as {
  ...
  terminal("preserves active terminals after updating global console preferences", async () => {
    ...
    const headers = { "x-neo-directory": project.path }
