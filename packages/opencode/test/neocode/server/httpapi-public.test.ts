...
import { AgentBuilderPaths } from "../../../src/neocode/server/httpapi/groups/agent-builder"
import { BackgroundProcessPaths } from "../../../src/neocode/server/httpapi/groups/background-process"
import { BranchNamePaths } from "../../../src/neocode/server/httpapi/groups/branch-name"
import { ConfigConsolePaths } from "../../../src/neocode/server/httpapi/groups/config-console"
import { IndexingPaths, NeoEmbeddingModel } from "../../../src/neocode/server/httpapi/groups/indexing"
import { NeoGatewayPaths } from "../../../src/neocode/server/httpapi/groups/neo-gateway"
import { NeocodePaths } from "../../../src/neocode/server/httpapi/groups/neocode"
import { MemoryPaths } from "../../../src/neocode/server/httpapi/groups/memory"
import { NetworkPaths } from "../../../src/neocode/server/httpapi/groups/network"
import { TelemetryPaths } from "../../../src/neocode/server/httpapi/groups/telemetry"
...
describe("Neo PublicApi OpenAPI contract", () => {
  test("uses Neo branding", () => {
    const spec = OpenApi.fromApi(PublicApi)
    expect(spec.info.title).toBe("neo")
    expect(spec.info.description).toBe("neo api")
  })

  test("includes legacy Neo events in the generated SDK contract", () => {
  ...
  test("constrains embedding model metadata", () => {
    const accepts = (dimension: number, scoreThreshold: number) =>
      Result.isSuccess(
        EffectSchema.decodeUnknownResult(NeoEmbeddingModel)({
          id: "provider/model",
          name: "Model",
          dimension,
          scoreThreshold,
  ...
  test("keeps directory routing queries on Neo Console routes", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const routes = [
  ...
  test("keeps workspace routing queries on all Neo-owned routed endpoints", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const routes = [
      ...
      { method: "get", path: NeocodePaths.sessionModelUsage },
  ...
  test("keeps personal organization resets nullable", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const body = spec.paths[NeoGatewayPaths.organization]?.post?.requestBody as Body | undefined
  ...
  test("keeps Neo gateway responses nullable", () => {
    const spec = OpenApi.fromApi(PublicApi)
    ...
    const profile = response(NeoGatewayPaths.profile)?.properties
    expect(profile?.balance).toEqual({ anyOf: [expect.objectContaining({ type: "object" }), { type: "null" }] })
    expect(profile?.neoPass).toEqual({ anyOf: [expect.objectContaining({ type: "object" }), { type: "null" }] })
    ...
    const pass = profile?.neoPass?.anyOf?.find((item) => item.type === "object")?.properties
    ...
    const auth = response(NeoGatewayPaths.authStatus)?.properties
    expect(auth).toEqual({
      authenticated: { type: "boolean" },
      type: { type: "string", enum: ["api", "oauth"] },
    })

    const sessions = response(NeoGatewayPaths.cloudSessions)?.properties
    ...
    const claw = response(NeoGatewayPaths.clawStatus)?.properties
    ...
    expect(response(NeoGatewayPaths.clawChatCredentials)).toEqual({
  ...
  test("keeps transcription prompts in the public contract", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const body = spec.paths[NeoGatewayPaths.audioTranscriptions]?.post?.requestBody as Body | undefined
    const schema = body?.content?.["application/json"]?.schema
