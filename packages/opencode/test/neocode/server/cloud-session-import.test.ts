...
const [runtime, gateway, server, fixture] = await (async () => {
  const disabled = process.env.NEO_DISABLE_SESSION_INGEST
  const base = process.env.NEO_SESSION_INGEST_URL
  process.env.NEO_DISABLE_SESSION_INGEST = "1"
  process.env.NEO_SESSION_INGEST_URL = ingest.url.origin
  try {
    return await Promise.all([
      import("../../../src/effect/app-runtime"),
      import("../../../src/neocode/server/httpapi/groups/neo-gateway"),
      import("../../../src/server/routes/instance/httpapi/server"),
      import("../../fixture/fixture"),
    ])
  } finally {
    if (disabled === undefined) delete process.env.NEO_DISABLE_SESSION_INGEST
    else process.env.NEO_DISABLE_SESSION_INGEST = disabled
    if (base === undefined) delete process.env.NEO_SESSION_INGEST_URL
    else process.env.NEO_SESSION_INGEST_URL = base
  }
})()
const { AppRuntime } = runtime
const { NeoGatewayPaths } = gateway
...
async function request(directory: string, body: unknown) {
  const auth = process.env.NEO_AUTH_CONTENT
  const token = `test-${crypto.randomUUID()}`
  bodies.set(token, body)
  process.env.NEO_AUTH_CONTENT = JSON.stringify({ neo: { type: "api", key: token } })
  try {
    return await HttpApiApp.webHandler().handler(
      new Request(`http://localhost${NeoGatewayPaths.cloudSessionImport}`, {
        method: "POST",
        headers: { "content-type": "application/json", "x-neo-directory": directory },
    ...
    bodies.delete(token)
    if (auth === undefined) delete process.env.NEO_AUTH_CONTENT
    else process.env.NEO_AUTH_CONTENT = auth
  }
}

async function routed<T>(id: string, run: () => Promise<T>) {
  const workspace = Flag.NEO_WORKSPACE_ID
  Flag.NEO_WORKSPACE_ID = id
  try {
    return await run()
  } finally {
    Flag.NEO_WORKSPACE_ID = workspace
  }
}
