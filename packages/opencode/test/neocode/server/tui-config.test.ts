...
describe("TUI config routes", () => {
  ...
  test("loads legacy .neocode TUI config and ignores .opencode", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, ".neocode", "tui.json"), JSON.stringify({ theme: "nord" }))
      },
    })
  ...
  test("patches project TUI config", async () => {
    ...
    const saved = await Bun.file(path.join(tmp.path, ".neo", "tui.json")).json()
