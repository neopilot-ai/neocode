...
import { invalidateAfterProviderAuthChange } from "../../../src/neocode/server/provider-auth-lifecycle"
...
it.effect("clears provider models before disposing instances after auth changes", () =>
  Effect.gen(function* () {
    ...
    yield* invalidateAfterProviderAuthChange("neo").pipe(Effect.provide(layer(events)))

    expect(yield* Ref.get(events)).toEqual(["clear:neo", "dispose"])
  }),
)
