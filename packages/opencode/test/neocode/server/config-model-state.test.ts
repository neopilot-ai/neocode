...
describe("config model state routes", () => {
  test("reads TUI model favorites", async () => {
    ...
    await Bun.write(
      ...
      JSON.stringify({
        recent: [{ providerID: "neo", modelID: "gpt-5.5" }],
        favorite: [
          { providerID: "neo", modelID: "gpt-5.5" },
          { providerID: "neo", modelID: "qwen/qwen3-8b" },
        ],
        model: {},
        variant: {},
      }),
  ...
  test("updates favorites while preserving recents", async () => {
    ...
    await Bun.write(
      path.join(tmp.path, "model.json"),
      JSON.stringify({
        recent: [{ providerID: "neo", modelID: "recent" }],
        favorite: [],
        model: {},
        variant: {},
      }),
    ...
    const body = await json<{ recent: unknown[]; favorite: unknown[] }>(
      await req("/config/model-state", {
        ...
        body: JSON.stringify({ favorite: [{ providerID: "neo", modelID: "gpt-5.5" }] }),
      }),
    )

    expect(body.recent).toEqual([{ providerID: "neo", modelID: "recent" }])
    expect(body.favorite).toEqual([{ providerID: "neo", modelID: "gpt-5.5" }])
  })
})
