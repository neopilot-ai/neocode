...
import { NeoGatewayApi, NeoGatewayPaths } from "../../../src/neocode/server/httpapi/groups/neo-gateway"
import { neoGatewayHandlers } from "../../../src/neocode/server/httpapi/handlers/neo-gateway"
...
const TestHttpApi = HttpApi.make("opencode-instance").addHttpApi(NeoGatewayApi)
...
const layer = HttpRouter.serve(
  HttpApiBuilder.layer(TestHttpApi).pipe(
    Layer.provide(neoGatewayHandlers),
...
describe("Neo gateway HttpApi statuses", () => {
  it.live("reports locally stored API authentication without a Gateway request", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(NeoGatewayPaths.authStatus)
  ...
  it.live("preserves cloud session list rate limits", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(NeoGatewayPaths.cloudSessions)
  ...
  it.live("preserves missing cloud session previews", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(NeoGatewayPaths.cloudSession.replace(":id", "missing"))
  ...
  it.live("preserves cloud session preview server failures", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(NeoGatewayPaths.cloudSession.replace(":id", "failed"))
  ...
  it.live("preserves cloud session import authentication failures", () =>
    Effect.gen(function* () {
      ...
      const response = yield* post(NeoGatewayPaths.cloudSessionImport, { sessionId: "unauthorized" })
  ...
  it.live("maps cloud session import transport failures to internal errors", () =>
    Effect.gen(function* () {
      ...
      const response = yield* post(NeoGatewayPaths.cloudSessionImport, { sessionId: "failed" })
  ...
  it.live("preserves NeoClaw worker failures", () =>
    Effect.gen(function* () {
      yield* stub(() => new Response("worker failed", { status: 500 }))

      const response = yield* HttpClient.get(NeoGatewayPaths.clawStatus)

      expect(response.status).toBe(500)
      expect(yield* response.json).toEqual({ error: "NeoClaw request failed: 500 worker failed" })
    }),
  )

  it.live("normalizes numeric NeoClaw timestamps", () =>
    Effect.gen(function* () {
  ...
  it.live("maps NeoClaw transport failures to bad gateway", () =>
    Effect.gen(function* () {
      yield* stub(() => Promise.reject(new TypeError("network error")))
      ...
      expect(yield* response.json).toEqual({ error: "Failed to reach NeoClaw" })
    }),
  )
})
