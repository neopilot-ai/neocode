...
const skill = async (dir: string) => {
  await Bun.write(
    path.join(dir, ".neo/skill/httpapi-remove/SKILL.md"),
    "---\nname: httpapi-remove\ndescription: HTTP API removal fixture.\n---\n# HTTP API remove\n",
  )
  await Bun.write(path.join(dir, ".neo/skill/httpapi-remove/KEEP.txt"), "synthetic sentinel\n")
}

const agent = async (dir: string) => {
  await Bun.write(
    path.join(dir, ".neo/agent/httpapi-remove.md"),
...
function enable(ctx: ScenarioContext) {
  const dir = directory(ctx)
  return Effect.promise(() => NeoMemory.enable({ ctx: { directory: dir, worktree: dir } }))
}
...
export const neoScenarios: Scenario[] = [
  ...
  http.protected.get("/neo/profile", "neo.profile").probe({ path: "/path" }).status(401),
  http.protected.get("/neo/auth-status", "neo.authStatus").json(200, (body) => {
    object(body)
    check(body.authenticated === false, "Neo auth status should report signed out")
    check(body.type === undefined, "Neo auth status should not expose a credential type while signed out")
  }),
  http.protected.get("/neo/modes", "neo.modes").json(200, (body) => {
    object(body)
    array(body.modes)
  }),
  http.protected
    .post("/neo/fim", "neo.fim")
    .at((ctx) => ({ path: "/neo/fim", headers: ctx.headers(), body: { prefix: "const value = ", suffix: "\n" } }))
    .status(401),
  http.protected
    .post("/neo/edit", "neo.edit")
    .at((ctx) => ({ path: "/neo/edit", headers: ctx.headers(), body: edit }))
    .status(401),
  http.protected
    .post("/neo/audio/transcriptions", "neo.audio.transcriptions")
    .at((ctx) => ({
      path: "/neo/audio/transcriptions",
      headers: ctx.headers(),
      body: { model: "whisper-1", input_audio: { data: "", format: "wav" } },
    }))
    .status(401),
  http.protected.get("/neo/notifications", "neo.notifications").json(200, array),
  http.protected.get("/neo/models/images", "neo.models.images").probe({ path: "/path" }).status(401),
  http.protected
    .post("/neo/organization", "neo.organization.set")
    .at((ctx) => ({ path: "/neo/organization", headers: ctx.headers(), body: { organizationId: null } }))
    .status(401),
  http.protected.get("/neo/claw/status", "neo.claw.status").probe({ path: "/path" }).status(401),
  http.protected.get("/neo/claw/chat-credentials", "neo.claw.chatCredentials").probe({ path: "/path" }).status(401),
  http.protected.get("/neo/cloud-sessions", "neo.cloudSessions").probe({ path: "/path" }).status(401),
  http.protected
    .get("/neo/cloud/session/{id}", "neo.cloud.session.get")
    .probe({ path: "/path" })
    .at((ctx) => ({ path: route("/neo/cloud/session/{id}", { id: "httpapi-missing" }), headers: ctx.headers() }))
    .status(401),
  http.protected
    .post("/neo/cloud/session/import", "neo.cloud.session.import")
    .at((ctx) => ({ path: "/neo/cloud/session/import", headers: ctx.headers(), body: { sessionId: "missing" } }))
    .status(401),
  ...
  http.protected
    ...
    .status(400),
  http.protected
    .get("/session/{sessionID}/model-usage", "neocode.sessionModelUsage")
  ...
  http.protected
    .post("/neocode/heap/snapshot", "neocode.heap.snapshot")
    .mutating()
    .jsonEffect(200, (body) =>
      Effect.gen(function* () {
  ...
  http.protected
    .get("/neocode/agent/requirements", "neocode.agentRequirements")
    .at((ctx) => ({ path: "/neocode/agent/requirements?agent=httpapi-agent", headers: ctx.headers() }))
    .json(200, (body, ctx) => {
      object(body)
  ...
  http.protected
    .post("/neocode/skill/remove", "neocode.removeSkill")
    .inProject({ git: true, init: skill })
    .mutating()
    .preserveDatabase()
    .at((ctx) => ({
      path: "/neocode/skill/remove",
      headers: ctx.headers(),
      body: { location: path.join(directory(ctx), ".neo/skill/httpapi-remove/SKILL.md") },
    }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "skill removal should return true")
        const location = path.join(directory(ctx), ".neo/skill/httpapi-remove/SKILL.md")
        const sentinel = path.join(directory(ctx), ".neo/skill/httpapi-remove/KEEP.txt")
        check(
  ...
  http.protected
    .post("/neocode/agent/remove", "neocode.removeAgent")
    .inProject({ git: true, init: agent })
    .mutating()
    .at((ctx) => ({ path: "/neocode/agent/remove", headers: ctx.headers(), body: { name: "httpapi-remove" } }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "agent removal should return true")
        const location = path.join(directory(ctx), ".neo/agent/httpapi-remove.md")
        check(!(yield* Effect.promise(() => Bun.file(location).exists())), "removed agent should not remain on disk")
      }),
    ),
  http.protected
    .post("/neocode/agent/remove", "neocode.removeAgent")
    .at((ctx) => ({ path: "/neocode/agent/remove", headers: ctx.headers(), body: { name: "httpapi-missing" } }))
    .status(400),
  http.protected
    .post("/neocode/session-import/project", "neocode.sessionImport.project")
    .mutating()
    .at((ctx) => ({
      path: "/neocode/session-import/project",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/neocode/session-import/session", "neocode.sessionImport.session")
    .mutating()
    .seeded((ctx) => ctx.project())
    .at((ctx) => ({
      path: "/neocode/session-import/session",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/neocode/session-import/message", "neocode.sessionImport.message")
    .mutating()
    .seeded((ctx) => ctx.session({ title: "Import message" }))
    .at((ctx) => ({
      path: "/neocode/session-import/message",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/neocode/session-import/part", "neocode.sessionImport.part")
    .mutating()
    .seeded((ctx) =>
      Effect.gen(function* () {
    ...
    .at((ctx) => ({
      path: "/neocode/session-import/part",
      headers: ctx.headers(),
      body: {
