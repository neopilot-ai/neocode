/**
 * Contract tests for Neo-specific fields on QuestionOption / QuestionInfo.
 *
 * packages/opencode/src/question/index.ts is a shared upstream file.
 * Two Neo additions to the Option schema have been silently dropped by
...
describe("QuestionOption schema — Neo-specific field contract", () => {
  ...
  // Static source checks — guard the neocode_change markers so a conflict
  // resolution that drops the fields is caught immediately.
  test("source declares mode as an optional field inside a neocode_change block", () => {
    const src = fs.readFileSync(SOURCE, "utf-8")
    expect(src).toMatch(/neocode_change start[^\n]*hint to UI clients/)
    expect(src).toMatch(/mode:\s*Schema\.optional\(Schema\.String\)/)
    expect(src).toMatch(/neocode_change end/)
  })

  test("source declares labelKey and descriptionKey inside a neocode_change block", () => {
    const src = fs.readFileSync(SOURCE, "utf-8")
    expect(src).toMatch(/neocode_change start[^\n]*i18n keys/)
