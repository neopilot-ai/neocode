        ...
        Effect.gen(function* () {
          ...
          const warning = parts.find(
            (part): part is MessageV2.TextPart =>
              part.type === "text" && part.text === NeoSessionProcessor.REASONING_LENGTH_WARNING,
          )

          expect(warning?.ignored).toBe(true)

          const modelMsgs = yield* MessageV2.toModelMessagesEffect([{ info: handle.message, parts }], mdl)
          expect(JSON.stringify(modelMsgs)).not.toContain(NeoSessionProcessor.REASONING_LENGTH_WARNING)
        }),
      { git: true },
    ),
  )
    ...
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          ...
          const warning = parts.find(
            (part): part is MessageV2.TextPart =>
              part.type === "text" && part.text === NeoSessionProcessor.OUTPUT_LENGTH_WARNING,
          )
          ...
          expect(json).not.toContain(NeoSessionProcessor.OUTPUT_LENGTH_WARNING)
        }),
      { git: true },
    ),
  )
      ...
      (dir) =>
        Effect.gen(function* () {
          ...
          const selection = {
            ...
            modelID: ModelV2.ID.make("neo-auto/efficient"),
          }
          ...
          yield* test.reply(
            ...
            LLMEvent.stepFinish({
              ...
              reason: "stop",
              usage: usage(),
              providerMetadata: { neocode: { routedModelID: "openai/gpt-5.5-20260423" } },
            }),
            LLMEvent.finish({ reason: "stop", usage: usage() }),
          )
