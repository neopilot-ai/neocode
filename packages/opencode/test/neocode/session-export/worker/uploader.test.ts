...
import { Storage } from "@/neocode/session-export/worker/storage"
import { Uploader, backoffFor } from "@/neocode/session-export/worker/uploader"
import { Config } from "@/neocode/session-export/config"
...
describe("Uploader", () => {
  ...
  beforeEach(() => {
    token = process.env.NEO_SESSION_EXPORT_AUTH_TOKEN
    delete process.env.NEO_SESSION_EXPORT_AUTH_TOKEN
  ...
  afterEach(() => {
    ...
    if (token === undefined) delete process.env.NEO_SESSION_EXPORT_AUTH_TOKEN
    else process.env.NEO_SESSION_EXPORT_AUTH_TOKEN = token
  })

  test("2xx response marks rows uploaded and deletes them", async () => {
    const telemetry: unknown[] = []
    const calls: Array<{ input: string; init: RequestInit }> = []
    process.env.NEO_SESSION_EXPORT_AUTH_TOKEN = "local-token"
    ...
    expect(headers.get("x-neo-export-api-version")).toBe("1")
    expect(headers.get("x-neo-export-schema-version")).toBe("1")
    expect(headers.get("x-neo-export-agent-version")).toBe("v0")
    expect(headers.get("x-neo-export-root-session-id")).toBe("s1")
    expect(headers.get("x-neo-export-session-id")).toBe("s1")
    expect(headers.get("x-neo-export-seq-start")).toBe("0")
    expect(headers.get("x-neo-export-seq-end")).toBe("0")
    expect(headers.get("x-neo-export-event-count")).toBe("1")
    expect(headers.get("x-neo-export-content-encoding")).toBe("identity")
    expect(headers.get("x-neo-export-client-sent-at")).toMatch(/^\d{4}-\d{2}-\d{2}T/)
    expect(headers.get("x-neo-export-payload-sha256")).toBe(await sha256(body))
  ...
  test("sends anonymous id with all export headers when auth token is absent", async () => {
    ...
    const headers = new Headers(calls[0].init.headers)
    const names = [
      "x-neo-export-api-version",
      "x-neo-export-schema-version",
      "x-neo-export-agent-version",
      "x-neo-export-surface",
      "x-neo-export-root-session-id",
      "x-neo-export-session-id",
      "x-neo-export-batch-id",
      "x-neo-export-seq-start",
      "x-neo-export-seq-end",
      "x-neo-export-event-count",
      "x-neo-export-payload-sha256",
      "x-neo-export-client-sent-at",
      "x-neo-export-content-encoding",
    ]

    expect(headers.get("authorization")).toBeNull()
    expect(headers.get("x-neo-anon-id")).toBe("install_abc")
  ...
  test("falls back to telemetry id when anon id is not provided", async () => {
    ...
    const headers = new Headers(calls[0].init.headers)
    expect(headers.get("authorization")).toBeNull()
    expect(headers.get("x-neo-anon-id")).toBe("install_from_disk")
  })
  ...
  test("includes surface in batch metadata and upload headers", async () => {
    ...
    expect(headers.get("x-neo-export-surface")).toBe("vscode-extension")
