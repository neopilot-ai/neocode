...
import { getKillSwitchReason, resetEligibility } from "@/neocode/session-export/eligibility"
...
describe("SessionExport worker respawn", () => {
  ...
  beforeEach(async () => {
    ...
    feature = process.env.NEOCODE_FEATURE
    resetEligibility()
  })
  ...
  afterEach(async () => {
    await SessionExport.shutdown()
    resetEligibility()
    if (feature === undefined) delete process.env.NEOCODE_FEATURE
    else process.env.NEOCODE_FEATURE = feature
  })

  test("passes surface to worker init", () => {
    const workers: FakeWorker[] = []
    process.env.NEOCODE_FEATURE = "cli"
    SessionExport.init({
      agentVersion: "v0",
      dbPath: ":memory:",
      subscribeAll: () => () => {},
