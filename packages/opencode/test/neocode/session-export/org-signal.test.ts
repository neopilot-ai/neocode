...
import { getActiveOrg, resetOrgSource, setOrgSource } from "@/neocode/session-export/eligibility"

const env = process.env.NEO_ORG_ID

describe("getActiveOrg", () => {
  beforeEach(() => {
    delete process.env.NEO_ORG_ID
    resetOrgSource()
  })

  afterEach(() => {
    if (env === undefined) delete process.env.NEO_ORG_ID
    else process.env.NEO_ORG_ID = env
    resetOrgSource()
  })
  ...
  test("returns env value when NEO_ORG_ID is set", async () => {
    setOrgSource(async () => ({ type: "org", id: "org_auth" }))
    process.env.NEO_ORG_ID = "org_envvar"
