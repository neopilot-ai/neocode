...
import { plain, session, supports, tui } from "../../src/neocode/cli/logo"

describe("neocode logo", () => {
  ...
  test("allows an override", () => {
    expect(supports({ NEO_UNICODE_LOGO: "1", SSH_TTY: "/dev/pts/0" }, "linux")).toBe(true)
    expect(supports({ NEO_UNICODE_LOGO: "0" }, "linux")).toBe(false)
  })

  test("uses modern and fallback logo variants", () => {
    expect(tui({ NEO_UNICODE_LOGO: "1" }, "linux").join("\n")).toContain("🬺🬏")
