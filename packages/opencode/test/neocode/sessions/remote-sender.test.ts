...
import { RemoteCommand } from "../../../src/neo-sessions/remote-command"
import { RemoteModelCatalog } from "../../../src/neo-sessions/remote-model-catalog"
import { RemoteSender } from "../../../src/neo-sessions/remote-sender"
import type { RemoteWS } from "../../../src/neo-sessions/remote-ws"
import type { RemoteProtocol } from "../../../src/neo-sessions/remote-protocol"
...
import { Suggestion } from "../../../src/neocode/suggestion" // neocode_change
...
describe("RemoteSender", () => {
  ...
  test("send_message keeps neocode-prefixed model unchanged before internal conversion", async () => {
    const { conn } = fakeConn()
    const calls: SessionPrompt.PromptInput[] = []
    const sender = RemoteSender.create({
      conn,
    ...
    sender.handle({
      type: "command",
      id: "req_model_neocode",
      command: "send_message",
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: "neocode/gpt-5-mini",
      },
    })
    ...
    expect(calls).toEqual([
      {
        sessionID: SessionID.make("ses_x"),
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: ProviderV2.ID.make("neo"), modelID: ModelV2.ID.make("gpt-5-mini") },
        ephemeralTools: { interactive_terminal: false },
      },
    ])
  })
  ...
  test("send_message does not special-case neo-prefixed model", async () => {
    const { conn, sent } = fakeConn()
    const calls: SessionPrompt.PromptInput[] = []
    const sender = RemoteSender.create({
      conn,
    ...
    sender.handle({
      ...
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: "neo/gpt-5-mini",
      },
    })

    await new Promise((r) => setTimeout(r, 0))

    expect(sent[0]).toEqual({ type: "response", id: "req_model_neo", result: {} })
    expect(calls).toEqual([
      {
        sessionID: SessionID.make("ses_x"),
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: ProviderV2.ID.make("neo"), modelID: ModelV2.ID.make("neo/gpt-5-mini") },
        ephemeralTools: { interactive_terminal: false },
      },
    ])
  })
...
// neocode_change start - remote slash command discovery and execution
