...
`,
      ...
      const client = process.env["NEO_CLIENT"]
      ...
      try {
        ...
        yield* Effect.promise(() => BackgroundProcess.stopSession(sessionID))
        if (client === undefined) delete process.env["NEO_CLIENT"]
        else process.env["NEO_CLIENT"] = client
      }
    }),
  )
...
`,
      ...
      const client = process.env["NEO_CLIENT"]
      const scans = process.env["NEO_BACKGROUND_PROCESS_PORTS"]
      process.env["NEO_CLIENT"] = "cli"
      process.env["NEO_BACKGROUND_PROCESS_PORTS"] = "true"
      ...
      try {
        ...
        if (scans === undefined) delete process.env["NEO_BACKGROUND_PROCESS_PORTS"]
        else process.env["NEO_BACKGROUND_PROCESS_PORTS"] = scans
      }
    }),
  )
...
`,
      ...
      const scans = process.env["NEO_BACKGROUND_PROCESS_PORTS"]
      process.env["NEO_CLIENT"] = "cli"
      delete process.env["NEO_BACKGROUND_PROCESS_PORTS"]
