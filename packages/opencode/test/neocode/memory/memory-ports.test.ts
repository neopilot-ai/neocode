...
import { MemoryModel, MemorySession } from "../../../src/neocode/memory/ports"
...
describe("memory ports", () => {
  test("session port extracts the latest turn, recall markers, and all assistant steps", async () => {
    ...
    const diffs = [
      {
        file: "packages/opencode/src/neocode/memory/ports.ts",
        additions: 4,
        deletions: 1,
    ...
    expect(view?.recent).toContain("Tool neo_memory_recall completed")
