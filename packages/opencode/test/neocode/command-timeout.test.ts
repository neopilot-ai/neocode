...
import { CommandTimeout } from "@/neocode/command-timeout"
...
const max = process.env.NEO_COMMAND_TIMEOUT_MAX_MS
const msg = process.env.NEO_COMMAND_TIMEOUT_MAX_MS_MESSAGE
...
afterEach(() => {
  if (max === undefined) delete process.env.NEO_COMMAND_TIMEOUT_MAX_MS
  else process.env.NEO_COMMAND_TIMEOUT_MAX_MS = max
  if (msg === undefined) delete process.env.NEO_COMMAND_TIMEOUT_MAX_MS_MESSAGE
  else process.env.NEO_COMMAND_TIMEOUT_MAX_MS_MESSAGE = msg
})
...
describe("CommandTimeout", () => {
  ...
  test("resolves hosted timeout policy", () => {
    for (const value of [undefined, "0", "-1", "abc"]) {
      if (value === undefined) delete process.env.NEO_COMMAND_TIMEOUT_MAX_MS
      else process.env.NEO_COMMAND_TIMEOUT_MAX_MS = value
      expect(CommandTimeout.env()).toBeUndefined()
    }

    process.env.NEO_COMMAND_TIMEOUT_MAX_MS = "250"
    process.env.NEO_COMMAND_TIMEOUT_MAX_MS_MESSAGE = " You're running in a sandbox. "
  ...
  it.effect("caps output draining at the exact environment deadline", () =>
    Effect.gen(function* () {
      process.env.NEO_COMMAND_TIMEOUT_MAX_MS = "25"
  ...
  it.effect("captures command substitutions concurrently without stderr", () =>
    Effect.gen(function* () {
      process.env.NEO_COMMAND_TIMEOUT_MAX_MS = "500"
