...
// Each NeoViewers.layer construction reads these env vars (post-refactor), so
// setting them here controls presence wiring per test.
process.env.NEO_EVENT_SERVICE_URL = "wss://test-presence"
process.env.NEO_PLATFORM = "cli"
...
const realSessions = await import("@/neo-sessions/neo-sessions")
const realSetAttached = realSessions.NeoSessions.setAttachedSessions
mock.module("@/neo-sessions/neo-sessions", () => ({
  ...realSessions,
  NeoSessions: {
    ...realSessions.NeoSessions,
...
mock.module("@/neocode/event-service/client", () => ({
...
const { NeoViewers } = await import("@/neocode/presence/service")
...
const layer = NeoViewers.layer.pipe(Layer.provide(authLayer))
...
function run(body: (viewers: {
  ...
  return Effect.gen(function* () {
    const v = yield* NeoViewers.Service
    yield* body(v)
...
describe("NeoViewers.Service presence contexts", () => {
  ...
  test("kill switch blocks the presence socket but attached union still reaches NeoSessions", async () => {
    attachedCalls.length = 0
    current = new FakeClient()
    const prev = process.env.NEO_DISABLE_PRESENCE
    process.env.NEO_DISABLE_PRESENCE = "1"
    try {
      await run((v) =>
        v.update({ viewer: { id: uid, active: true }, attached: ["ses_a"], visible: ["ses_a"] }),
      )
      ...
      if (prev === undefined) delete process.env.NEO_DISABLE_PRESENCE
      else process.env.NEO_DISABLE_PRESENCE = prev
    }
  })
})
...
describe("NeoViewers.Service viewer lifecycle", () => {
  ...
  test("account switch disconnects the old client before connecting the new one", async () => {
    ...
    await run(
      ...
      NeoViewers.layer.pipe(Layer.provide(mutableAuthLayer)),
    )
