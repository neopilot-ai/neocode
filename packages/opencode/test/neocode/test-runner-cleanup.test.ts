...
function env(marker: string) {
  const vars: NodeJS.ProcessEnv = { ...process.env, NEO_TEST_RUNNER_PID_FILE: marker }
  delete vars.NEO_TEST_PROFILE
  delete vars.NEO_TEST_SHARD
  return vars
}

function spawn(name: string, marker: string) {
  return Bun.spawn(
    ["bun", "run", "script/test-runner.ts", "--concurrency", "1", "--retries", "-1", `neocode/${name}`],
    {
      cwd: root,
      env: env(marker),
      stdout: "pipe",
...
describe("test runner cleanup", () => {
  test("removes the temp environment after an abrupt child exit", async () => {
    ...
    const state = { pid: 0 }
    const src = [
      "const marker = process.env.NEO_TEST_RUNNER_PID_FILE",
      'if (!marker) throw new Error("NEO_TEST_RUNNER_PID_FILE is required")',
