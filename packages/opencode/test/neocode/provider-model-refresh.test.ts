...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const flags = {
      source: Flag.NEO_MODELS_URL,
      path: Flag.NEO_MODELS_PATH,
      disabled: Flag.NEO_DISABLE_MODELS_FETCH,
      key: process.env.ACME_API_KEY,
    }

    yield* Effect.acquireUseRelease(
      Effect.promise(async () => {
        Flag.NEO_MODELS_URL = source
      ...
      () =>
        Effect.promise(async () => {
          Flag.NEO_MODELS_URL = flags.source
          Flag.NEO_MODELS_PATH = flags.path
          Flag.NEO_DISABLE_MODELS_FETCH = flags.disabled
  ...
  { config: { disabled_providers: ["neo", "apertis"] } },
)
