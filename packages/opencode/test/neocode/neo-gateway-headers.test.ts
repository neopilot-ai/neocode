...
import { buildNeoHeaders, getFeatureHeader, getEditorNameHeader } from "@neocode/neo-gateway"
import { HEADER_FEATURE, ENV_FEATURE, ENV_EDITOR_NAME, ENV_VERSION, DEFAULT_EDITOR_NAME } from "@neocode/neo-gateway"
...
describe("getEditorNameHeader", () => {
  ...
  it("returns default editor name without version when NEOCODE_VERSION is not set", () => {
    delete process.env[ENV_VERSION]
    expect(getEditorNameHeader()).toBe(DEFAULT_EDITOR_NAME)
  })

  it("appends version when NEOCODE_VERSION is set", () => {
    process.env[ENV_VERSION] = "1.2.3"
    expect(getEditorNameHeader()).toBe(`${DEFAULT_EDITOR_NAME} 1.2.3`)
  })
})

describe("buildNeoHeaders", () => {
  const original = process.env[ENV_FEATURE]
  ...
  it("always includes editor name header", () => {
    delete process.env[ENV_FEATURE]
    const headers = buildNeoHeaders()
    expect(headers["X-NEOCODE-EDITORNAME"]).toBe(getEditorNameHeader())
  })
