...
const script = join(import.meta.dir, "..", "..", "bin", "neo")
const platform = process.platform === "win32" ? "windows" : process.platform
const binary = platform === "windows" ? "neo.exe" : "neo"

describe("bin/neo tree-sitter resources", () => {
  async function setup(root: string, nested: boolean) {
    const dir = nested
      ? join(root, "node_modules", "@neocode", `cli-${platform}-${process.arch}`, "bin")
      : join(root, "node_modules", "@neocode", "cli", "bin")
    const wasm = join(dir, "tree-sitter")
    const bin = join(dir, nested ? binary : ".neo")
    ...
    return { bin, log, wasm, wrapper: join(dir, "neo") }
  }

  async function run(root: string, bin: string | undefined, log: string, wrapper?: string, failCached?: boolean) {
    const capture = `
const { EventEmitter } = require("events")
const neoFs = require("fs")
const neoChild = require("child_process")
const log = process.argv[1]
const wrapper = process.argv[2]
const failCached = process.argv[3] === "true"
const realpathSync = neoFs.realpathSync
neoFs.realpathSync = (file) => file === __filename ? wrapper || process.cwd() : realpathSync(file)
neoChild.spawn = (target) => {
  if (failCached && target.endsWith(".neo")) throw new Error("cached binary failed")
  neoFs.writeFileSync(log, process.env.NEO_TREE_SITTER_WASM_DIR || "")
...
`
  ...
  test("exports co-located tree-sitter WASM dir for optional package binary", async () => {
    const root = await mkdtemp(join(tmpdir(), "neo-bin-tree-sitter-"))
    try {
