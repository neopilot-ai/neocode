...
import { MemoryRecallTool } from "@/neocode/tool/memory-recall"
...
describe("neo_memory_recall description", () => {
...
describe("neo_memory_recall", () => {
  ...
  test("shows typed memory hits separately from session digests", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".neo"), async () => {
      const memory = { directory: dir.path, worktree: dir.path }
      const enabled = await NeoMemory.enable({ ctx: memory })
        ...
        ops: [
          {
            ...
            section: "Commands",
            key: "vscode_tests",
            text: "Run VS Code unit tests from packages/neo-vscode with bun run test:unit.",
          },
          {
            action: "add",
            file: "project.md",
      ...
      expect(typed.title).toContain("Neo memory typed")
      ...
      expect(digest.title).toContain("Neo memory digest")
  ...
  test("catalog mode lists all stored keys with optional filter", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".neo"), async () => {
        ...
        ops: [
          {
            ...
            file: "project.md",
            section: "Facts",
            key: "neo_was_originally_a_fork",
            text: "neo was originally a fork of roo and has a neocode-legacy repo",
          },
          {
            action: "add",
            file: "environment.md",
      ...
      expect(all.output).toContain("neo_was_originally_a_fork")
      expect(all.output).toContain("vscode_tests")
      expect(filtered.output).toContain("neo_was_originally_a_fork")
  ...
  test("search mode renders typed and digest memory without catalog mode", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".neo"), async () => {
      ...
      expect(result.title).toContain("Neo memory search")
  ...
  test("handles large unmatched recall queries without returning stored memory", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".neo"), async () => {
      ...
      expect(result.title).toBe("Neo memory search: no results")
