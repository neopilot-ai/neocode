...
import {
  ...
  IMAGE_MODELS,
  DEFAULT_MODEL,
} from "../../../src/neocode/tool/generate-image"
...
describe("generate-image provider resolver", () => {
  test("uses Neo cloud when Neo auth is present", () => {
    const result = resolveProvider({ type: "oauth", access: "neo-token", accountId: "org-123" }, undefined)
    expect(result).not.toBeNull()
    expect(result!.token).toBe("neo-token")
    expect(result!.organizationId).toBe("org-123")
    expect(result!.provider).toBe("neo")
    expect(result!.url).toContain("openrouter")
  })

  test("uses Neo cloud with API key auth", () => {
    const result = resolveProvider({ type: "api", key: "neo-api-key" }, undefined)
    expect(result!.token).toBe("neo-api-key")
    expect(result!.provider).toBe("neo")
  })

  test("falls back to OpenRouter with BYO key when no Neo auth", () => {
  ...
  test("prefers Neo auth over OpenRouter key", () => {
    const result = resolveProvider({ type: "oauth", access: "neo-token" }, "or-key")
    expect(result!.provider).toBe("neo")
    expect(result!.token).toBe("neo-token")
  })
})
