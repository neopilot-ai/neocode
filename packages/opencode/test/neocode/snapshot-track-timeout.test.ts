...
//
// Unit tests for NeoSnapshotTrack.wrap — the slow-repo guard that sits
...
import { NeoSnapshotTrack } from "../../src/neocode/snapshot/track"
import { NeoPartLifecycle } from "../../src/neocode/session/part-lifecycle"
...
const makeHooks = (
  answer: NeoSnapshotTrack.Answer | Promise<NeoSnapshotTrack.Answer>,
): { hooks: NeoSnapshotTrack.Hooks; calls: Calls } => {
  const calls: Calls = { ask: 0, persist: 0, progress: [] }
  const hooks: NeoSnapshotTrack.Hooks = {
    async ask() {
      calls.ask += 1
      return answer
    },
...
describe("NeoSnapshotTrack.protect", () => {
  it.effect("returns at the availability deadline without waiting for cancellation", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* NeoSnapshotTrack.protect({
        inner,
        state,
        fallback,
        operation: "patch",
  ...
  it.effect("bypasses later operations after a deadline opens the directory circuit", () =>
    Effect.gen(function* () {
      ...
      const started = yield* Deferred.make<void>()
      let calls = 0
      const first = yield* NeoSnapshotTrack.protect({
        inner: Effect.sync(() => {
          calls += 1
      ...
      const second = yield* NeoSnapshotTrack.protect({
        inner: Effect.sync(() => {
          calls += 1
          return "unexpected"
        }),
  ...
  test("keeps circuit state isolated by directory", () => {
    const states = NeoSnapshotTrack.makeStates()
...
describe("NeoSnapshotTrack.wrap", () => {
  ...
  test("concurrent timeout does not override an active continue choice", async () => {
    const state = NeoSnapshotTrack.makeState()
    const answer = Promise.withResolvers<NeoSnapshotTrack.Answer>()
    const asked = Promise.withResolvers<void>()
    const { hooks, calls } = makeHooks(answer.promise)
    const firstHooks: NeoSnapshotTrack.Hooks = {
      ...hooks,
      async ask(input) {
        asked.resolve()
...
describe("NeoSnapshotTrack progress indicator", () => {
  test("classifies persisted progress as transient", () => {
    const part = NeoSnapshotTrack.progressPart({
      sessionID: SESSION,
      messageID: MESSAGE,
    ...
    expect(NeoPartLifecycle.transient(part)).toBe(true)
  })
  ...
  test("slow-but-succeeding path (under timeout) starts then ends the indicator", async () => {
    ...
    expect(withoutFrame(firstText.text)).toBe(
      NeoSnapshotTrack.formatProgress(NeoSnapshotTrack.PROGRESS_INITIALIZING, "").trim(),
    )
  ...
  test("timed-out path keeps the initializing label (no text escalation)", async () => {
    ...
    const base = NeoSnapshotTrack.formatProgress(NeoSnapshotTrack.PROGRESS_INITIALIZING, "").trim()
    for (const evt of events) {
...
describe("NeoSnapshotTrack constants", () => {
  test("TIMEOUT_MS defaults to 10s and respects env override", () => {
    ...
    // with NEO_SNAPSHOT_TRACK_TIMEOUT_MS, which this test suite does not set.
    expect(NeoSnapshotTrack.TIMEOUT_MS).toBe(10_000)
  })

  test("exposes stable answer labels", () => {
    expect(NeoSnapshotTrack.ANSWER_CONTINUE).toBe("Continue with snapshots")
    expect(NeoSnapshotTrack.ANSWER_DISABLE).toBe("Disable for this project")
  })
})
