...
import { NotebookEditTool, NotebookExecuteTool, NotebookReadTool } from "@/neocode/tool/notebook-host"
...
test("uses dedicated VS Code notebook permission defaults only when enabled", () => {
  ...
  try {
    ...
    const disabled = NeoAgent.prepare({}).defaultsPatch
    expect(disabled.some((rule) => rule.permission.startsWith("notebook_"))).toBe(false)

    const rules = NeoAgent.prepare({ experimental: { native_notebook_tools: true } }).defaultsPatch
