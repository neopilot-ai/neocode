...
import { BackgroundProcess } from "../../src/neocode/background-process"
...
import * as SandboxPolicy from "../../src/neocode/sandbox/policy"
...
describe("Neo task nesting", () => {
  ...
  test("preserves inherited restrictions while refreshing prompt tool toggles", () => {
    const permission = NeoSessionPrompt.mergeToolPermissions({
      existing: [
      ...
      () =>
        Effect.gen(function* () {
          ...
          const effective = Permission.merge(
            ...
            NeoSessionPrompt.guardPermissions({ agent: validator, session: child }),
          )
