...
it.live("nvidia provider includes neocode billing origin header", () =>
    ...
    withNvidiaKey(
      Provider.Service.use((provider) =>
        Effect.gen(function* () {
          ...
          expect(headers["HTTP-Referer"]).toBe("https://neo.khulnasoft.com/")
          expect(headers["X-Title"]).toBe("Neo Code")
          expect(headers["X-BILLING-INVOKE-ORIGIN"]).toBe("neocode")
        }),
      ),
    ),
  ),
