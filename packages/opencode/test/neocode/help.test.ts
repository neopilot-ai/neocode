...
import { generateHelp, generateCommandTable } from "../../src/neocode/help"
...
import { ProvidersCommand } from "../../src/cli/cmd/providers" // neocode_change — upstream renamed auth → providers
...
import { HelpCommand } from "../../src/neocode/help-command"
import { ProfileCommand } from "../../src/neocode/cli/cmd/profile"
import { DaemonCommand } from "../../src/neocode/cli/cmd/daemon"
import { NeoConsoleCommand } from "../../src/neocode/cli/cmd/console"
...
const AttachStub = {
  command: "attach <url>",
  describe: "attach to a running neo server",
  handler() {},
}
...
describe("neo help --all (markdown)", () => {
  test("contains ## heading for each known top-level command", async () => {
    ...
    for (const cmd of ["run", "auth", "debug", "mcp", "session", "agent", "profile"]) {
      expect(output).toContain(`## neo ${cmd}`)
    }
  })

  test("contains headings for nested subcommands", async () => {
    const output = await generateHelp({ all: true, format: "md", commands })
    expect(output).toContain("neo auth login")
    expect(output).toContain("neo auth logout")
    expect(output).toContain("neo debug config")
  })
})

describe("neo help --all (text)", () => {
...
describe("neo help <command>", () => {
  test("neo help auth contains auth subcommand headings", async () => {
    const output = await generateHelp({ command: "auth", format: "md", commands })
    expect(output).toContain("neo auth login")
    expect(output).toContain("neo auth logout")
    expect(output).toContain("neo auth list")
  })

  test("neo help auth does NOT contain run or debug headings", async () => {
    const output = await generateHelp({ command: "auth", format: "md", commands })
    expect(output).not.toContain("## neo run")
    expect(output).not.toContain("## neo debug")
  })

  test("documents console stop and foreground mode", async () => {
    const output = await generateHelp({ command: "console", format: "md", commands })
    expect(output).toContain("neo console stop")
  ...
  test("documents daemon foreground mode", async () => {
    ...
    expect(output).toContain("neo daemon start")
    expect(output).toContain("--foreground")
    expect(output).toContain("-f")
  })
})
...
describe("edge cases", () => {
  ...
  test("neo help nonexistent throws unknown command error", async () => {
...
describe("generateCommandTable", () => {
  ...
  test("contains rows for known commands", async () => {
    ...
    for (const name of ["run", "auth", "debug", "mcp"]) {
      expect(output).toContain(`neo ${name}`)
    }
  })

  test("default command appears as neo [project], not $0", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`neo [project]`")
  ...
  test("skips commands with no describe", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).not.toContain("`neo generate`")
  })

  test("contains neo completion row", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`neo completion`")
  })

  test("contains neo help row", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`neo help")
  })
})

describe("Neo CLI customizations are wired into index.ts", () => {
  const file = (rel: string) => Bun.file(path.resolve(import.meta.dir, rel)).text()
  const INDEX = "../../src/index.ts"
  const SETUP = "../../src/neocode/cli/setup.ts"
  const BARREL = "../../src/neocode/commands.ts"

  test("CLI is branded `neo`, not `opencode`", async () => {
    const index = await file(INDEX)
    expect(index).toContain('.scriptName("neo")')
    expect(index).not.toContain('.scriptName("opencode")')
  })

  test("index.ts invokes the NeoCli integration points", async () => {
    // These thin call-sites are the only wiring between upstream index.ts and the Neo
    // customizations in setup.ts. If a future upstream merge drops them, every Neo command
    // and the telemetry/lifecycle hooks silently disappear, exactly the regression this guards.
    const index = await file(INDEX)
    expect(index).toContain("NeoCli.register(")
    expect(index).toContain("NeoCli.bootstrap(")
    expect(index).toContain("NeoCli.shutdown(")
  })

  test("registers the local Neo Console instead of the upstream account console", async () => {
    const index = await file(INDEX)
    const setup = await file(SETUP)
    const barrel = await file(BARREL)
    expect(setup).toContain("NeoConsoleCommand")
