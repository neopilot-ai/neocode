...
import { createNeoClient } from "@neocode/sdk/v2/client"
...
const dbfile = path.join(os.tmpdir(), `neo-fork-${process.pid}-${crypto.randomUUID()}.db`)
...
describe("Session.fork task detachment", () => {
  ...
  test(
    ...
    async () => {
      const flag = Flag.NEO_EXPERIMENTAL_WORKSPACES
      ...
      try {
        ...
        Flag.NEO_EXPERIMENTAL_WORKSPACES = flag
      }
    },
    { timeout: 30000 },
  )
