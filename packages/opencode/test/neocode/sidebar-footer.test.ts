...
import { billable, creditLabel, format, passLine, resetLabel, scope } from "../../src/neocode/plugins/sidebar-footer"
...
describe("Neo sidebar footer", () => {
  ...
  test("shows pass period usage and reset date", () => {
    expect(passLine(neoPass)).toBe("$73 / $199")
    expect(resetLabel(neoPass.nextBillingAt)).toBe("Jul 1")
    expect(resetLabel(null)).toBeUndefined()
    expect(resetLabel("not-a-date")).toBeUndefined()
  })

  test("refreshes only after completed billed Neo turns", () => {
