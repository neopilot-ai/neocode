...
describe("indexing worktrees", () => {
  test("shares the primary checkout index with a linked worktree", async () => {
    ...
    const worktree = path.join(tmp.path, ".neo", "worktrees", "feature")
    ...
    await provideTestInstance({
      directory: worktree,
      fn: async () => {
        await NeoIndexing.search("worktree")
        expect((await NeoIndexing.current()).state).toBe("Complete")
        expect(await NeoIndexing.available()).toBe(true)
      },
    })
  ...
  test("does not classify an ordinary directory from its pathname", async () => {
    await using tmp = await tmpdir({ git: true, config: cfg })
    process.env["NEO_CONFIG_DIR"] = tmp.path
    const directory = path.join(tmp.path, ".neocode", "worktrees", "feature")
    ...
    await provideTestInstance({
      directory,
      fn: async () => {
        await NeoIndexing.search("ordinary directory")
        expect((await NeoIndexing.current()).state).toBe("Complete")
        expect(await NeoIndexing.available()).toBe(true)
      },
    })
