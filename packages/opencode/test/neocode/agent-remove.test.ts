...
import { remove } from "../../src/neocode/agent"
import type { Info as AgentInfo } from "../../src/agent/agent"
import { tmpdir } from "../fixture/fixture"

describe("Neo agent remove", () => {
  test("removes config-backed imported agents", async () => {
    await using tmp = await tmpdir()
    const dir = path.join(tmp.path, ".neo")
    const file = path.join(dir, "neo.jsonc")
  ...
  "agent": {
    ...
    expect(cfg.agent.code.model).toBe("neo/gpt-5")
  })
})
