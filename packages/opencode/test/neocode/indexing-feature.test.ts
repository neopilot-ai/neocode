...
} from "../../src/neocode/indexing-feature"
...
describe("indexing plugin helpers", () => {
  test("detects plugin-enabled configs", () => {
    ...
    expect(indexingEnabled({ plugin: ["@neocode/neo-indexing@1.0.0"] })).toBe(true)
  })
  ...
  test("does not add duplicate indexing plugin", () => {
    const list = ensureIndexingPlugin(["@neocode/neo-indexing@1.0.0"], INDEXING_PLUGIN)
    expect(list).toEqual(["@neocode/neo-indexing@1.0.0"])
  })
