...
import { filterPromptTrainingModels, nonEmptyProviders } from "../../../src/neocode/provider/model-filter"

function model(id: string, training?: boolean): Provider.Model {
  return {
    id: ModelV2.ID.make(id),
    providerID: ProviderV2.ID.neo,
    api: { id: "neo", url: "https://api.neo.khulnasoft.com", npm: "@neocode/neo-gateway" },
    name: id,
    capabilities: {
      temperature: true,
      reasoning: false,
...
describe("prompt-training model filter", () => {
  test("hides only explicitly marked Neo Gateway models", () => {
    const providers = {
      neo: provider("neo", {
    ...
    expect(Object.keys(result.neo.models)).toEqual(["private", "unknown"])
    expect(Object.keys(result.other.models)).toEqual(["training"])
    expect(Object.keys(providers.neo.models)).toEqual(["training", "private", "unknown"])
  })

  test("preserves the catalog when disabled", () => {
    const providers = { neo: provider("neo", { training: model("training", true) }) }
