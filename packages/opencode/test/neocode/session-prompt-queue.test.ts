...
import { NeoSessionCompaction } from "@/neocode/session/compaction"
import { NeoSessionPromptQueue } from "@/neocode/session/prompt-queue"
import { Suggestion } from "../../src/neocode/suggestion"
...
const previous = Flag.NEO_DB
const dbfile = path.join(os.tmpdir(), `neo-prompt-queue-${process.pid}-${crypto.randomUUID()}.db`)
...
describe("session prompt queue", () => {
  test("scopes queued turns without moving prior assistant history", async () => {
    ...
    const ids = await Effect.runPromise(
      NeoSessionPromptQueue.enqueue(
        sessionID,
        two,
        Effect.sync(() => NeoSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id)),
        Effect.succeed([]),
      ),
    )
  ...
  test("retarget keeps older queued prompts hidden", async () => {
    ...
    const result = await Effect.runPromise(
      NeoSessionPromptQueue.enqueue(
        ...
        base,
        Effect.sync(() => {
          NeoSessionPromptQueue.retarget(sessionID, injected)
          return {
            active: NeoSessionPromptQueue.active(sessionID),
            ids: NeoSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id),
          }
        }),
        ...
        const result = await Effect.runPromise(
          NeoSessionPromptQueue.enqueue(
            ...
            Effect.promise(async () => {
              ...
              return { compact, ids: NeoSessionPromptQueue.scope(session.id, messages).map((item) => item.info.id) }
            }),
  ...
  test("hasFollowup reports true only for prompts enqueued after the active slot started", async () => {
    ...
    const first = Effect.runPromise(
      NeoSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "first:start", value: NeoSessionPromptQueue.hasFollowup(sessionID) })
          firstStarted.resolve()
          yield* Effect.promise(() => firstReleased.promise)
          observed.push({ where: "first:end", value: NeoSessionPromptQueue.hasFollowup(sessionID) })
          return "first"
        }),
    ...
    const second = Effect.runPromise(
      NeoSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "second:start", value: NeoSessionPromptQueue.hasFollowup(sessionID) })
    ...
    expect(NeoSessionPromptQueue.hasFollowup(sessionID)).toBe(true)
    ...
    const third = Effect.runPromise(
      NeoSessionPromptQueue.enqueue(
        ...
        Effect.sync(() => {
          observed.push({ where: "third:start", value: NeoSessionPromptQueue.hasFollowup(sessionID) })
          return "third"
        }),
    ...
    expect(NeoSessionPromptQueue.hasFollowup(sessionID)).toBe(false)
  ...
  test("cancel on a session with no active tail is a no-op and does not leak state", async () => {
    ...
    await Effect.runPromise(NeoSessionPromptQueue.cancel(sessionID))

    expect(NeoSessionPromptQueue._hasInternalState(sessionID)).toBe(false)
          ...
          scoped(tmp.path, async (prompt) => {
            ...
            const ids = await Effect.runPromise(
              NeoSessionPromptQueue.enqueue(
                session.id,
                MessageID.make("msg_probe"),
                Effect.succeed(NeoSessionPromptQueue.scope(session.id, []).map((item) => item.info.id)),
                Effect.succeed([]),
              ),
            )
            expect(ids).toEqual([])
            expect(NeoSessionPromptQueue.hasFollowup(session.id)).toBe(false)
          }),
      })
    } finally {
      server.stop(true)
