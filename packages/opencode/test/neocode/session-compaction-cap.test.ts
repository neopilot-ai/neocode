    ...
    () =>
      provideTmpdirServer(
        Effect.fnUntraced(function* ({ llm }) {
          ...
          const turnClose = yield* Deferred.make<NeoSession.CloseReason>()
          const unsub = Bus.subscribe(NeoSession.Event.TurnClose, (evt) => {
          ...
          expect(yield* llm.calls).toBe(NeoSessionPrompt.MAX_COMPACTION_ATTEMPTS * 2 + 1)
...
describe("NeoSessionPrompt.guardCompactionAttempt", () => {
  it.effect("returns { exhausted: false } and does not mutate state below the cap", () =>
    Effect.sync(() => {
      ...
      const result = NeoSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_under",
        attempts: NeoSessionPrompt.MAX_COMPACTION_ATTEMPTS - 1,
        closeReasons,
        message: msg,
      })
  ...
  it.effect("sets close reason and attaches error once attempts reach the cap", () =>
    Effect.sync(() => {
      ...
      const result = NeoSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_cap",
        attempts: NeoSessionPrompt.MAX_COMPACTION_ATTEMPTS,
        closeReasons,
        message: msg,
      })
