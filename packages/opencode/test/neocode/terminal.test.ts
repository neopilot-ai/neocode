...
import { kitty, sequences } from "../../src/neocode/cli/cmd/tui/util/terminal"

const keys = ["TERM_PROGRAM", "MSYSTEM", "NEO_DISABLE_KITTY_KEYBOARD", "NEO_ENABLE_KITTY_KEYBOARD"] as const
...
test("allows explicitly enabling Kitty keyboard", () => {
  env({ NEO_ENABLE_KITTY_KEYBOARD: "1", MSYSTEM: "MINGW64" })
...
test("allows explicitly disabling Kitty keyboard", () => {
  env({ NEO_DISABLE_KITTY_KEYBOARD: "1", NEO_ENABLE_KITTY_KEYBOARD: "1" })
...
test("resets common terminal input modes", () => {
  env({ NEO_DISABLE_KITTY_KEYBOARD: "1" })
