// Regression test: OAuth accountId must flow into model fetch as neocodeOrganizationId
...
type Options = Parameters<ModelCache.NeoModels["fetch"]>[0]

function layer(info: Auth.Info | undefined, captured: Ref.Ref<Options | undefined>) {
  const auth = Layer.mock(Auth.Service)({
    get: (id) => Effect.succeed(id === "neo" ? info : undefined),
  })
...
it.live("model fetch uses accountId from OAuth auth as neocodeOrganizationId", () =>
  Effect.gen(function* () {
    ...
    yield* ModelCache.Service.use((cache) => cache.fetch("neo")).pipe(Effect.provide(layer(info, captured)))
    expect(yield* Ref.get(captured)).toMatchObject({
      neocodeToken: "test-oauth-token",
      neocodeOrganizationId: "org-enterprise-123",
    })
  }),
)

it.live("model fetch without OAuth accountId does not set neocodeOrganizationId", () =>
  Effect.gen(function* () {
    const captured = yield* Ref.make<Options | undefined>(undefined)
    const info = new Auth.Oauth({
      type: "oauth",
    ...
    yield* ModelCache.Service.use((cache) => cache.fetch("neo")).pipe(Effect.provide(layer(info, captured)))
    expect(yield* Ref.get(captured)).toMatchObject({ neocodeToken: "test-personal-token" })
    expect((yield* Ref.get(captured))?.neocodeOrganizationId).toBeUndefined()
  }),
)
  ...
  Effect.gen(function* () {
    ...
    yield* ModelCache.Service.use((cache) =>
      Effect.gen(function* () {
        ...
        expect(yield* cache.get("neo")).toBeDefined()

        yield* cache.clear("neo")
        expect(yield* cache.get("neo")).toBeUndefined()
