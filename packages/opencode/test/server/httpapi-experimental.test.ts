...
import { realpath } from "node:fs/promises" // neocode_change
      ...
      Effect.gen(function* () {
        ...
        yield* withCreatedWorktree(tmp.directory, (info) =>
          Effect.gen(function* () {
            ...
            const directory = yield* Effect.promise(() => realpath(info.directory)) // neocode_change
            expect(yield* json(listed)).toContainEqual({ directory, managed: true }) // neocode_change
