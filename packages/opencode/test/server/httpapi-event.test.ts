...
import * as Sse from "effect/unstable/encoding/Sse" // neocode_change - decode the legacy SSE wire format
import { EventPaths } from "../../src/server/routes/instance/httpapi/groups/event"
// neocode_change start - verify transformed EventV2 values at the legacy SSE boundary
...
// neocode_change start - inspect the real global SSE envelope
...
// neocode_change start - instance SSE also carries Neo's legacy Bus events and `sync` envelopes
...
/** Skip Neo's ambient instance events (indexing.status, sync envelopes, ...) until `type` shows up. */
...
// neocode_change start - read transformed values from the global SSE wire payload
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // neocode_change - the instance stream also carries Neo's ambient events (indexing.status, sync
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // neocode_change - skip ambient instance events that may interleave before session.created
        expect(yield* readEventOfType(reader, "session.created")).toMatchObject({ type: "session.created" })
      }),
    { git: true, config: { formatter: false, lsp: false } },
  )

  // neocode_change start - transformed EventV2 data is numeric on legacy SSE while domain data stays decoded
  ...
  v2.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // neocode_change - session.next.prompted is a durable event whose projector writes a session_message
        ...
        const sessionID = yield* Effect.promise(async () => {
          const created = await Server.Default().app.request("/session", {
            method: "POST",
            headers: { "x-neo-directory": directory, "content-type": "application/json" },
            body: "{}",
          })
        ...
        yield* events.publish(SessionEvent.Prompted, {
          ...
          delivery: "queue",
          prompt: new Prompt({ text: "hello", files: [], agents: [] }), // neocode_change - upstream made prompt a Prompt class
        })
