 ...
 *
 * The script intentionally isolates `NEO_DB` before importing modules that touch
...
import { neoScenarios } from "../../neocode/server/httpapi-exercise-scenarios" // neocode_change
...
const scenarios: Scenario[] = [
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-neo-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-neo-directory")
  }),
  ...
  http.protected.get("/neocode/notebook", "neocode.notebook.list").json(200, array),
  http.protected
    .post("/neocode/notebook/{requestID}/reply", "neocode.notebook.reply")
    .at((ctx) => ({
      path: route("/neocode/notebook/{requestID}/reply", { requestID: "nbr_httpapi_reply" }),
      headers: ctx.headers(),
      body: {
        result: {
          operation: "read",
    ...
    .json(404, object, "status"),
  http.protected
    .post("/neocode/notebook/{requestID}/reject", "neocode.notebook.reject")
    .at((ctx) => ({
      path: route("/neocode/notebook/{requestID}/reject", { requestID: "nbr_httpapi_reject" }),
      headers: ctx.headers(),
      body: { error: { code: "not_found", message: "Notebook not found" } },
    }))
    .json(404, object, "status"),
  http.protected.get("/neocode/agent-manager", "neocode.agentManager.list").json(200, array),
  http.protected
    .post("/neocode/agent-manager/{requestID}/reply", "neocode.agentManager.reply")
    .at((ctx) => ({
      path: route("/neocode/agent-manager/{requestID}/reply", { requestID: "amr_httpapi_reply" }),
      headers: ctx.headers(),
      body: { result: { operation: "overview", overview: { sections: [], ungrouped: [] } } },
    }))
    .json(404, object, "status"),
  http.protected
    .post("/neocode/agent-manager/{requestID}/reject", "neocode.agentManager.reject")
    .at((ctx) => ({
      path: route("/neocode/agent-manager/{requestID}/reject", { requestID: "amr_httpapi_reject" }),
      headers: ctx.headers(),
  ...
  http.protected
    ...
    .status(500, undefined, "status"), // neocode_change
  http.protected
