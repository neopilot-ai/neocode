    ...
    Effect.flatMap((context) =>
      Effect.gen(function* () {
        ...
        const base: ScenarioContext = {
          ...
          headers: (extra) => ({
            ...(context.dir?.path ? { "x-neo-directory": context.dir.path } : {}),
            ...extra,
          }),
          file: (name, content) =>
            Effect.promise(() => {
...
const resetState = Effect.promise(async () => {
  ...
  // neocode_change - each exerciser process already owns an isolated DB; unlinking it between scenarios races async Neo callbacks
  await Bun.sleep(25)
})
