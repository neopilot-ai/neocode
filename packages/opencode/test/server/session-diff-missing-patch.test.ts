 ...
 * session never loaded. // neocode_change
 * Neo still surfaces cumulative session diffs to its TUI and VS Code clients. // neocode_change
 *
 * This test inserts a session row with a missing-patch diff entry and
 * asserts that GET /session/<id>/diff returns 200 with the row intact. // neocode_change
 */
...
describe("session diff with missing patch (#26574)", () => {
  it.instance(
    "GET /session/<id>/diff returns cumulative session diffs", // neocode_change
    () =>
      Effect.gen(function* () {
        ...
        expect(yield* response.json).toEqual([{ file: "legacy.txt", additions: 1, deletions: 0 }]) // neocode_change
      }),
