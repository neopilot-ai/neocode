...
import { awaitWithTimeout, pollWithTimeout, testEffect } from "../lib/effect" // neocode_change
...
type Sdk = ReturnType<typeof createNeoClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.NEO_SERVER_PASSWORD = input?.password
      Flag.NEO_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".neo", "skills", "project-rest-skill", "SKILL.md"), // neocode_change
      `---
...
describe("HttpApi SDK", () => {
  ...
  httpapiInstance(
    ...
    ({ sdk, directory }) =>
      Effect.gen(function* () {
        ...
        const raw = yield* call(() => sdk.v2.fs.read({ path: "hello.txt" })) // neocode_change
        const session = yield* call(() => sdk.session.create({ title: "sdk" }))
        const v2session = yield* call(() => sdk.v2.session.create({ agent: "build" })) // neocode_change
        ...
        expect({ status: v2session.response.status, error: v2session.error }).toEqual({ status: 200, error: undefined }) // neocode_change
        expect(v2session.data).toMatchObject({ data: { location: { directory } } }) // neocode_change
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-neo-directory")).toBe(false)
        expect(request!.headers.has("x-neo-workspace")).toBe(false)

        // neocode_change start - encoded legacy directory headers still route on payload requests
        const legacy = yield* client("raw", undefined, {
          headers: { "x-neo-directory": encodeURIComponent(directory) },
        })
  ...
  httpapiInstance(
    ...
    ({ directory }) =>
      Effect.gen(function* () {
        ...
        // neocode_change start - match Hono AuthMiddleware username default ("neo")
        const badSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("neo", "wrong") },
        })
        const bad = yield* capture(() => badSdk.file.read({ path: "hello.txt" }))
        const goodSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("neo", "secret") },
        })
  ...
  // neocode_change start - verify invalid user images fail at the real SDK boundary
  ...
  // neocode_change start - verify provider errors remain in successful assistant messages
