...
const originalWorkspaces = Flag.NEO_EXPERIMENTAL_WORKSPACES
...
// neocode_change start - released V2 clients persisted media-shaped tool content
...
afterEach(async () => {
  Flag.NEO_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-neo-directory": test.directory }
  ...
  // neocode_change start - protect mixed-version session database compatibility
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const response = yield* request(`/api/session/${session.id}/message`, {
          headers: { "x-neo-directory": test.directory },
        })
        expect(response.status).toBe(200)
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-neo-directory": test.directory, "content-type": "application/json" }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.NEO_EXPERIMENTAL_WORKSPACES = true
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-neo-directory": test.directory, "content-type": "application/json" },
