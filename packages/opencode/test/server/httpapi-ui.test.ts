  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("NEO_SERVER_PASSWORD", original.envPassword)
        restoreEnv("NEO_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            NEO_SERVER_PASSWORD: input?.password,
            NEO_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  // neocode_change start - embedded UI is the only supported fallback; never proxy to app.opencode.ai
  ...
  it.live("accepts auth token for the web UI", () =>
    Effect.gen(function* () {
      let proxied = false // neocode_change
      const response = yield* uiApp({
        password: "secret",
        username: "neo", // neocode_change
        disableEmbeddedWebUi: true,
        // neocode_change start - authenticated requests still must not proxy when embedded UI is disabled
        client: httpClient(new Response("<html>neo</html>", { headers: { "content-type": "text/html" } }), () => {
          proxied = true
        }),
        // neocode_change end
      }).request(`/?auth_token=${btoa("neo:secret")}`)
  ...
  it.live("accepts basic auth for the web UI", () =>
    Effect.gen(function* () {
      ...
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("neo:secret")}` },
      })
  ...
  it.live("accepts basic auth passwords containing colons for the web UI", () =>
    Effect.gen(function* () {
      ...
      expect(response.status).toBe(404) // neocode_change - auth succeeds, but Neo does not proxy a fallback UI
    }),
  )
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "neo" }).request("/", {
        // neocode_change
        method: "OPTIONS",
        headers: {
