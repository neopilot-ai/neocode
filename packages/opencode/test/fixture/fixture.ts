...
import { ProjectV2 } from "@opencode-ai/core/project" // neocode_change
import { ProjectTable } from "@opencode-ai/core/project/sql" // neocode_change
import { AbsolutePath } from "@opencode-ai/core/schema" // neocode_change
import { ChildProcess, ChildProcessSpawner } from "effect/unstable/process"
import type { Config } from "@/config/config"
import { InstanceRef } from "../../src/effect/instance-ref"
import { InstanceBootstrap } from "../../src/project/bootstrap-service"
import { context as instanceContext, type InstanceContext } from "../../src/project/instance-context" // neocode_change
...
export async function provideTestInstance<R>(input: {
  ...
  try {
    ...
    return await instanceContext.provide(ctx, () => input.fn(ctx)) // neocode_change
  } finally {
...
export async function withTestInstance<R>(input: { directory: string; fn: (ctx: InstanceContext) => R }) {
  const ctx = await runTestInstanceStore((store) => store.load({ directory: input.directory }))
  return instanceContext.provide(ctx, () => input.fn(ctx)) // neocode_change
}
...
function clean(dir: string) {
  return cleanup(dir) // neocode_change
}
    ...
    if (options?.config) {
      ...
      yield* Effect.promise(() =>
        fs.writeFile(
          ...
          JSON.stringify({ $schema: "https://app.neo.khulnasoft.com/config.json", ...resolved }), // neocode_change
        ),
      )
    }
...
// neocode_change start - custom test runtimes need the instance project in their core database
