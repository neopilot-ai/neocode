...
export function createFetch(override?: FetchHandler) {
  ...
  const fetch = (async (input: RequestInfo | URL) => {
    ...
    switch (url.pathname) {
      case "/agent":
      case "/command":
      // case "/experimental/workspace": // neocode_change
      case "/experimental/workspace/status":
      case "/formatter":
      case "/lsp":
      case "/network": // neocode_change
      case "/background-process": // neocode_change
        return json([])
      case "/config":
      case "/experimental/resource":
      case "/global/config": // neocode_change
      case "/mcp":
      case "/provider/auth":
      case "/session/status":
        return json({})
      ...
      case "/project/current":
        return json({ id: "proj_test", worktree: "/tmp/project-root" }) // neocode_change
      case "/provider":
