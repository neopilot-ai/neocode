...
import * as launch from "../../src/lsp/launch" // neocode_change - spy on spawn
import { CrossSpawnSpawner } from "@opencode-ai/core/cross-spawn-spawner"
import { provideTestInstance, provideTmpdirInstance, TestInstance, tmpdir } from "../fixture/fixture" // neocode_change
import { awaitWithTimeout, testEffect } from "../lib/effect"
import { type InstanceContext } from "../../src/project/instance-context"
import { Flag } from "@opencode-ai/core/flag/flag" // neocode_change
import { TsCheck } from "../../src/neocode/ts-check" // neocode_change

// neocode_change - Typescript.spawn ignores ctx, so a cast is fine here.
...
describe("lsp.spawn", () => {
  ...
  // neocode_change start - provide the runtime flag so spawn() is reached past the TsClient short-circuit
  ...
  // neocode_change start - Typescript spawn is gated behind NEO_EXPERIMENTAL_LSP_TOOL.
  test("spawns tsgo LSP when NEO_EXPERIMENTAL_LSP_TOOL is enabled", async () => {
    const saved = Flag.NEO_EXPERIMENTAL_LSP_TOOL
    Flag.NEO_EXPERIMENTAL_LSP_TOOL = true
    ...
    try {
      ...
      Flag.NEO_EXPERIMENTAL_LSP_TOOL = saved
      spawnSpy.mockRestore()
      tsgoSpy.mockRestore()
    }
  })

  test("Typescript.spawn returns undefined when NEO_EXPERIMENTAL_LSP_TOOL is off", async () => {
    const saved = Flag.NEO_EXPERIMENTAL_LSP_TOOL
    Flag.NEO_EXPERIMENTAL_LSP_TOOL = false
    try {
