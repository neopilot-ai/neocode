...
const require = createRequire(path.resolve(import.meta.dirname, "../../../neo-sandbox/package.json"))

export namespace NeoSandboxNetwork {
  export const relay = "neo-sandbox-network-relay.js"
  export const seccomp = "neo-sandbox-seccomp"

  export async function bundle() {
    const result = await Bun.build({
      entrypoints: ["../neo-sandbox/src/neo-sandbox-network-relay.ts"],
      target: "bun",
      format: "esm",
      minify: true,
    })
    if (!result.success || result.outputs.length !== 1) throw new Error("Could not bundle Neo sandbox network relay")
  ...
  export async function copy(worker: Blob, dir: string, arch: "arm64" | "x64") {
    ...
    console.log(`copied Neo sandbox network relay and seccomp helper to ${dir}`)
  }
}
