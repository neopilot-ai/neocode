...
// Launch the neo CLI dev build against a locally running cloud dev server.
//   bun dev:local <project-dir> [--cloud <dir>] [--no-ingest] [--no-events] [--print] [-- <neo args>]
//
// Reads ports from <cloud>/dev/logs/manifest.json (+ .dev-port), probes the web
// server, and points the CLI at it (NEO_API_URL / NEO_SESSION_INGEST_URL /
// EVENT_SERVICE_URL).
// Auth/config/state/cache are isolated under ~/.neo-dev so it can't clash with
// your main neo install; real HOME is kept so git/ssh still work.
...
const neo = path.resolve(import.meta.dir, "../../..")
const home = path.join(os.homedir(), ".neo-dev")
...
async function main() {
  ...
  env.NEO_API_URL = `http://localhost:${webPort}`
  env.NEO_DEV_CWD = project
  env.NEO_DISABLE_AUTOUPDATE = "1"
  if (ingestPort) env.NEO_SESSION_INGEST_URL = `http://localhost:${ingestPort}`
  else env.NEO_DISABLE_SESSION_INGEST = "1"
  if (eventsPort) {
    env.EVENT_SERVICE_URL = `ws://localhost:${eventsPort}`
    delete env.NEO_DISABLE_PRESENCE
    delete env.NEO_EVENT_SERVICE_URL
  } else env.NEO_DISABLE_PRESENCE = "1"
  ...
  process.exit(await Bun.spawn({ cmd: ["bun", "run", "--cwd", "packages/opencode", "--conditions=browser", "src/index.ts", ...pass], cwd: neo, env, stdio: ["inherit", "inherit", "inherit"] }).exited)
}
