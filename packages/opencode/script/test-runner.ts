...
import { TestProfile } from "./neocode/test-profile"
import { TestShard } from "./neocode/test-shard"
import { remove } from "../test/neocode/cleanup"
...
if (argv.includes("--help") || argv.includes("-h")) {
  console.log(
    [
      ...
      "  --profile <name>     Run a curated test profile (env: NEO_TEST_PROFILE)",
      "  --shard <N/M>        Run one balanced file shard (env: NEO_TEST_SHARD)",
...
// resources (ports, global filesystem like ~/.local/share/neo), not CPU.
...
const env = process.env.NEO_TEST_PROFILE?.trim() || undefined
if (flag && env && flag !== env) {
  console.error(`Conflicting test profiles: --profile=${flag}, NEO_TEST_PROFILE=${env}`)
  process.exit(2)
}
const profile = flag ?? env
const shardFlag = text("shard")
const shardEnv = process.env.NEO_TEST_SHARD?.trim() || undefined
if (shardFlag && shardEnv && shardFlag !== shardEnv) {
  console.error(`Conflicting test shards: --shard=${shardFlag}, NEO_TEST_SHARD=${shardEnv}`)
  process.exit(2)
}
...
const matched =
  patterns.length > 0
    ...
    : selected
const candidates = patterns.length > 0 && !profile ? matched : matched.filter((file) => !skipped.has(file)) // neocode_change
