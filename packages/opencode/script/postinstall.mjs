...
// neocode_change start - variant detection matching bin/neo logic
const platformMap = {
  darwin: "darwin",
  linux: "linux",
  win32: "windows",
...
const base = `@neocode/cli-${platform}-${arch}`
const sourceBinary = platform === "windows" ? "neo.exe" : "neo"
const targetBinary = path.join(__dirname, "bin", ".neo")
...
// neocode_change start - copy runtime resources next to cached binary
function copyResources(source) {
  ...
  const worker = path.join(path.dirname(source), "neo-sandbox-mutation-worker.js")
  if (fs.existsSync(worker)) fs.copyFileSync(worker, path.join(__dirname, "bin", "neo-sandbox-mutation-worker.js"))
}
...
function main() {
  ...
  for (const name of packageNames()) {
    try {
      ...
      const temp = fs.mkdtempSync(path.join(os.tmpdir(), "neo-install-"))
      try {
  ...
  throw new Error(
    `It seems your package manager failed to install the right Neo CLI package. Try manually installing ${packageNames()
...
try {
  ...
  console.error("Failed to setup neo binary:", error.message)
  process.exit(1)
}
