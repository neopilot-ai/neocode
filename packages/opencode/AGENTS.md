...
The MCP `StdioClientTransport` (third-party SDK) is handled separately via a process shim in `src/mcp/index.ts` that sets `process.type = "browser"` when running inside the VS Code extension (`NEO_PLATFORM=vscode`), which causes the SDK's internal `isElectron()` check to return `true` and enable `windowsHide`.

## Storage

Filesystem-based JSON, not a database. Data lives in `~/.local/share/neo/storage/`. Keys are path arrays: `Storage.write(["session", projectID, sessionID], data)`.

## TUI

Built with **SolidJS + OpenTUI** (`@opentui/solid`) -- a terminal UI framework. JSX renders to the terminal using elements like `<box>`, `<text>`, `<scrollbox>`. The TUI communicates with the server via `@neocode/sdk`.
...
`opencode/` is a fork of upstream opencode. When a change must touch a shared upstream file, extract the Neo-specific logic into a mirror file under `src/neocode/<same/path>.ts` (tests under `test/neocode/<same/path>.test.ts`) and call into it from the upstream file behind a single `neocode_change` marker. Example: a Neo override for `src/cli/cmd/tui/component/dialog-provider.tsx` lives at `src/neocode/cli/cmd/tui/component/dialog-provider.tsx`. Avoid inlining Neo-specific logic directly into shared upstream files. Files and directories whose path contains `neocode` never need `neocode_change` markers.
