...
import { TsCheck } from "../neocode/ts-check" // neocode_change
import type { RuntimeFlags } from "@/effect/runtime-flags"

const log = Log.create({ service: "lsp.server" }) // neocode_change
...
// neocode_change start - tsgo native LSP or lightweight diagnostic client
// When NEO_EXPERIMENTAL_LSP_TOOL is enabled, spawn tsgo --lsp --stdio as a
...
export const Typescript: Info = {
  ...
  async spawn(root) {
    if (!Flag.NEO_EXPERIMENTAL_LSP_TOOL) return undefined
...
export const Clangd: Info = {
  ...
  async spawn(root, _ctx, flags) {
    ...
    // neocode_change start - reject release metadata before it becomes an executable path
    ...
    // neocode_change start - do not use remote metadata as a local path
  ...
  async spawn(root, _ctx, flags) {
    ...
    return {
      process: spawn(
        java,
        [
          "-Djava.import.generatesMetadataFilesAtProjectRoot=false", // neocode_change
          "-jar",
          launcherJar,
          "-configuration",
          configFile,
...
export const LuaLS: Info = {
  ...
  async spawn(root, _ctx, flags) {
    ...
    if (!bin) {
      ...
      // neocode_change start - use a fixed local archive name
