...
import { NeoViewers } from "@/neocode/presence/service"
// neocode_change end
...
import { SyncEvent } from "@/sync" // neocode_change
...
} from "@/neocode/server/reference-reconciler" // neocode_change
import { schemaErrorLayer as v2SchemaErrorLayer } from "@opencode-ai/server/middleware/schema-error"
import { workspaceHandlers } from "./handlers/workspace"
// neocode_change start
import {
  provide as provideNeoHttpApiHandlers,
  provideListener as provideNeoListenerRoutes,
} from "@/neocode/server/httpapi/server"
// neocode_change end
...
const instanceApiRoutes = HttpApiBuilder.layer(InstanceHttpApi).pipe(
  ...
  provideNeoHttpApiHandlers, // neocode_change
)
...
const serverRoutes = HttpApiBuilder.layer(Api).pipe(
  // neocode_change start - effective references must be ready before any V2 location consumer runs
...
const app = LayerNode.group([
  ...
  Plugin.node,
  ModelsDev.node,
  ModelCache.node, // neocode_change - export the shared Neo model cache to route handlers
  Provider.node,
  ProviderAuth.node,
  Agent.node,
  Skill.node,
...
// neocode_change start - keep listener routes local while application services come from AppRuntime
export function createListenerRoutes(corsOptions?: CorsOptions) {
  return Layer.mergeAll(rootApiRoutes, eventApiRoutes, ptyConnectApiRoutes, instanceRoutes, docRoute, uiRoute).pipe(
    provideNeoListenerRoutes(corsOptions),
  )
}
// neocode_change end
