...
import { mapValues, pickBy } from "remeda" // neocode_change
import { ModelCache } from "@/provider/model-cache" // neocode_change
import {
  disposeAllInstancesAfterProviderAuthCallback,
  invalidatePresence,
} from "@/neocode/server/provider-auth-lifecycle" // neocode_change
import { providerMetadata } from "@/neocode/provider/metadata" // neocode_change
import { filterPromptTrainingModels } from "@/neocode/provider/model-filter" // neocode_change
import { overlay as overlayAnacondaDesktop } from "@/neocode/anaconda-desktop/provider" // neocode_change
...
function mapProviderAuthError<A, R>(self: Effect.Effect<A, ProviderAuth.Error, R>) {
  return self.pipe(
    Effect.mapError((error) => {
      ...
      return new ProviderAuthApiError({ name: "BadRequest", data: { message: error.message } }) // neocode_change
    }),
  )
}
...
export const providerHandlers = HttpApiBuilder.group(InstanceHttpApi, "provider", (handlers) =>
  Effect.gen(function* () {
    ...
    const cache = yield* ModelCache.Service // neocode_change

    const list = Effect.fn("ProviderHttpApi.list")(function* () {
      const config = yield* cfg.get()
      const all = overlayAnacondaDesktop(yield* ModelsDev.Service.use((s) => s.get())) // neocode_change
    ...
    const callback = Effect.fn("ProviderHttpApi.callback")(function* (ctx: {
      ...
      // neocode_change start - drop old-user presence before instance disposal on Neo OAuth callback
      if (ctx.params.providerID === "neo") yield* invalidatePresence()
      // neocode_change end
      yield* disposeAllInstancesAfterProviderAuthCallback() // neocode_change
      return true
    })
