import { Image } from "@/image/image" // neocode_change - classify user image validation defects
import { NeoSessionHttpApi } from "@/neocode/server/httpapi/session-fork" // neocode_change
import { BlockedError as AgentRequirementError } from "@/neocode/agent-requirements" // neocode_change
...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const viewers = yield* NeoViewers.Service // neocode_change
    ...
    const forkRaw = NeoSessionHttpApi.forkRaw(fork) // neocode_change - carry upstream bodyless full-session fork support
    ...
    const prompt = Effect.fn("SessionHttpApi.prompt")(function* (ctx: {
      ...
      const message = yield* promptSvc
        .prompt({ ...ctx.payload, sessionID: ctx.params.sessionID } as unknown as SessionPrompt.PromptInput) // neocode_change
        .pipe(
          // neocode_change start - reject only typed user image validation defects as request errors
    ...
    const promptAsync = Effect.fn("SessionHttpApi.promptAsync")(function* (ctx: {
      ...
      yield* promptSvc
        ...
        .pipe(
          Effect.catchCause((cause) => {
            if (Cause.hasInterruptsOnly(cause)) return Effect.void // neocode_change - Stop is not an error
    ...
    return handlers
      ...
      .handleRaw("fork", forkRaw) // neocode_change - carry upstream bodyless full-session fork support
      .handle("abort", abort)
      .handle("init", init)
      .handle("share", share)
      ...
      .handle("viewed", viewed) // neocode_change
  }),
)
