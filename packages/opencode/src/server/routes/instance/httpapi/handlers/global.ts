...
import { disconnect } from "@/neocode/server/sse" // neocode_change
  ...
  return Effect.gen(function* () {
    ...
    return HttpServerResponse.stream(
      Stream.make({ payload: { id: EventV2.ID.create(), type: "server.connected", properties: {} } }).pipe(
        ...
        Stream.encodeText,
        // neocode_change start - prevent disconnected SSE clients from retaining full diff payloads
...
export const globalHandlers = HttpApiBuilder.group(RootHttpApi, "global", (handlers) =>
  Effect.gen(function* () {
    ...
    const event = Effect.fn("GlobalHttpApi.event")(function* () {
      const request = yield* HttpServerRequest.HttpServerRequest // neocode_change
      return yield* eventResponse(request) // neocode_change
    })
