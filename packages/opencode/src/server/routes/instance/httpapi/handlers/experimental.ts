...
import { EffectBridge } from "@/effect/bridge" // neocode_change
import { InstanceState } from "@/effect/instance-state"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { MCP } from "@/mcp"
import { Project } from "@/project/project"
import { Provider } from "@/provider/provider" // neocode_change
import { ModelV2 } from "@opencode-ai/core/model" // neocode_change
import { Session } from "@/session/session"
import type { SessionID } from "@/session/schema"
import { ToolJsonSchema } from "@/tool/json-schema"
import { ToolRegistry } from "@/tool/registry"
import { Filesystem } from "@/util/filesystem" // neocode_change
import { Review } from "@/neocode/review/review" // neocode_change
import { WorktreeDiff } from "@/neocode/review/worktree-diff" // neocode_change
import { WorktreeFamily } from "@/neocode/worktree-family" // neocode_change
import { Worktree } from "@/worktree"
import { Effect, Option } from "effect"
import * as HttpServerResponse from "effect/unstable/http/HttpServerResponse"
import { HttpApiBuilder, HttpApiError } from "effect/unstable/httpapi"
import * as Log from "@opencode-ai/core/util/log" // neocode_change
import path from "path" // neocode_change
  ...
  Effect.gen(function* () {
    ...
    const tool = Effect.fn("ExperimentalHttpApi.tool")(function* (ctx: { query: typeof ToolListQuery.Type }) {
      ...
      const list = yield* registry.tools({
        ...
        modelID: model ? ModelV2.ID.make(model.api.id) : ctx.query.model, // neocode_change
        family: model?.family, // neocode_change
    ...
    // neocode_change start - discover Agent Manager and external git worktrees
    ...
    // neocode_change start - worktree diff endpoints for agent manager
    ...
    const session = Effect.fn("ExperimentalHttpApi.session")(function* (ctx: { query: typeof SessionListQuery.Type }) {
      ...
      // neocode_change end
      if (roots && directory && !current) return HttpServerResponse.jsonUnsafe([]) // neocode_change
      const all = yield* sessions.listGlobal({
        projectID, // neocode_change
        directory: ctx.query.worktrees ? undefined : ctx.query.directory, // neocode_change
        directories: roots, // neocode_change
        currentDirectory: directory, // neocode_change
      ...
      // neocode_change start - resolve worktree folder name for each session
      const result = sorted
        ? all.map((session) => {
      ...
      return HttpServerResponse.jsonUnsafe(list, {
        headers:
          result.length > limit && list.length > 0 // neocode_change
