...
import { matchLegacyNeoOpenApi } from "@/neocode/server/httpapi/public"
import * as NeoServer from "@/neocode/server/server"
// neocode_change end
...
const QueryParameterSchemas: Record<string, OpenApiSchema> = {
  ...
  "GET /neo/cloud-sessions cursor": { type: "string" },
  "GET /neo/cloud-sessions limit": { type: "number" },
  // neocode_change end
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for the neo serve command", // neocode_change
...
function matchLegacyOpenApi(input: Record<string, unknown>) {
  ...
  matchLegacyNeoOpenApi(input) // neocode_change
  return input
}
...
function applyLegacySchemaOverrides(spec: OpenApiSpec) {
  ...
  // neocode_change start - preserve model reset sentinels in indexing config patches
...
function normalizeParameter(param: OpenApiParameter, route: string) {
  ...
  if (param.in === "path") {
    param.schema = pathParameterSchema(route, param.name) ?? stripOptionalNull(param.schema) // neocode_change
    return
  }
  if (param.in === "query") {
...
export const PublicApi = OpenCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: NeoServer.DOC_TITLE, // neocode_change
    version: "1.0.0",
    description: NeoServer.DOC_DESCRIPTION, // neocode_change
