...
import { forkTargetDirectory } from "@/neocode/server/routes/fork-routing" // neocode_change - fork honors explicit target directory
...
function configuredWorkspaceID(): WorkspaceV2.ID | undefined {
  return Flag.NEO_WORKSPACE_ID ? WorkspaceV2.ID.make(Flag.NEO_WORKSPACE_ID) : undefined
}
...
function defaultDirectory(request: HttpServerRequest.HttpServerRequest, url: URL): string {
  return url.searchParams.get("directory") || request.headers["x-neo-directory"] || process.cwd()
}
...
function planRequest(
  ...
  return Effect.gen(function* () {
    ...
    // neocode_change start - a fork targeting an explicit directory (e.g. a worktree) must not inherit the source session's directory
