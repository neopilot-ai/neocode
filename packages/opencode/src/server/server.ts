...
import { serverUrls } from "@/neocode/cli/server-urls" // neocode_change
...
import * as NeoListener from "@/neocode/server/listener" // neocode_change
...
export async function listen(opts: ListenOptions): Promise<Listener> {
  ...
  return {
    ...
    port: listener.port,
    url: listener.url,
    urls: listener.urls, // neocode_change
...
const listenEffect: (opts: ListenOptions) => Effect.Effect<EffectListener, unknown> = Effect.fn("Server.listen")(
  function* (opts: ListenOptions) {
    ...
    return {
      ...
      port: address.port,
      url: listenerUrl,
      urls: serverUrls(opts.hostname, address.port), // neocode_change
...
function listenerLayer(opts: ListenOptions, port: number) {
  return HttpRouter.serve(HttpApiApp.createListenerRoutes(opts), { // neocode_change
...
function startListener(opts: ListenOptions, port: number) {
  ...
  return NeoListener.build(listenerLayer(opts, port), scope).pipe( // neocode_change
