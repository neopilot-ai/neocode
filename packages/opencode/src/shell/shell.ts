...
import * as PowerShell from "@/neocode/shell/shell" // neocode_change - PowerShell args
...
export function gitbash() {
  ...
  if (Flag.NEO_GIT_BASH_PATH) return Flag.NEO_GIT_BASH_PATH
  const git = which("git")
  if (!git) return
...
export function args(file: string, command: string, cwd: string) {
  ...
  if (ps(file)) return PowerShell.args(command) // neocode_change - PowerShell args
  return ["-c", command]
}
