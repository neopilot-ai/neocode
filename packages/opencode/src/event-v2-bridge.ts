  ...
  Effect.gen(function* () {
    ...
    const unsubscribe = yield* events.listen((event) =>
      Effect.gen(function* () {
        ...
        // neocode_change start - legacy bus and SSE consumers require the schema's encoded representation
        ...
        GlobalBus.emit("event", {
          directory: event.location?.directory ?? ctx?.directory ?? "global", // neocode_change - instance-less events are tagged "global" on the wire
          project: ctx?.project.id,
          workspace: workspaceID,
          payload: { id: event.id, type: event.type, properties: data }, // neocode_change - encoded
        })
