...
import { NeoQuestion } from "@/neocode/question" // neocode_change
...
export const Option = Schema.Struct({
  ...
  // neocode_change start - optional i18n keys so clients can translate while still
  ...
  // neocode_change end
  // neocode_change start - hint to UI clients to switch the active agent/mode picker
...
const base = {
  ...
  // neocode_change start - optional i18n keys for question text and header
...
export interface Interface {
  readonly ask: (input: {
    ...
    blocking?: boolean // neocode_change
    tool?: Tool
  ...
  readonly dismissAll: (sessionID: SessionID) => Effect.Effect<void> // neocode_change
}
  ...
  Effect.gen(function* () {
    ...
    const ask = Effect.fn("Question.ask")(function* (input: {
      ...
      const info: Request = {
        ...
        blocking: input.blocking, // neocode_change
        tool: input.tool,
      }

      // neocode_change start
      yield* NeoQuestion.guardFollowup(input.sessionID, () => new RejectedError())
      // neocode_change end
      ...
      return yield* Effect.ensuring(
        ...
        // neocode_change start - every asked question gets a terminal event when its waiter is interrupted
        NeoQuestion.finalize({
          pending,
          id,
    ...
    // neocode_change start - body lives in @/neocode/question/NeoQuestion.makeDismissAll
    const dismissAll = NeoQuestion.makeDismissAll({
      state,
    ...
    return Service.of({ ask, reply, reject, list, dismissAll }) // neocode_change
  }),
)
