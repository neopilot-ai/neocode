...
import { NeoSessionProcessor, type ReviewTelemetry } from "@/neocode/session/processor"
import { NeoSessionOverflow } from "@/neocode/session/overflow"
import { NeoRoutedModel } from "@/neocode/session/routed-model"
import { Suggestion } from "@/neocode/suggestion"
// neocode_change end
...
export interface Handle {
  ...
  readonly compactError?: () => ReturnType<typeof MessageV2.ContextOverflowError.prototype.toObject> | undefined // neocode_change
}
...
interface ProcessorContext extends Input {
  ...
  toolmeta: Record<string, { title?: string; metadata?: Record<string, any> }> // neocode_change
  shouldBreak: boolean
  snapshot: string | undefined
  blocked: boolean
  needsCompaction: boolean
  compactionError: ReturnType<typeof MessageV2.ContextOverflowError.prototype.toObject> | undefined // neocode_change
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      ...
      // neocode_change start - pass turn context for slow-snapshot UI/policy handling
      ...
      const ctx: ProcessorContext = {
        ...
        model: input.model,
        toolcalls: {},
        toolmeta: {}, // neocode_change
        shouldBreak: false,
        snapshot: initialSnapshot,
        blocked: false,
        needsCompaction: false,
        compactionError: undefined, // neocode_change
      ...
      let aborted = false
      const ac = new AbortController() // neocode_change — abort controller for offline handler
      let attempt = NeoSessionProcessor.attempt() // neocode_change

      // neocode_change start
      const parse = (e: unknown) =>
        NeoSessionProcessor.parseError(e, {
          providerID: input.model.providerID,
          aborted,
        })
      const retryParse = (e: unknown) => {
        const error = parse(e)
        if (e instanceof NeoSessionProcessor.IncompleteResponseError) return NeoSessionProcessor.blockRetry(error)
        if (attempt.text || attempt.reasoning || attempt.tool) return NeoSessionProcessor.blockRetry(error)
        return error
      }
      // neocode_change end
      ...
      const settleToolCall = Effect.fn("SessionProcessor.settleToolCall")(function* (toolCallID: string) {
        ...
        delete ctx.toolmeta[toolCallID] // neocode_change
      ...
      // neocode_change start - tolerate deleted sessions during subagent cost reconciliation (#6321)
      ...
      // neocode_change start - buffer metadata emitted before tool-call registration
      ...
      const completeToolCall = Effect.fn("SessionProcessor.completeToolCall")(function* (
        ...
        // neocode_change start - accepted suggest review actions tag following LLM completion telemetry
        if (match.part.tool === "suggest") {
          ctx.telemetry = NeoSessionProcessor.suggestionReviewTelemetry(output.metadata) ?? ctx.telemetry
        }
        // neocode_change end
      ...
      const failToolCall = Effect.fn("SessionProcessor.failToolCall")(function* (toolCallID: string, error: unknown) {
        ...
        yield* session.updatePart({
          ...
          state: {
            ...
            metadata: match.part.state.metadata, // neocode_change - preserve running tool metadata on failure
            time: { start: match.part.state.time.start, end: Date.now() },
          },
        })
        // neocode_change start
      ...
      const handleEvent = Effect.fnUntraced(function* (value: StreamEvent) {
        NeoSessionProcessor.observe(attempt, value) // neocode_change
        switch (value.type) {
          ...
          case "reasoning-delta":
            ...
            if (value.text.trim()) ctx.step.reasoning = true // neocode_change
          ...
          case "tool-call": {
            ...
            ctx.step.tool = true // neocode_change
            const toolCall = yield* ensureToolCall(value)
            ...
            // neocode_change start - apply metadata buffered before the running transition
          ...
          case "tool-result": {
            ...
            // neocode_change start - dismissed suggestions stop the turn after persisting normalized output
          ...
          case "step-finish": {
            // neocode_change start - retry only terminally incomplete attempts before settlement
            if (
              !mirrorAssistant &&
              NeoSessionProcessor.replayable({
              ...
              return yield* Effect.fail(new NeoSessionProcessor.IncompleteResponseError())
            // neocode_change end
            // neocode_change start - pass turn context for slow-snapshot UI/policy handling
            ...
            // neocode_change start - capture any subagent cost propagated by tool calls during this step (#6321)
            yield* reconcile()
            // neocode_change end
            ...
            yield* session.updatePart({
              ...
              sessionID: ctx.assistantMessage.sessionID,
              type: "step-finish",
              ...(model ? { model } : {}), // neocode_change
              tokens: usage.tokens,
              cost: usage.cost,
            })
            // neocode_change start - surface output limit stops, with a stronger message for reasoning-only stops
            const warn = NeoSessionProcessor.lengthWarning({ msg: ctx.assistantMessage, step: ctx.step })
            if (warn) {
              yield* session.updatePart({
                id: PartID.ascending(),
                messageID: ctx.assistantMessage.id,
            ...
            const providerError = NeoSessionProcessor.providerFinishError(ctx.assistantMessage)
            if (providerError) {
          ...
          case "text-delta":
            ...
            if (value.text.trim()) ctx.step.text = true // neocode_change
          ...
          case "text-end":
            ...
            if (ctx.currentText.text.trim()) {
              attempt.text = true // neocode_change
              ctx.step.text = true
            } // neocode_change
            if (!ctx.assistantMessage.summary) {
              // TODO(v2): Temporary dual-write while migrating session messages to v2 events.
      ...
      const cleanup = Effect.fn("SessionProcessor.cleanup")(function* () {
        ...
        ctx.toolcalls = {}
        ctx.toolmeta = {} // neocode_change
        // neocode_change start - read parts through the upstream Effect database
        NeoSessionProcessor.guardEmptyToolCalls(
          ctx.assistantMessage,
          yield* MessageV2.parts(ctx.assistantMessage.id).pipe(Effect.provideService(Database.Service, database)),
        )
        // neocode_change end
        ctx.assistantMessage.time.completed = Date.now()
        // neocode_change start - reconcile cost with any subagent propagation written during tool calls (#6321)
        yield* reconcile()
        // neocode_change end
        yield* session.updateMessage(ctx.assistantMessage)
      })
      ...
      const halt = Effect.fn("SessionProcessor.halt")(function* (e: unknown) {
        // neocode_change start - internal preflight signal, not a provider error
        if (e instanceof NeoSessionOverflow.PreflightError) {
          ctx.needsCompaction = true
          return
        }
        // neocode_change end
        ...
        const error = parse(e)
        // neocode_change start
        if (e instanceof NeoSessionProcessor.IncompleteResponseError) ctx.assistantMessage.finish = "unknown"
      ...
      const process = Effect.fn("SessionProcessor.process")(function* (streamInput: LLM.StreamInput) {
        ...
        // neocode_change start - a deleted session cannot accept EventV2 writes under core FK enforcement
        ...
        ctx.compactionError = undefined // neocode_change
        ctx.shouldBreak = (yield* config.get()).experimental?.continue_loop_on_deny !== true

        return yield* Effect.gen(function* () {
          // neocode_change start - publish retry state consistently for provider and empty-response retries
          ...
          const request = () =>
            Effect.gen(function* () {
              ...
              Effect.onInterrupt(() =>
                Effect.gen(function* () {
                  aborted = true
                  ac.abort() // neocode_change — also abort offline handler
          ...
          const recover = () => {
            ...
            return NeoSessionProcessor.recover({
              run: Effect.fn("SessionProcessor.incompleteAttempt")(function* () {
                ...
                attempt = NeoSessionProcessor.attempt()
                yield* request()
              }),
              replayable: () =>
                !mirrorAssistant &&
      ...
      return {
        ...
        updateToolCall,
        metadata, // neocode_change
        completeToolCall,
        ...output, // neocode_change
        process,
      } satisfies Handle
    })
