...
export { EditorContext } from "@opencode-ai/core/v1/session" // neocode_change
...
import { Snapshot } from "@/snapshot" // neocode_change
import { SessionNetwork } from "./network" // neocode_change
import { CodexAuthExpiredError } from "@/neocode/provider/codex-refresh" // neocode_change
import { NeoSessionMessageOrder } from "@/neocode/session/message-order" // neocode_change
import * as TextStream from "@/neocode/text-stream" // neocode_change
...
// neocode_change - upstream moved these message/part types to SessionV1; re-export them so the
...
function truncateToolOutput(text: string, maxChars?: number) {
  ...
  // neocode_change start - avoid persisting malformed Unicode in compacted tool output
...
// neocode_change start - strip bloated metadata fields from stored parts to prevent multi-MB payloads
...
export function stripPartMetadata(part: Part): Part {
  // neocode_change - exported for testing
...
// neocode_change - apply stripping inside helpers so all read paths are covered
...
export const toModelMessagesEffect = Effect.fnUntraced(function* (
  ...
  for (const msg of input) {
    ...
    if (msg.info.role === "assistant") {
      ...
      for (const part of msg.parts) {
        // neocode_change - !part.ignored keeps local UI warnings out of future prompts
...
export function parts(messageID: MessageID) {
  return Effect.gen(function* () {
    ...
    const rows = yield* db
      ...
      .all()
      .pipe(Effect.orDie)
    return rows.map(part) // neocode_change - part() applies stripPartMetadata to cover all read paths
  })
}
...
export function filterCompacted(msgs: Iterable<WithParts>) {
  ...
  result.reverse()
  NeoSessionMessageOrder.annotate(result) // neocode_change - preserve chronology before retained-tail projection
...
export function fromError(
  ...
  switch (true) {
    ...
    case e instanceof CodexAuthExpiredError: // neocode_change start
      return new AuthError(
        {
          providerID: "openai",
          message: e.message,
        ...
        { cause: e },
      ).toObject() // neocode_change end
    case SessionNetwork.disconnected(e): // neocode_change start
      return new APIError(
        {
          message: SessionNetwork.message(e), // neocode_change end
          isRetryable: true,
          metadata: {
