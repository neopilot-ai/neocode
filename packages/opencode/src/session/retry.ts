...
import { isNeoError } from "@/neocode/neo-errors" // neocode_change
...
export type RetryReason = string & {} // neocode_change - Neo does not support OpenCode Go upsell reasons
...
// neocode_change - Neo does not emit OpenCode Go actions
export function retryable(error: Err, _provider?: string): Retryable | undefined {
  // context overflow errors should not be retried
  if (SessionV1.ContextOverflowError.isInstance(error)) return undefined
  if (SessionV1.APIError.isInstance(error)) {
    const status = error.data.statusCode
    // neocode_change start - Current Neo errors require user action (login/signup), don't retry
    if (isNeoError(error)) return undefined
    // neocode_change end
    ...
    // neocode_change start - Neo does not support OpenCode Go upsells. FreeUsageLimitError is not retryable: retrying
...
export function policy(opts: {
  ...
  return Schedule.fromStepWithMetadata(
    Effect.succeed((meta: Schedule.InputMetadata<unknown>) => {
      // neocode_change start — enforce retry limit
      ...
      return Effect.gen(function* () {
        // neocode_change start — handle network disconnect via offline handler
