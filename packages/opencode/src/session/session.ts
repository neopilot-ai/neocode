...
// neocode_change start - Neo session behavior extensions
import { BackgroundProcess } from "@/neocode/background-process"
import * as SandboxInheritance from "@/neocode/sandbox/inheritance"
import { InteractiveTerminal } from "@/neocode/interactive-terminal"
import { NeoSession } from "@/neocode/session"
import { neoSessionFork } from "@/neocode/session/fork-command"
import { NeoSessionEvent } from "@/neocode/session/event"
import { SessionExport } from "@/neocode/session-export"
import * as SandboxPolicy from "@/neocode/sandbox/policy"
import { carryForkDiff } from "@/neocode/session-portability/cumulative-diff" // neocode_change
import { BlockedError as AgentRequirementError } from "@/neocode/agent-requirements"
// neocode_change end
import { Effect, Layer, Option, Context, Schema, Types } from "effect"
import { NonNegativeInt, optionalOmitUndefined } from "@opencode-ai/core/schema"
import { AbsolutePath } from "@opencode-ai/core/schema" // neocode_change
...
const Summary = Schema.Struct({
  ...
  files: Schema.Finite,
  diffs: optionalOmitUndefined(Schema.Array(Snapshot.SummaryFileDiff)), // neocode_change - lightweight diff without patch
})
...
export const GlobalInfo = Schema.Struct({
  ...
  worktreeName: Schema.optional(Schema.String), // neocode_change - basename of the specific worktree directory
...
export const CreateInput = Schema.optional(
  Schema.Struct({
    ...
    platform: Schema.optional(Schema.String), // neocode_change - per-session platform override for telemetry attribution
    // neocode_change start - server-issued sandbox inheritance grant
...
export type GlobalListInput = {
  // neocode_change start - worktree-family filters for the Agent Manager
  projectID?: string
  directory?: string
  directories?: string[]
...
export const Event = {
  ...
  Error: EventV2.define({
    ...
    schema: {
      ...
      // neocode_change - carry pre-message requirement failures over session.error
  ...
  TurnOpen: NeoSessionEvent.TurnOpen,
  TurnClose: NeoSessionEvent.TurnClose,
  // neocode_change end
}

export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".neo", "plans") // neocode_change
...
export const getUsage = (input: {
  ...
  provider?: Provider.Info // neocode_change
}) => {
  ...
  // neocode_change start - Use provider-reported cost when available for OpenRouter/Neo
  const reported = NeoSession.providerCost({
...
export interface Interface {
  ...
  // neocode_change start - session create metadata and sandbox inheritance extensions
  ...
  readonly create: (input?: {
    ...
    permission?: PermissionV1.Ruleset
    platform?: string // neocode_change - per-session platform override for telemetry attribution
    workspaceID?: WorkspaceV2.ID
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // neocode_change start - inherited sandbox policy source
    const createNext = Effect.fn("Session.createNext")(function* (input: {
      ...
      permission?: PermissionV1.Ruleset
      platform?: string // neocode_change - per-session platform override for telemetry attribution
      sourceID?: SessionID // neocode_change - inherited sandbox policy source
      sourceDirectory?: string
      sandboxFallback?: SandboxPolicy.Snapshot // neocode_change - confinement to seed when source state lives in another directory
    }) {
      ...
      // neocode_change start - legacy sessions must satisfy the upstream project foreign key
      yield* db
        .insert(ProjectTable)
        .values({
          id: ctx.project.id,
      ...
      // neocode_change start - initialize inherited state before session.created subscribers run
      NeoSession.register({ id: result.id, parentID: result.parentID, platform: input.platform })
    ...
    // neocode_change start - preserve Neo's cross-project worktree-family filtering
    const listGlobal = Effect.fn("Session.listGlobal")((input?: GlobalListInput) =>
      NeoSession.listGlobal<GlobalInfo>({ ...input, fromRow }).pipe(Effect.provideService(Database.Service, database)),
    )
    // neocode_change end

    // neocode_change start - scope children by persisted parent project_id
    ...
    const remove: Interface["remove"] = Effect.fnUntraced(function* (sessionID: SessionID) {
      ...
      try {
        ...
        yield* SandboxPolicy.dispose(
          sessionID,
          Effect.gen(function* () {
            yield* Effect.promise(() => NeoSession.removeSession(sessionID)).pipe(Effect.ignore)
            NeoSession.clearPlatformOverride(sessionID)
            if (hasInstance) {
            ...
            // neocode_change - migrated from legacy sync.run/sync.remove to EventV2 (events.publish/remove)
            yield* events.publish(SessionV1.Event.Deleted, { sessionID, info: session })
            // neocode_change - capture final session-export workspace delta on close/delete
            const workspaceKey = hasInstance ? yield* InstanceState.directory : undefined // neocode_change
            yield* Effect.promise(() => SessionExport.onSessionClose(sessionID, workspaceKey)) // neocode_change
    ...
    const updateMessage = <T extends SessionV1.Info>(msg: T): Effect.Effect<T> =>
      Effect.gen(function* () {
        // neocode_change start - ignore FK errors when session was deleted while processor was still running
        yield* NeoSession.runSyncSafe(
    ...
    const create = Effect.fn("Session.create")(function* (input?: {
      ...
      // neocode_change end
      // neocode_change start - propagate trusted sandbox inheritance grant
      const session = yield* createNext({
        ...
        platform: input?.platform, // neocode_change
        sourceID: grant?.sessionID, // neocode_change
        sourceDirectory: grant?.directory, // neocode_change
    ...
    const fork = Effect.fn("Session.fork")(function* (input: { sessionID: SessionID; messageID?: MessageID }) {
      ...
      // neocode_change start - forks into another directory cannot read the source confinement from the new dir, so carry it over explicitly
      const sandboxFallback = yield* SandboxPolicy.peek(original.directory, input.sessionID)
      // neocode_change end
      // neocode_change start - historical forks must use the model from retained context, not a later source-session selection
      ...
      const session = yield* createNext({
        ...
        model, // neocode_change - preserve the model + variant active at the fork point
        sourceID: input.sessionID, // neocode_change - forks preserve initialized confinement
        sandboxFallback, // neocode_change - seed confinement from the source session's original directory
      })
      ...
      for (const msg of msgs) {
        ...
        const cloned = yield* updateMessage({
          ...
          id: newID,
          ...(msg.info.role === "assistant" && { cost: 0 }), // neocode_change - count only spend incurred after the fork
          ...(parentID && { parentID }),
        })

        for (const part of msg.parts) {
          // neocode_change - detach task calls + drop transient parts before copying the forked transcript
          const prepared = NeoSession.prepareForkedPart(part)
          ...
          const p: SessionV1.Part = {
            ...
            messageID: cloned.id,
            sessionID: session.id,
            ...(prepared.type === "step-finish" && { cost: 0 }), // neocode_change - exclude pre-fork spend from model stats
          }
      ...
      // neocode_change - preserve imported/cumulative diffs when forking (self-contained Storage runtime keeps this shared file off the legacy Storage layer)
...
function listByProject(
  ...
  // neocode_change start - NeoSession.filters keeps sessions visible across project_id changes
  ...
  const conditions =
    input.path !== undefined
      ...
      : NeoSession.filters({ projectID: input.projectID, directory: input.directory })
  // neocode_change end
  ...
  if (input.path !== undefined) {
    ...
    // neocode_change start - directory filtering handled by NeoSession.filters above
    // if (input.directory) {
...
// neocode_change start - delegate to NeoSession.listGlobal (adds projectID worktree family + directories[])
export function listGlobal(input?: {
  ...
  return NeoSession.listGlobal<GlobalInfo>({ ...input, fromRow })
}
// neocode_change end

// neocode_change - delegate the exported Promise facade to the Neo session runtime
export const fork = neoSessionFork
