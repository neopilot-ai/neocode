...
import { appendSessionDiffs, readSessionDiffBase } from "@/neocode/session-portability/cumulative-diff" // neocode_change
import { Storage } from "@/storage/storage" // neocode_change
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const storage = yield* Storage.Service // neocode_change
    ...
    const summarize = Effect.fn("SessionSummary.summarize")(function* (input: {
      ...
      if ((yield* config.get()).snapshot === false) return // neocode_change - respect snapshot config toggle

      // neocode_change start - preserve imported cumulative diffs when summarizing cloud-forked sessions
      ...
      yield* storage.write(["session_diff", input.sessionID], diffs).pipe(Effect.ignore) // neocode_change
      yield* events.publish(Session.Event.Diff, { sessionID: input.sessionID, diff: diffs })

      if (!target || target.info.role !== "user") return
      const msgDiffs = base.length > 0 ? local : yield* computeDiff({ messages }) // neocode_change
      target.info.summary = { ...target.info.summary, diffs: msgDiffs }
      yield* sessions.updateMessage(target.info)
    })

    const diff = Effect.fn("SessionSummary.diff")(function* (input: { sessionID: SessionID; messageID?: MessageID }) {
      // neocode_change start - retain cumulative diffs for legacy TUI and VS Code consumers
      if (!input.messageID) {
...
export const defaultLayer = Layer.suspend(() =>
  layer.pipe(
    ...
    Layer.provide(Storage.defaultLayer), // neocode_change
  ),
)
