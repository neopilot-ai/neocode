...
import { NeoRoutedModel } from "@/neocode/session/routed-model" // neocode_change
...
function usage(value: unknown) {
  ...
  const item = value as {
    ...
    raw?: Record<string, unknown> // neocode_change - preserve provider billing details
  }
  const entries = Object.entries({
    ...
    providerMetadata: item.raw ? { aiSdk: item.raw } : undefined, // neocode_change - retain Neo billing details
      ...
      return Effect.sync(() => {
        ...
        return [
          LLMEvent.stepFinish({
            ...
            providerMetadata: NeoRoutedModel.write(metadata, event.response?.modelId), // neocode_change
          }),
        ]
      })
