...
import { NeocodeInstruction } from "@/neocode/session/instruction" // neocode_change
import type { NeocodeMarkdown } from "@/neocode/config/markdown" // neocode_change
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const globalFiles = [
      // neocode_change start - prefer NEO_CONFIG_DIR profile when set
      ...(Flag.NEO_CONFIG_DIR ? [path.join(Flag.NEO_CONFIG_DIR, "AGENTS.md")] : []),
      // neocode_change end
    ...
    const relative = Effect.fnUntraced(function* (instruction: string) {
      ...
      // neocode_change - prefer NEO_CONFIG_DIR profile when set, else fall back to global.config
      const root = Flag.NEO_CONFIG_DIR ?? global.config
      return yield* fs.globUp(instruction, root, root).pipe(Effect.catch(() => Effect.succeed([] as string[]))) // neocode_change
    })

    // neocode_change start - project instructions cannot read env or files outside the project root
    const options = Effect.fnUntraced(function* (filepath: string, origin?: NeocodeMarkdown.Source) {
      const ctx = yield* InstanceState.context
    ...
    const read = Effect.fnUntraced(function* (filepath: string, origin?: NeocodeMarkdown.Source) {
      const opts = yield* options(filepath, origin)
      return yield* Effect.promise(() => NeocodeInstruction.read(filepath, opts).catch(() => ""))
    })
    // neocode_change end
    ...
    // neocode_change start - retain declaration provenance through instruction path expansion
    const systemSources = Effect.fn("Instruction.systemSources")(function* () {
      const config = yield* cfg.get()
      const ctx = yield* InstanceState.context
      const root = ctx.worktree === "/" ? ctx.directory : ctx.worktree
      const paths = new Map<string, NeocodeMarkdown.Source>()
      const add = (item: string, origin: NeocodeMarkdown.Source) => {
      ...
      if (config.instructions) {
        for (const raw of config.instructions) {
          ...
          const trusted = declared.trusted && (path.isAbsolute(instruction) || Flag.NEO_DISABLE_PROJECT_CONFIG)
    ...
    const system = Effect.fn("Instruction.system")(function* () {
      const config = yield* cfg.get()
      const sources = yield* systemSources() // neocode_change
      const paths = Array.from(sources.keys()) // neocode_change
