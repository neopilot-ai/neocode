...
import { capture } from "@/neocode/instance"
import type { InstanceContext } from "@/project/instance-context"
import { makeRuntime } from "@/effect/run-service"
import * as Log from "@opencode-ai/core/util/log"
import { fn } from "@/neocode/fn"
import { MCP } from "../mcp"
...
export namespace SessionNetwork {
  ...
  const urls = ["https://neo.khulnasoft.com", "https://example.com", "https://cloudflare.com/cdn-cgi/trace"]
  const POLL_MS = 3_000
  ...
  export function disconnected(err: unknown) {
    ...
    // neocode_change - recognize AbortSignal.timeout() errors
    for (const item of chain(err)) {
  ...
  export function message(err: unknown) {
    // neocode_change - check for timeout first
    for (const item of chain(err)) {
  ...
  export const reply = fn(
    ...
    async (input) => {
      ...
      // neocode_change start - reconnect failed remote MCP servers after network recovery
