...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const revert = Effect.fn("SessionRevert.revert")(function* (input: RevertInput) {
      ...
      // neocode_change start - compute diffs BEFORE reverting files so the diff
      ...
      yield* sessions.setRevert({
        ...
        summary: {
          ...
          files: diffs.length,
          diffs: summaryDiffs, // neocode_change
        },
      })
    ...
    const cleanup = Effect.fn("SessionRevert.cleanup")(function* (session: Session.Info) {
      ...
      if (session.revert.partID && target) {
        ...
        if (idx >= 0) {
          ...
          // neocode_change start - clear a reverted provider error from the retained assistant message
