import { Effect, Schema, Scope } from "effect" // neocode_change - stable object reads do not use Option
import { NonNegativeInt } from "@opencode-ai/core/schema"
import * as path from "path"
import { Readable } from "stream" // neocode_change
...
import { Config } from "@/config/config" // neocode_change - optional configured reference authorization
import { assertExternalDirectoryEffect } from "./external-directory"
import { Instruction } from "../session/instruction"
import { isPdfAttachment, sniffAttachmentMime } from "@/util/media"
// neocode_change start
import * as Encoding from "../neocode/encoding"
import { NeoReference } from "@/neocode/reference/contains"
import * as NeoConfiguredReference from "@/neocode/reference"
import { NeoReadObject } from "@/neocode/tool/read-object"
import * as Extract from "../neocode/tool/read-extract"
import * as TextStream from "../neocode/text-stream"
// neocode_change end

const DEFAULT_READ_LIMIT = 2000
const MAX_LINE_LENGTH = 2000
// neocode_change start - report the safe Unicode slice length
...
>(
  ...
  Effect.gen(function* () {
    ...
    // neocode_change start - authorize missing paths without enumerating sibling names
    ...
    // neocode_change start - extracted formats and text consume the authorized open object
    const lines = Effect.fn("ReadTool.lines")(
      (file: NeoReadObject.File, opts: { limit: number; offset: number }, abort: AbortSignal) =>
        Effect.tryPromise({
          try: async (signal) => {
    ...
    const isBinaryFile = (filepath: string, bytes: Uint8Array) => {
      ...
      // neocode_change start - UTF-16/32 BOM: NUL bytes are legitimate, skip the NUL/control-char heuristic
    ...
    const run = Effect.fn("ReadTool.execute")(function* (
      ...
      // neocode_change start - resolve V1 configured references without introducing a Core location-layer dependency
      const config = yield* Effect.serviceOption(Config.Service)
      const references =
        config._tag === "Some"
          ? NeoConfiguredReference.resolveAll({
          ...
          : []
      // neocode_change end
      // neocode_change start - fail before read authorization when the target is missing
      ...
      // neocode_change start - directory mentions expose only a bound listing, never child file bodies
      if (info.type === "Directory") {
        ...
        const explicit =
          ...
          (yield* NeoReference.path(fs, ctx.extra["referenceRoot"], target))
        const referenced =
          explicit ||
          (yield* NeoReference.contains({ fs, references, target }))
        ...
        // neocode_change start - reject any canonical path change after permission approval
        ...
        const limit = Math.max(1, params.limit ?? DEFAULT_READ_LIMIT) // neocode_change - prevent zero-limit loops
      ...
      // neocode_change start - authorize metadata, then bind every content read to the same reopened object
      const file = yield* NeoReadObject.file(requested)
      const explicit =
        typeof ctx.extra?.["referenceRoot"] === "string" &&
        (yield* NeoReference.path(fs, ctx.extra["referenceRoot"], file.target))
      const referenced =
        explicit ||
        (yield* NeoReference.contains({ fs, references, target: file.target }))
...
// neocode_change start - extracted formats use native readers; ordinary text is supplied by FSUtil above
async function collect(stream: Readable, opts: { limit: number; offset: number }) {
  ...
  try {
    for await (const text of rl) {
      ...
      // neocode_change start - keep truncated output valid Unicode
