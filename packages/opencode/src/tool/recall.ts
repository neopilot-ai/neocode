...
import { Session } from "../session/session" // neocode_change
import { SessionID } from "../session/schema" // neocode_change
import { RecallSearch } from "../neocode/session/recall-search" // neocode_change
...
export const RecallTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const sessions = yield* Session.Service // neocode_change
    return {
...
async function search(
  ...
  const dirs = await bridge.promise(WorktreeFamily.list().pipe(Effect.provideService(Git.Service, git))) // neocode_change
  const boundary = NeoSessionPromptQueue.active(ctx.sessionID) ?? RecallSearch.active(ctx.messages, ctx.messageID)
