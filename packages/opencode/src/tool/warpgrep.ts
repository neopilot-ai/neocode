...
import { WarpGrepClient } from "@morphllm/morphsdk/tools/warp-grep/client" // neocode_change
import { Telemetry } from "@neocode/neo-telemetry" // neocode_change
import { Instance } from "../neocode/instance" // neocode_change
import { EventV2Bridge } from "@/event-v2-bridge" // neocode_change
import { TuiEvent } from "@/server/tui-event" // neocode_change
import DESCRIPTION from "./warpgrep.txt"

// FREE_PERIOD_TODO: Remove NEO_WARPGREP_PROXY_URL constant and the proxy
// fallback below. After the free period ends, require MORPH_API_KEY and
// return an error when it is missing.
const NEO_WARPGREP_PROXY_URL = "https://api.neo.khulnasoft.com/api/gateway"

const Parameters = Schema.Struct({
  query: Schema.String.annotate({
    description: "Search query describing what code you are looking for. Be specific and descriptive for best results.", // neocode_change
  }),
})
...
export const CodebaseSearchTool = Tool.define(
  "codebase_search",
  Effect.gen(function* () {
    const events = yield* EventV2Bridge.Service // neocode_change
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          Telemetry.trackToolUsed("codebase_search", ctx.sessionID) // neocode_change
          ...
          const client = new WarpGrepClient({
            morphApiKey: apiKey ?? "neo-free",
            ...(apiKey ? {} : { morphApiUrl: NEO_WARPGREP_PROXY_URL }),
            timeout: 60_000,
          })
          ...
          if (!result.success || !result.contexts?.length) {
            ...
            const isAuthOrRateLimit =
              result.error && /401|402|429|rate.limit|free.period|unauthorized/i.test(result.error) // neocode_change
            const apiKeyMsg =
            ...
            if (isAuthOrRateLimit) {
              // neocode_change start - publish Neo's toast through upstream EventV2
              yield* events
