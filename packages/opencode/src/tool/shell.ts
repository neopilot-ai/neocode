import { Effect, Fiber, Stream } from "effect" // neocode_change - Fiber
import os from "os"
...
import { normalizeUrls } from "@/neocode/util/url" // neocode_change
import { CommandTimeout } from "@/neocode/command-timeout" // neocode_change
import { heredocs } from "@/neocode/tool/shell-heredoc" // neocode_change
...
const ask = Effect.fn("ShellTool.ask")(function* (
  ...
  scan: Scan,
  command: string,
  metadata: ReturnType<typeof heredocs>, // neocode_change
  description?: string, // neocode_change
) {
  // neocode_change
  if (scan.dirs.size > 0) {
    ...
    yield* ctx.ask({
      ...
      patterns: globs,
      always: globs,
      // neocode_change start - retain read classification alongside upstream permission context
      metadata: {
        command,
  ...
  yield* ctx.ask({
    ...
    metadata: { command: normalizeUrls(command), ...(description ? { description } : {}), ...metadata }, // neocode_change
  })
})

// neocode_change start - share bash permission scanning with Neo interactive terminal
  ...
  const check = Effect.fn("ShellTool.permission")(function* (ctx: Tool.Context, input: PermissionInput) {
    ...
    yield* Effect.scoped(
      Effect.gen(function* () {
        ...
        const metadata = heredocs(tree.rootNode, ShellID.toKind(Shell.name(input.shell))) // neocode_change
        if (!containsPath(input.cwd, instance)) {
          scan.dirs.add(input.cwd)
          scan.access = "unknown"
        }
        yield* ask(ctx, scan, input.command, metadata, input.description) // neocode_change
      }),
    )
  })
...
function cmd(shell: string, command: string, cwd: string, env: NodeJS.ProcessEnv) {
  if (process.platform === "win32" && Shell.ps(shell)) {
    // neocode_change start - PowerShell args
...
export const ShellTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const permission = yield* ShellPermission // neocode_change
    ...
    const run = Effect.fn("ShellTool.run")(function* (
      ...
      const code: number | null = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const reader = yield* Effect.forkScoped( // neocode_change - keep the fiber so trailing output can be drained
          ...
          const timeout = Effect.sleep(`${CommandTimeout.duration(input.timeout)} millis`) // neocode_change
          ...
          // neocode_change start - closing the scope interrupts the reader fiber, which can drop
        ...
        return {
          ...
          execute: (params: Parameters, ctx: Tool.Context) =>
            Effect.gen(function* () {
              ...
              const timeout = CommandTimeout.clamp(params.timeout ?? defaultTimeoutMs).timeout // neocode_change
              yield* permission.ask(ctx, { command: params.command, cwd, shell, description: params.description }) // neocode_change
              ...
              return yield* run(
                {
                  ...
                  timeout,
                  description: params.description ?? params.command, // neocode_change
                },
                ctx,
              )
            }),
