...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    // neocode_change start - install local themes into supported Neo config directories
    const base = path.basename(source_dir)
    const local_dir =
      base === ".neo" || base === ".neocode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".neo", "themes")
    // neocode_change end
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".neo", "tui.json"), // neocode_change
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.NEO_PURE ? [] : pluginOrigins
    if (Flag.NEO_PURE && pluginOrigins.length) {
    }
