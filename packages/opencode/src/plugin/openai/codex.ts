import type { Hooks, PluginInput } from "@neocode/plugin"
...
import { refreshCodexAuth } from "@/neocode/provider/codex-refresh" // neocode_change
import { OpenAIWebSocketPool } from "./ws-pool"

const log = Log.create({ service: "plugin.codex" }) // neocode_change
...
const ALLOWED_MODELS = new Set([
  ...
  "gpt-5.4",
  "gpt-5.4-mini",
  // neocode_change start - additional codex models supported by Neo
  "gpt-5.1-codex",
  "gpt-5.1-codex-max",
  "gpt-5.1-codex-mini",
  "gpt-5.2-codex",
...
async function refreshAccessToken(refreshToken: string, issuer = ISSUER, signal?: AbortSignal): Promise<TokenResponse> {
  const response = await fetch(`${issuer}/oauth/token`, {
    method: "POST",
    signal,
    headers: { "Content-Type": "application/x-www-form-urlencoded", "User-Agent": `neo/${InstallationVersion}` },
    // neocode_change end
    body: new URLSearchParams({
...
<html>
  <head>
    <!-- neocode_change start -->
    <title>Neo - Codex Authorization Successful</title>
    <!-- neocode_change end -->
    <style>
      body {
        font-family:
  ...
  <body>
    <div class="container">
      ...
      <!-- neocode_change start -->
      <p>You can close this window and return to Neo.</p>
      <!-- neocode_change end -->
    </div>
    <script>
...
<html>
  <head>
    <!-- neocode_change start -->
    <title>Neo - Codex Authorization Failed</title>
    <!-- neocode_change end -->
    <style>
      body {
        font-family:
        ...
        return Object.fromEntries(
          Object.entries(provider.models)
            .filter(([, model]) => {
              ...
              if (DISALLOWED_MODELS.has(model.api.id)) return false // neocode_change
