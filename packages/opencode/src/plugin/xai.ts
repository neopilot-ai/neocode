...
function authHeaders() {
  return {
    ...
    "User-Agent": `neocode/${InstallationVersion}`, // neocode_change
  }
}
...
export function buildAuthorizeUrl(
  ...
  // clients. `referrer=neocode` lets xAI attribute neocode-originated
  // neocode_change end
  ...
  const params = new URLSearchParams({
    ...
    nonce,
    plan: "generic",
    referrer: "neocode", // neocode_change
  })
...
<html>
  <head>
    <title>Neocode - xAI Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to Neocode.</p>
    </div>
    <script>
...
<html>
  <head>
    <title>Neocode - xAI Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // neocode_change start
      // behavior and crash the entire Neocode process. Matches the silent-
      // neocode_change end
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `neocode/${InstallationVersion}`) // neocode_change
