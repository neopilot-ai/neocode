...
import { NeocodeKeybinds } from "./keybinds"
import { Filesystem } from "@/util/filesystem"
import { isRecord } from "@/util/record"
import { GlobalBus } from "@/bus/global"
import { Event } from "@/server/event"

export namespace NeocodeTuiConfig {
  ...
  const dirs = [".neo", ".neocode"] as const
  ...
  async function projectTarget(input: { directory: string; worktree?: string }) {
    ...
    return path.join(input.directory, ".neo", "tui.json")
  }
  ...
  function writable(config: Patch | TuiConfig.Info, defaults = true): Editable {
    ...
    const keybinds: Record<string, string> = defaults
      ? Object.fromEntries(NeocodeKeybinds.list().map((item) => [item.id, item.default]))
      : {}
