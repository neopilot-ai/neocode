...
/**
 * Read project ID from .neo/config.json, falling back to .neocode/config.json
...
/**
 * Resolve project ID with priority: .neo/config.json -> .neocode/config.json -> git origin URL
 * @returns Normalized project ID or undefined
 */
async function resolveProjectId(): Promise<string | undefined> {
  const dir = Instance.directory

  // Priority 1: .neo/config.json (falls back to .neocode/config.json)
...
export namespace NeoProjectID {
  export interface Interface {
    readonly get: () => Effect.Effect<string | undefined>
  }

  export class Service extends Context.Service<Service, Interface>()("@neocode/NeoProjectID") {}

  export const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const state = yield* InstanceState.make(
        Effect.fn("NeoProjectID.state")(function* () {
...
const { runPromise } = makeRuntime(NeoProjectID.Service, NeoProjectID.defaultLayer)

/**
 * Get the project ID for the current Instance context (cached per-project)
 * @returns Normalized project ID or undefined
 */
export async function getNeoProjectId(): Promise<string | undefined> {
