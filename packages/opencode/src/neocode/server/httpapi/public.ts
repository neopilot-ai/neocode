...
export function matchLegacyNeoOpenApi(input: Record<string, unknown>) {
  rebrand(input)
  ...
  const body = spec.paths?.["/neo/organization"]?.post?.requestBody?.content?.["application/json"]?.schema
  ...
  const profile = json("/neo/profile")?.schema?.properties
  const pass = profile?.neoPass?.properties
  ...
  if (profile?.neoPass) profile.neoPass = nullable(profile.neoPass)
  if (profile?.currentOrgId) profile.currentOrgId = nullable(profile.currentOrgId)

  const sessions = json("/neo/cloud-sessions")?.schema?.properties
  ...
  const claw = json("/neo/claw/status")?.schema?.properties
  ...
  const credentials = json("/neo/claw/chat-credentials")
  ...
  const fim = spec.paths?.["/neo/fim"]?.post?.responses
  if (!fim) return
  fim["200"] = {
