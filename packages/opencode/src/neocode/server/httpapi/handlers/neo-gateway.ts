...
import {
  ...
  clearModesCache,
  fetchBalance,
  fetchNeocodeNotifications,
  fetchNeoPassState,
  fetchOrganizationModes,
  fetchProfile,
} from "@neocode/neo-gateway"
import { DIRECT_FIM_ENV, requestMistralFim, resolveFimTarget } from "@neocode/neo-gateway/fim"
import { DIRECT_EDIT_ENV, extractFencedBody, resolveEditTarget } from "@neocode/neo-gateway/edit"
import { buildMercuryEditPrompt } from "@neocode/neo-gateway/edit-prompt"
import { buildNeoHeaders } from "@neocode/neo-gateway"
...
import { AudioTranscriptionsBody, ClawStatus, CloudSessionImportError, EditBody, FimBody } from "../groups/neo-gateway"
...
const log = Log.create({ service: "neo-gateway" })
...
export const neoGatewayHandlers = HttpApiBuilder.group(InstanceHttpApi, "neo", (handlers) =>
  Effect.gen(function* () {
    ...
    const profile = Effect.fn("NeoGatewayHttpApi.profile")(function* () {
      const info = yield* auth.get("neo").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
      if (!info || info.type !== "oauth") return yield* Effect.fail(new HttpApiError.Unauthorized({}))

      const currentOrgId = info.accountId ?? null
      const [profile, balance, neoPass] = yield* Effect.tryPromise({
        try: () =>
          Promise.all([
            fetchProfile(info.access),
            fetchBalance(info.access, currentOrgId ?? undefined),
            fetchNeoPassState(info.access),
          ]),
        catch: () => new HttpApiError.BadRequest({}),
      })
      return { profile, balance, neoPass, currentOrgId }
    })

    const authStatus = Effect.fn("NeoGatewayHttpApi.authStatus")(function* () {
      const info = yield* auth.get("neo").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
    ...
    const proxyAuth = Effect.fn("NeoGatewayHttpApi.proxyAuth")(function* () {
      const info = yield* auth.get("neo").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      return {
        auth: info,
        token: getToken(info),
    ...
    const modes = Effect.fn("NeoGatewayHttpApi.modes")(function* () {
    ...
    const fim = Effect.fn("NeoGatewayHttpApi.fim")(function* (ctx: { payload: typeof FimBody.Type }) {
      const target = resolveFimTarget(ctx.payload.provider, ctx.payload.model)
      const info = target.provider === "neo" ? yield* proxyAuth() : undefined
      const token = yield* Effect.gen(function* () {
        if (target.provider === "neo") return info?.token
      ...
      if (target.provider === "neo" && !info?.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
          ...
          const run = async (url: string): Promise<Response> => {
            ...
            return fetch(url, {
              ...
              headers: {
                ...
                ...(target.provider === "neo"
                  ? buildNeoHeaders(undefined, { neocodeOrganizationId: info?.organizationId })
                  : {}),
                ...(target.provider === "neo" ? { [HEADER_FEATURE]: "autocomplete" } : {}),
              },
              signal,
              body: JSON.stringify({
                model: target.model,
    ...
    const edit = Effect.fn("NeoGatewayHttpApi.edit")(function* (ctx: { payload: typeof EditBody.Type }) {
      const target = resolveEditTarget(ctx.payload.provider, ctx.payload.model)
      if (target.provider === "neo" && !target.url) {
        return yield* Effect.fail(new HttpApiError.BadRequest({}))
      }
      const proxy = target.provider === "neo" ? yield* proxyAuth() : undefined
      const token = yield* Effect.gen(function* () {
        if (target.provider === "neo") return proxy?.token
        const item = yield* auth.get(target.provider).pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
        if (item?.type === "api") return item.key
        return DIRECT_EDIT_ENV[target.provider].map((key) => process.env[key]).find(Boolean)
      })
      if (target.provider === "neo" && !proxy?.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
        ...
        try {
          return await fetch(target.url, {
            ...
            headers: {
              ...
              Authorization: `Bearer ${token}`,
              ...(target.provider === "neo"
                ? buildNeoHeaders(undefined, { neocodeOrganizationId: proxy?.organizationId })
                : {}),
              ...(target.provider === "neo" ? { [HEADER_FEATURE]: "autocomplete" } : {}),
            },
            signal,
    ...
    const audioTranscriptions = Effect.fn("NeoGatewayHttpApi.audioTranscriptions")(function* (ctx: {
      ...
      const response = yield* Effect.tryPromise({
        try: () =>
          fetch(`${NEO_API_BASE}/api/gateway/v1/audio/transcriptions`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    const notifications = Effect.fn("NeoGatewayHttpApi.notifications")(function* () {
      // Locally-detected notice about leftover opencode config; appended so it reuses each client's dismissal path.
      const notice = NeocodeConfig.opencodeConfigNotification({
        directory: Instance.directory,
        worktree: Instance.worktree,
        scanProject: !Flag.NEO_DISABLE_PROJECT_CONFIG,
      })
      ...
      const cloud = yield* Effect.promise(() =>
        fetchNeocodeNotifications({
          neocodeToken: token,
          neocodeOrganizationId: getOrganizationId(info),
        }),
      )
      return append(cloud)
    })

    const organization = Effect.fn("NeoGatewayHttpApi.organization")(function* (ctx) {
      const info = yield* auth.get("neo").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      if (!info || info.type !== "oauth") return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      yield* cache.clear("neo")
      clearModesCache()
      yield* store.disposeAll().pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      return true
    })

    const clawStatus = Effect.fn("NeoGatewayHttpApi.clawStatus")(function* () {
      const info = yield* auth.get("neo").pipe(Effect.mapError(() => new HttpApiError.ServiceUnavailable({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      return yield* Effect.tryPromise({
        try: async () => {
          const response = await fetch(`${NEO_API_BASE}/api/neoclaw/status`, { headers })
        ...
        Effect.match({
          onFailure: (err) => {
            if (err instanceof GatewayError)
              return jsonError(`NeoClaw request failed: ${err.status} ${err.message}`, err.status)
            logError("claw/status", err)
            return jsonError("Failed to reach NeoClaw", 502)
          },
    ...
    const clawChatCredentials = Effect.fn("NeoGatewayHttpApi.clawChatCredentials")(function* () {
      const info = yield* auth.get("neo").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      return {
        ...
        neoChatUrl: NEO_CHAT_URL,
        eventServiceUrl: NEO_EVENT_SERVICE_URL,
      }
    })

    const cloudSessions = Effect.fn("NeoGatewayHttpApi.cloudSessions")(function* (ctx) {
      const info = yield* auth.get("neo").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSession = Effect.fn("NeoGatewayHttpApi.cloudSession")(function* (ctx) {
      const info = yield* auth.get("neo").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSessionImport = Effect.fn("NeoGatewayHttpApi.cloudSessionImport")(function* (ctx) {
      const info = yield* auth.get("neo").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const imageModels = Effect.fn("NeoGatewayHttpApi.imageModels")(function* () {
      const info = yield* proxyAuth()
      if (!info.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      if (!info.token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      const result = yield* Effect.tryPromise({
        try: () =>
          fetchNeoImageModels({
            neocodeToken: info.token,
            neocodeOrganizationId: info.organizationId,
          }),
        catch: () => new HttpApiError.BadRequest({}),
      })
