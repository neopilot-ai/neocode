      ...
      .add(
        HttpApiEndpoint.post("capture", TelemetryPaths.capture, {
          ...
          OpenApi.annotations({
            ...
            description: "Forward a telemetry event to PostHog via neo-telemetry.",
          }),
        ),
        HttpApiEndpoint.post("setEnabled", TelemetryPaths.setEnabled, {
          ...
          OpenApi.annotations({
            ...
            description:
              "Update the PostHog client's opt-in/out state at runtime. The CLI reads NEO_TELEMETRY_LEVEL once at spawn — this route lets clients (e.g. the VS Code extension) propagate runtime telemetry consent changes.",
          }),
        ),
      )
      .annotateMerge(
        OpenApi.annotations({
          title: "telemetry",
          description: "Neo telemetry routes.",
        }),
      )
