      ...
      .add(
        HttpApiEndpoint.post("reload", ReloadPaths.reload, {
          ...
          OpenApi.annotations({
            ...
            description:
              "Atomically dispose and reboot the current Neo instance, reloading config, skills, agents, commands, and MCP prompts from disk. Returns 409 if a session is actively running.",
          }),
        ),
      )
      .annotateMerge(
        OpenApi.annotations({
          ...
          description: "Neo instance reload route.",
        }),
      )
