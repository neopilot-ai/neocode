...
export namespace Daemon {
  ...
  function root() {
    return process.env.NEO_TEST_DAEMON_STATE_DIR ?? Global.Path.state
  }

  function logs() {
    return process.env.NEO_TEST_DAEMON_LOG_DIR ?? Global.Path.log
  }
  ...
  export function matches(state: State, input: Options, explicit: readonly NetworkOption[]) {
    if (state.password === "neo") return false
  ...
  async function port(input: Options) {
    ...
    if (input.env?.NEO_TEST_DAEMON_EPHEMERAL_PORT) return 0
    ...
    try {
      const child = spawn(cmd[0], [...cmd.slice(1), ...args(input)], {
        ...
        env: {
          ...process.env,
          ...input.env,
          NEO_SERVER_USERNAME: username,
          NEO_SERVER_PASSWORD: password,
          NEOCODE_FEATURE: "daemon",
        },
  ...
  async function line(out: string) {
    ...
    const match = text.match(/neo server listening on http:\/\/([^:\s]+):(\d+)/)
