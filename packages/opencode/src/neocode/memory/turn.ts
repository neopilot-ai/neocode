...
import { MemoryTurn as TurnCore } from "@neocode/neo-memory/effect/turn"
import { MemoryPaths } from "@neocode/neo-memory/effect/paths"
import { MemoryRedact } from "@neocode/neo-memory/redact"
...
import { NeoSessionPrompt } from "@/neocode/session/prompt"
...
export namespace MemoryLifecycle {
  export const subscribe = Effect.fn("MemoryLifecycle.subscribe")(function* (input: {
    ...
    yield* input.bus.subscribeCallback(NeoSession.Event.TurnOpen, (evt) =>
      bridge.fork(
    ...
    yield* input.bus.subscribeCallback(NeoSession.Event.TurnClose, (evt) =>
      bridge.fork(
        Effect.gen(function* () {
          const ctx = yield* InstanceState.context
          const enabled = yield* NeoSessionPrompt.memoryToolEnabled({ ctx })
          if (!enabled) return
