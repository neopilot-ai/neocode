...
import { MemoryConfig } from "@neocode/neo-memory/effect/config"
import { MemoryError } from "@neocode/neo-memory/effect/errors"
import type { MemoryPorts } from "@neocode/neo-memory/effect/ports"
import { MemoryRedact } from "@neocode/neo-memory/redact"
import { MemoryShared } from "@neocode/neo-memory/shared"
...
function recalledMemory(turn: Turn) {
  return [turn.user, ...turn.assistants].flatMap((item) => item.parts).some((part) => {
    ...
    const marker = (part.metadata as { neoMemory?: { type?: string; count?: number } } | undefined)?.neoMemory
