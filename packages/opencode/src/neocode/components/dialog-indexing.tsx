...
import type { IndexingConfig, Config } from "@neocode/sdk/v2"
import * as Log from "@opencode-ai/core/util/log"
import { hasNeoIndexingAuth, resolveNeoIndexingAuth, shouldDefaultIndexingToNeo } from "../indexing-auth"
import {
...
function hasNeoAuth(sync: ReturnType<typeof useSync>, scope: IndexingScope, indexing: IndexingConfig): boolean {
  const provider = sync.data.provider_next.all.find((item) => item.id === "neo")
  const config = indexingScopeConfig(scope, sync.data.config, sync.data.globalConfig, indexing)
  return hasNeoIndexingAuth({ config, provider })
}
...
function defaultIndexing(
  ...
  const provider = sync.data.provider_next.all.find((item) => item.id === "neo")
  const config = indexingScopeConfig(scope, sync.data.config, sync.data.globalConfig, indexing)
  const auth = resolveNeoIndexingAuth({ config, provider })
  if (!shouldDefaultIndexingToNeo({ ...global, ...indexing }, auth)) return indexing
  return { ...indexing, provider: "neo", model: null, dimension: null }
}
...
function ProviderSelect(props: SubDialogProps) {
  ...
  const options: DialogSelectOption<EmbeddingProvider>[] = (
    ...
    .filter(([value]) => value !== "neo" || hasNeoAuth(sync, props.scope, indexing) || indexing.provider === "neo")
    .map(([value, title]) => ({
      value,
      title,
...
function NeoModelSelect(props: SubDialogProps) {
  const dialog = useDialog()
  const sync = useSync()
  const sdk = props.useSDK()
  const toast = useToast()
  ...
  const [catalog] = createResource(() => loadNeoEmbeddingModels(setError))
  ...
  createEffect(() => {
    ...
    seen.state = state
    log.info("Neo embedding model resource changed", {
      state: catalog.state,
      models: cfg?.models.length ?? 0,
      current: currentNeoModel(cfg, indexing.model),
      defaultModel: cfg?.defaultModel || undefined,
      scope: props.scope,
    })
  })
  const options = createMemo(() => neoModelOptions(catalog()))
  const current = createMemo(() => currentNeoModel(catalog(), indexing.model))

  return (
    <DialogSelect
      title="Neo Embedding Model"
      options={options()}
      current={current()}
      renderFilter={(catalog()?.models.length ?? 0) > 0}
      onSelect={async (option) => {
        if (!option.value || !catalog()?.models.some((model) => model.id === option.value)) return
        log.info("selected Neo embedding model", { model: option.value, scope: props.scope })
...
function ScopeSelect(props: DialogIndexingProps & { scope: IndexingScope }) {
  ...
  const options: DialogSelectOption<IndexingScope>[] = [
    ...
    { value: "project", title: "Project", description: "Stored in this repo's .neo config" },
  ]
  ...
  const options = createMemo<DialogSelectOption<string>[]>(() => {
    ...
    const result: DialogSelectOption<string>[] = [
      ...
      },
      {
        ...
        category: "Embedding",
        description: mark(
          indexing.provider === "neo" ? (indexing.model ?? "Neo catalog") : (indexing.model ?? "default"),
          [["model"]],
        ),
      },
      {
      ...
      onSelect={async (option) => {
        ...
        switch (option.value) {
          ...
          case "providerSettings":
            ...
            break
          case "model": {
            if (indexing.provider === "neo") {
              dialog.replace(() => (
                <NeoModelSelect useSDK={props.useSDK} scope={scope()} indexing={indexing} raw={raw} />
              ))
              break
            }
          ...
          case "dimension": {
            if (indexing.provider === "neo") break
