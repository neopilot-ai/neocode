 ...
 * Neo News Component
 *
 * Self-contained component that fetches and displays Neo news/notifications.
...
import { DialogNeoNotifications } from "./dialog-neo-notifications.js"
import { News } from "./news.js"

export function NeoNews() {
  const sync = useSync()
  const sdk = useSDK()
  ...
  const isNeoConnected = createMemo(() => sync.data.provider_next.connected.includes("neo"))
  const unread = createMemo(() => News.unread(notifications(), kv.get(News.key, [])))

  const openNewsDialog = () => {
    const items = unread()
    if (items.length === 0) return
    dialog.replace(() => <DialogNeoNotifications notifications={items} />)
  ...
  createEffect(
    on(
      ...
      async (status) => {
        ...
        if (!isNeoConnected()) return

        const result = await sdk.client.neo.notifications()
