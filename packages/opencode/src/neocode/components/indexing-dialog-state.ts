import { fetchNeoEmbeddingModelCatalog, resolveNeoGatewayBaseUrl } from "@neocode/neo-gateway"
import type { Config, IndexingConfig, NeoEmbeddingModelCatalog } from "@neocode/sdk/v2"
...
export async function loadNeoEmbeddingModels(onError?: (message: string) => void) {
  const endpoint = new URL("embedding-models", resolveNeoGatewayBaseUrl()).toString()
  log.info("loading Neo embedding model catalog", { endpoint })
  const catalog = await fetchNeoEmbeddingModelCatalog({
    onError: (issue) => {
      log.warn("failed to load Neo embedding model catalog", {
        code: issue.code,
        status: issue.status,
  ...
  log.info("loaded Neo embedding model catalog", {
...
export function neoModelOptions(catalog?: NeoEmbeddingModelCatalog) {
...
export function currentNeoModel(catalog: NeoEmbeddingModelCatalog | undefined, model?: string | null) {
