...
import { staticEnvLines, type EditorContext } from "@/neocode/editor-context"
import { NeoMemory } from "@neocode/neo-memory/effect"
import type { MemoryPaths } from "@neocode/neo-memory/effect/paths"
import { MemoryMarker } from "@/neocode/memory/marker"
import type { Provider } from "@/provider/provider"
import type { InstanceContext } from "@/project/instance-context"
import * as Log from "@opencode-ai/core/util/log"

const log = Log.create({ service: "neocode.system-prompt" })

export namespace NeocodeSystemPrompt {
  export function environment(input: { ctx: InstanceContext; model: Provider.Model; editor?: EditorContext }) {
    return [
      [
        ...
        `  Project config: .neo/command/*.md, .neo/agent/*.md, neo.json, AGENTS.md. Put new commands and agents in .neo/. Do not use .neocode/ or .opencode/.`,
  ...
  export function memoryBlocks(input: {
    ...
    return Effect.gen(function* () {
      ...
      const guidance = [
        "The following Neo memory blocks are saved project memory from this project's previous sessions. You do have this prior-session context; never claim you lack memory of earlier work here while these blocks are present.",
        ...
        "When the user explicitly asks you to remember, save, correct, update, or forget project memory, call neo_memory_save.",
        "When the user asks about prior work, project history, saved decisions, conventions, setup, or prior rationale beyond what the records below cover, call neo_memory_recall (mode=search with likely stored words, then mode=catalog) before relying on general knowledge.",
        "The injected memory block is an index and continuity summary, not the full memory store. When a request depends on exact saved details that are only listed as keys, topics, summaries, or truncated records, call neo_memory_recall before answering.",
        "When a request could depend on durable typed memory categories such as project facts, environment commands/paths/tooling, decisions, constraints, or corrections, call neo_memory_recall (mode=typed or mode=search) if the injected index only hints at the answer, may be incomplete, or does not include the exact detail needed.",
        ...
        "Use neo_memory_recall with mode=digest and sessionID=<id> when the injected digest is too thin but points to a real prior session.",
        "For topic-specific memory, use neo_memory_recall with mode=search or mode=typed.",
        "Use neo_local_recall with mode=read only when saved memory is insufficient and transcript detail is actually needed, or when the user asks for full transcript detail.",
