...
declare global {
  const NEO_SESSION_EXPORT_WORKER_PATH: string
}
...
function target(): WorkerTarget {
  if (typeof NEO_SESSION_EXPORT_WORKER_PATH !== "undefined") return NEO_SESSION_EXPORT_WORKER_PATH
...
function currentSurface(): string {
  return process.env.NEOCODE_FEATURE?.trim() || "unknown"
}
