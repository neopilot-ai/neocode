...
async function headers(args: HeaderArgs): Promise<Headers> {
  ...
  const out = new Headers({
    ...
    "x-neo-export-api-version": "1",
    "x-neo-export-schema-version": "1",
    "x-neo-export-agent-version": args.agentVersion,
    "x-neo-export-surface": args.surface,
    "x-neo-export-root-session-id": first.rootSessionId,
    "x-neo-export-session-id": first.sessionId,
    "x-neo-export-batch-id": args.batchId,
    "x-neo-export-seq-start": String(Math.min(...seqs)),
    "x-neo-export-seq-end": String(Math.max(...seqs)),
    "x-neo-export-event-count": String(args.rows.length),
    "x-neo-export-payload-sha256": await sha256Hex(args.body),
    "x-neo-export-client-sent-at": new Date().toISOString(),
    "x-neo-export-content-encoding": "identity",
  })
  const token = process.env.NEO_SESSION_EXPORT_AUTH_TOKEN
  if (token) out.set("authorization", `Bearer ${token}`)
  const anon = args.anonId ?? (await anonId(args.anonIdPath))
  if (!token && anon) out.set("x-neo-anon-id", anon)
  return out
}
