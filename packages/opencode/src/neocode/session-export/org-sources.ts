...
import { resolveNeoIndexingAuth } from "@/neocode/indexing-auth"
...
export async function getAuthOrgId(): Promise<OrgState> {
  try {
    const [cfg, info] = await Promise.all([
      ...
      auth.runPromise((svc) => svc.get("neo")),
    ])
    const id = resolveNeoIndexingAuth({ config: cfg, auth: info }).organizationId
