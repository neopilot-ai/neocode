...
import { EventServiceClient } from "@/neocode/event-service/client"
import { NEO_EVENT_SERVICE_URL } from "@neocode/neo-gateway"
...
const log = Log.create({ service: "neo-viewers" })

function inferPlatform(): Platform | undefined {
  const p = process.env.NEO_PLATFORM
...
function extract(auth: Auth.Info | undefined): { token: string | undefined; identity: string | undefined } {
  const envKey = process.env.NEO_API_KEY?.trim()
...
export namespace NeoViewers {
  export interface Interface {
    readonly update: (snapshot: ViewerSnapshot) => Effect.Effect<void>
    readonly invalidateAuth: () => Effect.Effect<void>
  }

  export class Service extends Context.Service<Service, Interface>()("@neocode/NeoViewers") {}

  export const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const auth = yield* Auth.Service
      const NeoSessions = (yield* Effect.promise(() => import("@/neo-sessions/neo-sessions"))).NeoSessions

      const platform = inferPlatform()
      const killSwitch = process.env.NEO_DISABLE_PRESENCE === "1"
      // Same endpoint the server envelope hands NeoClaw; NEO_EVENT_SERVICE_URL
      // is a presence-specific override on top of the gateway's EVENT_SERVICE_URL.
      const url = process.env.NEO_EVENT_SERVICE_URL || NEO_EVENT_SERVICE_URL
      ...
      function pushAttached() {
        ...
        if (!sameArr(union, s.prevAttached)) {
          ...
          NeoSessions.setAttachedSessions(union)
        }
      }
      ...
      const readAuth = auth.get("neo").pipe(Effect.orElseSucceed((): Auth.Info | undefined => undefined))

      const update = Effect.fn("NeoViewers.update")(function* (snapshot: ViewerSnapshot) {
      ...
      const invalidateAuth = Effect.fn("NeoViewers.invalidateAuth")(function* () {
        disconnectClient()
      ...
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          ...
          disconnectClient()
          s.viewers.clear()
          NeoSessions.setAttachedSessions([])
        }),
      )
