...
//          persist `"snapshot": false` to `.neo/neo.json`, and skip. All
...
//     Instead we write the file directly via `NeocodeConfig.updateProjectConfig`
...
//
// All of this is Neo-specific — the upstream snapshot module remains a thin
...
import { NeoPartLifecycle } from "@/neocode/session/part-lifecycle"
...
export namespace NeoSnapshotTrack {
  ...
  export const TIMEOUT_MS = duration("NEO_SNAPSHOT_TRACK_TIMEOUT_MS", 10_000)
  export const TURN_TIMEOUT_MS = duration("NEO_SNAPSHOT_TURN_TIMEOUT_MS", 120_000)
  ...
  export const progressPart = (input: {
    ...
    text: input.text,
    synthetic: true,
    metadata: { [NeoPartLifecycle.key]: "transient" },
  })
  ...
  async function currentDirectory(): Promise<{ directory: string; worktree?: string } | undefined> {
    const { Instance } = await import("@/neocode/instance")
    try {
