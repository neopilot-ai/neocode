import type { CodeIndexManager } from "@neocode/neo-indexing/engine"
...
async function init(request: Extract<Request, { method: "init" }>) {
  ...
  if (request.input.lancedbPath) process.env.NEO_LANCEDB_PATH = request.input.lancedbPath
  const [engine, status] = await Promise.all([
    import("@neocode/neo-indexing/engine"),
    import("@neocode/neo-indexing/status"),
  ])
