/**
 * NeoClaw full-screen view
 *
 * Main layout component for the /neoclaw route.
...
export function NeoClawView() {
  ...
  // Bot display name — sourced from the NeoClaw platform status (set by the
  // user during onboarding via patchBotIdentity). Falls back to the literal
  // "NeoClaw" while loading or for instances that skipped onboarding,
  // matching the web UI's fallback chain.
  const botName = createMemo(() => status()?.botName ?? "NeoClaw")

  // NeoClaw view commands — single source of truth for both the global
  // command palette / keybinds and the in-chat slash autocomplete.
  // The list is reactive on `chat.connected` so the slash menu only
  // exposes `/new` and `/conversations` once we're connected to neo-chat.
  const neoCommands = createMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        desc: "Start a new NeoClaw conversation",
        category: "NeoClaw",
        namespace: "palette",
        slashName: "new",
        slashAliases: [] as string[],
      ...
      {
        name: "neoclaw.conversations",
        title: "Conversations",
        desc: "Browse, rename, and delete NeoClaw conversations",
        category: "NeoClaw",
        namespace: "palette",
  ...
  useBindings(() => ({
    commands: neoCommands(),
    bindings: [{ key: "escape", cmd: "neoclaw.back" }],
  }))
  ...
  const clawSlashes = createMemo<ClawSlashOption[]>(() => {
    const visible = neoCommands().filter((c) => c.enabled !== false && !c.hidden && c.slashName)
