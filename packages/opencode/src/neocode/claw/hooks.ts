...
/**
 * NeoClaw SolidJS reactive helpers — Neo Chat protocol.
 *
 * Exposes:
 *   - `createClawStatus`: polled instance status
 *   - `createClawChat`: Neo Chat connection + active-conversation state
 *
...
/**
 * Poll the NeoClaw instance status every `interval` ms.
 */
export function createClawStatus(sdk: SDK, interval = 10_000) {
  ...
  onMount(() => {
    const poll = async () => {
      const res = await sdk.client.neo.claw.status().catch(() => null)
...
export function createClawChat(sdk: SDK) {
  ...
  onMount(async () => {
    ...
    const statusRes = await sdk.client.neo.claw.status().catch(() => null)
    ...
    const credsRes = await sdk.client.neo.claw.chatCredentials().catch((e: unknown) => {
    ...
    if (!envelope.neoChatUrl) missing.push("neoChatUrl")
