...
/**
 * NeoClaw Neo Chat client wrapper for the TUI.
 *
 * Hosts the HTTP and WebSocket clients, exposes a multi-conversation API
 * (list/create/select/rename/delete), and translates Neo Chat events for
...
import { NeoChatClient } from "./neo-chat-client"
...
export async function connect(input: ConnectInput): Promise<ClawChatClient> {
  ...
  const chat = new NeoChatClient({
    baseUrl: input.envelope.neoChatUrl,
    getToken: async () => input.envelope.token,
  })
  ...
  const sandboxCtx = `/neoclaw/${input.sandboxId}`
  ...
  function setActive(id: string | null): void {
    ...
    clearTyping()
    if (id) {
      activeCtx = `/neoclaw/${input.sandboxId}/${id}`
