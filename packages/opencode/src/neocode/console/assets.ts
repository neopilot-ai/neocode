...
export namespace ConsoleAssets {
  ...
  async function dir() {
    const override = process.env.NEO_CONSOLE_ASSET_DIR
  ...
  function source() {
    return path.resolve(import.meta.dirname, "../../../../neo-console")
  }
  ...
  async function build(app: string, out: string) {
    ...
    const run = runBuild(app, out).catch((err) => {
      log.warn("failed to build Neo Console assets", { err })
      return undefined
    })
    builds.set(app, run)
    return await run
  ...
  async function runBuild(app: string, out: string) {
    log.info("building Neo Console assets", { app })
    const proc = Bun.spawn([process.execPath, "run", "build"], {
      cwd: app,
      env: { ...process.env, NEO_CONSOLE_BASE: base },
      stdout: "pipe",
      stderr: "pipe",
      windowsHide: true,
    })
    ...
    if (code !== 0) throw new Error(`Neo Console build failed with exit code ${code}: ${stderr || stdout}`)
    if (!(await ready(out, true))) throw new Error("Neo Console build did not produce /console assets")
    return out
  }
