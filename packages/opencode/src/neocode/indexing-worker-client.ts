...
} from "@neocode/neo-indexing/engine"
import type { IndexingStatus } from "@neocode/neo-indexing/status"
...
declare global {
  const NEO_INDEXING_WORKER_PATH: string
}
...
export namespace IndexingWorker {
  ...
  const channel = () => {
    ...
    const file =
      typeof NEO_INDEXING_WORKER_PATH !== "undefined"
        ? NEO_INDEXING_WORKER_PATH
        ...
        return call(
          state,
          {
            ...
            input: {
              ...
              config,
              baselineDirectory,
              lancedbPath: process.env.NEO_LANCEDB_PATH,
            },
          },
          (message) => {
