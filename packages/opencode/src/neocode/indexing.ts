...
import { type IndexingTelemetryEvent, type VectorStoreSearchResult } from "@neocode/neo-indexing/engine"
import { toIndexingConfigInput, type IndexingConfig } from "@neocode/neo-indexing/config"
import { hasIndexingPlugin } from "@neocode/neo-indexing/detect"
import { IndexingStatus, disabledIndexingStatus } from "@neocode/neo-indexing/status"
...
import { LanceDBRuntime } from "./lancedb" // neocode_change
import { indexingWithNeoDefault, resolveNeoIndexingAuth, type NeoIndexingAuth } from "./indexing-auth" // neocode_change
import { primaryWorktree } from "./primary-worktree"

const log = Log.create({ service: "neocode-indexing" })
...
const baselineDirectory = Effect.fn("NeoIndexing.baselineDirectory")(function* (dir: string) {
...
async function neoAuth(cfg: Config.Info): Promise<NeoIndexingAuth> {
  const info = await auth.runPromise((svc) => svc.get("neo"))
  return resolveNeoIndexingAuth({ config: cfg, auth: info })
}

function enrichNeo(input: ReturnType<typeof toIndexingConfigInput>, auth: NeoIndexingAuth) {
  if (input.embedderProvider !== "neo") return input

  return {
    ...input,
    neoApiKey: input.neoApiKey ?? auth.apiKey,
    neoBaseUrl: input.neoBaseUrl ?? auth.baseUrl,
    neoOrganizationId: input.neoOrganizationId ?? auth.organizationId,
  }
}

async function model(input: ReturnType<typeof toIndexingConfigInput>, auth: NeoIndexingAuth) {
  if (input.embedderProvider !== "neo" || !input.enabled) return input

  const catalog = await fetchNeoEmbeddingModelCatalog({ baseURL: auth.baseUrl, token: auth.apiKey })
  ...
  if (!found) {
    if (input.modelId || input.modelDimension) {
      log.warn("ignoring unsupported Neo embedding model configuration", { model: input.modelId })
    }
...
export namespace NeoIndexing {
  ...
  const boot = async (hit: Cache): Promise<Entry> => {
    ...
    if (process.env["NEO_DISABLE_CODEBASE_INDEXING"] === "vscode-no-workspace") {
  ...
  export async function models() {
    try {
      ...
      const fallback = await fetchNeoEmbeddingModelCatalog()
      return fallback.models.length > 0 ? fallback : catalog
    } catch (err) {
      log.warn("falling back to public Neo embedding model catalog", { err })
      return fetchNeoEmbeddingModelCatalog()
    }
  }
