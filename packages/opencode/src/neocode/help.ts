...
async function getSubcommands(
  ...
  const inst = yargs([]).scriptName(`neo ${name}`).wrap(null)
  builder(inst)
  ...
  try {
    ...
    for (const [sub, handler] of Object.entries(handlers as Record<string, any>)) {
      ...
      const full = `${name} ${sub}`
      const subInst = yargs([]).scriptName(`neo ${full}`).wrap(null)
...
function formatMarkdown(
  ...
  for (const section of sections) {
    parts.push(`## ${section.name ? `neo ${section.name}` : "neo"}`)
    parts.push("")
    if (section.hidden) {
...
function formatText(
  ...
  for (const section of sections) {
    parts.push(rule)
    const display = section.name ? `neo ${section.name}` : "neo"
    ...
    for (const sub of section.subs) {
      const sublabel = sub.hidden ? `--- neo ${sub.name} [internal] ---` : `--- neo ${sub.name} ---`
      parts.push(sublabel)
      parts.push("")
      parts.push(sub.help)
      parts.push("")
...
export async function generateCommandTable(options?: { commands?: Cmd[] }) {
  ...
  for (const cmd of cmds) {
    ...
    const display = raw.startsWith("$0") ? "neo" + raw.slice(2) : "neo " + raw
