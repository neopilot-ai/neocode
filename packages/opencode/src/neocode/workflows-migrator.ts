...
export namespace WorkflowsMigrator {
  ...
  // .neocode first (lower precedence), .neo second (higher precedence / wins)
  const NEO_WORKFLOWS_DIRS = [".neocode/workflows", ".neo/workflows"]
  const globalWorkflowsDirs = () => [
    path.join(home(), ".neocode", "workflows"),
    path.join(home(), ".neo", "workflows"),
  ]

  export interface NeocodeWorkflow {
    name: string
    path: string
    content: string
  ...
  async function loadWorkflowsFromDir(
    ...
    root?: string,
    warnings: string[] = [],
  ): Promise<NeocodeWorkflow[]> {
    if (!(await Filesystem.isDir(dir))) return []
    const files = await findWorkflowFiles(dir)
    const workflows: NeocodeWorkflow[] = []
    for (const file of files) {
      const options = {
        trusted: source === "global",
        fileScope: source === "project" && root ? { root, source: file } : undefined,
      }
      const content = await NeocodeMarkdown.read(file, options)
        .then((text) => NeocodeMarkdown.substitute(text, file, options))
        .catch((err) => {
  ...
  export async function discoverWorkflows(
    ...
    if (!skipGlobalPaths) {
      ...
      const vscodeWorkflowsDir = path.join(NeocodePaths.vscodeGlobalStorage(), "workflows")
      workflows.push(...(await loadWorkflowsFromDir(vscodeWorkflowsDir, "global", undefined, warnings)))

      // 2. Home directories ~/.neocode/workflows and ~/.neo/workflows
      for (const dir of globalWorkflowsDirs()) {
        workflows.push(...(await loadWorkflowsFromDir(dir, "global", undefined, warnings)))
      }
    }

    // 3. Project workflows (.neo/workflows/ and .neocode/workflows/)
    for (const dir of NEO_WORKFLOWS_DIRS) {
  ...
  export function convertToCommand(workflow: NeocodeWorkflow): ConfigCommandV1.Info {
    return {
  ...
  export async function migrate(options: {
    ...
    const workflowsByName = new Map<string, NeocodeWorkflow>()
