...
import { NeocodePaths } from "@/neocode/paths"

export namespace ConfigProtection {
  /**
   * Config directory prefixes (relative paths, forward-slash normalized).
   * Matches .neo/ and legacy .neocode/ at any depth within the project.
   */
  const CONFIG_DIRS = [".neo/", ".neocode/"]
   ...
   */
  const CONFIG_ROOT_FILES = new Set(["neo.json", "neo.jsonc", "opencode.json", "opencode.jsonc", "AGENTS.md"])
  ...
  export function isRelative(pattern: string): boolean {
    ...
    for (const dir of CONFIG_DIRS) {
      const bare = dir.slice(0, -1) // e.g. ".neo"
      // Match at root (e.g. ".neo/foo") or nested (e.g. "packages/sub/.neo/foo")
  ...
  function configs(): string[] {
    return Array.from(
      new Set([Global.Path.config, process.env.XDG_CONFIG_HOME ? path.join(process.env.XDG_CONFIG_HOME, "neo") : ""]),
    ).filter(Boolean)
  }
  ...
  function skillRoot(pattern: string): string | undefined {
    ...
    const roots = [...configs(), ...NeocodePaths.globalDirs()]
  ...
  function fallback(p: string): boolean {
    ...
    return keys(p).some(
      (key) =>
        key.endsWith("/config/neo") ||
        key.includes("/config/neo/") ||
        key.endsWith("/.config/neo") ||
        key.includes("/.config/neo/"),
    )
  }
