...
import type { OrganizationMode } from "@neocode/neo-gateway"

export namespace ModesMigrator {
  // Neocode mode structure
  export interface NeocodeMode {
    slug: string
    name: string
  ...
  export interface NeocodeModesFile {
    customModes: NeocodeMode[]
  }
  ...
  export function convertPermissions(groups: NeocodeMode["groups"]): ConfigPermission.Info {
  ...
  export function convertMode(mode: NeocodeMode): ConfigAgentV1.Info {
  ...
  export async function readModesFile(filepath: string): Promise<NeocodeMode[]> {
    try {
      const content = await fs.readFile(filepath, "utf-8")
      // Wrap YAML content in frontmatter delimiters so gray-matter can parse it
      const wrapped = `---\n${content}\n---`
      const parsed = matter(wrapped).data as NeocodeModesFile
  ...
  export async function migrate(options: {
    ...
    const allModes: NeocodeMode[] = []

    if (!options.skipGlobalPaths) {
      // 1. VSCode extension global storage (primary location for global modes)
      const vscodeGlobalPath = path.join(NeocodePaths.vscodeGlobalStorage(), "settings", "custom_modes.yaml")
      allModes.push(...(await readModesFile(vscodeGlobalPath)))

      // 2. CLI global settings (fallback/alternative location)
      const cliGlobalPath = path.join(os.homedir(), ".neocode", "cli", "global", "settings", "custom_modes.yaml")
      allModes.push(...(await readModesFile(cliGlobalPath)))

      // 3. Home directory .neocodemodes
      const homeModesPath = path.join(os.homedir(), ".neocodemodes")
    ...
    // 5. Project .neocodemodes
    const projectModesPath = path.join(options.projectDir, ".neocodemodes")
    allModes.push(...(await readModesFile(projectModesPath)))

    // Deduplicate by slug (later entries win)
    const modesBySlug = new Map<string, NeocodeMode>()
