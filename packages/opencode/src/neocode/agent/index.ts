...
function planEditRules(worktree: string) {
  return {
    "*": "deny" as const,
    [path.join(".neo", "plans", "*.md")]: "allow" as const,
...
// Prepare neo-specific data derived from config. Call once per state initialization.
export function prepare(cfg: Config.Info): NeoData {
  const mcpRules = getMcpRules(cfg)
  const defaultsPatch = Permission.fromConfig({
    bash,
    recall: "ask",
    ...(Flag.NEO_CLIENT === "vscode" && cfg.experimental?.native_notebook_tools === true
      ? { notebook_read: "ask" as const, notebook_edit: "ask" as const, notebook_execute: "ask" as const }
      : {}),
    neo_memory_recall: "ask",
    neo_memory_save: "ask",
  })
...
// Lift Neo-internal metadata onto typed agent fields and remove it from `options`.
...
// Patch the base agents map in-place with all neo-specific changes:
// - Rename build → code
// - Patch plan with readOnlyBash, mcpRules, .neo paths
...
export function patchAgents(
  ...
  if (agents.plan) {
    agents.plan = {
      ...
      permission: Permission.merge(
        defaults,
        planGuard(worktree, neo.mcpRules),
        user,
  ...
  agents.ask = {
    ...
    prompt: PROMPT_ASK,
    options: {},
    permission: Permission.merge(defaults, askGuard(neo.mcpRules), user, askEditGuard(), denies(user)),
    mode: "primary",
    native: true,
  }
 ...
 * config-backed agent entries, and/or removing it from legacy .neocodemodes YAML files.
 * Scans all config directories for agent/mode .md files matching the name,
 * then also checks the .neocodemodes files the ModesMigrator reads.
 */
export async function remove(input: { name: string; agent?: AgentInfo; dirs: string[]; directory: string }) {
  ...
  // 2. Remove from legacy .neocodemodes YAML files (read by ModesMigrator)
  const { ModesMigrator } = await import("@/neocode/modes-migrator")
  const { NeocodePaths } = await import("@/neocode/paths")
  const os = await import("os")
  const matter = (await import("gray-matter")).default
  const home = os.default.homedir()
  const modesFiles = [
    path.join(NeocodePaths.vscodeGlobalStorage(), "settings", "custom_modes.yaml"),
    path.join(home, ".neocode", "cli", "global", "settings", "custom_modes.yaml"),
    path.join(home, ".neocodemodes"),
    path.join(input.directory, ".neocodemodes"),
  ]
...
async function removeConfigAgent(name: string, directory: string) {
  const { NeocodeConfigOverlay } = await import("@/neocode/config/overlay")
  const files = [
    NeocodeConfigOverlay.globalTarget(),
    await NeocodeConfigOverlay.projectTarget({ directory }),
  ]
  let found = false
