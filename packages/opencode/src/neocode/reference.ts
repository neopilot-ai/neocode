...
import { isInterrupted } from "@/neocode/effect/cause"
...
// Keep Core V2 tools on the same effective Neo config used by stable tools. Core's standalone
// scanner cannot see account/managed config or NEO_CONFIG_CONTENT, so replace its provisional
// references after both config systems finish booting.
export const sync = Effect.fn("NeoReference.sync")(function* (input: {
