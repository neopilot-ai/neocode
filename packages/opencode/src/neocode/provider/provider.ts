...
//
// Neo-specific provider logic extracted from packages/opencode/src/provider/provider.ts
...
// calls at well-defined injection points (each marked with neocode_change).

import { createNeo, type NeoProvider, AI_SDK_PROVIDERS, PROMPTS } from "@neocode/neo-gateway"
import { DEFAULT_HEADERS } from "@/neocode/const"
...
export const NEO_BUNDLED_PROVIDERS: Record<string, () => Promise<(options: any) => BundledSDK>> = {
  "@neocode/neo-gateway": async () => createNeo as unknown as (options: any) => BundledSDK,
}
...
export const NEO_MODEL_SCHEMA_EXTENSIONS = {
...
// ---------------------------------------------------------------------------
// fromModelsDevModel patch — returns neo-specific fields
// ---------------------------------------------------------------------------

export function patchModelsDevModel(providerID: string, source: any) {
  return {
    variants: providerID === "neo" ? (source.variants ?? {}) : {},
...
// ---------------------------------------------------------------------------
// Config model patch — merges neo-specific fields from config + existing
// ---------------------------------------------------------------------------
...
export function patchNeoProviderPrivacy(provider: { options?: Record<string, any> } | undefined, config: any) {
  if (!provider || config.hide_prompt_training_models !== true) return
  provider.options = { ...provider.options, dataCollection: "deny" }
}

export function neoCustomLoaders(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    neo: Effect.fnUntraced(function* (input: any) {
      ...
      const hasKey = yield* Effect.gen(function* () {
        ...
        if (config.provider?.["neo"]?.options?.apiKey) return true
        return false
      })

      const options: Record<string, string> = {}
      if (env.NEO_ORG_ID) {
        options.neocodeOrganizationId = env.NEO_ORG_ID
      }
      ...
      return {
        ...
        options,
        async getModel(sdk: NeoProvider, modelID: string) {
  ...
  switch (providerID) {
    ...
    case "cerebras":
      result.options.headers = {
        ...result.options.headers,
        "X-Cerebras-3rd-Party-Integration": "neo",
      }
      break
    case "azure": {
    ...
    // in provider.ts with single-line neocode_change markers
  }
}
...
export function neoSmallModelPriority(providerID: string): string[] | undefined {
  if (providerID.startsWith("neo")) return ["neo-auto/small"]
  return undefined
}
