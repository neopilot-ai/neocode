...
export namespace IgnoreMigrator {
  const log = Log.create({ service: "neocode.ignore-migrator" })

  const NEOCODEIGNORE_FILE = ".neocodeignore"
  const GLOBAL_NEOCODEIGNORE = path.join(os.homedir(), ".neocode", NEOCODEIGNORE_FILE)
  ...
  /**
   * Parse .neocodeignore content into patterns.
  ...
  /**
   * Load patterns from a .neocodeignore file
   */
  ...
  /**
   * Migrate .neocodeignore to Opencode permission config
   */
  export async function migrate(options: { projectDir: string; skipGlobalPaths?: boolean }): Promise<MigrationResult> {
    ...
    // 1. Load global .neocodeignore (lower priority)
    if (!options.skipGlobalPaths) {
      const globalPatterns = await loadIgnoreFile(GLOBAL_NEOCODEIGNORE, "global")
      allPatterns.push(...globalPatterns)

      if (globalPatterns.length > 0) {
        log.debug("loaded global .neocodeignore", { count: globalPatterns.length })
      }
    }

    // 2. Load project .neocodeignore (higher priority - added last)
    const projectIgnorePath = path.join(options.projectDir, NEOCODEIGNORE_FILE)
    const projectPatterns = await loadIgnoreFile(projectIgnorePath, "project")
    allPatterns.push(...projectPatterns)

    if (projectPatterns.length > 0) {
      log.debug("loaded project .neocodeignore", { count: projectPatterns.length })
    }
  ...
  /**
   * Load .neocodeignore and return permission config.
  ...
  export async function loadIgnoreConfig(
    ...
    try {
      ...
      if (result.patternCount > 0) {
        log.info("loaded .neocodeignore patterns", {
      ...
      log.warn("failed to load .neocodeignore", { error: err })
      return {}
    }
  }
}
