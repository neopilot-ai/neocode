...
export namespace NeoRoutedModel {
  const ns = "neocode"
  ...
  export function readAuto(
    ...
    if (input.providerID !== ProviderV2.ID.neo) return undefined
    if (!input.modelID.startsWith("neo-auto/") && !input.modelID.includes("fable")) return undefined
