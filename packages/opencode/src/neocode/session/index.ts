...
import { ENV_FEATURE } from "@neocode/neo-gateway"
import { existsSync } from "fs"
import path from "path"
import { NeoSessionEvent, type NeoSessionCloseReason } from "./event"

export namespace NeoSession {
  const log = Log.create({ service: "session.neo" })

  // ---------------------------------------------------------------------------
  // Events
  // ---------------------------------------------------------------------------

  export const Event = NeoSessionEvent
  export type CloseReason = NeoSessionCloseReason
  ...
  export function attribution(id: string): { rootID: string; feature?: string } {
    ...
    const platform = resolvePlatform(rootID) ?? process.env["NEO_PLATFORM"]
  ...
  // ---------------------------------------------------------------------------
  // Provider-reported cost (Neo / OpenRouter / Vercel AI Gateway)
  // ---------------------------------------------------------------------------
   ...
   *                                      (`costDetails.upstreamInferenceCost` for Neo)
   ...
   *
   * Neo does not charge end users a per-request fee, so for the Neo provider the
  ...
  export function providerCost(input: {
    ...
    const isNeo = (input.provider?.id ?? input.providerID) === "neo"
    ...
    if (orUsage) {
      ...
      // Neo doesn't charge a fee on top of the upstream inference cost, so for Neo
      ...
      const cost = isNeo && upstream !== undefined ? upstream : regular
      if (cost !== undefined) return cost
    }

    // 2. Anthropic Messages or OpenAI Responses via OpenRouter. The Neo Gateway wrapper
    //    restores the verbatim usage payload under the AI SDK's raw usage escape hatch.
    //    Neo doesn't charge end users a per-request fee, so only upstream cost is relevant.
    ...
    //    gateway fee that Neo would pass through, but Neo doesn't charge end users a
  ...
  export function unshareSession(id: SessionID) {
    return EffectBridge.fromPromise(async () => {
      const { NeoSessions } = await import("@/neo-sessions/neo-sessions")
      await NeoSessions.unshare(id)
    }).pipe(Effect.catchCause((cause) => Effect.fail(Cause.squash(cause))))
  }

  export async function removeSession(id: string): Promise<void> {
    const { NeoSessions } = await import("@/neo-sessions/neo-sessions")
    await NeoSessions.remove(id).catch(() => {})
  }
    ...
    return Effect.gen(function* () {
      ...
      const worktree = (dir: string) => {
        for (const root of sorted) {
          ...
          if ((parts[0] === ".neo" || parts[0] === ".neocode") && parts[1] === "worktrees" && parts[2]) {
...
export { neoSessionFork } from "./fork-command"
