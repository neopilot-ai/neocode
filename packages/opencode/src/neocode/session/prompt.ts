...
import { PlanFollowup } from "@/neocode/plan-followup"
import { PlanFile } from "@/neocode/plan-file"
import { NeoSession } from "@/neocode/session"
import { NeoSessionMessageOrder } from "@/neocode/session/message-order"
import { NeoSessionPromptQueue } from "@/neocode/session/prompt-queue"
import { Permission } from "@/permission"
import { Question } from "@/question"
import { environmentDetails } from "@/neocode/editor-context"
import { Identifier } from "@/id/id"
import { Filesystem } from "@/util/filesystem"
import NATIVE_PLAN_PROMPT from "@/neocode/session/native-plan-prompt.txt"
...
import { NeocodeSystemPrompt } from "@/neocode/system-prompt"
import { NeoToolRegistry } from "@/neocode/tool/registry"
import CODE_SWITCH from "@/session/prompt/code-switch.txt"

export namespace NeoSessionPrompt {
  ...
  export const abortIntakes = Effect.fn("NeoSessionPrompt.abortIntakes")(function* (sessionID: SessionID) {
    const entries = [...(intakes.get(sessionID) ?? [])]
  ...
  function supportsPlanFollowup() {
    return ["cli", "vscode", "jetbrains"].includes(Flag.NEO_CLIENT)
  }
  ...
  export const cancelTree = Effect.fn("NeoSessionPrompt.cancelTree")(function* (input: {
    sessionID: SessionID
    ...
    yield* Effect.forEach(
      ...
      (sessionID) =>
        Effect.gen(function* () {
          yield* NeoSessionPromptQueue.cancel(sessionID)
  ...
  export const recoverDanglingAssistant = Effect.fn("NeoSessionPrompt.recoverDanglingAssistant")(function* (input: {
    sessionID: SessionID
  ...
  export const recoverProviderFinishError = Effect.fn("NeoSessionPrompt.recoverProviderFinishError")(
    function* (input: {
      sessionID: SessionID
      status: Pick<SessionStatus.Interface, "get">
      sessions: Pick<Session.Interface, "messages" | "removeMessage">
  ...
  export const askPermission = Effect.fn("NeoSessionPrompt.askPermission")(function* (input: {
  ...
  export function memoryToolEnabled(input: { ctx: MemoryPaths.Ctx }) {
    return NeoToolRegistry.memoryToolsEnabled({ ctx: input.ctx })
  }
  ...
  export const memoryInject = Effect.fn("NeoSessionPrompt.memoryInject")(function* (input: {
    ctx: MemoryPaths.Ctx
    sessionID: SessionID
    record: boolean
    ...
    const verbose =
      input.cache.verbose ??
      (enabled
        ? yield* Effect.tryPromise(() => NeoMemory.status({ ctx: input.ctx })).pipe(
    ...
    const built =
      cached?.enabled === enabled
        ? cached
        : yield* NeocodeSystemPrompt.memoryBlocks({
            ctx: input.ctx,
            sessionID: input.sessionID,
            record: input.record,
            enabled,
   ...
   * OneDrive ReparsePoint directories in some Node versions (neocode#9755).
   */
  ...
  export function resolveCloseReason(input: {
    sessionID: string
    closeReasons: Map<string, NeoSession.CloseReason>
    exit: Exit.Exit<any, any>
  }): NeoSession.CloseReason {
  ...
  export function trimBeforeLastSummary(msgs: MessageV2.WithParts[]): MessageV2.WithParts[] {
    const summary = msgs.reduce<{ msg: MessageV2.WithParts; index: number } | undefined>((latest, msg, index) => {
      ...
      if (!latest || NeoSessionMessageOrder.compare(msg, latest.msg, index, latest.index) > 0) return { msg, index }
      return latest
    }, undefined)
