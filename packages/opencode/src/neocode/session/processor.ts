...
import { Telemetry, type ReviewCommand } from "@neocode/neo-telemetry"
import { SessionNetwork } from "@/session/network"
import type { SessionID } from "@/session/schema"
import type { SessionStatus } from "@/session/status"
import { MessageV2 } from "@/session/message-v2"
import { isRecord } from "@/util/record"
import { parseReviewCommand, reviewCommandName } from "@/neocode/review/command"
...
export namespace NeoSessionProcessor {
  const log = Log.create({ service: "session.processor.neo" })
  ...
  export const PROVIDER_FINISH_ERROR_MESSAGE =
    "The provider ended the response with an error before returning details. Start a new message to retry; Neo will compact the oversized conversation first if needed."
  ...
  /**
   * Returns the Neo-specific retry policy options (limit + offline handler).
  ...
  export function retryOpts(input: {
    ...
    const limit = Flag.NEO_SESSION_RETRY_LIMIT
    return {
