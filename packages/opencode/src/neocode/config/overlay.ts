...
import { NeocodeConfigSources } from "./sources"

export namespace NeocodeConfigOverlay {
  const log = Log.create({ service: "neocode.config.overlay" })
  ...
  export const Result = z.object({
    ...
    sources: z.array(NeocodeConfigSources.Source),
    targets: z.object({
  ...
  export type Input = {
    ...
    global: Config.Info
    sources: NeocodeConfigSources.Source[]
  }

  const files = ["neo.jsonc", "neo.json", "opencode.jsonc", "opencode.json"] as const
  const dirs = [".neocode", ".neo"] as const
  ...
  const fieldPaths = [
    ...
    ["indexing", "neo", "apiKey"],
    ["indexing", "neo", "baseUrl"],
    ["indexing", "neo", "organizationId"],
  ...
  export async function project(input: { directory: string; worktree?: string }): Promise<Config.Info> {
    ...
    // neocode_change - project config is untrusted; confine {file:} reads to the project root
    ...
    return configs.reduce((result, cfg) => NeocodeConfig.mergeConfig(result, cfg), {} as Config.Info)
  }
  ...
  export async function projectTarget(input: { directory: string; worktree?: string }) {
    ...
    return candidates.find((file) => existsSync(file)) ?? path.join(input.directory, ".neo", "neo.jsonc")
  }
  ...
  export async function resolve(input: Input): Promise<Result> {
    // neocode_change start - project agents untrusted, {file:} confined to the project root; global agents trusted
    const root = input.worktree && input.worktree !== "/" ? input.worktree : input.directory
  ...
  function globalDirs() {
    return [Global.Path.config, path.join(Global.Path.home, ".neocode"), path.join(Global.Path.home, ".neo")]
  }

  // neocode_change start - root confines untrusted agent {file:} reads
  async function withAgents(input: Config.Info, dirs: string[], trusted: boolean, root?: string): Promise<Config.Info> {
    ...
    const next = NeocodeConfig.mergeConfig(NeocodeConfig.mergeConfig(input, { agent }), { agent: mode })
  ...
  async function load(file: string, fileScope?: ConfigVariable.FileScope): Promise<Config.Info> {
    // neocode_change start - a single unsafe/invalid project config file must not break the settings overlay;
  ...
  async function loadUnsafe(file: string, fileScope?: ConfigVariable.FileScope): Promise<Config.Info> {
    ...
    // neocode_change - overlay reads project config files: {env:} rejected, {file:} confined to fileScope.root
