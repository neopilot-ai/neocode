...
import { NeocodeConfig } from "./config"

export namespace NeocodeConfigSources {
  ...
  const roots = [".neocode", ".neo"] as const
  const global = ["config.json", "neo.json", "neo.jsonc", "opencode.json", "opencode.jsonc"] as const

  export async function list(input: Input): Promise<Result> {
    const project = Flag.NEO_DISABLE_PROJECT_CONFIG ? [] : await projectSources(input)
    const dirs = Flag.NEO_DISABLE_PROJECT_CONFIG ? [] : await configDirSources(input)
    const sources = [
  ...
  async function envFileSources(): Promise<Pending[]> {
    if (!Flag.NEO_CONFIG) return []
    return [
      await fileSource({
        kind: "env-file",
        scope: "env",
        label: "NEO_CONFIG",
        file: Flag.NEO_CONFIG,
        reason: "Explicit config file from NEO_CONFIG.",
      }),
    ]
  }

  async function projectSources(input: Input): Promise<Pending[]> {
    const neo = await projectFiles("neo", input)
    const opencode = await projectFiles("opencode", input)
    return Promise.all(
      [...neo, ...opencode].map((file) =>
  ...
  async function configDirSources(input: Input): Promise<Pending[]> {
    ...
    const env = Flag.NEO_CONFIG_DIR ? [Flag.NEO_CONFIG_DIR] : []
    const dirs = unique([Global.Path.config, ...project, ...home, ...env]).filter((dir) =>
      NeocodeConfig.isConfigDir(dir, Flag.NEO_CONFIG_DIR),
    )
    ...
    for (const dir of dirs) {
      ...
      for (const name of NeocodeConfig.ALL_CONFIG_FILES) {
  ...
  function dirScope(dir: string, input: { project: string[]; home: string[] }): Scope {
    if (dir === Flag.NEO_CONFIG_DIR) return "env"
  ...
  function envContentSources(): Pending[] {
    ...
    if (process.env.NEO_CONFIG_CONTENT) {
      sources.push({
        kind: "env-content",
        scope: "env",
        label: "NEO_CONFIG_CONTENT",
        source: "NEO_CONFIG_CONTENT",
        exists: true,
        editable: false,
    ...
    if (Flag.NEO_CONFIG_DIR) {
      sources.push({
        kind: "runtime-env",
        scope: "env",
        label: "NEO_CONFIG_DIR",
        source: "NEO_CONFIG_DIR",
        path: Flag.NEO_CONFIG_DIR,
        exists: true,
        editable: false,
    ...
    if (Flag.NEO_DISABLE_PROJECT_CONFIG) {
      sources.push({
        kind: "runtime-env",
        scope: "env",
        label: "NEO_DISABLE_PROJECT_CONFIG",
        source: "NEO_DISABLE_PROJECT_CONFIG",
        exists: true,
        editable: false,
    ...
    return [
      {
        kind: "cloud-org",
        scope: "cloud",
        label: "Neo Cloud organization config",
        source: `${account.url}/api/config`,
        exists: true,
        editable: false,
        reason: "Active organization config is managed by Neo Cloud; values are not exposed here.",
      },
    ]
  }
  ...
  async function managedSources(): Promise<Pending[]> {
    ...
    const files = await Promise.all(
      NeocodeConfig.ALL_CONFIG_FILES.map((name) =>
        fileSource({
          kind: "managed-file",
          scope: "managed",
          label: `Managed ${name}`,
  ...
  function runtimeSources(): Pending[] {
    return [
      runtimeSource("NEO_PERMISSION", Flag.NEO_PERMISSION, "Runtime permission overlay."),
      runtimeSource("NEO_DISABLE_AUTOCOMPACT", process.env.NEO_DISABLE_AUTOCOMPACT, "Disables automatic compaction."),
      runtimeSource("NEO_DISABLE_PRUNE", process.env.NEO_DISABLE_PRUNE, "Disables tool-output pruning."),
