...
import { fetchOrganizationModes } from "@neocode/neo-gateway"
...
export namespace NeocodeConfig {
  const log = Log.create({ service: "neocode.config" })
  ...
  /** Neo-specific config file names (highest-to-lowest precedence within neo). */
  export const NEO_CONFIG_FILES = ["neo.jsonc", "neo.json"] as const

  /** All config file names in precedence order (neo + opencode). */
  export const ALL_CONFIG_FILES = ["neo.jsonc", "neo.json", "opencode.jsonc", "opencode.json"] as const

  /** Config directory suffixes in update-target preference order. */
  export const NEO_DIR_SUFFIXES = [".neo", ".neocode"] as const

  /** Path patterns for resolving neo agent names from file paths. */
  export const AGENT_PATTERNS = ["/.neo/agent/", "/.neo/agents/", "/.neocode/agent/", "/.neocode/agents/"] as const

  /** Path patterns for resolving neo command names from file paths. */
   ...
   *
   * This mirrors the Neo project-config load chain: prefer existing config files
   * in ancestor config directories, then existing root config files, and create
   * `.neo/neo.jsonc` when no project config exists yet.
   */
  export const projectConfigUpdateTarget = Effect.fn("NeocodeConfig.projectConfigUpdateTarget")(function* (input: {
    fs: FSUtil.Interface
    directory: string
    worktree?: string
  }) {
    const dirs = yield* input.fs
      .up({ targets: [...NEO_DIR_SUFFIXES], start: input.directory, stop: input.worktree })
      .pipe(Effect.orDie)
    ...
    return files.find((file) => existsSync(file)) ?? path.join(input.directory, ".neo", "neo.jsonc")
  })

  export const updateProjectConfig = Effect.fn("NeocodeConfig.updateProjectConfig")(function* (input: {
    fs: FSUtil.Interface
    directory: string
    worktree?: string
    config: Config.Info
  ...
  /**
   * Load all Neocode legacy configs (modes, workflows, rules, MCP, ignore).
  ...
  export async function loadLegacyConfigs(input: {
    ...
    try {
      ...
      if (Object.keys(migration.agents).length > 0) {
        ...
        log.debug("loaded neocode custom modes", {
          count: Object.keys(migration.agents).length,
          modes: Object.keys(migration.agents),
        })
      }
      for (const skipped of migration.skipped) {
        log.debug("skipped neocode mode", { slug: skipped.slug, reason: skipped.reason })
      }
    } catch (err) {
      log.warn("failed to load neocode modes", { error: err })
    }

    // Load Neocode workflows as commands
    try {
      ...
      if (Object.keys(migration.commands).length > 0) {
        ...
        log.debug("loaded neocode workflows as commands", {
          count: Object.keys(migration.commands).length,
          commands: Object.keys(migration.commands),
        })
      }
    } catch (err) {
      log.warn("failed to load neocode workflows", { error: err })
    }
    ...
    try {
      ...
      if (migration.instructions.length > 0) {
        ...
        log.debug("loaded neocode rules", {
          count: migration.instructions.length,
          files: migration.instructions,
        })
      }
      for (const warning of migration.warnings) {
        log.debug("neocode rules warning", { warning })
      }
    } catch (err) {
      log.warn("failed to load neocode rules", { error: err })
    }

    // Load Neocode MCP servers (skip global VSCode extension paths unless running in an editor or Console daemon)
    const skipGlobal = process.env["NEO_PLATFORM"] !== "vscode" && process.env["NEOCODE_FEATURE"] !== "daemon"
    const mcp = await McpMigrator.loadMcpConfig(input.projectDir, skipGlobal)
    if (Object.keys(mcp).length > 0) {
      result = input.merge(result, { mcp })
    }

    // Load .neocodeignore patterns
    try {
      const permission = await IgnoreMigrator.loadIgnoreConfig(input.projectDir)
      if (Object.keys(permission).length > 0) {
        result = input.merge(result, { permission })
        log.debug("loaded neocode ignore patterns", {
          hasRead: !!(permission as Record<string, unknown>).read,
          hasEdit: !!(permission as Record<string, unknown>).edit,
        })
      }
    } catch (err) {
      log.warn("failed to load neocode ignore patterns", { error: err })
    }
  ...
  /**
   * Load organization custom modes from the Neo Cloud API.
  ...
  export async function loadOrganizationModes(
    ...
    try {
      const neo = auth["neo"]
      if (neo?.type === "oauth" && neo.access && neo.accountId) {
        const modes = await fetchOrganizationModes(neo.access, neo.accountId)
        if (modes.length > 0) {
  ...
  const GLOBAL_CONFIG_FILES = ["config.json", "neo.json", "neo.jsonc", "opencode.json", "opencode.jsonc"]
  ...
  export function isConfigDir(dir: string, flagDir?: string): boolean {
    return dir.endsWith(".neo") || dir.endsWith(".neocode") || dir === flagDir
  }

  // ── Opencode config migration notice ─────────────────────────────────

  /** Client-neutral docs page describing where Neo reads configuration from. */
  export const CONFIG_DOCS_URL = "https://neo.khulnasoft.com/docs/getting-started/settings"

  /** Stable id for the synthetic "move your opencode config" notification (used for client-side dismissal). */
  export const OPENCODE_NOTIFICATION_ID = "neo.local.opencode-config-detected"

  /**
   * Detect leftover opencode config directories. Neo used to fall back to
  ...
  export function detectOpencodeConfig(input: { directory: string; worktree?: string; scanProject: boolean }): string[] {
    ...
    // Global opencode config dir (sibling of the neo global config dir, e.g. ~/.config/opencode).
  ...
  export function opencodeConfigNotification(input: { directory: string; worktree?: string; scanProject: boolean }) {
    ...
    return {
      ...
      message:
        `Neo no longer falls back to opencode configuration. ` +
        `Found opencode config at ${found[0]}${suffix}. ` +
        `Move it into a .neo directory (project) or ${Global.Path.config} (global).`,
