# Neo CLI Configuration Reference

All config lives in `neo.json` (or `neo.jsonc`). Precedence low-to-high: remote well-known, global (`~/.config/neo/neo.json`), env `NEO_CONFIG`, project `./neo.json`, `.neo/neo.json`, `NEO_CONFIG_CONTENT`, managed (see Config File Locations). Deep-merged; later wins.

This also covers where Neo looks for config files, commands, agents, and skills across project, global, and legacy paths such as `.neo/`, `.neocode/`, and `~/.config/neo/`, plus Agent Manager setup/run scripts in the VS Code extension.

## Commands (`.neo/command/*.md`)

Markdown files with YAML frontmatter. The filename (minus `.md`) becomes the command name invoked via `/name`. Commands can live in `.neo/`, legacy `.neocode/`, and global config roots, with both `command/` and `commands/` directory names supported. See Config File Locations for the full search order.
...
2. `~/.neo/`
3. `~/.neocode/`
4. The `NEO_CONFIG_DIR` directory (if the env var is set)
5. project `.neo/` and `.neocode/` directories from the current working directory up to the worktree root
...
Also loaded from legacy `.neocode/` directories and plural `agents/` variants.
...
Markdown files in `.neo/workflows/` or `.neocode/workflows/` (project-level) and `~/.neo/workflows/` or `~/.neocode/workflows/` (global). These are automatically converted to commands at startup. The filename (minus `.md`) becomes the command name. Project workflows override global ones with the same name.

## Agent Manager Setup And Run Scripts

For the full product guidance, use the canonical [Agent Manager reference](https://neo.khulnasoft.com/docs/automate/agent-manager) and [Agent Manager Workflows guide](https://neo.khulnasoft.com/docs/automate/agent-manager-workflows). Prefer these links instead of guessing documentation paths.

Agent Manager setup/run scripts are project files in the main repository's `.neo/` directory. They are not `neo.json` settings and should not be configured inside generated `.neo/worktrees/<name>/` checkouts.

Agent Manager worktrees usually live under `.neo/worktrees/`. Think of each worktree as a separate checkout on its own branch: it enables parallel edits, but dependencies, build output, caches, databases, and generated files can consume significant disk space across many worktrees.
...
|---|---|
| macOS / Linux | `.neo/setup-script`, `.neo/setup-script.sh` |
| Windows | `.neo/setup-script.ps1`, `.neo/setup-script.cmd`, `.neo/setup-script.bat` |
...
- For port conflicts (`EADDRINUSE`, browser/tests hitting the wrong worktree), inspect the app's dev-server config as well as `.neo/run-script`. If fixing it requires application changes, ask whether the user wants the app made configurable or only wants a run-script workaround.
...
Tool permissions: `read`, `edit`, `glob`, `grep`, `list`, `bash`, `task`, `webfetch`, `websearch`, `semantic_search`, `neo_memory_save`, `neo_memory_recall`, `lsp`, `skill`, `external_directory`, `todowrite`, `todoread`, `question`, `doom_loop`.
...
For example, this configuration will hide all models from the built-in Neo Gateway as well as any from the OpenAI provider which may be enabled automatically through environment variables.

```jsonc
{
  "$schema": "https://app.neo.khulnasoft.com/config.json",
  "disabled_providers": ["neo", "openai"],
}
```

The provider ID is the lowercase name used in the `provider/model` format (e.g., `neo`, `openai`, `anthropic`, `google`, `groq`).
...
Skills are markdown files at `skills/<name>/SKILL.md` (or `skill/<name>/SKILL.md`) with `name` and `description` in frontmatter. Discovered inside `.neo/` and legacy `.neocode/` directories.
...
|---|---|---|---|
| Switch theme | `<leader>t` | `/themes` | Pick from 35+ built-in themes (neo, catppuccin, dracula, github, gruvbox, nord, tokyonight, etc.) |
| Toggle appearance (dark/light) | — | — | Ctrl+P → "Toggle appearance" |

Custom themes: place JSON files in `~/.config/neo/themes/` or `.neo/themes/`.
...
Notification settings are managed through `neo console` under **Settings > CLI > Notifications**, or through `attention` in `tui.json` / `tui.jsonc`. There is no notification slash command or command-palette toggle.
...
### Config files (neo.json)

| Scope | Path |
|---|---|
| Project | `./neo.json`, `./neo.jsonc`, `./opencode.json` (legacy), `./opencode.jsonc` (legacy) |
| Global | `~/.config/neo/neo.json`, `~/.config/neo/neo.jsonc`, `~/.config/neo/opencode.json` (legacy), `~/.config/neo/opencode.jsonc` (legacy), `~/.config/neo/config.json` (legacy) |
| Managed | Linux: `/etc/neo/`, macOS: `/Library/Application Support/neo/`, Windows: `%ProgramData%\neo\` — loads `neo.json`, `neo.jsonc`, `opencode.json`, `opencode.jsonc` (enterprise, highest priority) |

Each config directory (`.neo/` and legacy `.neocode/`) can also contain `neo.json`, `neo.jsonc`, `opencode.json`, or `opencode.jsonc`.

### Config directories

Two directory names are scanned: `.neo` (canonical) and `.neocode` (legacy fallback). Both are checked at each level, and `.neo` wins when both define the same entry. `.opencode` directories are not loaded.

- **Project**: walks up from CWD to the git worktree root, checking both directories at each level
- **Home**: `~/.neo/` and `~/.neocode/`
- **XDG global**: `~/.config/neo/` (always loaded, lowest file-based precedence)
...
Example: `~/.config/neo/command/*.md` (global), `~/.neocode/command/*.md` (legacy home), and `.neo/commands/*.md` (project) all load commands.
...
|---|---|
| `NEO_CONFIG` | Path to an additional config file (loaded after global) |
| `NEO_CONFIG_DIR` | Path to an additional config directory (appended to search list) |
| `NEO_CONFIG_CONTENT` | Inline JSON config string (high precedence, after project dirs) |
| `NEO_DISABLE_PROJECT_CONFIG` | Skip all project-level config (files and directories) |
