import type { TuiPlugin, TuiPluginApi, TuiPluginModule } from "@neocode/plugin/tui"
import { createMemo, createSignal, onCleanup, onMount, Show } from "solid-js"
import { Global } from "@opencode-ai/core/global"
import * as Log from "@opencode-ai/core/util/log"
import type { NeoPassState } from "@neocode/neo-gateway"
import type { Message } from "@neocode/sdk/v2"
import { onBalanceRefresh } from "../balance-refresh"

const id = "internal:neo-sidebar-footer"
...
export function passLine(pass: NeoPassState) {
...
// A billed turn: a completed Neo assistant message with a non-zero cost.
export function billable(info: Message) {
  if (info.role !== "assistant") return false
  return info.providerID === "neo" && info.time.completed !== undefined && info.cost > 0
}
              ...
              {(pass) => (
                <box gap={0}>
                  <box flexDirection="row" justifyContent="space-between" gap={1}>
                    <text fg={theme().textMuted}>{" └ Neo Pass"}</text>
      ...
      <Show when={show()}>
        ...
        >
          ...
          <box flexGrow={1} gap={1}>
            ...
            <text fg={theme().textMuted}>Neo includes free models so you can start immediately.</text>
