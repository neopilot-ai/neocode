...
import { enabled as sandboxed } from "@neocode/sandbox"
...
export const Params = Schema.Struct({
  ...
  persistent: Schema.optional(Schema.Boolean).annotate({
    description: "Keep the process running and manageable after the session or Neo exits",
  }),
}).check(
  Schema.makeFilter(
    (params: {
  ...
  Effect.succeed({
    ...
    execute: (params, ctx) =>
      Effect.gen(function* () {
        ...
        const parent = params.inherit ? NeoSession.resolveParent(ctx.sessionID) : undefined
