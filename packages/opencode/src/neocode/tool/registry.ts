...
import { AgentManager, HostError } from "@/neocode/agent-manager/service"
...
const log = Log.create({ service: "neocode-tool-registry" })
type Deps = { agent: Agent.Interface; truncate: Truncate.Interface; indexing?: boolean }
type Loaders = {
  indexing?: () => Promise<{ NeoIndexing: { ready: () => boolean } }>
  semantic?: () => Promise<Pick<typeof import("@/neocode/tool/semantic-search"), "SemanticSearchTool">>
}

export namespace NeoToolRegistry {
  const hint =
  ...
  export function usePatch(input: { modelID: string; family?: string }) {
    if (process.env["NEO_E2E_LLM_URL"]) return true
  ...
  /** Resolve Neo-specific tool Infos outside any InstanceState, so their Truncate/Agent deps are
  ...
  /** Finalize Neo-specific tools into Tool.Defs. Call this inside the InstanceState state Effect —
  ...
  function semanticTool(deps: Deps, loaders: Loaders) {
    return Effect.gen(function* () {
      const ready = yield* deps.indexing === undefined
        ? (() => {
            const indexing = loaders.indexing ?? (() => import("@/neocode/indexing"))
            return Effect.tryPromise(() => indexing().then((mod) => mod.NeoIndexing.ready())).pipe(
              Effect.catch((err) =>
                Effect.sync(() => {
      ...
      const semantic = loaders.semantic ?? (() => import("@/neocode/tool/semantic-search"))
  ...
  /** Neo-specific tools to append to the builtin list */
  export function extra(
    ...
    return [
      ...
      tools.save,
      tools.recall,
      ...(Flag.NEO_CLIENT === "cli" || Flag.NEO_CLIENT === "vscode" ? [tools.process] : []),
      ...(Flag.NEO_CLIENT === "cli" && tools.terminal ? [tools.terminal] : []),
      // Agent Manager tools are useful only when the extension can create and display their sessions.
      ...(Flag.NEO_CLIENT === "vscode" ? [tools.managerModels, tools.manager] : []),
      ...(Flag.NEO_CLIENT === "vscode" &&
  ...
  /** Per-turn cache of `NeoMemory.toolEnabled` keyed by root string, with a short TTL so the
   * step-loop coalesces probes inside a single turn. Cache is invalidated immediately on enable /
   * disable / purge / rebuild via the MemoryEvents bus (subscribed in neocode/bootstrap.ts). */
  export function memoryToolsEnabled(input: { ctx: MemoryPaths.Ctx }) {
    return Effect.gen(function* () {
      ...
      const enabled = yield* Effect.tryPromise({
        try: () => NeoMemory.toolEnabled({ ctx: input.ctx }),
        catch: (err) => err,
      }).pipe(
        Effect.catch((err) =>
          Effect.sync(() => {
  ...
  /** Hide Neo memory tools from the model when project memory is disabled. */
  export const applyVisibility = Effect.fn("NeoToolRegistry.applyVisibility")(function* (tools: Tool.Def[]) {
    const ctx = yield* InstanceState.context
    const memoryEnabled = yield* memoryToolsEnabled({ ctx })
    return tools.filter((tool) => {
      if (tool.id.startsWith("neo_memory_")) return memoryEnabled
      return true
    })
  })
