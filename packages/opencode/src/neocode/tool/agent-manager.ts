...
import { AgentManagerEvent, type AgentManagerTask } from "@/neocode/agent-manager/event"
import { AgentManager, HostError } from "@/neocode/agent-manager/service"
import type { Result } from "@/neocode/agent-manager/protocol"
...
// Prefer the provider the user already uses for the invoking turn, then the Neo Gateway,
    ...
    return {
      ...
      execute: (params, ctx) =>
        Effect.gen(function* () {
          ...
          const msg = NeoSessionMessageOrder.latest(ctx.messages).user
