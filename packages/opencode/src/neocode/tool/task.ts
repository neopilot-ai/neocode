...
const log = Log.create({ service: "neocode-task-model" })
...
export namespace NeoTask {
  ...
  /** Neo keeps delegation one level deep to avoid recursive subagent chains. */
   ...
   * Neo intentionally differs: parent denials remain hard ceilings for Plan Mode
  ...
  const saved = Effect.fn("NeoTask.savedModel")(function* (name: string) {
    if (Flag.NEO_CLIENT !== "cli") return undefined
  ...
  export const resolveModel = Effect.fn("NeoTask.resolveModel")(function* (input: {
    name: string
