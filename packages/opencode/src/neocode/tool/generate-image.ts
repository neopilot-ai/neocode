...
import { NEO_OPENROUTER_BASE } from "@neocode/neo-gateway"
import DESCRIPTION from "./generate-image.txt"

const log = Log.create({ service: "tool.generate_image" })

const NEO_OPENROUTER_URL = `${NEO_OPENROUTER_BASE}/chat/completions`
...
function buildRequest(resolved: ResolvedProvider, prompt: string, model: string, inputImage?: string): ResolvedRequest {
  ...
  if (resolved.organizationId) headers["X-NEOCODE-ORGANIZATIONID"] = resolved.organizationId
    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const auth = yield* authSvc.get("neo")
          ...
          if (!resolved) {
            return {
              ...
              output:
                "No image generation provider available. Log in to Neo or set OPENROUTER_API_KEY, then try again.",
