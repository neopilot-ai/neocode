...
import { ProvidersCommand } from "../cli/cmd/providers" // neocode_change — upstream renamed auth → providers
...
import { NeoConsoleCommand } from "./cli/cmd/console"
...
// from `neo help --all` and the docs table. They're also guarded the same way
// at the yargs registration site in src/index.ts, so the commands-in-sync
// regex in test/neocode/help.test.ts sees DevSetup/DevAlias on neither side.
...
export const commands = [
  ...
  GenerateCommand,
  DebugCommand,
  ProvidersCommand, // neocode_change — upstream renamed AuthCommand → ProvidersCommand
  AgentCommand,
  UpgradeCommand,
  UninstallCommand,
  ServeCommand,
