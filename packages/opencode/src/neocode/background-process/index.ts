...
import { Instance, type InstanceContext } from "@/neocode/instance"
...
export namespace BackgroundProcess {
  ...
  class StateService extends Context.Service<StateService, { readonly get: () => Effect.Effect<State> }>()(
    "@neocode/BackgroundProcess.State",
  ) {}
  ...
  async function claim(shared: Shared) {
    ...
    const pending = Flock.acquire(shared.key, {
      ...
      .catch((err) => {
        log.debug("persistent process scope is managed by another Neo process", { err, scope: shared.key })
        return false
      })
      .finally(() => {
  ...
  function infer() {
    return Flag.NEO_CLIENT === "cli" && process.env.NEO_BACKGROUND_PROCESS_PORTS === "true"
  }
  ...
  function env(id?: ID, token?: string) {
    const result: NodeJS.ProcessEnv = {
      ...process.env,
      TERM: "dumb",
      ...(id ? { NEO_BACKGROUND_PROCESS_ID: id } : {}),
      ...(token ? { NEO_BACKGROUND_PROCESS_TOKEN: token } : {}),
    }
    delete result.NEO_SERVER_PASSWORD
    delete result.NEO_SERVER_USERNAME
    delete result.NEO_BACKGROUND_PROCESS_PORTS
    return result
  }
  ...
  async function linux(active: Active): Promise<Probe> {
    ...
    if (leader && pgrp(leader) === pid) {
      ...
      if (data?.toString("utf8").split("\0").includes(`NEO_BACKGROUND_PROCESS_TOKEN=${token}`)) return "owned"
    }
  ...
  async function launch(state: State, input: StartInput, id = ID.ascending()) {
    ...
    if (logpath) {
      ...
      if (!(await claim(state.shared)))
        throw new Error("Persistent processes for this project are managed by another Neo process")
  ...
  NeoShutdown.register(() => runtime.dispose())
