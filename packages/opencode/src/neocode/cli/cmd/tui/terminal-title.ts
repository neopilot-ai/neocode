import { NeoTitleIcon } from "./title-icon"
...
export namespace NeoTerminalTitle {
  ...
  } satisfies Record<NeoTitleIcon.Value, Record<Indicator, string>>

  export function format(input: { base: string; title?: string; indicator: Indicator; icon?: NeoTitleIcon.Value }) {
    const text = input.title ? `${input.base} | ${truncate(input.title)}` : input.base
    const prefix = icons[input.icon ?? NeoTitleIcon.Default][input.indicator]
