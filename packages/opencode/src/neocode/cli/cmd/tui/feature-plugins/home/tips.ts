...
export const NEO_TIPS: Tip[] = [
  ...
  "Create {highlight}neo.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/neo/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.neo/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.neo/agent/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.neo/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.neo/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Neo from reading sensitive files",
  "Use {highlight}neo run{/highlight} for non-interactive scripting",
  "Use {highlight}neo --continue{/highlight} to resume the last session",
  "Use {highlight}neo run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}neo serve{/highlight} for headless API access to Neo",
  "Use {highlight}neo run --attach{/highlight} to connect to a running server",
  "Run {highlight}neo upgrade{/highlight} to update to the latest version",
  "Run {highlight}neo auth list{/highlight} to see all configured providers",
  "Run {highlight}neo agent create{/highlight} for guided agent creation",
  "Run {highlight}neo github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/neo fix this{/highlight} on issues to auto-create PRs",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.neo/themes/{/highlight} directory",
  ...
  "Run {highlight}neo debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/neopilot-ai/neocode{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with Neo Gateway for curated, tested models",
