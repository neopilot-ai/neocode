...
import { importCloudSession } from "@/neocode/cloud-session"
...
export namespace NeoTuiThreadDaemon {
  ...
  export function workerAuth() {
    const password = Flag.NEO_SERVER_PASSWORD ?? randomUUID()
    const username = Flag.NEO_SERVER_USERNAME ?? "neo"
    return {
      env: { NEO_SERVER_USERNAME: username, NEO_SERVER_PASSWORD: password },
