/**
 * Neo-specific overrides for the provider dialog.
 *
...
import type { ProviderAuthAuthorization } from "@neocode/sdk/v2"
import { NeoAutoMethod } from "@/neocode/components/dialog-neo-auto-method"
export { selectProvider } from "@/neocode/anaconda-desktop/tui/setup"
...
/**
 * If the provider is Neo Gateway, renders the custom `NeoAutoMethod`
...
export function renderAutoMethod(opts: {
  ...
  if (opts.providerID !== "neo") return undefined
  return () => (
    <NeoAutoMethod
 ...
 * provider is Neo Gateway. Returns `undefined` otherwise.
 */
export function renderApiDescription(
  ...
  if (providerID !== "neo") return undefined
  return () => (
    <box gap={1}>
      <text fg={theme.textMuted}>
        Neo Gateway gives you access to all the best coding models at the cheapest prices with a single API key.
      </text>
      <text fg={theme.text}>
        Go to <span style={{ fg: theme.primary }}>https://neo.khulnasoft.com/gateway</span> to get a key
      </text>
    </box>
  )
}
