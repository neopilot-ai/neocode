 ...
 * changed from the Neo Console. Fetched config stays serializable and is resolved into a fresh
  ...
  export function makeStore(initial: TuiConfig.Resolved) {
    ...
    const set: SetTuiConfig = (next) => {
      ...
      const config: TuiConfig.Resolved = {
        ...next,
        title_icon: next.title_icon ?? NeoTitleIcon.Default,
        attention: {
          ...
          sound_pack: next.attention?.sound_pack ?? "neo.default",
