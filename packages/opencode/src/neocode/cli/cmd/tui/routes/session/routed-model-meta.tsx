...
import type { AssistantMessage, Part, Provider, StepFinishPart } from "@neocode/sdk/v2"
...
export namespace RoutedModelMeta {
  ...
  export function label(list: Providers, model: StepFinishPart["model"]) {
    ...
    const id = NeoRoutedModel.display(model.modelID)
    ...
    return NeoRoutedModel.displayName(text)
  }

  function routed(model: StepFinishPart["model"], message: Message) {
    if (!model) return undefined
    if (message.providerID !== "neo") return undefined
    if (!message.modelID.startsWith("neo-auto/")) return undefined
