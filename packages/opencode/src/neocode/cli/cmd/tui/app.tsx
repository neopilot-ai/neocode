/**
 * Neo-specific TUI app customizations.
 *
...
import { isNeoError, showNeoErrorToast } from "@/neocode/neo-errors"
import { registerNeoCommands } from "@/neocode/neo-commands"
import { initializeTUIDependencies } from "@neocode/neo-gateway/tui"
import { DialogProcessList } from "@/neocode/cli/cmd/tui/component/dialog-process-list"
import { useIndexingWarnings } from "@/neocode/cli/cmd/tui/indexing-warning"
import { NeoTerminalTitle } from "./terminal-title"
import type { NeoTitleIcon } from "./title-icon"
import { Session as SessionApi } from "@/session/session"

// Re-export so upstream can render the route without importing directly
export { NeoClawView } from "@/neocode/claw/view"
export { NeoTerminalTitle } from "./terminal-title"

// Hot reload TUI-local settings (keybinds/theme/ui) when changed from the Neo Console.
// Called from the App body (below SDKProvider and the TuiConfig provider).
export { useTuiConfigHotReload } from "@/neocode/cli/cmd/tui/context/tui-config-hot-reload"
export { NeoTuiConfig } from "@/neocode/cli/cmd/tui/context/tui-config"
...
export const APP_TITLE = "Neo CLI"

/** Public docs URL shown in the command palette. */
export const DOCS_URL = "https://neo.khulnasoft.com/docs"
...
export function useSessionEffects(deps: {
  ...
  const pty = process.env.NEO_PTY_ID
...
export function getTerminalTitle(input: {
  ...
  icon?: NeoTitleIcon.Value
}): NeoTerminalTitle.Result | undefined {
  if (input.route.data.type === "home") {
    return {
      title: NeoTerminalTitle.format({ base: input.base, indicator: "none", icon: input.icon }),
      active: false,
      indicator: "none",
    }
  }
  ...
  if (input.route.data.type === "session") {
    const state = NeoTerminalTitle.session({
      base: input.base,
    ...
    return {
      ...state,
      title: NeoTerminalTitle.format({ base: input.base, title, indicator: state.indicator, icon: input.icon }),
    }
  }

  if (input.route.data.type === "plugin") {
    return {
      title: NeoTerminalTitle.format({
        base: input.base,
  ...
  if (input.route.data.type === "neoclaw") {
    return {
      title: NeoTerminalTitle.format({ base: input.base, title: "NeoClaw", indicator: "none", icon: input.icon }),
      active: false,
      indicator: "none",
    }
  }
...
/**
 * Intercepts Neo-specific errors and shows a warning toast.
 * Returns `true` if the error was handled, `false` otherwise.
 */
export function handleSessionError(error: unknown, toast: ReturnType<typeof useToast>): boolean {
  if (error && typeof error === "object" && isNeoError(error as any)) {
    showNeoErrorToast(error as any, toast)
    return true
  }
  return false
}
 ...
 *
 * - Injects TUI dependencies into neo-gateway
 * - Registers Neo Gateway commands (profile, teams, neoclaw, etc.)
...
export function init() {
  ...
  // Inject TUI dependencies for neo-gateway
  ...
  // Register Neo Gateway commands (profile, teams, neoclaw, remote, etc.)
  registerNeoCommands(useSDK)
