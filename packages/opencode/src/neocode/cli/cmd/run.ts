import { createNeoClient, type NeoClient } from "@neocode/sdk/v2"
import { UI } from "@/cli/ui"
import { DaemonClient } from "@/neocode/daemon/client"
import { isBuiltinCommand, type BuiltinCommand } from "@/neocode/session/builtin-commands"
import { Provider } from "@/provider/provider"
import { Filesystem } from "@/util/filesystem"

export namespace NeoRun {
  export async function resolveBuiltin(sdk: NeoClient, command?: string, directory?: string) {
...
export namespace NeoRunDaemon {
  export type Input = {
    directory?: string
    execute: (client: NeoClient) => Promise<void>
  }
  ...
  export async function attach(input: Input) {
    ...
    const client = createNeoClient({ baseUrl: daemon.url, directory: dir, headers: daemon.headers })
