...
import { serverUrls } from "@/neocode/cli/server-urls"
import { AppRuntime } from "@/effect/app-runtime"
import { Daemon } from "@/neocode/daemon/daemon"
import { warnPort } from "@/neocode/cli/port-warning"
import { hasDisplay } from "@/neocode/cli/cmd/tui/util/display"
import { StopCommand } from "@/neocode/cli/cmd/daemon"
...
const OpenCommand = cmd({
  command: "$0",
  describe: "open the local Neo Console",
  builder: (yargs) =>
  ...
  handler: async (args) => {
    const run = async (signal?: AbortSignal) => {
      ...
      if (!state) throw new Error("Neo daemon did not provide connection state")
      if (signal?.aborted) return state
      if (daemon.restarted) console.warn("Restarted the Neo daemon to apply the requested network options")
      ...
      if (hasDisplay()) {
        ...
        console.warn("No display detected; open the Neo Console URL manually")
      }
      console.log("Neo Console:")
    ...
    await Daemon.foreground(async (signal) => {
      ...
      if (!signal.aborted) console.log("Press Ctrl+C to stop the Neo daemon.")
      return state
    })
  },
})

export const NeoConsoleCommand = cmd({
  command: "console",
  describe: "open or stop the local Neo Console",
