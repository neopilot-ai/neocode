...
import { fetchBalance, fetchProfile, type NeocodeBalance, type NeocodeProfile } from "@neocode/neo-gateway"
...
interface Args {
  ...
  getProfile?: (token: string) => Promise<NeocodeProfile>
  getBalance?: (token: string, organizationId?: string) => Promise<NeocodeBalance | null>
...
export const ProfileCommand = cmd({
  command: "profile",
  describe: "show Neo account profile",
  builder: (yargs: Argv) =>
    yargs.option("json", {
