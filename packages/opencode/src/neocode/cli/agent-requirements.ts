import type { AgentRequirementResult } from "@neocode/sdk/v2"
...
export async function check(input: { client: Client; agent: string; directory: string }): Promise<Check> {
  const response = await input.client.neocode.agentRequirements(
...
export function format(input: ErrorData | RequirementError) {
  ...
  if (info.vscode_extensions.length) {
    lines.push(
      ...
      "This agent requires VS Code extensions and is not supported in this CLI environment. Use the Neo VS Code extension instead.",
    )
  }
