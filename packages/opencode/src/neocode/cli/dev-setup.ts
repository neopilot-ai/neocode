...
const MARKER_START = "# >>> neodev launcher >>>"
const MARKER_END = "# <<< neodev launcher <<<"

export const DevSetupCommand = cmd({
  command: "dev-setup",
  describe: "install a `neodev` shell alias for this checkout",
  builder: (y) =>
    y
      .option("shell", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    UI.empty()
    UI.println(`  ${B}Neo CLI dev launcher setup${N}`)
    UI.empty()
    ...
    UI.println(
      `  ${action === "install" ? "This will add" : "This will replace"} one line so ${H}neodev${N} runs this checkout from any directory:`,
    )
    UI.empty()
    UI.println(`      ${snippet}`)
    UI.empty()
...
// Deprecated: kept so `neo dev-alias` keeps working for scripts/docs.
...
function aliasLine(shell: Shell, repo: string): string {
  const sh = path.join(repo, "bin", "neodev")
  const bat = path.join(repo, "bin", "neodev.cmd")
  const q = (s: string) => `'${s.replaceAll("'", `'\\''`)}'`
  if (shell === "fish") return `alias neodev ${q(sh)}`
  if (shell === "powershell") return `function neodev { & '${bat.replaceAll("'", "''")}' @args }`
  return `alias neodev=${q(sh)}`
}
...
async function applyBlock(rc: string, snippet: string): Promise<{ created: boolean; backup?: string }> {
  ...
  const backup = `${rc}.neodev.bak.${stamp()}`
...
export async function detectRepo(): Promise<string> {
  const hint = process.env.NEO_DEV_REPO
  if (hint) return hint
  ...
  // For a local build it's <repo>/packages/opencode/dist/<target>/bin/neo, so
  ...
  throw new Error(
    "cannot locate neocode source checkout; set NEO_DEV_REPO=/path/to/neocode or run ./bin/neodev dev-setup from the repo",
  )
}
