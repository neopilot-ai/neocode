...
import { context as instanceContext, type InstanceContext } from "./instance-context" // neocode_change
  ...
  Effect.gen(function* () {
    ...
    const boot = (input: LoadInput & { directory: string }) =>
      Effect.gen(function* () {
        ...
        // neocode_change start - run bootstrap inside the Instance ALS so NeocodeBootstrap
    ...
    const disposeContext = Effect.fn("InstanceStore.disposeContext")(function* (ctx: InstanceContext) {
      ...
      yield* Effect.promise(() => instanceContext.provide(ctx, () => runDisposers(ctx.directory))) // neocode_change
      yield* emitDisposed({ directory: ctx.directory, project: ctx.project.id })
    })

    const disposeEntry = Effect.fnUntraced(function* (directory: string, entry: Entry, ctx: InstanceContext) {
      if (cache.get(directory) !== entry) return false
      // neocode_change start - remove disposed entries even when event publication fails
      ...
      return Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            if (previous) {
              // neocode_change start - dispose reloads under the previous instance context
    ...
    const disposeAllOnce = Effect.fnUntraced(function* () {
      ...
      // neocode_change start - dispose independent worktrees concurrently without interrupting siblings
