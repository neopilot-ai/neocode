...
import * as NeoOAuthCallback from "../neocode/mcp-oauth-callback" // neocode_change

const log = Log.create({ service: "mcp.oauth-callback" }) // neocode_change
...
<head>
  ...
  <title>Neo - Authorization Successful</title>
  <!-- neocode_change end -->
  <style>
...
<head>
  <!-- neocode_change start -->
  <title>Neo - Authorization Failed</title>
  <!-- neocode_change end -->
  <style>
    body { font-family: system-ui, -apple-system, sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #1a1a2e; color: #eee; }
    .container { text-align: center; padding: 2rem; }
...
export async function ensureRunning(redirectUri?: string): Promise<void> {
  // neocode_change start - delegate Neo-specific callback binding from here because OAuth state lives in this module
  await NeoOAuthCallback.ensureRunning({
    redirectUri,
  ...
  await new Promise<void>((resolve, reject) => {
    // neocode_change start - EADDRINUSE can still fire when another process
    ...
    server!.listen(currentPort, () => {
      server!.off("error", onError) // neocode_change
