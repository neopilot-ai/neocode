...
if (process.platform === "win32" && !("type" in process)) {
  Object.defineProperty(process, "type", { value: "neo-bun", configurable: true })
}
// neocode_change end
...
import * as SandboxNetwork from "@/neocode/sandbox/network" // neocode_change
import { McpCatalog } from "./catalog"

const DEFAULT_TIMEOUT = 30_000

// neocode_change start - inject --rm for Docker containers to prevent stopped container accumulation
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const connectTransport = (transport: Transport, timeout: number) =>
      Effect.acquireUseRelease(
        ...
        (t) =>
          Effect.tryPromise({
            try: () => {
              const client = new Client({ name: "neo", version: InstallationVersion }) // neocode_change
            ...
            if (isAuthError) {
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: neo mcp auth ${key}`, // neocode_change
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      const finalArgs = ensureDockerRm(cmd, args) // neocode_change
      const baseDir = yield* InstanceState.directory
      const cwd = mcp.cwd ? path.resolve(baseDir, mcp.cwd) : baseDir
      const bridge = yield* EffectBridge.make() // neocode_change - drain child stderr without writing over the TUI
      const transport = new StdioClientTransport({
        stderr: "pipe",
        command: cmd,
        args: finalArgs, // neocode_change
        cwd,
        env: {
          ...process.env,
      ...
      // neocode_change start - a piped stderr stream must be consumed or verbose MCP servers can block
    ...
    const tools = Effect.fn("MCP.tools")(function* () {
      ...
      for (const [clientName, client] of Object.entries(s.clients)) {
        ...
        for (const mcpTool of listed) {
          ...
          // neocode_change start - remote MCP calls must use the sandbox network authority
    ...
    function collectFromConnected<T extends { name: string }>(
      ...
      return Effect.gen(function* () {
        ...
        return yield* Effect.forEach(
          ...
          ([clientName, client]) =>
            McpCatalog.collect(
              // neocode_change - distinguish collection from direct network fetch
              clientName,
              client,
    ...
    // neocode_change start - `opts?: { callback?: boolean }` parameter is Neo-specific
    const startAuth = Effect.fn("MCP.startAuth")(function* (mcpName: string, opts?: { callback?: boolean }) {
      ...
      // neocode_change start - authenticate() defers binding the callback port until a redirect is needed
    ...
    const authenticate = Effect.fn("MCP.authenticate")(function* (mcpName: string) {
      const result = yield* startAuth(mcpName, { callback: false }) // neocode_change
      ...
      // neocode_change start - bind only after redirect exists, and clean up if binding fails
      const mcpConfig = yield* getMcpConfig(mcpName)
