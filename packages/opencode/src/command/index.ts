...
import { legacyReviewCommand, reviewCommand } from "@/neocode/review/command" // neocode_change
...
// neocode_change start - skills can share names with slash commands
  ...
  Effect.gen(function* () {
    ...
    const init = Effect.fn("Command.state")(function* (ctx: InstanceContext) {
      ...
      for (const item of yield* skill.all()) {
        ...
        commands[item.name] = fromSkill(item) // neocode_change
      }
    ...
    const get = Effect.fn("Command.get")(function* (name: string) {
      ...
      const exact = s.commands[name] // neocode_change
      if (exact) return exact // neocode_change
      const alias = legacyReviewCommand(name) // neocode_change
      if (alias) return alias // neocode_change
      ...
      // neocode_change end
      return undefined // neocode_change
    })
