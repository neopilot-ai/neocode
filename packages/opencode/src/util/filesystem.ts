import { chmod, mkdir, readFile, rename, stat as statFile, writeFile } from "fs/promises" // neocode_change
import { createWriteStream, existsSync, statSync } from "fs"
import { realpathSync } from "fs"
// neocode_change start - harden containment checks
...
export function stat(p: string): ReturnType<typeof statSync> | undefined {
  // neocode_change start - also treat ENOTDIR/EACCES as absent, every caller expects undefined
  try {
...
export async function write(p: string, content: string | Buffer | Uint8Array, mode?: number): Promise<void> {
  // neocode_change start - atomic write via temp-file + rename to avoid partial reads on concurrent saves
