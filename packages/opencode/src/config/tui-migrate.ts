...
import { access, constants } from "fs/promises" // neocode_change
...
const TUI_SCHEMA_URL = "https://app.neo.khulnasoft.com/tui.json" // neocode_change
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  // neocode_change start: use neo directory everywhere
  const project = Flag.NEO_DISABLE_PROJECT_CONFIG
    ? []
    : await Filesystem.findUp(["neo.json", "neo.jsonc"], input.cwd, undefined, { rootFirst: true })
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "neo")]
  // neocode_change end
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "neo"))
  }
  if (Flag.NEO_CONFIG) files.push(Flag.NEO_CONFIG)
