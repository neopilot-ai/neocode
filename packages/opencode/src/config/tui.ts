...
import { NeocodeDefaultPlugins } from "@/neocode/config/default-plugins" // neocode_change
...
export interface Interface {
  ...
  readonly info: () => Effect.Effect<Info> // neocode_change - editable config for Neo console
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // neocode_change start - trusted gates {env:}; fileScope confines untrusted {file:} reads
  const load = (
    text: string,
    ...
    // neocode_change end
    Effect.gen(function* () {
      // neocode_change start - only trusted tui config resolves {env:}; untrusted {file:} confined to fileScope
  ...
  // neocode_change start - trusted + fileScope threaded to load
  const loadFile = (filepath: string, trusted: boolean, fileScope?: ConfigVariable.FileScope): Effect.Effect<Info> =>
    ...
    Effect.gen(function* () {
      ...
      return yield* load(text, filepath, trusted, fileScope) // neocode_change
    })

  // neocode_change start - trusted + fileScope threaded to loadFile
  const mergeFile = (acc: Acc, file: string, trusted: boolean, fileScope?: ConfigVariable.FileScope) =>
    // neocode_change end
    Effect.gen(function* () {
      const data = yield* loadFile(file, trusted, fileScope) // neocode_change
  ...
  // neocode_change start - discover canonical and legacy Neo config directories
  // Every config dir we may read from: global config, .neo and legacy .neocode
  // folders between cwd and home, and NEO_CONFIG_DIR.
  // neocode_change end
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.NEO_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  for (const file of ConfigPaths.fileInDirectory(Global.Path.config, "tui")) {
    yield* mergeFile(acc, file, true) // neocode_change - global config is trusted
  }

  // 2. Explicit NEO_TUI_CONFIG override, if set.
  if (Flag.NEO_TUI_CONFIG) {
    const configFile = Flag.NEO_TUI_CONFIG
    yield* mergeFile(acc, configFile, true) // neocode_change - explicit env-provided path is trusted
    yield* Effect.logDebug("loaded custom tui config", { path: configFile })
  }

  // 3. Project tui files, applied root-first so the closest file wins.
  for (const file of projectFiles) {
    yield* mergeFile(acc, file, false, { root: ctx.directory, source: file }) // neocode_change - untrusted, {file:} confined to project
  }

  // neocode_change start - load tui.json from supported Neo config directories
  // 4. `.neo` and legacy `.neocode` directories (and NEO_CONFIG_DIR)
  ...
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".neo") || dir.endsWith(".neocode") || dir === Flag.NEO_CONFIG_DIR,
  )
  // neocode_change end

  for (const dir of dirs) {
    // neocode_change start - trust global (home/NEO_CONFIG_DIR) dirs like config.ts; in-repo .neo/.neocode stay untrusted
  ...
  if (acc.result.attention?.sound_pack === undefined) result.attention.sound_pack = "neo.default" // neocode_change - preserve Neo's default sound pack

  // neocode_change start - inject Neo default plugins to keep TUI aligned with server config
  const defaults = NeocodeDefaultPlugins.apply(
    { plugin: result.plugin ? [...result.plugin] : undefined, plugin_origins: acc.plugin_origins },
    { disabled: Flag.NEO_DISABLE_DEFAULT_PLUGINS },
  )
  ...
  return {
    config,
    info: { ...acc.result, plugin: defaults.plugin }, // neocode_change - include applied Neo defaults
    pluginOrigins: defaults.plugin_origins ?? [], // neocode_change - exclude builtins from dependency installation
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const info = Effect.fn("TuiConfig.info")(() => Effect.succeed(data.info)) // neocode_change
    const pluginOrigins = Effect.fn("TuiConfig.pluginOrigins")(() => Effect.succeed(data.pluginOrigins))

    const waitForDependencies = Effect.fn("TuiConfig.waitForDependencies")(() =>
      Effect.forEach(deps, Fiber.join, { concurrency: "unbounded" }).pipe(Effect.ignore(), Effect.asVoid),
    )
    return Service.of({ get, info, pluginOrigins, waitForDependencies }) // neocode_change
