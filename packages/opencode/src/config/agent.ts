...
import { ConfigVariable } from "./variable" // neocode_change
...
import { report } from "@/neocode/config/report"
import type { Warning } from "./config"
// neocode_change end

const log = Log.create({ service: "config" })

// neocode_change start - trusted gates {env:}; fileScope confines untrusted agent prompt {file:} reads
export async function load(
  ...
  for (const item of await Glob.scan("{agent,agents}/**/*.md", {
    ...
    const md = await ConfigMarkdown.parse(item, { trusted, fileScope, sourceScope }).catch(async (err) => {
      ...
      try {
        const { capture } = await import("@/neocode/instance")
        const ctx = capture()
    ...
    // neocode_change start - substitute agent prompt variables relative to the agent file. Project agents are
    ...
    // neocode_change end
    // neocode_change start - use Effect schema (propertyOrder: original) + non-fatal handleInvalid
    try {
      result[config.name] = ConfigParse.schema(ConfigAgentV1.Info, config, item)
    } catch (err) {
      if (ConfigError.InvalidError.isInstance(err)) {
        await NeocodeConfig.handleInvalid("agent", item, err.data.issues ?? [], err, warnings)
        continue
      }
      throw err
    }
