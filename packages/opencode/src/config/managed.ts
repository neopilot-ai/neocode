...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/neo" // neocode_change
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "neo") // neocode_change
    default:
      return "/etc/neo" // neocode_change
  }
}

export function managedConfigDir() {
  return process.env.NEO_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
