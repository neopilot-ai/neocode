...
import { ConfigVariableGuard } from "@/neocode/config/variable" // neocode_change
...
type SubstituteInput = ParseSource & {
  text: string
  missing?: "error" | "empty"
  escapeJson?: boolean // neocode_change
  // neocode_change start - trust gates {env:}; untrusted project config may only read files inside fileScope.root
  trusted?: boolean
...
// neocode_change start - a token is inert when its line is commented out with //
...
export async function substitute(input: SubstituteInput) {
  ...
  const escape = input.escapeJson ?? true // neocode_change
  // neocode_change start - untrusted (project) config cannot read environment variables. {env:} has no safe
  ...
  let text = input.text.replace(/\{env:([^}]+)\}/g, (match, varName, offset: number) => {
    // neocode_change start - leave commented tokens literal; reject server credentials
  ...
  for (const match of fileMatches) {
    ...
    // neocode_change start - skip tokens on commented-out lines
    ...
    // neocode_change start - validate and read one opened file to prevent credential substitution races;
    ...
    const fileContent = (
      await ConfigVariableGuard.read(resolvedPath, input.fileScope && { ...input.fileScope, token }).catch(
        (error: NodeJS.ErrnoException) => {
          // neocode_change - a deliberate scope block must always reject; only genuine missing/IO errors are
    ...
    out += escape ? JSON.stringify(fileContent).slice(1, -1) : fileContent // neocode_change
    cursor = index + token.length
  }
