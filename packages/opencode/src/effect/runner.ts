...
import { NeoRunner } from "@/neocode/effect/runner" // neocode_change
...
export const make = <A, E = never>(
  ...
  // neocode_change start - do not let work publish busy before the Running state is committed
  const startRun = (work: Effect.Effect<A, E>, done: Deferred.Deferred<A, E | Cancelled>) => {
    const id = next()
    return NeoRunner.start({
      work,
      scope,
  ...
  // neocode_change start - open work only after the Running state is committed
    ...
    SynchronizedRef.modifyEffect(
      ...
      Effect.fnUntraced(function* (st) {
        ...
        if (st._tag === "ShellThenRun" && st.shell.id === id) {
          return yield* NeoRunner.commit(startRun(st.run.work, st.run.done), Effect.void)
        }
      ...
      Effect.fnUntraced(function* (st) {
        switch (st._tag) {
          ...
          case "Idle": {
            ...
            return yield* NeoRunner.commit(startRun(work, done), awaitDone(done))
          }
        }
      }),
    ).pipe(Effect.flatten)
