...
import { context as instanceContext } from "@/project/instance-context" // neocode_change
...
export function attach<A, E, R>(effect: Effect.Effect<A, E, R>): Effect.Effect<A, E, R> {
  ...
  // neocode_change start - bridge legacy AsyncLocalStorage instance context into Effect runtimes
...
export function makeRuntime<I, S, E>(service: Context.Service<I, S>, layer: Layer.Layer<I, E>) {
  ...
  return {
    ...
    // neocode_change start - allow Neo-owned service runtimes to release persistent resources
    dispose: async () => {
      const current = rt
      rt = undefined
