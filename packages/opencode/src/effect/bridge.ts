...
import { Instance, type InstanceContext } from "@/neocode/instance" // neocode_change
...
// neocode_change start - preserve legacy Neo contexts across Promise callbacks
 ...
 * legacy AsyncLocalStorage contexts for callback code that still reads them. // neocode_change
 *
 * Mirrors `Effect.promise` but restores Neo compatibility contexts first. // neocode_change
 */
  ...
  return Effect.gen(function* () {
    ...
    return {
      promise: <A, E, R>(effect: Effect.Effect<A, E, R>) =>
        restore(instance, workspace, () => Effect.runPromise(wrap(effect))), // neocode_change
      fork: <A, E, R>(effect: Effect.Effect<A, E, R>) =>
        restore(instance, workspace, () => Effect.runFork(wrap(effect))), // neocode_change
      ...
      bind:
        ...
        (...args: Args) =>
          restore(instance, workspace, () => Effect.runSync(wrap(Effect.sync(() => fn(...args))))), // neocode_change
    } satisfies Shape
  })
}
