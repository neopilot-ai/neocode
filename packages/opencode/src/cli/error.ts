...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed.` // neocode_change
  }
  ...
  // ProviderModelNotFoundError: { providerID: string, modelID: string, suggestions?: string[], modelsEmpty?: boolean } // neocode_change
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      ...(providerModelNotFound.modelsEmpty === true ? ["No models are currently available."] : []), // neocode_change
      `Try: \`neo models\` to list available models`, // neocode_change
      `Or check your neo.json provider/model names`, // neocode_change
    ].join("\n")
  }
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`neo auth login ${url}\` to re-authenticate.`] : []), // neocode_change
    ].join("\n")
  }
