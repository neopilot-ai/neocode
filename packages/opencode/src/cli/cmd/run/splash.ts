...
//
// Renders the full Neo entry logo and a compact [O] exit badge, plus // neocode_change
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    push(lines, body_left, top, "Neo", right, undefined, TextAttributes.BOLD) // neocode_change
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `neo run -i -s ${meta.session_id}`, // neocode_change
      right,
      undefined,
      TextAttributes.BOLD,
    )
