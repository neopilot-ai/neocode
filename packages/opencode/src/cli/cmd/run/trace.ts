...
//
// Enable with NEO_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/neo/log/direct/<timestamp>-<pid>.jsonl. Also writes // neocode_change
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.NEO_DIRECT_TRACE) {
    state = false
    return undefined
  }
