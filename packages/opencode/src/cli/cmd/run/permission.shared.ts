...
export function permissionAlwaysLines(request: PermissionRequest): string[] {
  if (request.always.length === 1 && request.always[0] === "*") {
    return [`This will allow ${request.permission} until Neo is restarted.`] // neocode_change
  }

  return [
    "This will allow the following patterns until Neo is restarted.", // neocode_change
