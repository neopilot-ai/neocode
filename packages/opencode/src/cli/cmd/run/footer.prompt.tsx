...
import { slashDisplay, slashMatches } from "@/neocode/cli/cmd/command-display" // neocode_change
...
import { NEO_BASE_MODE, useBindings } from "@opencode-ai/tui/keymap"
...
export function createPromptState(input: PromptInput): PromptState {
  ...
  const slashOptions = createMemo<SlashOption[]>(() => {
    const builtins = [
      ...
      { kind: "slash", name: "exit", display: "/exit", description: "close direct mode" } satisfies SlashOption, // neocode_change
    ]
    ...
    return [
      ...(showSkillMenu
        ...
        : []),
      ...(input.commands() ?? [])
        .filter((item) => !hidden.has(item.name)) // neocode_change - suggest skills as slash commands
        .map(
          (item) =>
            ({
              kind: "slash",
              name: item.name,
              display: slashDisplay(item), // neocode_change
              description: item.description,
            }) satisfies SlashOption,
        ),
      ...builtins,
  ...
  const select = (item?: PromptOption) => {
    ...
    if (next.kind === "slash") {
      ...
      const text = `${next.display}${separator}` // neocode_change
