...
import { RUN_INTERACTIVE_TERMINAL_ROWS } from "@/neocode/cli/cmd/run/interactive-terminal" // neocode_change
...
type RunFooterOptions = {
  ...
  onTerminalWrite: (input: { terminalID: string; data: string }) => Promise<void> // neocode_change
  onTerminalResize: (input: { terminalID: string; cols: number; rows: number }) => Promise<void> // neocode_change
  onTerminalClose: (terminalID: string) => Promise<void> // neocode_change
