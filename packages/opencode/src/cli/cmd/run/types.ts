...
import type { NeoClient, PermissionRequest, QuestionRequest, ToolPart } from "@neocode/sdk/v2"
import type { RunInteractiveTerminalSnapshot } from "@/neocode/cli/cmd/run/types" // neocode_change
...
type PromptModel = Parameters<NeoClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<NeoClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<NeoClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<NeoClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<NeoClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<NeoClient["experimental"]["resource"]["list"]>>["data"]>
...
export type FooterView =
  ...
  | { type: "interactive_terminal"; terminal: RunInteractiveTerminalSnapshot } // neocode_change
...
export type PermissionReply = Parameters<NeoClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<NeoClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<NeoClient["question"]["reject"]>[0]
