...
const FilesCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.debug.rg.files")(function* (args) {
    ...
    const files = yield* ripgrep
      ...
      .pipe(Effect.orDie)
    process.stdout.write(files.items.map((file) => file.path).join(EOL) + EOL) // neocode_change
  }),
})
...
const SearchCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.debug.rg.search")(function* (args) {
    ...
    const results = yield* ripgrep
      ...
      .pipe(Effect.orDie)
    process.stdout.write(JSON.stringify(results.items, null, 2) + EOL) // neocode_change - preserve debug output shape
  }),
})
