...
import type { Hooks } from "@neocode/plugin"
import { Process } from "@/util/process"
import { errorMessage } from "@/util/error"
import { text } from "node:stream/consumers"
import { Effect, Option } from "effect"
import { remove as removeAuth } from "@/neocode/auth/remove" // neocode_change
...
export const ProvidersCommand = cmd({
  // neocode_change start - keep "auth" as primary command name
  command: "auth",
  aliases: ["providers"],
  // neocode_change end
...
export const ProvidersLoginCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (args.provider) {
      ...
      // neocode_change start - accept codex as an alias for OpenAI ChatGPT auth
    ...
    if (provider === "other") {
      ...
      yield* Prompt.log.warn(
        `This only stores a credential for ${provider} - you will need configure it in neo.json, check the docs for examples.`, // neocode_change
      )
    }
    ...
    if (provider === "amazon-bedrock") {
      yield* Prompt.log.info(
        "Amazon Bedrock authentication priority:\n" +
          ...
          "Configure via neo.json options (profile, region, endpoint) or\n" + // neocode_change
    ...
    if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
      yield* Prompt.log.info(
        "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://neo.khulnasoft.com/docs/ai-providers/cloudflare", // neocode_change
      )
    }
...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    yield* removeAuth(provider) // neocode_change
