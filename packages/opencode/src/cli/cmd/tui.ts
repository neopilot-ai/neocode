...
import type { GlobalEvent } from "@neocode/sdk/v2"
import type { EventSource } from "@opencode-ai/tui/context/sdk"
import { importCloudSession, localSessionID, validateCloudFork } from "@/neocode/cloud-session" // neocode_change
import { createNeoClient } from "@neocode/sdk/v2" // neocode_change
import { writeHeapSnapshot } from "v8"
import { NeoTuiThreadDaemon, type StartInput } from "@/neocode/cli/cmd/tui/thread" // neocode_change
import { win32InstallCtrlCGuard } from "@opencode-ai/tui/terminal-win32"
import { validateSession } from "../tui/validate-session"
// neocode_change start - correlate the TUI worker with its parent process
import {
  NEO_PROCESS_ROLE,
  NEO_RUN_ID,
  ensureRunID,
...
// neocode_change end
import { createParentRemoteExitBridge, type RemoteExitBridgeClient } from "@/neocode/cli/cmd/tui/remote-exit-bridge" // neocode_change
import type { Exit } from "@opencode-ai/tui/context/exit" // neocode_change

declare global {
  const NEO_WORKER_PATH: string
}

type RpcClient = ReturnType<typeof Rpc.client<typeof rpc>>

// neocode_change start - bridge remote exit only for the embedded worker transport
...
// neocode_change start - share the extracted TUI runner between daemon and worker paths
...
async function target() {
  if (typeof NEO_WORKER_PATH !== "undefined") return NEO_WORKER_PATH
...
export function resolveThreadDirectory(project?: string, envPWD = process.env.PWD, cwd = process.cwd()) {
  // neocode_change start - ignore stale PWD from wrappers such as `bun --cwd`, except neo-dev's caller cwd
  const dev = process.env.NEO_DEV_CWD
  ...
  return dev ? root : real // neocode_change
}

export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start neo tui", // neocode_change
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start neo in", // neocode_change
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      // neocode_change start - default TUI sessions attach to the daemon unless explicitly disabled
      if (await NeoTuiThreadDaemon.attach({ args, cwd, input: () => input(args.prompt), start })) return
      // neocode_change end
      const auth = NeoTuiThreadDaemon.workerAuth() // neocode_change - protect TUI-owned HTTP routes from unauthenticated local callers
      // neocode_change start - propagate stable run metadata and an explicit worker role
      const env = sanitizedProcessEnv({
        [NEO_PROCESS_ROLE]: "worker",
        [NEO_RUN_ID]: ensureRunID(),
        ...auth.env,
        NEO_BACKGROUND_PROCESS_PORTS: "true",
      })
      // neocode_change end
      const worker = new Worker(file, {
        preload: ["@opentui/solid/preload"], // neocode_change - Bun workers do not inherit the parent preload
        env, // neocode_change
      })
      worker.onerror = (e) => {
      ...
      // neocode_change start - graceful shutdown on external signals
      ...
      const transport = external
        ? {
            ...
            fetch: undefined,
            headers: auth.headers, // neocode_change
            events: undefined,
          }
        : {
            url: "http://neo.internal",
      ...
      try {
        ...
        try {
          await validateSession({
            url: transport.url, // neocode_change
            sessionID: localSessionID(args), // neocode_change
            directory: cwd,
            fetch: transport.fetch,
            headers: transport.headers, // neocode_change
          })
        ...
        await start(
          {
            // neocode_change - shared lazy loader also supports daemon attach
            url: transport.url,
            async onSnapshot() {
