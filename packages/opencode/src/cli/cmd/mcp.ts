...
import { NeocodeMcpConfig } from "@/neocode/cli/cmd/mcp" // neocode_change
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: neo mcp add") // neocode_change
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in neo.json:") // neocode_change
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // neocode_change start - prefer supported Neo config directories over root files
  const roots = [
    path.join(baseDir, "neo.jsonc"),
    path.join(baseDir, "neo.json"),
  ...
  const candidates = global
    ? roots
    : [
        path.join(baseDir, ".neo", "neo.jsonc"),
        path.join(baseDir, ".neo", "neo.json"),
        path.join(baseDir, ".neo", "opencode.jsonc"),
        path.join(baseDir, ".neo", "opencode.json"),
        path.join(baseDir, ".neocode", "neo.jsonc"),
        path.join(baseDir, ".neocode", "neo.json"),
        path.join(baseDir, ".neocode", "opencode.jsonc"),
        path.join(baseDir, ".neocode", "opencode.json"),
        ...roots,
      ]
  ...
  // Default to neo.json if none exist
  return path.join(baseDir, "neo.json")
  // neocode_change end
}
...
async function addMcpToConfig(name: string, mcpConfig: ConfigMCPV1.Info, configPath: string) {
  ...
  const result = NeocodeMcpConfig.format(configPath, applyEdits(text, edits)) // neocode_change
...
export const McpAddCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.add")(function* (args) {
    ...
    const ctx = maybeCtx
    const global = Flag.NEO_CONFIG_DIR ?? Global.Path.config // neocode_change - honor the active Neo config profile
    yield* Effect.promise(async () => {
      ...
      if (args.name) {
        ...
        const configPath = await resolveConfigPath(global, true) // neocode_change
      ...
      const [projectConfigPath, globalConfigPath] = await Promise.all([
        ...
        resolveConfigPath(global, true), // neocode_change
      ])
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., neo x @modelcontextprotocol/server-filesystem", // neocode_change
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "neo-debug", version: InstallationVersion }, // neocode_change
            },
            id: 1,
          }),
        })
