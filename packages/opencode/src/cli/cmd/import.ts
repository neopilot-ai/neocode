import type { Session as SDKSession, Message, Part } from "@neocode/sdk/v2"
...
const log = Log.create({ service: "import" }) // neocode_change
...
/** Extract share ID from a Neo share URL like https://app.neo.khulnasoft.com/s/abc123 */
export function parseShareUrl(url: string): string | null {
  const match = url.match(/^https?:\/\/app\.neo\.ai\/s\/([a-zA-Z0-9_-]+)$/)
...
async function ingestBootstrap(sessionId: string) {
  ...
  return NeoSessions.bootstrap(sessionId)
}
...
const runImport = Effect.fn("Cli.import.body")(function* (file: string, ctx: InstanceContext) {
  ...
  if (isUrl) {
    // neocode_change start - Migrate to upstream ShareNext architecture #10281
    const slug = parseShareUrl(file)
    if (!slug) {
      process.stdout.write(`Invalid URL format. Expected: https://app.neo.khulnasoft.com/s/<id>`)
