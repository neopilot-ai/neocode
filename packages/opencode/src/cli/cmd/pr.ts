...
import { existsSync } from "node:fs" // neocode_change

const subcommand = "pr" // neocode_change

// neocode_change start - resolve the currently running CLI instead of hardcoding opencode
export function cliCommand(
  ...
  if (script === subcommand) return [input.execPath] // neocode_change
...
export const PrCommand = effectCmd({
  command: `${subcommand} <number>`, // neocode_change
  describe: "fetch and checkout a GitHub PR branch, then run neo", // neocode_change
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    const cli = cliCommand() // neocode_change
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        const sessionMatch = prInfo.body.match(/https:\/\/app\.neo\.ai\/s\/([a-zA-Z0-9_-]+)/) // neocode_change
        if (sessionMatch) {
    ...
    UI.println()
    UI.println("Starting neo...") // neocode_change
    UI.println()

    const run = sessionId ? [...cli, "-s", sessionId] : cli // neocode_change
    ...
    if (code !== 0) return yield* Effect.die(new Error(`neo exited with code ${code}`)) // neocode_change
  }),
})
