...
import { GitHubSecurity } from "@/neocode/security/github" // neocode_change
...
const AGENT_USERNAME = "neoconnect[bot]" // neocode_change
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/neo.yml" // neocode_change
  ...
  yield* Effect.promise(async () => {
    {
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            step2,
            "",
            "    3. Go to a GitHub issue and comment `/neo summarize` to see the agent in action", // neocode_change
            "",
            "   Learn more about the GitHub agent - https://neo.khulnasoft.com/docs/code-with-ai/platforms/github", // neocode_change
          ].join("\n"),
        )
      }
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/neoconnect" // neocode_change
        const command =
        ...
        async function getInstallation() {
          // neocode_change start - updated to new endpoint
          return await fetch(`https://api.neo.khulnasoft.com/api/integrations/github/check-installation?owner=${app.owner}`)
            .then((res) => res.json())
      ...
      async function addWorkflowFiles() {
        // neocode_change start - updated workflow template with Neo branding and gateway secrets
        ...
        const envStr = providerEnvStr || neoGatewayEnv ? `\n        env:${providerEnvStr}${neoGatewayEnv}` : ""
...
jobs:
  neo:
    if: |
      ...
      startsWith(github.event.comment.body, '/neo')
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.neo.khulnasoft.com" : "https://neo.khulnasoft.com" // neocode_change
    ...
    try {
      ...
      // Setup neo session // neocode_change
      ...
      console.log("neo session", session.id) // neocode_change
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.neo.khulnasoft.com" // neocode_change
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/neo,/kc") // neocode_change
        .split(",")
      ...
      for (const m of matches) {
        const tag = m[0]
        // neocode_change start - only fetch canonical GitHub attachment routes
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to neo...") // neocode_change
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("neo-github-action") // neocode_change
      } catch (error) {
    ...
    async function exchangeForAppToken(token: string) {
      // neocode_change start - updated endpoint URLs per new API structure
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `neo/${type}-${hex}-${timestamp}` // neocode_change
      }
      return `neo/${type}${issueId}-${timestamp}` // neocode_change
    }
    ...
    function footer(opts?: { image?: boolean }) {
      // neocode_change start - simplified footer with text branding (no image backend yet)
      const share = shareId ? `[neo session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
      return `\n\n---\n*Powered by [Neo](https://neo.khulnasoft.com)*&nbsp;&nbsp;|&nbsp;&nbsp;${share}[github run](${runUrl})`
      // neocode_change end
    }
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the neo infrastructure after your response", // neocode_change
