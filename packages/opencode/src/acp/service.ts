...
import type { Message, NeoClient, SessionMessageResponse } from "@neocode/sdk/v2"
...
export const AuthMethodID = "neo-login" // neocode_change
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `neo auth login` in the terminal",
      name: "Login with Neo",
      id: AuthMethodID,
    }
    // neocode_change end
...
function makeDirectoryService(sdk: NeoClient) {
...
function makeUsageService(sdk: NeoClient) {
...
async function loadDirectorySnapshot(sdk: NeoClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  const neoProvider = providers[ProviderV2.ID.make("neo")] // neocode_change
  const neoModel = neoProvider ? Provider.sort(Object.values(neoProvider.models))[0] : undefined // neocode_change
  if (neoProvider && neoModel) return { providerID: neoProvider.id, modelID: neoModel.id } // neocode_change
