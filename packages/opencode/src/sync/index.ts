...
import { EventSequenceTable, EventTable } from "@opencode-ai/core/event/sql" // neocode_change - upstream moved the event tables to core
...
import * as EventWire from "@/neocode/event-wire" // neocode_change
...
> = {
  ...
  wire?: boolean // neocode_change - EventV2 rows cross persistence and bus boundaries as encoded data
}
...
// neocode_change start - serialized rows carry the schema's encoded representation
...
export const layer = Layer.effect(Service)(
  Effect.gen(function* () {
    ...
    const replay: Interface["replay"] = Effect.fn("SyncEvent.replay")(function* (event, options) {
      ...
      // neocode_change start - decode only EventV2 rows
...
export function init(input: { projectors: Array<[Definition, ProjectorFunc]>; convertEvent?: ConvertEvent }) {
  ...
  for (let entry of EventV2.registry.values()) {
    if (!entry.sync) continue // neocode_change - EventV2 stores legacy sync metadata under sync
    register({
      type: entry.type,
      version: entry.sync.version, // neocode_change
      aggregate: entry.sync.aggregate, // neocode_change
      properties: entry.data,
      schema: entry.data,
      wire: true, // neocode_change
    })
  }
...
function process<Def extends Definition>(
  ...
  Database.transaction((tx) => {
    ...
    const data = def.wire ? EventWire.encode(def.schema, event.data) : event.data // neocode_change
    ...
    if (options.experimentalWorkspaces) {
      ...
      tx.insert(EventTable)
        .values({
          id: EventV2.ID.make(event.id), // neocode_change - core event table uses the branded EventV2 ID
          seq: event.seq,
          aggregate_id: event.aggregateID,
          type: versionedType(def.type, def.version),
          data: data as Record<string, unknown>, // neocode_change
        })
        .run()
    }
    ...
    Database.effect(() => {
      ...
      const publish = (value: unknown) =>
        ...
        options.bridge.fork(
          Effect.gen(function* () {
            // neocode_change start - encode EventV2 properties before crossing the legacy boundary
            if (def.wire) {
...
export function effectPayloads() {
  return [
    ...
    ...EventV2.registry
      ...
      .filter(
        (definition) =>
          definition.sync !== undefined && // neocode_change
          !registry.has(versionedType(definition.type, definition.sync.version)), // neocode_change
      )
      .map((definition) =>
        EffectSchema.Struct({
          type: EffectSchema.Literal("sync"),
          name: EffectSchema.Literal(versionedType(definition.type, definition.sync!.version)), // neocode_change
          id: EffectSchema.String,
          seq: EffectSchema.Finite,
          aggregateID: EffectSchema.Literal(definition.sync!.aggregate), // neocode_change
