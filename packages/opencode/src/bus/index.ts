// neocode_change - Neo compatibility layer. Upstream deleted this Bus (Effect PubSub) service in v1.16.2
// in favour of EventV2; Neo keeps it ONLY for existing Neo-owned callers (neocode/* features) that rely on
// its eager-callback subscription + fork-atomicity semantics. Do NOT add new shared/upstream-shaped consumers.
// Full migration of Neo callers onto core EventV2 is tracked as a dedicated follow-up.
import { Effect, Exit, Fiber, Layer, PubSub, Scope, Context, Stream, Schema } from "effect" // neocode_change
...
import { context as instanceContext, type InstanceContext } from "@/project/instance-context" // neocode_change
import { InstanceRef } from "@/effect/instance-ref"
import { LocalContext } from "@/util/local-context" // neocode_change
import { LayerNode } from "@opencode-ai/core/effect/layer-node" // neocode_change
...
export const node = LayerNode.make(layer, []) // neocode_change

const { runPromise } = makeRuntime(Service, layer) // neocode_change
...
// neocode_change start - legacy callback facade inherits the active instance context
function active() {
