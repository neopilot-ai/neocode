...
import { GlobalBus } from "@/bus/global" // neocode_change - unified channel for legacy Bus + EventV2Bridge emissions
...
import { IngestQueue } from "@/neo-sessions/ingest-queue"
import { clearInFlightCache, withInFlightCache } from "@/neo-sessions/inflight-cache"
import type * as SDK from "@neocode/sdk/v2"
import z from "zod"
import { Context, Effect, Layer, Schema } from "effect"
import { NEO_API_BASE } from "@neocode/neo-gateway"
import { Config } from "@/config/config"
import { InstanceState } from "@/effect/instance-state"
import { Instance } from "@/neocode/instance"
import { Vcs } from "@/project/vcs"
import simpleGit from "simple-git"
import { RemoteWS } from "@/neo-sessions/remote-ws"
import { RemoteSender } from "@/neo-sessions/remote-sender"
import { AttachedState } from "@/neo-sessions/attached-state"
...
import { cumulativeSessionDiff } from "@/neocode/session-portability/cumulative-diff"
import { LayerNode } from "@opencode-ai/core/effect/layer-node"

async function provide<R>(input: { directory: string; fn: () => R }): Promise<R> {
  const { provide } = await import("@/neocode/instance")
  return provide(input)
}

// neocode_change removed: `same` helper is no longer used now that the
// presence/pending set logic lives in AttachedState.

export namespace NeoSessions {
  export const Event = {
    RemoteStatusChanged: BusEvent.define(
      "neo-sessions.remote-status-changed",
      Schema.Struct({
  ...
  export class Service extends Context.Service<Service, Interface>()("@neocode/NeoSessions") {}

  const log = Log.create({ service: "neo-sessions" })
  // neocode_change - narrow `log` to the warn-only shape AttachedState needs.
  ...
  const tokenValidKeyTemplate = "neo-sessions:token-valid:"
  let tokenValidKey = tokenValidKeyTemplate + "unknown"

  const tokenKey = "neo-sessions:token"
  const orgKey = "neo-sessions:org"
  const clientKey = "neo-sessions:client"
  const gitUrlKeyPrefix = "neo-sessions:git-url:"
  ...
  async function authValid(token: string) {
    ...
    return withInFlightCache(tokenValidKey, 15 * 60_000, async () => {
      const response = await fetch(`${NEO_API_BASE}/api/user`, {
        headers: {
  ...
  async function neocodeToken() {
    return withInFlightCache(tokenKey, ttlMs, async () => {
      const auth = await runtime.runPromise((svc) => svc.get("neo"))
      ...
      const key = process.env["NEO_API_KEY"]?.trim()
      if (key) return key
      return undefined
    })
  }
  ...
  async function getClient(): Promise<Client | undefined> {
    return withInFlightCache(clientKey, ttlMs, async () => {
      const token = await neocodeToken()
      if (!token) return undefined

      const valid = await authValid(token)
      if (!valid) return undefined

      const base = process.env["NEO_SESSION_INGEST_URL"] ?? "https://ingest.neosessions.ai"
  ...
  const shareDisabled = process.env["NEO_DISABLE_SHARE"] === "true" || process.env["NEO_DISABLE_SHARE"] === "1"
  const ingestDisabled =
    process.env["NEO_DISABLE_SESSION_INGEST"] === "true" || process.env["NEO_DISABLE_SESSION_INGEST"] === "1"
  const debugIngest =
    process.env["NEO_DEBUG_SESSION_INGEST"] === "true" || process.env["NEO_DEBUG_SESSION_INGEST"] === "1"
  ...
  const remoteEnabled = process.env["NEO_REMOTE"] === "1"
  let remote: { conn: RemoteWS.Connection; sender: RemoteSender.Sender } | undefined
  let enabling: Promise<void> | undefined
  let remoteSeq = 0
  // neocode_change start - separate presence-owned attached session ids from
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const state = yield* InstanceState.make(
        Effect.fn("NeoSessions.state")(function* (ctx) {
          if (ingestDisabled) return

          // neocode_change - register event callbacks into a type→callback dispatch map, drained by a single
          ...
          watch(Session.Event.Updated, async (evt) => {
            ...
            await ingest.sync(sessionID, [
              { type: "neo_meta", data: await meta(sessionID) },
          ...
          // neocode_change - one GlobalBus listener drains the dispatch map. This state is cached per-directory
      ...
      const init = Effect.fn("NeoSessions.init")(function* () {
  ...
  export async function enableRemote() {
    ...
    enabling = (async () => {
      const token = await neocodeToken()
      if (!token) {
        throw new Error("Unable to enable remote: no Neo credentials found. Run `neo auth login`.")
      }

      const valid = await authValid(token)
      if (valid === false) {
        throw new Error("Unable to enable remote: invalid or expired Neo credentials. Run `neo auth login`.")
      }
      if (valid === undefined) throw new Error("Unable to enable remote: failed to verify Neo credentials.")

      const url = (process.env["NEO_SESSION_INGEST_URL"] ?? "https://ingest.neosessions.ai")
        .replace(/^https:\/\//, "wss://")
        .replace(/^http:\/\//, "ws://")
      ...
      const getSessions = async () => {
        ...
        // neocode_change - advertise both presence-owned and pending-created ids
  ...
  export function disableRemote() {
    ...
    enabling = undefined
    // neocode_change - clear both presence and pending-created ids so the
  ...
  export function setAttachedSessions(ids: readonly string[]) {
    // neocode_change - delegate to the two-set state so a concurrent create
    // announcement is not dropped by a presence clear+rebuild.
    attachedState.setPresence(ids)
  }

  // neocode_change start - duplicate-safe single-session attach used by the
  ...
  export async function share(sessionId: string) {
    if (ingestDisabled) {
      throw new Error("Session ingest is disabled (NEO_DISABLE_SESSION_INGEST=1)")
    }

    if (shareDisabled) {
      throw new Error("Sharing is disabled (NEO_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to share session: no Neo credentials found. Run `neo auth login`.")
    }
    ...
    const url = `https://app.neo.khulnasoft.com/s/${result.public_id}`
  ...
  export async function unshare(sessionId: string) {
    ...
    if (shareDisabled) {
      throw new Error("Unshare is disabled (NEO_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to unshare session: no Neo credentials found. Run `neo auth login`.")
    }
  ...
  async function meta(sessionId?: string) {
    const override = sessionId ? NeoSession.resolvePlatform(sessionId) : undefined
    const platform = override || process.env["NEO_PLATFORM"] || "cli"
  ...
  async function getOrgId(): Promise<Uuid | undefined> {
    const env = process.env["NEO_ORG_ID"]
