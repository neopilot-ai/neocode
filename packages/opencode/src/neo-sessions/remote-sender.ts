import { RemoteCommand } from "@/neo-sessions/remote-command"
import { RemoteExit } from "@/neo-sessions/remote-exit"
import { RemoteModelCatalog } from "@/neo-sessions/remote-model-catalog"
import { RemoteProtocol } from "@/neo-sessions/remote-protocol"
import type { RemoteWS } from "@/neo-sessions/remote-ws"
...
type Provide = typeof import("@/neocode/instance").provide
...
// neocode_change start - create_session: strict v1 request, no other fields accepted
...
// neocode_change start - redact anything but the error class so messages/credentials
...
// neocode_change start — lazy init to avoid circular dependency
...
function normalizeModel(model: string | RemoteModelCatalog.ModelRef | undefined) {
  ...
  return {
    providerID: ProviderV2.ID.make("neo"),
    modelID: ModelV2.ID.make(model.startsWith("neocode/") ? model.slice("neocode/".length) : model),
  }
}
...
export namespace RemoteSender {
  export type Options = {
    ...
    session?: {
      ...
      // neocode_change start - injectable create hook for create_session.
      ...
      // neocode_change - injectable remove hook used to roll back an orphan
    ...
    // neocode_change start - duplicate-safe attach hook used by create_session.
    // Production wires this to NeoSessions.attachRemoteSession so the attached
  ...
  export function create(options: Options): Sender {
    ...
    // neocode_change start - orphan rollback for create_session: when
    ...
    // neocode_change end
    // neocode_change start - session create + duplicate-safe attach used by create_session
    const sessionCreate =
      session.create ??
    ...
    const attachSession =
      ...
      (async (id: SessionID) => {
        const { NeoSessions } = await import("@/neo-sessions/neo-sessions")
        await NeoSessions.attachRemoteSession(id)
      })
    // neocode_change end
    // neocode_change start - injectable slash command discovery + execution
    ...
    function dispatch(msg: RemoteProtocol.Command) {
      // neocode_change start - slash command discovery and execution
      ...
      if (msg.command === "create_session") {
        // neocode_change start - remote /new creation: root session, attached + heartbeat before response
