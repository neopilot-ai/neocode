// neocode_change - extracted state machine that separates presence-owned
...
export namespace AttachedState {
  ...
  // neocode_change - collision-safe union key. The historical "|" join
  ...
  export function create(options: Options): Interface {
    ...
    // neocode_change - in-flight dedup. Concurrent announce(id) callers
    ...
    // neocode_change end
    let lastSentKey = ""
    // neocode_change - lifecycle generation. Incremented on reset() so a
    ...
    return {
      ...
      async announce(id) {
        ...
        // neocode_change - join an in-flight Promise for this id instead
        ...
        // neocode_change - capture the lifecycle generation so a late
      ...
      reset() {
        ...
        inflight.clear()
        lastSentKey = ""
        // neocode_change - bump the lifecycle generation so any in-flight
