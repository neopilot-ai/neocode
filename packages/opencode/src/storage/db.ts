...
declare const NEO_MIGRATIONS: { sql: string; timestamp: number; name: string }[] | undefined
...
export function getChannelPath(flags: Pick<DatabaseFlags, "disableChannelDb"> = readRuntimeFlags()) {
  if (["latest", "beta", "prod"].includes(InstallationChannel) || flags.disableChannelDb)
    return path.join(Global.Path.data, "neo.db") // neocode_change
  const safe = InstallationChannel.replace(/[^a-zA-Z0-9._-]/g, "-")
  const next = path.join(Global.Path.data, `neo-${safe}.db`) // neocode_change
  const prev = path.join(Global.Path.data, `opencode-${safe}.db`) // neocode_change
  if (!existsSync(next) && existsSync(prev)) return prev // neocode_change
  return next // neocode_change
}

export const getPath = (flags?: Pick<DatabaseFlags, "disableChannelDb">) => {
  if (Flag.NEO_DB) {
    if (Flag.NEO_DB === ":memory:" || path.isAbsolute(Flag.NEO_DB)) return Flag.NEO_DB
    return path.join(Global.Path.data, Flag.NEO_DB)
  }
...
export const Client = Object.assign(
  (flags: DatabaseFlags = readRuntimeFlags()): Client => {
    ...
    const entries =
      typeof NEO_MIGRATIONS !== "undefined"
        ? NEO_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof NEO_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
