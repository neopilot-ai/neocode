...
import { Struct } from "effect" // neocode_change
...
import { EffectFlock } from "@opencode-ai/core/util/effect-flock" // neocode_change
import { Config } from "@/config/config"
import { Global } from "@opencode-ai/core/global"
// neocode_change start
import { Flag } from "@opencode-ai/core/flag/flag"
import { DiffFull } from "../neocode/snapshot/diff-full"
import { NeoSnapshotTrack } from "../neocode/snapshot/track"
import { NeoSnapshotSeed } from "../neocode/snapshot/seed"
import { NeoSnapshotMaterialize } from "../neocode/snapshot/materialize"
...
}).pipe(withStatics((s) => ({ zod: zod(s) }))) // neocode_change
...
// neocode_change start - lightweight FileDiff without patch for session summaries
...
const prune = "7.days"
const retention = 7 * 24 * 60 * 60 * 1000 // neocode_change
...
export const MAX_DIFF_SIZE = 256 * 1024 // neocode_change
...
export interface Interface {
  ...
  // neocode_change start - pass prompt context and managed initialization policy
  readonly track: (opts?: {
    sessionID?: SessionID
    messageID?: MessageID
    snapshotInitialization?: NeoSnapshotTrack.SnapshotInitialization
...
export const layer: Layer.Layer<Service, never, Requirements> =
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const flock = yield* EffectFlock.Service // neocode_change
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Snapshot.state")(function* (ctx) {
          ...
          // neocode_change start - serialize snapshot repositories across CLI and extension processes
          ...
          const enabled = Effect.fnUntraced(function* () {
            ...
            if (Flag.NEO_CLIENT === "acp") return false // neocode_change - ACP clients do not support snapshots
          ...
          const add = Effect.fnUntraced(function* (opts?: { env?: Record<string, string>; root?: boolean }) {
            ...
            // neocode_change start - initial seeded writes stay protected by the source pin
            yield* stage(
          ...
          const materialize = Effect.fnUntraced(function* () {
            yield* locked(NeoSnapshotMaterialize.run({ gitdir: state.gitdir, git, fs }).pipe(Effect.orDie)).pipe(
          ...
          const cleanup = Effect.fnUntraced(function* () {
            return yield* locked(
              Effect.gen(function* () {
                ...
                // neocode_change start - retain snapshots for the same seven-day window as object pruning
                yield* NeoSnapshotMaterialize.prune({ gitdir: state.gitdir, git, fs }, Date.now() - retention)
                // neocode_change end
          ...
          const track = Effect.fnUntraced(function* (opts?: Parameters<Interface["track"]>[0]) {
            ...
            return yield* locked(
              Effect.gen(function* () {
                ...
                const seeded: { value?: NeoSnapshotSeed.Output } = {} // neocode_change
                ...
                if (!existed) {
                  ...
                  // neocode_change start - seed all eligible new snapshots from the worktree index
                  seeded.value = yield* NeoSnapshotSeed.seed({
                    dir: state.directory,
                ...
                // neocode_change start - pin every snapshot before background materialization
                ...
                if (
                  seed &&
                  !(yield* NeoSnapshotMaterialize.localize({
                    gitdir: state.gitdir,
                    git,
                    fs,
                ...
                if (
                  seed &&
                  !(yield* NeoSnapshotMaterialize.localizeTrees(
                    { gitdir: state.gitdir, git, fs, staging: seed.staging },
                    hash,
                  ))
                )
                  return
                if (!(yield* NeoSnapshotMaterialize.pin({ gitdir: state.gitdir, git, fs }, hash))) return
          ...
          const diffFull = Effect.fnUntraced(function* (from: string, to: string) {
            return yield* locked(
              Effect.gen(function* () {
                ...
                // neocode_change start - use git patches to avoid blocking the event loop on large diffs
          ...
          yield* materialize() // neocode_change - resume interrupted snapshot object materialization
      ...
      // neocode_change start - service-local state and cache avoid leaking across Snapshot layer instances
      const trackState = NeoSnapshotTrack.makeStates()
      ...
      return Service.of({
        ...
        // neocode_change start - isolate turn-facing snapshot work from poisoned locks
        track: Effect.fn("Snapshot.track")(function* (opts) {
          const ctx = yield* InstanceState.context
          const guard = trackState(ctx.worktree)
          return yield* NeoSnapshotTrack.protect({
            inner: NeoSnapshotTrack.wrap({
        ...
        diffFull: Effect.fn("Snapshot.diffFull")(function* (from: string, to: string) {
          // neocode_change start - cache full diffs at the service boundary
          if (from === to) return []
...
export const defaultLayer = layer.pipe(
  ...
  Layer.provide(EffectFlock.defaultLayer), // neocode_change
)

export const node = LayerNode.make(layer, [FSUtil.node, AppProcess.node, Config.node, EffectFlock.node]) // neocode_change
