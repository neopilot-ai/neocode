...
import * as Config from "@/config/config" // neocode_change
import { InstanceState } from "@/effect/instance-state"
import { Wildcard } from "@opencode-ai/core/util/wildcard"
import { Deferred, Effect, Layer, Context } from "effect"
import os from "os"
import z from "zod" // neocode_change
import { zod } from "@opencode-ai/core/effect-zod" // neocode_change
import { PermissionV1 } from "@opencode-ai/core/v1/permission"
import { Database } from "@opencode-ai/core/database/database" // neocode_change
import { EventV2Bridge } from "@/event-v2-bridge"
import { EventV2 } from "@opencode-ai/core/event"
import { SessionID } from "@/session/schema" // neocode_change - used by AllowEverythingInput
// neocode_change start
import { ConfigProtection } from "@/neocode/permission/config-paths"
import { NeoHeadless } from "@/neocode/permission/headless"
import { drainCovered } from "@/neocode/permission/drain"
import { ReadPermission } from "@/neocode/permission/read"
import { AgentManagerPermission } from "@/neocode/permission/agent-manager" // neocode_change
import { ExternalDirectoryPermission } from "@/neocode/permission/external-directory"
// neocode_change end
...
// neocode_change start - upstream moved these types into PermissionV1; re-export them here so existing
// Neo callers that import off `Permission.*` keep working without a repo-wide rewrite
...
// Neo extends upstream's AskInput with an optional hardRuleset (consumed by drain + session/prompt)
...
interface State {
  ...
  approved: Rule[]
  session: Record<string, Ruleset> // neocode_change
}
...
export function resolve(permission: string, pattern: string, ruleset: Ruleset, ...overrides: Ruleset[]): Rule {
  ...
  const saved = AgentManagerPermission.harden(permission, pattern, evalFn(permission, pattern, ...overrides)) // neocode_change
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const config = yield* Config.Service // neocode_change
    const database = yield* Database.Service // neocode_change
    const state = yield* InstanceState.make<State>(
      Effect.fn("Permission.state")(function* (ctx) {
        void ctx
        const state = {
          pending: new Map<PermissionV1.ID, PendingEntry>(),
          approved: [] as Rule[], // neocode_change - upstream dropped DB-seeded approvals; Neo persists via config.updateGlobal
          session: {} as Record<string, Ruleset>, // neocode_change
        }
    ...
    const ask = Effect.fn("Permission.ask")(function* (input: AskInput) {
      ...
      // neocode_change start - protect config access while honoring explicit global skill trust
      ...
      for (const pattern of request.patterns) {
        const rule = resolve(request.permission, pattern, ruleset, approved, local) // neocode_change — include session-scoped rules
        yield* Effect.logInfo("evaluated", { permission: request.permission, pattern, action: rule })
        // neocode_change start — saved/session approvals cannot override hard Ask/Plan denials
        ...
        if (rule.action === "deny") {
          return yield* new DeniedError({
            ruleset: subset(request.permission, ruleset), // neocode_change
          })
        }
        // neocode_change start - override "allow" to "ask" for protected config paths
      ...
      // neocode_change start - headless subagent asks fail instead of queuing for a reply that never comes (#11903)
      if (yield* NeoHeadless.denies(request.sessionID).pipe(Effect.provideService(Database.Service, database))) {
      ...
      const info: PermissionV1.Request = {
        ...
        // neocode_change start - disable persistence for protected config paths outside one exact global skill
        metadata: {
          ...request.metadata,
          ...(skill ? { rules: [skill] } : {}),
        ...
        // neocode_change end
        always: skill ? [skill] : request.always, // neocode_change - persist only the exact global skill subtree
        tool: request.tool,
      }
      yield* Effect.logInfo("asking", { id, permission: info.permission, patterns: info.patterns })

      const deferred = yield* Deferred.make<void, RejectedError | CorrectedError>()
      pending.set(id, { info, ruleset, hardRuleset, deferred }) // neocode_change
      yield* events.publish(Event.Asked, info) // neocode_change - was bus.publish
    ...
    const reply = Effect.fn("Permission.reply")(function* (input: PermissionV1.ReplyInput) {
      ...
      // neocode_change start - downgrade "always" to "once" for protected config paths
      if (ConfigProtection.isRequest(existing.info) && !ConfigProtection.isGlobalSkillRequest(existing.info)) return
      // neocode_change end

      for (const pattern of existing.info.always) {
        // neocode_change start — saveAlwaysRules may have already persisted selected always-rules
        if (!existing.saved) {
          approved.push({
      ...
      ) // neocode_change - drain publishes replies through the same EventV2Bridge channel
    ...
    const saveAlwaysRules = Effect.fn("Permission.saveAlwaysRules")(function* (
      ...
      // neocode_change - drain publishes replies through the same EventV2Bridge channel (was DeniedError)
      yield* drainCovered(
    ...
    return Service.of({ ask, reply, list, saveAlwaysRules, allowEverything, pending }) // neocode_change
  }),
)
...
export function fromConfig(permission: ConfigPermissionV1.Info) {
  ...
  for (const [key, value] of Object.entries(permission)) {
    ...
    if (value === null) continue // neocode_change — null is a delete sentinel
    ruleset.push(
      // neocode_change start — filter out null entries (delete sentinels)
...
// neocode_change start - Neo permission persistence and headless ancestry dependencies
...
// neocode_change start — inverse of fromConfig: convert rules back to config format
...
export const node = LayerNode.make(layer, [EventV2Bridge.node, Config.node, Database.node]) // neocode_change
