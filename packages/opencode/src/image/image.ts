...
export const MAX_BASE64_BYTES = 5 * 1024 * 1024 // neocode_change - share user file pre-read limit
const MAX_WIDTH = 2000
const MAX_HEIGHT = 2000
const AUTO_RESIZE = true
const JPEG_QUALITIES = [80, 85, 70, 55, 40]
// neocode_change start - preserve valid in-limit images when Photon is unavailable
  ...
  Effect.gen(function* () {
    ...
    const loadPhoton = yield* Effect.cached(
      Effect.sync(() => {
        ...
        ;(globalThis as typeof globalThis & { __NEOCODE_PHOTON_WASM_PATH?: string }).__NEOCODE_PHOTON_WASM_PATH = wasm
      }).pipe(
