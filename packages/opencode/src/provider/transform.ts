...
import { neoProviderOptions } from "@/neocode/provider-options"
import { isLing } from "@/neocode/model-match" // neocode_change
import { reasoningSummary } from "@/neocode/provider/reasoning-summary" // neocode_change
...
function sdkKey(npm: string): string | undefined {
  switch (npm) {
    ...
    case "@openrouter/ai-sdk-provider":
      return "openrouter"
    case "@neocode/neo-gateway": // neocode_change
      return "openrouter"
...
function normalizeMessages(
  ...
  // neocode_change start - only filter for Claude models on Bedrock, not all Bedrock models
...
export function temperature(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 0.3 // neocode_change
  return undefined
}
...
export function topP(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 0.95 // neocode_change
  return undefined
}
...
export function topK(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 20 // neocode_change
  return undefined
}
...
// neocode_change start - fable and sonnet-5 models are adaptive thinking models like opus-4.7/4.8
...
function anthropicAdaptiveEfforts(apiId: string): string[] | null {
  // neocode_change start - treat opus-4.8 and fable like opus-4.7
...
function anthropicOmitsThinking(apiId: string) {
  return anthropicOpus47OrLater(apiId) || anthropicClaude5(apiId) // neocode_change - include Neo's fable/sonnet-5 aliases
}
...
function googleThinkingVariants(model: Provider.Model): Record<string, Record<string, any>> {
  ...
  if (id.includes("gemma")) return {} // neocode_change
  if (id.includes("2.5")) {
    return {
...
export function variants(model: Provider.Model): Record<string, Record<string, any>> {
  ...
  if (
    ["@neocode/neo-gateway", "@ai-sdk/openai-compatible"].includes(model.api.npm) &&
    model.variants &&
  ...
  if (
    ...
    // id.includes("minimax") || // neocode_change
    // id.includes("glm") || // neocode_change
    // id.includes("kimi") || // neocode_change
  ...
  if (id.includes("grok") && id.includes("grok-3-mini")) {
    if (model.api.npm === "@openrouter/ai-sdk-provider" || model.api.npm === "@neocode/neo-gateway") {
      // neocode_change - add Neo Gateway support
      return {
  ...
  switch (model.api.npm) {
    case "@neocode/neo-gateway": // neocode_change
      ...
      if (
        ...
        !id.includes("claude") &&
        !model.id.includes("mercury") // neocode_change
      )
        return {}
      return Object.fromEntries(
        OPENAI_EFFORTS.map((effort) => [effort, { reasoning: { effort } }]), // neocode_change
      )

    case "@openrouter/ai-sdk-provider":
      // neocode_change start - preserve Neo thinking toggles for supported OpenRouter models
      if (id.includes("glm") || id.includes("kimi") || id.includes("qwen") || id.includes("minimax")) {
        return {
          instant: { reasoning: { enabled: false } },
          thinking: { reasoning: { enabled: true } },
      ...
      // neocode_change end
      // neocode_change start - Mercury exposes the full OpenRouter effort set
    ...
    case "@ai-sdk/openai": {
      ...
      return Object.fromEntries(
        efforts.map((effort) => [
          ...
          {
            reasoningEffort: effort,
            reasoningSummary: reasoningSummary(model), // neocode_change
            include: INCLUDE_ENCRYPTED_REASONING,
          },
        ]),
      )
    ...
    case "@ai-sdk/google-vertex/anthropic":
      ...
      // neocode_change start - MiniMax M-series toggles thinking on/off rather than exposing effort levels
    ...
    case "@ai-sdk/mistral":
      ...
      // https://docs.mistral.ai/studio-api/conversations/reasoning // neocode_change
      if (!model.capabilities.reasoning) return {}
...
export function options(input: {
  ...
  if (
    ...
    input.model.api.npm === "@neocode/neo-gateway" // neocode_change
  ) {
    result["usage"] = {
      include: true,
    }
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    if (!input.model.api.id.includes("gpt-5-pro")) {
      ...
      if (
        ...
        input.model.api.npm === "@ai-sdk/github-copilot" || // neocode_change
        input.model.api.npm === "@openrouter/ai-sdk-provider" || // neocode_change
        input.model.api.npm === "@neocode/neo-gateway" || // neocode_change
        input.model.api.npm === "@ai-sdk/amazon-bedrock/mantle"
      ) {
        result["reasoningSummary"] = reasoningSummary(input.model) // neocode_change
    ...
    if (
      // neocode_change start - gate textVerbosity to Responses-API providers
      (input.model.api.npm === "@ai-sdk/openai" ||
        input.model.api.npm === "@ai-sdk/azure" ||
        input.model.api.npm === "@ai-sdk/amazon-bedrock/mantle" ||
        input.model.api.npm === "@ai-sdk/github-copilot" ||
        input.model.api.npm === "@openrouter/ai-sdk-provider" ||
        input.model.api.npm === "@neocode/neo-gateway") &&
      // neocode_change end
...
export function smallOptions(model: Provider.Model) {
  ...
  if (
    ...
    model.api.npm === "@neocode/neo-gateway" // neocode_change
  ) {
    if (model.providerID === "openrouter" && small.reasoning?.effort === "low") {
      return { reasoning: { effort: "none" } }
    }
    if (!model.capabilities.reasoning) return {} // neocode_change - omit unsupported reasoning options
    return { reasoning: { enabled: true } } // neocode_change - use the model's supported default effort
  }
...
export function providerOptions(model: Provider.Model, options: { [x: string]: any }) {
  ...
  // neocode_change start
  if (model.api.npm === "@neocode/neo-gateway") {
    return neoProviderOptions(options)
  }
  // neocode_change end
  ...
  */
  ...
  if (model.providerID === "google" || model.api.id.includes("gemini")) {
    ...
    const sanitizeGemini = (obj: any): any => {
      ...
      // neocode_change start - Gemini rejects required entries without matching properties
