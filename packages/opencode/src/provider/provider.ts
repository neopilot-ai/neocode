...
import * as ModelsDev from "./models" // neocode_change - assemble dynamic Neo models around upstream core catalog
...
import {
  NEO_BUNDLED_PROVIDERS,
  neoCustomLoaders,
  NEO_MODEL_SCHEMA_EXTENSIONS,
  patchModelsDevModel as patchNeoModel,
  patchConfigModel as patchNeoConfigModel,
  patchCustomLoaderResult,
  patchNeoProviderPrivacy,
  neoSmallModelPriority,
  buildTimeoutSignal,
} from "@/neocode/provider/provider"
import * as ModelsRefresh from "@/neocode/provider/models-refresh"
// neocode_change end
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  ...NEO_BUNDLED_PROVIDERS, // neocode_change
}
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    azure: Effect.fnUntraced(function* (provider: Info) {
      ...
      // neocode_change start - prefer explicit Azure endpoint over resource name to avoid conflicting SDK options
      const endpoint = iife(() => {
        return [
      ...
      if (!resource && !endpoint) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              "Azure resource name or endpoint is missing. Set AZURE_RESOURCE_NAME, AZURE_OPENAI_RESOURCE_NAME, AZURE_OPENAI_ENDPOINT, or reconnect the azure provider.", // neocode_change
            )
          },
        }
      }
      ...
      return {
        ...
        options: {
          ...(endpoint ? { baseURL: endpoint } : { resourceName: resource }), // neocode_change
        },
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://neo.khulnasoft.com/", // neocode_change
            "X-Title": "Neo Code", // neocode_change
            "X-Source": "neo", // neocode_change
          },
        },
      }),
    openrouter: () =>
      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://neo.khulnasoft.com/", // neocode_change
            "X-Title": "Neo Code", // neocode_change
            "X-BILLING-INVOKE-ORIGIN": "neocode", // neocode_change
          },
        },
      }),
    vercel: () =>
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `neo/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`, // neocode_change
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `neo auth cloudflare-ai-gateway`.", // neocode_change
        )
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "Neo Code", // neocode_change
          },
        },
      }),
    neo: () =>
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !pat) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Set via env var, neo auth, or provider options.`, // neocode_change
            )
          },
        }
      }
...
export const Model = Schema.Struct({
  ...
  ...NEO_MODEL_SCHEMA_EXTENSIONS, // neocode_change
...
export const Info = Schema.Struct({
  id: ProviderV2.ID,
  name: Schema.String,
  description: optionalOmitUndefined(Schema.String), // neocode_change
  source: Schema.Literals(["env", "config", "custom", "api"]),
  env: Schema.Array(Schema.String),
  key: optionalOmitUndefined(Schema.String), // neocode_change
  metadata: optionalOmitUndefined(ProviderMetadata), // neocode_change
  options: Schema.Record(Schema.String, Schema.Any),
...
export const ListResult = Schema.Struct({
  ...
  failed: Schema.Array(Schema.String), // neocode_change
})
...
export class ModelNotFoundError extends Schema.TaggedErrorClass<ModelNotFoundError>()("ProviderModelNotFoundError", {
  ...
  modelsEmpty: Schema.optional(Schema.Boolean), // neocode_change
...
function fromModelsDevModel(provider: ModelsDev.Provider, model: ModelsDev.Model): Model {
  ...
  Object.assign(base, patchNeoModel(provider.id, model)) // neocode_change
...
export function fromModelsDevProvider(provider: ModelsDev.Provider): Info {
  ...
  return {
    ...
    source: "custom",
    name: provider.name,
    description: provider.description, // neocode_change
    env: [...(provider.env ?? [])],
    options: {},
    models,
  }
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        for (const [providerID, provider] of configProviders) {
          if (!provider) continue // neocode_change - null entries are transient delete sentinels
          ...
          for (const [modelID, model] of Object.entries(provider.models ?? {})) {
            if (!model) continue // neocode_change - null entries are transient delete sentinels
            ...
            const parsedModel: Model = {
              ...
              // variants: {}, // neocode_change, moved into patchNeoConfigModel
              ...patchNeoConfigModel(model, existingModel), // neocode_change
            }
            const merged = mergeDeep(ProviderTransform.variants(parsedModel), model.variants ?? {})
            parsedModel.variants = mapValues(
              pickBy(merged, (v): v is NonNullable<typeof v> => !!v && !v.disabled), // neocode_change - drop null delete sentinels
              (v) => omit(v, ["disabled"]),
            )
        ...
        // neocode_change start - load auths before env so OAuth plugins can override inherited credentials
        ...
        for (const [id, provider] of Object.entries(database)) {
          ...
          if (disabled.has(providerID)) continue
          // neocode_change start - prefer explicit OAuth auth over inherited env credentials
          if (
        ...
        // neocode_change start - resolve env once for patchCustomLoaderResult (azure env fallback)
        const neoEnv = yield* env.all()
        // neocode_change end
        for (const [id, fn] of Object.entries({ ...custom(dep), ...neoCustomLoaders(dep) })) {
          // neocode_change
          const providerID = ProviderV2.ID.make(id)
          if (disabled.has(providerID)) continue
          ...
          if (result) patchCustomLoaderResult(id, result, neoEnv) // neocode_change
        ...
        for (const [id, provider] of configProviders) {
          if (!provider) continue // neocode_change - null entries are transient delete sentinels
          const providerID = ProviderV2.ID.make(id)
          // neocode_change start - keep OAuth plugin source when config and Codex auth coexist
          const oauth =
        ...
        patchNeoProviderPrivacy(providers[ProviderV2.ID.make("neo")], cfg) // neocode_change

        const gitlab = ProviderV2.ID.make("gitlab")
        if (discoveryLoaders[gitlab] && providers[gitlab] && isProviderAllowed(gitlab)) {
          // neocode_change start - keep discovery failures visible instead of swallowing them
    ...
    yield* ModelsRefresh.watch(state) // neocode_change
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>) {
      try {
        ...
        options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
          ...
          const timeout = buildTimeoutSignal(options) // neocode_change - use cancellable timeout for connection phase
          ...
          if (timeout.signal) signals.push(timeout.signal) // neocode_change

          const combined = signals.length === 0 ? null : signals.length === 1 ? signals[0] : AbortSignal.any(signals)
          if (combined) opts.signal = combined

          // neocode_change start - clear connection-phase timeout once headers arrive
          try {
    ...
    const getModel = Effect.fn("Provider.getModel")(function* (providerID: ProviderV2.ID, modelID: ModelV2.ID) {
      ...
      if (!provider) {
        ...
        const suggestions = catalogProvider
          ...
          : fuzzysort
              ...
              .map((m) => m.target)
        const empty = false // neocode_change
        return yield* new ModelNotFoundError({ providerID, modelID, suggestions, modelsEmpty: empty }) // neocode_change
      }
      ...
      if (!info) {
        ...
        const empty = Object.keys(provider.models).length === 0 // neocode_change
        return yield* new ModelNotFoundError({ providerID, modelID, suggestions, modelsEmpty: empty }) // neocode_change
      }
      return info
    })
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderV2.ID) {
      ...
      // neocode_change - `let` (was upstream `const`) so the Neo override below can reassign
      let priority = providerID.startsWith("opencode")
        ...
        : providerID.startsWith("github-copilot")
          ...
          : defaultPriority
      // neocode_change start
      const neoPriority = neoSmallModelPriority(providerID)
      if (neoPriority) priority = neoPriority
      // neocode_change end
      ...
      // neocode_change start - fall back to neo's auto small model
      const neoFallback = s.providers[ProviderV2.ID.make("neo")]
      if (neoFallback?.models["neo-auto/small"]) return neoFallback.models["neo-auto/small"]
      // neocode_change end
