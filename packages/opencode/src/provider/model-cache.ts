...
import { fetchNeoModels, type NeoModelsResult } from "@neocode/neo-gateway"
...
type NeoOptions = NonNullable<Parameters<typeof fetchNeoModels>[0]>
type Options = { -readonly [K in keyof NeoOptions]?: NeoOptions[K] } & { apiKey?: string }
type Failure = NonNullable<NeoModelsResult["error"]>
...
export interface NeoModels {
  readonly fetch: (options: NeoOptions) => Effect.Effect<NeoModelsResult, unknown>
}

export class NeoModelsService extends Context.Service<NeoModelsService, NeoModels>()(
  "@neocode/ModelCache/NeoModels",
) {}

export const neoModelsLayer = Layer.succeed(
  NeoModelsService,
  NeoModelsService.of({ fetch: (options) => Effect.tryPromise(() => fetchNeoModels(options)) }),
)
type Cell = {
...
export class Service extends Context.Service<Service, Interface>()("@neocode/ModelCache") {}
...
export const layer: Layer.Layer<
  Service,
  never,
  Auth.Service | Config.Service | NeoModelsService | HttpClient.HttpClient
> = Layer.effect(
  Service,
  Effect.gen(function* () {
    const auth = yield* Auth.Service
    const cfg = yield* Config.Service
    const neo = yield* NeoModelsService
    ...
    const authOptions = Effect.fn("ModelCache.authOptions")(function* (providerID: string) {
      if (providerID !== "neo" && providerID !== "apertis") return {}
      const config = yield* cfg.get()
      const options: Options = {}

      if (providerID === "neo") {
        const item = config.provider?.[providerID]
        if (item?.options?.apiKey) options.neocodeToken = item.options.apiKey
        if (item?.options?.neocodeOrganizationId) options.neocodeOrganizationId = item.options.neocodeOrganizationId

        const info = yield* auth.get(providerID)
        if (info?.type === "api") options.neocodeToken = info.key
        if (info?.type === "oauth") {
          options.neocodeToken = info.access
          if (info.accountId) options.neocodeOrganizationId = info.accountId
        }

        if (process.env.NEO_API_KEY) options.neocodeToken = process.env.NEO_API_KEY
        if (process.env.NEO_ORG_ID) options.neocodeOrganizationId = process.env.NEO_ORG_ID
        log.debug("auth options resolved", {
          providerID,
          hasToken: !!options.neocodeToken,
          hasOrganizationId: !!options.neocodeOrganizationId,
        })
      }
    ...
    const fetchModels = (providerID: string, options: Options): Effect.Effect<Result, unknown> => {
      if (providerID === "neo") return neo.fetch(options)
    ...
    const key = (providerID: string, options?: Options) => {
      if (providerID === "neo") {
        return JSON.stringify([providerID, options?.baseURL, options?.neocodeOrganizationId, options?.neocodeToken])
      }
...
export const defaultLayer = layer.pipe(
  ...
  Layer.provide(neoModelsLayer),
)

const neoModels = LayerNode.make(neoModelsLayer, [])
export const node = LayerNode.make(layer, [Auth.node, Config.node, neoModels, httpClient])
