...
import type { AuthOAuthResult, Hooks } from "@neocode/plugin"
...
export const layer: Layer.Layer<Service, never, Auth.Service | Plugin.Service | ModelCache.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const callback = Effect.fn("ProviderAuth.callback")(function* (
      ...
      // neocode_change start - Update telemetry identity on Neo auth
...
export const node = LayerNode.make(layer, [Auth.node, Plugin.node, ModelCache.node]) // neocode_change
