...
- Author package entrypoint is `@neocode/plugin/tui`.
...
```json
{
  "$schema": "https://app.neo.khulnasoft.com/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- Import types from `@neocode/plugin/tui`.
- `@neocode/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@neocode/plugin/tui"
...
- The value is a semver range checked against the running Neo version.
...
- `neo plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `neo plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.neo` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.neo`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.neo`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.neo` or legacy `.neocode` directory, or `NEO_CONFIG_DIR`, Neo installs `@neocode/plugin` into that directory and writes:
...
That is what makes local config-scoped plugins able to import `@neocode/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Neo's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Neo registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Neo pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@neocode/plugin/tui`.
...
- `message` is required; `title` defaults to `"Neo"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `neo.default`.
...
- Local plugins persist installed themes under `.neo/themes`, or under `themes` beside a config source in `.neo` or legacy `.neocode`.
...
- `--pure` / `NEO_PURE` skips external TUI plugins only.
...
The upstream `.opencode` smoke fixtures remain in the repository for OpenCode compatibility, but Neo does not auto-load them. Neo-local examples must live under `.neo` or legacy `.neocode`.
