...
describe("Edit target resolution", () => {
  ...
  test("does NOT route the FIM Mercury model to the edit endpoint", () => {
    // `mercury-edit-2` (kind: fim) must fall through to the neo placeholder,
    // not the edit endpoint — only `mercury-next-edit` (kind: edit) is NES.
    expect(resolveEditTarget("inception", "mercury-edit-2").provider).toBe("neo")
  })

  test("routes the Neo Gateway next-edit model to the gateway proxy", () => {
    const target = resolveEditTarget("neo", "inception/mercury-next-edit")
    expect(target.provider).toBe("neo")
    expect(target.model).toBe("inception/mercury-edit-2")
    expect(target.url).toMatch(/\/api\/edit\/completions$/)
  })

  test("falls back to a neo placeholder (no upstream) for non-edit models", () => {
    expect(resolveEditTarget("neo", "mistralai/codestral-2508")).toEqual({
      provider: "neo",
  ...
  test("routes the default model to the gateway proxy", () => {
    expect(resolveEditTarget()).toEqual({
      ...
      url: "https://api.neo.khulnasoft.com/api/edit/completions",
    })
  })
})
