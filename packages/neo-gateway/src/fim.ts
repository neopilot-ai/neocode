...
const NEO_FIM_URL = NEO_API_BASE + "/api/fim/completions"
const INCEPTION_FIM_URL = "https://api.inceptionlabs.ai/v1/fim/completions"

function neoTarget(model?: string): FimTarget {
  return { provider: "neo", model: model ?? "mistralai/codestral-2501", url: NEO_FIM_URL }
}

export function resolveFimTarget(provider?: string, model?: string): FimTarget {
  if (!provider || provider === "neo") return neoTarget(model)
