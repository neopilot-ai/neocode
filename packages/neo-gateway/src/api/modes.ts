import { z } from "zod"
import { NEO_API_BASE, MODELS_FETCH_TIMEOUT_MS } from "./constants.js"
...
/**
 * Fetch custom modes for an organization from the Neo Cloud API.
 *
...
export async function fetchOrganizationModes(token: string, organizationId: string): Promise<OrganizationMode[]> {
  ...
  try {
    const url = `${NEO_API_BASE}/api/organizations/${encodeURIComponent(organizationId)}/modes`
    ...
    console.warn("[Neo Gateway] Error fetching organization modes:", err)
    return []
  }
}
