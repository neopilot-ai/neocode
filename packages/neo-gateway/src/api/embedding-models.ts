import { z } from "zod"
import { resolveNeoGatewayBaseUrl } from "./url.js"
...
export type NeoEmbeddingModelCatalogIssue = {
...
type Options = {
  ...
  signal?: AbortSignal
  attempts?: number
  onError?: (issue: NeoEmbeddingModelCatalogIssue) => void
}
...
export async function fetchNeoEmbeddingModelCatalog(options: Options = {}): Promise<NeoEmbeddingModelCatalog> {
  const url = new URL("embedding-models", resolveNeoGatewayBaseUrl({ baseURL: options.baseURL, token: options.token }))
  const requested = options.attempts ?? 3
  const attempts = Number.isFinite(requested) ? Math.min(3, Math.max(1, Math.floor(requested))) : 3
  const issue = { current: undefined as NeoEmbeddingModelCatalogIssue | undefined }
  ...
  for (const attempt of Array.from({ length: attempts }, (_, index) => index)) {
    ...
    try {
      ...
      if (!response.ok) {
        issue.current = {
          code: "http",
          message: `Unable to load Neo embedding models (HTTP ${response.status}).`,
      ...
      issue.current = {
        ...
        message: "Neo returned an invalid embedding model catalog.",
      }
      break
    } catch (err) {
      if (options.signal?.aborted) throw options.signal.reason
      issue.current = {
        code: "network",
        message: "Unable to connect to Neo to load embedding models. Check your network connection and try again.",
      }
  ...
  return EMPTY_NEO_EMBEDDING_MODEL_CATALOG
}
