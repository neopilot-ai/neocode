...
import { NEO_API_BASE, NEO_OPENROUTER_BASE, MODELS_FETCH_TIMEOUT_MS, PROMPTS, AI_SDK_PROVIDERS } from "./constants.js"

export type NeoModelsResult = {
...
/**
 * Fetch models from Neo API (OpenRouter-compatible endpoint)
 *
 * @param options - Configuration options
 * @returns Typed result with models and optional error info
 */
export async function fetchNeoModels(options?: {
  neocodeToken?: string
  neocodeOrganizationId?: string
  baseURL?: string
}): Promise<NeoModelsResult> {
  const raw = await fetchRawNeoModels(options)
  ...
  for (const model of raw.data) {
    ...
    // Skip models that don't support tools — Neo requires tool calling
...
export type NeoImageModel = {
  id: string
  name: string
  description?: string
}

export type NeoImageModelsResult = {
  models: NeoImageModel[]
  error?: { kind: "unauthorized" | "network" | "schema" | "http"; status?: number }
}

/**
 * Fetch image-capable models from Neo API (OpenRouter-compatible endpoint).
 * Uses the same raw fetch as {@link fetchNeoModels} but keeps only models
 * whose `output_modalities` include `"image"`.
 */
export async function fetchNeoImageModels(options?: {
  neocodeToken?: string
  neocodeOrganizationId?: string
  baseURL?: string
}): Promise<NeoImageModelsResult> {
  const raw = await fetchRawNeoModels(options)
  if (raw.error) return { models: [], error: raw.error }

  const models: NeoImageModel[] = []
...
/**
 * Shared raw fetch + validate used by both {@link fetchNeoModels} and {@link fetchNeoImageModels}.
 */
async function fetchRawNeoModels(options?: {
  neocodeToken?: string
  neocodeOrganizationId?: string
  baseURL?: string
}): Promise<
  { data: OpenRouterModel[]; error?: undefined } | { data?: undefined; error: NonNullable<NeoModelsResult["error"]> }
> {
  const token = options?.neocodeToken
  const organizationId = options?.neocodeOrganizationId

  // Construct base URL
  const defaultBaseURL = organizationId ? `${NEO_API_BASE}/api/organizations/${organizationId}` : NEO_OPENROUTER_BASE

  const baseURL = options?.baseURL ?? defaultBaseURL

  // Transform URL with token if available
  const finalBaseURL = token ? getNeoUrlFromToken(baseURL, token) : baseURL
  ...
  const response = await fetch(modelsURL, {
    headers: {
      ...getDefaultHeaders(),
      ...buildNeoHeaders(undefined, { neocodeOrganizationId: organizationId }),
