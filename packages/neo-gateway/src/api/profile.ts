...
import type { NeocodeProfile, Organization, NeocodeBalance } from "../types.js"
import { NEO_API_BASE, DEFAULT_MODEL, DEFAULT_FREE_MODEL } from "./constants.js"

/**
 * Fetch user profile from Neo API
 */
export async function fetchProfile(token: string): Promise<NeocodeProfile> {
  const response = await fetch(`${NEO_API_BASE}/api/profile`, {
    headers: {
  ...
  // Transform to flat NeocodeProfile structure
  return {
 ...
 */
export const getNeoProfile = fetchProfile
 ...
 */
export function defaultOrganizationId(profile: NeocodeProfile): string | undefined {
...
/**
 * Fetch user balance from Neo API
 * @param token - Authentication token
 * @param organizationId - Optional organization ID for team balance
 */
export async function fetchBalance(token: string, organizationId?: string): Promise<NeocodeBalance | null> {
  try {
    ...
    if (organizationId) {
      headers["x-neocode-organizationid"] = organizationId
    }

    const response = await fetch(`${NEO_API_BASE}/api/profile/balance`, { headers })
 ...
 * Alias for compatibility with existing code
 */
export const getNeoBalance = fetchBalance
 ...
 * Alias for compatibility with existing code
 */
export const getNeoDefaultModel = fetchDefaultModel
