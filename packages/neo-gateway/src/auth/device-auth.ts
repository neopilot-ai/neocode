...
import { getNeoProfile, getNeoDefaultModel, promptOrganizationSelection } from "../api/profile.js"
...
export async function authenticateWithDeviceAuth(): Promise<DeviceAuthResult> {
  ...
  const profileData = await getNeoProfile(token)
  ...
  const model = await getNeoDefaultModel(token, organizationId)
