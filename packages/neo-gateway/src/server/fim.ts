  ...
  const run = async (url: string) => {
    ...
    return fetch(url, {
      ...
      headers: {
        ...
        ...(target.provider === "neo"
          ? buildNeoHeaders(undefined, { neocodeOrganizationId: input.organizationId })
          : {}),
...
export function createFimHandler(Auth: Auth) {
  return async (c: any) => {
    ...
    const proxy = target.provider === "neo" ? await getProxyAuth(Auth) : undefined
    const token = target.provider === "neo" ? proxy?.token : await getProviderKey(Auth, target.provider)

    if (target.provider === "neo" && !proxy?.auth) {
      return c.json({ error: "Not authenticated with Neo Gateway" }, 401)
    }

    if (target.provider === "neo" && !token) {
