...
import { fetchNeoPassState } from "../api/neo-pass.js"
import { fetchNeocodeNotifications } from "../api/notifications.js"
import { clearModesCache } from "../api/modes.js"
import { HEADER_ORGANIZATIONID, NEO_API_BASE, NEO_CHAT_URL, NEO_EVENT_SERVICE_URL } from "../api/constants.js"
import type { NeocodeBalance, NeocodeProfile, NeoPassState } from "../types.js"
...
export interface NeoProfileResult {
  profile: NeocodeProfile
...
export interface AuthStore {
  get(provider: string): Promise<NeoAuth | undefined>
  set(provider: string, auth: Extract<NeoAuth, { type: "oauth" }>): Promise<void>
}
...
export function getToken(auth: NeoAuth | undefined) {
  if (auth?.type === "api") return auth.key
  if (auth?.type === "oauth") return auth.access
  return undefined
}

export function getOrganizationId(auth: NeoAuth | undefined) {
  if (auth?.type === "oauth") return auth.accountId
  return undefined
}

export async function getProfile(auth: AuthStore): Promise<NeoProfileResult> {
  const info = await auth.get("neo")
  if (!info || info.type !== "oauth") throw new UnauthorizedError("Not authenticated with Neo Gateway")

  const currentOrgId = info.accountId ?? null
  const [profile, balance, neoPass] = await Promise.all([
...
export async function getNotifications(auth: AuthStore) {
  ...
  return fetchNeocodeNotifications({
    neocodeToken: token,
...
export async function setOrganization(deps: OrganizationDeps, organizationId: string | null) {
  const info = await deps.auth.get("neo")
  if (!info || info.type !== "oauth") throw new UnauthorizedError("Not authenticated with Neo Gateway")

  await deps.auth.set("neo", {
    type: "oauth",
...
export async function getCloudSessions(token: string, input: CloudSessionsInput) {
  ...
  const response = await fetch(`${NEO_API_BASE}/api/trpc/cliSessionsV2.list?${params.toString()}`, {
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
      ...buildNeoHeaders(),
  ...
  if (!response.ok) {
    ...
    console.error("[Neo Gateway] cloud-sessions: tRPC request failed", {
