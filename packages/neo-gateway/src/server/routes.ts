/**
 * Neo Gateway specific routes
 * Handles profile fetching and organization management for Neo Gateway provider
 *
 * This factory function accepts OpenCode dependencies to create Neo-specific routes
 */

import { fetchNeocodeNotifications, NeocodeNotificationSchema } from "../api/notifications.js"
import { fetchNeoImageModels } from "../api/models.js"
import { fetchOrganizationModes, clearModesCache } from "../api/modes.js"
import { NEO_API_BASE, HEADER_FEATURE, HEADER_ORGANIZATIONID } from "../api/constants.js"
...
interface NeoRoutesDeps extends ImportDeps {
  Hono: new () => Hono
...
/**
 * Create Neo Gateway routes with OpenCode dependencies injected
 *
 * @example
 * ```typescript
 * import { createNeoRoutes } from "@neocode/neo-gateway"
 ...
 *
 * export const NeoRoutes = createNeoRoutes({
 *   Hono,
 *   describeRoute,
 *   validator,
 *   resolver,
 ...
 * ```
 */
export function createNeoRoutes(deps: NeoRoutesDeps) {
  const {
    Hono,
    describeRoute,
    validator,
  ...
  return new Hono()
    .get(
      ...
      async (c: any) => {
        try {
          ...
          return c.json({ error: "Not authenticated with Neo Gateway" }, 401)
        }
      },
    )
    .post(
      ...
      describeRoute({
        ...
        operationId: "neo.organization.set",
        responses: {
          200: {
    ...
    .post(
      ...
      describeRoute({
        ...
        operationId: "neo.audio.transcriptions",
        responses: {
          200: {
      ...
      async (c: any) => {
        ...
        if (!proxy.auth) return c.json({ error: "Not authenticated with Neo Gateway" }, 401)
        ...
        const response = await fetch(`${NEO_API_BASE}/api/gateway/v1/audio/transcriptions`, {
          method: "POST",
          headers,
    ...
    .get(
      ...
      async (c: any) => {
        try {
          ...
          const result = await fetchNeoImageModels({
            neocodeToken: proxy.token,
            neocodeOrganizationId: proxy.organizationId,
          })
          if (result.error) {
    ...
    .post(
      ...
      describeRoute({
        ...
        description:
          "Proxy an image generation request (chat-completions with modalities) to the Neo Gateway OpenRouter passthrough",
        operationId: "neo.image.generations",
        responses: {
          200: {
      ...
      async (c: any) => {
        ...
        const response = await fetch(`${NEO_API_BASE}/api/openrouter/chat/completions`, {
          method: "POST",
          headers,
          signal: c.req.raw.signal,
    ...
    .get(
      ...
      describeRoute({
        ...
        operationId: "neo.notifications",
        responses: {
          200: {
            description: "Notifications list",
            content: {
              "application/json": {
                schema: resolver(z.array(NeocodeNotificationSchema)),
              },
            },
          },
          ...errors(400, 401),
    ...
    .get(
      ...
      describeRoute({
        ...
        operationId: "neo.cloud.session.get",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          const auth = await Auth.get("neo")
          if (!auth) return c.json({ error: "Not authenticated with Neo Gateway" }, 401)
          const token = auth.type === "api" ? auth.key : auth.type === "oauth" ? auth.access : undefined
          ...
          console.error("[Neo Gateway] cloud/session/get: unhandled error", err?.message ?? err)
    ...
    .post(
      ...
      describeRoute({
        ...
        operationId: "neo.cloud.session.import",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          ...
          const auth = await Auth.get("neo")
          if (!auth) return c.json({ error: "Not authenticated with Neo" }, 401)
          const token = auth.type === "api" ? auth.key : auth.type === "oauth" ? auth.access : undefined
          if (!token) return c.json({ error: "No valid token found" }, 401)
          ...
          console.error("[Neo Gateway] cloud/session/import: unhandled error", err?.message ?? err)
          return c.json({ error: "Internal error" }, 500)
        }
      },
    )
              ...
              "application/json": {
                schema: resolver(
                  z.object({
                    ...
                    // cloud `services/neoclaw/src/index.ts` and the
                    // `PlatformStatusResponse` type in
                    // cloud/apps/web/src/lib/neoclaw/types.ts. Keeping them in
      ...
      async (c: any) => {
        try {
          ...
          if (err instanceof GatewayError) {
            return c.json({ error: `NeoClaw request failed: ${err.status} ${err.message}` }, err.status as any)
          }
          console.error("[Neo Gateway] claw/status: error", err?.message ?? err)
          return c.json({ error: "Failed to reach NeoClaw" }, 502)
        }
      },
    )
    .get(
      ...
      describeRoute({
        ...
        operationId: "neo.claw.chatCredentials",
        responses: {
          200: {
            description: "Neo Chat credentials or null",
            content: {
              "application/json": {
                schema: resolver(
                  z
    ...
    .get(
      ...
      async (c: any) => {
        try {
          ...
          console.error("[Neo Gateway] cloud-sessions: unhandled error", err?.message ?? err)
          return c.json({ error: "Internal error" }, 500)
        }
      },
    )
