...
export const CurrentProxy = Context.Reference<ProxyRuntime | undefined>("@neocode/sandbox/CurrentProxy", {
...
export const CurrentProxyFactory = Context.Reference<ProxyFactory>("@neocode/sandbox/CurrentProxyFactory", {
...
export async function startProxy(
  ...
  server.on("connect", (request, client, head) => {
    ...
    if (!authenticate(request.headers["proxy-authorization"], token)) {
      client.end('HTTP/1.1 407 Proxy Authentication Required\r\nProxy-Authenticate: Basic realm="neo"\r\n\r\n')
      return
    }
    try {
  ...
  server.on("request", async (request, response) => {
    if (!authenticate(request.headers["proxy-authorization"], token)) {
      response.writeHead(407, { "Proxy-Authenticate": 'Basic realm="neo"' })
      response.end()
      return
    }
    try {
  ...
  const dir = platform === "linux" ? await mkdtemp(path.join(os.tmpdir(), "neo-sandbox-proxy-")) : undefined
  ...
  const url = `http://neo:${encodeURIComponent(token)}@127.0.0.1:${port}`
  return {
    url,
    token,
    allowedHosts,
