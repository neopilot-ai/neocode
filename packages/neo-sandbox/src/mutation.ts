...
declare const NEO_SANDBOX_MUTATION_WORKER_PATH: string

function worker() {
  if (typeof NEO_SANDBOX_MUTATION_WORKER_PATH === "undefined") {
    return { path: fileURLToPath(new URL("./neo-sandbox-mutation-worker.ts", import.meta.url)), environment: {} }
  }
  const path = NEO_SANDBOX_MUTATION_WORKER_PATH.startsWith(".")
    ? fileURLToPath(new URL(NEO_SANDBOX_MUTATION_WORKER_PATH, import.meta.url))
    : resolve(dirname(process.execPath), NEO_SANDBOX_MUTATION_WORKER_PATH)
...
const CurrentRunner = Context.Reference<Runner>("@neocode/sandbox/CurrentMutationRunner", {
  defaultValue: () => mutate,
})
