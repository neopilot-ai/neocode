...
declare const NEO_BWRAP_SHA256: string | undefined
declare const NEO_SANDBOX_NETWORK_RELAY_PATH: string | undefined
declare const NEO_SANDBOX_SECCOMP_PATH: string | undefined
...
function relay() {
  if (typeof NEO_SANDBOX_NETWORK_RELAY_PATH === "undefined") {
    return { path: fileURLToPath(new URL("./neo-sandbox-network-relay.ts", import.meta.url)), environment: {} }
  }
  const target = NEO_SANDBOX_NETWORK_RELAY_PATH.startsWith(".")
    ? fileURLToPath(new URL(NEO_SANDBOX_NETWORK_RELAY_PATH, import.meta.url))
    : path.resolve(path.dirname(process.execPath), NEO_SANDBOX_NETWORK_RELAY_PATH)
  return { path: target, environment: { BUN_BE_BUN: "1" } }
}

function seccomp() {
  if (typeof NEO_SANDBOX_SECCOMP_PATH !== "undefined") {
    return path.resolve(path.dirname(process.execPath), NEO_SANDBOX_SECCOMP_PATH)
  }
...
function digest() {
  return typeof NEO_BWRAP_SHA256 === "undefined" ? undefined : NEO_BWRAP_SHA256
}
...
function select(): Selection {
  const override = process.env.NEO_BWRAP_PATH
