...
export default tool({
  ...
  args: {
    assignee: tool.schema
      .enum(["thdxr", "neocodeai", "rekram1-node", "fwang", "jayair", "kommander"])
  ...
  async execute(args) {
    ...
    const owner = "neopilot-ai"
    const repo = "neocode"

    const results: string[] = []

    if (args.assignee === "neocodeai" && !args.labels.includes("desktop")) {
      throw new Error("Only desktop issues should be assigned to neocodeai")
    }
