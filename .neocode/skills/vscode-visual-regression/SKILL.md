...
description: Write Storybook stories and visual regression tests for the Neo VS Code extension webview UI
---

Use this skill when the user asks you to add visual regression tests, screenshot tests, or Storybook stories for components in `packages/neo-vscode/`.
...
Stories live in `packages/neo-vscode/webview-ui/src/stories/`. Existing files and their scope:
...
Snapshots live at `packages/neo-vscode/tests/visual-regression.spec.ts-snapshots/`:
...
Key settings in `packages/neo-vscode/playwright.config.ts`:
...
- `packages/util/**`
- `packages/sdk/js/**`
- `packages/neo-vscode/webview-ui/**`
- `packages/neo-vscode/.storybook/**`
- `packages/neo-vscode/tests/visual-regression*`
...
neo-ui components are imported via deep subpaths:

```tsx
import { Part } from "@neocode/neo-ui/message-part"
import { BasicTool } from "@neocode/neo-ui/basic-tool"
...
```tsx
import type { AssistantMessage as SDKAssistantMessage, TextPart, ToolPart } from "@neocode/sdk/v2"
...
```
globals=colorScheme:dark;theme:neo-vscode;vscodeTheme:dark-modern
```

The `.storybook/preview.tsx` applies these via a decorator that calls `applyVscodeTheme()` / `applyNeoTheme()` from neo-ui. Stories do NOT need to handle theming — it happens automatically.
