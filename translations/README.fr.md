...
Neo Code est un agent de codage avec IA qui vous accompagne partout où vous travaillez : [VS Code](https://neo.khulnasoft.com/landing/vs-code), [JetBrains](https://neo.khulnasoft.com/features/jetbrains-native) et la [CLI](https://neo.khulnasoft.com/cli). Il est open source avec une tarification ouverte. Vous choisissez parmi plus de 500 modèles, vous pouvez en changer en cours de tâche et vous payez le tarif du fournisseur du modèle sans majoration. Aucune clé API n'est nécessaire pour commencer.

### Installation

Choisissez où vous voulez exécuter Neo.
...
Installez directement l'[extension Neo Code](vscode:extension/neocode.neo-code), ou récupérez-la sur le [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=neocode.Neo-Code). Créez un compte et vous aurez accès à plus de 500 modèles, dont GPT-5.5, Claude Opus 4.7, Claude Sonnet 4.6 et Gemini 3.1 Pro Preview, tous au prix fournisseur.
...
Exécutez ensuite `neo` dans n'importe quel répertoire de projet pour commencer.
...
Installez le [plugin Neo Code](https://plugins.jetbrains.com/plugin/28350-neo-code) depuis JetBrains Marketplace, ou recherchez "Neo Code" dans `Settings → Plugins` dans n'importe quel IDE JetBrains.
...
Exécutez Neo depuis le web, sans machine locale, sur [app.neo.khulnasoft.com/cloud](https://app.neo.khulnasoft.com/cloud).
...
Configurez des revues de code IA automatisées sur vos pull requests à l'adresse [app.neo.khulnasoft.com/code-reviews](https://app.neo.khulnasoft.com/code-reviews).
...
Lancez votre agent IA toujours actif sur [app.neo.khulnasoft.com/claw](https://app.neo.khulnasoft.com/claw).
...
Téléchargez le dernier binaire depuis la [page des releases](https://github.com/Neopilot-Ai/neocode/releases).

| Plateforme | Fichier |
|---|---|
| Windows (la plupart des PC) | `neo-windows-x64.zip` |
...
Notes : `x64-baseline` est une build de compatibilité pour les anciens processeurs sans AVX. `musl` est la build liée statiquement pour Alpine ou les images Docker minimales sans glibc. `neo-vscode-*.vsix` est le paquet de l'extension VS Code, pas la CLI. Les archives `Source code` servent à compiler depuis les sources.

</details>

### Agents

Neo inclut des agents spécialisés entre lesquels vous pouvez basculer selon la tâche. Vous pouvez aussi créer vos propres agents personnalisés.
...
En savoir plus sur les [agents et agents personnalisés](https://neo.khulnasoft.com/docs/code-with-ai/agents/using-agents).
...
Exécutez `neo run` avec `--auto` pour un fonctionnement entièrement autonome sans prompts, conçu pour les pipelines CI/CD :
...
<details>
<summary>D'où vient Neo CLI ?</summary>

Neo CLI est un fork d'[OpenCode](https://github.com/anomalyco/opencode), amélioré pour fonctionner au sein de la plateforme d'ingénierie agentique Neo.

</details>

---

**Rejoindre la communauté** [Discord](https://neo.khulnasoft.com/discord) | [X](https://x.com/neocode) | [Reddit](https://www.reddit.com/r/neocode/)
