...
Neo Code — это AI-агент для написания кода, который работает там, где работаете вы: в [VS Code](https://neo.khulnasoft.com/landing/vs-code), [JetBrains](https://neo.khulnasoft.com/features/jetbrains-native) и [CLI](https://neo.khulnasoft.com/cli). Он имеет открытый исходный код и открытую модель ценообразования. Вы выбираете из более чем 500 моделей, переключаетесь между ними во время задачи и платите по тарифу поставщика модели без наценки. Для начала не нужны API-ключи.

### Установка

Выберите, где вы хотите запускать Neo.
...
Установите [расширение Neo Code](vscode:extension/neocode.neo-code) напрямую или скачайте его из [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=neocode.Neo-Code). Создайте аккаунт и получите доступ к более чем 500 моделям, включая GPT-5.5, Claude Opus 4.7, Claude Sonnet 4.6 и Gemini 3.1 Pro Preview, все по ценам поставщиков.
...
Затем запустите `neo` в любом каталоге проекта.
...
Установите [плагин Neo Code](https://plugins.jetbrains.com/plugin/28350-neo-code) из JetBrains Marketplace или найдите "Neo Code" в `Settings → Plugins` в любой IDE JetBrains.
...
Запускайте Neo из веба без локальной машины на [app.neo.khulnasoft.com/cloud](https://app.neo.khulnasoft.com/cloud).
...
Настройте автоматические AI-ревью кода для ваших pull request на [app.neo.khulnasoft.com/code-reviews](https://app.neo.khulnasoft.com/code-reviews).
...
Запустите своего постоянно активного AI-агента на [app.neo.khulnasoft.com/claw](https://app.neo.khulnasoft.com/claw).
...
Скачайте последний бинарный файл со [страницы Releases](https://github.com/Neopilot-Ai/neocode/releases).

| Платформа | Файл |
|---|---|
| Windows (большинство ПК) | `neo-windows-x64.zip` |
...
Примечания: `x64-baseline` — совместимая сборка для старых CPU без AVX. `musl` — статически связанная сборка для Alpine или минимальных Docker-образов без glibc. `neo-vscode-*.vsix` — пакет расширения VS Code, а не CLI. Архивы `Source code` предназначены для сборки из исходного кода.

</details>

### Agents

Neo поставляется со специализированными agents, между которыми можно переключаться в зависимости от задачи. Вы также можете создавать собственные agents.
...
Подробнее об [agents и пользовательских agents](https://neo.khulnasoft.com/docs/code-with-ai/agents/using-agents).
...
Запустите `neo run` с `--auto` для полностью автономной работы без prompts, предназначенной для CI/CD-пайплайнов:
...
Для настройки и всего остального перейдите к [документации](https://neo.khulnasoft.com/docs).

### Участие

Мы приветствуем вклад разработчиков, авторов и всех желающих. Начните с [Contributing Guide](/CONTRIBUTING.md), чтобы настроить окружение, изучить стандарты кода и узнать, как открыть pull request. См. [RELEASING.md](../RELEASING.md) для процесса релиза расширения VS Code и CLI, а также [packages/neo-jetbrains/RELEASING.md](../packages/neo-jetbrains/RELEASING.md) для плагина JetBrains.
...
<details>
<summary>Откуда появился Neo CLI?</summary>

Neo CLI — это fork [OpenCode](https://github.com/anomalyco/opencode), расширенный для работы в платформе agentic engineering Neo.

</details>

---

**Присоединяйтесь к сообществу** [Discord](https://neo.khulnasoft.com/discord) | [X](https://x.com/neocode) | [Reddit](https://www.reddit.com/r/neocode/)
