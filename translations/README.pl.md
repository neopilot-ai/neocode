...
Neo Code to agent kodujący z AI, który działa wszędzie tam, gdzie pracujesz: w [VS Code](https://neo.khulnasoft.com/landing/vs-code), [JetBrains](https://neo.khulnasoft.com/features/jetbrains-native) i [CLI](https://neo.khulnasoft.com/cli). Jest open source i ma otwarte ceny. Wybierasz spośród ponad 500 modeli, przełączasz się między nimi w trakcie zadania i płacisz stawkę dostawcy modelu bez narzutów. Do rozpoczęcia nie są wymagane klucze API.

### Instalacja

Wybierz, gdzie chcesz uruchomić Neo.
...
Zainstaluj bezpośrednio [rozszerzenie Neo Code](vscode:extension/neocode.neo-code) albo pobierz je z [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=neocode.Neo-Code). Utwórz konto, a otrzymasz dostęp do ponad 500 modeli, w tym GPT-5.5, Claude Opus 4.7, Claude Sonnet 4.6 i Gemini 3.1 Pro Preview, wszystkie w cenach dostawców.
...
Następnie uruchom `neo` w dowolnym katalogu projektu.
...
Zainstaluj [plugin Neo Code](https://plugins.jetbrains.com/plugin/28350-neo-code) z JetBrains Marketplace albo wyszukaj "Neo Code" w `Settings → Plugins` w dowolnym IDE JetBrains.
...
Uruchom Neo z poziomu przeglądarki, bez lokalnej maszyny, na [app.neo.khulnasoft.com/cloud](https://app.neo.khulnasoft.com/cloud).
...
Skonfiguruj automatyczne przeglądy kodu AI dla swoich pull requestów na [app.neo.khulnasoft.com/code-reviews](https://app.neo.khulnasoft.com/code-reviews).
...
Uruchom swojego zawsze aktywnego agenta AI na [app.neo.khulnasoft.com/claw](https://app.neo.khulnasoft.com/claw).
...
Pobierz najnowszy plik binarny ze [strony Releases](https://github.com/Neopilot-Ai/neocode/releases).

| Platforma | Zasób |
|---|---|
| Windows (większość PC) | `neo-windows-x64.zip` |
...
Uwagi: `x64-baseline` to build zgodności dla starszych CPU bez AVX. `musl` to statycznie linkowany build dla Alpine lub minimalnych obrazów Docker bez glibc. `neo-vscode-*.vsix` to pakiet rozszerzenia VS Code, nie CLI. Archiwa `Source code` służą do budowania ze źródeł.

</details>

### Agents

Neo zawiera wyspecjalizowane agents, między którymi możesz przełączać się zależnie od zadania. Możesz też tworzyć własne niestandardowe agents.
...
Dowiedz się więcej o [agents i niestandardowych agents](https://neo.khulnasoft.com/docs/code-with-ai/agents/using-agents).
...
Uruchom `neo run` z `--auto`, aby działać w pełni autonomicznie bez promptów, z myślą o pipeline'ach CI/CD:
...
Konfigurację i wszystko inne znajdziesz w [dokumentacji](https://neo.khulnasoft.com/docs).

### Wkład

Zapraszamy do wkładu programistów, autorów i wszystkich innych. Zacznij od [Contributing Guide](/CONTRIBUTING.md), aby skonfigurować środowisko, poznać standardy kodowania i sposób otwierania pull requestów. Zobacz [RELEASING.md](../RELEASING.md) dla procesu wydawania rozszerzenia VS Code i CLI oraz [packages/neo-jetbrains/RELEASING.md](../packages/neo-jetbrains/RELEASING.md) dla pluginu JetBrains.
...
<details>
<summary>Skąd pochodzi Neo CLI?</summary>

Neo CLI jest forkiem [OpenCode](https://github.com/anomalyco/opencode), rozszerzonym do działania w platformie agentic engineering Neo.

</details>

---

**Dołącz do społeczności** [Discord](https://neo.khulnasoft.com/discord) | [X](https://x.com/neocode) | [Reddit](https://www.reddit.com/r/neocode/)
