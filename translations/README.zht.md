...
Neo Code 是一個 AI 編碼代理，可在你工作的任何地方使用：[VS Code](https://neo.khulnasoft.com/landing/vs-code)、[JetBrains](https://neo.khulnasoft.com/features/jetbrains-native) 和 [CLI](https://neo.khulnasoft.com/cli)。它是開源專案，並採用開放定價。你可以從 500 多個模型中選擇，在任務中途切換模型，並按模型供應商的價格付費，沒有加價。開始使用不需要 API 金鑰。
...
直接安裝 [Neo Code 擴充功能](vscode:extension/neocode.neo-code)，或從 [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=neocode.Neo-Code) 取得。建立帳戶後，你可以按供應商價格使用 500 多個模型，包括 GPT-5.5、Claude Opus 4.7、Claude Sonnet 4.6 和 Gemini 3.1 Pro Preview。
...
從 JetBrains Marketplace 安裝 [Neo Code 外掛](https://plugins.jetbrains.com/plugin/28350-neo-code)，或在任何 JetBrains IDE 的 `Settings → Plugins` 中搜尋 "Neo Code"。
...
無需本機電腦，在 Web 上透過 [app.neo.khulnasoft.com/cloud](https://app.neo.khulnasoft.com/cloud) 執行 Neo。
...
在 [app.neo.khulnasoft.com/code-reviews](https://app.neo.khulnasoft.com/code-reviews) 為你的 Pull Request 設定自動 AI 程式碼審查。
...
在 [app.neo.khulnasoft.com/claw](https://app.neo.khulnasoft.com/claw) 啟動你的常駐 AI 代理。
...
從 [Releases 頁面](https://github.com/Neopilot-Ai/neocode/releases) 下載最新二進位檔。

| 平台 | 資源 |
|---|---|
| Windows（大多數 PC） | `neo-windows-x64.zip` |
...
注意：`x64-baseline` 是面向不支援 AVX 的舊 CPU 的相容性建置。`musl` 是面向 Alpine 或沒有 glibc 的極簡 Docker 映像的靜態連結建置。`neo-vscode-*.vsix` 是 VS Code 擴充功能套件，不是 CLI。`Source code` 封存檔用於從原始碼建置。

</details>

### Agents

Neo 內建可依任務切換的專用 Agents。你也可以建立自己的自訂 Agents。
...
深入了解 [agents 和自訂 agents](https://neo.khulnasoft.com/docs/code-with-ai/agents/using-agents)。
...
使用 `--auto` 執行 `neo run`，可在 CI/CD 管線中進行無提示的完全自主操作：
...
關於設定和其他內容，請查看[文件](https://neo.khulnasoft.com/docs)。

### 貢獻

歡迎開發者、作者以及所有人參與貢獻。請先閱讀 [Contributing Guide](/CONTRIBUTING.md)，了解環境設定、程式碼標準以及如何建立 Pull Request。VS Code 擴充功能和 CLI 的發布流程請參閱 [RELEASING.md](../RELEASING.md)，JetBrains 外掛請參閱 [packages/neo-jetbrains/RELEASING.md](../packages/neo-jetbrains/RELEASING.md)。
...
<details>
<summary>Neo CLI 從何而來？</summary>

Neo CLI 是 [OpenCode](https://github.com/anomalyco/opencode) 的 fork，並增強為可在 Neo agentic engineering 平台中使用。

</details>

---

**加入社群** [Discord](https://neo.khulnasoft.com/discord) | [X](https://x.com/neocode) | [Reddit](https://www.reddit.com/r/neocode/)
