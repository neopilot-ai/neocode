 ...
 * remaining Neo Promise facades are migrated away. It also prevents tests
...
const testAllow: Record<string, { count: number; reason: string }> = {
  ...
  "neocode/config-resilience.test.ts": { count: 4, reason: "existing runtime integration test" },
  "neocode/config-validation.test.ts": { count: 2, reason: "existing runtime integration test" },
  "neocode/cli-shutdown.test.ts": { count: 1, reason: "mocked runtime boundary for shutdown unit tests" },
  "neocode/plan-followup.test.ts": { count: 3, reason: "existing runtime integration test" },
  "neocode/session-compaction-chunks.test.ts": {
    count: 2,
    reason: "disk-backed instance integration test cleanup",
  },
  "neocode/session-fork-remap.test.ts": {
    count: 2,
    reason: "disk-backed instance integration test cleanup",
  },
  "neocode/session/platform-attribution.test.ts": { count: 2, reason: "existing runtime integration test" },
  "neocode/session-prompt-queue.test.ts": { count: 6, reason: "prompt queue legacy instance bridge regression" },
  "server/experimental-session-list.test.ts": { count: 2, reason: "Neo session list integration test" },
  "neocode/server/cloud-session-import.test.ts": { count: 5, reason: "full app cloud import transaction integration" },
  "neocode/server/listener-runtime.test.ts": { count: 4, reason: "listener and AppRuntime integration test" },
  "tool/recall.test.ts": { count: 11, reason: "existing runtime integration test" },
}

const owned = (file: string) => file.startsWith("neocode/") || file.startsWith("neo-sessions/")
