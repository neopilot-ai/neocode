...
/**
 * Verifies that every Neo-specific change in shared upstream-owned source files
 * is annotated with a neocode_change marker.
 *
 * Usage:
 ...
 *   - contains a neocode_change marker comment           (inline annotation)
 *   - falls inside a neocode_change start/end block      (block annotation)
 ...
 *
 * Exempt paths (no markers needed — entirely Neo-specific):
 *   - packages/opencode/src/neocode/**
 *   - packages/opencode/test/neocode/**
 *   - Any path containing "neocode" in directory or filename
 *   - Any path with a directory starting with "neo-" (e.g. neo-sessions/)
 *   - script/upstream/**
 *   - Neo-specific annotation checker support files
 */
...
function isSource(file: string) {
  ...
  if (ext) return false
  return content(file).startsWith("#!") // neocode_change
}

// Parses the unified=0 diff for `file` against `base` and returns:
//   - added: every added line number on HEAD
//   - revert: true when the file's diff removes any neocode_change marker.
//     In that case the changes are reverting Neo modifications back to the
...
//     Neo construct (e.g. `unixSkip(` → `unix(`) often live in different
...
// Matches the start of a neocode_change marker in JS, JSX, YAML, TOML, and shell comments.
...
function coveredLines(text: string): { lines: string[]; covered: Set<number> } {
  ...
  // Whole-file annotation: first non-shebang non-empty line is a neocode_change - new file marker.
...
for (const file of files) {
  ...
  if (revert) continue // neocode_change - file is reverting Neo modifications back to upstream

  const text = content(file) // neocode_change
...
if (violations.length === 0) {
  console.log("All shared upstream changes are annotated with neocode_change markers.")
  process.exit(0)
}

console.error(
  [
    "Unannotated Neo changes found in shared upstream files:",
    "",
    ...violations,
    "",
    "Every Neo-specific change in shared upstream source files must be annotated.",
    "",
    "Checked paths:",
    ...SCOPES.map((scope) => `  - ${scope}/**`),
    "",
    "Inline (single line):",
    "  const url = Flag.NEO_MODELS_URL || 'https://models.dev' // neocode_change",
    "",
    ...
    "  - packages/opencode/src/neocode/**",
    "  - packages/opencode/test/neocode/**",
    "  - Any path containing 'neocode' in the directory or filename",
    "  - Any directory starting with 'neo-' (e.g. neo-sessions/)",
    "  - script/upstream/**",
    "  - Neo-specific annotation checker support files",
    "",
