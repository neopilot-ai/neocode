...
if (prop !== ver)
  throw new Error(`packages/neo-jetbrains/gradle.properties neo.jetbrains.version is ${prop}, expected ${ver}`)
if (!(await pinned())) {
...
const changelog = await Bun.file("packages/neo-jetbrains/CHANGELOG.md").text()
...
async function props() {
  const text = await Bun.file("packages/neo-jetbrains/gradle.properties").text()
  const line = text.split(/\r?\n/).find((item) => item.startsWith("neo.jetbrains.version="))
  const value = line?.split("=", 2)[1]?.trim()
  if (!value) throw new Error("packages/neo-jetbrains/gradle.properties is missing neo.jetbrains.version")
  return value
}
