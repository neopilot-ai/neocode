...
const OUTPUT = path.join(ROOT, "packages/neo-docs/source-links.md")

const check = process.argv.includes("--check")

const DIRS = [
  path.join(ROOT, "packages/neo-vscode/src"),
  path.join(ROOT, "packages/neo-vscode/webview-ui"),
...
const EXCLUDE_PATTERNS = [
  ...
  /^https?:\/\/api\.neo\.ai\/api\//,
  /^https?:\/\/supermassive-black-hole\.neoapps\.io\/v1\/session-export\//, // neocode_change
  /^https?:\/\/ingest\.neosessions\.ai/,
  /^https?:\/\/api\.openai\.com/,
  /^https?:\/\/api\.github\.com/,
  /^https?:\/\/api\.githubcopilot\.com/,
  /^https?:\/\/[^/]+\.openai\.azure\.com\/openai/, // neocode_change
...
const SKIP_FILES = ["check-forbidden-strings.ts"] // neocode_change
...
function shouldSkipFile(filepath: string): boolean {
  if (filepath === "packages/opencode/src/cli/cmd/account.ts") return true // neocode_change - command is not registered in Neo
  ...
  if (parts.includes("i18n") && path.basename(filepath) !== "en.ts") return true // neocode_change
...
if (check) {
  const committed = await Bun.file(OUTPUT)
    ...
    .catch(() => "")
  if (committed === output) {
    console.log("packages/neo-docs/source-links.md is up to date.")
    process.exit(0)
  }
  console.error(
    [
      "ERROR: packages/neo-docs/source-links.md is out of date.",
      "",
...
console.log(`Wrote ${sorted.length} unique URLs to packages/neo-docs/source-links.md`)
