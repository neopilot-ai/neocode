...
const props = new URL("../packages/neo-jetbrains/gradle.properties", import.meta.url).pathname
const log = new URL("../packages/neo-jetbrains/CHANGELOG.md", import.meta.url).pathname
...
if (!(await pinned())) {
  throw new Error("packages/neo-jetbrains/gradle.properties has neo.cli.pinned=false; JetBrains releases require neo.cli.pinned=true")
}
...
await $`git add packages/neo-jetbrains/gradle.properties packages/neo-jetbrains/CHANGELOG.md`
...
async function writeprops(ver: string) {
  ...
  const line = `neo.jetbrains.version=${ver}`
  const next = current.match(/^neo\.jetbrains\.version=/m)
    ? current.replace(/^neo\.jetbrains\.version=.*$/m, line)
    : `${current.trim()}\n${line}\n`
...
- Review \`packages/neo-jetbrains/gradle.properties\` and edit \`packages/neo-jetbrains/CHANGELOG.md\` before merging.
