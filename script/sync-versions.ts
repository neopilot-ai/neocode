...
// Sync every Neo version string across the monorepo to a single target.
//
...
// `.neo/command/upstream-manual-merge.md`). It's also handy mid-merge to
// rebase our version bumps onto any new Neo main releases.
//
// Usage:
...
//   - the five Neopilot-Ai download URLs inside that toml
//
// Intentionally NOT touched:
//   - `packages/neo-jetbrains/**` — the JetBrains plugin has its own release
...
for await (const rel of glob.scan({ cwd: root, onlyFiles: true })) {
  ...
  if (rel.startsWith("packages/neo-jetbrains/")) continue
...
if (await Bun.file(zed).exists()) {
  ...
  const next = text
    ...
    .replace(
      /https:\/\/github\.com\/Neopilot-Ai\/neocode\/releases\/download\/v[^/]+\//g,
      `https://github.com/Neopilot-Ai/neocode/releases/download/v${target}/`,
    )
  if (next !== text) {
