...
Scripts for automating the merge of upstream opencode changes into Neo.
...
bun run merge.ts --version v1.1.50 --base-branch catrielmuller/neo-opencode-v1.1.44
```
...
| `opencode-changesets.ts` | Generate Neo changesets from upstream opencode release notes |
| `fix-neocode-markers.ts` | Rebuild `neocode_change` markers for one file against the last merged upstream |
...
| Script | Description |
|---|---|
| `transforms/package-names.ts` | Transform opencode package names to neo |
| `transforms/preserve-versions.ts` | Preserve Neo's package versions |
| `transforms/keep-ours.ts` | Keep Neo's version of specific files |
| `transforms/skip-files.ts` | Skip/remove files that shouldn't exist in Neo |
| `transforms/transform-i18n.ts` | Transform i18n files with Neo branding |
| `transforms/transform-take-theirs.ts` | Take upstream + apply Neo branding for branding-only files |
| `transforms/transform-package-json.ts` | Enhanced package.json with Neo dependency injection and newest-Bun-wins reconciliation |
...
4. **Create branches**
   ...
   - `<author>/neo-opencode-<version>` - Merge target branch
   - `<author>/opencode-<version>` - Transformed upstream branch

5. **Apply ALL transformations to upstream branch (PRE-MERGE)**:
   - Remove files that should not exist in Neo (`skipFiles`)
   - Transform package names (opencode-ai -> @neocode/cli)
   - Preserve Neo's versions
   - Transform i18n files with Neo branding
   - Transform branding-only files (UI components, configs)
   - Transform package.json files (names, deps, Neo injections)
   - Transform script files (GitHub API references)
   - Transform extension files (Zed, etc.)
   - Transform web/docs files
   - Reset Neo-specific files

6. **Merge** transformed upstream into Neo branch
   - Since all branding transforms are applied pre-merge, conflicts should be minimal
   - Remaining conflicts are files with actual code differences (neocode_change markers)

7. **Auto-resolve** any remaining conflicts
   - Skip files that shouldn't exist in Neo
   - Keep Neo's version of specific files
...
```typescript
{
  ...
  packageMappings: [
    { from: "opencode-ai", to: "@neocode/cli" },
    { from: "@opencode-ai/cli", to: "@neocode/cli" },
    // ...
  ],

  // Files to always keep Neo's version (never take upstream)
  keepOurs: [
    "README.md",
    "CONTRIBUTING.md",
    "AGENTS.md",
  ...
  // Files to take upstream + apply Neo branding transforms
  takeTheirsAndTransform: [
    "packages/ui/src/**/*.tsx",
    // ...
  ],

  // Neo-specific directories (preserved)
  neoDirectories: [
    "packages/opencode/src/neocode",
    "packages/neo-gateway",
    "packages/neo-telemetry",
    // ...
  ],
}
```
...
**Key insight**: By applying all branding transforms to the upstream branch BEFORE merging, we eliminate most conflicts that would otherwise occur due to branding differences (OpenCode -> Neo).

### Transform Order (Pre-Merge)

The following transforms are applied to the opencode branch before merging:

1. **Skip files** - Remove upstream-only packages/files that should not exist in Neo
2. **Package names** - `opencode-ai` -> `@neocode/cli`, etc.
3. **Versions** - Preserve Neo's version numbers
4. **i18n files** - OpenCode -> Neo in user-visible strings
5. **Branding files** - UI components, configs with branding only
6. **package.json** - Names, dependencies, Neo injections
...
|---|---|---|
| i18n files | `i18n-transform` | Take upstream, apply Neo branding |
| UI components | `take-theirs-transform` | Take upstream, apply branding (no logic changes) |
| package.json | `package-transform` | Take upstream, transform names, inject Neo deps |
...
| README/docs | `keep-ours` | Keep Neo's version |
| GitHub workflows | `keep-ours` | Keep Neo's version (manual review) |
| Code with markers | `manual` | Has `neocode_change` markers, needs review |
...
- Neo had `Neo` branding
- Git saw these as conflicting changes

Now:

- We transform upstream to `Neo` branding BEFORE merge
- Both branches have the same branding
- Git sees no conflict for branding-only files

The only remaining conflicts are files with **actual code differences** - files with `neocode_change` markers that contain Neo-specific logic.

### Bun Version Safety

Root `package.json` reconciliation uses the newer valid `packageManager` Bun version from Neo and upstream. An older upstream version cannot downgrade Neo, while a newer upstream version is retained as an upgrade. Before the merge is finalized, `merge.ts` also validates the result against the pristine Neo base and upstream commit and aborts if the merged Bun version is lower than either input.
...
| `.worktrees/opencode-merge/neo-main` | The Neo base branch snapshot used for the merge |
...
- Neo-only paths: anything under `packages/neo-*/`, any `**/neocode/**` subdir, `script/upstream/`.
- Non-code assets: SVG, PNG, fonts, archives, lock files, etc. (see `SKIP_EXTENSIONS` / `SKIP_FILENAMES` in the script).
- Files covered by the merge config's `keepOurs` or `skipFiles` lists in `utils/config.ts` — these are intentionally preserved or removed in Neo and must not be bulk-reset.
...
| `markers-only` | Stripping `neocode_change` markers makes local match upstream | reset |
...
| `upstream-missing` | File does not exist upstream (neo-only, intentional) | skipped |
| `local-missing` | File tracked but missing locally (deleted in Neo) | skipped |
...
# Create PR: catrielmuller/neo-opencode-v1.1.44 -> main

# Second merge: v1.1.50 based on the previous PR (without waiting for approval)
bun run merge.ts --version v1.1.50 --base-branch catrielmuller/neo-opencode-v1.1.44

# Create PR: catrielmuller/neo-opencode-v1.1.50 -> catrielmuller/neo-opencode-v1.1.44
# OR: catrielmuller/neo-opencode-v1.1.50 -> main (once first PR is merged)
```
...
bun run analyze.ts --version v1.1.50 --base-branch catrielmuller/neo-opencode-v1.1.44

# 2. Run the merge
bun run merge.ts --version v1.1.50 --base-branch catrielmuller/neo-opencode-v1.1.44

# 3. Create PR from catrielmuller/neo-opencode-v1.1.50
#    - Target: catrielmuller/neo-opencode-v1.1.44 (if first PR not merged yet)
...
2. Look for `neocode_change` markers to identify Neo-specific code
3. Resolve conflicts, keeping Neo-specific changes
4. Stage and commit:
   ```bash
   git add -A
