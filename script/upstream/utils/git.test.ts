...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), "neo-upstream-git-"))
  process.chdir(dir)
...
test("finds previous compatibility commit for transformed base", async () => {
  ...
  await Bun.write("brand.txt", "neo A\n")
  const prior = await commit("refactor: neo compat for v1.0.0")
  ...
  await Bun.write("brand.txt", "neo B\n")
  await $`git add -A`.quiet()
  const tree = await writeTree()
  const next = await createCommit(tree, "refactor: neo compat for v1.0.1", prior)
...
test("finds previous compatibility commit when upstream tags diverge", async () => {
  ...
  await $`git checkout -b main ${old}`.quiet()
  await Bun.write("brand.txt", "neo 1.4.9\n")
  const ancient = await commit("refactor: neo compat for v1.4.9")
  await Bun.write("brand.txt", "neo 1.14.30\n")
  const prior = await commit("refactor: neo compat for v1.14.30")
