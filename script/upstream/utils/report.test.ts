...
test("recommends keep ours for Neo directories", () => {
  expect(getRecommendation("packages/neo-vscode/.prettierignore", [], []).recommendation).toBe("keep-ours")
  expect(getRecommendation("packages/neo-i18n/tsconfig.json", [], []).recommendation).toBe("keep-ours")
})
