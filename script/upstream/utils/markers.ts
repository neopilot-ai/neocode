#!/usr/bin/env bun
/**
 * Shared neocode_change marker helpers used by both the marker fixer and the
...
export const standalone = [
  /^\s*\/\/\s*neocode_change\b.*$/,
  /^\s*#\s*neocode_change\b.*$/,
  /^\s*\{?\s*\/\*\s*neocode_change\b.*\*\/\}?\s*$/,
]
export const start = /\bneocode_change\s+start\b/
export const end = /\bneocode_change\s+end\b/
export const freshmark = /\bneocode_change\s*-\s*new\s*file\b/
...
function strip(file: string, line: string): { line: string | null; mark?: string } {
  ...
  if (style(file) === "hash") return comment(line, [/^#\s*neocode_change\b/])
  return comment(line, [/^\{\/\*\s*neocode_change\b/, /^\/\*\s*neocode_change\b/, /^\/\/\s*neocode_change\b/])
}
...
function block(mode: Style, pad: string) {
  if (mode === "hash") return { start: `${pad}# neocode_change start`, end: `${pad}# neocode_change end` }
  if (mode === "jsx") return { start: `${pad}{/* neocode_change start */}`, end: `${pad}{/* neocode_change end */}` }
  if (mode === "block") return { start: `${pad}/* neocode_change start */`, end: `${pad}/* neocode_change end */` }
  return { start: `${pad}// neocode_change start`, end: `${pad}// neocode_change end` }
}

function note(mode: Style) {
  if (mode === "hash") return " # neocode_change"
  if (mode === "jsx") return " {/* neocode_change */}"
  if (mode === "block") return " /* neocode_change */"
  return " // neocode_change"
}
...
export function fresh(file: string, clean: Clean) {
  ...
  const line = clean.marks.file ?? (mode === "hash" ? "# neocode_change - new file" : "// neocode_change - new file")
...
export async function changed(base: Text, head: Text, opts?: { ignoreWhitespace?: boolean }): Promise<Diff> {
  const dir = await mkdtemp(path.join(tmpdir(), "neo-markers-"))
