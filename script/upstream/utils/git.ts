...
function compatTag(message: string): string | null {
  const prefix = "refactor: neo compat for "
...
export async function findLatestCompatCommit(base: string, target: string): Promise<CompatBase | null> {
  const grep = "^refactor: neo compat for "
 ...
 * Used to clean build artifacts from Neo-specific directories after checking
...
/**
 * Check if the "ours" version of a conflicted file contains neocode_change markers.
 ...
 */
export async function oursHasNeocodeChanges(file: string): Promise<boolean> {
  const result = await $`git show :2:${file}`.quiet().nothrow()
  if (result.exitCode !== 0) return false
  return result.stdout.toString().includes("neocode_change")
}
...
export async function trainRerere(grep: string): Promise<number> {
  ...
  const dir = join(tmpdir(), `neo-rerere-train-${randomUUID()}`)
