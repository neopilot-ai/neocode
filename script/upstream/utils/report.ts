 ...
 * Pass currentContent (our version of the file) to detect neocode_change markers
...
export function getRecommendation(
  ...
  // Check if file should be skipped entirely (doesn't exist in Neo, shouldn't be added)
  if (shouldSkipFile(path, skipFiles)) {
    return {
      recommendation: "skip",
      reason: "File should be skipped (does not exist in Neo fork)",
    }
  }
  ...
  if (keepOurs.some((pattern) => path.includes(pattern) || path === pattern)) {
    return {
      ...
      reason: "File is Neo-specific and should not be overwritten",
    }
  }

  // Neo directories should always keep ours
  if (
    matches(
      path,
      defaultConfig.neoDirectories.map((dir) => `${dir}/**`),
    )
  ) {
    return {
      recommendation: "keep-ours",
      reason: "File is in a Neo-specific directory",
    }
  }
  ...
  if (shouldTakeTheirsTransform(path)) {
    // If our version has neocode_change markers, flag for manual review
    if (currentContent?.includes("neocode_change")) {
      return {
        recommendation: "manual",
        reason: "File has neocode_change markers — auto-transform skipped, needs manual review",
      }
    }
    return {
      recommendation: "take-theirs-transform",
      reason: "Branding-only file: take upstream and apply Neo branding transforms",
    }
  }

  switch (type) {
    case "i18n":
      // i18n files that have neocode_change markers need manual review
      if (currentContent?.includes("neocode_change")) {
        return {
          recommendation: "manual",
          reason: "i18n file has neocode_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "i18n-transform",
        reason: "i18n file: take upstream translations and apply Neo branding",
      }
    case "script":
      if (currentContent?.includes("neocode_change")) {
        return {
          recommendation: "manual",
          reason: "Script file has neocode_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
    ...
    case "extension":
      if (currentContent?.includes("neocode_change")) {
        return {
          recommendation: "manual",
          reason: "Extension file has neocode_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "extension-transform",
        reason: "Extension file: take upstream and apply Neo branding",
      }
    case "web":
      if (currentContent?.includes("neocode_change")) {
        return {
          recommendation: "manual",
          reason: "Web/docs file has neocode_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "web-transform",
        reason: "Web/docs file: take upstream and apply Neo branding",
      }
    case "markdown":
      return {
        recommendation: "keep-ours",
        reason: "Markdown files are typically Neo-specific documentation",
      }
    case "package":
      if (currentContent?.includes("neocode_change")) {
        return {
          recommendation: "manual",
          reason: "package.json has neocode_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "package-transform",
        reason: "Package.json: take upstream, transform names, inject Neo deps, preserve version",
      }
    case "code":
      return {
        recommendation: "manual",
        reason: "Code files need manual review for neocode_change markers",
      }
    case "config":
      return {
        recommendation: "manual",
        reason: "Config files may have Neo-specific settings",
      }
    default:
      return {
        recommendation: "manual",
...
export async function analyzeConflicts(
  ...
  for (const path of files) {
    const type = classifyFile(path)
    // Read current file content (our version) to detect neocode_change markers
...
export function generateMarkdownReport(report: ConflictReport): string {
  ...
  for (const rec of order) {
    ...
    const titleMap: Record<ConflictFile["recommendation"], string> = {
      ...
      "i18n-transform": "i18n Transform (Auto-Apply Neo Branding)",
      "take-theirs-transform": "Take Upstream + Neo Branding (Auto)",
      ...
      "keep-ours": "Keep Neo Version (Ours)",
