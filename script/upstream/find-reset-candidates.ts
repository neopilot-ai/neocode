 ...
 *   - markers-only        : only diff is neocode_change markers wrapping
 ...
 *   - upstream-missing    : file does not exist upstream (neo-only, skipped)
...
const NEO_ONLY_PATHSPECS = [
  ":(exclude,glob)packages/neo-*/**",
  ":(exclude,glob)**/neocode/**",
...
async function candidates(commit: string, scope: string | undefined, top: string): Promise<CandidateSet> {
  const pathspecs = [scope ?? ".", ...NEO_ONLY_PATHSPECS]
...
/**
 * Files the upstream merge config marks as "keep ours" (Neo-specific preserved
 * versions) or "skip" (upstream-only, removed in Neo) should never be touched
 * by the bulk resetter. They show up in `git diff` against raw upstream but
 * resetting them would undo deliberate Neo decisions.
 */
