 ...
 *
 * Automates the process of merging upstream opencode changes into Neo.
 *
 * Usage:
...
function logWorktrees(refs: worktree.RefInfo, input: worktree.RefInput, baseName: string): void {
  ...
  logger.info(`  neo-main:  ${refs.main} (${baseName}, ${input.base.slice(0, 8)})`)
  ...
  logger.info(`  - Neo base main: ${refs.main}`)
...
async function main() {
  ...
  logger.header("Neo Upstream Merge Tool")
  ...
  if (keepOursCount > 0) {
    conflictReport.recommendations.push(`${keepOursCount} files will keep Neo's version`)
  }
  if (codemodCount > 0) {
  ...
  const neoVersion = await version.getCurrentNeoVersion()
  ...
  // Create Neo merge branch
  const neoBranch = `${author}/neo-opencode-${targetVersion.tag}`
  const neoBackup = await git.backupAndDeleteBranch(neoBranch)
  if (neoBackup) {
    logger.info(`Backed up existing branch to: ${neoBackup}`)
  }
  await git.createBranch(neoBranch)

  if (options.push) {
    await git.push(config.originRemote, neoBranch, true)
  }
  logger.info(`Created Neo branch: ${neoBranch}`)
  ...
  // This reduces conflicts by transforming upstream code to Neo conventions BEFORE merging
  logger.step(6, 8, "Applying transformations to opencode branch (pre-merge)...")

  logger.info("Removing files skipped in Neo...")
  ...
  // 6a. Transform package names (opencode-ai -> @neocode/cli)
  ...
  // 6b. Preserve Neo versions
  logger.info("Preserving Neo versions...")
  const versionResults = await preserveAllVersions({
    dryRun: false,
    verbose: options.verbose,
    targetVersion: neoVersion,
  })
  logger.success(`Preserved versions in ${versionResults.length} files`)

  // 6c. Transform i18n files (OpenCode -> Neo branding)
  ...
  if (i18nPreCount > 0) {
    logger.success(`Transformed ${i18nPreCount} i18n files with Neo branding`)
  }
  ...
  if (brandingCount > 0) {
    logger.success(`Transformed ${brandingCount} files with Neo branding`)
  }

  // 6f. Transform package.json files (names, deps, Neo injections)
  ...
  // 6j. Reset keep-ours files to Neo's version
  logger.info("Resetting Neo-specific files...")
  const keepOursResults = await resetToOurs(config.keepOurs, { dryRun: false, verbose: options.verbose })
  logger.success(`Reset ${keepOursResults.length} files to Neo's version`)
  ...
  // Clean untracked build artifacts from Neo-specific directories.
  ...
  logger.info("Cleaning Neo-specific directory artifacts...")
  await git.cleanDirectories(config.neoDirectories)

  // Commit all transformations
  await git.stageAll()
  const compatMessage = `refactor: neo compat for ${targetVersion.tag}`
  if (prior) {
  ...
  // Step 7: Merge into Neo branch
  logger.step(7, 8, "Merging into Neo branch...")

  await git.checkout(neoBranch)
  if (prior) {
    const linked = await git.recordAncestor(targetVersion.commit, `merge: record upstream ${targetVersion.tag}`)
    if (linked) logger.info(`Recorded upstream ${targetVersion.tag} as Neo branch ancestry`)
  }
  ...
  if (!mergeResult.success) {
    ...
    // These are likely files with neocode_change markers or actual logic differences.

    // Step 7a: Skip files that shouldn't exist in Neo
    logger.info("Removing files that shouldn't exist in Neo...")
    ...
    logger.info("Keeping Neo-specific files...")
    ...
    if (autoResolved.length > 0) {
      logger.success(`Auto-resolved ${autoResolved.length} conflicts (kept Neo's version)`)
    }
    ...
    // Files with neocode_change markers are flagged for manual resolution instead.
    ...
    if (conflictedFiles.length > 0) {
      ...
      // neocode_change markers, plus JSON/YAML/TOML key merges and other
      ...
      if (i18nFlagged.length > 0) {
        logger.warn(`${i18nFlagged.length} i18n file(s) have neocode_change markers — flagged for manual resolution`)
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (takeFlagged.length > 0) {
          logger.warn(
            `${takeFlagged.length} branding file(s) have neocode_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (pkgFlagged.length > 0) {
          logger.warn(
            `${pkgFlagged.length} package.json file(s) have neocode_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (scriptFlagged.length > 0) {
          logger.warn(
            `${scriptFlagged.length} script file(s) have neocode_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (extFlagged.length > 0) {
          logger.warn(
            `${extFlagged.length} extension file(s) have neocode_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (webFlagged.length > 0) {
          logger.warn(
            `${webFlagged.length} web/docs file(s) have neocode_change markers — flagged for manual resolution`,
          )
    ...
    // Combine git-reported conflicts with files flagged due to neocode_change markers
    const allManual = [...new Set([...remaining, ...flaggedFiles])]
    if (allManual.length > 0) {
      if (flaggedFiles.length > 0) {
        logger.warn(`${flaggedFiles.length} file(s) were flagged because they contain neocode_change markers:`)
        logger.list(flaggedFiles)
        logger.info("  These files have intentional Neo-specific changes. Keep our version or merge carefully.")
        logger.info("")
      }
      ...
      logger.info("")
      logger.info("These conflicts contain neocode_change markers or actual code differences.")
      ...
      conflictReport.mergeBranch = neoBranch
      const reportPath = `upstream-merge-report-${targetVersion.version}.md`
      await report.saveReport(conflictReport, reportPath)
      logger.success(`Report saved to ${reportPath}`)
      ...
      logger.info(`  3. git push ${config.originRemote} ${neoBranch}`)
      logger.info("  4. Create PR from " + neoBranch + " to " + config.baseBranch)
      logger.info("")
  ...
  if (options.push) {
    await git.push(config.originRemote, neoBranch)
    logger.success(`Pushed ${neoBranch} to ${config.originRemote}`)
  }
  ...
  logger.info(`Neo branch: ${neoBranch}`)
  ...
  if (remainingConflicts.length > 0) {
    ...
    logger.info("  2. Create PR from " + neoBranch + " to " + config.baseBranch)
  }
