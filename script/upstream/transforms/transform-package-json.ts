#!/usr/bin/env bun
/**
 * Enhanced package.json transform with Neo dependency injection
 *
 * This script handles package.json conflicts by:
 * 1. Taking upstream's version (to get new dependencies)
 * 2. Transforming package names (opencode -> neo)
 * 3. Injecting Neo-specific dependencies
 * 4. Preserving Neo's version number
 * 5. Preserving overrides and patchedDependencies
 * 6. Preserving Neo's repository configuration
...
import { oursHasNeocodeChanges } from "../utils/git"
...
export function fixPackageManager(
  ...
  changes.push(`packageManager: ${prior} -> ${next} (preserved Neo pin)`)
 ...
 *
 * Key order preserves ours' order first (so neo-only deps stay in their
  ...
  if (ours) {
    for (const [name, ourVersion] of Object.entries(ours)) {
      ...
      if (theirVersion === undefined) {
        ...
        changes.push(`${section}: preserved ${name}@${ourVersion} (neo-only)`)
        continue
      }
      ...
      if (cmp === null) {
        ...
        result[name] = ourVersion
        changes.push(`${section}: ${name} ${theirVersion} -> ${ourVersion} (neo newer)`)
      } else {
        result[name] = theirVersion
...
// Neo-specific dependencies to inject into specific packages
// NOTE: When adding new Neo-specific workspace dependencies (packages starting with @neocode/neo-*),
// add them here to prevent them from being removed during upstream merges
const NEO_DEPENDENCIES: Record<string, Record<string, string>> = {
  // packages/opencode/package.json needs these
  "packages/opencode/package.json": {
    "@neocode/neo-gateway": "workspace:*",
    "@neocode/neo-telemetry": "workspace:*",
  },
}

// Neo-specific bin entries to set on specific packages
const NEO_BIN: Record<string, Record<string, string>> = {
  "packages/opencode/package.json": {
    neo: "./bin/neo",
    neocode: "./bin/neo",
  },
...
const TRANSFORM_PACKAGE_NAMES: Record<string, string> = {
  "package.json": "@neocode/neo",
  "packages/opencode/package.json": "@neocode/cli",
  "packages/plugin/package.json": "@neocode/plugin",
  "packages/sdk/js/package.json": "@neocode/sdk",
}

// Neo-specific scripts to preserve from the base branch per package.json.
...
// Neo doesn't ship (desktop-electron, console/app, app) and would otherwise
...
// in by upstream features (e.g. desktop Sentry integration) that Neo doesn't
...
/**
 * Re-apply Neo-specific scripts on top of the upstream-shaped scripts block,
 * and prune upstream-only scripts that target packages Neo doesn't ship.
 */
export function fixScripts(
  ...
  for (const name of DELETE_UPSTREAM_SCRIPTS[path] || []) {
    if (theirs[name]) {
      delete theirs[name]
      changes.push(`scripts.${name}: removed (upstream-only, no Neo target)`)
    }
  }
...
/**
 * Prune upstream-only catalog entries that have no consumers in Neo.
 */
export function fixCatalog(pkg: Record<string, unknown>, path: string, changes: string[]): void {
  ...
  for (const name of DELETE_UPSTREAM_CATALOG[path] || []) {
    if (cat[name]) {
      delete cat[name]
      changes.push(`workspaces.catalog.${name}: removed (upstream-only, no Neo consumer)`)
    }
  }
}
...
export async function transformPackageJson(file: string, options: PackageJsonOptions = {}): Promise<PackageJsonResult> {
  ...
  // If our version has neocode_change markers, flag for manual resolution
  if (await oursHasNeocodeChanges(file)) {
    warn(`${file} has neocode_change markers — skipping auto-transform, needs manual resolution`)
    return { file, action: "flagged", changes: [], dryRun: false }
  }

  try {
    // Save Neo's version BEFORE taking theirs
    ...
    // 2. Preserve Neo version if requested
    if (options.preserveVersion !== false) {
      const neoVersion = await getCurrentVersion()
      if (pkg.version !== neoVersion) {
        changes.push(`version: ${pkg.version} -> ${neoVersion}`)
        pkg.version = neoVersion
      }
    }
    ...
    if (ourPkg) {
      ...
      // 5. Preserve patchedDependencies (Neo-specific, upstream won't have these)
      ...
      // 6. Preserve repository (Neo-specific, upstream doesn't have this)
      const ourRepo = ourPkg.repository
      if (ourRepo && JSON.stringify(pkg.repository) !== JSON.stringify(ourRepo)) {
        pkg.repository = ourRepo
        changes.push(`repository: preserved Neo's repository configuration`)
      }

      fixMetadata(pkg, relativePath, ourPkg, changes)

      // 7. Handle workspaces for root package.json
      // Neo has removed hosted platform packages (console/*, slack, etc.)
      // so we need to preserve Neo's workspace configuration instead of taking upstream's
      ...
      if (relativePath === "package.json" && ourWorkspaces?.packages) {
        ...
        changes.push(`workspaces.packages: preserved Neo's workspace configuration`)
      }
    ...
    // 7. Transform dependency names (opencode -> neo)
    if (pkg.dependencies) {
    ...
    // 8. Inject Neo-specific dependencies
    const neoDeps = NEO_DEPENDENCIES[relativePath]
    if (neoDeps) {
      pkg.dependencies = pkg.dependencies || {}
      for (const [name, version] of Object.entries(neoDeps)) {
    ...
    // 9. Set Neo-specific bin entries
    const neoBin = NEO_BIN[relativePath]
    if (neoBin) {
      pkg.bin = neoBin
      changes.push(`bin: set Neo bin entries`)
    }
...
/**
 * Get Neo's package.json from the base branch (main) for comparison
 * Used during pre-merge to compare upstream versions against Neo's versions
 */
async function getNeoPackageJson(path: string, baseBranch = "main"): Promise<Record<string, unknown> | null> {
  try {
 ...
 * This function merges Neo's versions with upstream, using "newest wins" strategy
 */
export async function transformAllPackageJson(options: PackageJsonOptions = {}): Promise<PackageJsonResult[]> {
  ...
  for await (const path of glob.scan({ absolute: false })) {
    ...
    try {
      ...
      // Get Neo's version from base branch for comparison
      const neoPkg = await getNeoPackageJson(path)
      ...
      fixPackageManager(pkg, path, neoPkg, changes)
      ...
      // 3. Merge dependencies with "newest wins" strategy (if Neo has this file)
      if (neoPkg) {
        pkg.dependencies = mergeWithNewestVersions(
          neoPkg.dependencies as Record<string, string> | undefined,
          pkg.dependencies,
          changes,
          "dependencies",
        )
        ...
        pkg.devDependencies = mergeWithNewestVersions(
          neoPkg.devDependencies as Record<string, string> | undefined,
          pkg.devDependencies,
          changes,
          "devDependencies",
        )
        ...
        pkg.peerDependencies = mergeWithNewestVersions(
          neoPkg.peerDependencies as Record<string, string> | undefined,
          pkg.peerDependencies,
          changes,
          "peerDependencies",
        )
        ...
        // 4. Preserve/merge overrides
        const neoOverrides = neoPkg.overrides as Record<string, string> | undefined
        if (neoOverrides || pkg.overrides) {
          pkg.overrides = mergeWithNewestVersions(neoOverrides, pkg.overrides, changes, "overrides")
        }

        // 5. Preserve patchedDependencies (Neo-specific, upstream won't have these)
        const neoPatchedDeps = neoPkg.patchedDependencies as Record<string, string> | undefined
        if (neoPatchedDeps) {
          pkg.patchedDependencies = pkg.patchedDependencies || {}
          for (const [name, patch] of Object.entries(neoPatchedDeps)) {
            if (!pkg.patchedDependencies[name]) {
              pkg.patchedDependencies[name] = patch
              changes.push(`patchedDependencies: preserved ${name}`)
            }
        ...
        // 6. Preserve repository (Neo-specific, upstream doesn't have this)
        const neoRepo = neoPkg.repository
        if (neoRepo && JSON.stringify(pkg.repository) !== JSON.stringify(neoRepo)) {
          pkg.repository = neoRepo
          changes.push(`repository: preserved Neo's repository configuration`)
        }

        fixMetadata(pkg, path, neoPkg, changes)

        // 7. Handle workspaces for root package.json
        // Neo has removed hosted platform packages (console/*, slack, etc.)
        // so we need to preserve Neo's workspace configuration instead of taking upstream's
        const neoWorkspaces = neoPkg.workspaces as
        ...
        if (path === "package.json" && neoWorkspaces?.packages) {
          pkg.workspaces = pkg.workspaces || {}
          pkg.workspaces.packages = neoWorkspaces.packages
          changes.push(`workspaces.packages: preserved Neo's workspace configuration`)
        }

        fixScripts(pkg, path, neoPkg, changes)

        // Merge catalog with "newest wins" strategy
        if (neoWorkspaces?.catalog || upstreamWorkspaces?.catalog) {
          pkg.workspaces = pkg.workspaces || {}
          pkg.workspaces.catalog = mergeWithNewestVersions(
            neoWorkspaces?.catalog,
      ...
      // 8. Inject Neo-specific dependencies
      const neoDeps = NEO_DEPENDENCIES[path]
      if (neoDeps) {
        pkg.dependencies = pkg.dependencies || {}
        for (const [name, version] of Object.entries(neoDeps)) {
          if (!pkg.dependencies[name]) {
 ...
 * neocode_change markers (manual review needed).
 */
export async function reconcilePackageJsonFromRefs(
  ...
  if (await oursHasNeocodeChanges(file)) {
    warn(`${file} has neocode_change markers — skipping reconcile, needs manual resolution`)
...
export async function reconcileAllPackageJson(options: ReconcileOptions): Promise<PackageJsonResult[]> {
  ...
  // upstream-only and neo-only files alike.
