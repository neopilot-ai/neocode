 ...
 *
 * These are files that exist in upstream but should NOT exist in Neo fork.
 ...
 * - Kept deleted if they don't exist in Neo
 */
...
export async function skipFiles(options: SkipOptions = {}): Promise<SkipResult[]> {
  ...
  for (const file of allFiles) {
    ...
    // Check if file existed in Neo before merge (HEAD~1 or the merge base)
    const existedInNeo = options.force ? false : await fileExistsInRef(file, "HEAD")

    if (existedInNeo) {
      debug(`Skipping ${file} - exists in Neo, not removing`)
      results.push({ file, action: "skipped", dryRun: options.dryRun ?? false })
      continue
    }

    // File doesn't exist in Neo - should be removed
    if (options.dryRun) {
