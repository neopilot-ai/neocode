#!/usr/bin/env bun
/**
 * Transform i18n translation files with Neo branding
 *
 * This script handles i18n files by:
 * 1. Taking upstream's version as the base (to get new translation keys)
 * 2. Applying intelligent string replacements for Neo branding
 * 3. Preserving lines marked with `// neocode_change`
 *
 * String replacement rules:
 * - opencode.ai -> neo.khulnasoft.com (domain)
 * - app.opencode.ai -> app.neo.khulnasoft.com (app domain)
 * - OpenCode -> Neo (product name in user-visible text)
 * - opencode upgrade -> neo upgrade (CLI commands)
 * - npx opencode -> npx neo (CLI invocation)
 * - anomalyco/opencode -> Neopilot-Ai/neocode (GitHub repo)
 *
 * Preserved (not replaced):
 * - opencode.json (actual config filename)
 * - .opencode/ (actual directory name)
 * - Lines with `// neocode_change`
 */
...
const I18N_REPLACEMENTS: StringReplacement[] = [
  // GitHub repo references
  {
    ...
    replacement: "github.com/Neopilot-Ai/neocode",
    description: "GitHub URL",
  },
  {
    pattern: /anomalyco\/opencode/g,
    replacement: "Neopilot-Ai/neocode",
  ...
  // Domain replacements (specific first)
  {
    ...
    replacement: "app.neo.khulnasoft.com",
    description: "App domain",
  },
  {
    pattern: /opencode\.ai(?!\/zen)/g,
    replacement: "neo.khulnasoft.com",
...
/**
 * Check if a line should be preserved (has neocode_change marker)
 */
function shouldPreserveLine(line: string): boolean {
  return line.includes("// neocode_change")
}

/**
 * Apply string replacements to content, preserving neocode_change lines
 */
export function transformI18nContent(
  ...
  for (const line of lines) {
    // Skip lines marked with neocode_change
 ...
 * Takes upstream version (theirs) and applies Neo branding
 */
export async function transformConflictedI18n(
  ...
  for (const file of files) {
    ...
    if (!options.dryRun && (await oursHasNeocodeChanges(file))) {
    ...
    // Then apply Neo branding transformations
    ...
    if (options.dryRun) {
      ...
      success(`Transformed ${file}: took upstream + ${result.replacements} Neo branding replacements`)
    }
  }
