#!/usr/bin/env bun
/**
 * Transform files by taking upstream version and applying Neo branding
 *
 * This script handles files that have only branding differences (no logic changes).
 * It takes the upstream version and applies Neo branding transforms.
 *
 * Use this for:
 * - UI components with OpenCode -> Neo branding
 * - Config files with predictable patterns
 * - Files without neocode_change logic blocks
 */
...
export function applyBrandingTransforms(content: string, verbose = false): { result: string; replacements: number } {
  ...
  for (const line of lines) {
    // Skip lines with neocode_change marker (already customized)
    if (line.includes("// neocode_change")) {
