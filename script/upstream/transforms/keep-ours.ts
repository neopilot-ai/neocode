#!/usr/bin/env bun
/**
 * Keep Neo's version of specific files during merge
 *
 * This script handles files that should always keep Neo's version
...
export function shouldKeepOurs(filePath: string, patterns: string[]): boolean {
  const dirs = defaultConfig.neoDirectories.map((dir) => `${dir}/**`)
  return matches(filePath, [...patterns, ...dirs]) || patterns.some((pattern) => filePath.includes(pattern))
}

/**
 * Keep Neo's version of conflicted files
 */
...
/**
 * Reset specific files to Neo's version (even if not conflicted)
 */
export async function resetToOurs(files: string[], options: KeepOursOptions = {}): Promise<KeepOursResult[]> {
  ...
  for (const file of files) {
    if (options.dryRun) {
      ...
      try {
        // Get the file from HEAD (Neo's version)
...
if (import.meta.main) {
  ...
  console.log()
  success(`Kept Neo's version for ${kept.length} files`)
