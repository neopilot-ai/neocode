...
test("fixScripts preserves Neo-only root scripts from base", () => {
  const ours = {
    scripts: {
      "dev-setup": "neo dev-setup",
      postinstall: "bun run --cwd packages/opencode fix-node-pty && bun run script/setup-git.ts",
      extension: "bun --cwd packages/neo-vscode script/launch.ts",
    },
  }
...
test("fixMetadata preserves opencode publish metadata from base", () => {
  const ours = { keywords: ["cli", "neo", "opencode"], private: false }
...
test("mergeWithNewestVersions preserves ours' key order so neo-only deps don't relocate", () => {
  // Regression: when ours has a neo-only dep in the middle (e.g. rotating-file-stream
...
test("selectBunPackageManager keeps the newer Bun version and prefers Neo on ties", () => {
  ...
  expect(selectBunPackageManager("bun@1.3.14+neo", "bun@1.3.14+upstream")).toBe("bun@1.3.14+neo")
})
...
test("fixPackageManager prevents root Bun downgrades", () => {
  ...
  expect(changes).toEqual(["packageManager: bun@1.3.13 -> bun@1.3.14 (preserved Neo pin)"])
})

test("fixPackageManager restores a valid Neo pin over malformed upstream", () => {
  ...
  expect(pkg.packageManager).toBe("bun@1.3.14")
  expect(changes).toEqual(["packageManager: bun@latest -> bun@1.3.14 (preserved Neo pin)"])
})
