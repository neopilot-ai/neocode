#!/usr/bin/env bun
/**
 * Rebuild neocode_change markers for one file by comparing it with the last
 * merged upstream version.
 *
 * Usage:
 *   bun run script/upstream/fix-neocode-markers.ts packages/opencode/src/file.ts
 *   bun run script/upstream/fix-neocode-markers.ts packages/opencode/src/file.ts --dry-run
 */
...
function usage() {
  console.log(`Usage: bun run script/upstream/fix-neocode-markers.ts <repo-relative-file> [--dry-run]

Rebuilds neocode_change markers by:
  ...
  4. Removing existing neocode_change markers and adding fresh markers around remaining changed lines.
...
async function main() {
  ...
  header("Fix neocode_change markers")
  ...
  if (base === null && annotates(file)) warn(`${file} does not exist upstream; marked as a new Neo file`)
  ...
  if (next === current) {
    success(`${file} already has normalized neocode_change markers`)
    return
  }
