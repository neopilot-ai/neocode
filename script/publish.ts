...
// neocode_change start - keep JetBrains CLI pin reviewable outside CLI release commits
const jetbrainsPkg = fileURLToPath(new URL("../packages/neo-jetbrains/package.json", import.meta.url))
const jetbrainsPin = await Bun.file(jetbrainsPkg).text()
// neocode_change end

// neocode_change start - consume changesets on the publish runner so changelog
...
{
  await $`bun install`
  const paths = ["packages/neo-vscode/CHANGELOG.md", "packages/opencode/CHANGELOG.md"]
...
for (const file of pkgjsons) {
  // neocode_change start - create a follow-up PR for JetBrains CLI pin bumps
...
if (Script.release) {
  // neocode_change start - commit, tag, and push with rebase + retry to handle
  ...
  // neocode_change start - publish channel-aware GitHub release notes
  const { publishNotes } = await import("./neocode/release-notes")
  await publishNotes({
...
// neocode_change - Neo does not ship the upstream preview CLI package
...
console.log("\n=== vscode ===\n")
await import(`../packages/neo-vscode/script/publish.ts`)
// neocode_change end

// neocode_change start - Neo does not ship the opencode desktop app
// if (Script.release) {
...
// neocode_change start - non-blocking JetBrains CLI pin bump PR after stable CLI release
...
async function createJetbrainsPinPr() {
  ...
  const result = await $`bun .neo/skills/release-jetbrains/script/set-pin.ts --version ${Script.version} --pr`.nothrow()
