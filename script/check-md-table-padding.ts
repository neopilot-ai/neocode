 ...
 *
 * Enforcement scope (Neo-owned paths only, to avoid upstream-sync churn):
 *   - Any top-level markdown file (TESTING.md, AGENTS.md, README.md, …)
 *   - Any path segment containing "neocode" or starting with "neo-"
...
function skip(file: string) {
  ...
  if (parts.some((p) => p.includes("neocode") || p.startsWith("neo-"))) return false
