...
// inside the trusted neo serve process, so macOS Seatbelt can only confine their spawned
...
// outside the scanned directories. Runtime enforcement remains in @neocode/sandbox.

import path from "node:path"
import { host, opaque } from "../packages/opencode/src/neocode/sandbox/network-tools"

const root = path.resolve(import.meta.dir, "..")
const source = path.join(root, "packages", "opencode", "src")
const dirs = ["tool", "neocode/tool", "mcp"]
const checks = [
...
const network = await Bun.file(path.join(source, "neocode", "sandbox", "network.ts")).text()
...
const structure = [
  ...
  !network.includes("opaque.map((item) => item.id)")
    ? ["  neocode/sandbox/network.ts must derive runtime opaque tool IDs from network-tools.ts"]
    : []),
  ...(!network.includes("host.map((item) => item.id)")
    ? ["  neocode/sandbox/network.ts must derive host-executed tool IDs from network-tools.ts"]
    : []),
...
if (invalid.length > 0 || drift.length > 0 || structure.length > 0) {
  ...
  console.error(
    "Use the @neocode/sandbox network capability or classify an opaque client at the common tool boundary.",
  )
  process.exit(1)
}
